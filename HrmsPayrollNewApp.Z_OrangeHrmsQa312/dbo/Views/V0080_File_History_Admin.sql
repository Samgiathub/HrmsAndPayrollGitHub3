...
CREATE View [dbo].[V0080_File_History_Admin]
As

	select distinct fh.FH_Id,fh.File_App_Id,fh.Emp_Id,fh.File_Apr_Id,em.Alpha_Emp_Code,em.Emp_Full_Name,
	format(isnull(qry.Application_Date,fa.Approve_Date),'dd/MM/yyyy') as Application_Date,EMSUP.Emp_Full_Name as Reporting_Manager,
	...
	,fh.H_File_App_Doc
	 ...
	 ,lgapp.Login_Name
	 ,(case when  lgapp.Login_Name like '%admin%' then 'Admin' else eapp.Emp_Full_Name end ) as LastUdatedby
	  ,CONVERT(VARCHAR(10), CAST(FH.CreatedDate AS TIME), 0)as updated_time
	  ,isnull(fa.File_Type_Number,'')as File_Type_Number--added on 20-08-22

			from T0115_File_Level_Approval_History FH
				inner join T0080_File_Approval fa on fa.File_App_Id=FH.File_App_Id
			----inner join T0080_File_Application fa on fa.File_App_Id=FH.File_App_Id
			INNER JOIN( SELECT Application_Date, File_App_Id
			FROM dbo.T0080_File_Application WITH (NOLOCK)  GROUP BY File_App_Id,Application_Date
						) AS Qry ON Qry.File_App_Id = fa.File_App_Id 
  ...
  --where fh.Tbl_Type<>'T0115_File_Level_Approval'
