...
Create VIEW [dbo].[V0115_TRAVEL_APPROVAL_OTHER_DETAIL_LEVEL_Backup_09052023]
AS
Select
distinct
--ROW_NUMBER() OVER (ORDER BY Travel_Apr_Other_Detail_Id) AS Sr_No,
		...
		TAO.Travel_Mode_Id,For_date,TAO.Description,Amount,Case when Self_Pay = 1 then 'Yes' else 'No' end as Self_Pay,
		TAO.modify_Date,Travel_Mode_Name,right(convert(varchar,For_date),7) as From_Time,
		right(convert(varchar,To_date),7) as To_Time,To_date,isnull(TAO.Curr_ID,0) as Curr_ID,
	    isnull(Cm.Curr_Symbol,'') as Currency,TLA.Travel_Application_ID,TLA.Rpt_Level
	    ,TM.GST_Applicable,TAO.SGST,TAO.CGST,TAO.IGST,TAO.GST_No,TAO.GST_Company_Name,TAOD.TRAVEL_MODE,TLA.Travel_Application_ID AS Travel_App_ID
From T0115_TRAVEL_APPROVAL_OTHER_DETAIL_LEVEL TAO WITH (NOLOCK)
		...
		Inner Join T0030_TRAVEL_MODE_MASTER TM WITH (NOLOCK) ON TAO.Travel_Mode_ID  = TM.Travel_Mode_ID 
