...
CREATE VIEW [dbo].[V0140_Travel_Settlement_Application_New_Level_No_Travel_Type]      
AS      
SELECT     distinct   TSA.Travel_Set_Application_id, TSA.Travel_Approval_ID,       
                      ...
                      TLA.pending_amount as Pending_Amount,      
                       TLA.Manager_comment as Comment
					  , TSA.[Document],TSA.For_date,       
                      ...
                      ,c.GST_No,TAD.TravelTypeId  ,case when isnull(rm.Reason_Name  ,'') ='' then 0 end as Reason_Name  
FROM         dbo.T0140_Travel_Settlement_Application TSA WITH (NOLOCK) INNER JOIN      
 ...
 left join T0040_Travel_Type TT With (Nolock) on TT.Travel_type_Id = TAD.TravelTypeId      
 ...
 where TAD.TravelTypeId=0 or TAD.TravelTypeId=Null        
...
FROM         dbo.T0140_Travel_Settlement_Application TSA WITH (NOLOCK) INNER JOIN      
 ...
 left join T0040_Travel_Type TT With (Nolock) on TT.Travel_type_Id = TSA.TravelTypeId      
 INNER JOIN T0010_COMPANY_MASTER c WITH (NOLOCK)  ON c.Cmp_Id = TSA.Cmp_ID      
