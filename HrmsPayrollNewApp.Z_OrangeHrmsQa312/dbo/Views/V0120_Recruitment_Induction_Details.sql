...
CREATE VIEW [dbo].[V0120_Recruitment_Induction_Details]
AS
SELECT   Schedule_Date, Induction_ID,dbo.T0120_HRMS_Induction_Details.Cmp_ID,Emp_ID,Contact_Person_ID,
 ...
 dbo.T0040_DEPARTMENT_MASTER.Dept_Name,T0040_DEPARTMENT_MASTER.Dept_Id,
...
(SELECT     (E.Alpha_Emp_Code + '-' + E.Emp_Full_Name) + ','
                            ...
                            WHERE      E.Emp_ID IN
                                                       (SELECT     cast(data AS numeric(18, 0))
                                                         FROM          dbo.Split(ISNULL(dbo.T0120_HRMS_Induction_Details.Contact_Person_ID, '0'), '#')
                                                         WHERE      data <> '') FOR XML path('')) AS Contact_Person_Name
