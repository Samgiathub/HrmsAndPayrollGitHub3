...
CREATE VIEW [dbo].[VIEW_File_FINAL_N_LEVEL_APPROVAL]
AS
SELECT   distinct GPA.Cmp_ID, GPA.File_App_Id, GPA.Emp_ID,gpa.Application_Date
  --format(gpa.Application_Date,'dd/MM/yyyy') as Application_Date
   ,ISNULL(qry.File_Apr_Id,0) AS FA_Id 
      ,(case when (qry.File_Apr_Id<>0 and qry.APR_Status not in(3,5)) then 1 else 0 end) as Final_Approver --added on 17-05-22
   ,GPA.Alpha_Emp_Code, GPA.Emp_Full_Name,GPA.Emp_First_Name,
   gpa.File_Number,
		  ...
		  qry.S_emp_ID AS S_Emp_ID_A,qry.S_emp_ID,qry.APR_Status as F_StatusId,
		   ...
		   ,qry.Tran_Id
			...
			--,isnull(right(qry.File_App_Doc, len(qry.File_App_Doc) - 17),'')as File_App_Doc--added
			 --added 1 july start
			,qry.Forward_Employee,qry.Forward_by_Employee,qry.Review_Employee,qry.Review_by_Employee
			 --added 1 july end
			 ,isnull(gpa.File_Type_Number,'')as File_Type_Number--added by mansi 23-08-22
