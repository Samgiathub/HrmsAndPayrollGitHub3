...
CREATE View [dbo].[V0080_File_History_Admin_approve]
As

	select distinct fh.FH_Id,fh.File_App_Id,fh.Emp_Id,fh.File_Apr_Id,em.Alpha_Emp_Code,em.Emp_Full_Name,
	(case when fh.File_App_Id=0 then format(fa.Approve_Date,'dd/MM/yyyy') else format(qry.Application_Date,'dd/MM/yyyy') end) as Application_Date,EMSUP.Emp_Full_Name as Reporting_Manager,
   ...
   --        qry.Rpt_Level AS Rpt_Level,qry.Approval_Comments AS Approval_Comments
		 ...
		 -- 	,qry.Scheme_ID--added
			...
			left join T0080_File_Approval fa on fa.File_Apr_Id=FH.File_Apr_Id
			--inner join T0080_File_Application fa on fa.File_App_Id=FH.File_App_Id
			left JOIN
						( SELECT  File_App_Id,Application_Date FROM dbo.T0080_File_Application WITH (NOLOCK)  
						 GROUP BY File_App_Id,Application_Date
						) AS Qry ON  Qry.File_App_Id = fh.File_App_Id 
