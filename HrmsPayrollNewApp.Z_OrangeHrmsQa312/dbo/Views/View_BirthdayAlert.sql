...
CREATE VIEW [dbo].[View_BirthdayAlert]
...
(CASE WHEN CONVERT(varchar, Actual_Date_Of_Birth,103) IS NOT NULL THEN CONVERT(varchar, Actual_Date_Of_Birth,103) ELSE CONVERT(varchar, Date_Of_Birth,103) END) AS 'DATE', -- added by Niraj (08082022) Sajid's Bday
...
WHERE MONTH(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) = MONTH (GETDATE()) AND DAY(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) = DAY(GETDATE()) -- added by Niraj (08082022) Sajid's Bday
...
(CASE WHEN ISNULL(EM.Emp_Annivarsary_Date,'') <> '' THEN CONVERT(varchar, EM.Emp_Annivarsary_Date,103) ELSE EM.Emp_Annivarsary_Date END) AS 'Date',
(CASE WHEN EM.Image_Name = '0.jpg' OR EM.Image_Name = ''  THEN (CASE WHEN EM.Gender = 'M' THEN 'Emp_Default.png' ELSE 'Emp_Default_Female.png' END) ELSE EM.Image_Name END) AS 'Image_Name' , 
...
EM.Emp_ID,(EM.Alpha_Emp_Code + ' - ' + EM.Initial + ' '+ ISNULL(EM.Emp_First_Name ,'')+ ' ' + ISNULL(EM.Emp_Second_Name,'')+  ' '+ISNULL(EM.Emp_Last_Name,'')) AS 'EmpName',
DP.Dept_Name,IC.Desig_Id,DM.Desig_Name,IC.Branch_ID,BM.Branch_Name,BM.Branch_Code,
CONVERT(varchar, EM.Date_Of_Join,103) AS 'Date',
...
SELECT (SELECT COUNT(*) FROM T0400_Employee_Like EL WITH (NOLOCK)  WHERE EM.Emp_ID = EL.Emp_Id AND EL.Like_Flag = 1  AND EL.Notification_Flag = 4) AS 'LikeCount',
(SELECT COUNT(*) FROM T0400_Employee_Comment EC WITH (NOLOCK)  WHERE EM.Emp_ID = EC.Emp_Id  AND EC.Notification_Flag = 4) AS 'CommentCount',
--COUNT(EL.Tran_Id) AS 'Like',COUNT(EC.Comment_Id) AS 'Comment',  
EM.Emp_ID,(EM.Alpha_Emp_Code + ' - ' + EM.Initial + ' '+ ISNULL(EM.Emp_First_Name ,'')+ ' ' + ISNULL(EM.Emp_Second_Name,'')+  ' '+ISNULL(EM.Emp_Last_Name,'')) AS 'EmpName',
DP.Dept_Name,IC.Desig_Id,DM.Desig_Name,IC.Branch_ID,BM.Branch_Name,BM.Branch_Code,
--CONVERT(varchar(11), Date_Of_Birth,103) AS 'Date',
...
WHERE (DAY(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) > DAY(GETDATE()) AND MONTH(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) = MONTH(GETDATE())) -- added by Niraj (08082022) Sajid's Bday
--AND (DAY(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) < DAY(GETDATE()+8)  AND MONTH(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) = MONTH(GETDATE())) -- added by Niraj (08082022) Sajid's Bday
AND (CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) < (GETDATE()+8)  --AND MONTH(CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) = MONTH(GETDATE())) -- added by Niraj (08082022) Sajid's Bday
AND (CASE WHEN Actual_Date_Of_Birth IS NOT NULL THEN Actual_Date_Of_Birth ELSE Date_Of_Birth END) <> '' AND (EM.Emp_Left = 'N' OR (EM.Emp_Left = 'Y' AND EM.Emp_Left_Date > GETDATE()))
--AND EL.Like_Flag = 1
...
--SELECT (SELECT COUNT(*) FROM T0400_Employee_Like EL WITH (NOLOCK)  WHERE EM.Emp_ID = EL.Emp_Id AND EL.Like_Flag = 1) AS 'LikeCount',
--(SELECT COUNT(*) FROM T0400_Employee_Comment EC WITH (NOLOCK)  WHERE EM.Emp_ID = EC.Emp_Id ) AS 'CommentCount',
SELECT (SELECT COUNT(*) FROM T0400_Employee_Like EL WITH (NOLOCK)  WHERE EM.Emp_ID = EL.Emp_Id AND EL.Like_Flag = 1  AND EL.Notification_Flag = 6) AS 'LikeCount',
(SELECT COUNT(*) FROM T0400_Employee_Comment EC WITH (NOLOCK)  WHERE EM.Emp_ID = EC.Emp_Id AND EC.Notification_Flag = 6 ) AS 'CommentCount',
--COUNT(EL.Tran_Id) AS 'Like',COUNT(EC.Comment_Id) AS 'Comment',  
EM.Emp_ID,(EM.Alpha_Emp_Code + ' - ' + EM.Initial + ' '+ ISNULL(EM.Emp_First_Name ,'')+ ' '+ ISNULL(EM.Emp_Second_Name,'')+  ' '+ISNULL(EM.Emp_Last_Name,'')) AS 'EmpName',
DP.Dept_Name,IC.Desig_Id,DM.Desig_Name,IC.Branch_ID,BM.Branch_Name,BM.Branch_Code,
--CONVERT(varchar(11), CONVERT(datetime,EM.Emp_Annivarsary_Date,103),103) as 'Date',
--(CASE WHEN ISNULL(EM.Emp_Annivarsary_Date,'') <> '' THEN CONVERT(varchar(11), CONVERT(datetime, EM.Emp_Annivarsary_Date,103),103) ELSE EM.Emp_Annivarsary_Date END) AS 'Date',
(CASE WHEN ISNULL(CONVERT(varchar, EM.Emp_Annivarsary_Date,103),'') <> '' THEN CONVERT(varchar, EM.Emp_Annivarsary_Date,103) ELSE  CONVERT(varchar, EM.Emp_Annivarsary_Date,103) END) AS 'Date',
(CASE WHEN EM.Image_Name = '0.jpg' OR EM.Image_Name = ''  THEN (CASE WHEN EM.Gender = 'M' THEN 'Emp_Default.png' ELSE 'Emp_Default_Female.png' END) ELSE EM.Image_Name END) AS 'Image_Name' , 
...
AND (Emp_Annivarsary_Date) < (GETDATE()+8)  --AND MONTH(Emp_Annivarsary_Date) = MONTH(GETDATE())) -- +8 added by Niraj(08102021)
...
SELECT (SELECT COUNT(*) FROM T0400_Employee_Like EL WITH (NOLOCK)  WHERE EM.Emp_ID = EL.Emp_Id AND EL.Like_Flag = 1 AND EL.Notification_Flag = 5) AS 'LikeCount',
(SELECT COUNT(*) FROM T0400_Employee_Comment EC WITH (NOLOCK)  WHERE EM.Emp_ID = EC.Emp_Id AND EC.Notification_Flag = 5) AS 'CommentCount',
--COUNT(EL.Tran_Id) AS 'Like',COUNT(EC.Comment_Id) AS 'Comment',  
EM.Emp_ID,(EM.Alpha_Emp_Code + ' - ' + EM.Initial + ' '+ ISNULL(EM.Emp_First_Name ,'')+' '+ ISNULL(EM.Emp_Second_Name,'')+  ' '+ISNULL(EM.Emp_Last_Name,'')) AS 'EmpName',
DP.Dept_Name,IC.Desig_Id,DM.Desig_Name,IC.Branch_ID,BM.Branch_Name,BM.Branch_Code,
CONVERT(varchar, EM.Date_Of_Join,103) as 'Date',
...
AND (Date_Of_Join) < (GETDATE()+8)  --AND MONTH(Date_Of_Join) = MONTH(GETDATE())) -- +8 added by Niraj(08102021)
