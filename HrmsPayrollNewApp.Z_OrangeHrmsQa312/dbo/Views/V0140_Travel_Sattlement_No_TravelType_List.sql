...
CREATE VIEW [dbo].[V0140_Travel_Sattlement_No_TravelType_List]              
AS              
--SELECT                   
...
SELECT ISNULL(TA.TRAVEL_APPROVAL_ID,0) AS TRAVEL_APPROVAL_ID,ISNULL(TRSA.APPROVAL_DATE,TSA.FOR_DATE) AS APPROVAL_DATE,TSA.COMMENT AS APPROVAL_COMMENTS,ISNULL(TSa.STATUS,'Draft') AS APPROVAL_STATUS,              
...
FROM    T0140_TRAVEL_SETTLEMENT_APPLICATION AS TSA  WITH (NOLOCK) LEFT JOIN            
 ...
 -- left join T0040_Reason_Master RM WITH (NOLOCK) ON RM.Res_Id=TAD.Reason_ID       
  ...
  WHERE TSA.STATUS='D' and TAD.TravelTypeId=0 or TAD.TravelTypeId=Null            
