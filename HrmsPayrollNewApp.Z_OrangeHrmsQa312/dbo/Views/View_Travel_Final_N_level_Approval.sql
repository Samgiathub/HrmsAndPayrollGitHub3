...
CREATE VIEW [dbo].[View_Travel_Final_N_level_Approval]  
AS  
SELECT  Distinct LAD.Emp_ID, LAD.Emp_Full_Name, LAD.Supervisor,LAD.Travel_Application_ID, LAD.Application_Code ,LAD.Branch_Name  
  ...
  ,LAD.Travel_Set_Application_id,LAD.travel_approval_id,LAD.Emp_First_Name,LAD.Branch_ID  
        ,qry.S_emp_id AS S_Emp_ID_A,dbo.F_GET_Emp_Visit(LAD.Cmp_ID,LAD.Travel_Application_ID,0) as Emp_Visit  
FROM         V0100_TRAVEL_APPLICATION_Without_TravelType LAD WITH (NOLOCK) INNER JOIN  
                          (SELECT     Tla.Travel_Application_ID, Tla.s_emp_id,Tla.Approval_Status  
                            FROM          T0115_TRAVEL_LEVEL_APPROVAL Tla WITH (NOLOCK)  INNER JOIN  
                                                   ...
                                                   V0100_TRAVEL_APPLICATION_Without_TravelType LA WITH (NOLOCK)  ON la.Travel_Application_ID = Tla.Travel_Application_ID  
