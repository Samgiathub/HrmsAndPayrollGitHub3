CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_Leave]
...
ELSE IF @Type = 'V' --- For Leave Validation
	BEGIN
		...
		---added by aswini 28/03/2024
		DECLARE @Leave_Opening NVARCHAR(MAX);
DECLARE @Leave_Used NVARCHAR(MAX);
declare @Leave_Credit nvarchar(max);
declare @Leave_Closing1 nvarchar(max);
declare @Display_LeaveBalance nvarchar(max);
declare @Leave_id1 nvarchar(max);
declare @Actual_Leave_Closing1 nvarchar(max);
declare @Leave_Type1 nvarchar(max);
declare @Leave_name nvarchar(max);
SELECT @Leave_Opening = Leave_Opening,
       @Leave_Used = Leave_Used,@Leave_Credit=Leave_Credit,@Leave_Closing1=Leave_Closing,@Display_LeaveBalance=Display_LeaveBalance,@Leave_Id1=Leave_ID,@Actual_Leave_Closing1=Actual_Leave_Closing
,@Leave_Type1 = Leave_Type,@Leave_name=Leave_name
FROM #LeaveBalanceData;
---end by aswini 28/03/2024
		...
		IF (@Period > @Leave_Closing) and  (@leave_name != 'On Duty')  --added aswini 28/03/2024
			BEGIN 
...
-------------------------------------------------End By Prapti 24/08/2022--------------------------------------------------------------------------
		...
		IF @Leave_Shutdown_Status = 1
			...
			EXEC Check_Leave_Clubbing @Emp_Id = @Emp_ID,@Cmp_Id = @Cmp_Id,@From_DateFE = @From_DateFE,@To_DateFE = @To_DateFE,@From_DateLE = @From_DateLE,@To_DateLE = @To_DateLE,@Tag = 'LP',@Leave_Id = @Leave_ID,@Leave_App_Id = 0,@Leave_Period = @Period,@Leave_Day = @Leave_Assign_As,@Leave_Half_Date = @Half_Leave_Date
...
ELSE IF @Type = 'R'  --- For Leave Balance 
	BEGIN
		...
		LB.P_To_Date,LB.Branch_Id,LB.Type_Name,LB.Desig_Dis_No,LB.Vertical_Name,LB.SubVertical_Name,LB.SubBranch_Name,
