...
Create PROCEDURE [dbo].[SP_RPT_EMP_CANTEEN_MANAGMENT_DiaminesLAtest] 
...
ALTER TABLE  #EMP_CONS ADD ENROLL_NO VARCHAR(50);
Alter Table  #EMP_CONS ADD grade Numeric(18,0);

--select * into TmpEmpcons from #EMP_CONS

UPDATE	#EMP_CONS 
SET		--ENROLL_NO=E.ENROLL_NO,
		ENROLL_NO=CASE WHEN  isnull(e.old_ref_no,'') = '' THEN cast(e.Enroll_No as varchar(50))  ELSE e.Old_Ref_No END, 
		GRADE=I.GRD_ID
...
IF EXISTS( SELECT 1 FROM #CANTEEN_MASTER )
	BEGIN
		...
		AND (CID.In_Out_flag='10' OR CID.IP_Address='Canteen' OR IP.Device_No >= 200) 
		and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
		AND  EM.EMP_LEFT='N'  
		...
		AND (CID.flag='Mobile' and CID.Reason='Mobile' OR IP.Device_No >= 200)
		 and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
		AND  EM.EMP_LEFT='N'  
		GROUP BY EM.EMP_ID,IP.IP_ID,CC.DEPT_NAME,EM.EMP_FULL_NAME,CC.SEGMENT_NAME,CM.CNT_NAME,EM.EMP_CODE
...
INTO	#CANTEEN
FROM	DBO.T0050_CANTEEN_MASTER M INNER JOIN T0050_CANTEEN_DETAIL D ON  M.CNT_ID=D.CNT_ID	
AND M.CMP_ID=D.CMP_ID 
WHERE	D.EFFECTIVE_DATE <= @TO_DATE 
AND M.CMP_ID=@CMP_ID
--select * from #CANTEEN
...
WHERE	(I.IN_OUT_FLAG='10' OR I.IP_ADDRESS='CANTEEN' OR IP.DEVICE_NO >= 200) 
		...
		AND  cast(I.IO_DateTime as time) >= cast(t.From_Time as time) 
		and  cast(I.IO_DateTime as time) <= cast(t.To_Time as time)
		ORDER BY E.EMP_ID,I.IO_DATETIME 

		
-- MOBILE CANTEEN
--SELECT	ROW_NUMBER() OVER(ORDER BY I.TRAN_ID) AS ROWID,I.CANTEEN_PUNCH_DATETIME AS IO_DATETIME,E.EMP_ID,T.CNT_ID,T.IP_ID,T.AMOUNT AS EMPLOYEE_CONTRIBUTION,(T.AMOUNT * ISNULL(I.QUANTITY,1)) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION, change by deepal
...
FROM	DBO.T0150_EMP_CANTEEN_PUNCH I  
		...
		WHERE		I.FLAG='Mobile' AND I.REASON='Mobile' AND I.CMP_ID=@CMP_ID  
					AND cast(I.CANTEEN_PUNCH_DATETIME as date) BETWEEN @FROM_DATE AND @TO_DATE
					--and I.Emp_ID <> 0
...
INSERT INTO #CANTEEN_SUBSIDY
		(EMP_ID,IP,TOTAL,EMPLOYEE_CONTRIBUTION,AMOUNT_OF_EMPLOYEES_CONTRIBUTION,SUBSIDY_BORN_BY_COMPANY
		,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,QUANTITY,TOTAL_AMOUNT,CANTEEN_ID)
		SELECT	T.EMP_ID,T.IP_ID,0,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0),SUM(ISNULL(T.AMOUNT_OF_EMPLOYEES_CONTRIBUTION ,0)), 
		ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0),SUM(ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)),SUM(T.QUANTITY),
		...
		GROUP BY T.EMP_ID,T.IP_ID,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0),ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0),T.Cnt_Id
...
END	
	...
	GROUP BY A.EMP_ID,B.IP_ADDRESS,A.EMP_CODE,A.EMP_FULL_NAME,I.DEPT_ID,A.DEPT_NAME
	,A.SEGMENT_NAME,CM.CNT_NAME,B.IP_ID,cm.Cnt_Id
	...
	--SELECT CC.EMP_CODE,CC.EMP_NAME,CC.DEPARTMENT_NAME,CC.BUSINESS_SEGMENT,CC.DIET AS DIET,CC.IP,CC.CANTEEN_ID,ED.*  --Comment by deepal 03092020
	...
	SET @QRY = 'SELECT EMP_ID,Canteen_Id,IP,(' + SUBSTRING(@ROW_WISETOTAL_COUNT,0,LEN(@ROW_WISETOTAL_COUNT)) + ') AS TOTAL_PRESENT
	 FROM #CANTEEN_FINAL_RECORDS GROUP BY  EMP_ID,Canteen_Id,IP'
	...
	FROM	#CANTEEN_SUBSIDY T 
			INNER JOIN	(
							SELECT	B.EMP_ID,B.IP,b.Canteen_Id,B.TOTAL_PRESENT
							FROM	#TOTAL_PRESENT_ROW_WISE B
							
						) B ON T.EMP_ID=B.EMP_ID  AND T.IP=B.IP and t.CANTEEN_ID = b.Canteen_Id
	

	SET @QRY = 'SELECT EMP_ID,'''' AS IP,SUM(QUANTITY) AS TOTAL,0 AS EMPLOYEE_CONTRIBUTION,SUM(AMOUNT_OF_EMPLOYEES_CONTRIBUTION) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0 AS SUBSIDY_BORN_BY_COMPANY,
	SUM(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,SUM(QUANTITY),SUM(TOTAL_AMOUNT) AS TOTAL_AMOUNT,CANTEEN_ID FROM #CANTEEN_SUBSIDY GROUP BY  EMP_ID,CANTEEN_ID'
	INSERT INTO #CANTEEN_SUBSIDY 
	EXEC(@QRY)
	
	
	SET @QRY = 'SELECT 9999 as EMP_CODE,'''' AS EMP_NAME,DEPARTMENT_NAME,'''' AS BUSINESS_SEGMENT,'''' AS IP,0 AS CANTEEN_ID,0 as EMP_ID,''TOTAL'' AS DIET,'''' AS IP_ID,' + SUBSTRING(@COLCAPTION_NAME,0,LEN(@COLCAPTION_NAME)) + ' FROM #CANTEEN_FINAL_RECORDS GROUP BY  DEPARTMENT_NAME'
	...
	LEFT JOIN T0030_BRANCH_MASTER TBM ON TBM.BRANCH_ID = I.Branch_ID		
	...
	--select * from  #CANTEEN_REGISTER_FINAL
 ...
 --   ORDER BY EMP_CODE,DIET,IP_ID desc
	...
	--select * from #CANTEEN_REGISTER cr
	--inner JOIN T0150_EMP_CANTEEN_PUNCH ECP ON CR.CANTEEN_ID =  ECP.CANTEEN_ID and cr.EMP_ID = ecp.Emp_ID
	--Select * from #CANTEEN_REGISTER distinct
	...
	SELECT  DISTINCT (CASE WHEN CR.EMP_CODE = '99999999' THEN '99999999' ELSE ROW_NUMBER() OVER(ORDER BY (SELECT 1)) END) 
	AS SRNO
	,ROW_NUMBER() OVER (PARTITION BY CR.EMP_ID ORDER BY CR.EMP_CODE,CR.DIET,CR.IP_ID DESC) AS SR_NO_TEMP
	,CR.*,ISNULL(EM.EMP_FULL_NAME,'Device') AS Distributor_NAME 
    ...
    IF(ISNULL(@FORMAT,0) = 0)
		BEGIN
			...
			DECLARE @QUERYF1 VARCHAR(MAX)	
				SET @QUERYF1 = 'SELECT ROW_NUMBER() OVER (PARTITION BY EMP_NAME ORDER BY EMP_NAME) RowNumber
				,(EMP_CODE + '' - '' + EMP_NAME) as Emp_Full_Name,Diet,Distributor_Name
				,Total as Total_Quantity,' + SUBSTRING(@COLCAPTION_NAME1,0,LEN(@COLCAPTION_NAME1)) + '
				,ISNULL(EMPLOYEE_CONTRIBUTION,0) as EMPLOYEE_CONTRIBUTION
				,ISNULL(AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0) as AMOUNT_OF_EMPLOYEES_CONTRIBUTION
				,ISNULL(SUBSIDY_BORN_BY_COMPANY,0) as SUBSIDY_BORN_BY_COMPANY
				,ISNULL(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0) as AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
				...
				,' + SUBSTRING(@COLCAPTION_NAME1,0,LEN(@COLCAPTION_NAME1)) + '
				,EMPLOYEE_CONTRIBUTION
				,AMOUNT_OF_EMPLOYEES_CONTRIBUTION
				,SUBSIDY_BORN_BY_COMPANY
				,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
				,TOTAL_AMOUNT,Canteen_id
				'
				EXEC(@QUERYF1)
			

			Select  Diet,canteen_id,Distributor_Name,
			sum(isnull(Total_Quantity,0)) as QUANTITY,
			...
			group by Diet,canteen_id,Distributor_Name
			
			
			--SELECT	--ROW_NUMBER() OVER(ORDER BY DIET) AS SRNO
			--ROW_NUMBER() OVER (PARTITION BY EMP_NAME ORDER BY EMP_NAME) AS SRNO
			--		,EMP_NAME
			--		,count(ISNULL(TOTAL,0)) AS QUANTITY
			--		,SUM(ISNULL(EMPLOYEE_CONTRIBUTION,0)) AS EMPLOYEE_CONTRIBUTION,
			--		SUM(ISNULL(AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0)) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION,
			--		SUM(ISNULL(SUBSIDY_BORN_BY_COMPANY,0)) AS SUBSIDY_BORN_BY_COMPANY,
			--		SUM(ISNULL(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
			...
			--FROM	#CANTEEN_REGISTER_FINAL
			--WHERE	DIET not in ('TOTAL','GRAND TOTAL') 
			--GROUP BY EMP_NAME--,CANTEEN_ID
			--ORDER BY EMP_NAME--SRNO	
			
			--Select  
			--sum(isnull(QUANTITY,0)),
			--sum(isnull(EMPLOYEE_CONTRIBUTION,0)) ,
			--Sum(isnull(AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0)),
			--Sum(isnull(SUBSIDY_BORN_BY_COMPANY,0)),
			--Sum(isnull(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)),
			--Sum(isnull(TOTAL_AMOUNT,0))
			--from #CANTEEN_SUMMARY 
			
			
			--SELECT	ROW_NUMBER() OVER(ORDER BY DIET) AS SRNO,DIET,sUM(ISNULL(Total_Quantity,0)) AS QUANTITY
			--		,SUM(ISNULL(EMPLOYEE_CONTRIBUTION,0)) AS EMPLOYEE_CONTRIBUTION,
			--		SUM(ISNULL(AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0)) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION,
			--		SUM(ISNULL(SUBSIDY_BORN_BY_COMPANY,0)) AS SUBSIDY_BORN_BY_COMPANY,
			--		SUM(ISNULL(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
			...
			--		CANTEEN_ID
			--INTO	#CANTEEN_SUMMARY
			--FROM	##CANTEEN_DETAIL_TABLE
			--WHERE	DIET not in ('TOTAL','GRAND TOTAL') 
			--GROUP BY DIET,CANTEEN_ID
			--ORDER BY DIET--SRNO	
			--DROP TABLE ##CANTEEN_DETAIL_TABLE
			
			--SELECT ROW_NUMBER() OVER(ORDER BY DIET) AS SRNO, DIET,TOTAL as QUANTITY,
			--	ISNULL(EMPLOYEE_CONTRIBUTION,0) as EMPLOYEE_CONTRIBUTION,
			--	ISNULL(AMOUNT_OF_EMPLOYEES_CONTRIBUTION ,0)as AMOUNT_OF_EMPLOYEES_CONTRIBUTION,
			--	ISNULL(SUBSIDY_BORN_BY_COMPANY,0) AS SUBSIDY_BORN_BY_COMPANY,
			--	ISNULL(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
			...
			--	GROUP BY DIET,TOTAL,EMPLOYEE_CONTRIBUTION,
			--	AMOUNT_OF_EMPLOYEES_CONTRIBUTION 
			--	,SUBSIDY_BORN_BY_COMPANY
			--	,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,
			--	TOTAL_AMOUNT,CANTEEN_ID
			...
			SELECT	DIET, QUANTITY,EMPLOYEE_CONTRIBUTION,AMOUNT_OF_EMPLOYEES_CONTRIBUTION,SUBSIDY_BORN_BY_COMPANY,
			...
			SELECT Sum(QUANTITY) as QUANTITY--,Distributor_Name
			FROM #CANTEEN_SUMMARY --#CANTEEN_REGISTER_FINAL
			WHERE DIET NOT IN ('TOTAL','GRAND TOTAL') 
			--GROUP BY Distributor_Name
			
			drop table ##CANTEEN_SUMMARY
			
		END
	ELSE
		BEGIN

				--drop table #DEPARTMENT_WISE_SUM
				...
				SET @QUERYF = 'SELECT ROW_NUMBER() OVER (PARTITION BY EMP_NAME ORDER BY EMP_NAME) RowNumber
				,(EMP_CODE + '' - '' + EMP_NAME) as Emp_Full_Name,Diet,Distributor_Name
				,Total as Total_Quantity,' + SUBSTRING(@COLCAPTION_NAME1,0,LEN(@COLCAPTION_NAME1)) + '
				INTO ##CANTEEN_DETAIL_TABLE
				FROM #CANTEEN_REGISTER_FINAL
				WHERE DIET NOT IN  (''GRAND TOTAL'',''TOTAL'')
				GROUP BY EMP_CODE,EMP_NAME,DIET,TOTAL,Distributor_Name,' + SUBSTRING(@COLCAPTION_NAME1,0,LEN(@COLCAPTION_NAME1)) + ''
				EXEC(@QUERYF)
