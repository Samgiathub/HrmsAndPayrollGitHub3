...
-- exec SP_RPT_TRAVEL_Expense 1,'01, Jan,2010','25,Sep 2012',0,0,0,0,0,0,0,''
-- exec SP_RPT_TRAVEL_Expense 2,'01, Jan,2010','25,Sep 2013',0,0,0,0,0,0,0,''
---20/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
Create PROCEDURE [dbo].[SP_RPT_TRAVEL_Advance_22112023]
	 @Cmp_ID		Numeric
	,@From_Date		Datetime
	,@To_Date		Datetime
	,@Branch_ID		Numeric 
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
		...
		,TAA.Travel_Approval_ID as TAA_Approval_Id,
		TAA.Amount as TAA_Amount,
		TAA.Adv_Detail_Desc,TAA.Expence_Type as TAA_Expense_Type
		,CRM.Curr_Symbol,
		 case when CRM.Curr_Major='Y' then Amount
			else isnull(Amount_dollar,Amount) End as AmountRs,
		 case when CRM.Curr_Major='Y' then Amount
			else ISNULL(Amount_dollar,0) + Amount end as Amount_DollarRs
         ...
         --inner Join T0150_Travel_Settlement_Approval_Expense as TSAE on TAD.Travel_Set_Application_id =TSAE.Travel_Settlement_Id and tad.emp_id = TSAE.Emp_ID 
         inner join T0130_TRAVEL_APPROVAL_ADVDETAIL as TAA WITH (NOLOCK) on TSA.Travel_Approval_ID =TAA.Travel_Approval_ID
