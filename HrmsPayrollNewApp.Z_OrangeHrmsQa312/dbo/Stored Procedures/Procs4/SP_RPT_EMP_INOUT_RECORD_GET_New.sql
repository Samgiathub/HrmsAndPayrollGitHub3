...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET_New]      
 @Cmp_ID   numeric,      
 @From_Date  datetime,      
 @To_Date  datetime ,      
 @Branch_ID  numeric   ,      
...
--Ankit 12112013 
  ...
  BEGIN      
   ...
   Set @Leave_Assign_As =''
      ...
      --select @Insert_Out_Date = Out_Time  From T0150_emp_inout_record Where Emp_ID = @Emp_ID and In_Time = @Insert_In_Date      
			...
			Select @First_In_Last_Out_For_InOut_Calculation= isnull(First_In_Last_Out_For_InOut_Calculation,0) from dbo.T0040_GENERAL_SETTING WITH (NOLOCK) where Cmp_ID = @Cmp_ID and Branch_ID = (Select Branch_ID from dbo.T0080_EMP_MASTER WITH (NOLOCK) where Emp_ID=@Emp_ID)
 ...
 begin      
  select E_IO.*,Emp_full_Name,Alpha_Emp_Code, Emp_Code,Grd_Name,Shift_name,dept_name ,Type_Name,Desig_Name,CMP_NAME,CMP_ADDRESS,      
   ...
   ,@leave_Footer as Leave_Footer,Is_OT_Applicable,Monthly_Deficit_Adjust_OT_Hrs,Branch_Name
