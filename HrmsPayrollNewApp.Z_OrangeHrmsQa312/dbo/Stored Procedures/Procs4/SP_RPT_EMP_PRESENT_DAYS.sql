...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_PRESENT_DAYS]
	 @Cmp_ID 			numeric
	,@From_Date			datetime
	,@To_Date 			datetime 
	,@Branch_ID			numeric
...
if @Return_Record_set = 1 or @Return_Record_set = 2 or @Return_Record_set =3   or  @Return_Record_set =4
	Begin
		CREATE table #Data 
		 ( 
			Emp_Id 		numeric , 
	...
	/*Commented by Nimesh 28-May-2015
	Update #Data
			set Shift_ID   = Q1.Shift_ID,  
			    Shift_Type = q1.Shift_type
			from #Data d inner Join
	...
	*/
	...
	if @Return_Record_set =3 
		begin
				...
				--select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name, CONVERT(decimal(10,2), sum(Duration_in_Sec)/3600) 	 as Working_Hour ,CONVERT(decimal(10,2), sum(OT_SEc)/3600) as OT_Hour ,sum(P_days) as Present_Days
					
				--From #Data  OA inner join T0080_emp_master E on OA.Emp_ID = E.Emp_ID
				--Group by OA.emp_ID,E.Emp_Full_Name


				--Modified by Nimesh 02-Jun-2015 [CONVERT(DECIMAL(10,2), SUM(Duration_in_Sec)) to CAST(SUM(Duration_in_Sec) AS NUMERIC(18,2))]
				select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name, dbo.F_Return_Hours(CAST(SUM(Duration_in_Sec) AS NUMERIC(18,2))) As Working_Hour ,dbo.F_Return_Hours(CAST(SUM(OT_SEc) AS NUMERIC(18,2))) as OT_Hour ,SUM(P_days) as Present_Days,E.Emp_Code,SM.Shift_Name,CM.Cmp_Name,CM.Cmp_Address,Branch_Address,Dept_Name,Comp_Name,Desig_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender
