...
CREATE PROCEDURE [dbo].[SP_RPT_GET_EMP_PF_ENROLLMENT_RECORD]        
  @Cmp_ID  numeric        
 ,@From_Date  datetime        
 ,@To_Date  datetime         
 ...
 ,@PrintEmpName varchar(500) = '' 
 ,@reportPath    varchar(max) ='' 
 ,@Payment_Mode varchar(20) = ''  
 ,@For_Attendance tinyint = 0  
 ,@Is_Active tinyint = 0        
 ,@Type varchar(50)=''        
AS        
 SET NOCOUNT ON         
...
END
 ...
 IF @Report_Type =''        
  BEGIN       
   ...
   BEGIN    
   					...
   					Select e.Emp_ID,e.Alpha_Emp_Code,Emp_Full_Name as Emp_Full_Name,EED.ROw_ID,Emp_Code,format(Date_of_Join,'dd/MM/yyyy') as Date_of_Join,
					...
					case when (Gender='M') then 'MALE' when (Gender='F') then 'FEMALE' else '' end as Gender,SSN_No as PF_No,cm.PF_NO as Cmp_PF_No
					...
					,E.Emp_First_Name,TYPE_NAME,Grd_Name,Dept_Name,
					...
					INNER JOIN ( SELECT distinct I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Qry.Bank_ID,Qry.Inc_Bank_AC_No,
					I.Payment_Mode,Vertical_ID,SubVertical_ID,I.Emp_Fix_Salary FROM dbo.T0095_Increment I WITH (NOLOCK)
					INNER JOIN  ( SELECT max(T0095_Increment.Increment_Effective_Date) AS Increment_Effective_Date ,
					max(T0095_Increment.Increment_ID)as Increment_ID,max(T0095_Increment.Bank_ID)as Bank_ID,
					max(T0095_Increment.Inc_Bank_AC_No)as Inc_Bank_AC_No,T0095_Increment.Emp_ID FROM dbo.T0095_Increment WITH (NOLOCK) 
					...
					Where Date_of_Join >= @From_Date and Date_of_Join <= @To_Date and e.Cmp_Id =@Cmp_ID and
					(EED.End_Date in (select Max(End_Date) from T0090_Emp_Experience_Detail WITH (NOLOCK)
					group by Emp_ID) OR EED.End_Date is null)
 ...
 --     ,Reason_Name 
 --     ,E.Work_Email        
 --   FROM dbo.T0080_EMP_MASTER E WITH (NOLOCK) LEFT OUTER JOIN dbo.T0100_Left_Emp l WITH (NOLOCK) ON E.Emp_ID =  l.Emp_ID 
	...
	--  INNER JOIN  dbo.T0040_GRADE_MASTER GM WITH (NOLOCK) ON I_Q.Grd_ID = GM.Grd_ID LEFT OUTER JOIN        
