...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_LEAVE_BALANCE_GET_BKRB150424]
	 @Company_Id		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
...
AS
	...
	IF @Report_Type like 'Balance%' or @report_type = 'COMPOFF-RECORD'
		BEGIN
			...
			DECLARE @query_CompOff  AS NVARCHAR(MAX) --changed by Gadriwala 02102014
						...
						select    Leave_ID,Leave_Name,Cmp_ID
						into  #L_master from T0040_LEAVE_MASTER where Cmp_ID=@cmp_id 
						
						update #L_master set Leave_Name=Replace(Leave_Name,'_',' ') where Leave_Name like '%_%'
						--added by mansi end 
			IF @report_leave=''
				BEGIN
					IF @Report_Type = 'Balance' or @report_type = 'COMPOFF-RECORD'
			...
			ELSE
				BEGIN
					IF @Report_Type = 'Balance' or @report_type = 'COMPOFF-RECORD'
						BEGIN
							...
							INSERT INTO #TMP_LEAVE_DETAIL 
							SELECT	* 
							FROM	(SELECT	Leave_ID, REPLACE(rtrim(ltrim(Leave_Name)),' ','_') AS Leave_Name 
										FROM	T0040_LEAVE_MASTER LM
											INNER JOIN dbo.Split(@report_leave,'#') AS D1 ON LM.Leave_Name = D1.Data
										WHERE	Cmp_ID = @cmp_ID
										UNION
										SELECT	Leave_ID,REPLACE(rtrim(ltrim(Leave_Name)),' ','_') + '_Balance' AS leave_Name 
										FROM	T0040_LEAVE_MASTER LM
											INNER JOIN dbo.Split(@report_leave,'#') AS D1 ON LM.Leave_Name = D1.Data
										WHERE	Cmp_id = @cmp_ID) AS A
							ORDER BY leave_Name
				...
				SELECT @cols = STUFF((SELECT DISTINCT ',' + QUOTENAME(REPLACE(rtrim(ltrim(Leave_Name)),' ','_')) 
				...
				SELECT @Col_name = STUFF((SELECT DISTINCT ',' + QUOTENAME(leave_Name) + ' AS ' + '''' + leave_Name  +'''' 
							FROM #TMP_LEAVE_DETAIL 
					FOR XML PATH(''), TYPE
					).value('.', 'NVARCHAR(MAX)') ,1,1,'')        
		  ...
		  --      if exists(SELECT 1 FROM sys.tables where name ='t1_Comp' ) --Added by Gadriwala Muslim 02102014
  			...
  			--SET @sumColumn = 'sum(' + replace(@Col_name,']','),sum(')
				...
				CREATE UNIQUE CLUSTERED INDEX IX_T3_LEAVE ON #T3 (EMP_ID,FOR_DATE)
				...
				IF @report_leave = ''
					BEGIN
						...
						if exists(SELECT 1 FROM sys.tables where name ='leave_Balance_Comp_Temp' )
								drop table leave_Balance_Comp_Temp 
						if exists(SELECT 1 FROM sys.tables where name ='leave_Balance_Temp' )
						...
						IF @report_type = 'Balance' OR @report_type = 'COMPOFF-RECORD'
							BEGIN
								...
								exec GET_COMPOFF_DETAILS_ALL @To_Date,@Cmp_ID,@Constraint, @compOff_Leave_ID,2	
							
								
								IF EXISTS(SELECT 1 FROM #temp_CompOff)
									BEGIN
										INSERT INTO #leave_Balance_Comp_Temp
										SELECT Emp_ID,Leave_ID,@To_Date AS for_date,Leave_Closing,Replace(Leave_Name, ' ','_') 
				...
				ELSE
					BEGIN
						...
						SELECT  Emp_ID,Leave_ID,@to_Date AS For_Date, Leave_Closing, Leave_Name
						INTO	[leave_Balance_Temp]
						FROM    (SELECT	LT.Emp_ID, LT.Leave_ID, LT.For_Date, LT.Leave_Closing, LM.Leave_Name
								FROM    T0140_LEAVE_TRANSACTION AS LT 
										INNER JOIN (SELECT Emp_ID,Leave_ID,MAX(For_Date) AS For_Date
													FROM   T0140_LEAVE_TRANSACTION
													WHERE  isnull(IsMakerChaker,0) <> 1 and  (For_Date <= @to_Date)
													GROUP BY Emp_ID, Leave_ID) AS LT1 ON LT.Emp_ID = LT1.Emp_ID AND LT.Leave_ID = LT1.Leave_ID AND LT.For_Date = LT1.For_Date 
										INNER JOIN [#emp_cons] AS EC ON LT.Emp_ID = EC.Emp_ID 
										INNER JOIN T0040_LEAVE_MASTER AS LM ON LT.Leave_ID = LM.Leave_ID AND IsNull(lm.Default_Short_Name,'') <> 'COMP' --Changed by Gadriwala Muslim 02102014
										INNER JOIN dbo.Split(@report_leave,'#') AS RL on LM.Leave_Name = RL.Data) AS A
				...
				SELECT * INTO #leave_balance_temp FROM leave_balance_temp
					union all
				SELECT * FROM #leave_Balance_Comp_Temp --changed by Gadriwala Muslim 02102014
				...
				IF @report_type = 'Balance' or @report_type = 'COMPOFF-RECORD'
					BEGIN
						...
						SELECT @cols_balance = STUFF((SELECT DISTINCT ',' + QUOTENAME(Leave_Name) 
								FROM #leave_balance_temp 
						FOR XML PATH(''), TYPE
						).value('.', 'NVARCHAR(MAX)') ,1,1,'')
	      
	      
						SELECT @cols_name_bal = STUFF((SELECT DISTINCT ',' + QUOTENAME(Leave_Name) + ' AS ' + '''' + cast(Leave_Name AS VARCHAR) +'''' 
					   ...
					   --select Leave_name from #leave_balance_temp
						...
						SELECT DISTINCT Leave_Name FROM #leave_balance_temp 
						OPEN cur
						FETCH NEXT FROM cur INTO @leave_Code_temp
						WHILE @@FETCH_STATUS = 0
							BEGIN
				...
				SELECT	E.Alpha_Emp_Code,E.Emp_Full_Name,T3.*,I_Q.Branch_ID
	...
	ELSE IF @Report_Type like 'Total%'
		BEGIN		
			...
			SELECT @cols_T = STUFF((SELECT DISTINCT ',' + QUOTENAME(Leave_Name) 
						FROM #Leave_Name 
				FOR XML PATH(''), TYPE
				).value('.', 'NVARCHAR(MAX)') ,1,1,'')
	      
			SELECT @Col_name = STUFF((SELECT DISTINCT ',' + QUOTENAME(Leave_Name) + ' AS ' + '''' + cast(Leave_Name AS VARCHAR) +'''' 
						FROM #Leave_Name
				FOR XML PATH(''), TYPE
				).value('.', 'NVARCHAR(MAX)') ,1,1,'')        
			--Changed by Gadriwala Muslim 02102014
	...
	--      if exists(SELECT 1 FROM sys.tables where name ='t1_Comp' ) --Added by Gadriwala Muslim 02102014
		...
		SELECT * INTO #LEAVE_TRAN_MAIN
		FROM	#LEAVE_TRAN
		UNION ALL
		SELECT * FROM #LEAVE_TRAN_COMP
	...
	--      SET @Query = 'if exists(SELECT 1 FROM sys.tables where name =''t2'' )
		...
		SELECT	LTS.*,(isnull(LT2.Leave_Used,0) + ISNULL(LT3.CompOFF_Used,0)) AS Total_Leave_Used 
		INTO	#t3_T 
		FROM	#LEAVE_TRAN_SUM LTS
				LEFT OUTER JOIN (SELECT	Cmp_ID,Emp_ID,SUM(Leave_Used ) AS Leave_Used 
									FROM	(SELECT LT.Cmp_ID,LT.Emp_ID,
												SUM(CASE WHEN Apply_Hourly = 1 THEN ((LT.Leave_Used) * 0.125) ELSE (LT.Leave_Used + IsNULL(LT.Back_Dated_Leave,0)) end) AS Leave_Used
										FROM    T0140_LEAVE_TRANSACTION AS LT 
												INNER JOIN T0040_LEAVE_MASTER AS LM ON LT.Leave_ID = LM.Leave_ID AND IsNull(LM.Default_Short_Name,'') <> 'COMP' 
												INNER JOIN [#Emp_Cons] AS EC ON LT.Emp_ID = EC.emp_id
										WHERE isnull(LT.IsMakerChaker,0) <> 1 and  (LT.Cmp_ID = @Cmp_ID) AND (LT.For_Date >= @From_Date) AND (LT.For_Date <= @To_Date)
										GROUP BY LT.Cmp_ID,LT.Emp_ID,Apply_Hourly 
										) LT1  
								GROUP BY Cmp_ID,Emp_ID) LT2 ON LTS.Emp_ID = LT2.Emp_ID
				LEFT OUTER JOIN (SELECT	LT.Cmp_ID,LT.Emp_ID,
										CASE WHEN Apply_Hourly = 1 THEN (SUM(LT.CompOff_Used - LT.Leave_Encash_Days) * 0.125) ELSE SUM(LT.CompOff_Used - LT.Leave_Encash_Days) END AS CompOFF_Used --Added by Gadriwala Muslim 02102014
									FROM   T0140_LEAVE_TRANSACTION AS LT 
										INNER JOIN T0040_LEAVE_MASTER AS LM ON LT.Leave_ID = LM.Leave_ID AND IsNull(LM.Default_Short_Name,'') = 'COMP' 
										INNER JOIN [#Emp_Cons] AS EC ON LT.Emp_ID = EC.emp_id
									WHERE isnull(LT.IsMakerChaker,0) <> 1 and  (LT.Cmp_ID = @Cmp_ID) AND (LT.For_Date >= @from_date) AND (LT.For_Date <= @to_date)
									GROUP BY LT.Cmp_ID, LT.Emp_ID, LM.Apply_Hourly) LT3 ON LTS.Emp_ID = LT3.Emp_ID
		...
		SELECT	E.Alpha_Emp_Code,E.Emp_Full_Name,DM.Dept_Name as Department,CTM.CAT_Name,T3_T.* ,I_q.Branch_ID
		FROM	dbo.T0080_EMP_MASTER E 
				LEFT OUTER JOIN dbo.T0100_Left_Emp l on E.Emp_ID =  l.Emp_ID 
				INNER JOIN (SELECT	I.Emp_Id,Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID 
							FROM	dbo.T0095_INCREMENT I 
