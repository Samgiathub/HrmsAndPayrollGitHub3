...
CREATE PROCEDURE [dbo].[SP_RPT_TRAINING_HISTORY]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric  
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
	    ...
	    select TD.*, E.Emp_Code as Emp_code,Emp_superior,DM.Dept_Name,DGM.Desig_Name,Type_Name,Grd_Name,BM.Branch_Name,Date_of_Join,Gender,BM.Comp_Name,BM.Branch_Address,CM.Cmp_Name,Cm.Cmp_address  
						...
						(select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID from T0095_Increment I WITH (NOLOCK) inner join   
						  ...
						  WHERE TD.Cmp_ID = @Cmp_Id and Training_ID=@Training_id   and Training_date >=@From_date and Training_Date <= @To_date 
