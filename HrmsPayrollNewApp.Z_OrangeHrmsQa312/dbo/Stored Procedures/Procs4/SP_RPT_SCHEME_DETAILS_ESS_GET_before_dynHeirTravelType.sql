...
create PROCEDURE [dbo].[SP_RPT_SCHEME_DETAILS_ESS_GET_before_dynHeirTravelType]  
  @Cmp_ID  Numeric  
 ,@From_Date  Datetime  
 ,@To_Date  Datetime  
 ,@Branch_ID  Numeric   
...
--select * from #SCHEME   
 ...
 OPEN Emp_Scheme_Cursor  
   ...
   while @@fetch_status = 0  
    ...
    if exists(select 1 from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and ( Is_RM = 1 or  Is_PRM=1 or Dyn_Hier_Id = 1)  and Scheme_Id=@Scheme_Id1 and Rpt_Level=1)  
      begin  
        set @temp=''  
        set @rm_name=''   
        ...
        select @Is_PRM_level1 = Is_PRM ,@DynHierId = Dyn_Hier_Id from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and Scheme_Id=@Scheme_Id1 and (Is_PRM=1 or Dyn_Hier_Id = 1) and Rpt_Level=1  
