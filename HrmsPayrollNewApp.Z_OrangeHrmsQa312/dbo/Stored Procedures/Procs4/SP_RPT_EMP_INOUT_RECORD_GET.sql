CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET] 
	@Cmp_ID			numeric,      
	@From_Date		DATETIME,      
	@To_Date		DATETIME ,      
	@Branch_ID		numeric   ,      
...
Where NOT EXISTS (SELECT 1 from  #Data as I Where  I.emp_id = E.Emp_ID AND I.for_Date = E.CurrentDate)
	...
	,Duration_in_sec
	,isnull(Shift_Dur,0)
	,dbo.F_Return_Hours(isnull(Shift_Dur,0))
...
--------------------End Holiday Cursor---------------------
UPDATE  #Emp_Inout SET AB_LEAVE ='HO' ,A_days =0 
    ,Shift_ID= NULL -- copy from WO exist condition and set for holiday-  sandip -23102024
	,Shift_Sec =0
    ,Shift_St_Datetime=NULL,
	Shift_en_Datetime=NULL
...
-- Deepal ST 16-4-24 copy from above and paste here
 Update #Emp_Inout SET Total_Less_Work_Sec	= tblLess.Less_Work
						  ,Less_Work= dbo.F_Return_Hours(tblLess.Less_Work)
	From #Emp_Inout Inner Join
			(
				...
				from #Emp_Inout	group by emp_id,for_Date )as tblLess On #Emp_Inout.Emp_id = tblLess.Emp_id
				And #Emp_Inout.For_Date = tblLess.For_Date
				Where (Total_Work_Sec >0  OR Shift_Sec>0) and AB_LEAVE not in ('WO','HO')
-- Deepal ST 16-4-24 copy from above and paste here

-- Deepal DATE :- 16-04-24 Update the WeekOF OT SEC from #data to EMP_INout base of setting Week off Transfer to OT
Update #Emp_Inout SET
More_Work = Case when t.Weekoff_OT_Sec > 0 OR t.Holiday_OT_Sec > 0 then 
			Case when t.Weekoff_OT_Sec > 0 Then  dbo.F_Return_Hours(t.Weekoff_OT_Sec) else dbo.F_Return_Hours(t.Holiday_OT_Sec)
			END ELSE '00:00' END

from #Emp_Inout EIO inner join (
	Select D.Weekoff_OT_Sec,E.for_Date , E.emp_id ,D.Holiday_OT_Sec
	from #Emp_Inout E inner join #DATA D on E.emp_id = D.Emp_Id and E.for_Date = D.For_date
	where Weekoff_OT_Sec > 0 OR Holiday_OT_Sec > 0
) as t on  EIO.emp_id = t.emp_id and EIO.for_Date = t.for_Date
-- Deepal DATE :- 16-04-24 Update the WeekOF OT SEC from #data to EMP_INout base of setting Week off Transfer to OT
...
,Reason=(select top 1 isnull(LAD.Leave_Reason,'') from T0120_LEAVE_APPROVAL LA
join T0130_LEAVE_APPROVAL_DETAIL LAD on LAD.Leave_Approval_ID = LA.Leave_Approval_ID
where LA.Emp_ID = E.emp_id and LAD.From_Date=E.for_Date) --Added by ronakk 27042022
...
---------End -----------
	...
	---------- Corporate Office Report Add By Jignesh Patel 03-Mar-2022-----------
	
	
	IF @Report_call = 'IN-OUT' OR @Report_call = 'Inout_Page' OR @Report_call = 'Simsona' OR @Report_call = 'Corporate Office Report'
		BEGIN 
		   print  111222---mansi
							...
							IF @InOut_Tag = 'CSV'
								...
								END
									...
									if exists (select 1 from ##TCol)
										...
										------------ Add by jigensh Patel 03-Mar-2022-----------------
										if @Report_call = 'Corporate Office Report'
										Begin
											IF  object_id('tempdb..#tmpTblDeptWiseStrength') IS NOT NULL 
											BEGIN      
												drop table #tmpTblDeptWiseStrength  
											END  
	
											SELECT    count(I.Emp_Id) as Strength_Of_Dept ,I.Dept_ID
											Into #tmpTblDeptWiseStrength
												FROM    dbo.T0095_INCREMENT I WITH (NOLOCK) inner join       
															( SELECT    max(I.Increment_ID) AS Increment_ID, I.Emp_ID FROM dbo.T0095_INCREMENT I WITH (NOLOCK)      
												...
												select 
												Row_Number() over (Order by D.Dept_Name) as Sr_No
												,D.Dept_ID
												,D.Dept_Name as Department,
												max(DS.Strength_Of_Dept) as Strength_Of_Department 
												,sum(case when Leave_Days>0 then 1 else 0 end) as Employees_On_Leave 
												,sum(case when p_Days>0 then 1 else 0 end) as Actual_Present 
												,sum(case when Late_In_Sec>0 then 1 else 0 end) as Late_Coming 
												,sum(case when Early_Out_sec>0 then 1 else 0 end) as Early_Going 

												,sum(case when (Not In_Time Is Null And  Out_Time Is Null) then 1 else 0 end)
												+sum(case when ( In_Time Is Null And Not Out_Time Is Null) then 1 else 0 end) as One_Punch  
												,sum(case when AB_LEAVE = 'AB' then 1 else 0 end) as No_Punch
												from #INOUT_REPLICA AS A  Inner Join T0040_DEPARTMENT_MASTER as D 
												On A.Dept_id = D.Dept_Id
												Inner Join #tmpTblDeptWiseStrength  AS DS On D.Dept_Id = DS.Dept_ID
												where emp_id >0
										...
										print 121212123---mansi
											...
											SELECT
												...
												,CC.*
												,BM.Branch_Name
												,cm.Cmp_Name,--added by mansi 
												cm.Cmp_Address--added by mansi 
										    ...
										    left join T0010_COMPANY_MASTER cm on cm.Cmp_Id=@Cmp_ID
										End 
										...
										Begin
											...
											IF  object_id('tempdb..#tmpTblDeptWiseStrength_1') IS NOT NULL 
											BEGIN      
												drop table #tmpTblDeptWiseStrength_1  
											END  
	
											SELECT    count(I.Emp_Id) as Strength_Of_Dept ,I.Dept_ID
											Into #tmpTblDeptWiseStrength_1
												FROM    dbo.T0095_INCREMENT I WITH (NOLOCK) inner join       
															( SELECT    max(I.Increment_ID) AS Increment_ID, I.Emp_ID FROM dbo.T0095_INCREMENT I WITH (NOLOCK)      
															WHERE   Increment_effective_Date <= @From_Date    
															group by I.emp_ID  ) Qry on I.Emp_ID = Qry.Emp_ID and      
												...
												+sum(case when ( In_Time Is Null And Not Out_Time Is Null) then 1 else 0 end) as One_Punch  
												,sum(case when AB_LEAVE = 'AB' then 1 else 0 end) as No_Punch
												from #Emp_Inout AS A  Inner Join T0040_DEPARTMENT_MASTER as D 
												On A.Dept_id = D.Dept_Id
												Inner Join #tmpTblDeptWiseStrength_1  AS DS On D.Dept_Id = DS.Dept_ID
												where emp_id >0
												And For_Date =@From_Date
												Group by D.Dept_ID,D.Dept_Name
										End
...
--add by chetan 250517 for HMDA
	...
	ELSE IF @Report_call = 'Time_Card'
		BEGIN
				...
				and ( In_Time is not null  OR Out_Time is not null  OR ab_leave is not null ) 
				-- Order by 
				Order by E_IO.for_Date, Case When IsNumeric(e.Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + e.Alpha_Emp_Code, 20)
						When IsNumeric(e.Alpha_Emp_Code) = 0 then Left(e.Alpha_Emp_Code + Replicate('',21), 20)
