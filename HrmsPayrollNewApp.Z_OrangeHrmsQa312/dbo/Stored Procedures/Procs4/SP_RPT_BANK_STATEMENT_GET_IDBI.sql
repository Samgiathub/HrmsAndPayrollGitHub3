...
CREATE PROCEDURE [dbo].[SP_RPT_BANK_STATEMENT_GET_IDBI]
	 @CMP_ID		 NUMERIC  
	,@FROM_DATE		 DATETIME  
	,@TO_DATE		 DATETIME  
...
SET ARITHABORT ON
	...
	SELECT		EM.ALPHA_EMP_CODE AS ALPHA_EMP_CODE,BM.BRANCH_NAME,'="' + CAST(CASE WHEN MS.Salary_Status = 'Hold' THEN '0' ELSE MS.NET_AMOUNT END AS VARCHAR) + '"' AS AMOUNT,
				--'' AS 'SENDER A/C NUMBER',EM.IFSC_CODE as 'BENEFICIERY IFSC','="' +INC_QRY.INC_BANK_AC_NO + '"' AS 'BENEFICIERY A/C NUMBER',
				 '="' + BM1.Bank_Ac_No + '"'   AS 'SENDER A/C NUMBER'
				,EM.IFSC_CODE as 'BENEFICIERY IFSC','="' +INC_QRY.INC_BANK_AC_NO + '"' AS 'BENEFICIERY A/C NUMBER',
				'' AS 'BENEFICIERY A/C TYPE',ISNULL(EM.EMPNAME_ALIAS_PRIMARYBANK,EM.EMP_FULL_NAME) AS 'BENEFICIERY A/C NAME',
				INC_QRY.BANK_BRANCH_NAME AS 'BENEFICIERY CUST ADDR 1',
				--'' AS 'SENDER TO RECEIVER INFO 1'
				'Salary' + DATENAME(month, @TO_DATE) + Convert(Varchar(4),DATEPART(Year,@TO_DATE)) AS 'SENDER TO RECEIVER INFO 1',	
				--'' AS 'ORIGINATOR OF REMITTANCE 1'--,INC_QRY.PAYMENT_MODE
				CM.CMP_NAME AS 'ORIGINATOR OF REMITTANCE 1'--,INC_QRY.PAYMENT_MODE
				,CM.CMP_NAME
				,CM.CMP_ADDRESS,BM.COMP_NAME,BM.BRANCH_ADDRESS
				,@FROM_DATE AS FROM_DATE,@TO_DATE AS TO_DATE
				,@PAYMENT_MODE  AS  PAYMENT_MODE  --added by jimit 23122016	
				,DP.DEPT_NAME,DM.DESIG_NAME,TM.TYPE_NAME,GM.GRD_NAME , BN.BANK_ID		--added by jimit 26122016
	INTO		#FINAL_TABLE					
	FROM		T0080_EMP_MASTER EM WITH (NOLOCK)
			...
			LEFT OUTER JOIN  --Added By Jimit 03122018
				(
	...
	WHERE		MONTH(MS.MONTH_END_DATE) = MONTH(@TO_DATE) AND YEAR(MS.MONTH_END_DATE) = YEAR(@TO_DATE) 
				...
				AND ISNULL(MS.Is_FNF,0) = 0 AND MS.Net_Amount > 0
	...
	--Added By Ramiz as per Requirement of Golcha -- No Need to Show Amount if Status is Hold
