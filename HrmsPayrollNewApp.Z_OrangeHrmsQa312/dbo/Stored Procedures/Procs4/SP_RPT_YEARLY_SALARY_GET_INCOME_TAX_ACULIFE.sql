...
CREATE PROCEDURE [dbo].[SP_RPT_YEARLY_SALARY_GET_INCOME_TAX_ACULIFE]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	...
	,@Segment_Id  	varchar(max) = 0		 -- Added By Gadriwala Muslim 21082013
	,@Vertical_Id 	varchar(max) = 0		 -- Added By Gadriwala Muslim 21082013
	,@SubVertical_Id varchar(max) = 0	 -- Added By Gadriwala Muslim 21082013	
	,@SubBranch_Id 	varchar(max) = 0		 -- Added By Gadriwala Muslim 21082013	
...
SET ARITHABORT ON
	...
	delete #Emp_Cons from #Emp_Cons EC Inner Join T0095_INCREMENT I on EC.Increment_ID = I.Increment_ID where Cat_ID=2	
	...
	ELSE
		...
		(Select ms.Cmp_ID,EC.Emp_ID,month(Ms.Month_End_Date),YEAR(ms.Month_End_Date),isnull(SPE.Is_Publish,0) FROM T0200_MONTHLY_SALARY Ms WITH (NOLOCK) 
		...
		Create nonclustered index IX_Yearly_Salary_Emp_ID_Def_ID_AD_ID_Loan_id on #Yearly_Salary (	Emp_ID,AD_ID,Loan_id) INCLUDE (Month_1,Month_2,Month_3,Month_4,Month_5,Month_6,Month_7,Month_8,Month_9,Month_10,Month_11,Month_12)
			...
			select DISTINCT @Cmp_ID,EC.emp_ID,'i'+CAST(Isnull(Isnull(IFD.AD_ID,IFD.Rimb_ID),Default_Def_Id)  as varchar(max)),Field_Name ,Isnull(IFD.AD_ID,IFD.Rimb_ID),Row_ID ,Default_Def_Id
		...
		Declare @Default_Def_Id_Leave_Encash  tinyint
		Set @Default_Def_Id_Leave_Encash = 2
		...
		while @Temp_Date >=@From_Date 
			Begin
					...
					set @sqlQuery= ''					
						...
						set @sqlQuery = 'Update #Yearly_Salary  set ' + @Str_Month + ' = Round(Salary_Amount + isnull(Arear_Basic ,0) + isnull(Qry.S_Salary_Amount,0),0)
					...
					exec (@sqlQuery)
					set @sqlQuery= ''
				/* -- Commented by Hardik 05/06/2020 for Aculife as Bonus amount not adding for F&F case so added below query and sum of amount

						set @sqlQuery = 'Update #Yearly_Salary  set ' + @Str_Month + '= (m_AD_AMOUNT + isnull(M_AREAR_AMOUNT ,0) + isnull(MS_arear.ms_amount,0))
				...
				*/
						...
						(select MAD.Emp_Id, MAD.AD_ID, (sum(m_AD_AMOUNT) + isnull(sum(M_AREAR_AMOUNT) ,0) + isnull(sum(MS_arear.ms_amount),0))  As M_AD_Amount
							From #Yearly_Salary  Ys  inner join T0210_MONTHLY_AD_DETAIL MAD WITH (NOLOCK) on YS.AD_ID = MAD.AD_ID and ys.emp_ID = MAD.emp_ID 
							...
							Where Month(to_Date)= ' + Cast(@Month as varchar(3)) + ' And Year(To_Date) = '+ Cast(@Year as varchar(4)) + ' 
							and isnull(mad.S_Sal_Tran_ID,0) = 0	 and (SP.Publish_Flag = 1 or isnull(ms.is_fnf,0)=1)
							group by MAD.Emp_ID, MAD.AD_ID) Qry On Y.Emp_Id = Qry.Emp_Id And Y.AD_id = Qry.AD_ID'
						...
						set @sqlQuery= ''
								...
								From #Yearly_Salary YS Inner Join
									...
									(Select Emp_ID,RC_Id,isnull(sum(Taxable),0) + isnull(sum(Tax_free_amount),0)As Amount
							...
							-- Added by Hardik 27/04/2018 for Aculife for Manual Import Allowance
							set @sqlQuery = ' Update #Yearly_Salary Set  ' + @Str_Month + ' = isnull(Amount,0)
												From #Yearly_Salary YS Inner Join
													T0190_MONTHLY_AD_DETAIL_IMPORT T On YS.Ad_Id=T.Ad_Id And YS.Emp_Id=T.Emp_Id Inner Join 
													T0050_AD_MASTER A On T.AD_Id=A.AD_Id
												where Isnull(A.Is_Calculated_On_Imported_Value,0)=1 And
														Month(for_Date)= ' + Cast(@Month as varchar(3)) + ' and year(for_Date) = ' + Cast(@Year as varchar(4)) 
		...
		If @Report_Call = '' or @Report_Call = 'All'
			...
			Begin
				...
				from (select Alpha_Emp_code,Emp_Full_Name,Branch_Name, Dept_Name,Desig_Name,Date_of_Join,Lable_Name, isnull(Total,0) as Total from #YS_IT) 
				as data pivot 
				( sum(Total) 
				...
				select * from #YS_IT_1 order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)
