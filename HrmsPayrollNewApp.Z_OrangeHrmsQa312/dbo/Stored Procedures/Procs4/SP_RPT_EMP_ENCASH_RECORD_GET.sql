...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_ENCASH_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	--,@Branch_ID		numeric   = 0
			...
			Begin
								...
								select I_Q.* ,E.Emp_Full_Name , E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,Left_Date,BM.Comp_Name,BM.Branch_Address,Left_Reason
									...
									WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
											(select Emp_ID from t0120_leave_encash_approval WITH (NOLOCK) where Lv_encash_apr_date >= @From_Date and Lv_encash_apr_date <= @To_Date and Cmp_ID=@Cmp_ID and lv_encash_apr_status=(CASE WHEN @Status = 'ALL' THEN Lv_Encash_Apr_Status ELSE @Status END)) 
					...
					from T0080_EMP_MASTER E WITH (NOLOCK) left outer join T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
						( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID from T0095_Increment I WITH (NOLOCK) inner join 
							...
							on E.Emp_ID = I_Q.Emp_ID  inner join
								...
								T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID
					WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
							(select Emp_ID from t0120_leave_encash_approval WITH (NOLOCK) where Lv_encash_apr_date >= @From_Date and Lv_encash_apr_date <= @To_Date and Cmp_ID=@Cmp_ID) 
							And E.Emp_ID in (select Emp_ID From #Emp_Cons)
					Order by Case When IsNumeric(e.Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + e.Alpha_Emp_Code, 20)
			When IsNumeric(e.Alpha_Emp_Code) = 0 then Left(e.Alpha_Emp_Code + Replicate('',21), 20)
				Else e.Alpha_Emp_Code
