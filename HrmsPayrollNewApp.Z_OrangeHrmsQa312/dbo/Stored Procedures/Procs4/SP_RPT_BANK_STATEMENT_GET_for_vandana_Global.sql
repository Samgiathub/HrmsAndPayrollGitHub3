...
CREATE PROCEDURE [dbo].[SP_RPT_BANK_STATEMENT_GET_for_vandana_Global]  
  @Cmp_ID			numeric  
 ,@From_Date		datetime  
 ,@To_Date			datetime  
 ,@Branch_ID		numeric  
...
FROM T0040_BANK_MASTER BM
IF LEN(ISNULL(@Bank_ID, '')) = 0
	INSERT INTO #BANK_MASTER 
	SELECT  * FROM T0040_BANK_MASTER BM
	WHERE BM.CMP_ID = @CMP_ID
ELSE
	INSERT INTO #BANK_MASTER 
	SELECT  BM.* FROM T0040_BANK_MASTER BM
 ...
 /* FOR BINDING COLUMNS */
	IF @IS_COLUMN = 1 AND @Report_Type = 'Details'
		...
		print 111---mansi
			...
			FROM #EMP_SALARY MS
				INNER JOIN		T0080_EMP_MASTER E ON MS.emp_ID = E.emp_ID 
				INNER JOIN		T0095_Increment I_Q ON MS.Increment_ID = I_Q.Increment_ID
				LEFT OUTER JOIN #BANK_MASTER BK ON I_Q.Bank_ID = BK.Bank_ID
				LEFT OUTER JOIN T0030_CATEGORY_MASTER CM ON I_Q.Cat_ID = CM.Cat_ID
			RETURN	
		END
 ...
 ELSE IF @Sal_Type = 1   
		...
		print 456---mansi
			...
			SELECT S_Sal_Tran_ID, S_Sal_Receipt_No, ms.Emp_ID, Cmp_ID, ms.Increment_ID, S_Month_St_Date, S_Month_End_Date, S_Sal_Generate_Date, S_Sal_Cal_Days, S_M_Present_Days,   
				  ...
				  S_PT_F_T_Limit , '' 
			 FROM T0201_MONTHLY_SALARY_Sett ms 
			 inner join #Emp_Cons ec on ms.emp_ID =ec.emp_ID   
		...
		print 00 --mansi
					 ...
					 IF @Director_details = 0
						BEGIN
							 SELECT MS.*,Isnull(EmpName_Alias_PrimaryBank,Emp_full_Name) As Emp_full_Name,Branch_Address,Branch_name,Comp_name,Grd_Name,DAteName(M,Month_End_Date)as Month,YEar(Month_End_Date)as Year
							  ...
							  ,ctm.Cat_Name--added by mansi 15_09_22
								,DATEDIFF (yy, DATE_OF_JOIN, GETDATE()) as Years--added by mansi 15_09_22
							  ,DATEDIFF (MONTH, DATE_OF_JOIN, GETDATE()) as Months--added by mansi 21_09_22
							 ,DATEDIFF (DAY, DATE_OF_JOIN, GETDATE()) as Number_of_Days--added by mansi 21_09_22 
							  INTO #FINAL_TABLE
							  FROM #Emp_Salary MS 
								INNER JOIN		T0080_EMP_MASTER E on MS.emp_ID = E.emp_ID
								INNER JOIN		T0095_Increment I_Q on Ms.Increment_ID = I_Q.Increment_ID
								INNER JOIN		T0030_CATEGORY_MASTER ctm ON I_Q.Cat_ID = ctm.Cat_ID--added by mansi 15_09_22
								...
								LEFT OUTER JOIN #BANK_MASTER bk on i_Q.Bank_ID = Bk.Bank_ID
								LEFT OUTER JOIN T0040_COST_CENTER_MASTER CCM ON CCM.Center_ID = I_Q.Center_ID
								INNER JOIN		T0010_COMPANY_MASTER CM ON MS.CMP_ID = CM.CMP_ID --ADDED BY JAINA 10-08-2015  
						...
						BEGIN
							SELECT MS.*,Isnull(EmpName_Alias_PrimaryBank,Emp_full_Name) As Emp_full_Name,Branch_Address,branch_name,Comp_name,Grd_Name,DAteName(M,Month_End_Date)as Month,YEar(Month_End_Date)as Year 
							 ...
							 FROM #Emp_Salary MS 
								   ...
								   INNER JOIN		T0030_Branch_Master BM on I_Q.Branch_ID = BM.Branch_ID 
								   Left outer Join  #BANK_MASTER bk on i_Q.Bank_ID = Bk.Bank_ID
								   INNER JOIN		T0010_COMPANY_MASTER CM ON MS.CMP_ID = CM.CMP_ID
								   LEFT OUTER JOIN  #Total   tem on cm.cmp_id = tem.cmp_id and I_q.Bank_ID = tem.Bank_Id
								   LEFT OUTER JOIN
												(SELECT top 1 * FROM T0010_COMPANY_DIRECTOR_DETAIL WHERE Cmp_Id = @Cmp_ID ) CDD ON CDD.Cmp_Id = CM.Cmp_Id
								WHERE E.Cmp_ID = @Cmp_Id   
							   --and isnull(i_Q.Bank_ID,0) = isnull(@Bank_ID,isnull(i_Q.Bank_ID,0)) -- and Salary_Amount >0
							   and I_q.Payment_mode = @Payment_mode  
							   and Month(Month_End_Date) = Month(@To_Date) and Year(Month_End_Date) =Year(@To_Date)
			...
			ELSE
				...
				print 1--mansi
						...
						FROM T0200_MONTHLY_SALARY M 
							INNER JOIN	T0095_Increment I on M.Emp_ID = I.Emp_ID and m.Increment_ID = i.Increment_ID
							INNER JOIN	T0080_EMP_MASTER EM ON M.Emp_ID = EM.Emp_ID
