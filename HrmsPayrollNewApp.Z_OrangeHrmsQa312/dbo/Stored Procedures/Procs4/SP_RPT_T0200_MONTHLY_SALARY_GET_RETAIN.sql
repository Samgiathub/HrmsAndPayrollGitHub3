...
CREATE PROCEDURE [dbo].[SP_RPT_T0200_MONTHLY_SALARY_GET_RETAIN]            
 @Cmp_ID   numeric            
 ,@From_Date  datetime            
 ,@To_Date   datetime            
 ,@Branch_ID  numeric            
...
CREATE table #Emp_Cons   
 ...
 if day(@From_Date) <> 1  
  ...
  (            
   ...
   Retain_Days   numeric(18, 2)  default 0
  )     
...
--wHERE EC.Emp_Id=10515 AND EC.mst_Tran_Id =@VendarCodeID  
 ...
 if @Sal_Type = 0     
   ...
   INSERT INTO #Emp_Salary            
      ...
      PT_F_T_Limit,Late_Days,Net_Salary_Round_Diff_Amount,GatePass_Deduct_Days,Retain_Days)      -- Added by Gadriwala Muslim 18032015       
   ...
   select Sal_Tran_ID, Sal_Receipt_No, ms.Emp_ID, ms.Cmp_ID, ms.Increment_ID, Month_St_Date, Month_End_Date, Sal_Generate_Date, Sal_Cal_Days, Present_Days,           
      ...
      PT_Calculated_Amount, Total_Claim_Amount, M_OT_Hours, M_Adv_Amount, M_Loan_Amount, M_IT_Tax, LWF_Amount, Revenue_Amount,           
      PT_F_T_Limit  ,isnull(Late_Days,0),Net_Salary_Round_Diff_Amount,GatePass_Deduct_Days,Retain_Days   -- Added by Gadriwala Muslim 18032015   
   ...
   Advance_Amount, Other_Dedu_Amount, Total_Dedu_Amount, Due_Loan_Amount, Net_Amount, Actually_Gross_Salary, PT_Amount,             
   PT_Calculated_Amount, Total_Claim_Amount, M_OT_Hours, M_Adv_Amount, M_Loan_Amount, M_IT_Tax, LWF_Amount, Revenue_Amount,             
   PT_F_T_Limit,Net_Salary_Round_Diff_Amount,Retain_Days)            
   ...
   S_PT_F_T_Limit ,0,MS1.Retain_Days           
 ...
 set S_Sal_Tran_ID = ms.S_Sal_Tran_ID,             
  ...
  , LWF_Amount = LWF_Amount + S_LWF_Amount, Revenue_Amount = Revenue_Amount + s_Revenue_Amount, PT_F_T_Limit = S_PT_F_T_Limit , Retain_Days = es.Retain_Days           
 ...
 set L_Sal_Tran_ID = ms.L_Sal_Tran_ID,             
  ...
  , LWF_Amount = LWF_Amount + L_LWF_Amount, Revenue_Amount = Revenue_Amount + L_Revenue_Amount, PT_F_T_Limit = L_PT_F_T_Limit   , Retain_Days = es.Retain_Days         
 ...
 SET @From_Date = @From_Date-DAY(@From_Date)+1
  ...
  ,Month(Ms.Month_St_Date)as Month,YEar(Ms.Month_St_Date)as Year  
    ...
    ,dbo.F_Get_Age(E.Date_OF_Birth,getdate(),'Y','Y') as Emp_Age_In_Words,MSMA.M_WO_OT_Hours, MSMA.M_HO_OT_Hours  
