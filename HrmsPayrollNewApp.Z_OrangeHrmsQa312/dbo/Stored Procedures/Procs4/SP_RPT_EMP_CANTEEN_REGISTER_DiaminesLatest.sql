CREATE  PROCEDURE [dbo].[SP_RPT_EMP_CANTEEN_REGISTER_DiaminesLatest] 
...
(
	...
	SET		--ENROLL_NO=E.ENROLL_NO,
			ENROLL_NO=CASE WHEN  isnull(e.old_ref_no,'') = '' THEN e.Enroll_No ELSE e.Old_Ref_No END,
			GRADE=I.GRD_ID
	...
	FROM T0080_EMP_MASTER EM INNER JOIN #EMP_CONS EC ON  EM.EMP_ID = EC.EMP_ID
	LEFT OUTER JOIN
				(
	 ...
	 LEFT OUTER JOIN T0040_BUSINESS_SEGMENT BS ON B.SEGMENT_ID = BS.SEGMENT_ID
	 INNER JOIN T0050_CANTEEN_MASTER CM ON B.CMP_ID = CM.CMP_ID 
	...
	IF EXISTS( SELECT 1 FROM #CANTEEN_MASTER )
		BEGIN
			...
			INNER JOIN T0080_EMP_MASTER EM ON (CASE WHEN  ISNULL(EM.OLD_REF_NO,'') = '' THEN CAST(EM.ENROLL_NO AS VARCHAR(15)) ELSE EM.OLD_REF_NO END) = CAST(CID.ENROLL_NO AS VARCHAR(15))
			INNER JOIN DBO.T0050_CANTEEN_MASTER CM ON CM.IP_ID = IP.IP_ID
			...
			INNER JOIN DBO.T0050_CANTEEN_MASTER CM ON CM.CNT_ID = CID.CANTEEN_ID
			INNER JOIN T0040_IP_MASTER IP ON CM.IP_ID=IP.IP_ID 
			INNER JOIN T0080_EMP_MASTER EM ON EM.EMP_ID = CID.EMP_ID
			INNER JOIN #EMP_CONS EC ON EM.EMP_ID = EC.EMP_ID
			INNER JOIN #GET_CANTEEN_EMPLOYEE CC ON CC.EMP_ID = EC.EMP_ID
			INNER JOIN #CANTEEN_MASTER TCM ON CM.CNT_ID = TCM.CANTEEN_ID
			WHERE CID.CANTEEN_PUNCH_DATETIME BETWEEN @FROM_DATE AND @TO_DATE
			AND (CID.flag='Mobile' and CID.Reason='Mobile' OR IP.Device_No >= 200) and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
			AND  EM.EMP_LEFT='N'  
			GROUP BY EM.EMP_ID,IP.IP_ID,CC.DEPT_NAME,EM.EMP_FULL_NAME,CC.SEGMENT_NAME,CM.CNT_NAME,EM.EMP_CODE
	...
	SELECT EM.EMP_ID,CID.IO_DATETIME,'1',IP.IP_ID FROM T9999_DEVICE_INOUT_DETAIL CID
	INNER JOIN T0040_IP_MASTER IP ON CID.IP_ADDRESS=IP.IP_ADDRESS 
	--INNER JOIN T0080_EMP_MASTER EM ON EM.ENROLL_NO = CID.ENROLL_NO
	INNER JOIN T0080_EMP_MASTER EM ON (CASE WHEN  isnull(EM.old_ref_no,'') = '' THEN CAST(EM.ENROLL_NO AS VARCHAR(50)) ELSE EM.Old_Ref_No END) = CAST(CID.ENROLL_NO AS VARCHAR(50))
	--INNER JOIN T0080_EMP_MASTER EM ON (CASE WHEN  isnull(EM.old_ref_no,'') = '' THEN EM.ENROLL_NO ELSE EM.Old_Ref_No END) = CID.ENROLL_NO
	INNER JOIN DBO.T0050_CANTEEN_MASTER CM ON CM.IP_ID = IP.IP_ID -- added on 05032019
	INNER JOIN #EMP_CONS EC ON EM.EMP_ID = EC.EMP_ID 
	WHERE CID.IO_DATETIME BETWEEN @FROM_DATE AND @TO_DATE
	AND CONVERT(VARCHAR(5),CID.IO_DATETIME,108) BETWEEN CM.From_Time AND CM.To_Time -- added on 05032019
	AND (CID.In_Out_flag='10' OR CID.IP_Address='Canteen' OR IP.Device_No >= 200) and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
	...
	--INSERT INTO #CANTEEN_ATT_CONS(EMP_ID,FOR_DATE,[STATUS],IP_ID)
	--SELECT EM.EMP_ID,cast(CID.CANTEEN_PUNCH_DATETIME as date) AS IO_DATETIME,SUM(CID.QUANTITY),IP.IP_ID FROM T0150_EMP_CANTEEN_PUNCH CID
	--INNER JOIN T0050_CANTEEN_MASTER CM ON CID.CANTEEN_ID=CM.CNT_ID 
	--INNER JOIN T0040_IP_MASTER IP ON CM.IP_ID=IP.IP_ID 
	--INNER JOIN T0080_EMP_MASTER EM ON EM.EMP_ID = CID.EMP_ID
	--INNER JOIN #EMP_CONS EC ON EM.EMP_ID = EC.EMP_ID 
	--WHERE CID.CANTEEN_PUNCH_DATETIME BETWEEN @FROM_DATE AND @TO_DATE
	--AND (CID.flag='Mobile' OR CID.Reason='Mobile' OR IP.Device_No >= 200) and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
	--AND EM.EMP_LEFT='N'
	--GROUP BY cast(CID.CANTEEN_PUNCH_DATETIME as date),EM.EMP_ID,IP.IP_ID
	----- END		
	INSERT INTO #CANTEEN_ATT_CONS(EMP_ID,CanteenName,FOR_DATE,[STATUS],IP_ID)
	SELECT EM.EMP_ID,Cnt_Name,cast(CID.CANTEEN_PUNCH_DATETIME as date) AS IO_DATETIME,SUM(CID.QUANTITY),IP.IP_ID FROM T0150_EMP_CANTEEN_PUNCH CID 
	INNER JOIN T0050_CANTEEN_MASTER CM ON CID.CANTEEN_ID=CM.CNT_ID 
	INNER JOIN T0040_IP_MASTER IP ON CM.IP_ID=IP.IP_ID 
	INNER JOIN T0080_EMP_MASTER EM ON EM.EMP_ID = CID.EMP_ID
	INNER JOIN #EMP_CONS EC ON EM.EMP_ID = EC.EMP_ID 
	...
	AND (CID.flag='Mobile' OR CID.Reason='Mobile' OR IP.Device_No >= 200) and IP.Is_Canteen =1 AND IP.CMP_ID=@CMP_ID
	AND EM.EMP_LEFT='N'
	GROUP BY cast(CID.CANTEEN_PUNCH_DATETIME as date),EM.EMP_ID,IP.IP_ID,Cnt_Name
	...
	--INTO	#CANTEEN
	--FROM	DBO.T0050_CANTEEN_MASTER M INNER JOIN T0050_CANTEEN_DETAIL D ON  M.CNT_ID=D.CNT_ID	AND M.CMP_ID=D.CMP_ID 
	--WHERE	D.EFFECTIVE_DATE <= @TO_DATE AND M.CMP_ID=@CMP_ID 

	SELECT	M.CMP_ID,GRD_ID,M.CNT_ID,IP_ID,
			AMOUNT,Q_I.TOTAL_AMOUNT,Q_I.SUBSIDY_AMOUNT
			,CAST(From_Time As DateTime) As From_Time
			, Cast(To_Time As DateTime) As To_Time
	INTO	#CANTEEN
	FROM	DBO.T0050_CANTEEN_MASTER M WITH (NOLOCK) 
			INNER JOIN  (SELECT  I.Cnt_Id, I.Effective_Date,i.Amount,Cmp_Id,i.grd_id,i.Subsidy_Amount,i.Total_Amount
								 FROM   T0050_CANTEEN_DETAIL I
								 INNER JOIN (SELECT Max(Effective_Date) AS For_Date,Max(Tran_Id) as Trans_Id , Cnt_Id ,grd_id
										  FROM   T0050_CANTEEN_DETAIL 
										  WHERE  Effective_Date <= @TO_DATE  AND cmp_id = @CMP_ID 
										  GROUP  BY Cnt_Id,grd_id) Qry 
										  ON I.Cnt_Id = Qry.Cnt_Id AND I.Effective_Date = Qry.For_Date and I.Tran_Id = Qry.Trans_Id 
										  and I.grd_id = Qry.grd_id
										  ) Q_I ON  M.CNT_ID=Q_I.CNT_ID	AND M.CMP_ID = Q_I.CMP_ID 
								
			  WHERE	Q_I.EFFECTIVE_DATE <= @TO_DATE AND M.CMP_ID=@CMP_ID 
	...
	0 AS TOTAL_AMOUNT
	INTO	#TEMP 
	FROM	DBO.T9999_DEVICE_INOUT_DETAIL I  
			INNER JOIN	T0040_IP_MASTER IP ON I.IP_ADDRESS=IP.IP_ADDRESS AND IP.CMP_ID=@CMP_ID
	...
	-- MOBILE CANTEEN
	SELECT	ROW_NUMBER() OVER(ORDER BY I.TRAN_ID) AS ROWID,I.CANTEEN_PUNCH_DATETIME AS IO_DATETIME
	,E.EMP_ID,T.CNT_ID,T.IP_ID
	,T.AMOUNT AS EMPLOYEE_CONTRIBUTION
	,(T.AMOUNT * ISNULL(I.QUANTITY,1)) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION
	,SUBSIDY_AMOUNT AS  SUBSIDY_BORN_BY_COMPANY,
			(ISNULL(I.QUANTITY,1) * ISNULL(SUBSIDY_AMOUNT,0)) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
			,0 AS TOTAL_AMOUNT,I.QUANTITY,I.FLAG
	INTO	#MOBILE_CANTEEN
	FROM	DBO.T0150_EMP_CANTEEN_PUNCH I  
			...
			WHERE		I.FLAG='Mobile' AND I.REASON='Mobile' AND I.CMP_ID=@CMP_ID	
				and I.Canteen_Punch_Datetime BETWEEN @FROM_DATE AND @TO_DATE
	...
	----Select Cnt_Id,SUM(Quantity),EMP_ID,IP_ID from #MOBILE_CANTEEN t
	----where EMP_ID = 41 --and Cnt_Id = 5
	----And IO_DATETIME between @FROM_DATE and @TO_DATE
	----group by Cnt_Id,EMP_ID,IP_ID
	--Select T.Cnt_Id,T.EMP_ID,T.IP_ID,0,T.Quantity
	--,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0) as EMPLOYEE_CONTRIBUTION
	--,ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0) as AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY 
	--from #MOBILE_CANTEEN T
	--where Cnt_Id = 7
	--		GROUP BY T.EMP_ID,T.IP_ID
	--		,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0)
	--		,ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)
	--		,T.Cnt_Id,T.Quantity 
	--		 order by EMP_ID
	-- select * from #MOBILE_CANTEEN where EMP_ID in (41) and Cnt_Id = 7
		--	Select T.EMP_ID ,T.IP_ID--,0,
	...
	-- MOBILE PUNCH
	INSERT INTO #CANTEEN_SUBSIDY(EMP_ID,IP,TOTAL,QUANTITY,EMPLOYEE_CONTRIBUTION,AMOUNT_OF_EMPLOYEES_CONTRIBUTION
	,SUBSIDY_BORN_BY_COMPANY,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,TOTAL_AMOUNT,CANTEEN_ID)
	SELECT	T.EMP_ID,T.IP_ID,0,SUM(T.QUANTITY)
			,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0)
			,SUM(ISNULL(T.AMOUNT_OF_EMPLOYEES_CONTRIBUTION ,0)), -- SUM(ISNULL(T.EMPLOYEE_CONTRIBUTION ,0)) COMMENTED ON 22022019
			ISNULL(T.SUBSIDY_BORN_BY_COMPANY,0)
			,SUM(ISNULL(T.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)),
			...
			FROM #MOBILE_CANTEEN T where --EMP_ID = 41 and
			 IO_DATETIME between @FROM_DATE and @TO_DATE
			GROUP BY T.EMP_ID,T.IP_ID
			,ISNULL(T.EMPLOYEE_CONTRIBUTION ,0)
			,ISNULL(T.SUBSIDY_BORN_BY_COMPANY,0)
			,T.Cnt_Id
		...
		Select ROW_NUMBER() OVER(ORDER BY EMP_ID) as Row_ID,* into #tempCANTEEN_SUBSIDY from #CANTEEN_SUBSIDY
		
		--select * from #CANTEEN_SUBSIDY

		--Start Exemption Logic @CanteenExemption
		DECLARE @CanteenExemption as NUMERIC(18,2) = 30 
		DECLARE @rowCount int = 1
		DECLARE @count int = 0
		Declare @EmpCont numeric(18,2) = 0
		SELECT @count = count(1) from #tempCANTEEN_SUBSIDY 
		WHILE  @rowCount <= @count
		BEGIN
			Declare @TempEmpCont numeric(18,2) = 0
			SELECT @TempEmpCont = EMPLOYEE_CONTRIBUTION from #tempCANTEEN_SUBSIDY  where Row_ID = @rowCount order by CANTEEN_ID
			print @TempEmpCont
			SET @EmpCont = @EmpCont + @TempEmpCont
			If @EmpCont <= @CanteenExemption
			BEGIN
				print @EmpCont
			END
			else
			BEGIN
					update CS set
					Cs.SUBSIDY_BORN_BY_COMPANY = 0,
					cs.EMPLOYEE_CONTRIBUTION = t.TOTAL_AMOUNT,
					CS.AMOUNT_OF_EMPLOYEES_CONTRIBUTION = (t.TOTAL_AMOUNT * t.QUANTITY),
					CS.AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY = 0
					from #CANTEEN_SUBSIDY CS inner join #tempCANTEEN_SUBSIDY t 
					on CS.CANTEEN_ID = t.CANTEEN_ID
		...
		IF OBJECT_ID('tempdb..#tempCANTEEN_SUBSIDY') IS NOT NULL    
				DROP TABLE #tempCANTEEN_SUBSIDY  

		--select * from #CANTEEN_SUBSIDY
	
	
	INSERT INTO #EMP_DATES (ROW_ID,DIET, EMP_ID,IP_ID)
SELECT ROW_NUMBER() OVER(PARTITION BY EMP_ID ORDER BY EMP_ID) AS ROW_ID,CNT_NAME, EMP_ID,IP_ID
FROM #CANTEEN_CONSOLIDATE1 GROUP BY EMP_ID,IP_ID,CNT_NAME
	...
	DECLARE @COLCAPTION_NAME1 NVARCHAR(MAX)
	SET @COLCAPTION_NAME1 = ''
	...
	WHILE (@TEMP_DATE  <= @TO_DATE)
		BEGIN
			...
			SET @UPDATE_TEMPLATE =	'UPDATE E SET [' + @COL_NAME + ']=A.STATUS
			    						FROM	#EMP_DATES E
												INNER JOIN #CANTEEN_ATT_CONS A ON E.EMP_ID=A.EMP_ID AND E.IP_ID=A.IP_ID and e.DIET = a.CanteenName--AND E.SHIFT_ID=A.SHIFT_ID
			...
			Set @COLCAPTION_NAME = @COLCAPTION_NAME + 'SUM(cast(' + @COL_NAME + ' as numeric)) ' + @COL_NAME  + ',' --- FOR COLUMN WISE TOTAL
				...
				Set @COLCAPTION_NAME1 = @COLCAPTION_NAME1 + '' + @COL_NAME  + ','  -- Add by deepal 03092020
		...
		INNER JOIN T0040_IP_MASTER B ON A.IP_ID = B.IP_ID
		INNER JOIN T0050_CANTEEN_MASTER CM on B.IP_ID = CM.IP_ID 
		...
		LEFT OUTER JOIN  (SELECT	I.Dept_ID,I.Emp_ID,I.Branch_ID,I.Segment_ID
								 FROM		dbo.T0095_INCREMENT AS i 
											INNER JOIN		(SELECT     MAX(I2.Increment_ID) AS Increment_ID, I2.Emp_ID
																FROM          dbo.T0095_INCREMENT AS I2 INNER JOIN
																						   (SELECT     MAX(Increment_Effective_Date) AS INCREMENT_EFFECTIVE_DATE, Emp_ID
																							 FROM          dbo.T0095_INCREMENT AS I3
		...
		--														GROUP BY I2.Emp_ID) AS I2 ON i.Emp_ID = I2.Emp_ID AND i.Increment_ID = I2.Increment_ID 
		--						) I ON A.Emp_ID = i.Emp_ID 
		--WHERE MC.CNT_ID IN ( SELECT CANTEEN_ID AS CNT_ID FROM T0150_EMP_CANTEEN_PUNCH 
		--					WHERE Flag='Mobile' and Reason='Mobile' )
		...
		--Change by deepal 03092020
		INSERT INTO #CANTEEN_CONSOLIDATE(EMP_ID,EMP_CODE,EMP_NAME,DEPT_ID,DEPARTMENT_NAME,BUSINESS_SEGMENT,DIET,IP,CANTEEN_ID)
		select distinct A.EMP_ID,A.EMP_CODE,A.EMP_FULL_NAME,I.DEPT_ID,A.DEPT_NAME,A.SEGMENT_NAME,B.CNT_NAME,B.IP_ID,B.Cnt_Id
		 from #CANTEEN_CONSOLIDATE1 A
			inner join T0050_CANTEEN_MASTER B on a.CNT_NAME=b.Cnt_Name 
			INNER JOIN T0040_IP_MASTER C ON A.IP_ID = C.IP_ID
			left JOIN #MOBILE_CANTEEN MC ON B.CNT_ID = MC.CNT_ID AND B.IP_ID = MC.IP_ID 
			LEFT OUTER JOIN  (SELECT	I.Dept_ID,I.Emp_ID,I.Branch_ID,I.Segment_ID
								 FROM		dbo.T0095_INCREMENT AS i 
											INNER JOIN		(SELECT     MAX(I2.Increment_ID) AS Increment_ID, I2.Emp_ID
																FROM          dbo.T0095_INCREMENT AS I2 INNER JOIN
		...
		WHERE MC.CNT_ID IN (SELECT CANTEEN_ID AS CNT_ID FROM T0150_EMP_CANTEEN_PUNCH WHERE Flag='Mobile' and Reason='Mobile' ) AND
		MC.CNT_ID NOT IN ( SELECT CANTEEN_ID FROM #CANTEEN_CONSOLIDATE ) and
		B.cmp_id = @CMP_ID
		--GROUP BY A.EMP_ID,A.EMP_CODE,A.EMP_FULL_NAME,I.DEPT_ID,A.DEPT_NAME,A.SEGMENT_NAME,B.IP_ID
		

		--SELECT CC.EMP_CODE,CC.EMP_NAME,CC.DEPARTMENT_NAME,CC.BUSINESS_SEGMENT,CC.DIET AS DIET,CC.IP
		--,CC.CANTEEN_ID,ED.* 
		--INTO #CANTEEN_FINAL_RECORDS
		--FROM #EMP_DATES ED INNER JOIN #CANTEEN_CONSOLIDATE CC 
		--ON ED.EMP_ID=CC.EMP_ID AND ED.IP_ID=CC.IP --and ED.IPADDRESS = CC.IP_ID
		----WHERE ED.EMP_ID IN (14868) 
		SELECT CC.EMP_CODE,CC.EMP_NAME,CC.DEPARTMENT_NAME,CC.BUSINESS_SEGMENT,CC.IP,CC.CANTEEN_ID,ED.* 
		INTO #CANTEEN_FINAL_RECORDS
		FROM #EMP_DATES ED INNER JOIN #CANTEEN_CONSOLIDATE CC ON ED.EMP_ID=CC.EMP_ID AND ED.IP_ID=CC.IP 
		and ed.DIET = cc.DIET 
		...
		--LEFT JOIN T0030_BRANCH_MASTER TBM ON TBM.BRANCH_ID = I.Branch_ID	-- ADDED ON 04032019		
		--ORDER BY EMP_CODE,DIET,IP_ID
		--Select Emp_code,diet,Total
		--FROM #CANTEEN_FINAL_RECORDS CFR 
		--inner JOIN #CANTEEN_SUBSIDY CS 
		--ON CFR.IP = CS.IP AND CFR.EMP_ID=CS.EMP_ID AND CFR.CANTEEN_ID = CS.CANTEEN_ID 
		--where EMP_CODE = 1615
		
		--Select * from #CANTEEN_SUBSIDY where EMP_ID = 41
		
		--Select EMP_CODE,TOTAL
		--FROM #CANTEEN_FINAL_RECORDS CFR 
		--inner JOIN #CANTEEN_SUBSIDY CS ON CFR.IP = CS.IP AND CFR.EMP_ID=CS.EMP_ID AND CFR.CANTEEN_ID = CS.CANTEEN_ID
		--where cs.EMP_ID = 41

		
		
		SELECT CFR.*,TOTAL,EMPLOYEE_CONTRIBUTION,
		AMOUNT_OF_EMPLOYEES_CONTRIBUTION,SUBSIDY_BORN_BY_COMPANY,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
		,QUANTITY,TOTAL_AMOUNT,TBM.BRANCH_NAME
		INTO #CANTEEN_REGISTER
		FROM #CANTEEN_FINAL_RECORDS CFR 
		inner JOIN #CANTEEN_SUBSIDY CS ON CFR.IP = CS.IP AND CFR.EMP_ID=CS.EMP_ID AND CFR.CANTEEN_ID = CS.CANTEEN_ID 
		LEFT OUTER JOIN  (SELECT	I.Dept_ID,I.Emp_ID,I.Branch_ID,I.Segment_ID -- ADDED ON 04032019
	    ...
	    ALTER TABLE #CANTEEN_REGISTER_FINAL DROP COLUMN EMP_ID,IP_ID--,SR_NO_TEMP
	    ...
	    IF(ISNULL(@TYPE_ID,0) = 1)
			BEGIN
				...
				--ORDER BY SRNO	
				SELECT	ROW_NUMBER() OVER(ORDER BY DIET) AS SRNO
				,DEPARTMENT_NAME,BRANCH_NAME,DIET
					,SUM(ISNULL(TOTAL,0)) AS QUANTITY
					,SUM(ISNULL(EMPLOYEE_CONTRIBUTION,0)) AS EMPLOYEE_CONTRIBUTION,
					SUM(ISNULL(AMOUNT_OF_EMPLOYEES_CONTRIBUTION,0)) AS AMOUNT_OF_EMPLOYEES_CONTRIBUTION,
					SUM(Cast(ISNULL(SUBSIDY_BORN_BY_COMPANY,0) as numeric(18,2))) AS SUBSIDY_BORN_BY_COMPANY,
					SUM(ISNULL(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,0)) AS AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY
					,SUM(ISNULL(TOTAL_AMOUNT,0)) AS TOTAL_AMOUNT
					,CANTEEN_ID
				INTO	#CANTEEN_SUMMARY
				FROM	#CANTEEN_REGISTER_FINAL
				WHERE	DIET <> 'TOTAL'
				GROUP BY DEPARTMENT_NAME,DIET,BRANCH_NAME,CANTEEN_ID
				...
				update #CANTEEN_SUMMARY set 
				SUBSIDY_BORN_BY_COMPANY = (Select sum(SUBSIDY_BORN_BY_COMPANY) from #CANTEEN_SUMMARY WHERE DIET <> 'GRAND TOTAL')
				,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY = (Select sum(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY) from #CANTEEN_SUMMARY WHERE DIET <> 'GRAND TOTAL')
				,TOTAL_AMOUNT = (Select sum(TOTAL_AMOUNT) from #CANTEEN_SUMMARY WHERE DIET <> 'GRAND TOTAL')
				WHERE	DIET = 'GRAND TOTAL'
				
				
				
				
				
				SELECT	SRNO,DEPARTMENT_NAME,BRANCH_NAME,DIET,QUANTITY,EMPLOYEE_CONTRIBUTION,AMOUNT_OF_EMPLOYEES_CONTRIBUTION,SUBSIDY_BORN_BY_COMPANY,
					AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,TOTAL_AMOUNT--,CANTEEN_ID,B.CANTEEN_IMAGE
				FROM	#CANTEEN_SUMMARY A
				LEFT JOIN T0050_CANTEEN_MASTER B ON A.CANTEEN_ID = B.CNT_ID
		...
		ELSE
			BEGIN
					...
					FROM #CANTEEN_REGISTER_FINAL CRF 
					INNER JOIN #DEPARTMENT_WISE_SUM DWS ON CRF.DEPARTMENT_NAME = DWS.DEPARTMENT_NAME
					WHERE CRF.EMP_CODE = 9999
					
				
					update #CANTEEN_REGISTER_FINAL set 
					EMPLOYEE_CONTRIBUTION  = (Select sum(EMPLOYEE_CONTRIBUTION) from #CANTEEN_REGISTER_FINAL  WHERE DEPARTMENT_NAME <> 'Z')
					,AMOUNT_OF_EMPLOYEES_CONTRIBUTION = (Select sum(AMOUNT_OF_EMPLOYEES_CONTRIBUTION) from #CANTEEN_REGISTER_FINAL  WHERE DEPARTMENT_NAME <> 'Z')					
					,SUBSIDY_BORN_BY_COMPANY = (Select sum(SUBSIDY_BORN_BY_COMPANY) from #CANTEEN_REGISTER_FINAL  WHERE DEPARTMENT_NAME <> 'Z')
					,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY = (Select sum(AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY) from #CANTEEN_REGISTER_FINAL WHERE DEPARTMENT_NAME <> 'Z')
					,TOTAL_AMOUNT = (Select sum(TOTAL_AMOUNT) from #CANTEEN_REGISTER_FINAL WHERE DEPARTMENT_NAME <> 'Z')
					...
					Quantity as TOTAL,EMPLOYEE_CONTRIBUTION,AMOUNT_OF_EMPLOYEES_CONTRIBUTION,SUBSIDY_BORN_BY_COMPANY,AMOUNT_OF_SUBSIDY_BORN_BY_COMPANY,TOTAL_AMOUNT
					FROM #CANTEEN_REGISTER_FINAL
					GROUP BY DEPARTMENT_NAME,EMP_CODE,EMP_NAME,BRANCH_NAME,BUSINESS_SEGMENT,DIET,Quantity,
