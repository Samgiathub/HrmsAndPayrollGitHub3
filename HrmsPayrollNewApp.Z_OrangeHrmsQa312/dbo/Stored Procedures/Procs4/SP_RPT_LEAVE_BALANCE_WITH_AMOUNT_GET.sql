...
CREATE PROCEDURE [dbo].[SP_RPT_LEAVE_BALANCE_WITH_AMOUNT_GET]
	 @Cmp_ID		Numeric
	,@To_Date		Datetime
	,@Branch_ID		Numeric 
	,@Cat_ID		Numeric
...
SET ARITHABORT ON
		...
		If isnull(@Leave_ID,0) = 0 
			begin
				...
				Insert into @Emp_Leave_Bal
					select @Cmp_ID,Emp_ID,0,'',0,0,0,0,@To_Date,0,0,Leave_ID,Lv_Encase_Calculation_Day from T0040_LEAVE_MASTER LM WITH (NOLOCK)
					Inner join	 @Emp_Cons EC on  EC.Cmp_ID = LM.Cmp_ID
					Where LM.Leave_Type = 'Encashable'  and LM.Apply_Hourly = 0
			end
		else
			begin
				Insert into @Emp_Leave_Bal
							...
							begin
										...
										if @Lv_Encash_W_Day > 0
											begin
													...
													Update @Emp_Leave_Bal set Leave_Amount = Case when @IS_ROUNDING =1 then round((calc_Amount/@Lv_Encash_W_Day)*Leave_Closing,0) Else 
														 round((calc_Amount/@Lv_Encash_W_Day)*Leave_Closing,2) end
											...
											begin
													...
													Update @Emp_Leave_Bal set Leave_Amount = Case when @IS_ROUNDING =1 then round((calc_Amount/@Working_Days )*Leave_Closing,0) else
														round((calc_Amount/@Working_Days )*Leave_Closing,2) end
													from @Emp_Leave_Bal  ELB
													Inner join 
													(
															select Emp_ID,Leave_ID from @Emp_Leave_Bal where Cmp_ID =@Cmp_ID
							...
							begin
								...
								if @Lv_Encash_W_Day > 0
											begin
												...
												Update @Emp_Leave_Bal set Leave_Amount = Case when @IS_ROUNDING =1 then round((calc_Amount/@Lv_Encash_W_Day)*Leave_Closing,0) else
												round((calc_Amount/@Lv_Encash_W_Day)*Leave_Closing,2) end from @Emp_Leave_Bal  ELB
												Inner join 
												(
											...
											begin
												Update @Emp_Leave_Bal set Leave_Amount = Case when @IS_ROUNDING =1 then round((calc_Amount/@Working_Days)*Leave_Closing,0) else
												round((calc_Amount/@Working_Days)*Leave_Closing,2) end from @Emp_Leave_Bal  ELB
												Inner join 
												(
														select Emp_ID,Leave_ID from @Emp_Leave_Bal where Cmp_ID =@Cmp_ID) Qry 
														on ELB.Emp_ID = Qry.Emp_ID and ELB.Leave_ID = Qry.Leave_ID
	...
	select el.*,Leave_Name,Emp_Full_Name,Emp_Code,Alpha_Emp_Code,Emp_First_Name,g.Grd_Name,b.BRanch_Address,b.Comp_name
		...
		,@To_Date P_To_Date,b.Branch_ID,vs.Vertical_ID,vs.Vertical_Name
		,Tm.Type_Name          --added jimit 26062015
	From @Emp_Leave_Bal el Inner Join T0040_LEAVE_MASTER as l WITH (NOLOCK) on el.Leave_ID = l.Leave_ID and l.Leave_Type = 'Encashable' inner join
	 T0080_EMP_MASTER e WITH (NOLOCK) on el.Emp_ID =e.Emp_ID inner join 
	 (select I.Emp_Id ,Grd_ID,Branch_ID,Dept_ID,Desig_ID,Type_ID,Vertical_ID from T0095_Increment I WITH (NOLOCK) inner join 
					( select max(Increment_ID) as Increment_ID , Emp_ID from T0095_Increment WITH (NOLOCK)
					where Increment_Effective_date <= @To_Date
					and Cmp_ID = @Cmp_ID
					group by emp_ID  ) Qry on
	 ...
	 T0040_TYPE_MASTER TM WITH (NOLOCK) on IQ.Type_ID = Tm.Type_ID   left outer join        --added jimit 26062015
	 T0040_Designation_Master dgm WITH (NOLOCK) on iq.desig_ID =dgm.Desig_ID inner join 
	 T0010_Company_master as CM WITH (NOLOCK) on e.cmp_ID = cm.Cmp_ID left Join 
