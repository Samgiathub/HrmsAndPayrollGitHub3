...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_RECORD_GET_Emp_List]                
  @Cmp_ID   varchar(max)        
 ,@From_Date  datetime                
 ,@To_Date  datetime                 
...
AS                
 ...
 begin     
  ...
  --   print  @company_Count --mansi  
  --print @Show_Left_Employee_for_Salary --mansi  
  ...
  begin    
    ...
    FROM T0040_SETTING S WITH (NOLOCK)      
     inner join #Cmp_Cons C on C.Cmp_ID=S.Cmp_ID    
           WHERE S.Cmp_ID in(C.Cmp_ID) AND Setting_Name = 'Hide Attendance For Fix Salary Employee'       
 end     
 else    
 begin     
  SELECT @Hide_Attendance_For_Fix_Salary = ISNULL(Setting_Value,0)                 
...
Begin      
 --- print 111  
 EXEC SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN_demo @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@constraint,0,@Salary_Cycle_id,@Segment_Id,@Vertical_Id,@SubVertical_Id,@SubBranch_Id,@New_Join_emp,@Left_Emp,0,'',0,0 
...
--  end                
 ...
 IF @Report_Type =''                
  BEGIN         
     --print @company_Count  
      -- print @branch_Id--mansi       
   IF @Type=''                
   BEGIN             
       if(@company_Count>=1 and @Cmp_ID<>cast(0 as varchar))    
    begin     
   -- print 1234  
     ...
     T0095_Increment.Emp_ID                 
      ...
      #Emp_Cons EC WITH (NOLOCK) ON T0095_Increment.Emp_ID = EC.Emp_ID  inner join    
      #Cmp_Cons CC with(NOLOCK) on T0095_Increment.Cmp_ID=CC.Cmp_ID    
      WHERE Increment_Effective_date <= @To_Date AND T0095_Increment.Cmp_ID in(CC.Cmp_ID)            
      GROUP BY T0095_Increment.emp_ID                  
     ) Qry on                
     I.Emp_ID = Qry.Emp_ID AND I.Increment_Effective_Date = Qry.Increment_Effective_Date)I_Q      
     ON E.Emp_ID = I_Q.Emp_ID --and E.Cmp_ID = #Cmp_Cons.Cmp_ID     
     INNER JOIN                
     ...
     #Cmp_Cons CC with(NOLOCK) on E.Cmp_ID=CC.Cmp_ID inner join    
    ...
    begin    
     ...
     I.Emp_ID = Qry.Emp_ID AND I.Increment_Effective_Date = Qry.Increment_Effective_Date  ) I_Q  ON E.Emp_ID = I_Q.Emp_ID INNER JOIN --and E.Cmp_ID = @Cmp_ID INNER JOIN                
     dbo.T0040_GRADE_MASTER GM WITH (NOLOCK) ON I_Q.Grd_ID = GM.Grd_ID LEFT OUTER JOIN                
     dbo.T0040_TYPE_MASTER ETM WITH (NOLOCK) ON I_Q.Type_ID = ETM.Type_ID LEFT OUTER JOIN                
     dbo.T0040_DESIGNATION_MASTER DGM WITH (NOLOCK) ON I_Q.Desig_Id = DGM.Desig_Id LEFT OUTER JOIN                
     dbo.T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) ON I_Q.Dept_Id = DM.Dept_Id INNER JOIN                 
     dbo.T0030_BRANCH_MASTER BM WITH (NOLOCK) ON I_Q.BRANCH_ID = BM.BRANCH_ID LEFT OUTER JOIN                
     dbo.T0050_SubBranch SBM WITH (NOLOCK) ON BM.BRANCH_ID = SBM.SubBranch_ID   left JOIN --And BM.Cmp_ID = SBM.Cmp_ID INNER JOIN                
     dbo.T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID INNER JOIN                 
     --#Emp_Cons EC WITH (NOLOCK) ON E.Emp_ID = EC.Emp_ID INNER JOIN                
     --T0040_SETTING S WITH (NOLOCK) ON E.Cmp_ID = S.Cmp_ID AND S.Setting_Name='Add initial in employee full name' INNER JOIN                 
      T0040_SETTING S WITH (NOLOCK) ON  S.Setting_Name='Add initial in employee full name' INNER JOIN      
    T0011_Login LO WITH (NOLOCK) ON LO.Emp_Id = E.Emp_Id LEFT OUTER JOIN                
     T0040_Reason_Master Rm WITH (NOLOCK) ON rm.Res_Id = l.Res_Id                 
     WHERE --E.Cmp_ID = @Cmp_Id  AND               
      E.Date_Of_Join <= ISNULL(e.Emp_Left_Date , @To_Date)                
     AND I_Q.Payment_Mode = (CASE WHEN (@Payment_Mode = '--Select--' or @Payment_Mode = '')                
     THEN I_Q.Payment_Mode                
     ELSE @Payment_Mode                
      END)                
  ...
  begin    
    ...
    ( SELECT max(T0095_Increment.Increment_Effective_Date) AS Increment_ID , T0095_Increment.Emp_ID                 
     FROM dbo.T0095_Increment WITH (NOLOCK) INNER JOIN                 
	   ...
	   inner join #Cmp_Cons CC with(NOLOCK) on T0095_Increment.Cmp_ID=CC.Cmp_ID  
     WHERE Increment_Effective_date <= @To_Date AND T0095_Increment.Cmp_ID in(CC.Cmp_ID)               
    ...
    dbo.T0040_TYPE_MASTER ETM WITH (NOLOCK) ON I_Q.Type_ID = ETM.Type_ID LEFT OUTER JOIN                
    dbo.T0040_DESIGNATION_MASTER DGM WITH (NOLOCK) ON I_Q.Desig_Id = DGM.Desig_Id LEFT OUTER JOIN                
    dbo.T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) ON I_Q.Dept_Id = DM.Dept_Id INNER JOIN                     dbo.T0030_BRANCH_MASTER BM WITH (NOLOCK) ON I_Q.BRANCH_ID = BM.BRANCH_ID  INNER JOIN   
    #Cmp_Cons CC with(NOLOCK) on E.Cmp_ID=CC.Cmp_ID inner join    
    dbo.T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID INNER JOIN                
    #Emp_Cons EC WITH (NOLOCK) ON E.Emp_ID = EC.Emp_ID INNER JOIN                
    ...
    INNER JOIN T0110_Uniform_Dispatch_Detail UDD WITH (NOLOCK) ON UDD.Emp_ID=EC.Emp_ID                 
     WHERE E.Cmp_ID in(cc.Cmp_ID) AND (UDD.Dispatch_Date >= @From_Date and UDD.Dispatch_Date <= @To_Date)                
 ...
 ELSE                     
  IF @Report_Type = 'ID Card' or @Report_Type='QR Code'                
   BEGIN                
   --   print 1
	  --select dbo.fn_getReportingManager (@Cmp_ID,0,@To_Date)as agrg  
	  print 250
 ...
 Branch_Name,E.Date_of_Join,E.Date_Of_Birth,e.Emp_Mark_Of_Identification,e.Gender,@From_Date as From_Date ,@To_Date as To_Date,Cmp_Name,Cmp_Address,                
	...
	--inner join #Cmp_Cons cc on cc.Cmp_ID=E.Cmp_ID
	LEFT OUTER JOIN dbo.T0100_Left_Emp l ON E.Emp_ID =  l.Emp_ID INNER JOIN                
	 ...
	 ( SELECT distinct I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,                
      Type_ID,Vertical_id,SubVertical_id,I.subBranch_ID,I.Segment_ID FROM dbo.T0095_Increment I WITH (NOLOCK) INNER JOIN                 
       (   SELECT max(Increment_Effective_Date) AS Increment_ID , Emp_ID 
    ...
    #Emp_Cons EC WITH (NOLOCK) on E.Emp_ID = EC.Emp_ID                
       ...
       -- where Cmp_ID = @Cmp_Id) ECD On Ecd.Emp_ID = E.Emp_ID and ecd.Cmp_ID = E.Cmp_ID                
         LEFT JOIN                
        T0020_STATE_MASTER S WITH (NOLOCK) ON S.Cmp_ID in(CC.Cmp_ID) and s.State_ID = Cm.State_ID LEFT OUTER JOIN                
        ...
        FROM T0090_EMP_EMERGENCY_CONTACT_DETAIL EMCD WITH (NOLOCK) INNER JOIN  
		    #Cmp_Cons CC WITH (NOLOCK) on CC.Cmp_ID=EMCD.Cmp_ID INNER JOIN 
         (SELECT Min(EMCD.Row_ID) AS Row_Id, EC.Emp_ID                
          FROM T0090_EMP_EMERGENCY_CONTACT_DETAIL EMCD WITH (NOLOCK) INNER JOIN                 
          #Emp_Cons EC WITH (NOLOCK) ON EMCD.Emp_ID = EC.Emp_ID GROUP BY EC.Emp_ID) Qry
		  ON EMCD.Row_ID=Qry.Row_Id And EMCD.Emp_ID=Qry.Emp_ID                
        WHERE EMCD.Cmp_ID in(CC.Cmp_ID) GROUP BY EMCD.Emp_ID,EMCD.Home_Mobile_No) Emergency_Contact ON E.Emp_ID= Emergency_Contact.Emp_ID                
	   ...
	   --inner join #Cmp_Cons CC1 on CC1.Cmp_ID =bs.Cmp_ID
	      LEFT OUTER JOIN dbo.fn_getReportingManager_for_more_then_One_Cmp(@Cmp_ID,0,@To_Date) Manager On EC.Emp_ID = Manager.Emp_ID 
