...
CREATE PROCEDURE [dbo].[SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN_demo]          
  @Cmp_ID			varchar(max)          
 ,@From_Date		datetime          
 ,@To_Date			datetime          
			...
			FROM	#Emp_Cons E						
					INNER JOIN (SELECT	I1.EMP_ID, I1.INCREMENT_ID, I1.BRANCH_ID
								FROM	T0095_INCREMENT I1 WITH (NOLOCK) INNER JOIN #Emp_Cons E1 ON I1.Emp_ID=E1.EMP_ID
								          inner join #Cmp_Cons CC1 with(NOLOCK) on CC1.Cmp_ID=I1.Cmp_ID 
										INNER JOIN (SELECT	MAX(I2.Increment_ID) AS Increment_ID, I2.Emp_ID
													FROM	T0095_INCREMENT I2 WITH (NOLOCK) 
													inner join #Cmp_Cons CC with(NOLOCK) on CC.Cmp_ID=I2.Cmp_ID 
