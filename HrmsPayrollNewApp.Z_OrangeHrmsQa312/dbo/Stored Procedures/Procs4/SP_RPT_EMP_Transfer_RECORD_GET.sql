...
-- Created by rohit For Create Transfer letter on 12082014
---13/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[SP_RPT_EMP_Transfer_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	--,@Branch_ID		numeric   = 0
	...
	--	end
				select I_Q.* ,E.Emp_Full_Name,E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,Left_Date,BM.Comp_Name,BM.Branch_Address,Left_Reason
					...
					WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
							(select Emp_ID from T0095_Increment WITH (NOLOCK) where Increment_Effective_Date >= @From_Date and Increment_Effective_Date <= @To_Date and  Increment_Type='Transfer') 
