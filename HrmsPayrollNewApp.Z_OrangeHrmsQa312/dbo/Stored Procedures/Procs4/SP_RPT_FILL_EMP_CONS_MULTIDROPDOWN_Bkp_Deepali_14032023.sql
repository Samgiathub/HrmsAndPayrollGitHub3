--USE [Orange_HRMS_01012023]
--GO
--/****** Object:  StoredProcedure [dbo].[SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN]    Script Date: 14/03/2024 12:20:38 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
---20/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
create PROCEDURE [dbo].[SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN_Bkp_Deepali_14032023]          
  @Cmp_ID			numeric          
 ,@From_Date		datetime          
 ,@To_Date			datetime          
 ...
 ,@Cat_ID			varchar(MAX)            
 ,@Grd_ID			varchar(MAX)        
 --,@Type_ID			numeric = 0 --commented the code 
 ,@Type_ID          varchar(Max) 
 ,@Dept_ID			varchar(MAX)            
 ...
 ,@With_Ctc			Numeric = 0
 ,@Type				numeric = 0
 ,@Bank_ID			varchar(MAX) = '0'	-- Added By Divyaraj Kiri on 25/09/2023
 ,@TDS_Flag  numeric=0 --added by Deepali
AS 
SET NOCOUNT ON 
...
SET ARITHABORT ON
	...
	set @TDS_Flag = @Show_Left_Employee_for_Salary
	...
	IF @Bank_ID = '' or @Bank_ID = '0'         -- Added by Divyaraj Kiri on 25/09/2023
	  SET @Bank_ID = null
  ...
  ---change by nilay : 26/112014
			...
			if @Bank_ID is not null		-- Added by Divyaraj Kiri on 25/09/2023
				Begin 
					Set @Where1 = @Where1 + ' and ISNULL(VE.Bank_ID,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(''' + Cast(@Bank_ID AS varchar(max)) + ''',ISNULL(VE.Bank_ID,0)),''#'') ) '
				End
	...
	IF @Constraint <> ''
		BEGIN
			...
			Where T.Data <> '' -- Added By Sajid 14-02-2024
	...
	ELSE IF @TYPE = 5 -- For Rotation Dashboard Summary Report ''Added by nilesh patel on 25022015
		Begin
			...
			Set @W_SQL ='INSERT INTO #Emp_Cons 	
			  ...
			  FROM dbo.V_Emp_Cons VE  inner join T0040_GENERAL_SETTING g WITH (NOLOCK) on VE.branch_id=g.branch_id 
		...
		ELSE IF @With_Ctc = 1		-- Use SP : SP_RPT_YEARLY_SALARY_GET
			BEGIN
					...
					*/
					...
					Set @W_SQL = 'INSERT INTO #Emp_Cons
					SELECT DISTINCT VE.emp_id,branch_id,VE.Increment_ID FROM V_Emp_Cons VE 
			...
			ELSE IF @Type = 1 -- Active Employee
				BEGIN
					...
					--print 10
					 ...
					 Set @W_SQL = 'INSERT INTO #Emp_Cons  
					 SELECT DISTINCT emp_id,branch_id,Increment_ID FROM V_Emp_Cons VE
							LEFT OUTER JOIN  (SELECT DISTINCT ESC.SalDate_id,ESC.emp_id as eid FROM T0095_Emp_Salary_Cycle ESC WITH (NOLOCK)
							INNER JOIN (SELECT max(Effective_date) as Effective_date,emp_id 
	...
	ELSE IF @With_Ctc = 2		-- Use SP : SP_RPT_SALART_SUMMARY ( With FNF Employees )( Note:- IS_FNF Column is Added in #EmpCons in this )
		...
		END
	ELSE IF @SalScyle_Flag = 5	-- Deepal add the logic to get the employee which are left DT:- 20102022 as per sajid bhai requirement for Kataria.
		BEGIN
			print 's'
	...
	ELSE 
		BEGIN
		--Added by Deepali - 14032024 start  for TDS -Show Left Employee for 6 months - Start
		print 't'
			If(@TDS_Flag= 1)
			begin
			...
			Set @W_SQL ='INSERT INTO #Emp_Cons 	
			  ...
			  WHERE VE.Cmp_id= ' + Cast(@Cmp_ID As Varchar(100)) + ' 
				   ...
				   AND Increment_Effective_Date <= ''' + Cast(@To_Date As Varchar(100)) + ''' 
				   AND ( 	
							( ''' + Cast(@To_Date As Varchar(100)) + '''  >= join_Date  AND ''' + Cast(@To_Date As Varchar(100)) + ''' <= DateAdd(M,6,Left_Date) )  						
							or (Left_date is null AND ''' + Cast(@To_Date As Varchar(100)) + ''' >= Join_Date)   
							or (''' + Cast(@To_Date As Varchar(100)) + ''' >  DateAdd(M,6,Left_Date)   AND  ''' + Cast(@From_Date As Varchar(100)) + ''' <=  DateAdd(M,6,Left_Date)  )
							or 1 = (CASE WHEN (( ' + cast(@Show_Left_Employee_for_Salary As Varchar(1)) + ' = 1) AND (''' + Cast(@To_Date As Varchar(100)) + ''' >=  DateAdd(M,6,Left_Date)   AND  ''' + Cast(@From_Date As Varchar(100)) + ''' <=  DateAdd(M,6,Left_Date)  )) then 1 else 0 end)      
						) 
			ORDER BY Emp_ID'

			end
			else
				--Added by Deepali - 14032024 start  for TDS -Show Left Employee for 6 months - end
			begin
