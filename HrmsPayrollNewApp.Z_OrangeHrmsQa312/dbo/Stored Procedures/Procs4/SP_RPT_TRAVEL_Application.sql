...
CREATE  PROCEDURE [dbo].[SP_RPT_TRAVEL_Application]
	 @Cmp_ID		Numeric
	,@From_Date		Datetime
	,@To_Date		Datetime
	,@Branch_ID		Numeric 
	...
	if @Constraint <> ''
		...
		Else
			Begin
				...
				,Dm.Desig_Name
				,TBM.BandName
					,bm.Branch_Name as 'Place of Posting'
					...
					from		T0080_EMP_MASTER EMP WITH (NOLOCK) inner join @Emp_cons EC on Emp.Emp_ID = EC.Emp_ID 
					inner join T0100_TRAVEL_APPLICATION TA WITH (NOLOCK) on TA.emp_id=EC.Emp_ID and TA.cmp_id=Emp.Cmp_ID
					left join T0110_TRAVEL_APPLICATION_DETAIL TAP WITH (NOLOCK) on TAP.Travel_App_ID=TA.Travel_Application_ID
					--left join T0130_TRAVEL_APPROVAL_DETAIL TC WITH (NOLOCK) on ta.Travel_Approval_ID =TC.Travel_Approval_ID
					left join T0030_CITY_MASTER lcm WITH (NOLOCK) on lcm.City_ID=TAP.City_ID
					LEFT OUTER JOIN (
									SELECT	EMP_ID, BRANCH_ID,Band_Id,Desig_Id FROM T0095_INCREMENT I WITH (NOLOCK)
									WHERE	Increment_Effective_Date=(SELECT	MAX(Increment_Effective_Date)
									FROM		T0095_INCREMENT I1 WITH (NOLOCK)
									WHERE		I1.Cmp_ID=I.Cmp_ID AND I1.Emp_ID=I.Emp_ID
									AND I1.Increment_Effective_Date<= @To_Date
					...
					inner join T0030_BRANCH_MASTER BM on Bm.Branch_ID=inc.Branch_ID
					left join tblBandMaster TBM on TBM.BandId=INC.Band_Id
					left join T0040_DESIGNATION_MASTER Dm on Dm.Desig_ID=inc.Desig_Id
					where		EMP.Cmp_ID=@Cmp_ID --and isnull(tc.City_ID,0) <> 0 
					and TA.Application_Date >=@From_Date and  TA.Application_Date <=@To_Date + 1
					...
					begin  
						...
						ELSE
							...
							Select TA.*,e.Emp_Full_name,e.Emp_Code,e.Alpha_Emp_Code,e.Emp_First_Name,GM.Grd_Name,Branch_Name
								  ,Dept_Name,DGM.Desig_Name,type_Name,Cmp_Name,Cmp_Address ,comp_name,Branch_address,@From_Date as From_Date,@To_Date as To_Date, BM.Branch_ID
								  ,TAP.From_Date as TAD_From_Date,TAP.To_Date as TAD_To_Date,TAP.Place_Of_Visit 
								  ,TAP.Instruct_Emp_ID as TAD_instruct_EMP_id
								  --,TAP.Leave_Approval_ID,TAP.Leave_ID
								  ,TAP.Period,TAP.Remarks as TAD_Remark,TAP.Travel_Mode_ID,TAP.Travel_Purpose,TAP.Travel_App_Detail_ID
								  ,IEM.Emp_Full_Name,TMM.Travel_Mode_Name--,LM.Leave_Name
								  ,TAP.Travel_App_ID,CM.cmp_logo,TA.Application_Status
								  ,isnull(SEM.Emp_Full_Name,'Admin') as Approved_Emp_Name ,isnull(SDM.Desig_Name,'Admin') as S_Desig_Name
								  ,ta.Tour_Agenda,Ta.IMP_Business_Appoint,TA.KRA_Tour,isnull(Ta.chk_Adv,0) as chk_Adv,isnull(ta.chk_Agenda,0) as chk_Agenda
								  ,SUPR.Emp_Full_Name As Emp_Superior_Name
								  ,SUPD.Desig_Name AS Emp_Superior_DesigName      
						 ...
						 inner join T0110_TRAVEL_APPLICATION_DETAIL TAP WITH (NOLOCK) ON TA.Travel_Application_ID=TAP.Travel_App_ID --and TAD.emp_id =TSA.emp_id
						 inner join @Emp_cons ec on TA.Emp_ID = ec.emp_ID 
						 --inner Join T0130_TRAVEL_APPROVAL_DETAIL TAD1 ON TSA.Travel_Approval_ID = TAD1.Travel_Approval_ID and TSA.Cmp_id =TAD1.Cmp_id
						 --left Join T0130_TRAVEL_APPROVAL_DETAIL TAD1 WITH (NOLOCK) ON TAP.Travel_Approval_ID = TAD1.Travel_Approval_ID and TSA.Cmp_id =TAD1.Cmp_id
						 --inner Join T0120_TRAVEL_APPROVAL TA ON TA.Travel_Approval_ID = TAD1.Travel_Approval_ID and TA.Cmp_id =TAD1.Cmp_id
						 --Left Join T0120_TRAVEL_APPROVAL TA WITH (NOLOCK) ON TA.Travel_Approval_ID = TAD1.Travel_Approval_ID and TA.Cmp_id =TAD1.Cmp_id
						 ...
						 inner join
									( select I.Emp_Id , Cmp_ID,Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Increment_effective_Date from T0095_Increment I WITH (NOLOCK) inner join 
										...
										on E.Emp_ID = I_Q.Emp_ID  inner join
											...
											T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) ON I_Q.Dept_Id = DM.Dept_Id Inner join 
											T0030_Branch_Master BM WITH (NOLOCK) on I_Q.Branch_ID = BM.Branch_ID  left join 
											T0080_EMP_MASTER IEM WITH (NOLOCK) on TAP.Instruct_Emp_ID =IEm.Emp_ID left join
											T0030_TRAVEL_MODE_MASTER TMM WITH (NOLOCK) on TAP.Travel_Mode_ID = TMM.Travel_Mode_ID  --left join
											--T0040_LEAVE_MASTER LM WITH (NOLOCK) on TA.Leave_ID = LM.Leave_ID 
											left join T0080_EMP_MASTER SEM WITH (NOLOCK) on TAP.Instruct_Emp_ID = SEM.Emp_ID
											left join T0040_DESIGNATION_MASTER SDM WITH (NOLOCK) on SEM.Desig_Id =SDM.Desig_ID  
											left join T0080_EMP_MASTER SUPR WITH (NOLOCK) on e.Emp_Superior = SUPR.Emp_ID
											left join T0040_DESIGNATION_MASTER SUPD WITH (NOLOCK) on SUPR.Desig_Id =SUPD.Desig_ID  
											left join t0020_State_Master SM WITH (NOLOCK) on SM.State_ID=TAP.State_ID
											left join T0030_CITY_MASTER CTM WITH (NOLOCK) on CTM.City_ID=TAP.City_ID
											left join T0001_LOCATION_MASTER LCM WITH (NOLOCK) on LCM.Loc_ID=TAP.Loc_ID
											--left join T0040_CURRENCY_MASTER CRM on CRM.Curr_ID=TSA.Cu
						...
						TA.Cmp_ID = @Cmp_ID 
						and TA.Application_Date >=@From_Date and  TA.Application_Date <=@To_Date + 1
						AND TA.Travel_Application_ID = @Travel_App_ID -- ADDED BY RAJPUT ON 09072019 CASE FROM KICH CLIENT
