...
CREATE PROCEDURE [dbo].[SP_TRAININIG_WISE_DETAILS]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric  
...
AS
	...
	if @Constraint <> ''
	    ...
	    select TD.*, E.Emp_Code,E.Emp_Full_Name as Emp_Full_Name,Emp_superior,Type_Name,Date_of_Join,Gender,BM.Comp_Name,BM.Branch_Address,CM.Cmp_Name,Cm.Cmp_address  
							from V0120_HRMS_TRAINING_APPROVAL TD  Left outer join 
						...
						(select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID from T0095_Increment I WITH (NOLOCK) inner join   
						  ...
						  WHERE TD.Cmp_ID = @Cmp_Id and isnull(training_id,0) = isnull(@Training_id ,isnull(training_id,0)) and Training_date >=@From_date and Training_Date <= @To_date 
