CREATE  PROCEDURE [dbo].[SP_RPT_TRAVEL_Settlement]  
  @Cmp_ID  Numeric  
 ,@From_Date  Datetime  
 ,@To_Date  Datetime  
 ,@Branch_ID  Numeric   
   ...
   Begin  
    ...
    select distinct EMP.Alpha_Emp_Code,EMP.Emp_ID,EMP.Emp_Full_Name  
     ...
     Select TAD.*,e.Emp_Full_name,e.Emp_Code,e.Alpha_Emp_Code,e.Emp_First_Name,GM.Grd_Name,Branch_Name  
      ...
      INNER JOIN T0040_Reason_Master RM WITH (NOLOCK) on RM.Res_Id = TAD1.Reason_ID   
      inner join  
        ( select I.Emp_Id , Cmp_ID,Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Increment_effective_Date from T0095_Increment I WITH (NOLOCK) inner join   
         ...
         on E.Emp_ID = I_Q.Emp_ID  inner join  
          ...
          left join t0020_State_Master FSM WITH (NOLOCK) on FSM.State_ID=TAD1.From_State_id  
          left join T0030_CITY_MASTER FCTM WITH (NOLOCK) on FCTM.City_ID=TAD1.From_City_id  
