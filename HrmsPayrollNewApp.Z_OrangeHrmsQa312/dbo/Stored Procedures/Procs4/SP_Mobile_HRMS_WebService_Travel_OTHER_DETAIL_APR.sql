...
CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_Travel_OTHER_DETAIL_APR]
		...
		BEGIN
			...
			(ROW_NUMBER() OVER(ORDER BY Table1.value('(Travel_App_Other_Detail_Id/text())[1]','NUMERIC(18,0)'))) AS Rownum,
				...
				INTO #ItemTemp FROM @Travel_Other_Details.nodes('/NewDataSet/TravelOtherDetails') as Temp(Table1)
				 ...
				 --select * FROM #ItemTemp WHERE Rownum = @COUNT				
					 ...
					 IF NOT EXISTS(SELECT Travel_Apr_Other_Detail_Id FROM T0115_TRAVEL_APPROVAL_OTHER_DETAIL_LEVEL WITH (NOLOCK) WHERE Cmp_ID=@Cmp_ID 
					    AND Travel_Mode_Id = @Travel_Mode and DESCRIPTION = @TDescription and Amount = @Amount and Tran_ID = @Tran_ID and Travel_Mode_ID = @Travel_Mode
					...
					--select * from T0115_TRAVEL_APPROVAL_OTHER_DETAIL_LEVEL where Travel_Apr_Other_Detail_Id = @Travel_Apr_Other_Detail_Id
