...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_CMP_TRANSFER_SALARYDETAIL]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric  = 0
...
SET ARITHABORT ON
	...
	Else 
		...
		FROM   V0100_EMP_EARN_DEDUCTION ED INNER JOIN
			   #Emp_Cons EC on ED.Emp_Id = EC.Emp_ID
		WHERE ED.Cmp_Id = @Cmp_Id	
				AND (CASE WHEN @SHOW_HIDDEN_ALLOWANCE = 0  AND  ED.HIDE_IN_REPORTS = 1 AND ED.AD_NOT_EFFECT_SALARY=1 THEN 0 ELSE 1 END )=1  --CHANGE BY JAINA 16-05-2017
		ORDER BY RIGHT(REPLICATE(N' ', 500) + ED.AD_NAME, 500) 
		...
		FROM   T0100_EMP_COMPANY_TRANSFER_EARN_DEDUCTION ED WITH (NOLOCK) LEFT OUTER JOIN
			   ...
			   #Emp_Cons EC on ED.Old_Emp_Id = EC.Emp_ID
		WHERE ED.Old_Cmp_Id = @Cmp_Id	
				AND (CASE WHEN @SHOW_HIDDEN_ALLOWANCE = 0  AND  NAM.HIDE_IN_REPORTS = 1 AND NAM.AD_NOT_EFFECT_SALARY=1 THEN 0 ELSE 1 END )=1  --CHANGE BY JAINA 16-05-2017
		ORDER BY RIGHT(REPLICATE(N' ', 500) + NAM.AD_NAME, 500) 
