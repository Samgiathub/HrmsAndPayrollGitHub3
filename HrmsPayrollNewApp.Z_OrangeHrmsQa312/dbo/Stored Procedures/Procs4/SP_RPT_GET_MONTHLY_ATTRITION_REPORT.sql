...
CREATE PROCEDURE [dbo].[SP_RPT_GET_MONTHLY_ATTRITION_REPORT]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
...
SET ARITHABORT ON
	...
	ELSE IF @SUMMARY = 9
        ...
        ON_ROLL_BUDGETED_STRENGTH AS "On Roll Budgeted Strength", 
        ON_ROLL_ACTUAL_STRENGTH AS "On Roll Actual Strength",
        OFF_ROLL_BUDGETED_STRENGTH AS "Off Roll Budgeted Strength",
        OFF_ROLL_ACTUAL_STRENGTH AS "Off Roll Actual Strength",
        (ON_ROLL_BUDGETED_STRENGTH + OFF_ROLL_BUDGETED_STRENGTH) AS "Total Budgeted Strength",
        (ON_ROLL_ACTUAL_STRENGTH + OFF_ROLL_ACTUAL_STRENGTH) AS "Total Actual Strength"
FROM 	#MAN_POWER_DETAILS_REPORT
	...
	ELSE IF @Report_Name = 2   ------For Man Power Age_Analysis
		BEGIN
			...
			SELECT Age_Band As AgeBand,Total FROM #Man_Power_Age_Analysis_Report
