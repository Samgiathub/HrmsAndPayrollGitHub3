...
CREATE PROCEDURE [dbo].[SP_RPT_Audit_EMP_INOUT_RECORD_GET_NEW] 
 @Cmp_ID   numeric,      
 @From_Date  datetime,      
 @To_Date  datetime ,      
 @Branch_ID  numeric   ,      
...
if  object_id('tempdb..#Emp_Inout') IS NOT NULL --exists (select 1 from [tempdb].dbo.sysobjects where name like '#Emp_Inout' )        
 ...
 ( select I.Emp_Id ,Type_ID ,Grd_ID,Dept_ID,Desig_Id,Isnull(Emp_Late_Limit,'00:00') as Emp_Late_Limit,
		...
		Branch_ID,I.Vertical_ID,I.SubVertical_ID,I.Customer_Audit
	 from dbo.T0095_INCREMENT I WITH (NOLOCK) inner join       
     ( select max(i.Increment_ID) as Increment_ID, i.Emp_ID 
	   from dbo.T0095_INCREMENT I WITH (NOLOCK) inner Join #Emp_Cons EC on i.Emp_ID = ec.Emp_ID	-- Ankit 08092014 for Same Date Increment    
		where Increment_effective_Date <= @To_Date      
			and Cmp_ID = @Cmp_ID     
		group by i.emp_ID  
  ...
  WHERE E.Cmp_ID = @Cmp_ID and Inc_Qry.Customer_Audit = 1     --Added By Jaina 08-09-2016  
 ...
 WHILE @@FETCH_STATUS = 0      
  BEGIN      
     ...
     begin      
						...
						Begin
							 ...
							 -- select Min(In_time),Max(Out_Time),Reason from dbo.T0150_AUDIT_EMP_INOUT_RECORD Where Emp_ID =@Emp_ID and For_Date = @Temp_Month_Date    
								...
								select Min(In_time),
									 ...
									 Case When Max_In > Max(Out_Time) Then Max_In Else Max(Out_time) End ,
									--Case When Max_In > Max(Out_Time) Then Max_In Else Max_In End 
									max(Reason)
      ...
      while @@fetch_Status = 0      
       begin      
		  ...
		  --select @Insert_In_Date,@Insert_Out_Date
          ...
          --Ankit Start 12112013
			...
			--exec dbo.GET_HalfDay_Date @Cmp_ID,@Emp_ID,@From_Date,@To_Date,0,@HalfDayDate output --added By Mukti 28112014 
						select @HalfDayDate = isnull(@HalfDayDate,'')			
   ...
   select @P_Days = isnull(Calculate_Days,0) from T0050_SHIFT_DETAIL WITH (NOLOCK) where Shift_ID =@shift_id and (@Working_sec/3600) > From_Hour and (@Working_sec/3600) <= To_Hour -- Added by rohit on 08082016   
     ...
     Begin  
				...
				Begin
					...
					(Emp_id,For_Date , Dept_id ,Grd_ID,Type_ID,Desig_ID ,Shift_ID,In_Time ,Out_Time ,Duration ,      
					 ...
					 (@emp_id ,@Temp_Month_Date ,@Dept_id ,@Grd_ID ,@Type_ID,@Desig_ID,@Shift_ID,Null,Null ,'-',      
					   0, '-' ,'-', '-', '-', '',@Shift_Sec,@shift_Dur,'-','-','-','-',0,0,0,0,0,@Shift_St_Time,@Shift_End_Time --Null,Null Commented and added by Sumit 18112015
					  ,0,0,'-','HO',0,@Emp_OT,@Monthly_Deficit_Adjust_OT_Hrs,@Late_Comm_sec,@Branch_Id_Cur,@Shift_Sec
					  ,@Vertical_Id,@SubVertical_Id,@P_Days)   --added jimit 15062016
					End
			Else
				Begin	
					...
					(Emp_id,For_Date , Dept_id ,Grd_ID,Type_ID,Desig_ID ,Shift_ID,In_Time ,Out_Time ,Duration ,      
					  ...
					  VALUES   
					 (@emp_id ,@Temp_Month_Date ,@Dept_id ,@Grd_ID ,@Type_ID,@Desig_ID,@Shift_ID,Null,Null ,'-',      
					   0, '-' ,'-', '-', '-', '',0,'-','-','-','-','-',0,0,0,0,0,Null,Null --Commented and added by Sumit 18112015
       ...
       begin
			...
			Begin
				...
				VALUES   
				(@emp_id ,@Temp_Month_Date ,@Dept_id ,@Grd_ID ,@Type_ID,@Desig_ID,@Shift_ID,Null,Null ,'-',      
				0, '-' ,'-', '-', '-', '',@Shift_Sec,@shift_Dur,'-','-','-','-',0,0,0,0,0,@Shift_St_Time,@Shift_End_Time--Null,Null 
				,0,0,'-','HHO',0,@Emp_OT,@Monthly_Deficit_Adjust_OT_Hrs,@Late_Comm_sec,@Branch_Id_Cur,@Shift_Sec 
				,@Vertical_Id,@SubVertical_Id,@P_Days)   --added jimit 15062016
			 End
			Else
				Begin
		...
		begin
			...
			IF charindex(cast(@Temp_Month_Date as varchar(11)),@StrCancelHoliday_Date,0) > 0 
				BEGIN
					...
					Update #Emp_Inout Set --AB_LEAVE = 'HO', 
						Total_More_work_Sec = Total_Work_Sec, More_Work = Total_work,Shift_Sec = 0 
					from #Emp_Inout EI inner join #Emp_Holiday EH  on EH.Emp_Id = EI.emp_id and EH.For_Date = EI.for_Date 
					 Where EI.emp_id = @Emp_ID And EI.for_Date = @Temp_Month_Date and EH.is_Half = 0
					  
					 Update #Emp_Inout Set --AB_LEAVE = 'HHO', 
						Total_More_work_Sec = cast(Total_Work_Sec as numeric(18,2)) - Datediff(s,Shift_St_Datetime,Shift_en_Datetime)/2, More_Work =  dbo.F_Return_Hours(cast(isnull(Total_Work_Sec,0) as numeric(18,2))- Datediff(s,Shift_St_Datetime,Shift_en_Datetime)/2),Shift_Sec = 0 
					from #Emp_Inout EI inner join #Emp_Holiday EH  on EH.Emp_Id = EI.emp_id and EH.For_Date = EI.for_Date 
					 Where EI.emp_id = @Emp_ID And EI.for_Date = @Temp_Month_Date and EH.Is_Half = 1
				END
			ELSE
		...
		If @Emp_OT = 1
			Begin 
				...
				IF CHARINDEX(CAST(@Temp_Month_Date AS VARCHAR(11)),@StrCancelWeekoff_Date,0) > 0 
					BEGIN
						UPDATE #Emp_Inout SET --AB_LEAVE = 'WO', 
							Total_More_work_Sec = Total_Work_Sec, More_Work = Total_work, Total_Less_work_Sec = 0,Less_Work = '',Shift_Sec = 0 
						Where emp_id = @Emp_ID And for_Date = @Temp_Month_Date   
					END
				ELSE
					BEGIN
   ...
   SUM(Total_Work_Sec) as Total_Work_Sec, SUM(Shift_Sec) as Shift_Sec,
    ...
    ,Case When Monthly_Deficit_Adjust_OT_Hrs = 1 then 
		Case When Sum(Total_Less_work_Sec) > SUM(Total_More_work_Sec) Then
			Case When COUNT(Case When Shift_Sec = 0 OR AB_LEAVE = 'WO' or AB_LEAVE = 'HO' OR AB_LEAVE = 'OHO' Then Null Else 1 End) * Isnull(Late_Comm_sec,0) > Sum(Total_Less_work_Sec) - SUM(Total_More_work_Sec) Then
					0
				Else (Sum(Total_Less_work_Sec) - SUM(Total_More_work_Sec)) - (COUNT(Case When Shift_Sec = 0 OR AB_LEAVE = 'WO' OR AB_LEAVE = 'OHO' or AB_LEAVE = 'HO' Then Null Else 1 End) * Isnull(Late_Comm_sec,0))
			End
		Else 0 End
	Else 
		SUM(Total_Less_work_Sec)
	   ...
	   Cast(Replace(dbo.F_Return_Hours(Total_Work_Sec_new - Required_Hrs_Till_date),':','.') As numeric(18,2)) as Total_Work_Hours,
		...
		,bm.Branch_Name   --added jimit 21072016
		   ...
		   Left Outer Join
				...
				Where cast(cast(For_Date as varchar(11)) as smalldatetime) >= cast(cast(@From_Date as varchar(11)) as smalldatetime)      
				and cast(cast(For_Date as varchar(11)) as smalldatetime) <= cast(cast(GETDATE()  as varchar(11)) as smalldatetime)       
				And (AB_LEAVE <> 'WO' AND AB_LEAVE <> 'HO' AND AB_LEAVE <> 'AB' and AB_LEAVE <> 'OHO' and AB_LEAVE not like '%Full day%') OR AB_LEAVE Is null)As Qry1 Group by Emp_id) As Qry4 
				on E_IO.emp_id = Qry4.emp_id 
			Left Outer Join
				(Select Emp_id, Isnull(SUM(Duration_sec),0) as Dur_Sec 
				From #Emp_Inout 
				Where (AB_LEAVE <> 'WO' AND AB_LEAVE <> 'HO' AND AB_LEAVE <> 'AB' and AB_LEAVE <> 'OHO' and  AB_LEAVE not like '%Full day%') OR AB_LEAVE Is null Group by Emp_id) Qry2 on E_IO.emp_id = Qry2.emp_id
			Left Outer Join
				(select Emp_id, Isnull(SUM(Shift_Sec),0) as Total_Work_Sec_new
				From #Emp_Inout 
				Where cast(cast(For_Date as varchar(11)) as smalldatetime) >= cast(cast(@From_Date as varchar(11)) as smalldatetime)      
