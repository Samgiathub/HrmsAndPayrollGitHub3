...
--Created by chetan 260916
---21/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[SP_RPT_FORMX_APPOINTMENT_LETTER]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   = 0
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
		...
		CONVERT(VARCHAR(10),isnull(E.Date_Of_Join,'01/01/1900'),103)as Date_Of_Join,E.Alpha_Emp_Code,BM.Comp_Name,
		(case when isnull(BM.Branch_Address,'') <> ''  then BM.Branch_Address else C.Cmp_Address End) Branch_Address,
		...
		ISNULL(te.Months,0) as Tot_Exp,Cmp_State_Name ,I_Q.Basic_Salary,I_Q.Gross_Salary ,
		C.Registration_No,SM.State_Name ,EED.E_AD_AMOUNT ,E.Gender ,
		(case when isnull(E.Emp_Second_Name,'') <> '' then E.Emp_First_Name+' '+E.Emp_Second_Name +' '+E.Emp_Last_Name Else E.Emp_First_Name+' ' +E.Emp_Last_Name END) Emp_Name
		,ELR.Reference_No,ELR.Issue_Date		
		from T0080_EMP_MASTER E WITH (NOLOCK) 
        inner join ( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Isnull(Basic_Salary,0) Basic_Salary,Isnull(Gross_Salary,0) Gross_Salary from T0095_Increment I WITH (NOLOCK) inner join 
						  ...
						  Left Join T0020_State_master SM WITH (NOLOCK) ON  C.Cmp_ID = SM.Cmp_ID  And C.State_ID = SM.State_ID
						  Left Join V0100_EMP_EARN_DEDUCTION EED ON EED.Cmp_Id = E.Cmp_ID and EED.AD_DEF_ID=11
