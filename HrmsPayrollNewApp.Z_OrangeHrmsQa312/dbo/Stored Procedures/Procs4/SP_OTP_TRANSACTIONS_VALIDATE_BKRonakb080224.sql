...
-- =============================================
Create PROCEDURE [dbo].[SP_OTP_TRANSACTIONS_VALIDATE_BKRonakb080224]
...
SET ARITHABORT ON
	...
	Begin
		...
		else if exists(Select 1 from (Select max(T.CreatedDate) as CreatedDate, Emp_ID  from T0011_OTP_TRANSACTIONS T WITH (NOLOCK) Where isnull(Emp_Id,0) = isnull(@Emp_Id,0) Group by Emp_id) Qry Inner join T0011_OTP_TRANSACTIONS VT WITH (NOLOCK) on VT.Emp_ID =Qry.Emp_ID And VT.CreatedDate = Qry.CreatedDate where VT.Otp_TypeID = 1 and VT.ExpiredDate <= DATEADD(minute, -15, @Curr_Date) )
		begin	
	...
	Begin
		...
		else if exists(Select 1 from (Select max(T.CreatedDate) as CreatedDate, Email  from T0011_OTP_TRANSACTIONS T WITH (NOLOCK) Where isnull(Email,'') = isnull(@Email,'') Group by Email) Qry Inner join T0011_OTP_TRANSACTIONS VT WITH (NOLOCK) on VT.Email =Qry.Email And VT.CreatedDate = Qry.CreatedDate where VT.Otp_TypeID = 1 and VT.ExpiredDate <= DATEADD(minute, -15, @Curr_Date) )
		begin	
				Set @Msg = 'Verification code expired.'
		end
		else
