...
CREATE PROCEDURE [dbo].[SP_Timesheet_Reminder_OTL]
	...
	WHILE @@FETCH_STATUS = 0                    
		BEGIN
			  ...
			  IF ISNULL(@PROFILE,'') = ''
			   IF ISNULL(@PROFILE,'') = ''
			  BEGIN
				SELECT @PROFILE = ISNULL(DB_MAIL_PROFILE_NAME,'') FROM T9999_REMINDER_MAIL_PROFILE WHERE  DB_Mail_Profile_Id = 11 --CMP_ID = @CMP_ID AND
			  END
