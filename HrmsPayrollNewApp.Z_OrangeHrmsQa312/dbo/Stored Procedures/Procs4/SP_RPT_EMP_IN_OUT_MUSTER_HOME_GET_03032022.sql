...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_IN_OUT_MUSTER_HOME_GET_03032022]  
  @Cmp_ID   numeric  
 ,@From_Date  datetime  
 ,@To_Date   datetime  
 ,@Branch_ID  numeric  
...
AS  
 ...
 IF @Report_For = 'EMP RECORD'  
  BEGIN  
   ...
   From #Emp_Cons EC INNER JOIN  T0080_EMP_MASTER E WITH (NOLOCK) ON EC.EMP_ID =E.EMP_ID  INNER JOIN   
    T0095_Increment Q_I WITH (NOLOCK) ON     
   E.EMP_ID = Q_I.EMP_ID AND E.Increment_ID = Q_I.Increment_ID  
   INNER JOIN T0040_GRADE_MASTER GM WITH (NOLOCK) ON Q_I.Grd_Id = gm.Grd_ID INNER JOIN   
 ...
 IF OBJECT_ID('tempdb..#TMP_EMP_0150_INOUT') IS NULL  
  BEGIN  
   ...
   --SELECT EIR.* FROM dbo.T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK) INNER JOIN #EMP_CONS EC ON EIR.EMP_ID=EC.EMP_ID  
   --WHERE FOR_DATE BETWEEN (@FROM_DATE - 7) AND (@To_Date + 7)  
   SELECT IO_Tran_Id,EIR.Emp_ID,Cmp_ID,For_Date
	--,case when isnull(In_Time,'') = '' then In_Date_Time else In_Time end as In_Time 
	--,case when Out_Time is null then Out_Date_Time else Out_Time end as Out_Time 
	,case when isnull(In_Time,'') = '' then In_Date_Time else In_Time  end as In_Time 
	,case when isnull(Out_Time,'') = '' then Out_Date_Time else Out_Time  end as Out_Time 
	...
	,Is_Cancel_Late_In,Is_Cancel_Early_Out,Is_Default_In,Is_Default_Out,Cmp_prp_in_flag,Cmp_prp_out_flag,is_Cmp_purpose,App_Date,Apr_Date,System_date
	,Other_Reason,ManualEntryFlag,StatusFlag,In_Admin_Time,Out_Admin_Time FROM dbo.T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)  INNER JOIN #EMP_CONS EC ON EIR.EMP_ID=EC.EMP_ID
	WHERE	FOR_DATE BETWEEN (@FROM_DATE - 7) AND (@To_Date + 7)
  END  
 ...
 -- Commented by Hardik 25/05/2020  
 ...
 -- Added by Hardik 25/05/2020  
 insert into #Att_Period  
 SELECT  TOP (DATEDIFF(DAY, @For_Date, @New_To_Date) + 1)  
   Date = DATEADD(DAY, ROW_NUMBER() OVER(ORDER BY a.object_id) - 1, @For_Date),  
   ROW_NUMBER( )OVER (ORDER BY a.Object_Id) AS ROW_ID  
 ...
 (           
  ...
  P_days  numeric(12,1) default 0, -- Added by Niraj (25012022)   
 ...
 (  
  ...
  Att_Approval_Days Numeric(18,2) default 0,  
  Att_App_ID Numeric(18,0) default 0,  
  Att_Apr_Status Char(1) default '',  
  Shift_Duration Varchar(10) default NULL,  
  OT_Apr_ID Numeric(18,0) default 0,  
  Comp_off_App Numeric(18,0) default 0,  
  Comp_off_Apr Numeric(18,0) default 0,  
 ...
 select  E.Emp_ID ,E.Cmp_ID,EC.branch_ID,For_Date,row_ID FROM #Att_Period cross join #Emp_Cons EC Inner Join T0080_EMP_MASTER E WITH (NOLOCK) ON EC.Emp_ID = E.Emp_ID  
 ...
 *********************************************************************/  
 ...
 BEGIN  
  DECLARE @CONSTRAINT_HW VARCHAR(MAX)  
  ...
  IF @CONSTRAINT = '' AND @Emp_ID > 0  
   SET @CONSTRAINT_HW = @Emp_ID  
  ELSE  
   SET @CONSTRAINT_HW = @CONSTRAINT  
    
 --Here Condition is Added By Ramiz on 14/04/2016 after discussion with Hardik Bhai  
  If @Report_For <> 'ABSENT_CON'  
   EXEC SP_GET_HW_ALL @CONSTRAINT=@CONSTRAINT_HW,@CMP_ID=@Cmp_ID, @FROM_DATE=@FROM_DATE, @TO_DATE=@TO_DATE, @All_Weekoff = 0, @Exec_Mode=0  
  ...
  BEGIN  
   ...
   FROM #ATT_MUSTER AM INNER JOIN  
    ...
    (  
     ...
     FROM T0040_GENERAL_SETTING Q1  WITH (NOLOCK) INNER JOIN  
      (  
      SELECT MAX(FOR_DATE)AS FOR_DATE,BRANCH_ID FROM T0040_GENERAL_SETTING  WITH (NOLOCK)  
 ...
 FROM T0040_SHIFT_MASTER SM WITH (NOLOCK) INNER JOIN #Att_Muster AM ON   
 ...
 from #Att_Muster AM inner join    --Changed By Gadriwala Muslim 01102014  
  ...
  (  
   ...
   (   
    ...
    (   
      SELECT LEAVE_ID FROM T0140_LEAVE_TRANSACTION AS LT2  WITH (NOLOCK)  
    ...
    LEFT OUTER JOIN T0040_LEAVE_MASTER AS EA WITH (NOLOCK) ON LT1.LEAVE_ID = EA.LEAVE_ID WHERE EA.CMP_ID  = @CMP_ID AND LT1.LEAVE_USED > 0   
...
(   
 ...
 set OD_count = (LT.Leave_Used) --Changed By Gadriwala Muslim 01102014  
  ...
  ,OD= LT.Leave_code  
  ,Main_Status='OD'  
  ,Detail_Status= 'OD-'+ cast(dbo.F_Lower_Round(LT.Leave_Used,Lt.Cmp_ID)as varchar) --substring(LT.Leave_code,2,LEN(LT.Leave_code)) +'-'+ cast(dbo.F_Lower_Round(LT.Leave_Used,Lt.Cmp_ID)as varchar)  
 FROM #ATT_MUSTER AM INNER JOIN  
  (  
  SELECT LT1.CMP_ID, LT1.LEAVE_USED,LT1.EMP_ID,  
  LT1.FOR_DATE,EA.LEAVE_CODE,EA.LEAVE_TYPE FROM T0140_LEAVE_TRANSACTION LT1  WITH (NOLOCK)  
  INNER JOIN #EMP_CONS EC ON LT1.EMP_ID = EC.EMP_ID   
  LEFT OUTER JOIN  T0040_LEAVE_MASTER EA WITH (NOLOCK) ON LT1.LEAVE_ID=EA.LEAVE_ID WHERE EA.LEAVE_TYPE='Company Purpose'   
 ...
 SET OD_COUNT = (LT.COMPOFF_USED -IsNull(LT.LEAVE_ENCASH_DAYS,0)) --CHANGED BY GADRIWALA MUSLIM 01102014  
  ...
  ,OD= LT.LEAVE_CODE  
  ,MAIN_STATUS='OD'  
  ,DETAIL_STATUS='OD-'+ CAST(DBO.F_LOWER_ROUND(LT.COMPOFF_USED,LT.CMP_ID)AS VARCHAR) --SUBSTRING(LT.LEAVE_CODE,2,LEN(LT.LEAVE_CODE)) +'-'+ CAST(DBO.F_LOWER_ROUND(LT.COMPOFF_USED,LT.CMP_ID)AS VARCHAR)  
 FROM #ATT_MUSTER AM INNER JOIN   
 (  
  SELECT LT1.CMP_ID, LT1.COMPOFF_USED,LT1.LEAVE_ENCASH_DAYS,LT1.EMP_ID,  
  LT1.FOR_DATE,EA.LEAVE_CODE,EA.LEAVE_TYPE FROM T0140_LEAVE_TRANSACTION LT1 WITH (NOLOCK)   
  INNER JOIN #EMP_CONS EC ON LT1.EMP_ID = EC.EMP_ID   
  LEFT OUTER JOIN T0040_LEAVE_MASTER EA WITH (NOLOCK) ON LT1.LEAVE_ID=EA.LEAVE_ID   
 ...
 --added by satish for the leave-status first and second half 10-12-2020 for vivo-guj mobile application  
 UPDATE #ATT_MUSTER  
 SET DETAIL_STATUS = DETAIL_STATUS + '-' + LE.Leave_Assign_As  
 FROM #ATT_MUSTER AM INNER JOIN   
 (  
    SELECT EM.Emp_ID,LD.From_Date,LD.To_Date,LD.Half_Leave_Date,LD.Leave_Assign_As  
    FROM #EMP_CONS EM  
    INNER JOIN T0120_LEAVE_APPROVAL LA  WITH (NOLOCK) on LA.Emp_ID=Em.Emp_ID   
    INNER JOIN T0130_LEAVE_APPROVAL_DETAIL LD  WITH (NOLOCK) on LD.Leave_Approval_ID = LA.Leave_Approval_ID  
    where LA.Approval_Status='A' and LD.Half_Leave_Date <> ''  
     and LD.From_Date >= @FROM_DATE and LD.To_Date  <= @MAIN_TO_DATE   
 ) LE on AM.EMP_ID = LE.EMP_ID AND AM.FOR_DATE = LE.Half_Leave_Date   
 AND AM.FOR_DATE >=@FROM_DATE AND AM.FOR_DATE <=@MAIN_TO_DATE  
 ...
 (  
   ...
   From T0160_Attendance_Application WITH (NOLOCK)  
   WHERE For_Date >=@From_Date AND For_Date <=@To_Date  
 )q on Am.Emp_ID =q.Emp_ID  AND am.for_date = Q.For_Date  
 ...
 (  
   ...
   From T0165_Attendance_Approval WITH (NOLOCK)  
 ...
 (  
  Select Compoff_App_ID,Emp_ID,Extra_Work_Date   
   From T0100_CompOff_Application CA WITH (NOLOCK)  
  Where Cmp_ID = @Cmp_ID AND Extra_Work_Date >= @From_Date AND Extra_Work_Date <= @To_Date AND Application_Status = 'P'   
  and Not Exists(Select 1 FROM T0120_CompOff_Approval CCA WITH (NOLOCK) WHERE CCA.CompOff_App_ID = CA.Compoff_App_ID)  
 ...
 FROM #Att_Muster A   
   ...
   LEFT OUTER JOIN T0040_SHIFT_MASTER SM WITH (NOLOCK) ON D.Shift_ID=SM.Shift_ID AND SM.Cmp_ID=@Cmp_ID  
 ...
 FROM  #Att_Muster AM INNER JOIN   
    ...
    T0040_SHIFT_MASTER S WITH (NOLOCK) ON D.Shift_ID = S.Shift_ID  
...
-- Added by rohit on 28012013  
 ...
 -- Late Minute condition added by Hardik 21/12/2015 for 12 AM Shift  
  ...
  Early_minute = case when DateDiff(mi,Out_Date,cast(cast((case when AM.sh_in_time > AM.sh_out_time then DateAdd(d,1,AM.for_date) else AM.for_date end) as varchar(11)) + ' ' + AM.sh_out_time as datetime))<0 then   
        0   
         when DateDiff(mi,Out_Date,cast(cast((case when AM.sh_in_time > AM.sh_out_time then DateAdd(d,1,AM.for_date) else AM.for_date end) as varchar(11)) + ' ' + AM.sh_out_time as datetime)) >  LTRIM(DATEDIFF(MINUTE, 0, Emp_Early_Limit)) then --Added ByJimit 21052019   
 ...
 FROM #ATT_MUSTER EL  
  INNER JOIN  
   (  
   SELECT LA.LEAVE_APPROVAL_ID,LA.EMP_ID,LAD.HALF_LEAVE_DATE,LEAVE_ASSIGN_AS FROM T0120_LEAVE_APPROVAL LA WITH (NOLOCK)   
 ...
 from #Att_Muster EL  
  inner join (select la.Leave_Approval_ID,la.Emp_ID,lad.From_Date FROM T0120_LEAVE_APPROVAL la  WITH (NOLOCK)  
    INNER JOIN #Emp_Cons E ON LA.Emp_ID=E.Emp_ID  
    inner join T0130_LEAVE_APPROVAL_DETAIL lad WITH (NOLOCK) on la.Leave_Approval_ID = lad.Leave_Approval_ID  
    inner join T0040_LEAVE_MASTER LM  WITH (NOLOCK) on lad.Leave_ID=lm.Leave_ID AND lad.Cmp_ID=lm.Cmp_ID  
...
if @Report_for = 'IN-OUT'  
 BEGIN  
  ...
  if exists(select 1 FROM #Data WHERE Emp_ID = @Emp_ID AND For_Date = Convert(datetime,Convert(Varchar(8),Getdate(),112),106) AND P_days = 0 AND IN_TIME IS NOT NULL and Chk_By_Superior = 1)  
   BEGIN  
    update #Data   
     Set P_days = 1  
...
-- Start Added by Niraj (18012022)
--Update #data Set P_days = 0 where Chk_By_Superior = 1
Update #Att_Muster Set Main_Status = 'A' where Chk_By_Superior = 'Pending'  and (Is_Cancel_Late_In = 0 and Is_Cancel_Early_Out = 0)
--OR isnull(Chk_By_Superior,'') = '' ) and isnull(in_date,'') <> '' and isnull(Out_Date,'') <> ''
-- End Added by Niraj (18012022)
  ...
  --Added by Mukti Reporting Manager of employee(start)04092017  
   ...
   FROM T0090_EMP_REPORTING_DETAIL esd  WITH (NOLOCK) INNER JOIN    
     (SELECT MAX(Effect_Date) AS For_Date,ESD.Emp_ID FROM T0090_EMP_REPORTING_DETAIL ESD WITH (NOLOCK)  
...
--Added by mehul for not displaying default shift in and out time 29102021
Update #Att_Muster 
set Status = case when sh_in_time = CAST(In_Date as time) then NULL Else In_Date end,
Status_2 = case when sh_out_time = CAST(Out_Date as time) then NULL Else Out_Date end
where Status = sh_in_time and Status_2 = sh_out_time
 ...
 IF @CONSTRAINT = ''   
  ...
  ELSE  
   BEGIN  
    ...
    IF @Report_For = 'Att Reg Report'  
     begin  
      ...
      From #Att_Muster AM   
        ...
        LEFT JOIN T0080_EMP_MASTER E1  WITH (NOLOCK) on AM.R_Emp_ID = E1.Emp_Id --- Changed by Hardik and above line commented by Hardik as Duplicate Records coming Redmine Bug id : 1263  
	...
	END
	 ...
	 -- AM.*,  
		am.Emp_Id, AM.Cmp_ID, AM.branch_ID, AM.For_Date, case when isnull(AM.Status,'''')  = '''' then convert(char(5),AM.In_Date,108) else AM.Status end as Status
		, AM.Leave_code, AM.Leave_Count, AM.OD, AM.OD_Count, AM.WO_HO
		--, AM.Status_2
		,case when isnull(AM.Status_2,'''')  = '''' then convert(char(5),AM.Out_Date,108) else AM.Status_2 end as Status_2
		, AM.Row_ID, AM.In_Date, AM.Out_Date
		, AM.shift_id, AM.shift_name, AM.sh_in_time, AM.sh_out_time, AM.holiday, AM.late_limit, AM.Reason, AM.Half_Full_Day, AM.Chk_By_Superior, AM.Sup_Comment
		, AM.Is_Cancel_Late_In, AM.Is_Cancel_Early_Out, AM.Early_Limit, AM.Main_Status, AM.Detail_Status, AM.Is_Late_Calc_On_HO_WO, AM.Is_Early_Calc_On_HO_WO
		, AM.late_minute, AM.Early_minute, AM.Is_Leave_App, AM.Other_Reason, AM.R_Emp_ID, AM.Att_Approval_Days, AM.Att_App_ID, AM.Att_Apr_Status, AM.Shift_Duration, AM.OT_Apr_ID
		,AM.Comp_off_App, AM.Comp_off_Apr, AM.OT_Applicable, AM.Display_Birth, AM.Display_Marriage_Date, AM.Date_of_join, AM.Left_date
      ,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + '' '' + AM.late_limit as datetime))late_time   
     ,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + '' '' + AM.Early_Limit as datetime))early_time 
       ,AM.late_minute as late_minutes  
       ,AM.Early_minute as early_out  
       ...
       ,Emp_Late_mark,Emp_Early_mark  
       ,(CASE WHEN IsNull(AM.Reason,'''') <> '''' THEN '''' ELSE ''disabled'' END) AS Disable_Comment,Q_I.Grd_ID--,AM.R_Emp_ID     
       ,DBO.F_RETURN_HOURS (Case When  ' + Cast(@Tras_Week_OT as Varchar(5)) + ' = 1 AND WO_HO = ''W'' THEN Weekoff_OT_Sec When  ' + Cast(@Tras_Week_OT as Varchar(5)) + ' = 1 AND  WO_HO = ''HO'' THEN Holiday_OT_Sec ELSE DURATION_IN_SEC END) AS WORKING_HOUR  --,IsNull(d.Duration_in_sec,0) As Working_Sec  
	 ...
	 into ##temp
      ...
      INNER JOIN ( SELECT I.Branch_ID,I.Grd_ID,I.Dept_ID,I.Desig_ID,I.Emp_ID,IsNull(I.Emp_Late_mark,0) Emp_Late_mark,IsNull(I.Emp_Early_mark,0) Emp_Early_mark   
       ...
       T0040_DEPARTMENT_MASTER DM  WITH (NOLOCK) ON Q_I.DEPT_ID = DM.DEPT_ID LEFT OUTER JOIN   
       T0040_DESIGNATION_MASTER DGM  WITH (NOLOCK) ON Q_I.DESIG_ID = DGM.DESIG_ID   
       LEFT OUTER JOIN T0050_SHIFT_DETAIL TSD  WITH (NOLOCK) ON TSD.Shift_ID = AM.Shift_ID and TSD.Calculate_Days = 1  
       LEFT OUTER JOIN #Data d ON AM.Emp_ID=d.Emp_Id AND AM.For_Date=d.For_Date ' + @filter + '   
      Order by Emp_Code,Am.For_Date'  
		...
		-- Start Added by Niraj(18012022)
		update ##temp set
		[Status] = '00:00', [Status_2] = '00:00', WORKING_HOUR = 0, p_days = 0
		where Chk_By_Superior = 'Pending' and Main_Status = 'A' and P_days <> '0.5'
		-- End Added by Niraj(18012022)

		select Emp_Id ,Cmp_ID ,branch_ID ,For_Date ,Status ,Leave_code ,Leave_Count ,OD ,OD_Count ,WO_HO ,Status_2 
		,Row_ID ,In_Date ,Out_Date ,shift_id ,shift_name ,sh_in_time ,sh_out_time ,holiday ,late_limit ,Reason ,Half_Full_Day ,Chk_By_Superior ,Sup_Comment ,Is_Cancel_Late_In ,Is_Cancel_Early_Out ,Early_Limit ,Main_Status ,Detail_Status ,Is_Late_Calc_On_HO_WO ,Is_Early_Calc_On_HO_WO ,late_minute ,Early_minute ,Is_Leave_App ,Other_Reason ,Att_Approval_Days ,Att_App_ID ,Att_Apr_Status ,Shift_Duration ,OT_Apr_ID ,Comp_off_App ,Comp_off_Apr ,OT_Applicable ,Display_Birth ,Display_Marriage_Date ,Date_of_join ,Left_date ,late_time ,early_time ,late_minutes ,early_out ,Alpha_Emp_Code ,Emp_full_Name ,Branch_Name ,Dept_Name ,Grd_Name ,Desig_Name ,Branch_Address ,Comp_Name ,DBRD_Code ,P_days ,Emp_Late_mark ,Emp_Early_mark ,Disable_Comment ,Grd_ID ,R_Emp_ID ,WORKING_HOUR ,Working_Sec ,Req_For_App ,Slab_Shift_Hours ,Week_Off_OT
		from ##temp
     END  
   END       
  ...
  Select AM.*,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + ' ' + AM.late_limit as datetime))late_time   
   ...
   --,case when DateDiff(mi,Out_Date,cast(cast((case when AM.sh_in_time > AM.sh_out_time then DateAdd(d,1,AM.for_date) else AM.for_date end) as varchar(11)) + ' ' + AM.sh_out_time as datetime))<0 then 0 else DateDiff(mi,Out_Date,cast(cast((case when AM.sh
_in_time > AM.sh_out_time then DateAdd(d,1,AM.for_date) else AM.for_date end) as varchar(11)) + ' ' + AM.sh_out_time as datetime)) end early_out  --Alpesh 04-Apr-2012 for night shift  
    ,AM.late_minute as late_minutes  
   ,AM.Early_minute as early_out   
   -- Ended by rohit on 18102013  
  ...
  */  
  ...
  BEGIN  
   ...
   Select
   --AM.*,  --Add by Deepal isnull issue resolved in the Lubi 11/11/2021 (#19323)
   AM.Emp_Id, AM.Cmp_ID, AM.branch_ID, AM.For_Date, isnull(AM.Status,'') as [Status], AM.Leave_code
					, isnull(AM.Leave_Count,0) as Leave_Count, AM.OD, isnull(AM.OD_Count,0) as OD_Count
					, isnull(AM.WO_HO,'') as WO_HO, isnull(AM.Status_2,0) as Status_2, AM.Row_ID, AM.In_Date 
					, AM.Out_Date, AM.shift_id
					, AM.shift_name, AM.sh_in_time, AM.sh_out_time, isnull(AM.holiday,0) as holiday,
      DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + ' ' + AM.late_limit as datetime))late_time   
     ,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + ' ' + AM.Early_Limit as datetime))early_time   
     ,isnull(AM.late_minute,0) as late_minutes  
     ,AM.Early_minute as early_out  
 ...
 ELSE IF @Report_For = 'BulkRegularization_Mobile'  
  ...
  Select IsNull(AM.Emp_Id,0) As 'Emp_Id',IsNull(E.Cmp_ID,0) As 'Cmp_ID',IsNull(AM.branch_ID,0) AS 'branch_ID',IsNull(AM.For_Date,0) AS 'For_Date',  
     ...
     IsNull(AM.Detail_Status,'') As Detail_Status,IsNull(AM.Is_Late_Calc_On_HO_WO,0) As Is_Late_Calc_On_HO_WO,IsNull(AM.Is_Early_Calc_On_HO_WO,0) As Is_Early_Calc_On_HO_WO,  
     IsNull(AM.late_minute,0) As late_minute,IsNull(AM.Early_minute,0) As Early_minute,IsNull(AM.Is_Leave_App,0) As Is_Leave_App,IsNull(AM.Other_Reason,'') As Other_Reason,  
     IsNull(E.Date_of_join,0) As Date_of_join,IsNull(E.Emp_Left_Date,0) As EMP_Left_date,  -- modified by Niraj(28102021)
     DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + ' ' + AM.late_limit as datetime))late_time   
     ,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + ' ' + AM.Early_Limit as datetime))early_time   
     ,IsNull(AM.late_minute,0) as 'late_minutes'  
     ,IsNull(AM.Early_minute,0) as 'early_out'  
     ...
     LEFT OUTER JOIN T0040_DESIGNATION_MASTER DGM  WITH (NOLOCK) ON Q_I.DESIG_ID = DGM.DESIG_ID   
     LEFT OUTER JOIN #Data d ON AM.Emp_ID=d.Emp_Id AND AM.For_Date=d.For_Date   
     Left outer join #tmpMobileInOut MIO on MIO.Emp_ID = am.Emp_Id  
     --and cast(MIO.IO_Datetime as Date) = cast(am.For_Date as Date)   
 ...
 ELSE IF @constraint ='R'  
  ...
  /* COMMENTED BY GADRIWALA MUSLIM 07102016 - INCREMENT ID USED DIRECTLY FROM #EMP_CONS INNER JOIN ( SELECT I.Branch_ID,I.Grd_ID,I.Dept_ID,I.Desig_ID,I.Emp_ID,IsNull(I.Emp_Late_mark,0) Emp_Late_mark,IsNull(I.Emp_Early_mark,0) Emp_Early_mark FROM T0095_Inc
rement I inner join   
      --( select max(Increment_effective_Date) as For_Date , Emp_ID FROM T0095_Increment  
      --where Increment_Effective_date <= @To_Date  
      --and Cmp_ID = @Cmp_ID  
   ...
   T0040_GRADE_MASTER GM  WITH (NOLOCK) ON INC.GRD_ID = GM.GRD_ID INNER JOIN   
   T0030_BRANCH_MASTER BM  WITH (NOLOCK) ON INC.BRANCH_ID = BM.BRANCH_ID LEFT OUTER JOIN  
   T0040_DEPARTMENT_MASTER DM  WITH (NOLOCK) ON INC.DEPT_ID = DM.DEPT_ID LEFT OUTER JOIN   
   T0040_DESIGNATION_MASTER DGM  WITH (NOLOCK) ON INC.DESIG_ID = DGM.DESIG_ID LEFT OUTER JOIN   
   #Data d ON AM.Emp_ID=d.Emp_Id AND AM.For_Date=d.For_Date   
 ...
 Else IF @Report_For = 'Att Reg Report'  
  ...
  INNER JOIN #EMP_CONS EC ON AM.EMP_ID = EC.EMP_ID  
  INNER JOIN T0095_INCREMENT INC  WITH (NOLOCK) ON INC.INCREMENT_ID = EC.INCREMENT_ID  
  /* COMMENTED BY GADRIWALA MUSLIM 07102016 - - INCREMENT ID USED DIRECTLY FROM #EMP_CONS  INNER JOIN ( SELECT I.BRANCH_ID,I.GRD_ID,I.DEPT_ID,I.DESIG_ID,I.EMP_ID,IsNull(I.EMP_LATE_MARK,0) EMP_LATE_MARK,IsNull(I.EMP_EARLY_MARK,0) EMP_EARLY_MARK FROM T0095_
INCREMENT I INNER JOIN   
      ( SELECT MAX(INCREMENT_EFFECTIVE_DATE) AS FOR_DATE , EMP_ID FROM T0095_INCREMENT  
      WHERE INCREMENT_EFFECTIVE_DATE <= @TO_DATE  
      AND CMP_ID = @CMP_ID  
      GROUP BY EMP_ID    
 ...
 insert into #P_Day(Total,emp_id)  
  ...
  (  
   ...
   WHERE (MAIN_STATUS='L' OR LEAVE_COUNT > 0 ) AND LEAVE_CODE <> 'LWP' AND FOR_DATE<=@TO_DATE GROUP BY EMP_ID  
   )AM -- GADRIWALA 04022014 CHANGE GETDATE() TO @TO_DATE  
  ON PD.EMP_ID=AM.EMP_ID  
