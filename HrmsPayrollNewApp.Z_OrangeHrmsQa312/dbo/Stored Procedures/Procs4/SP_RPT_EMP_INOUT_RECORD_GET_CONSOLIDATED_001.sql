...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET_CONSOLIDATED_001]      
    @Cmp_ID         NUMERIC,      
    @From_Date      DATETIME,      
    @To_Date        DATETIME ,      
    @Branch_ID      NUMERIC   ,      
...
-- Insert INTO #Emp_Inout(emp_id,for_Date,A_days)
--Insert INTO #Emp_Inout(emp_id,for_Date,A_days)
--Select  Emp_ID,CurrentDate,1 From (
--select Emp_ID,CurrentDate from  #Emp_Cons Cross join #tblMonthDay
--) as E 
--Where NOT EXISTS (SELECT 1 from #Emp_Inout as I Where  I.emp_id = E.Emp_ID AND I.for_Date = E.CurrentDate)
...
Where NOT EXISTS (SELECT 1 from  #Data as I Where  I.emp_id = E.Emp_ID AND I.for_Date = E.CurrentDate)
	...
	Shift_en_Datetime,More_Work,Total_More_work_Sec,A_days
	,Late_In_Sec
	,Early_Out_sec
	,Total_Work_Sec
	,Shift_Sec
	...
	Shift_End_Time, cast(dbo.F_Return_Hours(isnull(OT_Sec,0)+  isnull(Weekoff_OT_Sec,0) + isnull(Holiday_OT_Sec,0))AS Varchar)
    ...
    ---select * from #Emp_Inout  ,AB_LEAVE
    ...
    --Update #Emp_Inout SET Total_Less_Work_Sec=(Shift_Sec-Total_Work_Sec)*-1
...
-----Late_In_Sec=5000,Early_Out_sec=10000
----datediff(s,@Insert_In_Date,@Shift_St_Datetime)
--select *,datediff(s,OUT_Time,Shift_End_Time),
--CASE WHEN datediff(s,OUT_Time,Shift_End_Time)>= 0 THEN  datediff(s,OUT_Time,Shift_End_Time) ELSE 0 End
--from #Data
-----select *,datediff(s,@Insert_In_Date,@Shift_St_Datetime) from  #Emp_Inout
 ...
 /* -----13-Nov-2019----  
  ...
  ----------- */
   -- select * from #Data
    ...
    Update E SET AB_LEAVE = 'HF' FROM #Data D inner join #Emp_Inout E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days=0.5 And A_Days=0.5 --And E.Emp_Id = @Emp_Id
    ...
    Update E SET AB_LEAVE = 'QD', A_days = 1 - (IsNull(D.P_days,0) + ISNULL(Leave_Days,0)) FROM #Data D inner join #Emp_Inout E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days=0.25
    Update E SET AB_LEAVE = '3QD', A_days = 1 - (IsNull(D.P_days,0) + ISNULL(Leave_Days,0)) FROM #Data D inner join #Emp_Inout E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days=0.75
    --Update #Emp_Inout SET A_days = (P_days - D. FROM #Data D inner join #Emp_Inout E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE d.P_days<>0 And AB_LEAVE Is null --And E.Emp_Id = @Emp_Id
    ...
    IF not @query Is null       
        Begin
            ...
            SET @query= 'Insert INTO #Final_Table SELECT * FROM ( SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols1 + ',P_Days,A_Days,Weekoff,Holiday,' + @colsPivot_leave + ' FROM #consolidated_Temp C Left Outer Join #Leave_Count_Pivot L On C.Emp_Id = L.Emp_Id and C.OrderColumns = L.Id
                Where PvtColumns = ''In_Time'' or PvtColumns = ''Out_Time''
                Union
                SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols + ',P_Days,A_Days,Weekoff,Holiday,' + @colsPivot_leave + 'from #consolidated_Temp C Left Outer Join #Leave_Count_Pivot L On C.Emp_Id = L.Emp_Id and C.OrderColumns = L.Id
    ...
    Else
        Begin
            ...
            SET @query= 'Insert INTO #Final_Table SELECT * FROM (SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols1 + ',P_Days,A_Days,Weekoff,Holiday FROM #consolidated_Temp C 
                Where PvtColumns = ''In_Time'' or PvtColumns = ''Out_Time''
                Union
                SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols + ',P_Days,A_Days,Weekoff,Holiday FROM #consolidated_Temp C 
                Where PvtColumns = ''Duration'' or PvtColumns = ''WOHO'' or PvtColumns = ''Shift_Time'' or PvtColumns = ''OverTime'' or PvtColumns = ''Late_Hrs'')p '
        End
    ...
    SET @query = 'SELECT Case When OrderColumns = 1 Then Row_No Else NULL End AS Sr_No,
                ...
                (SELECT Row_Number() OVER(Order By ' + @Order_By + ') AS Row_No, T.emp_id AS empid FROM 
                    (SELECT Emp_ID,Desig_ID FROM #Emp_Inout Group By Emp_ID,Desig_ID) T INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON T.Emp_ID=E.Emp_ID left outer join T0040_Designation_Master DD WITH (NOLOCK) on DD.Desig_Id = T.Desig_ID) RID on RID.empid=Q.Emp_ID
