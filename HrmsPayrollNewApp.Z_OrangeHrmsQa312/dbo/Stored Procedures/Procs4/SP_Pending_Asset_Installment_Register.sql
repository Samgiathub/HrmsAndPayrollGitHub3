...
CREATE PROCEDURE [dbo].[SP_Pending_Asset_Installment_Register]
	 @cmp_Id numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	,@Branch_ID 	numeric
...
Declare @Emp_Cons Table
	...
	if @Constraint <> ''
        ...
        SELECT  distinct E.Alpha_Emp_Code,E.Emp_Full_Name,E.Emp_ID,e.Mobile_No,
        DM.Dept_Name,DGM.Desig_Name,am.asset_name,Aad.Asset_Code,Ap.Asset_Approval_Date,Aad.Deduction_Type,ISNULL(At.Issue_amount,0) as Purchase_Amount, 
		Aad.Installment_Amount,sum(ISNULL(At.Receive_Amount,0)) as Paid_Amount,
		(ISNULL(At.Issue_Amount,0)-SUM(ISNULL(At.Receive_Amount,0))) as Pending_Amount
		--,at.emp_id,ad.assetm_id
		...
		ad.assetm_id,Aad.Deduction_Type,Aad.Asset_Code,Aad.Installment_Amount,DGM.Desig_Name,DM.Dept_Name,Ap.Asset_Approval_Date,E.Emp_ID,e.Mobile_No
