...
-- =============================================
CREATE PROCEDURE [dbo].[T_Check_Home_Listing_Detail_HRMS]
	@Cmp_ID numeric(18,0),  
...
BEGIN
	...
	--	BEGIN
			...
			--(Team)team assessment
				IF EXISTS(
					...
					BEGIN
						SELECT '1P', CASE WHEN isnull(Form_url,'SelfAppraisal_Form.aspx')<>'' THEN Form_url  ELSE 'SelfAppraisal_Form.aspx' END Form_url
			...
			--(self)
				...
				BEGIN				
					...
					Set @tstore=''
					SELECT @tstore='3P',@init_Id =InitiateId--,Cmp_ID,Emp_Id,AppraiserId,SA_Startdate,SA_Enddate 
					...
					WHERE Emp_Id = @emp_id AND   
						  SA_Startdate <= CONVERT(varchar(10),GETDATE(),120) and t.show = 1 --and ( SA_Enddate >= CONVERT(varchar(10),GETDATE(),120) and SA_Status<>1) and SA_SendToRM <> 1
							print @init_Id	
					SELECT @tstore, CASE WHEN isnull(Form_url,'SelfAppraisal_Form.aspx')<>'' THEN Form_url +'?Initid=' + cast(@init_Id AS VARCHAR) ELSE 'SelfAppraisal_Form.aspx?Initid=' + cast(@init_Id AS VARCHAR) END Form_url
					FROM T0000_DEFAULT_FORM WITH (NOLOCK)
					WHERE Form_Name='TD_Home_ESS_294'		
				END
			ELSE
			...
			Set @tstore=''
				...
				WHEN @KPA_Default=1 THEN 'Ess_EmpAssessment.aspx 5P' ELSE 'Ess_PerformanceAssessment.aspx 5P' END Form_url
			...
			SELECT @initcount AS InitiateId ,CASE WHEN isnull(Form_url,'Ess_AppraisalFinalization.aspx')<>'' 
			THEN Form_url + '6P'  ELSE 'Ess_AppraisalFinalization.aspx ' + '6P' END Form_url
			FROM T0000_DEFAULT_FORM WITH (NOLOCK)
			...
			SELECT @initcount AS InitiateId ,CASE WHEN isnull(Form_url,'Ess_ApprisalHODApproval.aspx')<>'' 
			THEN Form_url  + '7P' ELSE 'Ess_ApprisalHODApproval.aspx ' + '7P' END Form_url
			FROM T0000_DEFAULT_FORM WITH (NOLOCK)
	...
	--	BEGIN  
			...
			IF @msg  = 1
				BEGIN
					SELECT  '8P'+ 'Start the ' + case when @KPI_AlertType =1 then 'Interim' else 'Final' end + ' Appraisal process' as response,
			...
			If @KPIPMS_Status =1
				BEGIN
					SELECT '9P'+'Approve your KPI rating' as response,'Ess_KPI_PMS_AppraisalForm.aspx' as Url
				END
			ELSE
				BEGIN
					SELECT '0' +'10P' as response
				END
				
			SELECT '11P',Form_url FROM T0000_DEFAULT_FORM WITH (NOLOCK)
			WHERE (Form_Name='ESS Employee Goal Setting' or Form_Name ='KPI Objectives') and Page_Flag='EP' and Is_Active_For_menu = 1
			...
			IF @msg = 1
				SELECT '13P', 'Prepare Objectives for this Financial year' as response
			ELSE
				SELECT '14P','0' as response
			-----------------------------------------------------

			------------Apprisal_SupNotify-----------------------
			SELECT '16sP' as p ,COUNT(KPIPMS_Status) AS cnt  
			...
			IF @int_pref = 0
				BEGIN	
					SELECT '16Pd' as p ,COUNT(KPIPMS_Status)  as cnt 
					FROM  T0080_KPIPMS_EVAL k WITH (NOLOCK) left join 
						  T0080_EMP_MASTER as e WITH (NOLOCK) on e.Emp_ID=k.Emp_ID 
			...
			SELECT '17P',COUNT(KPIPMS_Status)  as cnt 
			FROM  T0080_KPIPMS_EVAL k WITH (NOLOCK) left join 
				  T0080_EMP_MASTER as e WITH (NOLOCK) on e.Emp_ID=k.Emp_ID 
			WHERE k.cmp_id=@cmp_Id and KPIPMS_Status=5  and e.Emp_Superior=@emp_id
				
	
			SELECT '18P',COUNT(EmpKPI_Id)EmpKPI_Id--,k.Emp_Id,k.Cmp_Id,Status,FinancialYr,emp_full_name,cat_name,dept_name,desig_name 
			...
			and k.FinancialYr=datepart(year,getdate()) and Status=2 and e.Emp_Superior=@emp_Id
					...
					SELECT '19P',COUNT(EmpKPI_Id)EmpKPI_Id--,k.Emp_Id,k.Cmp_Id,Status,FinancialYr,emp_full_name,cat_name,dept_name,desig_name 
					FROM T0080_EmpKPI AS k WITH (NOLOCK) left join 
					t0080_emp_master AS e WITH (NOLOCK) ON e.emp_id=k.emp_id left join 
					t0095_increment i WITH (NOLOCK) ON i.emp_id=e.emp_id INNER join 
					(
			...
			SELECT '20P',COUNT(EmpKPI_Id)EmpKPI_Id--,k.Emp_Id,k.Cmp_Id,Status,FinancialYr,emp_full_name,cat_name,dept_name,desig_name 
			FROM T0080_EmpKPI AS k WITH (NOLOCK) left join 
			t0080_emp_master AS e WITH (NOLOCK) ON e.emp_id=k.emp_id left join 
			t0095_increment i WITH (NOLOCK) ON i.emp_id=e.emp_id INNER join 
			(
			...
			--END TRY
			--BEGIN CATCH
			--SELECT COUNT(1)apprcnt FROM #FinTable_1
			--END catch
