Create PROCEDURE [dbo].[SP_RPT_EMP_CANTEEN_MANAGMENT_diamines] 
...
END	
	...
	-- CANTEEN PUNCH
	--select * from #CANTEEN_CONSOLIDATE1
   ...
   -- Select * from #CANTEEN_REGISTER_FINAL
    ...
    IF(ISNULL(@FORMAT,0) = 0)
		BEGIN
			...
			SELECT	ROW_NUMBER() OVER(ORDER BY DIET) AS SRNO,DIET,SUM(ISNULL(TOTAL,0)) AS QUANTITY,SUM(ISNULL(EMPLOYEE_CONTRIBUTION,0)) AS EMPLOYEE_CONTRIBUTION,
	...
	ELSE
		BEGIN
				...
				SET @QUERYF = 'SELECT ROW_NUMBER() OVER (PARTITION BY EMP_NAME ORDER BY EMP_NAME) RowNumber,(EMP_CODE + '' - '' + EMP_NAME) as Emp_Full_Name,Diet,Distributor_Name,Total as Total_Quantity,' + SUBSTRING(@COLCAPTION_NAME1,0,LEN(@COLCAPTION_NAME1)) + '
