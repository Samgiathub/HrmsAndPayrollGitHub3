...
CREATE PROCEDURE [dbo].[SP_Rpt_P0160_OT_Apprvoval]
	 @Cmp_ID 			numeric
	,@From_Date			datetime
	,@To_Date 			datetime 
	,@Branch_ID			numeric
...
AS
	...
	if @Constraint <> ''
		...
		begin
		 ...
		 Select Emp_ID,For_Date,Working_Sec,OT_Sec,Approved_OT_Sec from T0160_OT_Approval WITH (NOLOCK) where cmp_ID=@Cmp_ID 
		 and Emp_ID=@Emp_ID
		 ...
		 Insert into #Data(Emp_ID,For_Date,Working_Sec,OT_Sec,Approve_OT_Sec)
				...
				Update #Data
					set Working_SEc = EIR.Working_Sec,
						OT_Sec =EIR.OT_Sec,
						Approve_OT_Sec =EIR.Approve_OT_Sec
				From #Data  EIR inner join T0160_OT_Approval OA on EIR.For_Date = OA.For_Date 
				Where Cmp_ID=@Cmp_ID and EIR.Emp_ID=@Emp_ID
							
	
				
			Select Sum(Working_Sec) as Working_SEc,Sum(Approve_OT_Sec) as Approve_OT_Sec,Sum(OT_Sec) as OT_Sec
