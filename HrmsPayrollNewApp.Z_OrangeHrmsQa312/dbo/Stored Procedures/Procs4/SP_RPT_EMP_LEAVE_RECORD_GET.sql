...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_LEAVE_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	--,@Branch_ID		numeric   = 0
	...
	,@Left_Emp		Numeric = 0
	,@Status        VARCHAR(10)='SELECT'
	,@Vertical_Id   varchar(max)=''   --Added By Jaina 1-10-2015 
	,@SubVertical_Id varchar(max)=''  --Added By Jaina 1-10-2015
	,@Report_Type	Varchar(20)	= 'Leave Approval'  --Added By Jimit 21112017
	,@Segment_Id    varchar(max) = ''    --Added By Jimit 13122017
	,@SubBranch_Id  varchar(max) = ''    --Added By Jimit 13122017
...
AS	
    ...
    exec SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,
											@Dept_ID,@Desig_ID,@Emp_ID,@constraint,0,0,@Segment_Id,
											@Vertical_Id,@SubVertical_Id,@SubBranch_Id,@New_Join_emp,@Left_Emp,0,'0',0,0   --Change Bu Jaina 1-10-2015
				...
				If @REport_Type = 'Leave Approval'
					BEGIN
						IF  (@Status <> 'ALL' )  
							begin
								...
								WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
										(select Emp_ID from T0120_Leave_Approval LA WITH (NOLOCK) inner join T0130_Leave_Approval_Detail LAD WITH (NOLOCK) on LA.leave_approval_id=LAD.leave_approval_id   where LAD.TO_DATE <= @TO_DATE AND LAD.FROM_DATE >= @FROM_DATE AND LA.APPROVAL_STATUS=@Status) 
								...
								from T0080_EMP_MASTER E WITH (NOLOCK) left outer join T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
									( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID from T0095_Increment I WITH (NOLOCK) inner join 
										...
										on E.Emp_ID = I_Q.Emp_ID  inner join
											...
											T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID
								WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
										(select Emp_ID from T0120_Leave_Approval LA WITH (NOLOCK) inner join T0130_Leave_Approval_Detail LAD WITH (NOLOCK) on LA.leave_approval_id=LAD.leave_approval_id   where LAD.TO_DATE <= @TO_DATE AND LAD.FROM_DATE >= @FROM_DATE) 
										And E.Emp_ID in (select Emp_ID From #Emp_Cons)
								Order by Case When IsNumeric(e.Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + e.Alpha_Emp_Code, 20)
						When IsNumeric(e.Alpha_Emp_Code) = 0 then Left(e.Alpha_Emp_Code + Replicate('',21), 20)
							Else e.Alpha_Emp_Code
						End
							...
							END
					END
				ELse IF @REport_Type = 'Leave Application'  --added By Jimit 21112017
					BEGIN	
						IF  (@Status <> 'ALL')  
							begin
								...
								WHERE E.Cmp_ID = @Cmp_Id And 
										(
											E.emp_ID in (
															...
															where	((@FROM_DATE BETWEEN LAD.from_date and LAD.To_Date) OR (@To_date BETWEEN LAD.from_date and LAD.To_Date) OR
																	 (LAD.from_date BETWEEN @FROM_DATE and @To_date) OR (LAD.To_Date BETWEEN @FROM_DATE and @To_date))
																	 and LAD.Cmp_ID = @Cmp_Id and LA.Approval_Status = @Status
														) 										
											OR
											E.emp_ID in 
														(
															select	Emp_ID 
															from	T0110_LEAVE_APPLICATION_DETAIL LAD WITH (NOLOCK) INNER JOIN
																	T0100_LEAVE_APPLICATION LA WITH (NOLOCK) ON La.Leave_Application_ID = Lad.Leave_Application_ID and La.Cmp_ID = Lad.Cmp_ID
															where	((@FROM_DATE BETWEEN LAD.from_date and LAD.To_Date) OR (@To_date BETWEEN LAD.from_date and LAD.To_Date) OR
																	 (LAD.from_date BETWEEN @FROM_DATE and @To_date) OR (LAD.To_Date BETWEEN @FROM_DATE and @To_date))
																	 and LAD.Cmp_ID = @Cmp_Id and LA.Application_Status = @Status
														 ) 
										)
										And E.Emp_ID in (select Emp_ID From #Emp_Cons)
								Order by Case When IsNumeric(e.Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + e.Alpha_Emp_Code, 20)
