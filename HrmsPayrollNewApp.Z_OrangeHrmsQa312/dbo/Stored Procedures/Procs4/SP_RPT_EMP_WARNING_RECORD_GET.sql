...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_WARNING_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	...
	,@Vertical_Id  varchar(Max)=''  --Added By Jaina 1-10-2015 For Privilege
	,@SubVertical_Id varchar(Max)='' --Added By Jaina 1-10-2015 For Privilege
	,@SubBranch_Id varchar(max)='' --added ronakb150724
	,@Bank_ID varchar(max)=''
...
SET ARITHABORT ON
	...
	exec SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@constraint,0,0,'',@Vertical_Id,@SubVertical_Id,@SubBranch_Id,@New_Join_emp,@Left_Emp,0,'0',0,0,@Bank_ID   --Change By Jaina 1-10-2015
	...
	--	end */
				...
				select I_Q.* ,E.Emp_Full_Name , E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,Left_Date,BM.Comp_Name,BM.Branch_Address,Left_Reason
					...
					WHERE E.Cmp_ID = @Cmp_Id	 And E.emp_ID in 
							(select Emp_ID from T0100_Warning_Detail WITH (NOLOCK) where Cmp_ID=@Cmp_ID) 
