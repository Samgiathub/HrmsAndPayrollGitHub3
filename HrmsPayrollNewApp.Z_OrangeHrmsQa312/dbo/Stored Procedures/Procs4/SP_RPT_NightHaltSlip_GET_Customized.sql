...
CREATE PROCEDURE [dbo].[SP_RPT_NightHaltSlip_GET_Customized]        
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
...
Set @With_Arear_Amount = 0
 ...
 CREATE TABLE #Emp_Cons 
	...
	exec SP_RPT_FILL_EMP_CONS_MULTIDROPDOWN @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@constraint,@Sal_Type,@Salary_Cycle_id,@Segment_Id,@Vertical_Id,@SubVertical_Id,@SubBranch_Id,0,0,0,'0',0,0
...
,BM.Branch_Name,VS.Vertical_Name  
  From (select *,dbo.GET_MONTH_ST_DATE (Eff_Month,Eff_Year) As St_Date from T0120_NIGHT_HALT_APPROVAL WITH (NOLOCK) where Eff_Month <> 0 or Eff_Year<>0) MAD Inner join       
  ...
  WHERE 
  E.Cmp_ID = @Cmp_Id  
  and MAD.St_Date >= cast(@From_Date as smalldatetime) and MAD.St_Date <= cast(@To_Date as smalldatetime)
