...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_OFFER_RECORD_GET_BK151124]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	--,@Branch_ID		numeric   = 0
	...
	--(select Emp_ID From #Emp_Cons)
				select I_Q.* ,E.Emp_Full_Name , E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,Left_Date,BM.Comp_Name,BM.Branch_Address,Left_Reason
					...
					WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
							(select Emp_ID from T0080_EMP_MASTER WITH (NOLOCK) where Date_Of_Join >= @From_Date and Date_Of_Join <= @To_Date and Cmp_ID=@Cmp_ID) 
