CREATE PROCEDURE [dbo].[SP_MOBILE_HRMS_WEBSERVICE_GetActiveToken]                  
   @Cmp_ID INT = 0                  
 ,@Emp_ID INT = 0                  
...
BEGIN          
 ...
 and DeviceID <> '' and DeviceID IS NOT NULL  and IS_Active = 1                
  ...
  ELSE IF (@Type = 'ClaimApplication')  or (@Type = 'AttendanceRegularizeInsert') or (@Type = 'ExitApplications') or (@Type = 'LeaveApplication')   or (@Type = 'TravelApplication')  or (@Type = 'TravelSetApplication')               
 BEGIN      
...
else if  (@Type = 'TravelSetApplication')    
begin    
set @Schme_type='Travel Settlement'    
end
--========================================================================================    
...
-- select * from T0080_EMP_MASTER where Cmp_ID=120      
 ...
 set @S_Emp_IDs=(select Emp_id from T0080_EMP_MASTER where cast(Alpha_Emp_Code as nvarchar) = Cast(SUBSTRING(@Rpt_Manager, 1, 5) as nvarchar))--and Cmp_ID=@Emp_cmp_id)
  ...
  End                
    ...
    ELSE IF (@Type = 'ClaimApprovalUpdate') or (@Type = 'AttendanceRegularizeApproval') or (@Type = 'LeaveApproval')  or (@Type = 'ExitApproval')   or (@Type = 'TravelApproval') or (@Type = 'TravelSettlementApproval')     
 BEGIN      
...
else if  (@Type = 'TravelSettlementApproval')    
begin    
set @Schme_type_approval='Travel Settlement'    
end
--========================================================================================    
 ...
 Declare @Cmp_ids as integer        
    if @Rpt_lvl>0     
 begin     
 declare @str Nvarchar(max)=('select '+cast(concat('Rpt_Mgr_',@Rpt_lvl) as nvarchar)+',cmp_id  from V0050_Scheme_Detail where Scheme_Id='+cast(@scheme_id as varchar(100)))      
 insert into #S_EMP_IDs (emp_id,cmp_id)      
 exec (@str)   
 
 
 set @Rpt_Manager=(select isnull(Cast(emp_id as nvarchar),0) from #S_EMP_IDs)       
set @Cmp_ids=(select cmp_id from #S_EMP_IDs)              
...
and rd.Effect_Date=(select Max(Effect_Date) as Effect_Date from  T0090_EMP_REPORTING_DETAIL  where emp_id=@S_Emp_IDs and Cmp_ID=@Emp_cmp_id )      
 ...
 set @S_Emp_IDs=(select  mn.Emp_id from T0080_EMP_MASTER EM  inner join T0095_INCREMENT INC on EM.emp_id=INC.Emp_ID      
 and inc.Increment_Effective_Date=(select Max(Increment_Effective_Date) as Increment_Effective_Date from T0095_INCREMENT where EMP_id=@Emp_ID and Cmp_ID=@Emp_cmp_id )      
inner join T0030_BRANCH_MASTER BM on bm.Branch_ID=INC.Branch_ID  inner join T0095_MANAGERS MN on mn.branch_id=bm.Branch_ID and bm.Cmp_id=mn.Cmp_id Where  inc.Emp_ID=@Emp_ID)      
...
-- select * from T0080_EMP_MASTER where Cmp_ID=120      
--select @Rpt_Manager
 set @S_Emp_IDs=(select Emp_id from T0080_EMP_MASTER where cast(Alpha_Emp_Code as nvarchar) = Cast(SUBSTRING(@Rpt_Manager, 1, 5) as nvarchar) and Cmp_ID=@Cmp_ids) 
  ...
  --Added by Yogesh on 07032024-----End---------------------------------------              
   ELSE IF (@Type = 'TemplateApplication') -- Added by Divyaraj Kiri on 08/04/2024
   begin
		--Select @Emp_cmp_id,@Emp_ID
		--set @S_Emp_IDs = (Select EM.Emp_Superior from T0080_EMP_MASTER EM where EM.Emp_ID=@Emp_ID and EM.Cmp_ID=@Emp_cmp_id)
		set @S_Emp_IDs =(select rd.R_Emp_ID from T0080_EMP_MASTER em inner join T0090_EMP_REPORTING_DETAIL RD on em.Emp_ID=rd.Emp_ID       
		and rd.Effect_Date=(select Max(Effect_Date) as Effect_Date from  T0090_EMP_REPORTING_DETAIL  where emp_id=@Emp_ID and Cmp_ID=@Emp_cmp_id )      
		 where rd.Emp_ID=@Emp_ID and rd.Cmp_ID=@Emp_cmp_id)
		Select top 1 DeviceID from T0095_Emp_IMEI_Details where Emp_ID = @S_Emp_IDs and IS_Active = 1 		
