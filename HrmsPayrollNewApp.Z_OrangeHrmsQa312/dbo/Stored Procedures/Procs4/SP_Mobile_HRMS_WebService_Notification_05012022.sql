CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_Notification_05012022]
	@Emp_ID numeric(18,0),
	@Cmp_ID numeric(18,0),
...
ELSE IF @Type = 'B' --- For BirthDay & Anniversary Notification
	BEGIN
		...
		'' AS 'DocPath',VM.CommentCount,VM.LikeCount,ISNULL(EL.Like_Flag,0) AS 'Like_Flag', DATEDIFF(YEAR, VM.Date, GETDATE()) as Total_Year -- Added By Niraj(08102021)
		FROM #tblBirthday TB
		INNER JOIN View_BirthdayAlert VM WITH (NOLOCK) ON TB.Emp_ID = VM.Emp_ID
...
ELSE IF @Type = 'D' --- For Document
	BEGIN
		...
		(
			...
			@Emp_ID as Emp_ID,ISNULL(TPM.Dept_Id,0) AS 'Dept_Id',ISNULL(TRD.Is_Mobile_Read,0) AS 'Is_Mobile_Read',
			...
			LEFT JOIN T0090_EMP_DOC_DETAIL ED WITH (NOLOCK) ON DM.Doc_ID = ED.Doc_ID AND EM.EMP_ID = ED.EMP_ID
	...
	BEGIN
		IF @GalleryType = 1 -- For Images
			BEGIN
				...
				FROM T0010_Emp_Gallery EG WITH (NOLOCK) 
...
ELSE IF @Type = 'R' --- For Request Details
	BEGIN
		...
		FROM V0090_Common_Request_Detail VC WITH (NOLOCK)
		INNER JOIN T0011_LOGIN LM WITH (NOLOCK) ON VC.Login_id = LM.Login_ID
		INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON LM.Emp_ID = EM.Emp_ID
		INNER JOIN T0011_LOGIN TLM WITH (NOLOCK) ON VC.Login_id = TLM.Login_ID
		INNER JOIN T0080_EMP_MASTER TEM WITH (NOLOCK) ON TLM.Emp_ID = TEM.Emp_ID
		...
		(CASE WHEN TEM.Image_Name = '0.jpg' OR TEM.Image_Name = '' THEN (CASE WHEN TEM.Gender = 'Male' THEN 'Emp_Default.png' ELSE 'Emp_Default_Female.png' END) ELSE TEM.Image_Name END) AS 'RImageName'
		FROM V0090_Common_Request_Detail VC WITH (NOLOCK)
		INNER JOIN T0011_LOGIN LM WITH (NOLOCK) ON VC.Emp_Login_id = LM.Login_ID
		INNER JOIN T0080_EMP_MASTER EM  WITH (NOLOCK) ON LM.Emp_ID = EM.Emp_ID
		INNER JOIN T0011_LOGIN TLM WITH (NOLOCK) ON VC.Login_id = TLM.Login_ID
		INNER JOIN T0080_EMP_MASTER TEM WITH (NOLOCK) ON TLM.Emp_ID = TEM.Emp_ID
		WHERE LM.Emp_ID = @Emp_ID
...
ELSE IF @Type = 'T' --- For Get Employee Document Type
	BEGIN
		SELECT * FROM T0040_DOCUMENT_MASTER WITH (NOLOCK) WHERE Cmp_ID = @Cmp_ID --AND Doc_Required = 1 
...
ELSE IF @Type = 'H' --- For Get Holiday List
	BEGIN
		...
		SELECT @Branch_id = Branch_id  FROM T0095_increment AS A WITH (NOLOCK)
		INNER JOIN(
