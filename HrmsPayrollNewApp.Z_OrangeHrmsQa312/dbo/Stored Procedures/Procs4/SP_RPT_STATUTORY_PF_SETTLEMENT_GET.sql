...
CREATE PROCEDURE [dbo].[SP_RPT_STATUTORY_PF_SETTLEMENT_GET]
...
AS
	...
	--------
		...
		DECLARE @EMP_SALARY TABLE
		    ...
		    INSERT INTO @EMP_SALARY (Cmp_Id,Emp_Id,Month,Year,Month_St_Date,Month_end_Date,PF_Salary_Amount,PF_Per,PF_Amount,PF_Limit,PF_367,PF_833,PF_DIFF_6500,Emp_Age)
		    select MAD.Cmp_ID,MAD.Emp_ID,Month(For_Date),Year(For_Date),max(For_Date),max(for_Date),SUM(M_AD_CALCULATED_AMOUNT),max(M_AD_PERCENTAGE),sum(M_AD_AMOUNT),@PF_Limit,0,0,0,0 From 
		     T0210_MONTHLY_AD_DETAIL MAD WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AM WITH (NOLOCK) ON MAD.AD_ID = AM.AD_ID  AND ad_DEF_id = @PF_DEF_ID and ad_not_effect_salary <> 1 and MAD.CMP_ID=@CMP_ID 
		     INNER JOIN @EMP_CONS EC ON MAD.EMP_ID =EC.EMP_ID 
		     WHERE MAD.CMP_iD =@CMP_ID AND M_AD_EFFECT_DATE >=@FROM_DATE AND M_AD_EFFECT_DATE <=@TO_DATE and isnull(Sal_Type,0)<>0
		    GROUP BY MAD.CMP_ID,MAD.EMP_ID,MONTH(FOR_dAte),YEAR(FOR_DATE)
		    
			
			
		   UPDATE @EMP_SALARY 
		    SET PF_Salary_Amount = PF_Salary_Amount + isnull(mad.M_AD_calculated_Amount,0),
				PF_Amount = PF_Amount + isnull(mad.M_AD_Amount,0)
			FROM @EMP_SALARY ES INNER JOIN T0210_MONTHLY_AD_DETAIL MAD ON ES.EMP_ID = MAD.EMP_ID AND ISNULL(MAD.SAL_tYPE,0) =0 
				  AND ES.MONTH = MONTH(MAD.FOR_DATE) AND ES.YEAR = YEAR(MAD.FOR_DATE)	
			INNER JOIN T0050_AD_MASTER AM ON MAD.AD_ID = AM.AD_ID  AND ad_DEF_id = @PF_DEF_ID and ad_not_effect_salary <> 1 and MAD.CMP_ID=@CMP_ID 
			
			
		    Update @EMP_SALARY
		    Set Emp_AGe =isnull(dbo.F_GET_AGE(Date_of_Birth,MONTH_ST_DATE,'N','N'),0)
		    From @EMP_SALARY es inner join T0080_Emp_MAster e on es.emp_id =e.emp_ID
...
--------------------------------------------- PF CHALLAN CALCULATION
	/*CREATE table #EMP_SALARY_Challan 
		(
	...
	WHILE @TEMP_DATE <=@TO_DATE
		BEGIN
				...
				SELECT @Total_Family_Pension_Wages_Amount = sum(PF_SALARY_AMOUNT) from @EMP_SALARY
				Where Emp_Age >@PF_PEnsion_Age and @PF_PEnsion_Age >0

				set @Total_EDLI_Subscriber = @Total_Subscriber 
				set @Total_EDLI_Wages_Amount = @Total_Wages_Due
				...
				set @dblAC21_1 = round(@Total_Wages_Due * @AC_21_1/100 ,0)

				if  ( @AC_22_3 *  @Total_Wages_Due )/100 > 200 
					set @dblAC22_3 =  Round((@AC_22_3 *  @Total_Wages_Due )/100,0)
				else
					set @dblAC22_3 = 200

				
				set @dblAC22_4 =  Round((@AC_22_4 *  @Total_Wages_Due )/100,0)
