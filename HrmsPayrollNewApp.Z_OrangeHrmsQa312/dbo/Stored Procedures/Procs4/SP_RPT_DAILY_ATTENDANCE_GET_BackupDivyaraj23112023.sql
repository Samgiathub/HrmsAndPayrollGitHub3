...
CREATE PROCEDURE [dbo].[SP_RPT_DAILY_ATTENDANCE_GET_BackupDivyaraj23112023]
	@Cmp_ID 			numeric
	,@From_Date			datetime
	,@To_Date 			datetime 
	,@Branch_ID			numeric
			...
			IF @Format = 3
				BEGIN
				 --   --commented by mansi 28-01-22 start
					...
					insert into @deptTable (dt_Dept_id,dt_Total,dt_Selected_Format,dt_Shift_id)
					select distinct P.Dept_Id ,count(p.Emp_id) as Total , @Format,shift_id FROM #PRESENT P group by P.dept_id,p.shift_id
