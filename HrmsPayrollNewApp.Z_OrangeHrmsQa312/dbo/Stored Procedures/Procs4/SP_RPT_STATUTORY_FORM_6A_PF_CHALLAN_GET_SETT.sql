...
CREATE PROCEDURE [dbo].[SP_RPT_STATUTORY_FORM_6A_PF_CHALLAN_GET_SETT]    
  ...
  DECLARE @EMP_SALARY TABLE    
   (    
    ...
    EMP_AGE     NUMERIC(5,1)     
      ...
      SELECT  sg.Cmp_ID ,SG.EMP_ID,MONTH(S_MONTH_ST_DATe),YEAR(S_MONTH_ST_DATE),SG.S_Salary_Amount     
     ,0 ,sg.S_Month_st_Date,SG.S_Month_End_date    
     ,MAD.PF_PER,MAD.PF_AMOUNT  , MAD.m_ad_Calculated_Amount ,@PF_Limit,0,0,0,dbo.F_GET_AGE(Date_of_Birth,S_MONTH_ST_DATE,'N','N')    
    ...
    ( select Emp_ID , m_ad_Percentage as PF_PER , m_ad_Amount as PF_Amount , m_ad_Calculated_Amount ,SAL_tRAN_ID from     
     ...
     and AD.CMP_ID = @CMP_ID   And isnull(Sal_Type,0)=1)
