...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_ATTENDANCE_MUSTER_GET]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
AS
	...
	IF @Export_Type = 'Calendar'  or @Export_Type = 'Attendance Regularization' or  (@Report_For='Complete_Absent' And @Export_Type='999') --Added by Jaina 18-07-2020 Attendance Condition
		BEGIN
			...
			Declare @IsDeficit Numeric(2,0) --Added by nilesh patel 
			set @IsDeficit = 0
...
----PRINT 'STATE 7.2 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
	...
	UPDATE	#Att_Muster
	SET		Status = CAST(round(AB_Days,3) as numeric(18,2))--AB_Days --+ Isnull(Half_Cnt,0) -- Comment by nilesh on 16102015 after discussion with Hardik bhai  -- Changed By Sajid 17-10-2024
	FROM	#Att_Muster AM 
	...
	Else
		Begin
			...
			update #Att_Muster
			set Status = CAST(round(Leave -Isnull(Leave_ADD_In_Present,0) ,3) as numeric(18,2))    --Leave  --Changed by Sumit on 0302201  -- Changed By Sajid 17-10-2024
			from #Att_Muster AM inner join       --Commented by Alpesh 27-Jul-2011 bcoz all leave has diff status 
			(select Emp_Id ,sum(Isnull(Leave_Count,0)) as Leave From #Att_Muster --(select Emp_Id ,count(Leave_Count) as Leave From #Att_Muster --added by Falak 01-APR-2011
				Where  For_Date>=@From_Date and For_DAte <=@to_Date --Status = 'L' and // 
				group by Emp_ID)Q on Am.Emp_ID = Q.Emp_ID
   ...
   ----PRINT 'STATE 7.4 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
	...
	----PRINT 'STATE 9 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
			...
			BEGIN
					...
					if @Late_Mark_Scenario = 2  and 
					Exists(select 1 from T0050_GENERAL_LATEMARK_SLAB GLS inner join #EMP_GEN_SETTINGS_ATT EG on EG.Gen_Id = GLS.GEN_ID where GLS.DEDUCTION_TYPE = 'Hours' 
					and CMP_ID = @Cmp_ID)
					BEGIN
							exec rpt_Late_Early_Mark_Combine_Deduction @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@constraint,1
					END
					if @Late_Mark_Scenario = 2  and @IsDeficit = 1 -- Deepal 30102024
					BEGIN
					
							exec rpt_Late_Early_Mark_Combine_Deduction @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID
							,@Desig_ID,@Emp_ID,@constraint,1
							--select * from #Late_Early_Deduction
					END
					ELSE
					BEGIN
							exec rpt_Late_Early_Mark_Deduction_Details @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@constraint,'Late'
							,0,1
					END
		...
		ELSE
			...
			if @Late_Mark_Scenario = 2  and  Exists(select 1 from T0050_GENERAL_LATEMARK_SLAB GLS 
			inner join #EMP_GEN_SETTINGS_ATT EG on EG.Gen_Id = GLS.GEN_ID where GLS.DEDUCTION_TYPE = 'Hours' and CMP_ID = @Cmp_ID)
			begin
			...
			BEGIN
							...
							Select @TotalSumOfLateAndEarlyDeduction = Max(Deduct_Days)
							From #Late_Early_Deduction where Deduct_Days > 0 
							Group by Emp_ID
		...
		if @Export_Type  = 'EXCEL'
			...
			*/
			...
			IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Dishman')) --Added Condition for Dishman Company show Absent As LWP 30082017
				...
				UPDATE A
				set 
				Status = Replace(a1.TotalSum,':','.')
				from #Att_Muster A inner join (
					Select dbo.F_Return_Hours(SUM(Duration_in_sec)) as TotalSum,Emp_Id 
					from #DATA 
					where For_date between @From_Date and @To_date
					GROUP By Emp_Id		
				) A1 on A.Emp_Id = A1.Emp_Id
				where A.Row_ID = 38
				--- End of Added by Mihir Adeshara 07062012
	...
	begin
		...
		if @Report_For <> 'ABSENT_CON'	
			...
			if (@Export_Type='999') --For Auto Leave Adjust with Absent --Hardik 23/03/2020
				Begin
					Select *,0 as Leave_Id ,0 as Leave_Period from #Att_Muster_with_shift Where A_days In (1,0.5)
				End
			Else  
				Begin
...
--		--delete from #Att_Muster where Row_id in (36)
...
--			UPDATE	#Att_Muster SET [Status]= @Payable_Present_days 
--			WHERE	Emp_Id = @EmpID AND Cmp_ID=@CmpID AND Row_ID=37
...
--				L_Day		NUMERIC(18,2),  --34
--				W_Day		NUMERIC(18,2),  --35
--				H_Day		NUMERIC(18,2),  --36
--				PD_Days		As Cast(ROUND(P_Day + A_Day + L_Day +  W_Day + H_Day ,0) As Numeric(5,2))		
				
--			)
			
--			--SET @COL =	   'P_Day,A_Day,W_Day,H_Day,T_Day'
--			SET @SUM_COL = 'SUM(P_Day),SUM(A_Day),SUM(L_Day),SUM(W_Day),SUM(H_Day)'
...
--									pivot 
--									(
--										MAX(Status) for Row_Id in ([32],[33],[34],[35],[36])										
--									) p 									
--								)Q 
--								' 
--			EXEC (@query1)	
...
ELSE IF @Report_For = 'Form - D'
		...
		END
			...
			UPdate  A
			SET  [Status] = dbo.F_Return_Hours_D (OT)
			FRom  #Att_Muster A 
			      left join 
							(	SELECT	(Sum(isnull(Approved_OT_Sec,0)) + Sum(isnull(Approved_WO_OT_Sec,0)) + Sum(isnull(Approved_HO_OT_Sec,0))) AS OT ,EMP_Id
								FROM	T0160_OT_APPROVAL
		...
		CREATE TABLE #TempPivotTable
			(
				...
				W_Day		NUMERIC(18,2),  --35
				H_Day		NUMERIC(18,2),  --36
				PD_Days		As Cast(ROUND(P_Day + A_Day + L_Day +  W_Day + H_Day ,0) As Numeric(18,2))		
