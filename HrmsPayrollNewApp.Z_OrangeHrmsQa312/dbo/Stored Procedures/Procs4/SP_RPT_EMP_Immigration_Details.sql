...
-- Developed By Muslim 18022014
---18/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[SP_RPT_EMP_Immigration_Details]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   = 0
 ...
 (      
  ...
  Increment_ID numeric    
	...
	if @Constraint <> ''
		...
		;with CTE as(	select Distinct  ROW_NUMBER() OVER(PARTITION BY E.Emp_Full_Name ORDER BY E.Emp_Full_Name) As RowID, E.Emp_ID, E.Alpha_Emp_Code as Emp_Code,E.Emp_Full_Name as Employee_Name, bm.Branch_Name as Branch,dm.Dept_Name as Department,
						EID.Imm_Type as Types_Of_Detail,LM.Loc_name as Citizenship,EID.Imm_No as [Passport/Visa_Number], REPLACE(CONVERT(VARCHAR(11),EID.Imm_Issue_Date,103), ' ','/') as Issue_Date,REPLACE(CONVERT(VARCHAR(11),EID.Imm_Date_of_Expiry,103), ' ','/') as Expired_Date, REPLACE(CONVERT(VARCHAR(11),EID.Imm_Review_Date,103), ' ','/')as Review_Date,
						EID.Imm_Comments as Comments,EID.Imm_Issue_Status as Status 
			...
			( select  I.Emp_Id,Branch_ID,I.Emp_Full_PF,I.Emp_Auto_Vpf  from dbo.T0095_Increment I WITH (NOLOCK) inner join 
					...
					T0090_EMP_IMMIGRATION_DETAIL EID WITH (NOLOCK) on EID.Emp_ID = E.Emp_ID Inner Join
					T0001_Location_Master LM WITH (NOLOCK) on LM.Loc_ID = EID.Loc_ID Inner Join
...
Case When RowID = 1 then Department else '' end as Department,Types_Of_Detail,Citizenship,[Passport/Visa_Number],Issue_Date,Expired_Date,Review_Date,Comments,Status
