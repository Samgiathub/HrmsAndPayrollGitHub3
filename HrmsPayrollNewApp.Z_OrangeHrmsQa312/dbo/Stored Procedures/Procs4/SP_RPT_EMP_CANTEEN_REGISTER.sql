...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_CANTEEN_REGISTER] 
...
SET ARITHABORT ON
	...
	SELECT EM.EMP_ID,CID.IO_DATETIME,'1',IP.IP_ID FROM T9999_DEVICE_INOUT_DETAIL CID WITH (NOLOCK)
	INNER JOIN T0040_IP_MASTER IP WITH (NOLOCK) ON CID.IP_ADDRESS=IP.IP_ADDRESS 
	--INNER JOIN T0080_EMP_MASTER EM ON EM.ENROLL_NO = CID.ENROLL_NO
	...
	INSERT INTO #CANTEEN_ATT_CONS(EMP_ID,FOR_DATE,[STATUS],IP_ID)
	SELECT EM.EMP_ID,cast(CID.CANTEEN_PUNCH_DATETIME as date) AS IO_DATETIME,SUM(CID.QUANTITY),IP.IP_ID FROM T0150_EMP_CANTEEN_PUNCH CID WITH (NOLOCK)
	...
	WHILE (@TEMP_DATE  <= @TO_DATE)
		...
		WHERE MC.CNT_ID IN ( SELECT CANTEEN_ID AS CNT_ID FROM T0150_EMP_CANTEEN_PUNCH WITH (NOLOCK) WHERE Flag='Mobile' and Reason='Mobile' )
	 ...
	 --   UPDATE	T	COMMENTED ON 20022019
		--SET		
