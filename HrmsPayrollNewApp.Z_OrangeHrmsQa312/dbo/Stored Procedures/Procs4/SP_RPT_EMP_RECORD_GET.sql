...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_RECORD_GET] @Cmp_ID NUMERIC
	,@From_Date DATETIME
	,@To_Date DATETIME
...
WHERE Cmp_ID = @Cmp_ID
	AND Setting_Name LIKE 'Show Left Employee for Salary'
...
BEGIN
	...
	FROM T0040_SETTING WITH (NOLOCK)
	WHERE Cmp_ID = @Cmp_ID
		AND Setting_Name = 'Hide Attendance For Fix Salary Employee'
END
...
CREATE TABLE #TEMP_Project (EMP_ID NUMERIC)
...
BEGIN
	IF @Type = ''
	BEGIN
		DECLARE @crosscmp VARCHAR(100)
		DECLARE @Setting_value INT

		SELECT @Setting_value = Setting_Value
		FROM t0040_setting
		WHERE setting_name = 'Reporting TO Reporting On Downline Employee'
			AND cmp_id = @Cmp_ID

		SELECT @crosscmp = coalesce(@crosscmp + ',', '') + convert(VARCHAR(12), Cmp_Id)
		FROM T0010_COMPANY_MASTER
		WHERE is_GroupOFCmp = (
				SELECT TOP 1 is_GroupOFCmp
				FROM T0010_COMPANY_MASTER
				WHERE Cmp_Id = 1
				)
			AND Cmp_Id <> 1

		IF @Setting_value = 1 and  @ESS_downline_Report = 1
		BEGIN
			...
			FROM (
				SELECT  E.Cmp_ID
					...
					,E.Alpha_Emp_Code
					,CASE 
						WHEN S.Setting_Value = 1
							THEN isnull(E.Initial, '') + ' ' + E.Emp_First_Name + ' ' + isnull(E.Emp_Second_Name, '') + ' ' + isnull(E.Emp_Last_Name, '')
						ELSE E.Emp_First_Name + ' ' + isnull(E.Emp_Second_Name, '') + ' ' + isnull(E.Emp_Last_Name, '')
						END AS Emp_Full_Name
					,Left_Date
					,BM.Comp_Name
					,BM.Branch_Address --,Left_Reason as Left_Reason1         
					,Dm.Dept_Name
					,Dgm.Desig_Name
					,Etm.Type_Name
					,Gm.Grd_Name
					...
					,E.Present_Post_Box
					,l.left_reason
					,DATEDIFF(YY, ISNULL(E.Date_of_bIRTH, getdate()), GETDATE()) AS AGE
					,Nature_of_Business
					,Cmp_City
					,Cmp_State_Name
					,Cmp_PinCode
					,E.mobile_no --,I_Q.Bank_ID,I_Q.Inc_Bank_AC_No          
					,E.Enroll_No
					,CASE 
						WHEN Is_Terminate = 1
							THEN 'Terminated'
				...
				INNER JOIN (
					...
					INNER JOIN (
						...
						and I.Increment_ID = Qry.Increment_ID  --ronakb Bug #27284 -- Tejas 20112024 remove the backdated increment
				...
				LEFT OUTER JOIN dbo.T0050_SubBranch SBM WITH (NOLOCK) ON BM.BRANCH_ID = SBM.SubBranch_ID
					AND BM.Cmp_ID = SBM.Cmp_ID
				LEFT OUTER JOIN T0040_Vertical_Segment V WITH (NOLOCK) ON I_Q.Vertical_ID = V.Vertical_ID
				...
				INNER JOIN T0040_SETTING S WITH (NOLOCK) ON E.Cmp_ID = S.Cmp_ID
					AND S.Setting_Name = 'Add initial in employee full name'
				INNER JOIN T0011_Login LO WITH (NOLOCK) ON LO.Emp_Id = E.Emp_Id
				LEFT OUTER JOIN T0040_Reason_Master Rm WITH (NOLOCK) ON rm.Res_Id = l.Res_Id
				WHERE E.Cmp_ID = @Cmp_Id
					AND E.Date_Of_Join <= ISNULL(e.Emp_Left_Date, @To_Date)
					AND I_Q.Payment_Mode = (
						CASE 
							WHEN (
									@Payment_Mode = '--Select--'
									OR @Payment_Mode = ''
					...
					AND (
						CASE 
							WHEN @Hide_Attendance_For_Fix_Salary = 1
								AND I_Q.Emp_Fix_Salary = 1
								THEN 1
							ELSE 0
							END
						) = 0 --Added By Jaina 07-09-2016                             
					AND 1 = (
						CASE 
							WHEN @Is_Active <> 2
								AND LO.is_Active = @Is_Active
								THEN 1
							WHEN @Is_Active = 2
								THEN 1
							ELSE 0
				...
				INNER JOIN
					...
					T0040_SETTING S WITH (NOLOCK) ON E.Cmp_ID = S.Cmp_ID
					AND S.Setting_Name = 'Add initial in employee full name'
				INNER JOIN T0011_Login LO WITH (NOLOCK) ON LO.Emp_Id = E.Emp_Id
				LEFT OUTER JOIN T0040_Reason_Master Rm WITH (NOLOCK) ON rm.Res_Id = l.Res_Id
				WHERE E.Cmp_ID IN (
			...
			ORDER BY CASE 
					WHEN IsNumeric(T.Alpha_Emp_Code) = 1
						THEN Right(Replicate('0', 21) + T.Alpha_Emp_Code, 20)
					ELSE Left(T.Alpha_Emp_Code + Replicate('', 21), 20)
					END
		END
		ELSE
		BEGIN
			...
			ORDER BY CASE 
					...
					ELSE Left(e.Alpha_Emp_Code + Replicate('', 21), 20)
					END
		END
	END
	ELSE IF @Type = 'Unifrom Cost'
	BEGIN
		SELECT I_Q.*
			,E.Emp_Last_Name
			,E.Emp_Second_Name
			...
			,CASE 
				WHEN S.Setting_Value = 1
					THEN --Added By Hardik 04/02/2016          
			...
			,CASE 
				...
				WHEN Is_Absconded = 1
					THEN 'Absconded'
				ELSE 'Resignation'
				END AS Reason_Type --Added By Ramiz on 18/08/2015          
			,DGM.Desig_Dis_No --added jimit 21082015          
			,I_Q.Vertical_ID
			,I_Q.SubVertical_ID --Added By Jaina 5-10-2015          
		...
		INNER JOIN (
			...
			FROM dbo.T0095_Increment I WITH (NOLOCK)
			INNER JOIN (
				SELECT max(T0095_Increment.Increment_Effective_Date) AS Increment_ID
					,T0095_Increment.Emp_ID
				FROM dbo.T0095_Increment WITH (NOLOCK)
				...
				AND I.Increment_Effective_Date = Qry.Increment_ID
			) I_Q ON E.Emp_ID = I_Q.Emp_ID
		INNER JOIN dbo.T0040_GRADE_MASTER GM WITH (NOLOCK) ON I_Q.Grd_ID = GM.Grd_ID
		LEFT OUTER JOIN dbo.T0040_TYPE_MASTER ETM WITH (NOLOCK) ON I_Q.Type_ID = ETM.Type_ID
		LEFT OUTER JOIN dbo.T0040_DESIGNATION_MASTER DGM WITH (NOLOCK) ON I_Q.Desig_Id = DGM.Desig_Id
		...
		INNER JOIN #Emp_Cons EC WITH (NOLOCK) ON E.Emp_ID = EC.Emp_ID
		INNER JOIN T0040_SETTING S WITH (NOLOCK) ON E.Cmp_ID = S.Cmp_ID
			AND S.Setting_Name = 'Add initial in employee full name' --Added Condition by Hardik 04/02/2016                      
		INNER JOIN T0011_Login LO WITH (NOLOCK) ON LO.Emp_Id = E.Emp_Id
		LEFT OUTER JOIN T0040_Reason_Master Rm WITH (NOLOCK) ON rm.Res_Id = l.Res_Id --Added By Jimit 25122018          
		INNER JOIN T0110_Uniform_Dispatch_Detail UDD WITH (NOLOCK) ON UDD.Emp_ID = EC.Emp_ID
		WHERE E.Cmp_ID = @Cmp_Id
			AND (
				UDD.Dispatch_Date >= @From_Date
				AND UDD.Dispatch_Date <= @To_Date
				)
		...
		ORDER BY CASE 
				...
				ELSE Left(e.Alpha_Emp_Code + Replicate('', 21), 20)
				END
			-- Added By Tejas For get cross company Employee
	END
END
...
BEGIN
	...
	ELSE
	BEGIN
		EXEC SP_EMP_SALARY_Constraint_adult_worker @Cmp_ID
			,@From_Date
			,@To_Date
			,0
			,@Cat_ID
...
ELSE IF @Report_Type = 'ID Card'
	OR @Report_Type = 'QR Code'
BEGIN
	SELECT I_Q.*
		,E.Emp_Last_Name
		,E.Emp_Second_Name
		...
		,E.Home_Tel_No
		,isnull(Vertical_Code, '') AS Vertical_Code
		,isnull(Vertical_name, '') AS Vertical_name
		,isnull(Vertical_description, '') AS Vertical_description
		,isnull(SubVertical_Code, '') AS SubVertical_Code
		,isnull(SubVertical_name, '') AS SubVertical_name
		,isnull(SubVertical_description, '') AS SubVertical_description
		...
		,E.Home_Tel_no AS Emergency_No1
		,E.Mobile_No AS Emergency_No2
		,CM.Cmp_Phone
		,S.State_Name
		,cm.CIT_City
		,--added jimit 29022016          
		Emergency_Contact.Home_Mobile_No
		,E.Initial --added by chetan 280817          
		,E.Tehsil_Wok
		,CM.Cmp_Email
		,CM.Cmp_Web
		,CAST(0 AS VARBINARY(MAX)) AS Emp_Signature
		,E.Signature_Image_Name --added by chetan 08012018          
		,CAST(0 AS VARBINARY(max)) AS Authorized_Sign
		,AUS.Signature_Image_Name AS Authorized_Sign_ImgName --Added by Jaina 16-04-2018          
		,E.UAN_No
		,E.SIN_No
		,BS.Segment_Name
		,E.Old_Ref_No
		...
		,E.Work_Email
		,Manager.R_Emp_ID
		,EM1.Emp_Full_Name AS Manager_Name
		,CAST(0 AS VARBINARY(MAX)) AS Emp_QR_Code
		,E.Street_1 AS PermanentAddress
		,CM.Cmp_TAN_No AS Cmp_RegistrationNo
		,DGM.Desig_Code
		,DM.Dept_Code
		,EM.Home_Mobile_No
	FROM dbo.T0080_EMP_MASTER E WITH (NOLOCK)
	...
	INNER JOIN (
		...
		FROM dbo.T0095_Increment I WITH (NOLOCK)
		INNER JOIN (
			SELECT max(Increment_Effective_Date) AS Increment_ID
				,Emp_ID
	...
	INNER JOIN dbo.T0030_BRANCH_MASTER BM WITH (NOLOCK) ON I_Q.BRANCH_ID = BM.BRANCH_ID
	INNER JOIN dbo.T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID
	LEFT OUTER JOIN -- Changed by Aswini-23062023          
		dbo.T0090_EMP_EMERGENCY_CONTACT_DETAIL EM WITH (NOLOCK) ON E.Emp_ID = EM.Emp_ID
	INNER JOIN #Emp_Cons EC WITH (NOLOCK) ON E.Emp_ID = EC.Emp_ID
	...
	LEFT OUTER JOIN dbo.T0050_SubBranch AS SBM WITH (NOLOCK) ON SBM.BRANCH_ID = BM.BRANCH_ID
		AND sbm.SubBranch_ID = I_Q.subBranch_ID
	...
	LEFT JOIN T0020_STATE_MASTER S WITH (NOLOCK) ON S.Cmp_ID = Cm.Cmp_Id
		AND s.State_ID = Cm.State_ID
	LEFT OUTER JOIN (
		SELECT Min(EMCD.Row_ID) AS Row_Id
			,EMCD.Emp_ID
			,EMCD.Home_Mobile_No
		FROM T0090_EMP_EMERGENCY_CONTACT_DETAIL EMCD WITH (NOLOCK)
		INNER JOIN (
			SELECT Min(EMCD.Row_ID) AS Row_Id
				,EC.Emp_ID
			FROM T0090_EMP_EMERGENCY_CONTACT_DETAIL EMCD WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC WITH (NOLOCK) ON EMCD.Emp_ID = EC.Emp_ID
			GROUP BY EC.Emp_ID
			) Qry ON EMCD.Row_ID = Qry.Row_Id
			AND EMCD.Emp_ID = Qry.Emp_ID
		WHERE Cmp_ID = @Cmp_Id
		GROUP BY EMCD.Emp_ID
			,EMCD.Home_Mobile_No
		) Emergency_Contact ON E.Emp_ID = Emergency_Contact.Emp_ID
	LEFT OUTER JOIN (
		SELECT TOP 1 max(AU.Effective_Date) AS Effective_date
			,AU.Emp_ID
			,AU.Branch_ID
			,E_Sign.Signature_Image_Name --Added by Jaina 16-04-2018          
		...
		GROUP BY AU.Branch_ID
			,AU.Emp_ID
			,E_Sign.Signature_Image_Name
