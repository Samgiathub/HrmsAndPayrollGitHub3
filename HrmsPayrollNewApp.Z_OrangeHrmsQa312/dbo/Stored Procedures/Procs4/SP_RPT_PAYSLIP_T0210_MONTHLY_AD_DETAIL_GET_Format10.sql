...
CREATE PROCEDURE [dbo].[SP_RPT_PAYSLIP_T0210_MONTHLY_AD_DETAIL_GET_Format10]        
	 @Cmp_ID   numeric        
	,@From_Date  datetime        
	,@To_Date   datetime        
	,@Branch_ID  numeric        
...
If @Sal_Type = 3 
	...
	IF IsNull(@Salary_Cycle_id,0) = 0
		SELECT @Salary_Cycle_id = ES.SalDate_id
		FROM	T0095_Emp_Salary_Cycle ES WITH (NOLOCK)
				INNER JOIN #Emp_Cons EC  ON ES.Emp_id=EC.Emp_ID
		WHERE	ES.Effective_date <= @To_Date
	...
	If @With_Arear_Amount = 0  
		Begin
		   ...
		   Select mad.Emp_Id,mad.Cmp_ID,mad.AD_ID,mad.Sal_Tran_ID,sum(mad.m_AD_Amount),sum(mad.M_AD_Actual_Per_amount),sum(mad.M_AD_Calculated_amount),mad.To_Date,mad.M_AD_Flag,sum(isnull(M_AREAR_AMOUNT,0))        
			 ...
			 #Emp_Cons EC ON MAD.EMP_ID = EC.EMP_ID         
			WHERE MAD.Cmp_ID = @Cmp_Id and Month(To_Date) = Month(@To_Date) and Year(To_Date) = Year(@To_Date)--For_date >=@From_Date and To_date <=@To_Date          
			   and (M_AD_NOT_EFFECT_SALARY = 0 or isnull(MAD.reimShow,0)= 1  )    and Sal_Tran_ID is not null  
			   and isnull(Sal_Type,0) = isnull(@Sal_Type,Sal_Type) --and M_AD_Percentage =0        
			Group by Mad.Emp_ID,mad.AD_ID, mad.Cmp_ID, mad.To_Date, mad.M_AD_Flag, mad.Sal_Tran_ID,S_Sal_Tran_ID   
 ...
 if @Sal_Type =0         
  begin  
   ...
   select ms.Emp_ID,Cmp_ID,null,'Basic Salary',Sal_Tran_ID,Salary_amount,Basic_Salary,0,Month_end_Date ,'I' , ms.Arear_Basic      
    ...
    and   Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) And Is_FNF = 0   /*Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date */      
   ...
   select ms.Emp_ID,Cmp_ID,null,'OT Amount',Sal_Tran_ID,OT_Amount,null,Gross_Salary,Month_end_Date ,'I',0        
    ...
    From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
    and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date)  and isnull(Other_Allow_Amount,0) >0        -- and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date
	...
	If @With_Arear_Amount = 0
		Begin        
			...
			select  ms.Emp_ID,Cmp_ID,null,'Arrear Amount',Sal_Tran_ID,Settelement_Amount,null,0,Month_end_Date ,'I',0        
			 From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
			 and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date)  and isnull(Settelement_Amount,0) >0       -- and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date 
		End
	Else
		Begin
			...
			select  ms.Emp_ID,MS.Cmp_ID,null,'Arrear Gross Amount',0,SUM(S_Gross_Salary),null,0,S_Eff_Date ,'I',0        
			 ...
			 Group by ms.Emp_ID,MS.Cmp_ID,S_Eff_Date,S_Sal_Tran_ID
		End      
    ...
    select  ms.Emp_ID,Cmp_ID,null,'Leave Encash Amount',Sal_Tran_ID,Leave_salary_Amount,null,0,Month_end_Date ,'I'        
     From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
     and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) and isnull(Leave_Salary_Amount,0) >0         --and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date
  ...
  and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) -- and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date    
    ...
    Insert into #Pay_slip (Emp_ID,Cmp_ID,AD_ID,AD_Description,Sal_Tran_ID,AD_Amount,AD_ACtual_Amount,AD_Calculated_Amount,For_Date,M_AD_Flag)
				select  ms.Emp_ID,Cmp_ID,null,'Bonus',Sal_Tran_ID,Bonus_Amount,null,0,Month_end_Date ,'I'
					From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID 
				and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) and isnull(Bonus_Amount,0) >0	 --	and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date   
...
----Added by Gadriwala Muslim 06012015- End 
   ...
   Insert into #Pay_slip (Emp_ID,Cmp_ID,AD_ID,AD_Description,Sal_Tran_ID,AD_Amount,AD_ACtual_Amount,AD_Calculated_Amount,For_Date,M_AD_Flag,M_Arrear_Days)
				select  ms.Emp_ID,Cmp_ID,null,'Week Off Working',Sal_Tran_ID,M_WO_OT_Amount,M_WO_OT_Amount,0,Month_end_Date ,'I',0
					From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID 
					and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) and isnull(M_WO_OT_Amount,0) >0--and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date 
	...
	---YTD Column-- Ankit 10102013---
       ...
       --added by jimit 28072017
	Update #Pay_slip Set YTD = Late_Dedu_Amount  From
		 (Select Mad.Emp_ID,sum(Mad.Late_Dedu_Amount) as Late_Dedu_Amount
			From T0200_MONTHLY_SALARY  MAD WITH (NOLOCK) INNER JOIN
			#Emp_Cons EC ON MAD.EMP_ID = EC.EMP_ID
			WHERE MAD.Cmp_ID = @Cmp_Id and (MAD.Month_St_Date Between @F_StartDate and @F_EndDate) and MAD.Month_End_Date<=@To_Date
			Group by Mad.Emp_ID) Qry
			Inner join #Pay_slip p on Qry.Emp_ID = p.Emp_ID 
		Where P.Cmp_ID = @Cmp_ID And AD_Description = 'Late Deduction Amt'	
	--ended 
...
--added By Mukti(end)25032015
  ...
  begin        
   ...
   select ms.Emp_ID,Cmp_ID,null,'Basic Salary',null,S_Salary_amount,S_Basic_Salary,0,s_Month_end_Date ,'I',0 ,S_Sal_Tran_ID       
    From T0201_Monthly_Salary_Sett  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
    and Month(S_Month_end_Date) = Month(@To_Date) and Year(S_Month_end_Date) = Year(@To_Date) --S_Month_St_DAte >=@From_Date and S_Month_end_Date <=@To_Date        
  ...
  --   and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date and isnull(Other_Allow_Amount,0) >0
    ...
    select  ms.Emp_ID,Cmp_ID,null,'Settlement Amount',Sal_Tran_ID,Settelement_Amount,null,0,Month_end_Date ,'I'        
     From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
    and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date) and isnull(Settelement_Amount,0) >0  -- and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date and isnull(Settelement_Amount,0) >0        
        
    Insert into #Pay_slip (Emp_ID,Cmp_ID,AD_ID,AD_Description,Sal_Tran_ID,AD_Amount,AD_ACtual_Amount,AD_Calculated_Amount,For_Date,M_AD_Flag)        
    select  ms.Emp_ID,Cmp_ID,null,'Leave Encash Amount',Sal_Tran_ID,Leave_salary_Amount,null,0,Month_end_Date ,'I'        
     From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID         
    and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date)  and isnull(Leave_Salary_Amount,0) >0  --and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date and isnull(Leave_Salary_Amount,0) >0        
    ...
    select ms.Emp_ID,Cmp_ID,null,'Advance Amount',Sal_Tran_ID,Advance_Amount,null,Gross_Salary,Month_end_Date ,'D'        
	 ...
	 select  ms.Emp_ID,Cmp_ID,null,'Bonus',Sal_Tran_ID,Bonus_Amount,null,0,Month_end_Date ,'I'
		From T0200_Monthly_Salary  ms WITH (NOLOCK) Inner Join #Emp_Cons ec on ms.Emp_ID =ec.emp_ID 
     and  Month(Month_end_Date) = Month(@To_Date) and Year(Month_end_Date) = Year(@To_Date)  and isnull(Bonus_Amount,0) >0		--and Month_St_DAte >=@From_Date and Month_end_Date <=@To_Date and isnull(Bonus_Amount,0) >0	        
 ...
 Select ISNULL(EmpName_Alias_Salary,Emp_Full_Name) as Emp_full_Name,Grd_Name,Comp_Name,Branch_Address,EMP_CODE,Type_Name,Dept_Name,Desig_Name,AD_Name ,ADM.AD_LEVEL,
 MAD.Emp_ID,Mad.Cmp_ID,Mad.AD_ID,Mad.Sal_Tran_ID,Mad.AD_Description,Mad.AD_Amount,dbo.F_Remove_Zero_Decimal(Mad.AD_Actual_Amount) as AD_Actual_Amount,Mad.AD_Calculated_Amount,Mad.For_Date,Mad.M_AD_Flag,Mad.Loan_Id,   
 Mad.Def_ID,Mad.M_Arrear_Days,Mad.YTD       
-- Select Emp_full_Name,Grd_Name,Comp_Name,Branch_Address,EMP_CODE,Type_Name,Dept_Name,Desig_Name,AD_Name ,AD_LEVEL ,MAD.*        
	  ...
	  --			 as AD_Amount_on_basic_for_per,
	   ...
	   , case when GA.ad_mode = '%' then  dbo.F_Remove_Zero_Decimal(EED.E_AD_Amount)
