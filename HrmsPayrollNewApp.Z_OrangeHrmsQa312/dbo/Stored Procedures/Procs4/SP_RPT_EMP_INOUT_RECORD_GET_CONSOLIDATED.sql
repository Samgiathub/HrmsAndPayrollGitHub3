...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET_CONSOLIDATED]      
    @Cmp_ID         NUMERIC,      
    @From_Date      DATETIME,      
    @To_Date        DATETIME ,      
    @Branch_ID      NUMERIC   ,      
    ...
    @Fill_Table		Int =0  --- Add By Jignesh 03-Jan-2020 ---- 
AS      
SET NOCOUNT ON 
...
SET ARITHABORT ON
    ...
    FROM    T0010_COMPANY_MASTER C WITH (NOLOCK) Inner Join T0001_LOCATION_MASTER L WITH (NOLOCK) On C.Loc_ID = L.Loc_ID 
...
---------- Month Table --Update by krushna 06-07-2020----------
...
SET @StartDate = @From_Date --cast(CAST(MONTH( @From_Date) as varchar(5))+'-01-'++CAST(year(@From_Date) as varchar(5)) AS datetime)
SET @EndDate = @To_Date --DATEADD(s,-1,DATEADD(mm, DATEDIFF(m,0, @From_Date)+1,0))
...
insert into #tblMonthDay(CurrentDate)
...
IF @Report_call <> 'Monthly Generate'
    BEGIN   
        IF  OBJECT_ID('tempdb..#Emp_Inout3') IS NOT NULL --exists (SELECT 1 FROM [tempdb].dbo.sysobjects WHERE name like '#Emp_Inout3' )        
            DROP TABLE #Emp_Inout3  
        ...
        CREATE NONCLUSTERED INDEX IX_Emp_Inout ON dbo.#Emp_Inout3 (Emp_ID,for_Date) INCLUDE(In_Time,Out_Time) 
...
Where NOT EXISTS (SELECT 1 from  #Data as I Where  I.emp_id = E.Emp_ID AND I.for_Date = E.CurrentDate)
	...
	Insert INTO #Emp_Inout3 (emp_id,for_Date,In_Time,Out_Time,Duration_sec,P_days,Shift_ID,Shift_St_Datetime,
	Shift_en_Datetime,
	More_Work,
	Total_More_work_Sec,
	A_days
    ...
    Update #Emp_Inout3 SET Shift_ID= DATA.Shift_ID ,Shift_Sec =DATA.Shift_Sec 
    ,Shift_St_Datetime=DATA.Shift_St_Datetime,
	Shift_en_Datetime=DATA.Shift_en_Datetime
     From #Emp_Inout3 AS E Inner Join 
    (
    select  ROW_NUMBER() OVER(PARTITION BY emp_id ORDER BY for_Date DESC) 
    AS Row#, emp_id,Shift_ID,Shift_Sec,for_Date,Shift_St_Datetime,Shift_en_Datetime from #Emp_Inout3
    ...
    Update #Emp_Inout3 SET Total_Less_Work_Sec	= 
	...
	UPDATE #Emp_Inout3 SET Dept_id = Inc_Qry.Dept_id,Grd_ID = Inc_Qry.Grd_ID,
	...
	From  #Emp_Inout3 AS A Inner JOIN 
	...
	UPDATE  #Emp_Inout3 SET AB_LEAVE ='WO' ,A_days =0,WeekOff_Days=1 
	,Shift_ID= NULL 
	,Shift_Sec =0
    ,Shift_St_Datetime=NULL,
	Shift_en_Datetime=NULL
FROM  #Emp_Inout3 AS EI Inner JOIN
...
-------------- Holiday / Week Off -----------
		...
		--------------------End Holiday Cursor---------------------
		UPDATE  #Emp_Inout3 SET AB_LEAVE ='HO' ,A_days =0 FROM  #Emp_Inout3 AS EI 
		Inner JOIN
		(select Emp_ID,HoDate as HoDate from #EmpHoliday )as HO
		ON EI.emp_id = HO.Emp_ID And EI.for_Date= cast(HO.HoDate as date)
	
		-------- Leave Details ------------
			------------ Add by Jignesh Patel 29-12-2021--------- Ref. Sajid & Sandip
		---	  And (Leave_Type <> 'Company Purpose' AND Leave_Code <> 'COMP')


		Update E SET AB_LEAVE = 
		case when left(Leave_Desc ,1)='-' Then replace(Leave_Desc ,'-','')else Leave_Desc End

		------------ Modifty by Jignesh Patel 29-12-2021--------- Ref. Sajid & Sandip
		----,A_days= case When E.AB_LEAVE LIKE '%WO%' THEN 0 ELSE  1-(ISNULL(E.P_days,0)+Isnull(Leave_Used,0)) END 
		,A_days= case When E.AB_LEAVE LIKE '%WO%'  And (Leave_Type <> 'Company Purpose' AND Leave_Code <> 'COMP') THEN 0 ELSE  1-(ISNULL(E.P_days,0)+Isnull(Leave_Used,0)) END 


		------------ Modifty by Jignesh Patel 29-12-2021--------- Ref. Sajid & Sandip
		----,Leave_Days=Isnull(Leave_Used,0)
		,Leave_Days= case when  (Leave_Type <> 'Company Purpose' AND Leave_Code <> 'COMP') then Isnull(Leave_Used,0) else 0 end

		,WeekOff_Days = CASE WHEN E.AB_LEAVE LIKE '%WO%' THEN WeekOff_Days-Leave_Used ELSE 0 END
		,Temp_LvDays =Leave_Used_1
		FROM #Emp_Inout3 E INNER JOIN
		(select 
		...
		(
		SELECT 
		Lm.Leave_Type,Lm.Leave_Code,
		p1.Emp_ID,p1.For_Date,
			...
			CASE WHEN  LM.APPLY_HOURLY =1 THEN (p1.Leave_Used*.125) ELSE  0 END END  Leave_Used_1,
				...
				GROUP BY p1.Emp_ID,p1.For_Date,LM.APPLY_HOURLY,Leave_Code,Leave_Used,Compoff_Used,Leave_Assign_As ,Lm.Leave_Type
			) as Qry 
			group BY Emp_ID,For_Date,Leave_Desc ,Leave_Code,Leave_Type
		)as TLeave 
		ON E.emp_id = TLeave.Emp_ID And E.for_Date = TLeave.For_Date 
		WHERE RowNo =1


		Update #Emp_Inout3 SET A_days =isnull(Leave_Days,0)-isnull(Temp_LvDays,0),Leave_Days= isnull(Temp_LvDays  ,0)
		Where AB_LEAVE LIKE '%LWP%' AND Temp_LvDays>0 AND WeekOff_Days = 0


		Update #Emp_Inout3 SET A_days =Leave_Days ,Leave_Days=0   Where AB_LEAVE LIKE '%LWP%' AND WeekOff_Days>0

		Update #Emp_Inout3 SET P_days=0,A_days=0,Leave_Days=0,WeekOff_Days=0,AB_LEAVE='-'
		From  #Emp_Inout3 AS A Inner JOIN 
...
---select * from #Emp_Inout3
 /* -----13-Nov-2019----  
            ...
            WHILE @Temp_Month_Date <= @Temp_End_Date      
                    ...
                    BEGIN      
                     ...
                     --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 5:007:003'
                        ...
                        FROM    dbo.T0040_SHIFT_MASTER SM inner join           
                                (SELECT distinct Shift_ID FROM #Emp_Inout3 ) q on SM.Shift_ID =  q.shift_ID          
                        WHERE   Is_Half_Day = 1   
                        ...
                        IF EXISTS(SELECT EI.Emp_ID  FROM #Emp_Inout3 AS EI  WHERE EI.Emp_ID = @Emp_ID and EI.For_DAte = @temp_Month_DAte )  
                            BEGIN  
                                --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 5:007:013'
                                SELECT  @Toatl_Working_sec = isnull(sum(Duration_Sec) ,0)  
                                FROM    #Emp_Inout3 AS EI  WHERE EI.Emp_ID = @Emp_ID and EI.For_DAte = @temp_Month_DAte  
     
                                SELECT  @Working_Sec_AfterShift = isnull(sum(Working_Sec_AfterShift) ,0)  
                                FROM    #Emp_Inout3 AS EI  
                                ...
                                SELECT @Min_In_Time = MIN(In_Time) FROM #Emp_Inout3 AS EI WHERE EI.Emp_ID = @Emp_ID and EI.For_DAte = @temp_Month_Date 
                       ...
                       End
                        ...
                        INSERT INTO #Emp_Inout3 (Emp_id,For_Date , Dept_id ,Grd_ID,Type_ID,Desig_ID ,Shift_ID,In_Time ,Out_Time ,Duration ,      
                        ...
                        IF exists(SELECT EI.Emp_ID  FROM #Emp_Inout3 AS EI  WHERE EI.Emp_ID = @Emp_ID AND EI.For_DAte = @temp_Month_DAte )  
                            BEGIN  
                                ...
                                SELECT top(1) @Late_In_N = Late_In FROM #Emp_Inout3 AS EI WHERE EI.Emp_ID = @Emp_ID and EI.For_DAte = @temp_Month_DAte 
                        ...
                        SELECT @Min_In_Time = MIN(In_Time) FROM #Emp_Inout3 AS EI WHERE EI.Emp_ID = @Emp_ID   
                        and EI.For_DAte = @temp_Month_Date 
       
                        update #Emp_Inout3 SET   
                ...
                IF CHARINDEX(CAST(@Temp_Month_Date AS VARCHAR(11)),@StrHoliday_Date,0) > 0   
                    BEGIN  
                        ...
                        --  Begin
                        --      select @StrHoliday_Date--* from #Emp_Inout3
                        --  End 
                        IF not exists (SELECT 1 FROM #Emp_Inout3 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date)  
                            BEGIN  
                                --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 5:009'                                
                                INSERT INTO #Emp_Inout3   
                                ...
                                from #Emp_Inout3 EI inner join #Emp_WeekOff_Holiday EWH on EI.Emp_ID=EWH.Emp_ID
                        ...
                        ELSE  
                            BEGIN  
                                ...
                                IF @Emp_OT = 1
                                    --Update #Emp_Inout3 SET AB_LEAVE = 'HO', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End,Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date  
                                    IF CHARINDEX(cast(@Temp_Month_Date AS VARCHAR(11)),@StrCancelHoliday_Date,0) > 0    /* IF Sandwich Policy Applicable and employee has present on Holiday Then cancel Holiday    --Ankit 08012015  */
                                        Update #Emp_Inout3 SET AB_LEAVE = '', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End,Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date  
                                    ELSE
                                        Update #Emp_Inout3 SET AB_LEAVE = 'HO', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End,Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date    
                                Else
                                    Update #Emp_Inout3 SET AB_LEAVE = 'HO',Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date
                            END  
                    END   
                    ...
                    BEGIN  
                        ...
                        ELSE  
                            BEGIN  
                                ...
                                IF @Emp_OT = 1
                                    --Update #Emp_Inout3 SET AB_LEAVE = 'WO', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End, Total_Less_work_Sec = 0,Less_Work = '',Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date      
                                    IF CHARINDEX(cast(@Temp_Month_Date AS VARCHAR(11)),@StrCancelWeekoff_Date,0) > 0
                                        UPDATE #Emp_Inout3 SET AB_LEAVE = '', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End, Total_Less_work_Sec = 0,Less_Work = '',Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date  
                                    ELSE
                                        UPDATE #Emp_Inout3 SET AB_LEAVE = 'WO', Total_More_work_Sec = Case When @Tras_Weekoff_OT = 1 Then Total_Work_Sec End, More_Work = Case When @Tras_Weekoff_OT = 1 Then Total_work End, Total_Less_work_Sec = 0,Less_Work = '',Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date    
                                ELSE
                                    UPDATE #Emp_Inout3 SET AB_LEAVE = 'WO',Shift_Sec = 0 WHERE emp_id = @Emp_ID And for_Date = @Temp_Month_Date  
                            END  
                    END     
                            ...
                            BEGIN
                                ...
                                IF UPPER(@Country_Name) <> 'PAKISTAN'
                                    BEGIN   
                                        ...
                                        Update #Emp_Inout3 SET AB_LEAVE = 
                ...
                Duration_sec  , Late_In ,Late_Out , Early_In , Early_Out , Leave ,Shift_Sec,Shift_Dur,Total_Work,Less_Work,More_work,Reason,Late_in_sec,Early_Out_sec,Total_Less_work_sec,Late_in_Count,Early_Out_count,Shift_St_Datetime,Shift_en_Datetime      
                 ...
                 Else 
                                ...
                                FROM    #Emp_Inout3 EI LEFT OUTER JOIN #Data D ON EI.Emp_ID=D.Emp_ID AND EI.For_Date=D.For_Date
  ...
  ----------- */
    ...
    --Update E SET A_days = 1 FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE d.P_days=0 And AB_LEAVE Is null --And E.Emp_Id = @Emp_Id
    --Update E SET A_days = 1 - (IsNull(Leave_Days, 0) + D.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE d.P_days=0.5 ---And AB_LEAVE Is null --And E.Emp_Id = @Emp_Id            
    
    ----Update E SET AB_LEAVE =(CASE WHEN D.P_DAYS = 0.25 OR  D.P_DAYS = 0.375 OR D.P_DAYS = 0.125 THEN 'QD' END ), A_days = 1 - (IsNull(Leave_Days, 0) + D.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE D.P_DAYS IN (0.125, 0.25, 0.375 , 0.5 , 0.625,  0.75, 0.875)
    ----Update E SET AB_LEAVE = (CASE WHEN D.P_DAYS = 0.75 OR D.P_DAYS = 0.625 OR D.P_DAYS = 0.875 THEN '3QD' END ), A_days = 1 - (IsNull(Leave_Days, 0) + D.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE D.P_DAYS IN (0.125, 0.25, 0.375 , 0.5 , 0.625,  0.75, 0.875)
                    
    Update E SET A_days = 1 - (IsNull(Leave_Days, 0) + E.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE  E.P_days=0.5 
	Update E SET A_days = 0 FROM #Emp_Inout3 E WHERE IsNull(E.Leave_Days, 0) = 1 --- added by Hardik 05/10/2020 for Honda as showing absent on Full Day leave as punches are there.
    --2000
    ---Update E SET A_days = 0.5 FROM  #Emp_Inout3 E WHERE (ISNULL(AB_LEAVE,'') LIKE '%SH%' OR ISNULL(AB_LEAVE,'') LIKE '%FH%') AND E.P_days =0
        
    Update  E 
    SET     P_days = D.P_days 
    FROM    #Data D 
            INNER JOIN #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date and Total_work <> '' And total_work <> '-' 
    
       
    Update E SET AB_LEAVE = 'HF' FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days=0.5 And E.A_Days=0.5 --And E.Emp_Id = @Emp_Id
    
    Update E SET AB_LEAVE = AB_LEAVE + '-C' FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days = 1 and (AB_Leave = 'HO' or AB_Leave = 'WO' or AB_Leave = 'OHO')
    
    Update E SET AB_LEAVE = 'P' FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.P_days=1 and AB_Leave is null
    
    Update E SET AB_LEAVE = AB_LEAVE + '/W' FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.WeekOff_Days=0.5 
    
    --Update E SET AB_LEAVE = 'AB' FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.A_days=1
    Update E SET AB_LEAVE = CASE WHEN AB_LEAVE LIKE '%LWP%' THEN AB_LEAVE ELSE 'AB' end FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE E.A_days=1
    ...
    CASE WHEN isnull(AB_LEAVE,'') LIKE '%FH%' THEN  isnull(AB_LEAVE,'AB')+'/QD' ELSE 'QD/'+isnull(AB_LEAVE,'AB') END
     END ), A_days = 1 - (IsNull(Leave_Days, 0) + D.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE D.P_DAYS IN (0.125, 0.25, 0.375  )--, 0.625,  0.75, 0.875
    ...
    END ), A_days = 1 - (IsNull(Leave_Days, 0) + D.P_days) FROM #Data D inner join #Emp_Inout3 E on D.Emp_Id = E.emp_id And D.For_date = E.for_Date WHERE D.P_DAYS IN ( 0.625,  0.75, 0.875)
   
   Update #Emp_Inout3 SET AB_LEAVE = 
   CASE WHEN AB_LEAVE LIKE '%SH%' Then
		SUBSTRING(AB_LEAVE,CHARINDEX('/', AB_LEAVE)+1,LEN(AB_LEAVE))+'/'+SUBSTRING(AB_LEAVE,0,CHARINDEX('/', AB_LEAVE)) 
   ELSE AB_LEAVE END

   --- When give version to Honda, Uncomment Below Code, it's specific for Emerland honda, 21/07/2020

	---- Added by Hardik for Emerland Honda on 21/07/2020
		...
		--UPDATE #Emp_Inout3 SET LATE_IN = NULL, EARLY_OUT=NULL, Late_In_count = 0,  Early_Out_Count = 0
	
		--UPDATE L SET LATE_IN = LED.Late_Sec, EARLY_OUT = LED.Early_Sec , Late_In_Count = Late_Deduct_Days, Early_Out_Count =Early_Deduct_Days 
		--FROM #Emp_Inout3 L INNER JOIN #Late_Early_Deduction LED ON L.EMP_ID = LED.Emp_ID AND L.FOR_DATE=LED.For_Date

		--UPDATE Att SET AB_Leave = CASE WHEN Late_Sec > 0 THEN 'PL' WHEN Early_Sec >0 THEN 'PE' ELSE AB_Leave END -- Added by Hardik for Emerland Honda
		--FROM #Emp_Inout3 ATT INNER JOIN #Late_Early_Deduction LED ON ATT.EMP_ID = LED.Emp_ID AND ATT.FOR_DATE = LED.For_Date
		--WHERE  ATT.AB_Leave <> 'A' and LED.Late_Deduct_Days = 0 and LED.Early_Deduct_Days=0 

		--UPDATE Att SET AB_Leave = 'P2' -- Added by Hardik for Emerland Honda
		--FROM #Emp_Inout3 ATT INNER JOIN #Late_Early_Deduction LED ON ATT.EMP_ID = LED.Emp_ID AND ATT.FOR_DATE = LED.For_Date
		--WHERE  ATT.AB_Leave <> 'A' and (LED.Late_Deduct_Days>0 OR LED.Early_Deduct_Days>0) And P_days<>0.5 

		--UPDATE Att SET AB_Leave = 'P2' -- Added by Hardik for Emerland Honda
		--FROM #Emp_Inout3 ATT 
		--WHERE  ATT.AB_Leave = 'HF'
   	...
   	If @Fill_Table =1 
   	Begin
   		Insert INTO #TempEmp_Inout Select * from #Emp_Inout3
   		Return
   	End
    ...
    SELECT @cols =
      STUFF(( SELECT ',' + QUOTENAME(cast(day(Convert(VARCHAR(max),Q.for_date,106)) AS VARCHAR(2)) + '_' + left(datename(dw,Q.for_date),2)) AS ColName FROM 
            (SELECT Distinct cast(T.For_Date as date) as For_date FROM dbo.#Emp_Inout3 t) Q Order by Q.For_Date  
    ...
    --      From 
    --      (SELECT Distinct T.For_Date FROM dbo.#Emp_Inout3 t) Q Order by Q.For_Date  
    --         FOR XML PATH(''), TYPE).value ('.', 'nVARCHAR(max)'), 1, 1, '')     
    ...
    SELECT Emp_Id,For_Date,MIN(In_Time) AS Min_In_Time,MAX(Out_Time) AS Max_Out_time into #Emp_Inout1 FROM #Emp_Inout3 Group by Emp_Id,For_Date
    ...
    SET @query =
     'Insert Into #Consolidated_Temp
      SELECT * FROM (
            ...
            FROM dbo.#Emp_Inout3 t Inner Join #Emp_Inout1 t1 on t.Emp_Id = t1.Emp_Id And t.For_Date = t1.For_date CROSS APPLY (
    ...
    (
        SELECT Emp_ID,Leave_ID,Max(For_Date) AS For_Date  FROM T0140_LEAVE_TRANSACTION WITH (NOLOCK) group by Emp_ID,Leave_ID
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            SET @Leave_Name_1=replace(@Leave_Name_1,'-','_')
            SET @test ='alter  table #Emp_Inout3 ADD ['+ @Leave_Name_1 +']  decimal(18,2) default ''''' 
            ...
            --SET @test1 ='Update EI  SET '+ @Leave_Name_1 + ' = LB.Leave_Closing FROM #Emp_Inout3 EI inner join #Leave_Balance LB on  LB.EMP_ID = EI.Emp_ID and replace(LB.Leave_Name,'' '',''_'') = ''' + @Leave_Name_1 + ''''
            SET @test1 ='Update EI  SET ['+ @Leave_Name_1 + '] = LB.Leave_Closing FROM #Emp_Inout3 EI inner join( SELECT Min(For_Date) AS For_Date FROM #Emp_Inout3 group by Emp_ID) qry on qry.For_Date = EI.For_Date inner join #Leave_Balance LB on  LB.EMP_ID = EI.Emp_ID and replace(LB.Leave_Name,'' '',''_'')+''_Closing'' = ''[' + @Leave_Name_1 + ']'''
    ...
    Update #Consolidated_Temp SET P_days = Isnull(E.P_days,0)
        From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(P_days) P_Days FROM #Emp_Inout3 Group by emp_id) E on T.Emp_Id=E.emp_id 
    Where Pvtcolumns = 'In_Time'

    Update #Consolidated_Temp SET A_days = E.A_days 
        From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(A_days) A_Days FROM #Emp_Inout3 Group by emp_id) E on T.Emp_Id=E.emp_id 
    Where Pvtcolumns = 'In_Time'

    --Update #Consolidated_Temp SET Weekoff = E.Weekoff
    --    From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Count(AB_LEAVE) Weekoff FROM #Emp_Inout3 WHERE AB_LEAVE = 'WO' or AB_LEAVE = 'WO-C'  Group by emp_id) E on T.Emp_Id=E.emp_id 
    --Where Pvtcolumns = 'In_Time'

     Update #Consolidated_Temp SET Weekoff = E.Weekoff
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(WeekOff_Days) Weekoff FROM #Emp_Inout3 Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time'
  
    Update #Consolidated_Temp SET Holiday = E.Holiday
        From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Count(AB_LEAVE) Holiday FROM #Emp_Inout3 WHERE AB_LEAVE = 'HO' or AB_LEAVE = 'HO-C' or AB_Leave = 'OHO' Group by emp_id) E on T.Emp_Id=E.emp_id 
    Where Pvtcolumns = 'In_Time'
    --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 12'
	
	---Add By Jignesh 19-12-2019----
	Update #Consolidated_Temp SET Leave = E.Leave
        From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Leave_Days) Leave FROM #Emp_Inout3 where Isnull(Leave_Days,0)>0 Group by emp_id) E on T.Emp_Id=E.emp_id 
    Where Pvtcolumns = 'In_Time'
    ...
    if @Comp_OD_As_Present = 1
        begin
		 ...
		 -- END Added by Sajid For Add Customized Column on 06122021  


 	 ----------Added by Prapti---------- Salary Days /    Leave Days /    Late/Early ------------START(11072022)--------------------

	 ------------------ Salary Days -----------------------

		ALTER TABLE #Final_Table ADD Sal_Cal_Days NUMERIC(12,2) default 0;
		Drop Table IF Exists ##pday 
		
		 CREATE TABLE #pday
		 (    
			 PAYABLE_DAYS NUMERIC(9,2), 
			 CODE VARCHAR(50)
			 ,Emp_ID integer
		 )  


		exec SP_RPT_EMP_ATTENDANCE_GET_PAYBLE_DAYS @Cmp_ID=@Cmp_ID,@From_Date=@From_Date,@To_Date=@To_Date,
		@Branch_ID=0,@Cat_ID=0,@Grd_ID=0,@Type_ID=0,@Dept_ID=0,@Desig_ID=0,@Emp_ID=0,@Constraint=@Constraint,@Report_For='',@Type=0,@Order_By=@Order_By
	

		UPDATE F SET F.Sal_Cal_Days = m.PAYABLE_DAYS
		FROM #Final_Table as F INNER JOIN ##pday as m 
		ON F.Emp_ID = m.Emp_ID 
		where OrderColumns =1 

		--select * from ##pday
		
		--declare @start1 as int = 0
		--declare @limit1 as int = 1
		--declare @emp_count1 int

		--select m.Emp_ID into #emp_count1 from ##pday as m 
		--inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID 
		--where F.Emp_ID = m.Emp_ID 
		--group by m.Emp_ID
		--order by m.Emp_ID

		--select @emp_count1 = count(Emp_ID) from #emp_count1


		--WHILE @start1 <= (@emp_count1)
  --      BEGIN

		--	 Declare @UDEMPID1 as int
		
		--		select @UDEMPID1 = m.Emp_ID from ##pday as m 
		--		inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID 
		--		where F.Emp_ID = m.Emp_ID 
		--		group by m.Emp_ID 
		--		order by m.Emp_ID
		--		OFFSET @start1 ROWS
		--	    FETCH NEXT @limit1 ROWS ONLY
		--	    SET @start1 = @start1 + 1
		
			
		--		UPDATE F SET 
		--		F.Sal_Cal_Days = m.PAYABLE_DAYS
		--		FROM #Final_Table as F INNER JOIN ##pday as m 
		--		ON F.Emp_ID = m.Emp_ID 
		--		where OrderColumns =1 and F.Emp_ID=@UDEMPID1
  ...
  --      End
	...
	DECLARE @cols2 AS NVARCHAR(MAX),
		...
		select @cols3 = STUFF((SELECT distinct ',' + 'Case When OrderColumns = 1 Then ' + QUOTENAME(Leave_Code) + ' Else Null End AS ' + QUOTENAME(Leave_Code)
					   from T0130_LEAVE_APPROVAL_DETAIL as lad inner join T0120_LEAVE_APPROVAL as la
		on lad.Leave_Approval_ID = la.Leave_Approval_ID
		inner join T0040_LEAVE_MASTER as Lm on lm.Leave_ID = lad.Leave_ID
		where lad.Cmp_ID = @Cmp_ID and la.Emp_ID in (select Emp_ID from  #Emp_Cons) and Approval_Status = 'A' and From_Date >= @From_Date and To_Date <= @To_Date
					FOR XML PATH(''), TYPE
					).value('.', 'NVARCHAR(MAX)') 
					,1,1,'')


	select @cols2 = STUFF((SELECT distinct ',' + QUOTENAME(Leave_Code)
				   from T0130_LEAVE_APPROVAL_DETAIL as lad inner join T0120_LEAVE_APPROVAL as la
					on lad.Leave_Approval_ID = la.Leave_Approval_ID
					inner join T0040_LEAVE_MASTER as Lm on lm.Leave_ID = lad.Leave_ID
					where lad.Cmp_ID = @Cmp_ID and la.Emp_ID in (select Emp_ID from  #Emp_Cons) and Approval_Status = 'A' and From_Date >= @From_Date and To_Date <= @To_Date
	...
	DROP TABLE IF EXISTS temp1234

			
	if @cols2 is not null
	Begin
		set @query2 = 'SELECT  '+ @cols2 +',Emp_ID into temp1234 from 
				 (
					select Leave_Code,Leave_Period,Emp_ID 
					 from T0130_LEAVE_APPROVAL_DETAIL as lad inner join T0120_LEAVE_APPROVAL as la on lad.Leave_Approval_ID = la.Leave_Approval_ID
					 inner join T0040_LEAVE_MASTER as Lm on lm.Leave_ID = lad.Leave_ID where lad.Cmp_ID = '+cast(@Cmp_ID as varchar(300))+' and  Approval_Status = ''A''  
					 and From_Date >= '''+convert(varchar, @From_Date, 23)+''' 
					 and To_Date <= '''+ convert(varchar, @To_date, 23) +'''
					 and la.Emp_ID in (select Emp_ID from  #Emp_Cons)
				) x
				pivot 
				(  sum(Leave_Period)
					for Leave_Code in (' + @cols2 + ')
				) p '
	End
	Else
	Begin
	
	   set @query2 = 'SELECT  distinct(Emp_ID) into temp1234 from 
				 (
					select Leave_Code,Leave_Period,Emp_ID 
					 from T0130_LEAVE_APPROVAL_DETAIL as lad inner join T0120_LEAVE_APPROVAL as la on lad.Leave_Approval_ID = la.Leave_Approval_ID
					 inner join T0040_LEAVE_MASTER as Lm on lm.Leave_ID = lad.Leave_ID where lad.Cmp_ID = '+cast(@Cmp_ID as varchar(300))+'
	...
	--exec SP_RPT_EMP_INOUT_RECORD_GET_for_Defecit @Cmp_ID=120,@From_Date='2022-06-01 00:00:00',@To_Date='2022-06-30 00:00:00',@Branch_ID=0,@Cat_ID=0,
	--@Grd_ID=0,@Type_ID=0,@Dept_ID=0,@Desig_ID=0,@Emp_ID=0,@Constraint='27948',@IN_TIME_FROM='',@IN_TIME_TO='',@OUT_TIME_FROM='',@OUT_TIME_TO=''        

	
		exec SP_RPT_EMP_INOUT_RECORD_GET_for_Defecit @Cmp_ID=@Cmp_ID,@From_Date=@From_Date,@To_Date=@To_Date,@Branch_ID=@Branch_ID,@Cat_ID=@Cat_ID,
		@Grd_ID=@Grd_ID,@Type_ID=@Type_ID,@Dept_ID=@Dept_ID,@Desig_ID=@Desig_ID,@Emp_ID=@Emp_ID,@Constraint=@Constraint,@IN_TIME_FROM='',@IN_TIME_TO='',@OUT_TIME_FROM='',@OUT_TIME_TO=''        


		ALTER TABLE #Final_Table ADD Total_Dedcution Numeric(18,2),Late_Deduction_1 Numeric(18,2),Early_Deduction_1 Numeric(18,2),Short_Hours varchar(100)

		Declare @total_Deducation Numeric(18,2),@total1 Numeric(18,2),@total2 Numeric(18,2), @Hour nvarchar(100), @Minut nvarchar(100), @sec int , @Defict varchar(100)
 

		select Late_Deduction,For_Date,m.Emp_ID into #ld from #Emp_Late_Early as m inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID group by For_Date,Late_Deduction,m.Emp_ID 
		select Early_Deduction,For_Date,m.Emp_ID into #ed from #Emp_Late_Early as m inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID group by Early_Deduction,For_Date,m.Emp_ID 
		select Total_Less_Work_Sec,For_Date,m.emp_id into #def from ##defecit as m inner join #Final_Table as F ON F.Emp_ID = m.emp_id group by Total_Less_Work_Sec,For_Date,m.emp_id 
	
		declare @start as int = 0
		declare @limit as int = 1
		declare @emp_count int

		select m.Emp_ID into #emp_count from #Emp_Late_Early as m 
		inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID 
		where F.Emp_ID = m.Emp_ID 
		group by m.Emp_ID
		order by m.Emp_ID

		select @emp_count = count(Emp_ID) from #emp_count

		--select Emp_ID from #emp_count

		WHILE @start <= (@emp_count)
        BEGIN
		     
			 --set @emp_count = @emp_count- 1

			 Declare @UDEMPID as int
		
				select @UDEMPID= m.Emp_ID from #Emp_Late_Early as m 
				inner join #Final_Table as F ON F.Emp_ID = m.Emp_ID 
				where F.Emp_ID = m.Emp_ID 
				group by m.Emp_ID 
				order by m.Emp_ID
				...
				select @total1 = sum(Late_Deduction) from #ld  where Emp_ID = @UDEMPID
				select @total2 = sum(Early_Deduction) from #ed where Emp_ID = @UDEMPID
		
		
				select @sec = sum(Total_Less_Work_Sec) from #def where Emp_ID = @UDEMPID
				set @Hour =  @sec/3600 
				set @Minut =  @sec % 3600 / 60 
				select  @Defict = @Hour + ':'+ @Minut
		
				select  @total_Deducation = (@total1 + @total2) from #Emp_Late_Early as m 
				inner join  #Final_Table as F ON F.Emp_ID = m.Emp_ID
				where F.Emp_ID = @UDEMPID
		
				UPDATE F SET 
				Total_Dedcution = @total_Deducation,
				F.Late_Deduction_1 = @total1, 
				F.Early_Deduction_1 = @total2
				,F.Short_Hours = @Defict
				FROM #Final_Table as F INNER JOIN #Emp_Late_Early as m 
				ON F.Emp_ID = m.Emp_ID where OrderColumns = 1 and F.Emp_ID=@UDEMPID
		
 
		
        End
	  	 --------------------------------------------- Salary Days /    Leave Days /    Late/Early -----------END-----------------------
	...
	SET @query = 'SELECT Case When OrderColumns = 1 Then Row_No Else NULL End AS Sr_No,
						...
						CC.*, --Added by Sajid For Add Customized Column on 06122021  
						...
						Case When OrderColumns = 1 Then Short_Hours Else Null End Short_Hours, --added by prapti  22072022
                        Case When OrderColumns = 1 Then OT_Hours Else Null End OT_Hours,
						Case When OrderColumns = 1 Then Total_Dedcution Else Null End Total_Dedcution,	--added by prapti 11072022
						Case When OrderColumns = 1 Then Late_Deduction_1 Else Null End Late_Deduction,	--added by prapti  11072022
						Case When OrderColumns = 1 Then Early_Deduction_1 Else Null End Early_Deduction,  --added by prapti  11072022
						Case When OrderColumns = 1 Then Sal_Cal_Days Else Null End Sal_Cal_Days, --added by prapti  11072022
						'+  @cols3 + ', --added by prapti  11072022
						'+  @Col_Str + '
                 ...
                 From temp1234 t1 right join  #Final_Table F on t1.Emp_id = F.Emp_id  --added by prapti 08072022
				 	Inner Join 
                (SELECT Emp_Id,Grd_Id,Branch_Id,Desig_ID,Dept_Id,Type_Id, dbo.F_Return_Hours(sum(late_in_sec)) AS Late_In_Hours,
                        ...
                        count(case when Early_Out_sec = 0 then Null else 1 end) AS Early_Days,
						dbo.F_Return_Hours(SUM(Shift_Sec)) AS Total_Hours,dbo.F_Return_Hours(SUM(Total_Work_Sec)) AS Total_Require, 
						dbo.F_Return_Hours(SUM(Total_Less_work_sec)) AS Total_Short_hours,
						'+ @Col_Str_Sum + '
                    From #Emp_Inout3 Group by Emp_Id,Grd_Id,Branch_Id,Desig_ID,Dept_Id,Type_Id) Q on F.Emp_Id = Q.Emp_Id 
					inner Join 
                    (SELECT I.Emp_Id,I.Increment_ID,I.Vertical_ID,I.SubVertical_ID,I.subBranch_ID,I.SalDate_id,I.Segment_ID,I.Increment_effective_Date FROM dbo.T0095_Increment I WITH (NOLOCK) inner join dbo.T0080_Emp_Master e WITH (NOLOCK) on i.Emp_ID = E.Emp_ID inner join
                ...
                T0030_Branch_Master B WITH (NOLOCK) On Q.Branch_Id = B.Branch_Id Inner Join
                T0080_Emp_Master E WITH (NOLOCK) on Q.Emp_Id = E.Emp_Id Left Outer Join
                T0040_Grade_Master G WITH (NOLOCK) on Q.Grd_Id = G.Grd_Id Left Outer Join
                T0040_DEPARTMENT_MASTER D WITH (NOLOCK) on Q.Dept_Id = D.Dept_Id Left Outer Join
                T0040_Designation_Master DD WITH (NOLOCK) on Q.Desig_Id = DD.Desig_Id Left Outer Join
                T0040_Type_Master T WITH (NOLOCK) on Q.Type_Id = T.Type_Id left join
                --T0095_Emp_Salary_Cycle ESC WITH (NOLOCK) on ESC.Emp_ID=Q.Emp_ID left join
                T0040_Salary_Cycle_Master SCM WITH (NOLOCK) on SCM.Tran_ID=Qry.SalDate_id left join
                T0040_Business_Segment BS WITH (NOLOCK) on BS.Segment_ID=Qry.Segment_ID left join
                T0040_Vertical_Segment VS WITH (NOLOCK) on VS.Vertical_ID=Qry.Vertical_ID left join
                T0050_SubVertical SV WITH (NOLOCK) on SV.SubVertical_ID=Qry.SubVertical_ID left join
                T0050_SubBranch SB WITH (NOLOCK) on SB.SubBranch_ID=Qry.SubBranch_ID inner join 
                (SELECT Row_Number() OVER(Order By ' + @Order_By + ') AS Row_No, T.emp_id AS empid FROM 
                    (SELECT Emp_ID,Desig_ID FROM #Emp_Inout3 Group By Emp_ID,Desig_ID) T 
					INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON T.Emp_ID=E.Emp_ID 
					left outer join T0040_Designation_Master DD WITH (NOLOCK) on DD.Desig_Id = T.Desig_ID) RID on RID.empid=Q.Emp_ID
				left outer join #Cust_Column CC WITH (NOLOCK) on CC.Emp_ID = F.Emp_Id --Added by Sajid For Add Customized Column on 06122021  
