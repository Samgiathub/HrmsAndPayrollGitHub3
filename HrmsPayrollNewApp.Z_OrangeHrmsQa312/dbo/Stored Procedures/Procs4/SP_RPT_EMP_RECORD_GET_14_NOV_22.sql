...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_RECORD_GET_14_NOV_22]        
  @Cmp_ID  numeric        
 ,@From_Date  datetime        
 ,@To_Date  datetime         
 ...
 ,@Is_Active tinyint = 0        
 ,@Type varchar(50)=''        
 ,@Project_ID NUMERIC = 0 --ADDED BY MEHUL ON 14112022
AS        
 SET NOCOUNT ON         
  ...
  BEGIN       
   ...
   BEGIN     
    ...
    ( SELECT distinct I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Qry.Bank_ID,Qry.Inc_Bank_AC_No,I.Payment_Mode,Vertical_ID,SubVertical_ID,I.Emp_Fix_Salary         
      ...
      I.Emp_ID = Qry.Emp_ID AND I.Increment_Effective_Date = Qry.Increment_Effective_Date  and I.Increment_ID = Qry.Increment_ID  ) I_Q  ON E.Emp_ID = I_Q.Emp_ID and E.Cmp_ID = @Cmp_ID INNER JOIN        
  ...
  END   
   ...
   END        
    ...
    ( SELECT distinct I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Qry.Bank_ID,Qry.Inc_Bank_AC_No,I.Payment_Mode,Vertical_ID,SubVertical_ID,I.Emp_Fix_Salary         
	  ...
	  LEFT OUTER JOIN T0050_TS_Project_Detail PDL WITH (NOLOCK) ON PDL.Cmp_ID = E.Cmp_ID
    WHERE E.Cmp_ID = @Cmp_Id         
    AND E.Date_Of_Join <= ISNULL(e.Emp_Left_Date , @To_Date)        
    AND I_Q.Payment_Mode = (CASE WHEN (@Payment_Mode = '--Select--' or @Payment_Mode = '')        
            THEN I_Q.Payment_Mode        
