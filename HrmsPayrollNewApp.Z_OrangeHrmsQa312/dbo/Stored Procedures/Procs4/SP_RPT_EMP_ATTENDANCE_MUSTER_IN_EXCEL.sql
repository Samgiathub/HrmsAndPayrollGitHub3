CREATE PROCEDURE [dbo].[SP_RPT_EMP_ATTENDANCE_MUSTER_IN_EXCEL]    
  @CMP_ID   NUMERIC    
 ,@FROM_DATE  DATETIME    
 ,@TO_DATE   DATETIME    
 ,@BRANCH_ID  NUMERIC    
...
SET ARITHABORT ON    
 ...
 (          
  ...
  Increment_ID numeric    
     ...
     if (@EMP_ID is not null) and (@EMP_ID <> 0)
	 Begin
	    print @EMP_ID
 ...
 AND STATUS NOT LIKE '%LC%' ---Add By Jignesh 19-01-2021-----    
	...
	--Added by ronakk 29112022
	 UPDATE DBO.#ATT_MUSTER_EXCEL SET STATUS = STATUS  +  '-EC'  WHERE STATUS_2 = 'EC'     
 AND STATUS NOT LIKE '%EC%' 
    --Added by ronakk 29112022
 ...
 */-----------------------    
 ...
 SELECT A.EMP_CODE,A.EMP_FULL_NAME
 ,CASE WHEN ISNULL(STATUS_2,'') = '*' THEN STATUS + STATUS_2 ELSE A.STATUS END AS STATUS
 ,ISNULL(STATUS_2,'') AS  STATUS_2, P_DAYS,A_DAYS,WO_HO_DAY,  --Change by Jaina 15-12-2017 in Status (Hightlight latemark)    
   CASE ROW_ID      
    WHEN 32    
     THEN 'P'     
    WHEN 33    
 ...
 */    
     ...
     FROM T0040_GENERAL_SETTING G WITH (NOLOCK) INNER JOIN     
      ...
      FROM T0095_INCREMENT INC1 WITH (NOLOCK) INNER JOIN #CROSSTAB T ON INC1.Emp_ID=T.EMP_ID    
        INNER JOIN (    
		 ...
		 group by INC2.Emp_ID  -- ADDED by Deepal To resolved the bug ID 22258
 ...
 ELSE    
  ...
  WHILE @@FETCH_STATUS = 0    
   BEGIN    
    IF NOT(ISNULL(@OD_COMOFF_SETTING,0) = 1 or @Leave_Type='Company Purpose') --Mukti(27032017)    
     BEGIN    
 ...
 --ALTER TABLE   #CROSSTABDATA DROP COLUMN EMP_ID    
	 ...
	 --update #TEMP set LC = LC + EARLY_DEDUCT_DAYS --Added by ronakk 04102022


     SELECT * into ##TempWo FROM #TEMP   --Change by ronakk 17112022  
 ...
 IF ISNULL(@PAGE_SIZE,0) > 0    
  BEGIN    
   ...
   SELECT @COUNT = COUNT(1) FROM ##TempWo    
   ...
   SELECT * 
   FROM    ##TempWo    
   WHERE   SR_NO  BETWEEN ((@PAGE_NO - 1) * @PAGE_SIZE + 1)    
     AND (@PAGE_NO * @PAGE_SIZE)    
 
  END    
 ELSE    
  select * from ##TempWo --Added by ronakk 17112022
  


  Drop Table ##TempWo --Added by ronakk 17112022
