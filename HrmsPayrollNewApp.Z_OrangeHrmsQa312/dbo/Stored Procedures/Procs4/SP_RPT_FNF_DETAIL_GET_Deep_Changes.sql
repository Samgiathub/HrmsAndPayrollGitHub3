...
Create PROCEDURE [dbo].[SP_RPT_FNF_DETAIL_GET_Deep_Changes]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	,@Branch_ID 	numeric
...
SET ARITHABORT ON	
	...
	CREATE TABLE #Pay_slip 
		...
		select ms.Emp_ID,Cmp_ID,null,'Basic Salary',Sal_Tran_ID,Salary_amount,Basic_Salary,0,Month_end_Date 
		,'I',ISNULL(Arear_Basic,0)+ ISNULL(Basic_Salary_Arear_cutoff,0),''
		...
		INSERT INTO #Pay_slip (Emp_ID,Cmp_ID,AD_ID,Sal_Tran_ID,AD_Amount,AD_ACtual_Amount,AD_Calculated_Amount,For_Date,M_AD_Flag,M_AREAR_AMOUNT,Paid_month)
		SELECT distinct mad.Emp_Id,mad.Cmp_ID,mad.AD_ID,NULL,SUM(CASE WHEN MAD.REIMAMOUNT <> 0  THEN MAD.REIMAMOUNT ELSE MAD.M_AD_AMOUNT END),SUM(mad.M_AD_Actual_Per_amount),SUM(mad.M_AD_Calculated_amount),mad.For_Date,mad.M_AD_Flag,MAD.M_AREAR_AMOUNT+ MAD.M_AREAR_AMOUNT_Cutoff , --SUM(CASE WHEN MAD.REIMAMOUNT > 0 COMMENTED BY RAJPUT ON 12032018 QUERY WAS MINUS AMOUNT WAS NOT COME (INDUCTOTHERM CASE WHILE FNF)
				(('(' + CONVERT(varchar(3), FA.From_Date, 100) + '-' + cast(YEAR(FA.From_Date) as varchar(5))) + ' To ' +
				(CONVERT(varchar(3), FA.To_Date, 100) + '-' + cast(YEAR(FA.To_Date) as varchar(5)))+')')
		 FROM T0210_MONTHLY_AD_DETAIL  MAD WITH (NOLOCK) INNER  JOIN 
		...
		WHERE MAD.Cmp_ID = @Cmp_Id AND MONTH(MAD.To_date) = MONTH(@To_Date) AND YEAR(MAD.To_date) = YEAR(@To_Date) 
			  ...
			  (MAD.M_AD_Flag = 'I') AND (AD_NOT_EFFECT_SALARY = 0 OR ISNULL(MAD.ReimShow,0) = 1)-- or (MAD.M_AD_Amount <> 0)) --COMMENTED BY Deepali -RAJPUT ON 12032018 QUERY WAS REIM. ALLOWANCE AMOUNT NOT COME AT FNF TIME (INDUCTOTHERM CLIENT)
...
---Put By Nikunj  14-March-2011--------------
	...
	--	select AD_ID,*from #Pay_slip where Emp_ID= 27199
	...
	Select Emp_full_Name,Grd_Name,Comp_Name,Branch_Address,EMP_CODE,Type_Name,
			...
			left OUTER JOIN	EMP_FOR_FNF_ALLOWANCE F WITH (NOLOCK) ON F.AD_Id = ADM.AD_ID and E.Emp_ID = F.Emp_id

			AND F.FOR_DATE Between @From_Date And @To_Date --Added by Jaina 27-06-2017 (FOR_DATE ADDED BY RAMIZ ON 25092017)	
	
			WHERE E.Cmp_ID = @Cmp_Id
			and (case when MAD.ad_amount = 0 or MAD.ad_amount is null then F.amount else mad.ad_amount end) > 0	
			and isnull( M_AREAR_AMOUNT,0) is not null    --Added by Jaina 31-10-2020
