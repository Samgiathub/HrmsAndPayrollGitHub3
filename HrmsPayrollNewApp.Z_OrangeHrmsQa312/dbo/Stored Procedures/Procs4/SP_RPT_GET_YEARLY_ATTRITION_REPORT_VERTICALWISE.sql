...
-- DESCRIPTION:	<DESCRIPTION,,YEARLY VARTICAL WISE ATTRITION>
---29/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
-- =============================================

CREATE PROCEDURE [dbo].[SP_RPT_GET_YEARLY_ATTRITION_REPORT_VERTICALWISE]
	 @CMP_ID 		NUMERIC
	,@FROM_DATE 	DATETIME
	,@TO_DATE 		DATETIME
	,@BRANCH_ID 	NUMERIC
...
DECLARE @VERTICAL_ID_CUR NUMERIC
DECLARE @VERTICAL_NAME AS VARCHAR(50)
DECLARE @TEMP_DATE DATETIME
DECLARE @COUNT NUMERIC 


DECLARE GROUP_CURSOR CURSOR FOR
	SELECT VERTICAL_ID,VERTICAL_NAME FROM T0040_VERTICAL_SEGMENT WITH (NOLOCK) WHERE CMP_ID=@CMP_ID AND VERTICAL_ID = ISNULL(@VERTICAL_ID,VERTICAL_ID)
OPEN GROUP_CURSOR
	FETCH NEXT FROM GROUP_CURSOR into @VERTICAL_ID_CUR,@VERTICAL_NAME 
	WHILE @@FETCH_STATUS = 0
		BEGIN
				INSERT INTO #YEARLY_ATTRITION_REPORT(CMP_ID,VERTICAL_ID)
				SELECT @CMP_ID,@VERTICAL_ID_CUR		
						...
						BEGIN
							...
							SET MONTH_1_OP = (
												SELECT COUNT(EM.EMP_ID) FROM T0080_EMP_MASTER EM WITH (NOLOCK) INNER JOIN
												 (
														SELECT IE.EMP_ID,IE.Vertical_ID from T0095_INCREMENT IE WITH (NOLOCK) INNER JOIN 
														(
												...
												AND INC_QRY.Vertical_ID =@VERTICAL_ID_CUR AND 
												(
													EMP_LEFT ='N' 
													OR 
													(
							...
							WHERE VERTICAL_ID =@VERTICAL_ID_CUR
							...
							SET MONTH_1_CR = (
												...
												AND YEAR(DATE_OF_JOIN)=YEAR(@TEMP_DATE) AND CMP_ID = @CMP_ID AND EM.Vertical_ID = @VERTICAL_ID_CUR
											 ) 
							WHERE VERTICAL_ID = @VERTICAL_ID_CUR
							...
							SET MONTH_1_DR = 
										(
											...
											AND CMP_ID = @CMP_ID AND INC_QRY.Vertical_ID = @VERTICAL_ID_CUR) 
							WHERE VERTICAL_ID = @VERTICAL_ID_CUR
...
DEALLOCATE GROUP_CURSOR
	...
	WHILE @I<=12
		BEGIN
				IF @I = 1
					BEGIN
						INSERT INTO #VERTICAL_CHART (CMP_ID,VERTICAL_ID,MONTH_NAME,ATTRITION_VALUE,Sorting_No)
...
SUM(MONTH_12_DR),	SUM(MONTH_12_CR),	SUM(MONTH_12_CL)
 FROM #YEARLY_ATTRITION_REPORT					
	...
	SELECT  YS.*,YS.VERTICAL_ID AS GROUP_ID , VS.Vertical_Name AS GROUP_NAME ,CM.CMP_NAME,CASE WHEN YS.VERTICAL_ID = 0 THEN 'SUMMARY' ELSE VS.Vertical_Name END VERTICAL_NAME
			,CMP_NAME,CMP_ADDRESS,
			@FROM_DATE AS P_FROM_DATE , @TO_DATE AS P_TO_DATE , 'Vertical' as GROUP_BY
		FROM #YEARLY_ATTRITION_REPORT  YS 
			LEFT OUTER JOIN	T0040_VERTICAL_SEGMENT VS WITH (NOLOCK) on Ys.VERTICAL_ID = VS.Vertical_ID
			INNER JOIN T0010_COMPANY_MASTER CM WITH (NOLOCK) ON YS.CMP_ID = CM.CMP_ID
	
	SELECT * FROM #VERTICAL_CHART	ORDER BY SORTING_NO
