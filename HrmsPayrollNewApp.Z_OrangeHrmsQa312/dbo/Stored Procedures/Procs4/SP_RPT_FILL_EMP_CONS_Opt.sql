...
-- =============================================
CREATE PROCEDURE [dbo].[SP_RPT_FILL_EMP_CONS_Opt]          
  @Cmp_ID			numeric          
 ,@From_Date		datetime          
 ,@To_Date			datetime          
 ,@Branch_ID		numeric          
		...
		BEGIN				
			...
			FROM	#Emp_Cons E						
					INNER JOIN (SELECT	I1.EMP_ID, I1.INCREMENT_ID, I1.BRANCH_ID
								FROM	T0095_INCREMENT I1 WITH (NOLOCK)  INNER JOIN #Emp_Cons E1 WITH (NOLOCK) ON I1.Emp_ID=E1.EMP_ID
										INNER JOIN (SELECT	MAX(I2.Increment_ID) AS Increment_ID, I2.Emp_ID
													FROM	T0095_INCREMENT I2 WITH (NOLOCK)  INNER JOIN #Emp_Cons E2 WITH (NOLOCK) ON I2.Emp_ID=E2.EMP_ID
															INNER JOIN (SELECT	MAX(INCREMENT_EFFECTIVE_DATE) AS INCREMENT_EFFECTIVE_DATE, I3.EMP_ID
																		FROM	T0095_INCREMENT I3  WITH (NOLOCK) INNER JOIN #Emp_Cons E3 WITH (NOLOCK) ON I3.Emp_ID=E3.EMP_ID
		...
		BEGIN
			...
			ELSE						
				BEGIN
							...
							INNER JOIN (SELECT	I1.EMP_ID, MAX(I1.Increment_ID) As Increment_ID
										FROM	T0095_INCREMENT I1 WITH (NOLOCK) 
												INNER JOIN (SELECT	Emp_ID, Max(Increment_Effective_Date) As Increment_Effective_Date
															...
															GROUP BY I2.Emp_ID) I2 ON I1.Emp_ID=I2.Emp_ID 
					...
					DELETE	EC
					FROM	#EMP_CONS EC
							LEFT OUTER JOIN T0080_EMP_MASTER E  WITH (NOLOCK) ON EC.EMP_ID=E.EMP_ID AND Date_Of_Join <= @To_Date
							AND (Emp_Left_Date >= @From_Date or Emp_Left_Date is null)
