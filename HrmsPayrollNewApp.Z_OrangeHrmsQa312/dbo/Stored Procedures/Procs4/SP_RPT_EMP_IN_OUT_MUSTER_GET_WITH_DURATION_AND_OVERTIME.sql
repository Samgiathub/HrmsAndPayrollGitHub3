CREATE PROCEDURE [dbo].[SP_RPT_EMP_IN_OUT_MUSTER_GET_WITH_DURATION_AND_OVERTIME] 
	 @Cmp_ID NUMERIC
	,@From_Date DATETIME
	,@To_Date DATETIME
	,@Branch_ID NUMERIC
...
BEGIN
	...
	WHERE EXISTS (
			...
			WHERE DATA <> ''
				AND CAST(DATA AS NUMERIC) = I.EMP_ID
			)
END
ELSE
BEGIN
...
INNER JOIN (
	...
	) GS1 ON GS1.Branch_ID = GS.Branch_ID
...
CREATE TABLE #Att_Muster (
	...
	,WO_Count NUMERIC(5, 2)
	,------ Jignesh Patel 29-12-2021--    
	HO_Count NUMERIC(5, 2)
	,------ Jignesh Patel 29-12-2021--    
	AvgHour VARCHAR(max) ---Added By tejas at 06/11/2023
	)
...
BEGIN
	...
	CREATE NONCLUSTERED INDEX ix_Data ON #Data (
		Emp_ID
		,For_Date
		)
END
...
BEGIN
	...
	CREATE UNIQUE CLUSTERED INDEX IX_EMP_HOLIDAY_EMPID_FORDATE ON #EMP_HOLIDAY (
		EMP_ID
		,FOR_DATE
		);
END
...
BEGIN
	...
	CREATE CLUSTERED INDEX IX_Emp_WeekOff_EmpID_ForDate ON #Emp_WeekOff (
		Emp_ID
		,For_Date
		)
END
...
IF @Report_For = 'Format 3'
	OR @Report_For = 'EMP RECORD' ----------- EMP RECORD Add By Jignesh Patel 29-12-2021    
BEGIN
	CREATE TABLE #ATT_DAYS (
		...
		,RET_DAYS NUMERIC(18, 2) DEFAULT 0
		,--- Added By deepali-15062023    
		AvgHours VARCHAR(max)
		)

	CREATE CLUSTERED INDEX IX_ATT_DAYS_EmpID ON #ATT_DAYS (Emp_ID)
END

EXEC SP_CALCULATE_PRESENT_DAYS @Cmp_ID
	,@From_Date
	,@To_Date
	,@Branch_ID
	,@Cat_ID
...
SELECT @Comp_OD_As_Present = ISNULL(SETTING_VALUE, 0)
FROM T0040_SETTING WITH (NOLOCK)
WHERE SETTING_NAME = 'OD and CompOff Leave Consider As Present'
	AND CMP_ID = @CMP_ID
...
IF GETDATE() < @TO_DATE
	...
	SET Is_Cancel = 1
	FROM #Emp_WeekOff EW
	WHERE EW.For_Date BETWEEN GETDATE()
			AND @TO_DATE
...
IF GETDATE() < @TO_DATE
	...
	SET Is_Cancel = 1
	FROM #EMP_HOLIDAY EH
	WHERE EH.For_Date BETWEEN GETDATE()
			AND @TO_DATE
...
WHERE EW.For_Date BETWEEN ERS.Start_Date
		AND ERS.End_Date
...
WHERE EH.For_Date BETWEEN ERS1.Start_Date
		AND ERS1.End_Date

DELETE #DATA
FROM #DATA DA
LEFT OUTER JOIN T0100_EMP_RETAINTION_STATUS ERS2 ON ERS2.Emp_ID = DA.Emp_ID
WHERE DA.For_Date BETWEEN ERS2.Start_Date
		AND ERS2.End_Date

IF @Report_For = 'Format 3'
	OR @Report_For = 'EMP RECORD' --- Emp Record Add By Jignsh Patel 29-12-2021---    
BEGIN
	...
	SELECT E.Emp_ID
		,IsNull(P_DAY.TOTAL_P_DAYS, 0)
		,0
		,IsNull(TOTAL_HF_DAYS, 0)
		,0
		,IsNull(Total_WO, 0)
		,IsNull(Total_HO, 0)
	FROM #Emp_Cons E
	LEFT OUTER JOIN (
		...
		GROUP BY Emp_ID
		) P_DAY ON E.Emp_ID = P_DAY.Emp_ID -- -- Changed By Sajid 02-12-2023 P_Days Include Half Day 
	LEFT OUTER JOIN (
		SELECT EMP_ID
			,SUM(W_Day) AS Total_WO
		FROM #Emp_WeekOff
		WHERE For_Date BETWEEN @From_Date
				AND @To_Date
			AND Is_Cancel = 0
			AND W_Day > 0
		GROUP BY Emp_ID
		) WO ON E.Emp_ID = WO.Emp_ID
	LEFT OUTER JOIN (
		SELECT EMP_ID
			,SUM(H_DAY) AS Total_HO
		FROM #EMP_HOLIDAY
		WHERE For_Date BETWEEN @From_Date
				AND @To_Date
			AND Is_Cancel = 0
			AND H_DAY > 0
		GROUP BY Emp_ID
		) HO ON E.Emp_ID = HO.Emp_ID
	LEFT OUTER JOIN (
		SELECT EMP_ID
			,COUNT(1) AS TOTAL_HF_DAYS
		FROM #DATA D
		WHERE For_Date BETWEEN @From_Date
				AND @To_Date
			AND P_days = 0.5
		GROUP BY Emp_ID
		) D_P ON E.Emp_ID = D_P.Emp_ID
	...
	BEGIN
		...
		FROM #ATT_DAYS A
		LEFT OUTER JOIN (
			SELECT sum((IsNull(LT.CompOff_Used, 0) + IsNull(LT.Leave_Used, 0)) * CASE 
			...
			WHERE (
					Leave_Type = 'Company Purpose'
					OR Leave_Code = 'COMP'
					)
				AND LT.Cmp_ID = @Cmp_ID
				AND LT.FOR_DATE BETWEEN @FROM_DATE
					AND @TO_dATE
			GROUP BY Emp_ID
...
DECLARE @AvgHours NVARCHAR(max) = 0
...
BEGIN
	SET @AvgHours = 0 -- Added By tejas at 06/11/2023

	SELECT @Branch_ID = BRANCH_ID
	FROM #EMP_CONS
	WHERE EMP_ID = @EMP_ID

	SELECT @Is_Cancel_Holiday = isnull(Is_Cancel_Holiday, 0)
	...
	BEGIN
		...
		BEGIN
			...
			---------------------- End -----------------------    
			SET @AvgHours = CASE 
					WHEN ISNUMERIC(@WorkingDuration) = 1
						THEN cast(cast(@avghours AS INT) + CAST(@WorkingDuration AS INT) AS VARCHAR)
					ELSE @avghours
					END
			SET @strQry = 'Update #Att_Muster    
     ...
     */
		...
		IF EXISTS (
				SELECT 1
				FROM #Emp_Weekoff EW
				WHERE EW.Emp_Id = @Emp_Id_Cur
					AND EW.For_Date = @Temp_Date
					AND W_Day > 0
					AND EW.For_Date <= isnull(@Left_Date, EW.For_Date)
				)
		BEGIN
			SET @strQry = 'Update #Att_Muster    
                            set WO_COHO = ''WO'', ' + @status_3 + ' = ''WO'',    
		...
		IF EXISTS (
				SELECT 1
				FROM #Emp_Holiday EH
				WHERE EH.Emp_Id = @Emp_Id_Cur
					AND EH.For_Date = @Temp_Date
					AND isnull(EH.is_Half, 0) = 0
					AND EH.For_Date <= isnull(@Left_Date, EH.For_Date)
				)
		BEGIN
			SET @strQry = 'Update #Att_Muster    
                            set WO_COHO = ''HO'', ' + @status_3 + ' = ''HO'',    
      ...
      --CASE WHEN ''' + CAST(isnull(@Left_Date, @DEFAULTDATE) AS VARCHAR(25)) + ''' = ''' + cast(@DEFAULTDATE AS VARCHAR(25)) + '''  THEN  ''AB''  ELSE ''-'' END    
                        Where Emp_Id = ' + cast(@Emp_Id_Cur AS VARCHAR(10)) + ' And ' + @Status_1 + ' Is Null' + ' And ' + @Status_2 + ' Is Null' + ' And ' + @Status_3 + ' Is Null' + ' And (' + @Status_4 + ' = ''00:00'' or ' + @Status_4 + ' Is NULL)' + ' And ' + @status_5 + ' Is Null' + ' And ' + @Status_6 + ' Is Null'
      ...
      set ' + @status_3 + ' = case When Day(''' + cast(@Temp_date AS VARCHAR(11)) + ''')  > Day( Getdate() ) Then ''-'' Else ' + @status_3 + ' End     
		...
		IF EXISTS (
				SELECT 1
				FROM T0100_EMP_RETAINTION_STATUS OA
				WHERE OA.Emp_Id = @Emp_Id_Cur
					AND OA.is_retain_On = 0
				)
		BEGIN
			SELECT @curRetDate = For_Date
			FROM #Att_Muster
			WHERE @Emp_ID = @Emp_Id_Cur

			DECLARE Curs_Retain CURSOR
			FOR
			SELECT emp_id
				,CMP_Id
				,For_Date
			...
			BEGIN
				...
				IF (
						(
							SELECT 1
							FROM T0100_EMP_RETAINTION_STATUS OA
							WHERE OA.Emp_Id = @curemp_id
								AND OA.Cmp_Id = @curCM_ID
								AND @curRetDate BETWEEN OA.Start_Date
									AND OA.End_Date
								AND OA.is_retain_On = 0
							) = 1
						)
	...
	BEGIN
		IF @Comp_OD_As_Present = 1
			...
			FROM #ATT_DAYS A
			LEFT OUTER JOIN (
				SELECT SUM(IsNull(LT.Leave_Used, 0) * CASE 
							WHEN LM.Apply_Hourly = 1
								THEN 0.125
							ELSE 1
							END) AS Leave_Used
				...
				WHERE (
					...
					AND LT.FOR_DATE BETWEEN @FROM_DATE
						AND @TO_dATE
					AND lm.Leave_Paid_Unpaid <> 'U' --Added by ronakk 10032023    
				GROUP BY Emp_ID
				) Q1 ON A.Emp_ID = Q1.Emp_ID
		...
		ELSE
			...
			WHERE (
					LT.Leave_Used > 0
					OR LT.CompOff_Used > 0
					)
				AND lt.Emp_Id = @Emp_Id_Cur
				AND lt.for_date BETWEEN @From_Date
					AND @To_Date
				AND lm.Leave_Paid_Unpaid <> 'U' --Added by ronakk 10032023    
		...
		---SET @Date_Diff = DATEDIFF(d, @From_Date, @To_Date) + 1    
		SET @Date_Diff = DATEDIFF(d, @From_Date, CASE 
					WHEN @To_Date < Getdate()
						THEN @To_Date
					ELSE GETDATE()
					END) + 1 -- comment by sandip    
			--SS    

		--Added By Sajid 02-05-2023    
		IF @From_Date < @EMP_JOINING_DATE
			SET @Date_Diff = DATEDIFF(d, @EMP_JOINING_DATE, CASE 
						WHEN @To_Date < Getdate()
							THEN @To_Date
						ELSE GETDATE()
						END) + 1
		...
		UPDATE #ATT_DAYS
		SET A_DAYS = (@Date_Diff - (P_DAYS + L_DAYS + HO_DAYS + WO_DAYS + @RET_DAYS))
			,RET_Days = @RET_DAYS
		WHERE EMP_ID = @Emp_Id_Cur
...
SELECT @OD_Compoff_As_Present = Isnull(Setting_Value, 0)
FROM T0040_SETTING WITH (NOLOCK)
WHERE Setting_Name = 'OD and CompOff Leave Consider As Present'
	AND Cmp_ID = @Cmp_ID

UPDATE #Att_Muster
SET Present_Days = CASE 
		WHEN @OD_Compoff_As_Present = 0
			THEN P_days
		ELSE Qry.P_Days + Isnull(Qry1.OD_Compoff_Leave, 0)
		END
	,Total_Overtime = Qry.OT_hours
FROM #Att_Muster A
INNER JOIN (
	SELECT Emp_Id
		,Isnull(Sum(P_days), 0) AS P_Days
		,dbo.F_Return_Hours(Isnull(Sum(OT_Sec + Weekoff_OT_Sec + Holiday_OT_Sec), 0)) AS OT_hours
	FROM #Data d
	GROUP BY Emp_Id
	) Qry ON A.Emp_Id = Qry.Emp_Id
LEFT OUTER JOIN (
	SELECT Am.Emp_Id
		,isnull(Sum(Leave_Used), 0) + isnull(Sum(CompOff_Used), 0) AS OD_Compoff_Leave
	FROM #Att_Muster AM
	INNER JOIN T0140_LEAVE_TRANSACTION LT WITH (NOLOCK) ON AM.EMP_ID = LT.EMP_ID
	INNER JOIN T0040_leave_master lm WITH (NOLOCK) ON lt.leavE_ID = lm.leave_ID
	WHERE (
			LT.Leave_Used > 0
			OR LT.CompOff_Used > 0
		...
		AND (
			Leave_Type = 'Company Purpose'
			OR Default_Short_Name = 'COMP'
			)
	GROUP BY AM.Emp_Id
	) Qry1 ON A.Emp_Id = Qry1.Emp_Id
...
SET PAYABLE_DAYS = (
		...
		ISNULL(CAST((
					CASE 
						WHEN isnull(P_DAYS, 0) = 0
							THEN 0
						ELSE ISNULL(P_DAYS, 0)
						END
					) AS NUMERIC(18, 2)), 0) + CASE 
			WHEN Inc_Holiday = 0
				THEN 0
			ELSE ISNULL(CAST((
							CASE 
								WHEN isnull(HO_DAYS, 0) = 0
									THEN 0
								ELSE ISNULL(HO_DAYS, 0)
								END
							) AS NUMERIC(18, 2)), 0)
			END + CASE 
			WHEN Inc_Weekoff = 0
			...
			ELSE ISNULL(CAST((
							CASE 
								WHEN isnull(WO_DAYS, 0) = 0
									THEN 0
								ELSE ISNULL(WO_DAYS, 0)
								END
							) AS NUMERIC(18, 2)), 0)
			END + ISNULL(CAST((
					CASE 
						WHEN isnull(L_DAYS, 0) = 0
							THEN 0
						ELSE ISNULL(L_DAYS, 0)
						END
					) AS NUMERIC(18, 2)), 0)
		)
FROM #ATT_DAYS T
...
BEGIN
	...
	ORDER BY CASE 
			WHEN IsNumeric(Alpha_Emp_Code) = 1
				THEN Right(Replicate('0', 21) + Alpha_Emp_Code, 20)
			WHEN IsNumeric(Alpha_Emp_Code) = 0
				THEN Left(Alpha_Emp_Code + Replicate('', 21), 20)
			ELSE Alpha_Emp_Code
			END
END
ELSE
BEGIN
	SELECT AM.*
		...
		,ATT.HO_DAYS
		,ATT.PAYABLE_DAYS
		,ATT.RET_DAYS AS RT_Days
		,-------------- Add By Deepali-15062023    
		CASE 
			WHEN p_days > 0
				THEN dbo.F_Return_Hours(CAST((CAST(AM.AvgHour AS INT)) / P_DAYS AS INT)) /*Added By tejas at 06/11/2023 for Getting Average hour for total working days*/
			ELSE AM.Avghour
			END AvgHours
	FROM #Att_Muster AM
	INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON AM.EMP_ID = E.EMP_ID
