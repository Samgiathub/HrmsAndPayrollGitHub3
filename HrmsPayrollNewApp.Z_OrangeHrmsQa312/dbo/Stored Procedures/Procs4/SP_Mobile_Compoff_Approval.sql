...
CREATE PROCEDURE [dbo].[SP_Mobile_Compoff_Approval]
	@CompOff_Approval_ID Numeric = 0 output  
...
IF @TRAN_TYPE = 'A'
	BEGIN
			 ...
			 -- Add By Mukti 05072016(end)	
				...
				EXEC SP_GET_HW_ALL @CONSTRAINT=@CONSTRAINT,@CMP_ID=@Cmp_ID, @FROM_DATE=@Extra_Work_Date, @TO_DATE=@Extra_Work_Date, @All_Weekoff = 0, @Exec_Mode=0		
		   ...
		   --	set @sanctione_hrs = floor(@sanctione_hrs*2)/2
			...
			VALUES (@CompOff_Approval_ID, @CompOff_Application_ID, @Cmp_ID, @Emp_ID, @S_Emp_ID, @Extra_Work_Date, @Approval_Date, @Extra_Work_Hours, @Sanctioned_Hours, @Approval_Status, @Extra_Work_Reason, @Approval_Comments, ISNULL(@CONTACT_NO,''), isnull(@Email_ID,''), @Login_ID, @System_Date, @Com_Days)   
