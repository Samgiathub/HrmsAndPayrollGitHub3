...
CREATE PROCEDURE [dbo].[SP_RPT_STATUTORY_ESIC_DEPENDANT_GET]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	,@Branch_ID 	numeric
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
		...
		SELECT edd.* , Emp_Code,ISNULL(EmpName_Alias_PF,Emp_Full_Name) as Emp_Full_Name
		FROM T0090_EMP_DEPENDANT_DETAIL edd WITH (NOLOCK) inner join  T0080_EMP_MASTER E WITH (NOLOCK) on edd.Emp_ID = e.Emp_ID INNER JOIN
