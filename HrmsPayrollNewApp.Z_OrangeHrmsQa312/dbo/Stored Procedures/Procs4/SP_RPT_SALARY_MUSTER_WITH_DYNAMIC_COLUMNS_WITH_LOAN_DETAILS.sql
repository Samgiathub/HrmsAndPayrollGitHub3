...
---30/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
	CREATE PROCEDURE [dbo].[SP_RPT_SALARY_MUSTER_WITH_DYNAMIC_COLUMNS_WITH_LOAN_DETAILS]        
	  @Cmp_ID  numeric        
	 ,@From_Date  datetime        
	 ,@To_Date  datetime         
	 ,@Branch_ID  numeric   = 0        
	 ...
	 AS      
	  ...
	  DECLARE @W_Days as numeric(22,2)        
	  DECLARE @T_Days as numeric(22,2)        
	  ...
	  DECLARE @ROUNDING AS NUMERIC(18,0)   --Ankit 15072014        
	   SET @ROUNDING = 0        
	   DECLARE @Loan_Name as VARCHAR(50) --added jimit 05042016      
	   DECLARE @Gross_Deduction as NUMERIC(18,2) --added jimit 06042016    
	 ...
	 declare Cur_Loan   cursor for        
	  ...
	  --and Emp_id = @Emp_Id        
	   ...
	   while @@fetch_status = 0        
		begin       
		 ...
		 if  @Label_Name <> NULL  or @Label_Name <> ''
	  BEGIN  
	  ...
	  -- AND Branch_ID = ISNULL(@Branch_ID ,Branch_ID)              
	   ...
	   AND Increment_Effective_Date <= @To_Date            AND         
	  ...
	  WHILE @@FETCH_STATUS = 0        
	   BEGIN        
		...
		Set @Round_Value = 0        
		Set @Total_Gross = 0        
		Set @Net_Round_Value = 0            Set @Total_Net_Payable = 0        
		...
		set @Asset_Installment=0  -- Added by Mukti 01042015        
		 SET @Gross_Deduction =0 --added jimit 06042016      
	              
	   If @Sal_Type = 0        
		Begin        
		 select @P_Days = isnull(Present_Days,0) ,@Sal=Basic_Salary,@A_Days = isnull(Absent_Days,0),@W_Days=isnull(Weekoff_Days,0),@T_Days=isnull(Working_Days,0),@TDS=isnull(M_IT_TAX,0), @Basic_Salary = Salary_Amount, @Act_Gross_salary = Actually_Gross_salary    
	,@Settl = Settelement_Amount,@OTher_Allow = ISNULL(Other_Allow_Amount,0),@Total_Allowance = Allow_Amount, @Leave_Amount = isnull(Leave_Salary_Amount,0),        
	   ...
	   Else        
		Begin        
		 select @P_Days = isnull(S_Sal_Cal_Days,0),@T_Days = isnull(S_Sal_Cal_Days,0) ,@Sal=S_Basic_Salary,@A_Days = 0,@W_Days=0,@TDS=isnull(S_M_IT_TAX,0), @Basic_Salary = S_Salary_Amount, @Act_Gross_salary = S_Actually_Gross_salary,@Settl = 0,@OTher_Allow = ISNULL(S_Other_Allow_Amount,0),@Total_Allowance = S_Allow_Amount, @Leave_Amount = 0 , @OT_Amount = ISNULL(S_OT_Amount,0)        
	   ...
	   INSERT INTO dbo.#Temp_Salary_Muster_Report        
	   (Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)        
	   VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'W Days', @W_Days,'',4,'P')        
	              
	   INSERT INTO dbo.#Temp_Salary_Muster_Report        
	   (Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)        
	   VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'T Days', @T_Days,'',5,'P')        
	  ...
	  /* INSERT INTO dbo.#Temp_Salary_Muster_Report        
	   ...
	   VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year,'Basic', @Basic_Salary,'',-1,'I',@Sal)        
	            
	   INSERT INTO dbo.#Temp_Salary_Muster_Report        
	   (Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)        
	   VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Settl', @Settl,'',7,'I')        
	              
	              
	   INSERT INTO dbo.#Temp_Salary_Muster_Report        
	   (Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)        
	   VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Other', @OTher_Allow,'',8,'I')        
	   ...
	   -- and isnull(t0050_ad_master.Ad_Not_Effect_Salary,0) = 0 and Ad_Active = 1 and AD_Flag = 'I' and MAD.Emp_ID  = @Emp_ID        
		 ...
		 on AD.AD_SORT_NAME =E1.Label_Name COLLATE SQL_Latin1_General_CP1_CI_AS      
		 and ad.cmp_id = e1.cmp_id--Falak,Hardik,Nikunj 16-05-2011        
	  ...
	  --Select * From #Temp_Salary_Muster_Report Where emp_Id=1 And MONTH=2 And Year=2011        
	   ...
	   and Emp_ID = @Emp_ID        
		...
		UPDATE    dbo.#Temp_Salary_Muster_Report        
		SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year,         
		Amount = @Total_Allowance+@Basic_Salary+isnull(@Settl,0)+ISNULL(@OTher_Allow,0)+isnull(@CO_Amount,0) + ISNULL(@Leave_Amount,0) + isnull(@OT_Amount,0), Value_String = '',M_AD_Flage='I'        
	   ...
	   */        
	   ...
	   and Emp_ID = @Emp_ID        
		...
		--  and Emp_ID = @Emp_ID        
		   ...
		   -------------------added jimit 05042016---------------------------------      
	             
		   set @Amount = 0      
		   declare Cur_Loan   cursor for        
		SELECT  Loan_Short_Name,Loan_Pay_Amount      
	  from V0210_MONTHLY_LOAN_PAYMENT       
	  where   Cmp_ID= @Cmp_Id and month(Loan_Payment_Date) = @Month and YEAR(Loan_Payment_Date) = @Year      
	  and Emp_id = @Emp_Id        
	   open Cur_Loan        
	   fetch next from Cur_Loan  into @Loan_Name ,@Amount      
	   while @@fetch_status = 0        
		begin        
		 select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like @Loan_Name         
	                
	                
	   UPDATE    dbo.#Temp_Salary_Muster_Report        
	   SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = @Amount,         
		  Value_String = '',M_AD_Flage='D'      
	   WHERE     (Label_Name = @Loan_Name) AND (Row_id = @Row_ID) and Emp_ID = @Emp_ID        
	 ...
	 --added by jimit 28072017
	   ...
	   WHERE     (Label_Name = 'Asset Inst.') AND (Row_id = @Row_ID) and Emp_ID = @Emp_ID           
		...
		WHERE     (Label_Name = 'Net Payable') AND (Row_id = @Row_ID) and Emp_ID = @Emp_ID        
		 ...
		 ------------------added jimit 06042016-----------------------------    
		 Select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'Gross Deduction'        
	        
		 Update E1    
		 set Amount =  Q.Amount,    
	   M_AD_Flage = 'D' ,    
	   Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year,Value_String = ''    
		 from #Temp_Salary_Muster_Report e1 inner JOIN    
		  (Select sum(IsNull(Amount,0)) as Amount,Emp_ID,Cmp_ID    
		  from #Temp_Salary_Muster_Report       
		   where Emp_ID =@Emp_ID and Cmp_ID=@Cmp_ID        
	   and M_AD_Flage ='D'     
	   Group by Emp_ID,Cmp_ID)Q On Q.emp_Id = e1.Emp_ID and Q.cmp_Id = e1.Cmp_ID    
	   where Label_Name = 'Gross Deduction'     
	   AND Row_id = @Row_ID    
		 -----------------------------------ended----------------------------------    
	  ...
	  SELECT [P Days] AS PDays,[A Days] As ADays,[W Days] As WDays,[T Days] As TDays,Emp_ID,Cmp_ID        INTO #PRESENSE        
	  FROM (        
	  SELECT Emp_ID,Cmp_ID,Label_Name,Amount        
	  FROM dbo.#Temp_Salary_Muster_Report        
		) AS ST        
		PIVOT        
		(        
	  SUM(Amount) FOR Label_Name IN ([P Days],[A Days],[W Days],[T Days])        
		) AS PT        
	          
	           
	  --Added by Nimesh        
	  DELETE FROM #Temp_Salary_Muster_Report         
	  WHERE Label_Name IN (        
		 SELECT Label_Name         
		 FROM #Temp_Salary_Muster_Report         
		 GROUP BY Label_Name         
	  ...
	  SELECT dbo.#Temp_Salary_Muster_Report.* ,ISNULL(E.EmpName_Alias_Salary,E.Emp_Full_Name) as Emp_Full_Name ,E.Alpha_Emp_Code as 'Emp_Code',GM.Grd_name,ETM.Type_name,DGM.Desig_Name,DM.Dept_Name,BM.Branch_Name,         
		...
		T_PRESENSE.PDays,T_PRESENSE.ADays,T_PRESENSE.WDays,T_PRESENSE.TDays,Inc_Qry.Basic_Salary, --E.Basic_Salary,        
		Case Upper(Inc_Qry.Payment_Mode)        
	  ...
	  ,(Select dbo.F_Number_TO_Word(sum(amount)) as Total_Amount_In_Words  from #Temp_Salary_Muster_Report where Label_Name= 'Gross' )as gross_In_Word
	  ,(Select dbo.F_Number_TO_Word(sum(amount)) as Total_Amount_In_Words  from #Temp_Salary_Muster_Report where Label_Name= 'Net' )as Net_In_Word
	  FROM dbo.#Temp_Salary_Muster_Report Inner join         
		...
		(         
	  SELECT I.Emp_Id ,Grd_ID,DEsig_ID ,Dept_ID,Inc_Bank_Ac_no,Branch_ID,Type_ID,Payment_Mode,Vertical_ID ,SubVertical_ID ,I.Basic_Salary        
	  FROM T0095_INCREMENT I WITH (NOLOCK) inner join         
		...
		LEFT OUTER JOIN t0040_department_Master WITH (NOLOCK) ON Inc_Qry.dept_ID = t0040_department_Master.Dept_ID LEFT OUTER JOIN        
		...
		T0040_Vertical_Segment VS WITH (NOLOCK) on Inc_Qry.Vertical_ID = VS.Vertical_ID LEFT OUTER JOIN           
		T0050_SubVertical SV WITH (NOLOCK) on Inc_Qry.SubVertical_ID = SV.SubVertical_ID               
		INNER JOIN  t0010_company_master CM WITH (NOLOCK) on E.cmp_id=CM.cmp_id        
		LEFT OUTER JOIN (        
	   SELECT PDays,ADays,WDays,TDays,Emp_ID,Cmp_ID        
	   FROM #PRESENSE        
		 ) T_PRESENSE ON T_PRESENSE.Cmp_ID=E.Cmp_ID AND T_PRESENSE.Emp_ID=E.Emp_ID        
	  WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days')        
	  ...
	  --WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days') and M_AD_Flage = 'I'        
	  ----where dbo.#Temp_Salary_Muster_Report.Amount > 0        
	  --ORDER BY Row_ID        
	          
	 --Select Distinct Label_name,Row_ID,M_AD_Flage  from dbo.#Temp_Salary_Muster_Report          
	  --WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days') and M_AD_Flage = 'D'        
	  ----where dbo.#Temp_Salary_Muster_Report.Amount > 0        
	  --ORDER BY Row_ID        
	       
	  --Select Distinct Label_name,Row_ID,M_AD_Flage  from dbo.#Temp_Salary_Muster_Report          
	  --WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days') and M_AD_Flage = 'L'        
	  ----where dbo.#Temp_Salary_Muster_Report.Amount > 0        
	  --ORDER BY Row_ID       
	      
	      
	  Select Distinct Label_name,Row_ID,M_AD_Flage,sum(amount) as Total_Amount
	  from dbo.#Temp_Salary_Muster_Report          
	  WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days') and M_AD_Flage = 'I'        
	  --where dbo.#Temp_Salary_Muster_Report.Amount > 0     
	  ...
	  --  Select Distinct Label_name,Row_ID,M_AD_Flage,sum(amount) as Total_Amount  from dbo.#Temp_Salary_Muster_Report          
	  --WHERE #Temp_Salary_Muster_Report.Label_Name NOT IN ('P Days', 'A Days', 'W Days', 'T Days') and M_AD_Flage = 'L'        
	  ----where dbo.#Temp_Salary_Muster_Report.Amount > 0     
	  --Group BY Label_name,Row_ID,M_AD_Flage       
	  --ORDER BY Row_ID    
	  ...
	  -- T0080_Emp_Master E on dbo.#Temp_Salary_Muster_Report.Emp_Id = E.Emp_ID inner join        
	  -- ( select I.Emp_Id ,Grd_ID,DEsig_ID ,Dept_ID,Inc_Bank_Ac_no,Branch_ID,Type_ID,Payment_Mode from t0095_Increment I inner join         
