...
--====================================================
--Last Change Nilay Add field VPF column -30-dec-2010
---29/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
--====================================================
CREATE  PROCEDURE [dbo].[SP_RPT_STATUTORY_PF_TRUST]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
SET ARITHABORT ON
	...
	WHERE gs.Cmp_Id=@cmp_Id and Branch_ID = isnull(@Branch_ID,Branch_ID)    
			...
			where g.Cmp_Id=@cmp_Id and Branch_ID = isnull(@Branch_ID,Branch_ID) and For_Date <=@To_Date ) 
	...
	( SELECT DISTINCT SG.CMP_ID,SG.EMP_ID ,E.EMP_CODE ,E.EMP_full_NAME ,SSN_NO as PF_NO FROM    T0200_MONTHLY_SALARY  SG  WITH (NOLOCK) INNER JOIN 
			( select Emp_ID , M_AD_Percentage as PF_PER , M_AD_Amount as PF_Amount ,sal_Tran_ID
					from T0210_MONTHLY_AD_DETAIL AD WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AM WITH (NOLOCK) ON AD.AD_ID = AM.AD_ID where AD_DEF_ID = 14 
	...
	IF	EXISTS (select * from [tempdb].dbo.sysobjects where name like '#EMP_SALARY' )		
		...
		CREATE table #EMP_SALARY 
		    ...
		    SELECT  SG.EMP_ID,MONTH(MONTH_ST_DATe),YEAR(MONTH_ST_DATE),SG.Salary_Amount 
				 ...
				 ,MAD.PF_PER,MAD.PF_AMOUNT  , m_ad_Calculated_Amount ,@PF_Limit,0,0,0,isnull(CMD.VPF,0),dbo.F_GET_AGE(Date_of_Birth,MONTH_ST_DATE,'N','N') ,0
				...
				(Select Emp_ID , m_ad_Percentage as PF_PER , m_ad_Amount as PF_Amount , m_ad_Calculated_Amount ,SAL_tRAN_ID from 
					T0210_MONTHLY_AD_DETAIL AD WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AM WITH (NOLOCK) ON AD.AD_ID = AM.AD_ID  where ad_DEF_id = 14  And ad_not_effect_salary <> 1 and sal_type<>1
					and AD.CMP_ID = @CMP_ID ) MAD on SG.Emp_ID = MAD.Emp_ID  
					AND SG.SAL_tRAN_ID = MAD.SAL_TRAN_ID INNER JOIN
					T0080_EMP_MASTER E WITH (NOLOCK) ON SG.EMP_ID = E.EMP_ID inner join
				@EMP_CONS E_S on E.Emp_ID = E_S.Emp_ID
				left outer join						
				(Select 	Emp_ID , M_AD_Amount as VPF,SAL_tRAN_ID  from 
					T0210_MONTHLY_AD_DETAIL AD WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AM WITH (NOLOCK) ON AD.AD_ID = AM.AD_ID  where ad_DEF_id = 15  And ad_not_effect_salary <> 1 and sal_type<>1
		...
		--select Top 1	@PF_Pension_Age = isnull(PF_Pension_Age,0)    from T0040_General_setting gs inner join     
     ...
     set PF_Amount = PF_Amount 
		...
		SELECT EPF.*--, (SALARY_AMOUNT + ISNULL(OTHER_PF_SALARY,0) ) as SALARY_AMOUNT
				...
				Emp_Full_Name,Grd_Name,Type_Name,dept_Name,Desig_Name
				 ,Cmp_Name,Cmp_Address,cm.PF_No as CPF_NO
				,@From_Date P_From_Date ,@To_Date P_To_Date,Father_Name,Le.Left_Date,Le.Left_Reason,MS.Absent_Days
