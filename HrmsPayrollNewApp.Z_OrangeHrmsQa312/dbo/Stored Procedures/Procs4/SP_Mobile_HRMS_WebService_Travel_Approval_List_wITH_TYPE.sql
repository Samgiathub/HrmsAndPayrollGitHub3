...
CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_Travel_Approval_List_wITH_TYPE]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
...
SET ARITHABORT ON
	...
	SET @Constrains = N'Application_Status = '''+@StrType+''''

		IF(@StrType = 'A' or @StrType = 'R')
		Begin
			Select 0 As Rpt_Level,0 As Scheme_Id,0 As Final_Approver,0 As Is_Fwd_Leave_Rej,0 as DynHierRepId,'' as 'Travel_Type_Name',
			0 as 'Travel_Type_Id,', 0 as 'Grd_ID'
			,(Select distinct Rpt_Level from T0115_TRAVEL_LEVEL_APPROVAL where S_Emp_ID=@Emp_ID) as Emp_Rpt_lvl
			,(Select distinct Approval_Date from T0115_TRAVEL_LEVEL_APPROVAL where S_Emp_ID=@Emp_ID and Travel_Application_ID=View_Travel_Final_N_level_Approval_With_TravelType.Travel_Application_ID) as Approval_Date
			,*
			From View_Travel_Final_N_level_Approval_With_TravelType
			Where S_Emp_ID_A = @Emp_ID And Application_Status = @StrType   Order by Application_Date desc	
		End
	...
	(
		...
		,Travel_Type_Name       varchar(200)
		,Travel_Type_Id			numeric(18,0) 
		,Grd_ID					numeric(18,0) 
		)
		...
		inner join (
			...
			AND Effective_Date = (SELECT max(Effective_Date) from T0095_EMP_SCHEME where Emp_ID in (select AppEmp from #AppEmp) And Type = 'Travel' AND Effective_Date <= getdate())  
					...
					If @Emp_ID_Cur > 0
						...
						--select * from #tbl_Leave_App
							...
							Select distinct	
									...
									,LAD.Travel_Set_Application_id,LAD.travel_approval_id as Travel_approval_id
									,isnull(Qry1.rpt_level + 1,'1') As Rpt_Level, '0' as Scheme_ID, SL.Final_Approver,SL.Is_Fwd_Leave_Rej ,DynHierColValue
										,Travel_Type_Name,lad.Travel_Type_Id,emp.Grd_ID
									From V0100_TRAVEL_APPLICATION LAD
									...
									left outer join T0080_EMP_MASTER emp on emp.Emp_ID = lad.Emp_ID -- added by prapti 15112022
					...
					Else
						...
						Insert INTO #Travel
							...
							left outer join T0080_EMP_MASTER emp on emp.Emp_ID = lad.Emp_ID -- added by prapti 15112022
									...
									On  LAD.Travel_Application_ID = Qry1.App_ID
							WHERE LAD.Cmp_ID = @Cmp_ID  order by Lad.travel_set_Application_id asc
