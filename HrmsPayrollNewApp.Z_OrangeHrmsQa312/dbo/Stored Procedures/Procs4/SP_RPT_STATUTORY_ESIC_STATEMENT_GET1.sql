...
CREATE PROCEDURE [dbo].[SP_RPT_STATUTORY_ESIC_STATEMENT_GET1]    
 @Cmp_ID   numeric    
,@From_Date  datetime    
,@To_Date   datetime    
...
select Distinct(Emp_ID) from #Emp_Cons     
 ...
 --IF @Flag_For_Total=1     
 if(@Report_format = 0 and @Export_Type = 8)  
  Begin  
 Select Distinct   
   Alpha_Emp_Code as Emp_code,   
      ISNULL(EmpName_Alias_ESIC,Emp_Full_Name) as Emp_full_Name,  
  SIN_NO AS ESIC_NO   
  ,MS.SAL_CAL_DAYS,   
  ...
  MAD.M_AD_Amount + ISNULL(ES.M_AD_Amount,0) + Isnull(MAD.M_AREAR_AMOUNT,0) + Isnull(MAD.M_AREAR_AMOUNT_cutoff,0) as M_AD_Amount  
  ,ceiling(G.ESIC_EMPLOYER_CONTRIBUTION  * (M_AD_Calculated_Amount + ISNULL(ES.M_AD_Calculate_Amount,0)) /100) as EMPLOYER_CONT_AMOUNT   
 ...
 ,  @From_Date as From_Date , @To_Date as To_Date    
  into #EMPESICDATA  
    ...
    T0095_INCREMENT I_Q WITH (NOLOCK) ON MS.INCREMENT_ID = I_Q.INCREMENT_ID left outer join    
    #Gen G on G.Branch_Id= I_Q.Branch_ID left outer JOIN   --Jaina 21-10-2015  
    ...
    select   
  Emp_code as [Empyee Code],Emp_full_Name as [Employee Name],ESIC_NO,cast(SAL_CAL_DAYS as varchar(20)) as [Present Days],M_AD_Calculated_Amount as [Salary/Wages],  
  M_AD_Amount as [Employee's Contribution(ESIC@ 0.75 %)] ,Cmp_Name,Cmp_Address,From_Date,To_Date from #EMPESICDATA
    
  Union ALL  
  select   
  '' as Emp_code,  
  '' as Full_name,  'Total :'as ESIC_NO  
  ,cast(Sum(SAL_CAL_DAYS) as varchar(20)) as p_Days,	
  Sum(M_AD_Calculated_Amount) as Salary ,  
  sum(M_AD_Amount) as M_AD_Amount    
  ,'' as cmp_name  ,null as Cmp_Address,NULL  as From_Date, NULL as To_Date  
  from #EMPESICDATA    
  ...
  NULL as Salary ,  
  NULL as M_AD_Amount  
 ,null as cmp_name  ,null as Cmp_Address,NULL  as From_Date, NULL as To_Date  
  
  Union ALL  
    
  select 'Salary /Wages' as Emp_code,  
  'Employee''s Contribution' as Full_name,   
  'Employer''s Contribution(ESIC @ 4.75 %)'as ESIC_NO,  
  'Total' as p_Days,  
  NULL as Salary ,  
  NULL as M_AD_Amount  
    ,null as cmp_name ,null as Cmp_Address ,NULL  as From_Date, NULL as To_Date  
 ...
 union ALL  
  ...
  select   
  cast(Sum(M_AD_Calculated_Amount) as varchar(20)) as Emp_code,  
  cast(sum(M_AD_Amount) as varchar(20)) as Full_name,  
  cast(sum(EMPLOYER_CONT_AMOUNT) as varchar(20))  as ESIC_NO  
  ,cast(sum(M_AD_Amount) +sum(EMPLOYER_CONT_AMOUNT) as varchar(20)) as p_Days,  
  NULL as Salary ,  
  NULL as M_AD_Amount  
   ,null as cmp_name ,null as Cmp_Address  ,NULL  as From_Date, NULL as To_Date  
  from #EMPESICDATA    
