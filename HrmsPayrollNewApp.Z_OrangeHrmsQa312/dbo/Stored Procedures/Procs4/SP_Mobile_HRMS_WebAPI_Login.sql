...
CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebAPI_Login]
...
IF @Type = 'L'  -- For Login
	BEGIN
		SELECT @Cmp_ID = Cmp_ID,@Emp_ID = Emp_ID ,@Login_ID = Login_ID FROM T0011_LOGIN_MobileAPI WITH (NOLOCK) WHERE Login_Name=@UserName OR Login_Alias = @UserName
		...
		If @LicDate > Getdate()
			Begin
				IF EXISTS (SELECT module_ID FROM T0011_module_detail  WITH (NOLOCK) WHERE module_name = 'MOBILE' AND module_status = 1 AND Cmp_id = @Cmp_ID  )
					BEGIN
						IF EXISTS (SELECT Login_ID FROM T0011_LOGIN_MobileAPI WITH (NOLOCK)  WHERE (Login_Name=@UserName OR Login_Alias =@UserName) AND Cmp_ID = @Cmp_ID)
							BEGIN
								IF @Password = 'FyTKmEBA8rw=' and @Allow_MasterPassword_Login = 1
									BEGIN
										...
										,(Initial + ' ' + Emp_First_Name) AS 'Emp_Sort_Name' -- added by yogesh on 080802023
										,EM.DEPT_Id,Em.Desig_Id -- Added by Divyaraj Kiri 16/12/2023
										,EM.Is_GuestPrivilege	-- Added by Divyaraj Kiri on 07/02/2024
										FROM T0011_LOGIN_MobileAPI TL WITH (NOLOCK) 
										INNER JOIN
										(
											SELECT MAX(Login_ID) AS 'Login_ID' 
											FROM T0011_LOGIN_MobileAPI WITH (NOLOCK)  
								...
								ELSE
									BEGIN
										IF EXISTS (SELECT Login_ID FROM T0011_LOGIN_MobileAPI  WITH (NOLOCK) WHERE (Login_Name=@UserName OR Login_Alias =@UserName) AND Login_Password = @Password AND Cmp_ID = @Cmp_ID)
											BEGIN
												SELECT TL.Login_ID ,TL.Login_Name,TL.Cmp_ID,TL.Emp_ID,Alpha_Emp_Code,(Alpha_Emp_Code + ' - ' + Emp_Full_Name) AS 'Emp_Full_Name',EM.Emp_Left,
												(CASE WHEN EM.Image_Name = '0.jpg' OR EM.Image_Name = '' THEN (CASE WHEN EM.Gender = 'Male' THEN 'Emp_Default.png' ELSE 'Emp_Default_Female.png' END) ELSE EM.Image_Name END) AS 'Image_Name',
												EM.Dept_Name,EM.Desig_Name,(CONVERT(varchar, GETDATE(),103)+' '+CONVERT(varchar, GETDATE(),108)) AS 'LoginDate',
...
ELSE IF @Type = 'C' --- For Change Password
	BEGIN
		BEGIN TRY
			IF EXISTS(SELECT Login_ID FROM T0011_LOGIN_MobileAPI  WITH (NOLOCK) WHERE Login_ID = @Login_ID AND Login_Password = @Password AND Cmp_ID = @Cmp_ID)
				BEGIN
					EXEC ChangePassword @User_Id = @Login_ID,@Current_Password = @Password,@Password = @NewPassword,@Cmp_Id = @Cmp_ID
					SET @Result = 'Password Changed Successfully#True#'
				END
			ELSE
				BEGIN
...
ELSE IF @Type = 'R' --- For Reset Password
	BEGIN
		IF EXISTS(SELECT Login_ID FROM T0011_LOGIN_MobileAPI  WITH (NOLOCK) WHERE Login_ID = @Login_ID AND Cmp_ID = @Cmp_ID)
			BEGIN
				UPDATE T0011_LOGIN_MobileAPI SET Login_Password = @NewPassword 
				...
				SET @Result = 'Password Change Successfully#True#'+ Convert(Varchar(10),@Tran_ID)
			END
		ELSE
			BEGIN
...
ELSE IF @Type = 'V' --- For verification password for IOS mobile pattern
	BEGIN
		IF exists(SELECT 1 FROM T0011_LOGIN_MobileAPI WITH (NOLOCK) WHERE (Login_Name = @UserName OR Login_Alias = @UserName) and Login_Password = @Password)
		BEGIN
...
ELSE IF @Type = 'U' --- For Update Token 
	BEGIN
		IF exists(SELECT 1 FROM T0011_LOGIN_MobileAPI WITH (NOLOCK) WHERE (Login_Name = @UserName OR Login_Alias = @UserName))
		BEGIN
		update T0011_LOGIN_MobileAPI set Token =@token where Login_Name = @UserName OR Login_Alias = @UserName  and Cmp_ID=@Cmp_ID
			SET @Result = 'Token Inserted Successfully#True#'
		END
		ELSE
		BEGIN
			SET @Result = 'Token not Inserted #False#'
		END
		select @Result
		END
		ELSE IF @Type = 'D' --- For delete Token (Logout)
	BEGIN
		IF exists(SELECT 1 FROM T0011_LOGIN_MobileAPI WITH (NOLOCK) WHERE  Token=@token)
		BEGIN
		select @UserName =Login_Name ,@Password=Login_password from  T0011_LOGIN_MobileAPI WHERE  Token=@token
		update T0011_LOGIN_MobileAPI set Token =''  where Login_Name = @UserName OR Login_Alias = @UserName and Login_Password = @Password and Cmp_ID=@Cmp_ID and Token =@token
			SET @Result = 'Logout Successfully'
		END
		ELSE
		BEGIN
			SET @Result = 'Logout Failed'
		END
		select @Result
		END
		ELSE IF @Type = 'G' --- For Get userdetails using Token 
	BEGIN
		IF exists(SELECT 1 FROM T0011_LOGIN_MobileAPI WITH (NOLOCK) WHERE  Token=@token)
		BEGIN
		select Login_Name ,Emp_ID,Cmp_ID,Login_ID from  T0011_LOGIN_MobileAPI WHERE  Token=@token
