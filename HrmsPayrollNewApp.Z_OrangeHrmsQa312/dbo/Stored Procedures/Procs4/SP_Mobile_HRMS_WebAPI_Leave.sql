CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebAPI_Leave]
...
ELSE IF @Type = 'S' --- For Leave Application Records
	...
	ELSE IF @Type = 'W' --- For Leave Application Records without From Date and TO Date
	BEGIN
		SELECT * FROM
		(
			SELECT  0 AS 'Leave_Approval_ID',VL.Leave_Application_ID, VL.Application_Status,(CASE WHEN VL.Application_Status = 'A' THEN 'Approved' ELSE (CASE WHEN VL.Application_Status = 'P' THEN 'Pending' ELSE 'Rejected' END )END) AS 'AppStatus',
...
ELSE IF @Type = 'R'  --- For Leave Balance with From Date and To Date
	BEGIN
...
ELSE IF @Type = 'L'  --- For Leave Balance without From Date and To Date
	BEGIN
		CREATE TABLE #LeaveMonthlyBalanceWithoutDate
		(
			Cmp_ID NUMERIC(18,0),
			Emp_ID NUMERIC(18,0),
			For_Date datetime,
		...
		EXEC SP_RPT_MONTHLY_LEAVE_BALANCE_GET @Cmp_ID = @Cmp_ID,
		...
		LB.P_To_Date,LB.Branch_Id,LB.Type_Name,LB.Desig_Dis_No,LB.Vertical_Name,LB.SubVertical_Name,LB.SubBranch_Name,
		LB.Leave_Code,LB.Gender   
		FROM #LeaveMonthlyBalanceWithoutDate LB
		INNER JOIN T0040_LEAVE_MASTER LM WITH (NOLOCK) ON LB.Leave_ID = LM.Leave_ID
		WHERE LM.Display_leave_balance = 1 AND LB.Leave_Closing <> 0.00
		ORDER BY LB.Leave_Name
		
		DROP TABLE #LeaveMonthlyBalanceWithoutDate
	END
