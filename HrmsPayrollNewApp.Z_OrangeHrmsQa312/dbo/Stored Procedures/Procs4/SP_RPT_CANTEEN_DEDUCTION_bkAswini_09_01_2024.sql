...
create PROCEDURE [dbo].[SP_RPT_CANTEEN_DEDUCTION_bkAswini_09/01/2024] 
	@Cmp_ID Numeric(18,0)	
	,@From_Date DateTime
	,@To_Date DateTime	
...
SET ARITHABORT ON
	...
	Select  Emp_ID, Alpha_Emp_Code,Emp_Full_Name,cast(IO_DateTime as Date) as For_Date,In_Time,IP_Address,Cnt_Name,Reason ,IO_Tran_Id into #RPTTmp
	from (
	SELECT * FROM #RPT
	UNION ALL
	SELECT * FROM #RPT_MOBILE_NFC_CANTEEN) Qry Order by IO_DateTime asc

	Select Distinct  Emp_ID, Alpha_Emp_Code,Emp_Full_Name,For_Date,In_Time,IP_Address,Cnt_Name,Reason ,IO_Tran_Id  from #RPTTmp order BY   For_Date ,IO_Tran_ID
	...
	SELECT E.Emp_ID,Alpha_Emp_Code, Alpha_Emp_Code + ' - ' + Emp_Full_Name As Emp_Full_Name  
