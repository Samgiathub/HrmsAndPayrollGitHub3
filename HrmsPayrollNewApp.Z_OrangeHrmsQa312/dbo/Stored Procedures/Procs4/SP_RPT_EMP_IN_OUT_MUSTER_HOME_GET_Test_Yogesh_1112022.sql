CREATE PROCEDURE [dbo].[SP_RPT_EMP_IN_OUT_MUSTER_HOME_GET_Test_Yogesh_1112022]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
--if object_id ('tempdb..#deb') is not null
----select * from #Att_Period
--return
...
FROM    sys.all_objects a
	...
	(
		...
		OT_Apr_ID Numeric(5,0) default 0,
	...
	update A 
	set Date_of_join = E.DATE_OF_JOIN, 
	...
	*********************************************************************/
	...
	While @for_Date <= @Att_date
		BEGIN
			...
			FROM	#Rotation R 
			INNER JOIN T0040_SHIFT_MASTER SM WITH (NOLOCK) ON R.R_ShiftID=SM.Shift_ID					
	...
	UPDATE AM
	SET SH_IN_TIME = QRY.HALF_ST_TIME,SH_OUT_TIME = QRY.HALF_END_TIME  
	FROM #ATT_MUSTER  AM INNER JOIN
			(
...
SET @Count = 0
	...
	update AM
	set Status = 'P'
	from #Att_Muster AM inner join #TMP_EMP_0150_INOUT EIR ON AM.EMP_ID = EIR.EMP_ID And AM.Cmp_ID = EIR.Cmp_ID
...
( 
	...
	IS_CANCEL_LATE_IN = IsNull(Q.IS_CANCEL_LATE_IN,0),
	IS_CANCEL_EARLY_OUT = IsNull(Q.IS_CANCEL_EARLY_OUT,0),
	Half_Full_Day = IsNull(Q.Half_Full_Day,'')
	FROM #ATT_MUSTER AM INNER JOIN 
	(
		SELECT SUP_COMMENT,FOR_DATE,EIR.EMP_ID,IS_CANCEL_LATE_IN,IS_CANCEL_EARLY_OUT,Half_Full_Day FROM #TMP_EMP_0150_INOUT EIR
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
...
----Added by Sid 25062014 Ends
		...
		--Added by Mukti Reporting Manager of employee(start)04092017
			UPDATE	AM 
			SET R_Emp_ID = isnull(rm.R_Emp_ID,0) --isnull for - if employee have no manager then it will be zero,bcoz  it will effect on ESS login Attendance Req. page
