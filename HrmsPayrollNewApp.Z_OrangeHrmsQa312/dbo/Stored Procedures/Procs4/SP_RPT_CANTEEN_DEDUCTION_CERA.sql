...
CREATE PROCEDURE [dbo].[SP_RPT_CANTEEN_DEDUCTION_CERA] 
	@Cmp_ID Numeric(18,0)	
	,@From_Date DateTime
	,@To_Date DateTime	
...
SET ARITHABORT ON
	...
	-- MOBILE PUNCH 
	SELECT	ROW_NUMBER() OVER(ORDER BY I.Tran_ID) As RowID,E.Emp_ID,T.*,I.CANTEEN_PUNCH_DATETIME AS IO_DateTime,I.USER_ID,I.QUANTITY
	...
	SELECT * FROM #RPT
	UNION ALL
	SELECT * FROM #RPT_MOBILE_NFC_CANTEEN
	...
	SELECT E.Emp_ID,Alpha_Emp_Code, Alpha_Emp_Code + ' - ' + Emp_Full_Name As Emp_Full_Name  FROM #Emp_Cons E INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON E.Emp_ID=EM.Emp_ID;
END
