...
CREATE PROCEDURE [dbo].[SP_RPT_Employee_Present_inout]    
  @Cmp_ID    numeric      
 ,@From_Date   datetime      
 ,@To_Date    datetime       
 ,@Branch_ID   numeric      
...
if @Report_For = 1 or @Report_For = 2 or @Report_For =''        
 Begin      
  CREATE table #Data       
   (       
   Emp_Id   numeric ,       
...
Exec dbo.P0050_UNPIVOT_EMP_ROTATION @Cmp_ID, NULL, @To_Date, @constraint
 ...
 while @Tmp_Date <=@To_Date      
   ...
   */
	...
	--Updating Shift ID from Employee Shift Detail where ForDate=@TempDate 
 ...
 if @Report_For =2       
  begin      
   ...
   select OA.*,E.Emp_Full_Name as Emp_Full_Name,E.Emp_Code,SM.Shift_Name,CM.Cmp_Name,CM.Cmp_Address,Branch_Address,Dept_Name,Comp_Name,Desig_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender   
         
,CONVERT(decimal(10,2), Duration_in_Sec/3600) as Working_Hour ,CONVERT(decimal(10,2), OT_SEc/3600) as OT_Hour  from #Data   OA      
...
Where OT_Sec > 0          
  ...
  Else if @Report_For=''
    ...
    begin
			...
			select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name, dbo.F_Return_Hours(CONVERT(decimal(10,2), sum(Duration_in_Sec))) 	 as Working_Hour ,dbo.F_Return_Hours(CONVERT(decimal(10,2), sum(OT_SEc))) as OT_Hour ,sum(P_days) as Present_Days,E.Emp_Code,SM.Shift_Name,CM.Cmp_Name,CM.Cmp_Address,Branch_Address,Dept_Name,Comp_Name,Desig_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender
    ...
    Group by  OA.Emp_Id,E.Emp_Full_Name,E.Emp_Code,SM.Shift_Name,CM.Cmp_Name,CM.Cmp_Address,Branch_Address,Dept_Name,Comp_Name,Desig_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender
          --Group by OA.emp_ID,E.Emp_Full_Name,E.Emp_Code,SM.Shift_Name,CM.Cmp_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender,OA.For_date,OA.Duration_in_sec,OA.Shift_ID,OA.Shift_Type,OA.Emp_OT,OA.Emp_OT_min_Limit,OA.Emp_OT_max_Limit,OA.P_days,OA.OT_Sec
			--       ,CM.Cmp_Address,Branch_Address,Dept_Name,Comp_Name,Desig_Name
