...
CREATE PROCEDURE [dbo].[SP_Payment_Process]
	@Bank_Flag			Numeric = 0,
	@Branch_ID			Numeric = 0,
	@Emp_ID				Numeric = 0,
	@cmp_id				Numeric = 0,
...
SET ARITHABORT ON
	...
	if @process_type_id > 9000
						...
						BEGIN
							INSERT INTO #PAYMENT_PROCESS
								(	Cmp_ID,Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,Bank_Name, Inc_Bank_AC_No,Payment_Mode,Bank_ID_Two,Payment_Mode_Two,Inc_Bank_AC_No_Two,
									...
									IT_M_ED_Cess_Amount,Salary_Status,process_Type , Ad_Id , BOND_APR_ID,Claim_Apr_Detail_Id
								)
							SELECT	BA.Cmp_ID, BA.Emp_ID, EM.Alpha_Emp_Code, EM.Emp_Full_Name, @From_Date as  Month_St_Date, @To_Date as  Month_End_Date, BM.Bank_Name,   
									...
									EM.Emp_Left, INC.Bank_ID, 0 as IT_M_ED_Cess_Amount, 'Done' as Salary_Status  ,@process_type as process_Type , 0 , Bond_Apr_Id,0 as Claim_Apr_Dtl_ID
			...
			begin
				...
				INSERT INTO #PAYMENT_PROCESS
								(	Cmp_ID,Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,Bank_Name, Inc_Bank_AC_No,Payment_Mode,Bank_ID_Two,Payment_Mode_Two,Inc_Bank_AC_No_Two,
									cmp_bank_ac_no,Cmp_bank_name,Cmp_bank_id,Net_Amount,Branch_ID, Dept_ID, Cat_ID, Type_ID, Desig_Id, Emp_Left, Bank_ID, 
									IT_M_ED_Cess_Amount,Salary_Status,process_Type , Claim_APR_ID,Claim_ID,Claim_Name,Claim_Apr_Detail_Id
								)
							SELECT 	CAD.Cmp_ID, CAD.Emp_ID, EM.Alpha_Emp_Code, EM.Emp_Full_Name, @From_Date as  Month_St_Date, @To_Date as  Month_End_Date, BM.Bank_Name,   
									...
									EM.Emp_Left, INC.Bank_ID, 0 as IT_M_ED_Cess_Amount, 'Done' as Salary_Status  ,@process_type as process_Type , cad.Claim_Apr_ID as Claim_APR_ID ,Cm.Claim_ID as Claim_ID, cm.Claim_Name as Claim_Name,Claim_Apr_Dtl_ID
							FROM    dbo.T0130_CLAIM_APPROVAL_DETAIL AS CAD  WITH (NOLOCK)
									INNER JOIN  dbo.T0040_CLAIM_MASTER AS CM WITH (NOLOCK) ON CAD.Claim_ID = CM.Claim_ID and cm.Claim_Apr_Deduct_From_Sal = 0
				...
				INSERT INTO #PAYMENT_PROCESS
								(	Cmp_ID,Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,Bank_Name, Inc_Bank_AC_No,Payment_Mode,Bank_ID_Two,Payment_Mode_Two,Inc_Bank_AC_No_Two,
									cmp_bank_ac_no,Cmp_bank_name,Cmp_bank_id,Net_Amount,Branch_ID, Dept_ID, Cat_ID, Type_ID, Desig_Id, Emp_Left, Bank_ID, 
									IT_M_ED_Cess_Amount,Salary_Status,process_Type , Claim_APR_ID,Cm.Claim_ID,Claim_Name,Claim_Apr_Detail_Id
								)
							SELECT	CAD.Cmp_ID, CAD.Emp_ID, EM.Alpha_Emp_Code, EM.Emp_Full_Name, @From_Date as  Month_St_Date, @To_Date as  Month_End_Date, BM.Bank_Name,   
									INC.Inc_Bank_AC_No, INC.Payment_Mode,INC.Bank_ID_Two,INC.Payment_Mode_Two,INC.Inc_Bank_AC_No_Two,  
									(SELECT     Bank_Ac_No FROM dbo.T0040_BANK_MASTER WITH (NOLOCK)  WHERE (Is_Default = 'Y') AND (Cmp_Id = CAD.Cmp_ID)) AS cmp_bank_ac_no,  
		...
		ELSE
			BEGIN
					...
					select @ad_id_Multi=Ad_Id_Multi,@Loan_Id_Multi=Loan_id_multi,@Leave_id_Multi=leave_id_multi  
					from T0301_Process_Type_Master WITH (NOLOCK) where Process_Type_Id = @process_type_id and cmp_id= @cmp_id  -- Added by rohit for Loan and Leave on 06012016
					
					--select * from T0301_Process_Type_Master WITH (NOLOCK) where Process_Type_Id = @process_type_id and cmp_id= @cmp_id  -- Added by rohit for Loan and Leave on 06012016
					...
					Begin 
					    ...
					    from T0301_Process_Type_Master WITH (NOLOCK) where Process_Type_Id = @process_type_id and cmp_id= @cmp_id 
						
					
					end
					--End by ronakk 14122022
					...
					--select * from T0200_MONTHLY_SALARY where emp_id = 26972
					--select * from T0210_MONTHLY_AD_DETAIL  where emp_id = 26972 and For_Date = '2023-05-01 00:00:00.000'
					--select * from T0050_AD_MASTER where CMP_ID = 220
			...
			--select * from t0302_process_detail
	...
	ELSE if @process_type = 'Bond'	--Added By Ramiz on 01/11/2018
		BEGIN
			SELECT	P.Cmp_ID,P.Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,
					...
					P.Ad_Id,B.Vertical_ID,B.SubVertical_ID,B.Grd_ID , me.payment_process_id,P.BOND_APR_ID ,BA.Bond_Id, '' as Claim_Name,0 as Claim_APR_ID,0 as Claim_Apr_Detail_Id
			FROM #PAYMENT_PROCESS AS P
				 INNER JOIN(
							SELECT	EMP_ID, Branch_ID, CMP_ID ,I.Vertical_ID,I.SubVertical_ID,I.Dept_ID,I.Grd_ID,I.Desig_Id
							FROM	T0095_INCREMENT I WITH (NOLOCK)
	...
	ELSE if @process_type = 'Claim'
		 ...
		 Select Distinct P.Cmp_ID,P.Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,
					...
					,B.Vertical_ID,B.SubVertical_ID  --Added By Jaina 30-09-2015
					,B.Grd_ID , 0 as BOND_APR_ID --Added By Dhruv 08-02-2017
					,P.Claim_APR_ID,Cm.Claim_ID,cm.Claim_Name,p.Claim_Apr_Detail_Id
			From #Payment_Process As P 
				 INNER JOIN(                  --Added By Jaina 30-09-2015 Start
					SELECT	EMP_ID, Branch_ID, CMP_ID,I.Vertical_ID,I.SubVertical_ID,I.Dept_ID,I.Grd_ID,I.Desig_Id
					FROM	T0095_INCREMENT I WITH (NOLOCK)
				 ...
				 INNER JOIN T0130_CLAIM_APPROVAL_DETAIL CAD WITH (NOLOCK) ON CAD.Claim_Apr_Dtl_ID = P.Claim_Apr_Detail_Id
			...
			where isnull(B.branch_id,0) = isnull(@Branch_ID ,Isnull(B.branch_id,0))    
					...
					and ISNULL(B.Desig_Id,0) = ISNULL(@Desig_Id,ISNULL(B.Desig_Id,0) ) --added By Dhruv(End) 08022017 
					and P.Net_Amount <> 0 ---Added by Jaina 24-11-2017
			order by alpha_emp_code,CM.Claim_ID	
	...
	ELSE
		BEGIN
			...
			Select P.Cmp_ID,P.Emp_ID,Alpha_Emp_Code,Emp_Full_Name,Month_St_Date,Month_End_Date,
					...
					,B.Vertical_ID,B.SubVertical_ID  --Added By Jaina 30-09-2015
					,B.Grd_ID , 0 as BOND_APR_ID --Added By Dhruv 08-02-2017
					,'' as Claim_Name,'' as Claim_APR_ID,'' as Claim_Apr_Detail_Id
			From #Payment_Process As P 
				 INNER JOIN(                  --Added By Jaina 30-09-2015 Start
					SELECT	EMP_ID, Branch_ID, CMP_ID,I.Vertical_ID,I.SubVertical_ID,I.Dept_ID,I.Grd_ID,I.Desig_Id
					FROM	T0095_INCREMENT I WITH (NOLOCK)
