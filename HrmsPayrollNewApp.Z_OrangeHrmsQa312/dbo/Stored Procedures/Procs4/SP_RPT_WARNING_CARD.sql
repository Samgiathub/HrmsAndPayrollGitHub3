...
CREATE PROCEDURE [dbo].[SP_RPT_WARNING_CARD]
	 @CMP_ID 		NUMERIC
	,@FROM_DATE 	DATETIME
	,@TO_DATE 		DATETIME
	,@BRANCH_ID 	NUMERIC
...
SET ARITHABORT ON	
	...
	if @Constraint <> ''
		...
		begin
			...
			select I_Q.* ,E.Emp_Full_Name as Emp_Full_Name,Wd.Warr_Date,CM.Cmp_Name,CM.Cmp_Address,WM.war_name
					,Dept_Name,Desig_Name,Type_Name,Grd_Name,Branch_Name,Date_of_Join,Gender
		from T0080_EMP_MASTER E WITH (NOLOCK) inner join 
		     T0100_Warning_Detail WD WITH (NOLOCK) on E.Emp_ID =WD.Emp_Id inner join
