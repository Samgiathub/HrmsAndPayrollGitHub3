...
CREATE  PROCEDURE [dbo].[SP_RPT_STATUTORY_GET_RECORD_SETT]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
SET ARITHABORT ON
	...
	CREATE table #EMP_PF_REPORT 
		...
		SELECT @PF_Limit = Isnull(PF_LIMIT,0)
		...
		AND For_Date = ( SELECT MAX(For_Date) FROM T0040_GENERAL_SETTING WITH (NOLOCK)
						  WHERE  Branch_ID = @Branch_ID AND Cmp_ID = @Cmp_ID And For_Date<= @To_Date)
	...
	if	exists (select * from [tempdb].dbo.sysobjects where name like '#EMP_SALARY_SETT' )		
		begin
			drop table #EMP_SALARY_SETT
		end
	
		CREATE table #EMP_SALARY_SETT 
			(
				EMP_ID					NUMERIC,
				MONTH					NUMERIC,
				YEAR					NUMERIC,
		...
		set PF_Diff_6500 = PF_SALARY_AMOUNT - PF_Limit
			,PF_833 = 541
			,PF_367 = PF_Amount - 541
		where PF_SALARY_AMOUNT > PF_Limit
		
--select * from #EMP_SALARY_SETT

Declare @Emp_ID_Sett NUmeric(18,0)
Declare @PF_367_Sett NUMERIC
Declare @PF_833_Sett NUMERIC
Declare @PF_DIFF_6500_Sett NUMERIC

Declare curSett cursor for
			select EMP_ID,PF_367,PF_833,PF_DIFF_6500 from #EMP_SALARY_SETT Order By EMP_ID
			open curSett
				fetch next from curSett into @Emp_ID_Sett,@PF_367_Sett,@PF_833_Sett,@PF_DIFF_6500_Sett
				while @@fetch_status = 0
					begin
						if @PF_DIFF_6500_Sett <> 0
							Begin
								Update #EMP_SALARY 
								set	 PF_367 = PF_367+ PF_833 
								where  Emp_ID=@Emp_ID_Sett
								
								Update #EMP_SALARY 
								set	PF_833 =0
								where  Emp_ID=@Emp_ID_Sett
							End
						Else if @PF_DIFF_6500_Sett = 0
							Begin
								Declare @Temp_Amt Numeric
								Declare @Actual Numeric
								set  @Temp_Amt = (541-@PF_833_Sett) 
								select @Actual = PF_833 from #EMP_SALARY where  Emp_ID=@Emp_ID_Sett
								
								if @Actual > @Temp_Amt
									Begin
									
										Declare @Final Numeric
										set @Final=@Actual-@Temp_Amt
		...
		/*SELECT EPF.*, (SALARY_AMOUNT + ISNULL(OTHER_PF_SALARY,0) )SALARY_AMOUNT
				...
				Emp_Full_Name,Grd_Name,Type_Name,dept_Name,Desig_Name
				 ,Cmp_Name,Cmp_Address	
				,@From_Date P_From_Date ,@To_Date P_To_Date
				
				
		  FROM #EMP_PF_REPORT EPF INNER JOIN T0080_EMP_MASTER E  ON EPF.EMP_ID = E.EMP_ID
		  LEFT OUTER JOIN 	#EMP_SALARY_SETT ES ON EPF.EMP_ID = ES.EMP_ID AND EPF.MONTH = ES.MONTH 
