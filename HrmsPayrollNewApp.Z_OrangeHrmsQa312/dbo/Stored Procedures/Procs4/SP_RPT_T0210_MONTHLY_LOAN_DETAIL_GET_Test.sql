...
CREATE PROCEDURE [dbo].[SP_RPT_T0210_MONTHLY_LOAN_DETAIL_GET_Test]
 @Cmp_ID 		numeric
,@From_Date 	datetime
,@To_Date 		datetime
,@Branch_ID 	numeric
...
SET ARITHABORT ON
					...
					From T0140_ADVANCE_TRANSACTION MLD WITH (NOLOCK) Inner join 
						...
						WHERE		E.Cmp_ID = @Cmp_Id and  MLD.For_Date >=@From_Date and MLD.For_Date <=@To_Date 
									...
									--group  by MLD.Emp_ID--,MLD.Adv_Closing
										...
										select ROW_NUMBER() OVER(PARTITION BY Cmp_Name ORDER BY Emp_id ASC) as Rno,* into #Pending_Advance from #Advance_Pending1
	...
	--================================================================================================================================================================================================
		...
		,(select distinct Emp_ID from T0080_emp_Master Where Alpha_Emp_Code=E.alpha_Emp_code and Cmp_Id=@Cmp_ID) as Emp_id
			...
			from 
				...
				group by E.alpha_Emp_code,e.Emp_full_Name,DM.Dept_Name,cm.Cmp_Id,cm.Cmp_name,cm.Cmp_Address--,Ap1.Alpha_Emp_Code,ap1.Adv_Closing--,LM.LOAN_NAME
	...
	--select distinct Emp_id from #LOAN_Data where Cmp_Id=@Cmp_ID and Emp_id in (select Emp_id from #Emp_Cons where emp_id not in (select Emp_id from #Pending_Advance))
	--return

		--while @Countter <= @Countt
		--begin
		...
		select PA.alpha_Emp_code,PA.Emp_full_Name,PA.Dept_Name,PA.Loan_Pending_Amount,PA.Adv_Closing,(isnull(Sum(PA.Loan_Pending_Amount),0)+ isnull(PA.Adv_Closing,0)) as [Total Pending Amount],PA.Cmp_Name,LD.Cmp_Address,PA.[For Date] as 'From Date',PA.[To Date] from #Pending_Advance PA Left outer join #LOAN_Data LD on LD.emp_id=PA.Emp_ID
