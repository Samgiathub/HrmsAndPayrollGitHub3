...
CREATE PROCEDURE [dbo].[SP_RPT_CALCULATE_PRESENT_DAYS_NEW]      
  @Cmp_ID    numeric      
 ,@From_Date   datetime      
 ,@To_Date    datetime       
 ,@Branch_ID   numeric      
...
if @Return_Record_set = 1 or @Return_Record_set = 2 or @Return_Record_set =3  or @Return_Record_set =6   or    @Return_Record_set =7 or @Return_Record_set = 8 OR @Return_Record_set = 9 or @Return_Record_set = 10
 Begin      
   CREATE table #Data         
   (         
   Emp_Id   numeric ,         
...
------------ End By Hardik for OT
     ...
     If @Report_Type = 1 --Pending
		begin
		 ...
		 case when @max_OTDaily = 0 then replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') when dbo.F_Return_Sec(replace(@max_OTDaily,'.',':')) < OA.OT_Sec then replace(@max_OTDaily,':','.') else replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') end as OT_Hour,
		  ...
		  ,DGM.Desig_Dis_No, OTA.Comments,case when OTA.Is_Approved = 1 then 'Approved' else  case when OTA.Is_Approved = 0 then 'Rejected' else 'Pending' end end as OT_Status       --added jimit 24082015	--Comments Added By Ramiz on 04/09/2015
		  INTO #Data_T
		  from #Data   OA      
			...
			Where				(OA.OT_Sec > 0  or OA.Weekoff_OT_Sec > 0 or OA.Holiday_OT_Sec > 0 )  and Is_Approved  is null    
		...
		begin
		 ...
		 case when @max_OTDaily = 0 then replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') when dbo.F_Return_Sec(replace(@max_OTDaily,'.',':')) < OA.OT_Sec then replace(@max_OTDaily,':','.') else replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') end as OT_Hour,
		  ...
		  from #Data   OA      
			...
			Where				(OA.OT_Sec > 0  or OA.Weekoff_OT_Sec > 0 or OA.Holiday_OT_Sec > 0 )  and Is_Approved = 1    
		...
		begin
		 ...
		 case when @max_OTDaily = 0 then replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') when dbo.F_Return_Sec(replace(@max_OTDaily,'.',':')) < OA.OT_Sec then replace(@max_OTDaily,':','.') else replace(dbo.F_Return_Hours(OA.OT_Sec),':','.') end as OT_Hour,
		  ...
		  from #Data   OA      
			...
			Where				(OA.OT_Sec > 0  or OA.Weekoff_OT_Sec > 0 or OA.Holiday_OT_Sec > 0)  and OTA.Is_Approved = 0    
				...
				begin
						if @Report_Type = 1 -- Pending
							begin
								...
								a.Shift_Type ,a.Type_Name ,a.Weekoff_OT_Hour ,a.Weekoff_OT_Sec ,a.Working_Hour , a.Desig_Dis_No , a.Comments,a.OT_Type,a.OT_Status	--Comments Added By Ramiz on 04/09/2015
							end
						else if  @Report_Type = 2 -- Approved
							begin
							...
							begin
								select sum(b.OT_Sec ) as Sum_ot,a.* into #MaxOtData3
 ...
 ----------------Comments By NIlay 3- nov -2010 -----------------------
		    ...
		    --select * from #data
				...
				--Comment By Jaina 24-08-2016 Start
			...
			--	end
			--Comment By Jaina 24-08-2016 End
  ...
  Else If @Return_Record_set = 10
		BEGIN			
			...
			from(
				Select QRy.emp_id , QRy.Emp_code ,  QRy.Emp_Full_Name ,  Sum(Qry.P_days) as WorkDay, 
					(Case WHEN QRy.Shift_ID in (SELECT Shift_ID FROM T0040_SHIFT_MASTER WITH (NOLOCK)
												where CONVERT(VARCHAR(8),Shift_St_Time,108) > CONVERT(VARCHAR(8),Shift_End_Time,108)
			...
			GROUP BY t1.Emp_id,t1.Emp_Full_Name,t1.Emp_code , Shift
					 ...
					 LEFT OUTER JOIN T0040_Vertical_Segment VS WITH (NOLOCK) ON I_Q.Vertical_ID = VS.Vertical_ID
					 LEFT OUTER JOIN T0050_SubVertical SV WITH (NOLOCK) ON I_Q.SubVertical_ID = SV.SubVertical_ID
					 Inner JOIN (	SELECT t1.emp_id as Emp_id, t1.WorkDay as Work_day , Isnull(t2.WorkDayNight,0) as WorkNight FROM #tmp t1	
									LEFT OUTER JOIN (select WorkDay  As WorkDayNight, emp_id from #TMP where Shift = 'Night') t2 ON t1.emp_id=t2.emp_id
									where t1.Shift='Day')QryShift on QryShift.Emp_id = I_Q.Emp_ID					 
