CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET_for_Defecit] 
	@Cmp_ID			numeric,      
	@From_Date		DATETIME,      
	@To_Date		DATETIME ,      
	@Branch_ID		numeric   ,      
...
Where NOT EXISTS (SELECT 1 from  #Data as I Where  I.emp_id = E.Emp_ID AND I.for_Date = E.CurrentDate)
    ...
    Where Row#=1
		...
		-- drop table if exists #smd prapti
		IF Object_ID('tempdb..##defecit') Is not null
			Begin
				Drop Table ##defecit
			End
		
  --		create table #defecit(
		--	shift_Dur numeric(18,0)
		--)
	 ...
	 From #Emp_Inout Inner Join
			...
			select Total_Less_Work_Sec,emp_id,for_Date into ##defecit from #Emp_Inout
