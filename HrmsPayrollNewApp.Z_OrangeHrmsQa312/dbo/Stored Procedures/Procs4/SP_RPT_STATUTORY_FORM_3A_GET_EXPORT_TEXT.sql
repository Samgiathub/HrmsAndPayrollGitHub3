CREATE PROCEDURE [dbo].[SP_RPT_STATUTORY_FORM_3A_GET_EXPORT_TEXT] @Cmp_ID NUMERIC
	,@From_Date DATETIME
	,@To_Date DATETIME
...
CREATE TABLE #EMP_SALARY (
	...
	,Additional NUMERIC(18, 2) -- Added by ronakk 10082023
	)
...
SET PF_Diff_6500 = CASE 
	...
	,Arrear_PF_367 = CASE 
		...
		END
WHERE Pension_Not_Applicable = 1 
...
WHERE Nationality NOT LIKE 'India%'
	AND Nationality <> ''
	AND Nationality NOT LIKE 'BHARAT%' 
...
-- Deepal 01-10-2024
if exists(select 1 from #EMP_SALARY where month(MONTH_ST_DATE) >= 9 and YEAR(MONTH_ST_DATE) >= 2024)
BEGIN

	UPDATE E
	SET  PF_833 = round(PF_SALARY_AMOUNT * 0.0833, 0)
		,PF_367 = PF_Amount - round(PF_SALARY_AMOUNT * 0.0833, 0)
		,PF_LIMIT = PF_SALARY_AMOUNT
		,Additional = round(((PF_SALARY_AMOUNT - PF_LIMIT) * 1.16) / 100,0)
	From #EMP_SALARY E inner join  t0095_Increment I on E.Emp_id  = I.emp_id
	WHERE PF_SALARY_AMOUNT >= PF_Limit and cmp_full_pf = 1 and I.FullPension = 1
...
BEGIN
	SELECT EPF.*
		...
		,Additional as Additonal_Earned_Salary
...
BEGIN
	...
	BEGIN
		...
		SELECT EPF.* --, (SALARY_AMOUNT + ISNULL(OTHER_PF_SALARY,0) ) as SALARY_AMOUNT
			...
			,EDLI_Wages
			,PF_SALARY_AMOUNT
			,Case when FullPension = 1 then PF_833 + Additional else  PF_833 END as PF_833
			,PF_367
			,PF_Diff_6500
			,EMP_SECOND_NAME
			,ES.VPF
			...
			--,Additional as Additional_FullPension
			--,FullPension
		FROM #EMP_PF_REPORT EPF
		INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON EPF.EMP_ID = E.EMP_ID
		LEFT OUTER JOIN T0100_left_emp LE WITH (NOLOCK) ON E.Emp_ID = Le.Emp_ID
