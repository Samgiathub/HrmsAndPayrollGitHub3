...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_CONF_Extension_RECORD_GET]
	 --@Cmp_ID		numeric
...
SET ARITHABORT ON
	...
	ELSE 
		...
		IF @Format_Type = 'Training Extend Confirmation' 
			BEGIN
				SET @Employee_Flag = 'Trainee'
				SET @Employee_Status = 0
			END
		ELSE IF @Format_Type = 'Probation Extend Confirmation' 
			BEGIN
				SET @Employee_Flag = 'Probation'
				SET @Employee_Status = 0
			END	
		...
		IF @Format_Type <> ''
			BEGIN
				...
				,Ep_Q1.New_Probation_EndDate as Extend_New_Probation_Date,Ep_Q1.Old_Probation_EndDate as Extend_Old_Probation_Date		 
				,CM.Cmp_City,ELR.Reference_No,ELR.Issue_Date  --added jimit 03082016
				FROM	T0080_EMP_MASTER E WITH (NOLOCK) INNER JOIN
						...
						( SELECT EP.Emp_Id ,Probation_Status,Evaluation_Date,Extend_Period,EP.New_Probation_EndDate AS New_Probation_EndDate,Flag ,EP.Old_Probation_Period,EP.Old_Probation_EndDate
						 ...
						 T0001_LOCATION_MASTER LM WITH (NOLOCK) ON Lm.Loc_ID = E.Loc_ID INNER JOIN
						
						 (SELECT EP.Emp_Id ,Probation_Status,Evaluation_Date,Extend_Period,EP.New_Probation_EndDate AS New_Probation_EndDate,Flag ,EP.Old_Probation_Period,EP.Old_Probation_EndDate
						  FROM T0095_EMP_PROBATION_MASTER EP WITH (NOLOCK) INNER JOIN
							@Emp_Cons EC1 ON EP.Emp_ID = EC1.Emp_ID INNER JOIN 
							( SELECT MAX(New_Probation_EndDate) AS New_Probation_EndDate , EP2.Emp_ID FROM T0095_EMP_PROBATION_MASTER EP2 WITH (NOLOCK) INNER JOIN @Emp_Cons EC2 ON EP2.Emp_ID = EC2.Emp_ID 
							  WHERE (New_Probation_EndDate <= @To_Date OR EP2.Evaluation_Date <= @To_Date) AND Cmp_ID = @Cmp_ID AND Probation_Status = 1 
								AND Flag = @Employee_Flag GROUP BY EP2.emp_ID  
							) Qry1 ON EP.Emp_ID = Qry1.Emp_ID AND EP.New_Probation_EndDate = Qry1.New_Probation_EndDate	 
						 )Ep_Q1 On Ep_Q1.Emp_ID = EP_Q.Emp_ID left join 
		...
		--				(SELECT Emp_ID FROM T0080_EMP_MASTER WHERE E.EMP_CONFIRM_DATE <= @TO_DATE AND E.EMP_CONFIRM_DATE >= @FROM_DATE) 
