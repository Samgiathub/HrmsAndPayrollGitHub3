CREATE PROCEDURE [dbo].[SP_RPT_EMP_IN_OUT_MUSTER_GET_WITH_DURATION_AND_OVERTIME_BKPS_29122023]    
     @Cmp_ID        numeric    
    ,@From_Date     datetime    
    ,@To_Date       datetime    
    ,@Branch_ID     numeric    
    ...
    IF  exists (select 1 from [tempdb].dbo.sysobjects where name like '#Att_Muster' )           
     ...
     CREATE TABLE #Att_Muster     
      (    
            ...
            Total_Overtime varchar(22),       WO_Count numeric(5,2),   ------ Jignesh Patel 29-12-2021--    
   ...
   --Select Emp_Id From #Att_Muster     
    ...
    ---------------------------------------------/* Here we are Inserting In-Time in Status_1  */-----------------------------------------------------------    
       ...
       ---------------------- End -----------------------    
							...
							SET @AvgHours =
							 case when ISNUMERIC(@WorkingDuration) =1 then
							cast(cast(@avghours as int) + CAST(@WorkingDuration as int) as varchar)
							else
							@avghours
							end 
        ...
        BEGIN           
            Select AM.* , E.Emp_code,E.Emp_full_Name ,Branch_Address,Comp_Name    
				...
				,ATT.P_DAYS,ATT.A_DAYS,ATT.HF_DAYS,ATT.L_DAYS,ATT.WO_DAYS,ATT.HO_DAYS,ATT.PAYABLE_DAYS,ATT.RET_DAYS as RT_Days,    -------------- Add By Deepali-15062023    
				 CASE 
				 WHEN p_days > 0 THEN  
					dbo.F_Return_Hours(CAST((CAST(AM.AvgHour  AS INT)) /P_DAYS AS INT)) /*Added By tejas at 06/11/2023 for Getting Average hour for total working days*/
				 ELSE
					AM.Avghour 
				 END AvgHours
			From #Att_Muster  AM     
