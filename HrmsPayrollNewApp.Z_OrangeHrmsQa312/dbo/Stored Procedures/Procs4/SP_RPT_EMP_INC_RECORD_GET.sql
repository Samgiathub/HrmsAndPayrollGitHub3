...
CREATE PROCEDURE [dbo].[SP_RPT_EMP_INC_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	--,@Branch_ID		numeric   = 0
	...
	,@Reason_Name varchar(200)='' --Added By Nilesh Patel on 22012016
	...
	*/
	...
	--	end
				select I_Q.* ,E.Emp_Full_Name,E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,Left_Date,BM.Comp_Name,BM.Branch_Address,Left_Reason
								...
								,Cmp_Name,Cmp_Address,Present_Street,Present_State,Present_City,Present_Post_Box,l.left_reason,I_Q.Reason_Name
					from T0080_EMP_MASTER E WITH (NOLOCK) left outer join T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
						( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,I.Reason_Name from T0095_Increment I WITH (NOLOCK) inner join 
					...
					WHERE E.Cmp_ID = @Cmp_Id And E.emp_ID in 
							(select Emp_ID from T0095_Increment WITH (NOLOCK) where Increment_Effective_Date >= @From_Date and Increment_Effective_Date <= @To_Date and  Increment_Type='Increment') 
							And E.Emp_ID in (select Emp_ID From #Emp_Cons)
							and isnull(I_Q.Reason_Name,'') = isnull(@Reason_Name ,isnull(I_Q.Reason_Name,''))
