Create PROCEDURE [dbo].[SP_RPT_EMP_IN_OUT_MUSTER_HOME_GET_Backup_26122024]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
AS
	...
	Declare @Increment_Effective_Date	Datetime -- Added by Divyaraj Kiri on 01042023
			...
			ELSE
				BEGIN
					...
					SELECT @SAL_ST_DATE  =SAL_ST_DATE ,@MANUAL_SALARY_PERIOD=IsNull(MANUAL_SALARY_PERIOD ,0),
					  ...
					  --select @SAL_ST_DATE,@MANUAL_SALARY_PERIOD,@IS_CANCEL_HOLIDAY_WO_HO_SAME_DAY,@Tras_Week_OT
				END 
				...
				BEGIN   
					IF @manual_salary_period = 0 
					   BEGIN
							...
							-- Added by Divyaraj Kiri on 01042023
							Select @Increment_Effective_Date = Increment_Effective_Date from T0095_INCREMENT 
							...
							if @Increment_Effective_Date < @Sal_End_Date 							
							begin
								SET @To_Date = @Sal_End_Date 
							end
							else
							begin
								SET @To_Date = @Sal_End_Date + 1
							end
							-- Ended by Divyaraj Kiri on 01042023							
						END 
					ELSE
						BEGIN
	...
	IF day(@Sal_St_Date) =1 or IsNull(@Sal_St_Date,'') = ''
	begin
		SET @New_To_Date = @To_Date --DateAdd(d,@date_diff,@To_Date)
	end
	else
	begin
		-- Added by Divyaraj Kiri on 31032023
		if @Increment_Effective_Date < @To_Date 							
		begin
			SET @New_To_Date = @To_Date 
		end
		else
		begin
			SET @New_To_Date = @To_Date - 1
		end
		-- Ended by Divyaraj Kiri on 31032023
	end
	...
	--Added by Mr.Mehul on 21-06-2023 for Bug #25484

	declare @birthday as numeric,@empactualdate as datetime

	select @birthday = Setting_Value from T0040_SETTING where Cmp_ID = @Cmp_ID and Setting_Name = 'Display Actual Birth Date'
	select @empactualdate = Actual_Date_Of_Birth from T0080_EMP_MASTER where Cmp_ID = @Cmp_ID and Emp_ID = @Emp_ID

	if @birthday = 0 or @empactualdate is null --Added by Mr.Mehul on 21-06-2023 for Bug #25484
	begin
	...
	begin
			update #Att_Muster set Date_of_join = E.Actual_Date_Of_Birth, 
							Left_date = E.EMP_LEFT_DATE,
							Display_Birth=(case when month(Actual_Date_Of_Birth) = month(For_Date) and day(Actual_Date_Of_Birth) = day(For_Date) then 'Y' else 'F' end),  --Added By Jimit 13022019
							Display_Marriage_Date = case when (Emp_Annivarsary_Date IS NULL or Convert(varchar(11),convert(datetime,Emp_Annivarsary_Date),121) = '1900-01-01' or Emp_Annivarsary_Date = '') then 'F' else (case when month(Emp_Annivarsary_Date) = month(For_Date) and day(Emp_Annivarsary_Date) = day(For_Date) then 'Y' else 'F' end)   end --Added By Jimit 13022019
			FROM T0080_EMP_MASTER E INNER JOIN #EMP_CONS EC ON EC.EMP_ID = E.EMP_ID
			inner JOIN #Att_Muster A ON A.Emp_Id = EC.Emp_ID	
	end
...
( 
	...
	-- Deepal 30-Mar-24
	Select A.For_Date, A.In_Date, A.Out_Date , T.In_time  , T.Out_Time , A.Emp_Id , A.cmp_id  into #tmpAtt
	from #TMP_EMP_0150_INOUT T 
	inner join #Att_Muster A on T.Emp_id = A.Emp_Id 
	where  A.In_Date <= T.In_time and A.Out_Date >= T.Out_Time --and 
	order by T.In_Time , T.Out_Time

	Update A1
	Set  A1.For_Date = T1.For_date
	From T0150_EMP_INOUT_RECORD A1 inner join #tmpAtt T1 on 
	A1.In_Time = T1.In_Time and A1.Out_Time = T1.Out_Time and A1.Emp_ID = T1.Emp_Id and A1.Cmp_ID = T1.Cmp_ID
	-- Deepal 30-Mar-24
...
-- Added by rohit on 28012013
	...
	WHERE	(CASE WHEN GS.IS_LATE_MARK = 0 THEN 0 ELSE I.Emp_Late_Mark END) = 1 --COALESCE(@IS_LATE_MARK, I.Emp_Late_Mark) = 1 
		or	(I.Emp_Early_mark = 1 ) --Added by ronakk 04032023
 ...
 Update #Att_Muster  
  ...
  -- Start Commented by Divyaraj Kiri on 19/01/2024
	--update #Att_Muster 
	--set late_minute = 0 ,Early_minute =0
  ...
  -- End Commented by Divyaraj Kiri on 19/01/2024
	
          UPDATE #ATT_MUSTER
	   SET Main_Status = 'A'
	   WHERe Chk_By_Superior = 'Rejected' and Main_Status = 'P'

	      UPDATE #ATT_MUSTER
	   SET Main_Status = 'P'
	   WHERe Chk_By_Superior = 'Approved' 
 --  UPDATE #ATT_MUSTER  
	--SET EARLY_MINUTE = 0
 --  FROM #ATT_MUSTER EL
	--	inner join 
...
--Select * from INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ExmptTable'
--return
...
BEGIN
			...
			--Condtion Added by ronakk 12042023
			IF NOt EXISTS(Select 1 from tempdb.sys.columns where [object_id] = object_id('tempdb..#Att_Muster') and name ='ExFlag')
			BEGIN
...
----Added by Sid 25062014 Ends
				...
				IF @Report_For = 'Att Reg Report'
					...
					begin
						...
						SET @w_sql = @w_sql + 'Select --AM.*,
							...
							,Am.OT_Applicable, Am.Display_Birth, Am.Display_Marriage_Date, Am.Date_of_join, Am.Left_date
							--, ltrim(Rtrim(Am.ExFlag)) as EXFLAG
							,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + '' '' + AM.late_limit as datetime))late_time 
							,DateDiff(mi,AM.for_date,cast(cast(AM.for_date as varchar(11)) + '' '' + AM.Early_Limit as datetime))early_time 
							,AM.late_minute as late_minutes
							,AM.Early_minute as early_out
	...
	-- Added by Divyaraj Kiri on 01042023
	IF @LEFT_DATE < @TO_DATE 
		SET @TOTAL = DateDiff(D,@LEFT_DATE,@New_To_Date) + 1
	ELSE IF @LEFT_DATE <= @FROM_DATE
		SET @TOTAL = DateDiff(D,@LEFT_DATE,@New_To_Date) + 1
	ELSE
		SET @TOTAL = DateDiff(D,@Temp_Date,@New_To_Date) + 1
	-- Ended by Divyaraj Kiri on 01042023
