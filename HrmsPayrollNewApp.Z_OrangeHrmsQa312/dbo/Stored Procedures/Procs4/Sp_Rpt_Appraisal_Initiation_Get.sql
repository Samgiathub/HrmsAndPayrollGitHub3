...
CREATE PROCEDURE [dbo].[Sp_Rpt_Appraisal_Initiation_Get]
	 @CMP_ID 		NUMERIC
	,@FROM_DATE 	DATETIME
	,@TO_DATE 		DATETIME
	,@BRANCH_ID 	NUMERIC
...
Declare @Emp_Cons Table
	...
	Select HAI.Appr_Int_Id,HAI.Cmp_Id,HAID.Emp_Id,HAI.For_date,HAID.Start_Date,HAID.End_date,EM.Emp_Code,EM.Emp_Full_Name,Em.Branch_Id,Em.Grd_ID,Em.Desig_Id,Em.Dept_Id, LO.Login_Name,BM.Comp_Name,BM.Branch_Address,CM.Cmp_Name,CM.Cmp_Address 
    ...
    --INNER JOIN dbo.T0040_Grade_Master GM ON Em.Grd_ID = GM.Grd_ID
    --INNER JOIN dbo.T0040_Department_Master DM ON DM.Cmp_Id = HAI.Cmp_Id 
    --INNER JOIN dbo.T0040_Designation_Master DEM ON DEM.Cmp_Id = HAI.Cmp_ID
    INNER JOIN dbo.T0010_Company_Master CM WITH (NOLOCK) ON Em.Cmp_Id = CM.Cmp_ID
    Where For_Date >=@From_Date and For_Date <=@To_Date And HAI.Cmp_Id=@Cmp_Id
