...
CREATE PROCEDURE [dbo].[SP_RPT_ATTENDANCE_MUSTER_GET]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
SET ARITHABORT ON
	...
	update #Att_Muster
	set Leave_Count = Leave_Used + isnull(Lt.CompOff_Used,0)
	...
	and Am.For_Date >=@From_Date and Am.For_Date <=@To_Date  -- Changed By Gadriwala Muslim 01102014
...
update #Att_Muster
	...
	set Status = Leave
	from #Att_Muster AM inner join 
	(select Emp_Id ,count(Status) as Leave From #Att_Muster 
		Where Status = 'L' and For_Date>=@From_Date and For_DAte <=@to_Date 
		group by Emp_ID)Q on Am.Emp_ID = Q.Emp_ID
	where Row_ID = 34
