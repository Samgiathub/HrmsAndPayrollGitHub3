CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_Notification]
	@Emp_ID numeric(18,0),
	@Cmp_ID numeric(18,0),
...
ELSE IF @Type = 'B' --- For BirthDay & Anniversary Notification
	BEGIN
		...
		EXEC Get_Birthday_Anniversary_reminder @Cmp_ID = @Cmp_ID,@pPrivilage_ID = '',@pPrivilage_Department = '',@pPrivilage_Vertical = '',
		@pPrivilage_Sub_Vertical = ''

		
		--SELECT DISTINCT TB.Emp_ID,TB.Cmp_ID,
		----VM.EmpName,
		--TB.Dept_Name,TB.Desig_Name,TB.Branch_Name,
		----VM.Branch_Code As 'Branch_Code',
		----VM.Groupby,
		----CONVERT(varchar,VM.Date,103) AS 'Date',
		--TB.Gender,
		...
		----'' AS 'DocPath',
		----VM.CommentCount,VM.LikeCount,
		----ISNULL(EL.Like_Flag,0) AS 'Like_Flag'--,
		--Total_Year
		--FROM #tblBirthday TB
		----INNER JOIN View_BirthdayAlert VM ON TB.Emp_ID = VM.Emp_ID
		----LEFT JOIN T0400_Employee_Like EL ON VM.Emp_Id = EL.Emp_ID AND MONTH(VM.Date) = MONTH(EL.For_date) 
		----AND DAY(VM.Date) = DAY(EL.For_date) AND EL.Emp_Like_Id = @Emp_ID 
		----AND VM.NotificationFlag = EL.Notification_Flag
		--WHERE Emp_FullName NOT LIKE '&nbsp%' 
		----and [Date] <> '01/01/1900' -- Added by Niraj(05012022)
		...
		VM.Groupby,
		CONVERT(varchar(11),VM.Date,103) AS 'Date',
		TB.Gender,
		(CASE WHEN ISNULL(VM.Image_Name,'') = '' OR VM.Image_Name = '0.jpg' THEN (CASE WHEN TB.Gender = 'M' 
		THEN 'Emp_Default.png' ELSE 'Emp_Default_Female.png' END) ELSE VM.Image_Name END) AS 'Image_Name',
		...
		FROM #tblBirthday TB
		INNER JOIN View_BirthdayAlert VM ON TB.Emp_ID = VM.Emp_ID
		LEFT JOIN T0400_Employee_Like EL ON VM.Emp_Id = EL.Emp_ID AND
		    MONTH(CONVERT(datetime,(VM.Date),103)) =  MONTH(CONVERT(datetime,(EL.For_date),103))
		AND DAY(CONVERT(datetime,(VM.Date),103)) = DAY(CONVERT(datetime,(EL.For_date),103)) 
		AND EL.Emp_Like_Id = @Emp_ID 
		AND VM.NotificationFlag = EL.Notification_Flag
