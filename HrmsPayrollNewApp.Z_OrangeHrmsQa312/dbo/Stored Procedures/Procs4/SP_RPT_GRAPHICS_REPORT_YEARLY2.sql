...
CREATE PROCEDURE [dbo].[SP_RPT_GRAPHICS_REPORT_YEARLY2]  
  @Cmp_ID   numeric  
 ,@From_Date  datetime  
 ,@To_Date   datetime  
 ,@Branch_ID  numeric  
...
SET ARITHABORT ON 
 ...
 if @Constraint <> ''  
  ...
  CREATE table #Yearly_Salary   
   ...
   select @Cmp_ID,emp_ID,3,'ESIC' From @Emp_Cons   
   ...
   Begin  
    ...
    if @count = 1   
     begin  
      ...
      set Month_1 = M_AD_Amount  
      From #Yearly_Salary  Ys  inner join T0210_Monthly_AD_Detail ms on ys.emp_ID = ms.emp_ID   
       inner join T0050_AD_Master AD  on ms.AD_ID= AD.AD_ID  
