...
CREATE PROCEDURE [dbo].[SP_RPT_T0100_WEEKOFF_ADJ_GET]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
SET ARITHABORT ON
   ...
   and I.Emp_ID in       
	...
	--Ronakb010824 add vertical for groupby
		Select ES.* ,Emp_full_Name,Emp_Code,Comp_Name,Branch_Address,Branch_NAme,V.Vertical_Name,SubVertical_Name,SubBranch_Name,Grd_Name,Dept_NAme,Type_Name,Desig_NAme,E.Alpha_Emp_Code
		...
		From #Weekoff_Report ES inner join T0080_Emp_master E WITH (NOLOCK) on Es.Emp_ID = E.Emp_ID inner join 
			( select I.Emp_Id , Grd_ID,Branch_ID,Vertical_ID,SubVertical_ID,SubBranch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Increment_effective_Date  from T0095_Increment I WITH (NOLOCK) inner join 
				...
				on E.Emp_ID = I_Q.Emp_ID  inner join
					...
					T0050_SubBranch SB  WITH (NOLOCK) ON I_Q.SubBranch_ID = SB.SubBranch_ID Inner join
