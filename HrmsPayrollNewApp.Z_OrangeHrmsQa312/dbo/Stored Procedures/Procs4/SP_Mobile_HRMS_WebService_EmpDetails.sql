CREATE PROCEDURE [dbo].[SP_Mobile_HRMS_WebService_EmpDetails]
	@Emp_ID numeric(18,0),
	@Cmp_ID numeric(18,0),
...
ELSE IF @Type = 'E' -- For Get Employee Details
	BEGIN
		...
		SELECT Emp_ID,VM.Cmp_ID,Alpha_Emp_Code AS 'Emp_code'
		,isnull(convert(varchar,convert(date,'27-December-2020',103),103),'') AS 'Emp_Annivarsary_Date',Emp_Full_Name_new
		,isnull(CONVERT(varchar(11),Date_Of_Join,103),'') AS 'Date_Of_Join'
		...
		'' AS 'Image_Path',Loc_name,BM.Bank_Name,VM.Bank_Branch_Name,Father_name
		,isnull(CONVERT(varchar(11),Date_Of_Birth,103),'') AS 'DOB',
		...
		-- start added by Niraj(14022022)
		Marital_Status, Emp_Full_Name, Emp_PT,
		City, [State], Zip_code, Mobile_No, Work_Tel_No, Sup_Mobile_No, Work_Email, Other_Email 
		-- End added by Niraj(14022022)
