...
CREATE PROCEDURE [dbo].[SP_RPT_EMPLOYEE_LOAN_DATA_NIIT]
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
...
--EL.*,lM.LOAN_NAME
	   select EL.*,lM.LOAN_NAME,E.Emp_Code,E.Alpha_Emp_Code,E.Emp_First_Name,E.Emp_Full_Name as Emp_Full_Name,E.Emp_Full_Name as Emp_Full_Name_only,Emp_superior      
	  ...
	  from T0080_EMP_MASTER E WITH (NOLOCK) inner join           
		...
		t0120_lOAN_aPPROVAL EL WITH (NOLOCK) on E.Emp_Id=EL.Emp_Id inner join    t0040_lOAN_MASTER lM WITH (NOLOCK)
		ON EL.LOAN_ID =LM.LOAN_ID INNER JOIN    
	 ...
	 and  isnull(EL.Loan_ID,0) = isnull(@Is_Attend ,isnull(EL.Loan_ID,0)) 
      ...
      --And E.emp_ID in (select Emp_ID from t0120_loan_approval where LOan_APr_Date <= @To_Date and  isnull(Loan_ID,0) = isnull(@Is_Attend ,isnull(Loan_ID,0)) 
