CREATE PROCEDURE [dbo].[SP_RPT_EMP_INOUT_RECORD_GET_CONSOLIDATED_Test]      
    @Cmp_ID         NUMERIC,      
    @From_Date      DATETIME,      
    @To_Date        DATETIME ,      
    @Branch_ID      NUMERIC   ,      
...
AS      
	...
	DECLARE @test_Leave                       AS VARCHAR(MAX) -- added by Yogseh   
	DECLARE @test1_Leave                      AS VARCHAR(MAX) -- added by Yogseh   
    DECLARE @test1                      AS VARCHAR(MAX) 
    DECLARE @Col_Str                    AS VARCHAR(Max)
	DECLARE @Col_Str1                    AS VARCHAR(Max) -- added by Yogseh
	DECLARE @Col_Str_Sum1                AS VARCHAR(Max) -- added by Yogseh
	...
	--===============================================================================================
    CREATE NONCLUSTERED INDEX ix_Leave_Used_EmpId_LeaveId on #Leave_Used_1 (Emp_Id,Leave_Id)   
    ...
    CREATE NONCLUSTERED INDEX ix_Leave_Used_EmpId_LeaveId_ForDate on #Leave_Used (Emp_Id,For_date,Leave_Id)      
	--===============================================================================================
    CREATE table #Data     
    (     
        Emp_Id     NUMERIC ,     
    ...
    SET @Leave_Name = '' 
    SET @Col_Str = ''
	SET @Col_Str1 = '' --added by yogesh
	SET @Col_Str_Sum1 = ''    --added by yogesh
    SET @Col_Str_Sum = ''      
...
SELECT p1.Emp_ID,p1.For_Date,
    ...
    CASE WHEN  LM.APPLY_HOURLY =1 THEN (p1.Leave_Used*.125) ELSE  0 END END  Leave_Used_1,
      ...
      LEFT outer JOIN 
      (Select Emp_ID,LD.Half_Leave_Date as From_Date, Leave_Assign_As  From T0120_LEAVE_APPROVAL US Inner Join  
...
---select * from #Emp_Inout
 ...
 End 
  ...
  ----------- */
	...
	Update E SET A_days = 0 FROM #Emp_Inout E WHERE IsNull(E.Leave_Days, 0) = 1 --- added by Hardik 05/10/2020 for Honda as showing absent on Full Day leave as punches are there.
    --2000
   ...
   --- When give version to Honda, Uncomment Below Code, it's specific for Emerland honda, 21/07/2020
	...
	---- Added by Hardik for Emerland Honda on 21/07/2020
		...
		--UPDATE #Emp_Inout SET LATE_IN = NULL, EARLY_OUT=NULL, Late_In_count = 0,  Early_Out_Count = 0
		...
		--FROM #Emp_Inout L INNER JOIN #Late_Early_Deduction LED ON L.EMP_ID = LED.Emp_ID AND L.FOR_DATE=LED.For_Date
		...
		--FROM #Emp_Inout ATT INNER JOIN #Late_Early_Deduction LED ON ATT.EMP_ID = LED.Emp_ID AND ATT.FOR_DATE = LED.For_Date
   	...
   	If @Fill_Table =1 
   	Begin
   		Insert INTO #TempEmp_Inout Select * from #Emp_Inout
   		Return
   	End
    ...
    DECLARE @cols nVARCHAR( max),
            ...
            @cols1 nVARCHAR( max)
			--==================================================================================================
			 DECLARE @cols_leave nVARCHAR( max),
            @query_Leave nVARCHAR(max),
            @cols1_Leave nVARCHAR( max)
			--==================================================================================================
	...
	DECLARE @colsPivot_leave_1 VARCHAR(max)-- added by Yogesh
...
--====================================================================================================
	...
	SELECT @colsPivot_leave_1 = STUFF((SELECT ',' + QUOTENAME(cast(Leave_Name AS VARCHAR(max))) 
                                    from (SELECT Distinct Leave_Name FROM  #Leave) Q
                                    order by Leave_Name 
                            FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')
...
--====================================================================================================
    ...
    ---19 12 2019
    SET @query = 'Alter  Table #Consolidated_Temp Add P_Days NUMERIC(18,2), A_Days NUMERIC(18,2), Weekoff NUMERIC(18,2), Holiday NUMERIC(18,2), Leave NUMERIC(18,2),PD NUMERIC(18,2)'
	...
	--=================================================================================
			 ...
			 SELECT distinct LT.Emp_ID,LT.Leave_ID,qry.Leave_Used as Leave_Used,qry.For_Date as For_Date,LM.Leave_Name into #abc
    From T0140_LEAVE_TRANSACTION LT inner JOIN 
    (
        SELECT Emp_ID,Leave_ID,Sum(Leave_Used) as Leave_Used,Max(For_Date) AS For_Date  FROM T0140_LEAVE_TRANSACTION group by Emp_ID,Leave_ID,Leave_Used,For_Date
    ) AS qry ON qry.For_Date = LT.For_Date and qry.Emp_ID = LT.Emp_ID and qry.Leave_ID = LT.Leave_ID 
    inner JOIN #Emp_Inout1 EO on LT.Emp_ID = EO.Emp_ID 
    inner JOIN T0040_LEAVE_MASTER LM on LM.Leave_ID = LT.Leave_ID
    Where  --LT.Leave_Closing <> 0 and 
			LM.Leave_Name <> 'LWP' 
			and isnull(lm.Display_leave_balance,0)=1
			and Qry.For_Date between @from_date and @To_Date
			and LT.Leave_Used<>0  
			--select * from #Emp_Inout1
			group by LT.Leave_ID,LT.Emp_ID,qry.Leave_Used,LM.Leave_Name,qry.For_Date
			
			select Emp_ID,Leave_ID,sum(Leave_Used) as Leave_Used,Leave_Name into #tempLeave from #abc Group by Emp_ID,Leave_ID,Leave_Name


	-- insert INTO #Leave_Used
	
	select distinct  tl.Emp_ID,tl.Leave_ID,tl.Leave_Used,Max(ab.For_Date)as For_Date,tl.Leave_Name into #Final_Leave from #abc AB
	inner join #tempLeave TL on Tl.Emp_ID=ab.Emp_ID and tl.Leave_ID=ab.Leave_ID --and ab.For_Date=Max(ab.For_Date)
	group by tl.Emp_ID,tl.Leave_ID,tl.Leave_Used, For_Date,tl.Leave_Name

	--select * from #Final_Leave
	insert INTO #Leave_Used
	SELECT distinct LT.Emp_ID,LT.Leave_ID,LT.Leave_Used,qry.For_Date,LM.Leave_Name
    From #Final_Leave LT inner JOIN 
    (
        SELECT Emp_ID,Leave_ID,Max(For_Date) AS For_Date  FROM T0140_LEAVE_TRANSACTION group by Emp_ID,Leave_ID
    ) AS qry ON qry.For_Date = LT.For_Date and qry.Emp_ID = LT.Emp_ID and qry.Leave_ID = LT.Leave_ID 
    inner JOIN #Emp_Inout1 EO on LT.Emp_ID = EO.Emp_ID 
    inner JOIN T0040_LEAVE_MASTER LM on LM.Leave_ID = LT.Leave_ID
	--inner join #tempLeave Tl on tl.Emp_ID=LT.Emp_ID 
    Where  --LT.Leave_Closing <> 0 and 
			LM.Leave_Name <> 'LWP' 
			and isnull(lm.Display_leave_balance,0)=1
...
--===========================================================================================
	...
	DECLARE @Leave_Name_2 VARCHAR(100)
	DECLARE Att_Master1 CURSOR FOR  
	
	
    SELECT distinct  Leave_Name + '_Used' FROM #Leave_Used group by Leave_Name
   
  
	OPEN Att_Master1
    FETCH NEXT FROM Att_Master1 into @Leave_Name_2
	
    WHILE @@FETCH_STATUS = 0
        BEGIN
		
            --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 10:01'
            SET @Leave_Name_2=replace(@Leave_Name_2,' ','_')
            SET @Leave_Name_2=replace(@Leave_Name_2,'-','_')
            SET @test_Leave ='alter  table #Emp_Inout ADD ['+ @Leave_Name_2 +']  decimal(18,2) default ''''' 
                 
			
            IF @Col_Str1 = ''
                SET @Col_Str1 =  'Case When OrderColumns = 1 Then ' + @Leave_Name_2 + ' Else Null End AS ' + @Leave_Name_2
            else
                SET @Col_Str1 = @Col_Str1 + ',' + 'Case When OrderColumns = 1 Then ' + @Leave_Name_2 + ' Else Null End AS ' + @Leave_Name_2 --@Leave_Name_1   

            IF @Col_Str_Sum1 = ''
                SET @Col_Str_Sum1 =  'SUM(Isnull('+ @Leave_Name_2 + ',0)) AS ' + @Leave_Name_2 + ''
            else
                SET @Col_Str_Sum1 = @Col_Str_Sum1 + ',' + 'SUM(Isnull('+ @Leave_Name_2 + ',0)) AS ' + @Leave_Name_2 + ''
            ...
            --SET @test1 ='Update EI  SET '+ @Leave_Name_1 + ' = LB.Leave_Closing FROM #Emp_Inout EI inner join #Leave_Balance LB on  LB.EMP_ID = EI.Emp_ID and replace(LB.Leave_Name,'' '',''_'') = ''' + @Leave_Name_1 + ''''
            SET @test1_Leave ='Update EI  SET '+ @Leave_Name_2 + ' = LC.Leave_Used FROM #Emp_Inout EI inner join( SELECT Min(For_Date) AS For_Date FROM #Emp_Inout group by Emp_ID) qry on qry.For_Date = EI.For_Date inner join #Leave_Used LC on  LC.EMP_ID = EI.Emp_ID and replace(LC.Leave_Name,'' '',''_'')+''_Used'' = ''' + @Leave_Name_2 + ''''
           --select @test1
        ...
        End 
    CLOSE Att_Master1         
    DEALLOCATE Att_Master1 
	
    --===========================================================================================
--===============================================================================================================
IF @Col_Str1 = ''
        BEGIN
            SET @Col_Str1 =  'Case When OrderColumns = 1 Then Leave_Used Else Null End AS Leave_Used ' 
            SET @Col_Str_Sum1 =  '0 AS Leave_Used'
        END
--===============================================================================================================
	...
	CREATE table #Leave_Used_Count_Pivot(Emp_id NUMERIC,ID INT);

    SET     @ALTER_COLS = null
    Select  @ALTER_COLS = COALESCE(@ALTER_COLS + ';', '') + 'ALTER TABLE  #Leave_Used_Count_Pivot ADD ' + DATA + ' VARCHAR(32)' FROM dbo.Split(@colsPivot_leave_1, ',');
    EXEC (@ALTER_COLS);
	
	
    --PRINT CONVERT(VARCHAR(20), GETDATE(), 114) + ' STEP 13'
	    SET @query_leave = 'insert into ##Leave_Used_Count_Pivot SELECT Emp_id,1 AS ID,'+@colsPivot_leave_1+' 
            from (SELECT Emp_ID,Leave_Name,Leave_Days FROM #leave) 
            as data pivot 
            ( sum(Leave_Days) 
            for Leave_Name in ('+ @colsPivot_leave_1 +') ) p' 
--=======================================================================================================================================
	...
	ALTER TABLE  #Final_Table ADD PD VARCHAR(10);
	...
	--select * from #Final_Table
	 ...
	 SET     @ALTER_COLS = null
    Select  @ALTER_COLS = COALESCE(@ALTER_COLS + ';', '') + 'ALTER TABLE  #Final_Table ADD ' + DATA + ' VARCHAR(20)' FROM dbo.Split(@colsPivot_leave_1, ',');
    EXEC (@ALTER_COLS);
--select * from #consolidated_Temp
    ...
    IF not @query Is null      
        Begin
            ...
            SET @query= 'Insert INTO #Final_Table SELECT * FROM ( SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols1 + ',P_Days,A_Days,Weekoff,Holiday,'+@colsPivot_leave_1+'Leave,' + @colsPivot_leave + ' FROM #consolidated_Temp C Left Outer Join #Leave_Count_Pivot L On C.Emp_Id = L.Emp_Id and C.OrderColumns = L.Id
                Where PvtColumns = ''In_Time'' or PvtColumns = ''Out_Time''
                Union
                SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols + ',P_Days,A_Days,Weekoff,Holiday,'+@colsPivot_leave_1+'Leave,' + @colsPivot_leave + 'from #consolidated_Temp C Left Outer Join #Leave_Count_Pivot L On C.Emp_Id = L.Emp_Id and C.OrderColumns = L.Id
    ...
    Else
        Begin
            ...
            SET @query= 'Insert INTO #Final_Table SELECT * FROM (SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols1 + ',P_Days,A_Days,Weekoff,Holiday,Leave,P_Days as PD FROM #consolidated_Temp C 
                Where PvtColumns = ''In_Time'' or PvtColumns = ''Out_Time''
                Union
                SELECT C.Emp_Id,OrderColumns,PvtColumns,' + @Cols + ',P_Days,A_Days,Weekoff,Holiday,Leave,P_Days as PD FROM #consolidated_Temp C 
                Where PvtColumns = ''Duration'' or PvtColumns = ''WOHO'' or PvtColumns = ''Shift_Time'' or PvtColumns = ''OverTime'' or PvtColumns = ''Late_Hrs'')p '
        End
	 ...
	 DECLARE @WO_Inc as tinyint,
							...
							@Late_Early_Ded_Combine numeric(18,2),	
							@Holiday numeric(18,2),	
							@LC_Days numeric(18,2),	
							@EC_Days numeric(18,2),	
							@Paid_Leave numeric(18,2),
							@Absent_Leave numeric(18,2),
 ...
 --  return
		...
		FAST_FORWARD 
		FOR 
		SELECT	A.Emp_ID,E.Cmp_ID,I.Branch_ID 
		FROM	#Final_Table A INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK)  ON E.Cmp_ID=@Cmp_ID AND A.Emp_Id=E.Emp_ID			
		...
		WHERE ISNULL(E.Extra_AB_Deduction,0) = 0 AND A.OrderColumns=1;
	...
	--select P_days from  where Emp_ID=21566 and OrderColumns=1
	
	--select T.P_Days,*
 --    From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),P_days)) P_Days FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id and t.OrderColumns=1
 --    Where Pvtcolumns = 'In_Time' 
	-- return
	...
	--set @Payable_Present_days=0
			 ...
			 select @WeekOff_Days= E.Weekoff
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),WeekOff_Days)) Weekoff FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id-- and t.OrderColumns=1
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	--select @Weekoff_Days
	
	 	 select @Holiday_Days= E.Holiday
      From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Count(AB_LEAVE) Holiday FROM #Emp_Inout WHERE AB_LEAVE = 'HO' or AB_LEAVE = 'HO-C' or AB_Leave = 'OHO' Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	 --select @Holiday_Days
	   --return
	 	 select @Paid_Leave= E.Leave
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),Leave)) Leave FROM #Emp_Inout where Isnull(Leave_Days,0)>0 Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	 
	 --	 select @Present_Days= (select distinct P_days from #Consolidated_Temp Where Pvtcolumns = 'In_Time' and Emp_ID=@Emp_ID)
		
		  select @Present_Days=E.P_Days
     From #Final_Table T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),P_days)) P_Days FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
  ...
  --   Where Pvtcolumns = 'In_Time' 
	 ...
	 select @LC_Days= E.Late_In
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),Late_In)) Late_In FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	
	  select @EC_Days= E.Early_Out
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),Early_Out)) Early_Out FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	
	 select @Absent_Leave= E.A_days
     From #Consolidated_Temp T Inner Join (SELECT Emp_Id, Sum(Convert(numeric(18,2),A_days)) A_days FROM #Emp_Inout Group by emp_id) E on T.Emp_Id=E.emp_id 
     Where Pvtcolumns = 'In_Time' and E.Emp_ID=@EmpID
	 
	--select @WeekOff_Days,@Paid_Leave,@Absent_Leave,@EC_Days,@LC_Days,@Present_Days,@Holiday_Days
 --return
	...
	--return
			...
			--select @HO_Inc,@Wo_Inc
		--	select @Payable_Present_days as a
			...
			--select @Payable_Present_days as b
			IF ISNULL(@Wo_Inc,0) = 1
				SET @Payable_Present_days = @Payable_Present_days +  ISNULL(@WeekOff_Days,0)	
				
				--	select @Payable_Present_days as c 

				if ISNULL(@Late_Early_Ded_Combine,0)=1


			SET	@Payable_Present_days = (@Payable_Present_days + ISNULL(@Paid_Leave,0) )--- (ISNULL(@LC_Days,0)-(ISNULL(@EC_Days,0)) ))
			
			--select @Payable_Present_days as d
		...
		--	select @EmpID
			...
			UPDATE	#Final_Table SET PD= @Payable_Present_days
			WHERE	Emp_Id = @EmpID  AND OrderColumns=1
    ...
    SET @query = 'SELECT Case When OrderColumns = 1 Then Row_No Else NULL End AS Sr_No,
            ...
            Case When OrderColumns = 1 Then Desig_Name Else Null End Designation_Name,
              ...
              case when OrderColumns = 1 Then VS.Vertical_Name Else Null End AS Vertical_Name,
						...
						Case When OrderColumns = 1 Then F.PD Else Null End PD,
                ...
                (SELECT Emp_Id,Grd_Id,Branch_Id,Desig_ID,Dept_Id,Type_Id, dbo.F_Return_Hours(sum(late_in_sec)) AS Late_In_Hours,
                        ...
                        count(case when Early_Out_sec = 0 then Null else 1 end) AS Early_Days,dbo.F_Return_Hours(SUM(Shift_Sec)) AS Total_Hours,dbo.F_Return_Hours(SUM(Total_Work_Sec)) AS Total_Require, dbo.F_Return_Hours(SUM(Total_Less_work_sec)) AS Total_Short_hours,'+@Col_Str_Sum1+','+ @Col_Str_Sum + '
