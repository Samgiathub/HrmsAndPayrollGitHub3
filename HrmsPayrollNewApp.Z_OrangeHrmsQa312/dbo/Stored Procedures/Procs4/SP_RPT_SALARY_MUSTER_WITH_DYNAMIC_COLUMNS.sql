...
CREATE PROCEDURE [dbo].[SP_RPT_SALARY_MUSTER_WITH_DYNAMIC_COLUMNS]
 @Cmp_ID		numeric
,@From_Date		datetime
,@To_Date		datetime 
,@Branch_ID		numeric   = 0
...
SET ARITHABORT ON
	...
	WHILE @@FETCH_STATUS = 0
					...
					If @Sal_Type = 0
						Begin
							select @P_Days = isnull(Present_Days,0) ,@Sal=Basic_Salary,@A_Days = isnull(Absent_Days,0),@W_Days=isnull(Weekoff_Days,0),@T_Days=isnull(Working_Days,0),@TDS=isnull(M_IT_TAX,0), @Basic_Salary = Salary_Amount, @Act_Gross_salary = Actually_Gross_salary,@Settl = Settelement_Amount,@OTher_Allow = ISNULL(Other_Allow_Amount,0),@Total_Allowance = Allow_Amount, @Leave_Amount = isnull(Leave_Salary_Amount,0),
								...
								,@Late_Deduct_Amount = Late_Dedu_Amount
		...
		--				VALUES     (@Emp_ID, @Company_ID, @Transaction_ID, @Month, @Year, 'Other Dedu', @Other_Dedu,'',@Row_ID)
						...
						select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'Late Dedu.'
								UPDATE    dbo.#Temp_Salary_Muster_Report
								SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = @Late_Deduct_Amount, 
											  Value_String = '',M_AD_Flage='D'
								WHERE     (Label_Name = 'Late Dedu.') AND (Row_id = @Row_ID) and Emp_ID = @Emp_ID			
	...
	SELECT	[P Days] AS PDays,[A Days] As ADays,[W Days] As WDays,[T Days] As TDays,Emp_ID,Cmp_ID
	INTO	#PRESENSE
	FROM	(
