...
CREATE PROCEDURE [dbo].[SP_RPT_MONTHLY_LEAVE_BALANCE_GET] @Cmp_ID NUMERIC
	,@From_Date DATETIME
	,@To_Date DATETIME
	,@Branch_ID NUMERIC
	,@Cat_ID NUMERIC
	...
	INNER JOIN (
		...
		WHERE cmp_Id = @Cmp_Id
			AND @To_Date <= (
				CASE 
					WHEN Leave_Status = 0
						THEN InActive_Effective_Date
					ELSE @To_date
					END
				) --Added By Jimit 03082018 only showing Active Leave and for In Active Leave consider Effective Date    
...
BEGIN
	...
	FROM T0040_LEAVE_MASTER WITH (NOLOCK)
	WHERE Cmp_ID = @Cmp_ID
		AND Display_leave_balance = 1 --commented jimit 14062016  -- uncomment by Jaina 20-12-2018    
		AND @To_Date <= (
			CASE 
				WHEN Leave_Status = 0
					THEN InActive_Effective_Date
...
IF @CONSTRAINT <> ''
	...
	WHERE Date_Of_Join < @To_Date
		AND IsNull(Emp_Left_Date, @To_Date) > @From_Date
ELSE
	INSERT INTO #EMP_CONS
	SELECT E.EMP_ID
		,I.INCREMENT_ID
...
SELECT @Leave_Bal_Display_FixOpening = Leave_Balance_Display_FixOpening
FROM T0010_COMPANY_MASTER WITH (NOLOCK)
WHERE Cmp_Id = @cmp_Id

IF @Leave_Bal_Display_FixOpening = 1
	AND EXISTS (
		SELECT 1
...
BEGIN
	...
	INNER JOIN T0140_LEAVE_TRANSACTION LT ON LB.Emp_ID = LT.Emp_ID
		AND LB.Leave_ID = LT.Leave_ID
	INNER JOIN (
		SELECT EMP_ID
			,LEAVE_ID
	...
	INNER JOIN (
		...
		GROUP BY Emp_ID
			,Leave_ID
		) Q ON lb.LEave_ID = Q.LEave_ID
		AND Lb.emp_ID = Q.Emp_ID
...
BEGIN
	...
	UPDATE #Emp_Leave_Bal
	SET Leave_Opening = CASE 
			WHEN LT1.FOR_DATE = @From_date
				THEN LT.Leave_Opening
			ELSE Lt.Leave_Closing
			END
	FROM #Emp_Leave_Bal LB
	INNER JOIN T0140_LEAVE_TRANSACTION LT ON LB.Emp_ID = LT.Emp_ID
		AND LB.Leave_ID = LT.Leave_ID
	...
	INNER JOIN (
		...
		WHERE Cmp_ID = @Cmp_ID
			AND (
				Leave_Posting IS NULL
				OR Leave_Posting <> 0
				)
			AND LEave_ID IN (
				SELECT Leave_ID
				FROM #Leave_ID
				) --= @Leave_ID    --Change by Jaina 24-01-2019    
	...
	INNER JOIN (
		SELECT Emp_ID
			...
			,Sum(Half_Payment_Days) AS Half_Payment_Days
	...
	INNER JOIN (
		...
		INNER JOIN (
			SELECT max(For_Date) For_Date
				...
				,leave_ID
			FROM T0140_leave_Transaction WITH (NOLOCK)
			WHERE CAST(CAST(For_date AS VARCHAR(11)) AS DATETIME) <= @To_Date
				AND Cmp_ID = @Cmp_ID
				AND LEave_ID IN (
					SELECT Leave_ID
					FROM #Leave_ID
		...
		AND LB.Emp_ID = leave_Bal.Emp_ID
END
...
IF EXISTS (
		SELECT 1
		FROM #Emp_Leave_Bal
		WHERE LEAVE_ID = @COMP_OFF_LEAVE_ID
		)
BEGIN
	...
	SET Leave_Used = CO.Leave_Used
		,Leave_opening = CO.leave_opening
		,--Added by Yogesh on 29-12-2023   
		Leave_Closing = CO.Leave_Closing
	FROM #Emp_Leave_Bal L
	INNER JOIN #temp_CompOff CO ON L.EMP_ID = CO.EMP_ID
		AND L.LEAVE_ID = CO.LEAVE_ID
...
INNER JOIN (
	...
	INNER JOIN (
		SELECT max(I2.Increment_ID) AS Increment_ID
			,I2.Emp_ID
		FROM T0095_Increment I2 WITH (NOLOCK) -- Ankit 08092014 for Same Date Increment    
		INNER JOIN (
...
INNER JOIN T0050_LEAVE_DETAIL LD WITH (NOLOCK) ON LD.Leave_ID = EL.Leave_ID
	AND LD.Grd_ID = IQ.Grd_ID --ADDED BY RAMIZ ON 13/09/2017    
