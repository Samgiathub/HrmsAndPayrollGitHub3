...
CREATE PROCEDURE [dbo].[SP_Rpt_Consolidation_Statement_Abstract_Report]
	@Cmp_ID Numeric(18),
	@From_Date Datetime,
	@To_Date Datetime
...
SET ARITHABORT ON
	...
	Create Table #Allowance_Details_1
	(
		Cmp_ID Numeric(18,0),
		Report_ID Numeric(18,0),
		Employee_Type Numeric(18,0),
	...
	Select Dept_Id,Dept_Name From T0040_DEPARTMENT_MASTER WITH (NOLOCK) Where Cmp_Id = 20 --and Dept_Id = 153
	Open Cur_Department
		...
		Fetch Next From Cur_Rport_Head into @Cur_Cmp_ID,@Cur_Report_ID,@Cur_Employee_Type,@Cur_Earning_Component_ID,@Cur_Earning_Short_Name,@Cur_Deduction_Component_ID,@Cur_Deduction_Short_Name,@Cur_Loan_ID,@Cur_Loan_Short_Name,@Cur_Dept_ID_1,@Cur_Dept_Name_1
			While @@fetch_status = 0
				Begin
					...
					(Select SUM(Loan_Pay_Amount) as LoanPayAmount,
					AD_temp.Report_ID,AD_temp.Employee_Type,AD_temp.Dept_ID From #Allowance_Details_temp AD_temp Inner JOIN  
	...
	--Insert into #Allowance_Details_1
	--(
	--	Cmp_ID,
	--	Report_ID,
	--	Employee_Type,
	...
	--Group by Cmp_ID,Report_ID,Employee_Type,To_Date,Flag
	
	--Select * From #Allowance_Details_1
	...
	--	From #Allowance_Details_1 AD Inner JOIN T0030_Report_Header_Master HM
	...
	--	Group by HM.Report_Header_Name,AD.To_Date,AD.Report_ID,AD.Flag,AD.GrossAllowanceSum,AD.RecoveriesSum,AD.NetSum
	 ...
	 Update #Allowance_Details
	    ...
	    From 
	    (Select SUM(AD.Amount) as Amount,Report_ID From  #Allowance_Details AD
	     Inner JOIN (SELECT SUM(EMp_ID) as EMp_ID ,EM.Type_ID as Type_ID,AD1.Dept_ID From #Allowance_Details AD1 inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK)
		...
		on AD.Employee_Type = qry_12.Type_ID and AD.Dept_ID = qry_12.Dept_ID
	     Where Flag = 1 group BY Report_ID
	    ) t
	    
	    Update #Allowance_Details
		Set RecoveriesSum = t.Amount
	    From 
	    (Select SUM(Amount) as Amount From  #Allowance_Details AD
		Inner JOIN (SELECT SUM(EMp_ID) as EMp_ID ,EM.Type_ID as Type_ID,AD1.Dept_ID From #Allowance_Details AD1 inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK)
		on EM.Dept_ID = AD1.Dept_ID GROUP BY Type_ID,AD1.Dept_ID) as qry_12
		on AD.Employee_Type = qry_12.Type_ID and AD.Dept_ID = qry_12.Dept_ID
	    Where Flag = 2
	...
	Select HM.Report_Header_Name,SUM(isnull(AD.Amount,0)) as Report_Header,AD.To_Date,AD.Report_ID,AD.Flag,AD.GrossAllowanceSum,AD.RecoveriesSum,AD.NetSum From #Allowance_Details AD
	Inner JOIN (SELECT SUM(EMp_ID) as EMp_ID ,EM.Type_ID as Type_ID,AD1.Dept_ID From #Allowance_Details AD1 inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK)
	on EM.Dept_ID = AD1.Dept_ID GROUP BY Type_ID,AD1.Dept_ID) as qry_12
	on AD.Employee_Type = qry_12.Type_ID and AD.Dept_ID = qry_12.Dept_ID
	Inner JOIN T0030_Report_Header_Master HM WITH (NOLOCK)
		ON HM.Report_Id = AD.Report_ID 
		Group by HM.Report_Header_Name,AD.To_Date,AD.Report_ID,AD.Flag,AD.GrossAllowanceSum,AD.RecoveriesSum,AD.NetSum
	...
	Union 
		...
		From #Allowance_Details AD Inner JOIN (SELECT SUM(EMp_ID) as EMp_ID ,EM.Type_ID as Type_ID,AD1.Dept_ID From #Allowance_Details AD1 inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK)
		on EM.Dept_ID = AD1.Dept_ID GROUP BY Type_ID,AD1.Dept_ID) as qry_12
		on AD.Employee_Type = qry_12.Type_ID and AD.Dept_ID = qry_12.Dept_ID
		Inner JOIN T0030_Report_Header_Master HM WITH (NOLOCK)
		ON HM.Report_Id = AD.Report_ID 
