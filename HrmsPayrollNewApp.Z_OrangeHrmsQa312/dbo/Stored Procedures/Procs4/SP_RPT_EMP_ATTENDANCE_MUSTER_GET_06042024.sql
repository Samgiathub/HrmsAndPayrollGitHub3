create PROCEDURE [dbo].[SP_RPT_EMP_ATTENDANCE_MUSTER_GET_06042024]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
AS
	...
	--Start Deepal  To Check the late mark scenario - 2 DT :- 22-03-24
	...
	--End Deepal  To Check the late mark scenario - 2 DT :- 22-03-24

	IF @Export_Type = 'Calendar'  or @Export_Type = 'Attendance Regularization' or  (@Report_For='Complete_Absent' And @Export_Type='999')
		BEGIN
	...
	*/
	...
	Declare @Cur_Impt_Emp_ID as numeric   
	-- Added by Gadriwala Muslim 15052015
...
----PRINT 'STATE 7.2 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
   ...
   ----PRINT 'STATE 7.4 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
	...
	--- Added by Mihir Adeshara 07062012
		...
		--Deepal Century Enka Late Early Scenario 2 Develop by Jignesh bhai 20-03-2024 ----------------------- jignesh patel 01-Dec-2021---------------
		IF  @Late_Mark_Scenario = 2 and @LateEarly_Combine = 1 and @LateEarly_MonthWise = 0 and Exists(select 1 from T0050_GENERAL_LATEMARK_SLAB where DEDUCTION_TYPE = 'Hours' and CMP_ID = @Cmp_ID)
		BEGIN
					...
					(
						...
						Select @Increment_Id = Increment_Id from #Emp_cons where Emp_id = @emp_id
						
						exec SP_CALCULATE_LATE_EARLY_DEDUCTION_COMBINE_MULTIPLE_EXEMPT @Emp_ID,@Cmp_ID,@From_Date,@To_Date,@Late_Absent_Day output
						,@Total_LMark output,@Total_Late_Sec output,@Increment_ID,@StrWeekoff_Date,@StrHoliday_Date,0,'',0,'',0,1 --Changes by ronakk 17052023

						SELECT   
								@Late_Absent_Day = 
								(Isnull(sum([dbo].[F_Return_Sec](replace(Late_Deduction,'.',':')) ),0) + 
								Isnull(sum([dbo].[F_Return_Sec](replace(Early_Deduction,'.',':')) ),0)) / (3600*8),
		...
		-- Deepal Add the Late mark scenario  22-03-24
		IF  @Late_Mark_Scenario = 2  and @LateEarly_Combine = 1 and @LateEarly_MonthWise = 0 and Exists(select 1 from T0050_GENERAL_LATEMARK_SLAB where DEDUCTION_TYPE = 'Hours' and CMP_ID = @Cmp_ID)
		BEGIN
			UPDATE #Att_Muster
			SET STATUS = isnull(Round(@Late_Absent_Day,2),0)
			WHERE row_ID = 37
		END
		-- Deepal Add the Late mark scenario  22-03-24
...
--select * from  #Att_Muster
	...
	While @@fetch_status = 0                    
			Begin  
				...
				if((select 1 from T0100_EMP_RETAINTION_STATUS OA  where OA.Emp_Id = @curemp_id and OA.Cmp_Id= @curCM_ID 
				and  CONVERT(VARCHAR(20),CAST(@curTempDate AS DATETIME),101)  between CONVERT(VARCHAR(20),CAST(OA.Start_Date AS DATETIME),101) 
				and CONVERT(VARCHAR(20),CAST(OA.End_Date AS DATETIME),101) and OA.Is_Retain_ON = 0)=1)
					Begin
					print @curTempDate
					print @curemp_id
						update #Att_Muster set Status = 'RT' ,A_Days= 0 , WO_HO_Day = 0 where For_Date = @curTempDate and Emp_Id =@curemp_id  and Cmp_ID=@curCM_ID
					    set @Days_cnt = @Days_cnt+1
						set @curTempDate  = DATEADD(day,1,@curTempDate) 
					End
	...
	deallocate Curs_Retain
		...
		Update #Att_Muster set Status = ( select count (A_Days) from #Att_Muster where For_Date between @From_Date and @To_Date and A_days= 1 and  Emp_Id =@curemp_id  and Cmp_ID=@curCM_ID )  where Row_Id = 33 and  Emp_Id =@curemp_id  and Cmp_ID=@curCM_ID
		Update #Att_Muster set Status = ( select count (WO_HO_Day) from #Att_Muster where For_Date between @From_Date and @To_Date and WO_HO_Day= 1  and  Emp_Id =@curemp_id  and Cmp_ID=@curCM_ID)  where Row_Id = 35  and  Emp_Id =@curemp_id  and Cmp_ID=@curCM_ID
