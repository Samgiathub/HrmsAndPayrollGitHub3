...
-- =============================================
CREATE PROCEDURE [dbo].[SP_RPT_EMP_CMP_TRANSFER_LOANDETAIL_HISTORY]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric  = 0
...
SET ARITHABORT ON
	...
	Else 
		...
		Select CLT.Emp_Id as Old_Emp_Id,CLT.Cmp_Id as Old_Cmp_ID, --CLT.Loan_Id AS Old_Loan_Id,OLM.Loan_Name as Old_Loan_Name,CLT.Old_Balance,
				CLT.New_Emp_Id as New_Emp_Id,CLT.New_Cmp_Id as Cmp_ID,CLT.Loan_Id,LM.Loan_Name as Loan_Name,CLT.Old_Balance
		From 
		--T0140_LOAN_TRANSACTION lt inner join 
		--	(Select max(For_Date) For_Date,Emp_ID,Loan_ID From T0140_LOAN_TRANSACTION 
		...
		--		group by Emp_ID,Loan_ID) q on lt.Emp_ID=q.Emp_ID and lt.Loan_ID=q.Loan_ID and lt.For_Date=q.For_Date inner join 
			T0040_LOAN_MASTER lm  WITH (NOLOCK) Inner Join 
			T0100_EMP_COMPANY_LOAN_TRANSFER CLT WITH (NOLOCK) ON clt.Loan_Id = lm.Loan_ID INNER JOIN
			#Emp_Cons_temp EC on CLT.Emp_Id = EC.Old_Emp_ID 
		--where Case When Loan_Closing > 0 Then Loan_Closing Else Old_Balance end > 0 
		--and	  CLT.Cmp_Id = @Cmp_Id	
