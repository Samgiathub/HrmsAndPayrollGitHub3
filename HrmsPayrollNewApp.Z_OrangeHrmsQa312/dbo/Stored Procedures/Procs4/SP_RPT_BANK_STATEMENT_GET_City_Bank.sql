...
CREATE PROCEDURE [dbo].[SP_RPT_BANK_STATEMENT_GET_City_Bank]  
  @Cmp_ID			numeric  
 ,@From_Date		datetime  
 ,@To_Date			datetime  
 ,@Branch_ID		numeric  
...
ELSE
 ...
 ELSE   
		BEGIN
			...
			Revenue_Amount = Revenue_Amount + s_Revenue_Amount, 
			PT_F_T_Limit = S_PT_F_T_Limit
			FROM #Emp_Salary ES 
				...
				BEGIN
					 IF @Director_details = 0
						BEGIN
							...
							,Type_Name,Dept_Name,Desig_Name,PAN_no,DAte_of_Birth,SSN_No as PF_No,SIN_No as ESIC_No ,dbo.F_Number_TO_Word(Net_Amount) as Net_Amount_In_Word  
							...
							Case When bk.Bank_Name like '%UTI%' or bk.Bank_Name like '%Axis%' Then Cast(RIGHT ('000000000000' + cast(tem.Total_Amount as varchar(15)),15) As varchar(20)) Else CAST(tem.Total_Amount as varchar(30)) End As Total_Amount,
							...
							Case When bk.Bank_Name like '%UTI%' or bk.Bank_Name like '%Axis%' Then Cast(RIGHT ('000000000000' + cast(Net_Amount as varchar(15)),15) As varchar(20)) Else CAST(Net_Amount as varchar(30)) End As Net_Amt
							...
							,cast(I_Q.Inc_Bank_Ac_no as varchar(30)) + '  '+ bk.Bank_Code + '    ' +'C'+ Cast(RIGHT ('00000000000000' + cast(Net_Amount as varchar(17)),17) As varchar(20)) + ' BYSALARY' as text_string --added by krushna 18112019 for HMP
							,'Sal - '+ LEFT(DATENAME(MONTH,Month_End_Date), 3)+' ' + RIGHT(YEAR(Month_End_Date), 2) AS Customer_Refer_No,2 as Ac_Type
							,E.Emp_code as Emp_Code
							,( '="' + BAM.Bank_Ac_No + '"')  as  Company_Bank_AC_NO
							INTO #FINAL_TABLE
							FROM #Emp_Salary MS 
							INNER JOIN		T0080_EMP_MASTER E WITH (NOLOCK) on MS.emp_ID = E.emp_ID
							INNER JOIN		T0095_Increment I_Q WITH (NOLOCK) on Ms.Increment_ID = I_Q.Increment_ID
							...
							LEFT OUTER JOIN T0040_BANK_MASTER BAM WITH (NOLOCK) on CM.Cmp_Id = BAM.Cmp_ID and BAM.Is_Default = 'Y'
							WHERE E.Cmp_ID = @Cmp_Id   
							--and isnull(i_Q.Bank_ID,0) = isnull(@Bank_ID,isnull(i_Q.Bank_ID,0)) -- and Salary_Amount >0
							and I_q.Payment_mode = @Payment_mode  
							and Month(Month_End_Date) = Month(@To_Date) and Year(Month_End_Date) =Year(@To_Date)
						...
						if @Export_Type <> 4 
							...
							begin
								SELECT	Company_Bank_AC_NO as 'Debit Account'
										,'ST' as 'Payment Type'
										,convert(varchar(10),getdate(),112) as 'Value Date'
										,upper(left(DATENAME(month,Month_Last_Date),3)) + right(cast(year(Month_Last_Date)as varchar(4)),2) + Alpha_Emp_Code as 'Transaction Reference'
										,Alpha_Emp_Code as 'EMP Code'
										,Emp_full_Name as 'Beneficiary Name'
										,Inc_Bank_Ac_no1 as 'Beneficary Account Number'
										,2 as 'Beneficary Account Type'
										,Ifsc_Code as 'IFSC Code'
										,'Salary for ' + upper(left(DATENAME(month,Month_Last_Date),3)) +' '+ cast(year(Month_Last_Date)as varchar(4)) as 'Payment Details 1'
										,'' as 'Payment Details 2'
