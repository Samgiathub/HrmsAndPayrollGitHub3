CREATE PROCEDURE Get_Rpt_Claim_Details_format3_dtl                
   @Cmp_ID NUMERIC                                      
  ,@From_Date  DATETIME                                  
  ,@To_Date   DATETIME                                  
  ,@Branch_ID  VARCHAR                         
...
--   set @Salary_Cycle_id = null                      
 ...
 IF @Constraint <> ''                                  
    BEGIN                                         
   select distinct cm.Claim_Name,cd.Claim_Apr_Code,cd.Claim_App_Ttl_Amount,  
   EM.Emp_Full_Name,EM.Emp_ID,EM.Branch_ID,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,  
   ...
   and cd.Claim_Apr_Date between    @From_Date and @To_Date  and Cm.Claim_id=@Claim_Id
