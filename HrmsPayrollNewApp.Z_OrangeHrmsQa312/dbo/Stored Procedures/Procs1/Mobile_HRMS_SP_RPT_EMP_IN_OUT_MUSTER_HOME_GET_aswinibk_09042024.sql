CREATE PROCEDURE [dbo].[Mobile_HRMS_SP_RPT_EMP_IN_OUT_MUSTER_HOME_GET_aswinibk_09042024]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
AS
	...
	FROM	T0040_GENERAL_SETTING GS 
			INNER JOIN (SELECT  MAX(FOR_DATE) FOR_DATE 
						...
						WHERE	FOR_DATE <= @TO_DATE --AND CMP_ID = @CMP_ID 
						AND BRANCH_ID = IsNull(@BRANCH_ID,BRANCH_ID)) QRY ON QRY.FOR_DATE = GS.FOR_DATE
	WHERE	--CMP_ID = @CMP_ID AND
	GS.BRANCH_ID = IsNull(@BRANCH_ID,GS.BRANCH_ID)	
	...
	Select	T.emp_id,emp_full_name,Alpha_Emp_Code,cast(Alpha_Emp_Code as varchar) + ' - '+ Emp_Full_Name as Emp_Name_Code,
			...
			Dm.Desig_Name ,T.Date_Of_Join,T.Cmp_ID,co.Cmp_Name,I.CTC  
			,I.Desig_Id  
			,I.Increment_ID
	INTO #TMP
	...
	*********************************************************************/
	...
	--Here Condition is Added By Ramiz on 14/04/2016 after discussion with Hardik Bhai
		...
		If @Report_For <> 'ABSENT_CON'
			--EXEC SP_GET_HW_ALL @CONSTRAINT=@CONSTRAINT,@CMP_ID=@Cmp_ID, @FROM_DATE=@FROM_DATE, @TO_DATE=@TO_DATE, @All_Weekoff = 0, @Exec_Mode=0 Deepal commenting the duplicate line 17082022
			EXEC SP_GET_HW_ALL @CONSTRAINT=@CONSTRAINT,@CMP_ID=@Cmp_ID, @FROM_DATE=@FROM_DATE, @TO_DATE=@TO_DATE, @All_Weekoff = 0, @Exec_Mode=0
		END
	...
	While @for_Date <= @Att_date
		BEGIN
			...
			UPDATE	AM SET SHIFT_ID = Shf.Shift_ID
			...
			WHERE	--SM.Cmp_ID=@Cmp_ID AND
			R.R_DayName = 'Day' + CAST(DATEPART(d, @for_Date) As Varchar) AND
			...
			------------------------------
				...
				set @Days_cnt =0 
			if((select 1 from T0100_EMP_RETAINTION_STATUS OA  INNER JOIN #EMP_CONS EC ON OA.EMP_ID = EC.EMP_ID and 
			OA.Cmp_Id= @Cmp_ID and 
			CONVERT(VARCHAR(20),CAST(@FOR_DATE AS DATETIME),101)  between CONVERT(VARCHAR(20),CAST(OA.Start_Date AS DATETIME),101) and CONVERT(VARCHAR(20),CAST(OA.End_Date AS DATETIME),101) and OA.Is_Retain_ON = 0)=1)
					Begin
	...
	from #Att_Muster AM inner join				--Changed By Gadriwala Muslim 01102014
		...
		(
			...
			( 
			 ...
			 LEFT OUTER JOIN T0040_LEAVE_MASTER AS EA ON LT1.LEAVE_ID = EA.LEAVE_ID 
			 WHERE LT1.LEAVE_USED > 0 AND EA.CMP_ID  = @CMP_ID 
 ...
 Update #Att_Muster  
  ...
  update #Att_Muster 
	...
	--end Krushna 13-02-2019
		...
		---- Deepal 17082022 Commented
		--exec rptLateEarlyCombination @Cmp_ID=@Cmp_ID,@From_Date=@From_Date,@To_Date=@To_Date,@Branch_ID=@Branch_ID,@Cat_ID=@Cat_ID
	...
	ELSE IF @Report_For = 'BulkRegularization_Mobile'
		BEGIN
			...
			Select	IsNull(AM.Emp_Id,0) As 'Emp_Id',IsNull(E.Cmp_ID,0) As 'Cmp_ID',IsNull(AM.branch_ID,0) AS 'branch_ID',IsNull(AM.For_Date,0) AS 'For_Date',
					...
					, MIO.Location
					, IsNull(E.Desig_Id,0) As 'Desig_Id'	-- Added by Divyaraj Kiri on 16/01/2024
					,IsNull(E.Dept_ID,0) As 'Dept_ID'	-- Added by Divyaraj Kiri on 24/01/2024
					,IsNull(E.Cat_ID,0) As 'Cat_ID'	-- Added by Divyaraj Kiri on 24/01/2024
					,IsNull(CM.Cat_Name,0) As 'Cat_Name'	-- Added by Divyaraj Kiri on 24/01/2024
					FROM #Att_Muster  AM 
					INNER JOIN T0080_EMP_MASTER E ON AM.EMP_ID = E.EMP_ID
					INNER JOIN (SELECT	I.Branch_ID,I.Grd_ID,I.Dept_ID,I.Desig_ID,I.Emp_ID,IsNull(I.Emp_Late_mark,0) Emp_Late_mark,IsNull(I.Emp_Early_mark,0) Emp_Early_mark 
								FROM	T0095_Increment I 
					...
					LEFT OUTER JOIN T0030_CATEGORY_MASTER CM ON CM.Cat_ID = E.Cat_ID	-- Added by Divyaraj Kiri on 24/01/2024
