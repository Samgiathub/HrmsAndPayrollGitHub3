...
CREATE PROCEDURE [dbo].[P0040_BUSINESS_SEGEMENT]        
 ...
 If Upper(@tran_type) ='I'    
   begin    
    ...
    INSERT INTO T0040_business_Segment    
                          (Segment_Id, Cmp_Id, Segment_Code, Segment_Name, Segment_Description,Is_MachineBased,MachineEmpType,IsActive,InActive_EffeDate)    
    VALUES     (@Segment_ID,@Cmp_Id,@Segment_code,@Segment_Name, @Segment_Description,@Is_MachineBased,@MachineEmpType,@IsActive,@InEffedate)  ----added by aswini 25/12/2023   )    
 ...
 Else If  Upper(@tran_type) ='U'     
   begin    
    ...
    if exists (Select Segment_ID  from T0040_business_Segment WITH (NOLOCK) Where Upper(Segment_Code) = Upper(@Segment_Code)and Segment_ID <> @Segment_ID and Cmp_ID = @Cmp_ID)     
     begin    
      ...
      Return     
          ...
          select @OldSegmentName  =ISNULL(Segment_Name,'') ,@OldCode  =isnull(Segment_Code,''), @OldSegmentDescription = ISNULL(Segment_Description,''),
		   @OldActive=isnull(IsActive,0),@OldEffDate=isnull(InActive_EffeDate,'')  --added by aswini 25/12/2023
		  From dbo.T0040_business_Segment WITH (NOLOCK) Where Cmp_ID = @Cmp_ID and Segment_ID = @Segment_ID      
    ...
    SET       Segment_Name = @Segment_Name, Segment_Code = @Segment_Code, Segment_Description = @Segment_Description     
	   ...
	   IsActive=@IsActive,InActive_EffeDate =@InEffedate   ----added by aswini 21/12/2023    
    WHERE     Segment_Id = @Segment_ID    
        
    set @OldValue = 'old Value' + '#'+ 'Segment Name :' + @OldSegmentName  + '#' + 'Segment Code:' + @OldCode  + '#' + 'Segment Description :' + @OldSegmentDescription   + '#' +   +  'IsActive :'+ @OldActive +'#' + 'InActive_EffeDate :' + @OldEffDate + '#' 
 ...
 Else If  Upper(@tran_type) ='D'    
   ...
   -- Add by nilesh patel on 09042016 --End    
    ...
    BEGIN    
     ...
     Return    
    END     
    select @OldSegmentName  =ISNULL(segment_Name,'') ,@OldSegmentDescription  =ISNULL(Segment_Description,''),@OldCode  =isnull(segment_Code,''),
	@OldActive= isnull(IsActive,0),@OldEffDate=isnull(InActive_EffeDate,'') ----added by aswini 25/12/2023
	From dbo.T0040_business_Segment WITH (NOLOCK) Where Cmp_ID = @Cmp_ID and Segment_ID = @Segment_ID      
        
    DELETE FROM T0040_business_Segment WHERE Segment_Id = @Segment_ID    
         
    set @OldValue = 'old Value' + '#'+ 'Segment Name :' +ISNULL( @OldSegmentName,'') + '#' + 'Segment Code :' + ISNULL( @OldCode,'') + '#' + 'Segment Description :' + ISNULL(@Segment_Description,'')  + '#' +  'IsActive :'+ @OldActive +'#' + 'InActive_EffeDate :' + @OldEffDate + '#'    ----added by aswini 21/12/2023     
    -----    
   End    
