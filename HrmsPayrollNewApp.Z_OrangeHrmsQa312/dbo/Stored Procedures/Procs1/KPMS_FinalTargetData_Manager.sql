CREATE PROCEDURE [dbo].[KPMS_FinalTargetData_Manager]                                                                                                                                                                         
(                            
 @Cmp_ID INT,                           
 @Emp_ID int,                      
 @GoalAlt_ID INT                                                
...
'                       
		...
		PIVOT                      
		   ...
		   SET @Title = ''<h1><td>Section Name</td><td>Goal Name</td><td>SubGoal Name</td><td>Depended_On</td><td>Depended_Type</td>'+replace(replace(replace(replace(@PivotColDedVar,'cast(',''),'as varchar(20))',''),'''',''),'+','')+'<td>Target</td><td>Actual_TargetValue</td><td>Frequency</td><td>LevelAchi</td><td>Score</td></h1>''            
		 ...
		 select @lResult6 = @lResult6 +  ''<b><p class="d-block font-weight-bold name">  '' + Emp_Full_Name + ''   :  '' + comment + ''   -  '' + ''</b><span class="date text-black-50">'' + date + ''</span></p>'' from kpms_tblComment  as tc inner join KPMS_T0020_Goal_Allotment_Master_Test on Goal_Allot_ID = goalAlt_id inner join T0080_EMP_MASTER as em on em.Emp_ID = tc.Eid where goal_Allot_ID= '+ cast(@GoalAlt_ID as varchar(20)) +' and tc.Cmp_Id ='+ cast(@Cmp_ID as varchar(20)) +'         
		  ...
		  select @Result as Result2 , @Title as Result3  ,@lResult4  as Result4 ,@lResult6  as Result6 , @lSatusResult as SatusResult         
   '               
   --select @Sql
  exec (@Sql)                     
