...
Create PROCEDURE [dbo].[Get_File_Dashboard_Details_Backupbymanisha_04122024]
	@Cmp_ID Numeric(18,0),
   ...
   if @ChartType = 'header'
		Begin
					...
					-- For Header Part
					 ...
					 select 
					 COUNT((Case When FA.F_StatusId = 0 or  FA.F_StatusId = 1 THEN 1 END)) as Pending,
					 COUNT((Case When FA.F_StatusId = 2 THEN 1 END)) as Hold,
					 COUNT((Case When FA.F_StatusId = 3 THEN 1 END)) as Forward,
					 COUNT((Case When FA.F_StatusId = 5 THEN 1 END)) as SentbackReview,
					 COUNT((Case When FA.F_StatusId = 4 THEN 1 END)) as Approved,   ---comment out by Manisha for Bug #32106

					  (select COUNT((Case When FAP.F_StatusId = 4 THEN 1 END))from  T0080_File_Approval FAP where FAP.Cmp_ID=@Cmp_ID) as Approved,
					 COUNT(1) as totalFile
					from T0080_File_Application FA
						inner join T0095_EMP_SCHEME es on es.Emp_ID=fa.Emp_ID and type='File Management' --added	
					 and es.Effective_Date=(select max(Effective_Date) from T0095_EMP_SCHEME where type='File Management' and Emp_Id=es.Emp_ID )--added on 30-09-22
			
					left join T0030_File_Status_Common FSC on FSC.S_ID = FA.F_StatusId
	...
	Else If @ChartType = 'pie'
		Begin
		   ...
		   End
					...
					SELECT 
					  (case when FSC.S_Name='Approved' then (select COUNT(Isnull(fap.File_App_Id,0)) FROM T0080_File_Application FA WITH (NOLOCK)
				inner join T0080_File_Approval fap on fap.File_App_Id=fa.File_App_Id where Fa.Cmp_ID=@Cmp_ID
				and fa.CreatedDate >= @FROM_DATE and fa.CreatedDate <= @TO_DATE) else COUNT(Isnull(fa.File_App_Id,0))end ) as Ticket_Count
				 
				  --COUNT(Isnull(fa.File_App_Id,0)) as Ticket_Count
				 ,(Case 
						When fA.F_StatusId = 0  then 'Pending' 
						else FSC.S_Name
			
				END) as File_Status
				  ,CAST(DATENAME(MONTH,@FROM_DATE) AS VARCHAR(3)) + '-' + CAST(YEAR(@FROM_DATE) AS VARCHAR(4)) AS MONTH_NAME
				FROM T0080_File_Application FA WITH (NOLOCK)
					inner join T0095_EMP_SCHEME es on es.Emp_ID=fa.Emp_ID and type='File Management' --added	
					 and es.Effective_Date=(select max(Effective_Date) from T0095_EMP_SCHEME where type='File Management' and Emp_Id=es.Emp_ID )--added on 30-09-22
			
				left join T0030_File_Status_Common FSC on FSC.S_ID = FA.F_StatusId
				Where  Fa.Cmp_ID=@Cmp_ID
				and fa.CreatedDate >= @FROM_DATE and fa.CreatedDate <= @TO_DATE
				GROUP BY fa.F_StatusId,fsc.S_Name
	...
	Begin
	      ...
	      SELECT 
		  	  (case when FSC.S_Name='Approved' then (select COUNT(Isnull(fap.File_App_Id,0)) FROM T0080_File_Application FA WITH (NOLOCK)
				inner join T0080_File_Approval fap on fap.File_App_Id=fa.File_App_Id where Fa.Cmp_ID=@Cmp_ID
				) else COUNT(Isnull(fa.File_App_Id,0))end ) as Ticket_Count
		--  COUNT(Isnull(fa.File_App_Id,0)) as Ticket_Count
		 ,(Case 
				When fA.F_StatusId = 0  then 'Pending' 
				else FSC.S_Name
	...
	Else if @ChartType = 'BarMod'
		...
		End
	Else if @ChartType = 'Total File'
		Begin
		  
		       -- for total records of file 
		      

				select ROW_NUMBER() OVER(ORDER BY File_app_id ASC) AS RowID,
                FORMAT(app_date, 'dd-MM-yyyy') as Application_Date
				--Application_Date
				,Alpha_Emp_Code,Emp_Full_Name,File_Number as Document_Number,File_Type_Number as File_Number,Subject,ApplicationStatus
				from V0080_File_App_Admin_Side
		        Where Cmp_ID = @Cmp_ID
	...
	Begin
				...
				,Alpha_Emp_Code,Emp_Full_Name,File_Number as Document_Number,File_Type_Number as File_Number,Subject,ApplicationStatus
				from V0080_File_App_Admin_Side
				     Where Cmp_ID = @Cmp_ID and ApplicationStatus='Hold'


	End
	Else if @ChartType = 'Forward'
	Begin
				...
				from V0080_File_App_Admin_Side
		        Where Cmp_ID = @Cmp_ID
				and ApplicationStatus='Forward'


	End
	Else if @ChartType = 'SentbackReview'
	Begin
	          ...
	          select ROW_NUMBER() OVER(ORDER BY File_app_id ASC) AS RowID,
			   FORMAT(app_date, 'dd-MM-yyyy') as Application_Date
				...
				,Alpha_Emp_Code,Emp_Full_Name,File_Number as Document_Number,File_Type_Number as File_Number,Subject,ApplicationStatus
				from V0080_File_App_Admin_Side
		        Where Cmp_ID = @Cmp_ID and ApplicationStatus='Sentback to Review'
