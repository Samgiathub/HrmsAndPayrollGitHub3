...
CREATE PROCEDURE [dbo].[Mobile_HRMS_SCHEME_DETAILS_ESS_GET_test]  
  @Cmp_ID  Numeric  
 ,@Branch_ID  Numeric   = 0 
 ,@Cat_ID  Numeric  = 0
 ,@Grd_ID  Numeric  = 0 
 ...
 ,@Travel_Application_ID NUMERIC(18,0) = 0
...
AS  
 ...
 declare @From_Date Datetime  = @date1
 declare @To_Date  Datetime   = @date1

 --------------------------------------------------------------------SCHEME_ID---------------------------------------------------------------------------
   DECLARE @SCHM_ID numeric

   	SELECT @SCHM_ID = Scheme_Id FROM T0050_Scheme_Detail where 
	Scheme_Id = (SELECT Top 1 T.Scheme_Id from T0095_EMP_SCHEME T
	Inner Join T0050_Scheme_Detail T1 ON T.Scheme_ID = T1.Scheme_Id where Emp_ID = @Emp_ID And Type = 'Travel'
	AND Effective_Date = (SELECT max(Effective_Date) 
	from T0095_EMP_SCHEME where Emp_ID = @Emp_ID And Type = 'Travel' AND Effective_Date <= getdate() ) 
	)
	GROUP BY Scheme_Id
...
--------------------------------------------------------------------CLAIM SCHEME_ID---------------------------------------------------------------------------
   DECLARE @Claim_SCHM_ID numeric

 --   SELECT distinct @Claim_SCHM_ID = Scheme_Id FROM T0050_Scheme_Detail where 
	--Scheme_Id = (SELECT Top 1 T.Scheme_Id from T0095_EMP_SCHEME T
	--Inner Join T0050_Scheme_Detail T1 ON T.Scheme_ID = T1.Scheme_Id where Emp_ID = @Emp_ID And Type = 'Claim'
	--AND Effective_Date = (SELECT max(Effective_Date) 
	--from T0095_EMP_SCHEME where Emp_ID = @Emp_ID And Type = 'Claim' AND Effective_Date <= getdate() ) 
	--AND (SELECT distinct(Claim_ID) from V0100_Claim_Application_New_Detail
	--where Claim_App_ID= @Claim_App_ID) IN (select data from dbo.split(leave,'#')))
	--GROUP BY Scheme_Id
       
	Select @Claim_SCHM_ID = Scheme_Id FROM T0050_Scheme_Detail where Scheme_Id = 
	(SELECT DISTINCT T.Scheme_Id from T0095_EMP_SCHEME T Inner Join T0050_Scheme_Detail T1 ON T.Scheme_ID = T1.Scheme_Id 
	where Emp_ID = @Emp_ID And Type='Claim' AND ((SELECT Claim_ID from V0100_Claim_Application_New where Claim_App_ID = @Claim_App_ID) 
	IN (select data from dbo.split(T1.leave,'#')) or (T1.Leave='0')) AND Effective_Date = (SELECT max(Effective_Date) 
	from T0095_EMP_SCHEME where Emp_ID = @Emp_ID AND Effective_Date <= getdate() And Type='Claim' )) GROUP BY Scheme_Id	

----------------------------------------------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------LEAVE SCHEME_ID---------------------------------------------------------------------------

    DECLARE @Leave_Scheme_ID numeric

	SELECT @Leave_Scheme_ID = Scheme_Id FROM T0050_Scheme_Detail where 
	Scheme_Id = (SELECT Top 1 T.Scheme_Id from T0095_EMP_SCHEME T
	Inner Join T0050_Scheme_Detail T1 ON T.Scheme_ID = T1.Scheme_Id where Emp_ID = @Emp_ID And Type = 'Leave'
	AND Effective_Date = (SELECT max(Effective_Date) 
	from T0095_EMP_SCHEME where Emp_ID = @Emp_ID And Type = 'Leave' AND Effective_Date <= getdate() ) 
	AND (SELECT  distinct(Leave_ID) from T0110_LEAVE_APPLICATION_DETAIL
	where Leave_Application_ID= @Leave_Application_ID) IN (select data from dbo.split(leave,'#')))
	GROUP BY Scheme_Id
       
----------------------------------------------------------------------------------------------------------------------------------------------------------

---------------------------------------------------------------Details About Manager Level------------------------------------------------------------------
...
OPEN Emp_Scheme  
   ...
   while @@fetch_status = 0  
    ...
    if exists(select 1 from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and ( Is_RM = 1 or  Is_PRM=1 )  and Scheme_Id=@Scheme_Id1 and Rpt_Level=1)  
      begin  
		...
		and Rpt_Level=1 and Is_PRM=1 --(Is_PRM=1 or Dyn_Hier_Id >0)  
       ...
       set @rm_name=''   
        ...
        Else  
         Begin  
		  ...
		  -- Code Commented by Prapti on 12062022------------------------------------------------------------------
		  -- Code Added by Prapti on 12062022------------------------------------------------------------------
		   SET @temp1 = (  
                    SELECT DISTINCT ((convert(nvarchar,Alpha_Emp_Code)) + '-' + (convert(nvarchar,EMP_FULL_NAME))) + ', '   
                    FROM T0080_EMP_MASTER E WITH (NOLOCK) INNER JOIN   
                      T0090_EMP_REPORTING_DETAIL ERD WITH (NOLOCK) INNER JOIN   
     ...
     -----------------------------------------------------------------------                     
      if exists(select 1 from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and (Is_BM=1 or Is_HOD=1 or Is_PRM=1  )and Scheme_Id=@Scheme_Id1 and Rpt_Level=3)  
       begin         
		...
		declare @Dyn_Hier_Id3 as tinyint=0
		...
		--and (Is_HOD=1 or Dyn_Hier_Id > 0) and Rpt_Level=3  
		and (Is_HOD=1 ) and Rpt_Level=3  
        ...
        End    
              Else  
			Begin  
			   -- Code Commented by Prapti on 06122022-------------------------------------------------------------------------
      --          set @temp=(SELECT  ((convert(nvarchar,Alpha_Emp_Code)) + '-' + (convert(nvarchar,EMP_FULL_NAME))) + ', '   
					 --FROM T0080_EMP_MASTER E WITH (NOLOCK)  
					 --INNER JOIN T0095_Department_Manager ERD WITH (NOLOCK) ON E.EMP_ID= ERD.Emp_id inner join  
						...
						--select @branch_Id1
				-- Code Commented by Prapti on 06122022-------------------------------------------------------------------------
				-- Code Added by Prapti on 06122022-------------------------------------------------------------------------
			set @temp=(SELECT distinct ((convert(nvarchar,Alpha_Emp_Code)) + '-' + (convert(nvarchar,EMP_FULL_NAME))) + ', '   
				 FROM T0080_EMP_MASTER E WITH (NOLOCK)  
				 INNER JOIN T0095_MANAGERS ERD WITH (NOLOCK) ON E.EMP_ID= ERD.Emp_id inner join  
				 (select max(effective_date) as effective_date,Branch_ID from T0095_MANAGERS IES WITH (NOLOCK)  
	 ...
	 declare @Dyn_Hier_Id4 as int=0 
	 if @Emp_ID2 = 0
	 Begin
		select @Dyn_Hier_Id4 = 0--Dyn_Hier_Id from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and Scheme_Id=@Scheme_Id1 and Rpt_Level=4  
	 END
     ...
     select @Emp_ID2=App_Emp_ID,@leave1=Leave from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and Scheme_Id=@Scheme_Id1 and Rpt_Level=5  
	  ...
	  declare @Dyn_Hier_Id5 as int =0
	 if @Emp_ID2 = 0
	 Begin
		select @Dyn_Hier_Id5 = 0 from T0050_Scheme_Detail WITH (NOLOCK) where  cmp_id=@cmp_id and Scheme_Id=@Scheme_Id1 and Rpt_Level=5  
	 END
 ...
 deallocate Emp_Scheme_Cursor  
   ...
   Begin
	 ...
	 IF @Report_Type <> ''  
	  BEGIN  
		  select Emp_ID1,Rpt_Mgr_1,Rpt_Mgr_2,Rpt_Mgr_3,Rpt_Mgr_4,Rpt_Mgr_5,Rpt_Mgr_6,Rpt_Mgr_7,Rpt_Mgr_8,Max_Level from #EMPSCHEME where Scheme_Id = Isnull(@SCHM_ID,0)  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END  
	 ELSE  
	  BEGIN  
			SELECT * FROM #EMPSCHEME where Scheme_Id = @SCHM_ID  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END   
	...
	V0100_TRAVEL_APPLICATION TA Inner JOIN T0080_EMP_MASTER EM ON TA.Emp_ID = EM.Emp_ID Where Travel_Application_ID = @Travel_Application_ID
   ...
   Begin 
	 ...
	 IF @Report_Type <> ''  
	  BEGIN  
			select Emp_ID1,Rpt_Mgr_1,Rpt_Mgr_2,Rpt_Mgr_3,Rpt_Mgr_4,Rpt_Mgr_5,Rpt_Mgr_6,Rpt_Mgr_7,Rpt_Mgr_8,Max_Level from #EMPSCHEME where Scheme_Id = Isnull(@Claim_SCHM_ID,0)  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END  
	 ELSE  
	  BEGIN  
			SELECT * FROM #EMPSCHEME where Scheme_Id = @Claim_SCHM_ID  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END   
		
				-----------------------------------------   Claim  -----------------------------------------------
	Select Convert(varchar(10), Claim_App_Date,103) as Application_Date, Claim_App_Status, 0 As Rpt_Level,
	Convert(varchar(10), TA.Claim_App_Date,103) as Application_Date, (EM.Alpha_Emp_Code + '-' + EM.Emp_Full_Name) as name From
	V0100_Claim_Application TA Inner JOIN T0080_EMP_MASTER EM ON TA.Emp_ID = EM.Emp_ID Where Claim_App_ID = @Claim_App_ID
	Union Select Convert(varchar(10), Approval_Date,103) as Approval_Date, Claim_apr_Status, Rpt_Level, Convert(varchar(10), TLA.System_Date,103) 
	as System_Date ,(EM.Alpha_Emp_Code + '-' + EM.Emp_Full_Name) as name From T0115_CLAIM_LEVEL_APPROVAL TLA Inner JOIN T0080_EMP_MASTER EM
	ON TLA.S_Emp_ID = EM.Emp_ID Where Claim_App_ID = @Claim_App_ID
   ...
   Begin 
	 ...
	 IF @Report_Type <> ''  
	  BEGIN  
			select Emp_ID1,Rpt_Mgr_1,Rpt_Mgr_2,Rpt_Mgr_3,Rpt_Mgr_4,Rpt_Mgr_5,Rpt_Mgr_6,Rpt_Mgr_7,Rpt_Mgr_8,Max_Level
			from #EMPSCHEME where Scheme_Id = Isnull(@Leave_Scheme_ID,0)  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END  
	 ELSE  
	  BEGIN  
			SELECT * FROM #EMPSCHEME where Scheme_Id = @Leave_Scheme_ID  order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
	  END   
		
	  Select Convert(varchar(10), Application_Date,103) as Application_Date, Application_Status, 0 As Rpt_Level,
	  Convert(varchar(10), TA.Application_Date,103) as Application_Date, (EM.Alpha_Emp_Code + '-' + EM.Emp_Full_Name) as name
	  From V0110_LEAVE_APPLICATION_DETAIL TA Inner JOIN T0080_EMP_MASTER EM ON TA.Emp_ID = EM.Emp_ID 
	  Where Leave_Application_ID = @Leave_Application_ID
	  Union Select Convert(varchar(10), Approval_Date,103) as Approval_Date, Approval_Status, Rpt_Level, 
	  Convert(varchar(10), TLA.System_Date,103)as System_Date ,(EM.Alpha_Emp_Code + '-' + EM.Emp_Full_Name) as name 
	  From T0115_Leave_Level_Approval TLA Inner JOIN T0080_EMP_MASTER EM
	  ON TLA.S_Emp_ID = EM.Emp_ID Where Leave_Application_ID = @Leave_Application_ID 
