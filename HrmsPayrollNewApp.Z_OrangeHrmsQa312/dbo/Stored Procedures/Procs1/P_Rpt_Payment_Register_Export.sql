...
---------------------------------------------
CREATE PROCEDURE [dbo].[P_Rpt_Payment_Register_Export]  
	 @CMP_ID		NUMERIC  
	,@FROM_DATE		DATETIME
	,@TO_DATE 		DATETIME
	,@BRANCH_ID		NUMERIC	
...
SET ARITHABORT ON
	...
	SET @VAL = @VAL + ' ALTER TABLE  #PAYMENTMAST ADD BRANCH_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD DEPT_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD DESIG_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD TYPE_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD CATE_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD SUBBRANCH_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD VERTICAL_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD SUBVERTICAL_CODE VARCHAR(30) DEFAULT '''';
						ALTER TABLE  #PAYMENTMAST ADD SEGMENT_CODE VARCHAR(30) DEFAULT '''';'
	EXEC (@VAL)					
	...
	SELECT		E.CMP_ID,E.EMP_ID,E.EMP_CODE,E.ALPHA_EMP_CODE,ISNULL(E.EMPNAME_ALIAS_SALARY,E.EMP_FULL_NAME),
				DM.DEPT_NAME AS DEPT_NAME ,BM.BRANCH_ID,BM.BRANCH_NAME AS BRANCH_NAME,
				DNM.DESIG_NAME AS DESIG_NAME,GA.GRD_NAME,TM.TYPE_NAME,CT.CAT_NAME AS CATEGORY,VT.VERTICAL_NAME,ST.SUBVERTICAL_NAME,SB.SUBBRANCH_NAME,
				BSG.SEGMENT_NAME,CC.CENTER_CODE,BM.BRANCH_CODE,DM.DEPT_CODE,DNM.DESIG_CODE,TM.TYPE_CODE,CT.CATE_CODE,SB.SUBBRANCH_CODE,
				VT.VERTICAL_CODE,ST.SUBVERTICAL_CODE,SEGMENT_CODE
	...
	SET @VAL = @VAL + ' ALTER TABLE  #PAYMENTMAST ADD LEGAL_WORKING_HOURS VARCHAR(10) DEFAULT ''00:00'';
						...
						ALTER TABLE  #PAYMENTMAST ADD BASIC NUMERIC(18,2) DEFAULT 0;											
	...
	-------------------------FOR ALLOWANCES---------------------------------
		...
		FROM	T0210_MONTHLY_AD_DETAIL T WITH (NOLOCK) INNER JOIN 
				T0050_AD_MASTER A WITH (NOLOCK) ON T.AD_ID = A.AD_ID AND T.CMP_ID = A.CMP_ID INNER JOIN
				#EMP_CONS EC ON EC.EMP_ID = T.Emp_ID
		WHERE	T.CMP_ID = @CMP_ID AND T.FOR_DATE >= @FROM_DATE AND T.TO_DATE <= @TO_DATE
				...
				AND ISNULL(T.M_AD_NOT_EFFECT_SALARY,0) = 1 
				--and ISNULL(A.AD_PART_OF_CTC,0) =1
		...
		EXEC (@VAL_UPDATE);
			...
			SET @VAL = ''
			SET @VAL = @VAL + 'ALTER TABLE  #PAYMENTMAST ADD GROSS NUMERIC(18,2) DEFAULT 0;'
	...
	--SET		STATUS = 'P' 
	--FROM	#ATT_MUSTER A 
	--WHERE	EXISTS(SELECT IO_TRAN_ID 
	--			   FROM	  T0150_EMP_INOUT_RECORD 
	--			   WHERE  EMP_ID = A.EMP_ID AND NOT IN_TIME IS NULL AND FOR_DATE = A.FOR_DATE)
	
	
	
	--UPDATE	A SET LEAVE_COUNT = ISNULL(LEAVE_USED,0) + ISNULL(COMPOFF_USED,0)
	--FROM	#ATT_MUSTER A INNER JOIN 
	--		T0140_LEAVE_TRANSACTION LT ON LT.EMP_ID = A.EMP_ID  AND LT.FOR_DATE = A.FOR_DATE 
	...
	--FROM	#ATT_MUSTER A 
	--WHERE	EXISTS( SELECT 1 FROM #EMP_HW_CONS HW 
	--				WHERE	HW.Emp_ID=A.EMP_ID AND 
	--						CHARINDEX(CAST(A.FOR_DATE AS VARCHAR(11)), HW.WeekOffDate + ';' + HW.CancelWeekOff) > 0 ) 
	
	
	--UPDATE	A SET DURATION=DBO.F_RETURN_HOURS(DATEDIFF(S,IN_TIME,OUT_TIME))
	--FROM	#ATT_MUSTER A INNER JOIN 
	--		T0150_EMP_INOUT_RECORD EIO ON A.EMP_ID = EIO.EMP_ID AND A.FOR_DATE = EIO.FOR_DATE
	...
	--FROM	#ATT_MUSTER A INNER JOIN
	--(
	--	SELECT  MIN(IN_TIME) AS IN_TIME,MAX(OUT_TIME) as OUT_TIME,A.EMP_ID,A.FOR_DATE 
	--	FROM	T0150_EMP_INOUT_RECORD EIO INNER JOIN 
	--			#ATT_MUSTER A ON EIO.EMP_ID = A.EMP_ID AND EIO.FOR_DATE = A.FOR_DATE
	...
	--SELECT * FROM #ATT_MUSTER		
		...
		UPDATE	PT
		SET		PT.NIGHT_SHIFT_HOUR = Q.NIGHT_SHIFT_HOUR
		FROM	#PAYMENTMAST PT INNER JOIN
				(
					SELECT  EMP_ID,
							DBO.F_RETURN_HOURS(ISNULL(SUM(DURATION_IN_SEC),0) - ISNULL(SUM(OT_SEC),0)) as NIGHT_SHIFT_HOUR												
					FROM	#DATA 
					WHERE	FOR_DATE BETWEEN @FROM_DATE AND @TO_DATE AND
							(CONVERT(VARCHAR(8),SHIFT_START_TIME,108) > CONVERT(VARCHAR(8),SHIFT_END_TIME,108))	
					GROUP BY EMP_ID													
			)Q ON Q.EMP_ID = PT.EMP_ID
		...
		FROM	#PAYMENTMAST PT INNER JOIN
				(
					SELECT  EMP_ID,DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(SM.F_Duration)),0)) AS RESERVERD_HOLIDAY							
					FROM	#EMP_HOLIDAY EH 
							CROSS APPLY (SELECT F_Duration 
									     FROM	T0040_SHIFT_MASTER SM WITH (NOLOCK)
										 WHERE	SM.SHIFT_ID=dbo.fn_get_Shift_From_Monthly_Rotation(@Cmp_ID,EH.Emp_ID,EH.For_Date)
										 ) SM
		...
		FROM	T0140_LEAVE_TRANSACTION LT WITH (NOLOCK) INNER JOIN
				#EMP_CONS PT ON PT.EMP_ID = LT.EMP_ID INNER JOIN
				T0040_LEAVE_MASTER LM WITH (NOLOCK) ON LM.LEAVE_ID = LT.LEAVE_ID
		WHERE   LT.FOR_DATE BETWEEN @FROM_DATE AND @TO_DATE AND 
				LM.LEAVE_TYPE <> 'COMPANY PURPOSE' AND LT.LEAVE_USED > 0
				AND (LM.LEAVE_CODE Like 'SL' OR UPPER(LM.Leave_Name) Like '%SICK%')
	
		UPDATE	PT
		SET		PT.SICKNESS = Q.LEAVE_HOURS				
		FROM	#PAYMENTMAST PT INNER JOIN
				(
					SELECT  DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(SM.F_Duration) * L.LEAVE_USED),0)) AS LEAVE_HOURS,L.EMP_ID
					FROM	#LEAVE L 
							CROSS APPLY (SELECT F_Duration 
									     FROM	T0040_SHIFT_MASTER SM WITH (NOLOCK)
		...
		---------Salary Table Content------------------------
				...
				SET		--LEAVE_LIABILITY = (ISNULL(QRY.SALARY_AMOUNT,0) * 5)/100,			
						GROSS = QRY.GROSS_SALARY,
						[BASIC] = QRY.SALARY_AMOUNT
				...
				UPDATE	PT
				SET		EXTRA_PAYMENT = QRY.OT_AMOUNT							
				FROM	#PAYMENTMAST PT INNER JOIN
						(
							SELECT	ISNULL(SUM(T.M_AD_Amount),0) AS OT_AMOUNT,T.Emp_ID
							FROM	T0210_MONTHLY_AD_DETAIL T WITH (NOLOCK) INNER JOIN 
									T0050_AD_MASTER A WITH (NOLOCK) ON T.AD_ID = A.AD_ID AND T.CMP_ID = A.CMP_ID
							WHERE	T.CMP_ID = @CMP_ID AND T.FOR_DATE >= @FROM_DATE AND T.TO_DATE <= @TO_DATE									
									AND AD_ACTIVE = 1 AND AD_FLAG = 'I' AND T.S_SAL_TRAN_ID IS NULL 
									ANd UPPER(A.AD_CALCULATE_ON) = 'TRANSFER OT'
							GROUP BY T.Emp_ID
						) AS QRY ON PT.EMP_ID = QRY.EMP_ID	
				...
				--Declare @Basic_in_Selcted_Columns tinyint = 0
				--Declare @Gross_in_Selcted_Columns tinyint = 0
				...
				IF @SelectedCols <> 'All'
					BEGIN
						...
						--WHERE	NOT EXISTS (SELECT DISTINCT Label_Name FROM #PAYMENTMAST T 
											--WHERE REPLACE(T1.FieldName, ' ', '_') = REPLACE(T.Label_Name, ' ', '_')) 
	
						
						--IF @Gross_in_Selcted_Columns = 1		
						--	 Begin 		
								
						set @SQL = 'UPDATE PT
									SET CTC = (QRY.CTC) + case when ''[GROSS]'' IN (' + @SELECT_COLS + ') then (Pt.Gross) else 0 end
											  + case when ''[BASIC]'' IN (' + @SELECT_COLS + ') then (Pt.BASIC) else 0 end
									FROM #PAYMENTMAST PT INNER JOIN	
											(
											SELECT		(SUM(ISNULL(T1.M_AD_AMOUNT,0))) CTC,T1.EMP_ID 
											FROM		T0050_AD_MASTER A WITH (NOLOCK) INNER JOIN
														T0210_MONTHLY_AD_DETAIL T1 WITH (NOLOCK) ON T1.AD_ID = A.AD_ID AND T1.CMP_ID = A.CMP_ID
															And month(T1.To_DAte) = month(''' + CONVERT(VARCHAR(20),@TO_DATE,120) + ''')
															and year(T1.To_date) = year(''' + CONVERT(VARCHAR(20),@TO_DATE,120) + ''') 
											WHERE		--ISNULL(A.AD_PART_OF_CTC,0) = 1 AND 
														ISNULL(T1.M_AD_NOT_EFFECT_SALARY,0) = 1 AND
														A.AD_FLAG = ''I'' 	
														AND ''['' + A.AD_SORT_Name + '']'' In (' + @SELECT_COLS + ')
											GROUP BY	T1.EMP_ID
										) AS QRY ON PT.EMP_ID = QRY.EMP_ID'	
						PRINT @SQL
						EXEC(@SQL)
		...
		IF @SUMMARY='0' --------FOR GROUPBY BRANCH---------------------------
			BEGIN 
					--SET @STRING = ' SELECT 0 AS FLAG, ROW_NUMBER() OVER (ORDER BY CM.BRANCH) AS ROW_ID,CM.BRANCH AS BRANCH_NAME'+ @STRING_2 +''+ @STRING_3 +',COUNT(EMP_ID) AS Number_Of_Workers,
					...
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.BRANCH_NAME) AS SR_NO,PT.Branch_CODE,PT.BRANCH_NAME AS BRANCH_NAME'+ @String_2 +''+ @String_3 +',COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOUR,SUM(PT.EXTRA_PAYMENT) AS OVERTIME_PAYMENT
									,SUM(PT.BASIC) AS BASIC									
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(PT.GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + ',
									--SUM(PT.SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(PT.LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY,
									SUM(CTC) AS CTC'--,SUM(CTC_EURO) AS CTC_EURO,SUM(CTC_EURO_HOUR) AS CTC_EURO_HOUR'  

					
					SET @STRING = @STRING + ' INTO ##BRANCH FROM #PAYMENTMAST PT GROUP BY BRANCH_NAME,Branch_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
--					Print @STRING
					EXEC(@STRING)					
					...
					--SET @STRING = 'INSERT INTO ##BRANCH SELECT 0 AS FLAG,(ISNULL(MAX(ROW_ID),0)+1),''TOTAL'''+ @STRING_2 +', SUM(Number_Of_Workers),ROUND((SUM(Number_Of_Workers)/'  + CAST(@AVG_EMP AS VARCHAR(10)) +'),0) ,SUM(BASIC_SALARY) ' + @SUM_OF_ALLOWNACES_EARNING + ',SUM(PRODUCTION_BONUS) AS PRODUCTION_BONUS,SUM(LEAVE_ENCASH_AMOUNT) AS LEAVE_ENCASH_AMOUNT,SUM(UNIFORM_REFUND_AMOUNT) AS UNIFORM_REFUND_AMOUNT,SUM(TOTAL_EARNING) AS TOTAL_EARNING, SUM(BASIC_ARREAR) AS BASIC_ARREAR'
					...
					SET @STRING = ' INSERT INTO ##BRANCH SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOUR,SUM(EXTRA_PAYMENT) AS OVERTIME_PAYMENT
									,SUM(BASIC) AS BASIC									
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + '
									--SUM(SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY
									,SUM(CTC) AS CTC--,SUM(CTC_EURO) AS CTC_EURO,SUM(CTC_EURO_HOUR)	AS CTC_EURO_HOUR
									 FROM #PAYMENTMAST';
--					Print @STRING
					EXEC(@STRING)
					
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 
										 ALTER TABLE  ##BRANCH DROP Column Gross
									END'

						--Print @SQL
						exec (@sql)

						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										
										ALTER TABLE  ##BRANCH DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='1' --------FOR GROUPBY GRADE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.GRADE) AS SR_NO,PT.GRADE AS GRADE'+ @String_2 +''+ @String_3 +',COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOUR,
									SUM(PT.EXTRA_PAYMENT) AS OVERTIME_PAYMENT,
									SUM(PT.BASIC) AS BASIC									
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(PT.GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + ',
									--SUM(PT.SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(PT.LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY
					...
					SET @STRING = ' INSERT INTO ##GRADE SELECT 9999,''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOUR,
									SUM(EXTRA_PAYMENT) AS OVERTIME_PAYMENT,SUM(BASIC) AS BASIC									
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + '
									--SUM(SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY
									,SUM(CTC) AS CTC--,SUM(CTC_EURO) AS CTC_EURO,SUM(CTC_EURO_HOUR) AS CTC_EURO_HOUR 
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##GRADE DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##GRADE DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='2' --------FOR GROUPBY CATEGORY---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.CATEGORY) AS SR_NO,CATE_CODE,PT.CATEGORY AS CATEGORY'+ @String_2 +''+ @String_3 +',COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOUR,
									SUM(PT.EXTRA_PAYMENT) AS OVERTIME_PAYMENT,SUM(PT.BASIC) AS BASIC
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(PT.GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + ',
									--SUM(PT.SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(PT.LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY
									SUM(CTC) AS CTC'--,SUM(CTC_EURO) AS CTC_EURO,SUM(CTC_EURO_HOUR) AS CTC_EURO_HOUR'  
					...
					SET @STRING = @STRING + ' INTO ##CATEGORY FROM #PAYMENTMAST PT GROUP BY CATEGORY,CATE_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
--					PRINT @STRING
					EXEC(@STRING)
					...
					SET @STRING = ' INSERT INTO ##CATEGORY SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##CATEGORY DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##CATEGORY DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='3' --------FOR GROUPBY DEPARTMENT---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.DEPARTMENT) AS SR_NO,DEPT_CODE,PT.DEPARTMENT AS DEPARTMENT'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##DEPARTMENT FROM #PAYMENTMAST PT GROUP BY DEPARTMENT,DEPT_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##DEPARTMENT SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##DEPARTMENT DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##DEPARTMENT DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='4' --------FOR GROUPBY DESIGNATION---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.DESIGNATION) AS SR_NO,DESIG_CODE,PT.DESIGNATION AS DESIGNATION'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##DESIGNATION FROM #PAYMENTMAST PT GROUP BY DESIGNATION,DESIG_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##DESIGNATION SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##DESIGNATION DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##DESIGNATION DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='5' --------FOR GROUPBY TYPENAME---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.TYPENAME) AS SR_NO,TYPE_CODE,PT.TYPENAME AS TYPENAME'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##TYPENAME FROM #PAYMENTMAST PT GROUP BY TYPENAME,TYPE_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##TYPENAME SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##TYPENAME DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##TYPENAME DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='6' --------FOR GROUPBY DIVISION WISE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.DIVISION) AS SR_NO,VERTICAL_CODE,PT.DIVISION AS DIVISION'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##DIVISION FROM #PAYMENTMAST PT GROUP BY DIVISION,VERTICAL_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##DIVISION SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##DIVISION DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##DIVISION DROP Column Basic
									END'
		...
		ELSE IF @SUMMARY='7' --------FOR GROUPBY VERTICAL WISE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.subvertical) AS SR_NO,SUBVERTICAL_CODE,PT.SUBVERTICAL AS subvertical'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##subvertical FROM #PAYMENTMAST PT GROUP BY subvertical,SUBVERTICAL_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
				...
				if @String_2 <> '' and @String_3 <> ''
					...
					SET @STRING = ' INSERT INTO ##subvertical SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##subvertical DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##subvertical DROP Column Basic
									END'
					
						--Print @SQL
						exec (@sql)

					SET @STRING = 'SELECT * FROM ##subvertical ORDER BY SR_NO';
		...
		ELSE IF @SUMMARY='8' --------FOR GROUPBY SUBBRANCH WISE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.SUB_BRANCH) AS SR_NO,SUBBRANCH_CODE,PT.SUB_BRANCH AS SUB_BRANCH'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##SUBBRANCH FROM #PAYMENTMAST PT GROUP BY SUB_BRANCH,SUBBRANCH_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##SUBBRANCH SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##SUBBRANCH DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##SUBBRANCH DROP Column Basic
									END'
					
						--Print @SQL
						exec (@sql)
					
					SET @STRING = 'SELECT * FROM ##SUBBRANCH ORDER BY SR_NO';
		...
		ELSE IF @SUMMARY='9' --------FOR GROUPBY SEGMENT_NAME WISE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.SEGMENT_NAME) AS SR_NO,SEGMENT_CODE,PT.SEGMENT_NAME AS SEGMENT_NAME'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
					...
					SET @STRING = @STRING + ' INTO ##SEGMENTNAME FROM #PAYMENTMAST PT GROUP BY SEGMENT_NAME,SEGMENT_CODE'+ @STRING_2 +''+ @STRING_3 +''--WHERE SAL_CAL_DAY <> 0
					...
					SET @STRING = ' INSERT INTO ##SEGMENTNAME SELECT 9999,'''',''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##SEGMENTNAME DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##SEGMENTNAME DROP Column Basic
									END'
					
						--Print @SQL
						exec (@sql)

					SET @STRING = 'SELECT * FROM ##SEGMENTNAME ORDER BY SR_NO';
		...
		ELSE IF @SUMMARY='10' --------FOR GROUPBY CENTER_CODE WISE---------------------------
			BEGIN 
					SET @STRING = ' SELECT ROW_NUMBER() OVER (ORDER BY PT.CENTER_CODE) AS SR_NO,PT.CENTER_CODE AS CENTER_CODE'+ @String_2 +''+ @String_3 +',
									COUNT(EMP_ID) AS Number_Of_Employee,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.UNPAID_HOLIDAY)),0)),'':'',''.'') AS UNPAID_HOLIDAY_HOURS,
									REplace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(PT.SICKNESS)),0)),'':'',''.'') AS SICKNESS_HOURS,SUM(PT.EXTRA_PAYMENT) AS OVERTIME_PAYMENT,
									SUM(PT.BASIC) AS BASIC
									'+ @SUM_OF_ALLOWNACES_EARNING + ',SUM(PT.GROSS) AS GROSS ' + @SUM_OF_ALLOWNACES_DEDUCT + ',
									--SUM(PT.SERVICE_CHARGES_TO_CONTRACTOR) AS SERVICE_CHARGES_TO_CONTRACTOR
									--SUM(PT.LEAVE_LIABILITY) AS LEAVE_LIABILITY,(COUNT(EMP_ID) * 80) AS WC_POLICY,
					...
					SET @STRING = ' INSERT INTO ##CENTERCODE SELECT 9999,''TOTAL'''+ @STRING_2 +', COUNT(EMP_ID),
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(LEGAL_WORKING_HOURS)),0)),'':'',''.'') AS LEGAL_WORKING_HOURS,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(NIGHT_SHIFT_HOUR)),0)),'':'',''.'') AS NIGHT_SHIFT_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(OVERTIME_HOUR)),0)),'':'',''.'') AS OVERTIME_HOUR,
									Replace(DBO.F_RETURN_HOURS(ISNULL(SUM(DBO.F_RETURN_SEC(RESERVERD_HOLIDAY)),0)),'':'',''.'') AS RESERVERD_HOLIDAY_HOUR,
					...
					SET @SQL = 'IF ''[GROSS]'' Not IN (' + @SELECT_COLS + ')			
									Begin	
										 ...
										 ALTER TABLE  ##CENTERCODE DROP Column Gross
									END'
						...
						SET @SQL = 'IF ''[Basic]'' Not IN (' + @SELECT_COLS + ')			
									Begin 	
										...
										ALTER TABLE  ##CENTERCODE DROP Column Basic
									END'
					
						--Print @SQL
						exec (@sql)

					SET @STRING = 'SELECT * FROM ##CENTERCODE ORDER BY SR_NO';
