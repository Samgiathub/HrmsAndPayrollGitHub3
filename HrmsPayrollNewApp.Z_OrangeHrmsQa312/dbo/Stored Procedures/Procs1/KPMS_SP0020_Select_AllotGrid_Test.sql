CREATE PROCEDURE [dbo].[KPMS_SP0020_Select_AllotGrid_Test]  
(              
@Cmp_ID INT,              
@GoalAlt_ID INT,              
@GS_Id Int = NULL,              
...
DECLARE @lResult varchar(max) =  '',@lPaging VARCHAR(MAX) = '',@lResult3 varchar(MAX)='',@lResult2 varchar(MAX)='',@lResult4 varchar(MAX)='',@lResult5 varchar(MAX) , @Type INT , @levelAssignId INT            
 ...
 BEGIN                
  Declare @PivotColDedVar as NVARCHAR(MAX)                  
    ...
    from (     SELECT STUFF((SELECT '    <th class="g2" width="10%">[' + Replace(CAST(Level_Name AS VARCHAR(100)),' ','_') + ']</th>'                  
      FROM KPMS_T0040_Level_Master    where level_Grp_Id = 2 and Cmp_ID = @Cmp_ID           
      FOR XML PATH(''), TYPE)                  
      .value('.','NVARCHAR(MAX)'),1,2,' ') List_Output                
    )  as T                  
   ...
   END              
   select row_number() OVER (order by Level_Id) as Sr_no,* into #TmpLevelMaster from KPMS_T0040_Level_Master  where level_Grp_Id = 1 and IsActive = 1  and Cmp_ID = @Cmp_ID           
   ...
   END              
   select row_number() OVER (order by Level_Id) as Sr_no,* into #TmpLevelMaster1 from KPMS_T0040_Level_Master  where level_Grp_Id = 2 and IsActive = 1  and Cmp_ID= @Cmp_ID            
 ...
 select @lResult3 = @lResult3 + dbo.[fnc_BindGridHeader](@Cmp_ID)            
  ...
  select @lResult2 = @lResult2 + '<tr attrSectionId="' + convert(varchar,gsg.GSG_GoalSettingSection_Id) + '" attrGoalId="' + convert(varchar,gsg.GSG_Goal_Id) + '" attrDependGoalId="' + convert(varchar,gsg.GSG_Goal_Id) + '"             
  ...
  <td  style="min-width: 150px;">' + sm.Section_Name + '</td>              
  <td  style="min-width: 150px;">'+ gm.Goal_Name +'</td>
  <td  style="min-width: 150px;">'+ ssm.SubGoal_Name +'</td> 
  <td  style="min-width: 150px;">'+case GSG_IsDependency when 0 then '-' else  dbo.Fnc_Depend(GSG_Depend_Goal_Id) END +'</td>
  <td  style="min-width: 150px;">'+case GSG_Depend_Type_Id when 0 then '-' else dbo.Fnc_DependType(GSG_Depend_Type_Id,GSG_Depend_Goal_Id)END +'</td>
  <td  style="min-width: 150px;"><select class="custom-select drpSectionWeightage"><option value="0">Select Type<option value="1">Number</option><option value="2">Percentage</option></select>'              
   +'</td>'+@strg+@strg1+'          
  <td  style="min-width: 150px;" > <input class="form-control form-control-lg" id="Targetid"  placeholder="Input" style="width:80px">        
  </td>          
  </tr>'              
  ...
  where GS_Id= @GS_Id and GSG_GoalSetting_Id = @GS_Id and sm.IsActive=1 and gm.IsActive = 1 and ssm.IsActive = 1 and sm.Cmp_ID = @Cmp_ID--group by Goal_Name,GSG_GoalSettingSection_Id,GSG_Goal_Id,GSG_Sub_Goal_Id              
