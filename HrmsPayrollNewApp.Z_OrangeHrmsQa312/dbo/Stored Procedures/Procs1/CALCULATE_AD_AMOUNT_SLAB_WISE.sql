...
CREATE PROCEDURE [dbo].[CALCULATE_AD_AMOUNT_SLAB_WISE]
	@Cmp_ID					NUMERIC ,
	@EMP_ID					NUMERIC ,	
	@AD_ID					Numeric,
	@For_date				Datetime,
	@CALCULATED_AMOUNT		NUMERIC(18,2)OUTPUT,
	@AMOUNT					NUMERIC(18,2)OUTPUT,
	@Working_Days			Numeric(18,2) = 0, --Added by Gadriwala Muslim 16022015
	@Salary_Cal_Day			Numeric(18,2) = 0 --Added by Gadriwala Muslim 16022015
		...
		If @Calc_Type1 <> 'OT Hours(Working Days)' And @Calc_Type1 <> 'OT Hours(Weekoff/Holiday)' And @Calc_Type1 <> 'Early In(Minutes)'
			Begin
				declare curADM cursor for
					select From_Slab,To_Slab,Amount,Calc_Type,Sal_Calc_Type from T0040_AD_Slab_Setting WITH (NOLOCK) where Cmp_id = @Cmp_ID and AD_Id = @AD_ID
					order by Tran_id	
				open curADM
				...
				If @Calc_Type1 = 'OT Hours(Working Days)'
					Begin
						...
						declare curOT cursor for
							Select For_Date, OT_Sec from #Data where Emp_id = @Emp_ID And OT_Sec >0 
						open curOT
					...
					Begin
						...
						INNER JOIN(
									...
									From #Data D 
										...
										LEFT OUTER JOIN T0140_LEAVE_TRANSACTION LT WITH (NOLOCK) ON LT.EMP_ID = D.EMP_ID AND LT.FOR_DATE = D.FOR_DATE AND LT.Leave_Used = 0.50
