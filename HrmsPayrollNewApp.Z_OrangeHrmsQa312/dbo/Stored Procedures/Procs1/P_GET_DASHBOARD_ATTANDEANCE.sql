...
-- =============================================
CREATE PROCEDURE [dbo].[P_GET_DASHBOARD_ATTANDEANCE]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
		...
		BEGIN
			...
			FROM	T0100_EMP_SHIFT_DETAIL esd WITH (NOLOCK) INNER JOIN  
					(SELECT MAX(For_Date) AS For_Date,ESD.Emp_ID FROM T0100_EMP_SHIFT_DETAIL ESD WITH (NOLOCK) INNER JOIN #Emp_Cons EC on ESD.Emp_ID = EC.Emp_ID
						WHERE Cmp_ID = IsNull(@Cmp_ID,Cmp_ID) AND For_Date <= @For_Date AND IsNull(Shift_Type,0) = 0 GROUP BY ESD.Emp_ID) S ON 
...
( 
	...
	FROM #ATT_MUSTER AM INNER JOIN
	 (
		...
		AND IsNull(DEFAULT_SHORT_NAME,'') NOT IN ('COMP','COPH','COND') --Added by Sumit on 01122016
		AND EA.CMP_ID = @CMP_ID AND LT1.LEAVE_USED>0
	...
	(
		...
		--AND IsNull(EA.DEFAULT_SHORT_NAME,'') = 'COMP' 
		AND IsNull(DEFAULT_SHORT_NAME,'') IN ('COMP','COPH','COND') --Added by Sumit on 01122016
		AND EA.CMP_ID = @CMP_ID AND LT1.COMPOFF_USED>0
	 )LT    --EA.Leave_code='OD' AND 
	...
	Set In_Date =In_time
	From #Att_Muster AM inner join 
	( select min(In_Time) In_Time ,EIR.Emp_Id,For_Date FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
	...
	From #Att_Muster AM inner join 
	( select Max(Out_Time) OUT_Time ,EIR.Emp_Id,For_Date FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
		Where Cmp_ID = @cmp_ID AND For_Date>=@From_Date AND For_Date <=@To_Date
		group by EIR.Emp_ID ,for_date 
	)q on Am.Emp_ID =q.emp_ID  AND am.for_Date = Q.for_Date Left Outer Join
	( select Max(In_Time) Max_In_Time ,EIR.Emp_Id,For_Date FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
		Where Cmp_ID = @cmp_ID AND For_Date>=@From_Date AND For_Date <=@To_Date
		group by EIR.Emp_ID ,for_date 
	...
	From #Att_Muster AM inner join 
	(select reason,Other_Reason,for_date,EIR.emp_ID FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
	...
	From #Att_Muster AM inner join 
	(Select Half_Full_Day,for_date,EIR.emp_ID FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
	...
	FROM #ATT_MUSTER AM INNER JOIN 
	(
		SELECT SUP_COMMENT,FOR_DATE,EIR.EMP_ID FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
	...
	FROM #ATT_MUSTER AM INNER JOIN 
	(
		SELECT IS_CANCEL_LATE_IN,FOR_DATE,EIR.EMP_ID FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
			INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
	...
	(
		SELECT IS_CANCEL_EARLY_OUT,FOR_DATE,EIR.EMP_ID FROM T0150_EMP_INOUT_RECORD EIR WITH (NOLOCK)
				INNER JOIN #Emp_Cons EC on EIR.Emp_ID = EC.Emp_ID
	...
	SET		shift_name=IsNull(SM.shift_name,A.shift_name), 
			...
			[Status] = CONVERT(varchar(5), D1.In_Time, 114),
			[Status_2] = CONVERT(varchar(5), D1.Out_Time, 114),
	...
	FROM  #Att_Muster AM INNER JOIN 
		  dbo.T0150_EMP_INOUT_RECORD EIR ON AM.EMP_ID = EIR.EMP_ID AND AM.FOR_DATE = EIR.FOR_DATE INNER JOIN 
...
----Added by Sid 25062014 Ends
		...
		--Added by Mukti Reporting Manager of employee(start)04092017
			UPDATE	#Att_Muster SET R_Emp_ID = rm.R_Emp_ID
				...
				IF @Report_For = 'Att Reg Report'
					begin
						...
						From	#Att_Muster AM 
								...
								LEFT JOIN T0090_EMP_REPORTING_DETAIL ESD WITH (NOLOCK) ON Am.R_Emp_ID = ESD.Emp_ID
								LEFT JOIN T0080_EMP_MASTER E1 WITH (NOLOCK) on ESD.Emp_ID = E1.Emp_Id
	...
	ELSE IF @Report_For = 'BulkRegularization_Mobile'
		BEGIN
			Select	IsNull(AM.Emp_Id,0) As 'Emp_Id',IsNull(AM.Cmp_ID,0) As 'Cmp_ID',IsNull(AM.branch_ID,0) AS 'branch_ID',IsNull(AM.For_Date,0) AS 'For_Date',
					IsNull(AM.Status,'') As Status,IsNull(AM.Leave_code,'') As Leave_code,IsNull(AM.Leave_Count,0) As Leave_Count,IsNull(AM.OD,'') As OD,IsNull(AM.OD_Count,0) As OD_Count,
					IsNull(AM.WO_HO,'') As WO_HO,IsNull(AM.Status_2,'') As Status_2,IsNull(AM.Row_ID,0) As Row_ID,IsNull(AM.In_Date,0) As In_Date,IsNull(AM.Out_Date,0) As Out_Date,
					IsNull(AM.shift_id,0) As shift_id,IsNull(AM.shift_name,'') As shift_name,IsNull(AM.sh_in_time,'') As sh_in_time,IsNull(AM.sh_out_time,'') As sh_out_time,
					IsNull(AM.holiday,'') As holiday,IsNull(AM.late_limit,'') As late_limit,IsNull(AM.Reason,'') As Reason,IsNull(AM.Half_Full_Day,'') As Half_Full_Day,
	...
	Absent = IsNull((@total - (IsNull(Present,0) + IsNull(WO,0) + IsNull(OD,0) + IsNull(Leave,0) + IsNull(HO,0) )),0)
 	...
 	Select P.Present,P.WO,P.HO,P.OD,P.Absent,P.Leave,P.Total FROM #P_Day P
	Drop Table #P_Day
