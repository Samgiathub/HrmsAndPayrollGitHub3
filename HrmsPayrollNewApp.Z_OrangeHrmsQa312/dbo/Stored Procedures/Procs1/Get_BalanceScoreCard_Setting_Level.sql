...
-- =============================================
CREATE PROCEDURE [dbo].[Get_BalanceScoreCard_Setting_Level]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
								   ...
								   Emp_ID numeric    
										...
										begin 
												...
												If @Rpt_level = 1
													Begin 
														Set @SqlQuery = 	
														'Select LAD.BSC_SettingId, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																 ' From V0090_BalanceScoreCard_Setting LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																Where LAD.BSC_SettingId Not In (Select BSC_SettingId From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK) 
													...
													Begin	 										
														...
														'Select LAD.BSC_SettingId, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	...
																	Where (LAD.BSC_SettingId Not In (Select BSC_SettingId From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK)
																				...
																				And LAD.BSC_SettingId In (Select T0110_BalanceScoreCard_Setting_Approval From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK)
													...
													Begin  
														...
														'Select LAD.BSC_SettingId, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave, '  +   cast(@Rpt_level as VARCHAR(2)) +
																...
																Where (LAD.BSC_SettingId Not In (Select BSC_SettingId From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK) 
																			...
																			And LAD.BSC_SettingId In (Select BSC_SettingId From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK) 
																												Where Rpt_Level = ' + Cast(@Rpt_level_Minus_1 as varchar(2)) + ')
																		   )'    
					...
					Begin	
						  ...
						  insert INTO #Appraisal
									Select distinct	
										...
										0,
										0
									From V0090_BalanceScoreCard_Setting ekpi
										left outer join (
										select RLA.BSC_SettingId As App_ID, 
											   Rpt_Level as Rpt_Level , 
											   RLA.approval_Status
											   From T0110_BalanceScoreCard_Setting_Approval RLA WITH (NOLOCK)
											inner join (Select max(rpt_level) as rpt_level1, BSC_SettingId
															From T0110_BalanceScoreCard_Setting_Approval WITH (NOLOCK)
															Where BSC_SettingId In (Select Leave_App_ID From #tbl_Leave_App)
															group by BSC_SettingId
														) Qry
											on qry.BSC_SettingId = RLA.BSC_SettingId and qry.rpt_level1 = RLA.rpt_level
											
										) As Qry1 
						On  ekpi.BSC_SettingId = Qry1.App_ID	
						Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = ekpi.BSC_SettingId
