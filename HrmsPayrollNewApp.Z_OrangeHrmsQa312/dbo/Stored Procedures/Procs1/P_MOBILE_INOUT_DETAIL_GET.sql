...
CREATE PROCEDURE [dbo].[P_MOBILE_INOUT_DETAIL_GET]  
  @Cmp_ID  numeric     
 ,@Emp_ID  numeric      
 ,@From_Date  datetime        
 ,@To_Date  datetime         
 ...
 ,@SortBy varchar(max) = 'IO_Tran_DetailsID'  
 ,@SortType varchar(max) = 'desc'  
AS  
BEGIN  
SET NOCOUNT ON   
...
SET ARITHABORT ON  
 ...
 BEGIN     
	    ...
	    and  datediff(day,md.IO_Datetime,''' + @lToDate + ''' ) >= 0) t ORDER by ' + @SortBy + ' ' + @SortType  
 ...
 ELSE  
  BEGIN  
	...
	Begin 
			select @Query = @Query + 'select IO_Tran_DetailsID,Emp_ID,IMEI_No,In_Out_Flag,IO_Datetime,Reason,Emp_Full_Name,Alpha_Emp_Code,Location,Approval_Status,ManagerComment from (
			select DISTINCT md.IO_Tran_DetailsID,md.Emp_ID,SUBSTRING(md.IMEI_No,8,15) as IMEI_No,case when md.In_Out_Flag =''I'' THEN ''In'' else ''Out'' END as In_Out_Flag, IO_Datetime,md.Reason,em.Emp_Full_Name,em.Alpha_Emp_Code,--right(md.Location, CHARINDEX('':'', REVERSE(md.Location)) - 1)as Location, 
			case when ISNULL(md.Location,'''')='''' THEN md.Location else right(md.Location, CHARINDEX('':'', REVERSE(md.Location)) - 1) END as Location, md.Approval_Status,md.ManagerComment 
			from T9999_MOBILE_INOUT_DETAIL md WITH (NOLOCK) 
			INNER JOIN T0080_EMP_MASTER em WITH (NOLOCK) on md.Emp_ID=em.Emp_ID 
			where em.Cmp_ID= ' + convert(varchar,@Cmp_ID) + ' 
			and datediff(day,''' + @lFromDate + ''',md.IO_Datetime) >=0 
	...
	BEGIN
			...
			where em.Cmp_ID= ' + convert(varchar,@Cmp_ID) + ' 
			and datediff(day,''' + @lFromDate + ''',md.IO_Datetime) >=0 
			and  datediff(day,md.IO_Datetime,''' + @lToDate + ''' ) >= 0  and EM.EMP_ID = ''' + cast(@Emp_ID as varchar(50)) + ''' 
			) t ORDER by ' + @SortBy + ' ' + @SortType
