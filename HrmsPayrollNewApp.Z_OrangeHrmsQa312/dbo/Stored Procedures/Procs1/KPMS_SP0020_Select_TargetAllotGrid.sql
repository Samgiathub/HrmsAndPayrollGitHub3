CREATE PROCEDURE [dbo].[KPMS_SP0020_Select_TargetAllotGrid]                                                                                                                                                                         
(                            
 @Cmp_ID INT,                           
 @Emp_ID int,                      
 @GoalAlt_ID INT,                            
...
'                       
			...
			FROM                       
			   (    select                        
					...
					,level_name   ---, B.la_LevelValue 
					,case when la_LvlGrpId = 1 and  LA.WeightageType = 2 then cast(B.la_LevelValue as varchar(20))  + ''%'' else  cast(B.la_LevelValue as varchar(20)) end	as la_LevelValue				
					...
					,cast(LA.WeightageType as varchar(20)) as WeightageType  
					...
					,[dbo].[fnc_BindFreqData](GSG_FrequecyId,'+ cast(@Emp_ID as varchar(20)) + ',la_LevelAssignId) as GSG_Frequecy   
					 ...
					 where gat.Emp_ID ='+ cast(@Emp_ID as varchar(20)) + ' and sm.IsActive=1 and gm.IsActive = 1 and ssm.IsActive = 1 and a.Cmp_ID='+ cast(@Cmp_ID as varchar(20)) +' and sm.Cmp_Id ='+ cast(@Cmp_ID as varchar(20)) +'                 
			      ) ps                      
					PIVOT                      
					   ( max (la_LevelValue)    
					   ...
					   SET @Title = ''<h1><td>Section Name</td><td>Goal Name</td><td>SubGoal Name</td><td>Depended_On</td><td>Depended_Type</td>'+replace(replace(replace(replace(@PivotColDedVar,'cast(',''),'as varchar(20))',''),'''',''),'+','')+'<td>Target</td><td>Frequency</td></h1>''            
					 ...
					 select @lResult6 = @lResult6 +  ''<b><p class="d-block font-weight-bold name">  '' + Emp_Full_Name + ''   :  '' + comment + ''   -  '' + ''</b><span class="date text-black-50">'' + date + ''</span></p>'' from kpms_tblComment  as tc inner join KPMS_T0020_Goal_Allotment_Master_Test on Goal_Allot_ID = goalAlt_id inner join T0080_EMP_MASTER as em on em.Emp_ID = tc.Eid where goal_Allot_ID= '+ cast(@GoalAlt_ID as varchar(20)) +' and tc.Cmp_Id ='+ cast(@Cmp_ID as varchar(20)) +'         
						...
						--select @Status = @Status + Status_Name from kpms_Approve_Status

					   select @Result as Result2 , @Title as Result3  ,@lResult4  as Result4 ,@lResult6  as Result6  ---,@Status as SatusResult          
			   '               
			   --select @Sql
			  exec (@Sql)                        
END                    
--+''<td> <input class="form-control form-control-lg" id="AchiveTargetid"  placeholder="Input"> </td>''  
----- , case GSG_Depend_Type_Id when 0 then ''-'' else [dbo].[Fnc_Actual_TargetValue](GSG_Goal_Id,GSG_Sub_Goal_Id,gsg.GSG_GoalSetting_Id,la.Goal_Allotment_Id) END as Actual_TargetValues