...
CREATE PROCEDURE [dbo].[Emp_Details_Form_B_Temp]    
  @Company_Id NUMERIC      
 ,@From_Date  DATETIME    
 ,@To_Date   DATETIME   
 ,@Branch_ID  VarChar     
	...
	IF @Constraint <> ''
		...
		begin
								   ...
								   group by I.Emp_ID,For_Date,M_AD_Flag,I.Cmp_ID,E.Emp_Full_Name
									...
									inner join T0210_MONTHLY_AD_DETAIL MD on I.Emp_id = MD.Emp_ID      
									inner join T0050_AD_MASTER AD on md.AD_ID = ad.ad_id 
									left join  #tmpIntSum TI on ti.Emp_ID =E.Emp_ID
									left join  #HRAAmt HR on hr.Emp_ID =E.Emp_ID
								...
								select Emp_ID,[2]as Pf,[3]as ESIC,Cmp_ID,Advance_Amount,PT_Amount,
                                  ...
                                  for AD_DEF_ID in([5],[6])
									...
									,'' as [Remarks_26] into #finaltbl
									 from  #finalTI FTI
									 left join #finalDedu fdu on fdu.Emp_ID=fti.Emp_ID
									 left join #finalEmployer fe on fe.Emp_ID=fti.Emp_ID
									 left join #otherDedu ot on ot.Emp_ID=fti.Emp_ID
