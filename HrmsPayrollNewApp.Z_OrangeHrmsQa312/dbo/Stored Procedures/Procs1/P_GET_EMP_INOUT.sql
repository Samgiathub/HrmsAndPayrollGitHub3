CREATE PROCEDURE [dbo].[P_GET_EMP_INOUT]    
 @Cmp_ID  NUMERIC(9,0),    
 @From_Date DateTime,  
 @To_Date DateTime,  
...
BEGIN  
 ...
 IF OBJECT_ID('tempdb..#TMP_EMP_0150_INOUT') IS NULL  
  BEGIN  
	...
	SELECT IO_Tran_Id,EIR.Emp_ID,Cmp_ID,For_Date,In_Time
	--,case when isnull(In_Time,'') = '' then In_Date_Time else In_Time end as In_Time  -- Divya Verify with Deepal Bhai resolved the direct present in Inout Record page if the AR is pending commenting the code DT : 23-07-2024 
	,Out_Time
	--,case when Out_Time is null then Out_Date_Time else Out_Time end as Out_Time  -- Divya Verify with Deepal Bhai resolved the direct present in Inout Record page if the AR is pending commenting the code DT : 23-07-2024
	,Duration,Reason,Ip_Address,In_Date_Time,Out_Date_Time,Skip_Count,Late_Calc_Not_App
	,Chk_By_Superior,Sup_Comment,Half_Full_day,Is_Cancel_Late_In,
	Is_Cancel_Early_Out,Is_Default_In,Is_Default_Out,Cmp_prp_in_flag,Cmp_prp_out_flag
	,is_Cmp_purpose,App_Date,Apr_Date,System_date,Other_Reason,ManualEntryFlag,StatusFlag,In_Admin_Time,Out_Admin_Time 
        ...
        IF @First_In_Last_Out_For_InOut_Calculation = 1  
         Begin   
          ...
          SELECT Emp_Id,Shift_St_Datetime,datediff(SECOND,min (In_Time), max(OUT_Time)) as Duration_In_sec,Emp_OT,Emp_OT_min_Limit,  
            ...
            Max(OUT_Time) as OUT_Time,Shift_ID --,0 --,WorkingSecShift -- Commented by Niraj(16062022)
 ...
 ELSE  
  BEGIN   
   --declare @Max_OT_Limit int = 0 
   --declare @Min_OT_Limit int = 0 
   ...
   WHILE @@FETCH_STATUS = 0  
    BEGIN  
	 ...
	 BEGIN
		...
		SELECT distinct EMP_ID,@cBrh,isnull(@First_In_Last_Out_For_InOut_Calculation,0),isnull(@Chk_otLimit_before_after_Shift_time,0)  
		FROM #EMP_CONS  WHERE BRANCH_ID=@cBrh  
     ...
     ELSE  
      BEGIN  
		...
		SELECT EIR.Emp_ID ,EIR.for_Date,        
		  ...
		  CAST(CONVERT(VARCHAR(16),In_Time,120) AS DATETIME)  
		  ,NULL,0,0,Chk_By_Superior,ISNULL(EIR.is_cmp_purpose ,0)  
		  ,CAST(CONVERT(VARCHAR(16),Out_Time,120) AS DATETIME)  
		  ,dbo.fn_get_Shift_From_Monthly_Rotation(@Cmp_ID,EIR.Emp_ID,EIR.for_Date) As Shift_ID  
		FROM #TMP_EMP_0150_INOUT EIR
		INNER JOIN #Emp_Cons Ec ON EIR.Emp_Id = ec.Emp_ID
		INNER JOIN (  
				SELECT I.Increment_ID,I.Emp_ID,Emp_OT, ISNULL(Emp_OT_min_Limit,'00:00')Emp_OT_min_Limit, ISNULL(Emp_OT_max_Limit,'00:00') Emp_OT_max_Limit FROM dbo.T0095_Increment  I  WITH (NOLOCK)
		) IQ on EIR.Emp_ID = IQ.emp_ID AND IQ.Increment_ID=EC.Increment_ID  
		INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK)  ON EC.EMP_ID=EM.EMP_ID  
		WHERE EM.Cmp_ID= @Cmp_ID AND EIR.for_Date >= @From_Date AND EIR.For_Date <= @To_Date AND ec.Branch_ID = @cBrh   
		GROUP BY EIR.Emp_ID,EIR.For_Date,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit ,In_Time,Out_Time ,Chk_By_Superior ,EIR.is_cmp_purpose ,In_Date_Time,Out_Date_Time  
		ORDER BY EIR.For_Date         
  ...
  SELECT EIR.Emp_ID,EIR.FOR_DATE,SUM(ISNULL(DATEDIFF(s,EIR.in_time,EIR.out_time),0)),0,  
    dbo.F_Return_Sec(IQ.Emp_OT_min_Limit),dbo.F_Return_Sec(IQ.Emp_OT_max_Limit),EIR.In_Time,null,0,0,1
  ...
  FROM (  
 		SELECT EIR.EMP_ID, EIR.FOR_DATE, MIN(EIR.IN_TIME) AS IN_TIME, MAX(EIR.OUT_TIME) AS OUT_TIME,MAX(EIR.is_cmp_purpose) AS is_cmp_purpose  
 ...
 Update D  
 SET  Duration_in_sec = DateDiff(s, In_Time, Out_Time) FROM #Data D
  ...
  IF Isnull(@IsNight,0) = 0  
   BEGIN   
    ...
    SELECT @FROM_DATE = Min(For_date), @To_date = Max(For_date) FROM #Data  
      
    SELECT ROW_ID, DATEADD(D, ROW_ID-1, @FROM_DATE) AS FOR_DATE  
    INTO #SHIFT_DATE  
    FROM (SELECT ROW_NUMBER() OVER(ORDER BY OBJECT_ID) AS ROW_ID FROM sys.tables ) t  
    ...
    SELECT Distinct EMP_ID,FOR_DATE FROM #EMP_CONS,#SHIFT_DATE
 ...
 IF @DIFF_HOUR > 0  
  BEGIN  
   ...
   INTO #EIO  
   FROM T0150_EMP_INOUT_RECORD EIO1 WITH (NOLOCK)  INNER JOIN  #Emp_Cons Ec ON EIO1.Emp_Id = ec.Emp_ID  
   WHERE EIO1.cmp_Id= @Cmp_ID  and EIO1.for_Date >=@From_Date and EIO1.For_Date <=@To_Date   
   ...
   WHERE LVL='D' AND Q.DiffSe <= 36000 AND Q.DiffSe >= @Total_second OPTION(MAXRECURSION 0) 
   ...
   FROM #Data_EIO_Diff EIO  INNER JOIN #Data D ON EIO.Emp_ID=D.Emp_Id AND EIO.For_date=D.For_date  
   WHERE EXISTS (SELECT 1 FROM T0050_SHIFT_DETAIL SD  WITH (NOLOCK) WHERE D.Shift_ID=SD.Shift_ID AND ISNULL(SD.Working_Hrs_End_Time,0) = 1)  
  ...
  END  
	...
	SELECT @GatePassPersonal = ISNULL(SETTING_VALUE,0) 
	FROM T0040_SETTING WITH (NOLOCK)  
	WHERE SETTING_NAME='Personal Gate Pass Duration Minus in Working Hours' AND CMP_ID=@Cmp_ID
	
	IF  @GatePassPersonal=1
	BEGIN 			
		UPDATE D  
		SET  Duration_In_Sec = Duration_In_Sec - dbo.F_Return_Sec(GPA.Hours)   
		FROM #DATA D              
		LEFT OUTER JOIN T0150_EMP_Gate_Pass_INOUT_RECORD GPA ON GPA.EMP_ID = D.EMP_ID AND GPA.FOR_DATE=D.FOR_DATE 
		LEFT OUTER JOIN T0040_Reason_Master RM ON RM.Res_ID = GPA.Reason_ID
