...
CREATE PROCEDURE [dbo].[KPMS_SP0020_Select_AnualGoal_Master]           
(          
@Cmp_ID INT,          
@GS_Id INT,          
...
BEGIN          
 ...
 BEGIN          
  ...
  <td>' + case when IsLock = 0 then '<a href="javascript:;" onclick="EditData(' + CONVERT(VARCHAR,GS_Id) +')"><i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i></a>'else'<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>'end+'     
 ...
 BEGIN          
  ...
  <div class="col-md-3" group="headersection"><label>Weightage Value</label><input type="text" name="Weightage_Value" class="form-control txtSectionWeightage" 
  onkeypress="return isNumberKey(event);" placeholder="80" value="' + convert(varchar,GSS_WeightageValue) + '" />
  </div><div class="col-md-1"><a href="javascript:;" onclick="AddSection(this);"><i class="fas fa-plus-circle float-right"></i></a></div><div class="col-md-0.25"> 
  <a href="javascript:;" onclick="RemoveSection(this);">
  <i class="fas fa-trash float-right"></i></a></div></div></div></div><div class="collapse show"><div class="card-body table-responsive p-0 dvSection">          
  ...
  <th width="5%">Action</th></tr></thead><tbody class="tblParentGoal">' + dbo.[fnc_GetSectionInnerResultForEditData](@Cmp_ID,GSS_SectionId) + '</tbody></table></div></div></div></div>'            
--  <th width="5%">Action</th></tr></thead><tbody class="tblParentGoal">' + dbo.fnc_GetSectionInnerResult(@Cmp_ID,GSS_SectionId,GSG_FrequecyId,GSG_WeightageType_Id) + '</tbody></table></div></div></div></div>'   
 from KPMS_T0100_Goal_Setting as GM          
  ...
  SELECT ISNULL(GS_Id,0) AS b_Id,ISNULL([GS_SheetName],'') as Title,          
...
--29_june_2022
-- select  @lSectionResult =  @lSectionResult + '<div class="col-md-12"><div class="card card-primary card-outline" group="sections"><div class="card-header"><div class="form-group"><div class="row">          
--  <div class="col-md-2" group="headersection"><label>Section Name</label><input type="text" name="Section_1" class="form-control txtSectionName" placeholder="Section A" value="' + Section_Name + '" /></div>         
--  <div class="col-md-3" group="headersection"><label>Weightage Value</label><input type="text" name="Weightage_Value" class="form-control txtSectionWeightage" onkeypress="return isNumberKey(event);" placeholder="80" value="' + convert(varchar,GSS_WeightageValue) + '" /></div><div class="col-md-1"><a href="javascript:;" onclick="AddSection(this);"><i class="fas fa-plus-circle float-right"></i></a></div><div class="col-md-0.25"> <a href="javascript:;" onclick="RemoveSection(this);"><i class="fas fa-trash float-right"></i></a></div></div></div></div><div class="collapse show"><div class="card-body table-responsive p-0 dvSection">          
--  <table class="table table-hover text-nowrap sectiontable"><thead><tr><th width="13%">Goal</th><th width="13%">Sub Goal</th><th width="10%">Frequency</th>         
--  <th width="8%">Weightage Value</th><th width="5%">Dependency</th><th width="10%">Depen. Module</th><th width="10%">Depen. Type</th>         
--  <th width="5%">Action</th></tr></thead><tbody class="tblParentGoal">' + dbo.[fnc_GetSectionInnerResultForEditData](@Cmp_ID,GSS_SectionId) + '</tbody></table></div></div></div></div>'            
----  <th width="5%">Action</th></tr></thead><tbody class="tblParentGoal">' + dbo.fnc_GetSectionInnerResult(@Cmp_ID,GSS_SectionId,GSG_FrequecyId,GSG_WeightageType_Id) + '</tbody></table></div></div></div></div>'   
...
--  select @lSectionResult =  '<div class="col-md-12"><div class="card card-primary card-outline" group="sections"><div class="card-header"><div class="form-group"><div class="row">          
--  <div class="col-md-2" group="headersection"><input type="text" name="Section_1" class="form-control txtSectionName" placeholder="Section A" value="' + Section_Name + '" /></div><div class="col-md-2" group="headersection">          
--  <select class="custom-select drpSectionWeightage"><option value="0"> Select </option>          
--  <option value="1" ' + CASE WHEN GSS_WeightageTypeId = 1 then 'selected="selected"' ELSE '' END + '>Number</option>          
--  <option value="2" ' + CASE WHEN GSS_WeightageTypeId = 2 then 'selected="selected"' ELSE '' END + '>Percentage</option></select></div>          
--  <div class="col-md-1" group="headersection"><input type="text" name="Weightage_Value" class="form-control txtSectionWeightage" onkeypress="return isNumberKey(event);" placeholder="80" value="' + convert(varchar,GSS_WeightageValue) + '" /></div>         
 
--  <div class="col-md-2" group="headersection"><select class="custom-select drpSectionStatus"><option value="0">  Select </option>          
--  <option value="1" ' + CASE WHEN GSS_StatusId = 1 then 'selected="selected"' ELSE '' END + '>Active</option>          
--  <option value="2" ' + CASE WHEN GS.GSS_StatusId = 2 then 'selected="selected"' ELSE '' END + '>Inactive</option></select></div>          
--  <div class="col-md-2" group="headersection"><select id="Section" class="custom-select drpSectionMonth">' + dbo.fnc_BindMonthByYearExcel(@Cmp_ID,GSS_MonthId) + '</select></div><div class="col-md-3">          
