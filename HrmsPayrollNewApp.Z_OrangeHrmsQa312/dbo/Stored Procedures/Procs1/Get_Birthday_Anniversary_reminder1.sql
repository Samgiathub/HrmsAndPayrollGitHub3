...
CREATE PROCEDURE [dbo].[Get_Birthday_Anniversary_reminder1]
   @Cmp_ID NUMERIC(18,0),  
...
BEGIN
	...
	SELECT	I.Cmp_ID,E.Emp_ID, E.Alpha_Emp_Code,(Initial + ' ' + Emp_First_Name + ' ' + Emp_Last_Name) As Emp_Name,
			Date_Of_Birth,I.Branch_ID,BM.Branch_Name,E.Image_Name,Desg.Desig_Name,DM.Dept_Name,GroupJoiningDate,--Date_Of_Join,
			0 AS Total_Completed_Years, IsNull(Chk_Birth,1) As Chk_Birth
			,Emp_Annivarsary_Date
			, I.Dept_ID, I.Vertical_ID, I.SubVertical_ID
	INTO	#EMP_DATA
	FROM	(SELECT Emp_ID, Initial, Emp_First_Name, Emp_Last_Name, Alpha_Emp_Code, Image_Name,GroupJoiningDate,-- Date_Of_Join,
				Emp_Annivarsary_Date,
...
---WHERE	IsNULL(CM.Chk_Birth,1) = 1 
		...
		BEGIN 				
			...
			If @ShowMarraigeAnniversary = 1
				BEGIN
					...
					SELECT	Emp_Name,Emp_Annivarsary_Date AS Date_Of_Birth,'',Branch_ID,Branch_Name,Image_Name,
							...
							Alpha_Emp_Code,Desig_Name,Dept_Name,Emp_ID,'' AS GroupJoiningDate ,0 AS Total_Completed_Years, Cmp_ID
					FROM	#EMP_DATA ED
					WHERE	Cmp_ID=@Display_Cmp_ID AND chk_Birth = 1
			...
			IF @ShowWorkAnni = 1
				BEGIN
					...
					INSERT	INTO #Birthday
					SELECT	Emp_Name,COALESCE(GroupJoiningDate,Date_Of_join)  AS Date_Of_Birth,'',Branch_ID,Branch_Name,Image_Name,		
							ROW_NUMBER() Over (ORDER BY MONTH(COALESCE(GroupJoiningDate,Date_Of_join)),DAY(COALESCE(GroupJoiningDate,Date_Of_join))),6,
							Alpha_Emp_Code,Desig_Name,Dept_Name,Emp_ID,GroupJoiningDate ,DATEDIFF(YEAR,COALESCE(GroupJoiningDate,Date_Of_join) ,GETDATE()) AS Total_Completed_Years,Cmp_ID
					FROM	#EMP_DATA ED
					WHERE	Cmp_ID=@Display_Cmp_ID AND Chk_Birth = 1
							AND MONTH(COALESCE(GroupJoiningDate,Date_Of_join))=MONTH(GETDATE()) AND DAY(COALESCE(GroupJoiningDate,Date_Of_join))=DAY(GETDATE()) 
							AND YEAR(COALESCE(GroupJoiningDate,Date_Of_join) ) > 1900 and Year(GETDATE()) > Year(COALESCE(GroupJoiningDate,Date_Of_join) )  --added by mehul 06102022 year to get proper employee for Work anniversary
	...
	WHILE @@FETCH_STATUS = 0        
		BEGIN					
			...
			SELECT	* FROM
			(
				SELECT Emp_Full_Name,Date_Of_Birth,Month_Name,Branch_ID,Branch_Name,Image_Name,Row_ID,Sorting_No,Alpha_Emp_Code,Desig_Name,Dept_Name,Emp_ID ,GroupJoiningDate,Total_Completed_Years,Cmp_Id
				FROM #UpcomingBDay
				UniON All
				SELECT ('&nbsp&nbsp&nbsp<b style="font-size:10px;color:#351224;"><i>Date : ' + Month_Name + ' </i></b>') AS Emp_Full_Name,'' AS Date_Of_Birth,Month_Name,0 AS Branch_ID,'' AS Branch_Name,'' AS Image_Name,0 AS Row_ID,Sorting_No,'' AS Alhpa_Emp_Code,'' AS Desig_Name,'' AS Dept_Name,0 AS Emp_ID,'' AS GroupJoiningDate,'' AS Total_Completed_Years,cmp_id
			...
			If @ShowMarraigeAnniversary = 1
				BEGIN		
					...
					SELECT	Emp_Name AS Emp_Full_Name,Emp_Annivarsary_Date AS Date_Of_Birth, 
							...
							10 AS Sorting_no,Alpha_Emp_Code,Desig_Name,Dept_Name,Emp_ID,'' AS GroupJoiningDate,0 AS Total_Completed_Years ,Cmp_Id
					...
					(
						...
						UNION	ALL
						SELECT	('&nbsp&nbsp&nbsp<b style="font-size:10px;color:#351224;"><i>Date : ' + Month_Name + ' </i></b>') AS Emp_Full_Name,'' AS Date_Of_Birth,Month_Name,0 AS Branch_ID,'' AS Branch_Name,'' AS Image_Name,0 AS Row_ID,Sorting_No - 0.00005,'' AS Alhpa_Emp_Code,'' AS Desig_Name,'' AS Dept_Name,0 AS Emp_ID,'' AS GroupJoiningDate,'' AS Total_Completed_Years,Cmp_Id
			...
			IF @ShowWorkAnni = 1
				BEGIN
					...
					SELECT	Emp_Name AS Emp_Full_Name,COALESCE(GroupJoiningDate,Date_Of_join)AS Date_Of_Birth,   Date_Of_join,
							(CAST(RIGHT('00' + CONVERT(NVARCHAR(2), DATEPART(DAY, GroupJoiningDate)), 2) AS VARCHAR(3)) + '-' + UPPER(dbo.F_GET_MONTH_NAME(MONTH(GroupJoiningDate)))) AS Month_Name,
							Branch_ID,Branch_Name,Image_Name,ROW_NUMBER() Over (ORDER BY MONTH(COALESCE(GroupJoiningDate,Date_Of_join)),DAY(COALESCE(GroupJoiningDate,Date_Of_join))) AS Row_ID,
							12 AS Sorting_no,Alpha_Emp_Code,Desig_Name,Dept_Name,Emp_ID,GroupJoiningDate,DATEDIFF(YEAR,GroupJoiningDate,GETDATE()) AS Total_Completed_Years,Cmp_Id
					INTO	#UpcomingWorkAnniversary --Mukti 11012016
					FROM	#EMP_DATA ED
					WHERE	Cmp_ID=@Display_Cmp_ID AND Chk_Birth = 1 					 
							AND (DATEADD(yy, YEAR(@FROM_DATE) - YEAR(COALESCE(GroupJoiningDate,Date_Of_join)),COALESCE(GroupJoiningDate,Date_Of_join)) BETWEEN @From_Date AND @To_Date)
					ORDER BY MONTH(COALESCE(GroupJoiningDate,Date_Of_join)),DAY(COALESCE(GroupJoiningDate,Date_Of_join)) 
	...
	DEALLOCATE Cur_Upcoming 
		...
		SELECT	Sorting_No,B.Cmp_Id,B.Emp_Full_Name,B.Month_Name,B.Branch_Name,
				...
				(CASE	WHEN B.GroupJoiningDate = '01/01/1900' THEN 
						'' 
				ELSE 
						CONVERT(VARCHAR(11), B.GroupJoiningDate, 103)
				END) AS GroupJoiningDate, B.Total_Completed_Years,E.Gender
		 FROM	#Birthday B 
