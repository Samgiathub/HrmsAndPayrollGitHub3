...
-- Description:	To get the In and Out time from Employee InOut table.
-- =============================================
CREATE PROCEDURE [dbo].[P_GET_EMP_INOUT_Test_Yogseh_1112022] 	
	@Cmp_ID		NUMERIC(9,0), 	
	@From_Date	DateTime,
	@To_Date	DateTime,
						...
						BEGIN
							...
							IF @Shift_St_Sec > @Shift_En_Sec       
							 ...
							 --IF CONVERT(varchar(5), @Shift_St_Time, 108) > CONVERT(varchar(5), @Shift_End_Time, 108)
								...
								SELECT @cBrh  = Branch_ID from T0095_Increment EI WITH(NOLOCK) where Increment_Effective_Date in (select max(Increment_effective_Date) as Increment_effective_Date 
								from T0095_Increment WITH(NOLOCK) where Increment_Effective_date <= @To_Date  and Cmp_ID = @Cmp_ID and Emp_ID = @curEmp_ID) and Emp_ID = @curEmp_ID
								...
								IF NOT EXISTS(SELECT 1 FROM #EMP_GEN_SETTINGS WITH(NOLOCK) WHERE EMP_ID=@curEmp_ID)
								...
								IF @First_In_Last_Out_For_InOut_Calculation = 1
									Begin	
										...
										--SET @buffer_Shift_Start_Time_Hrs = DATEDIFF(hh,@PREVIOUS_END_TIME,@Shift_St_Datetime)																		
												...
												--		SELECT	1
												--	end
										INSERT INTO #Data (Emp_ID,For_Date,Duration_In_sec,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,Shift_Start_Time,Shift_End_Time,OT_Start_Time,Shift_Change,Chk_By_Superior,IO_Tran_Id,OUT_Time,Shift_ID)        
										...
										SELECT	Emp_Id,Shift_St_Datetime,datediff(SECOND,min (In_Time), max(OUT_Time)) as Duration_In_sec,Emp_OT,Emp_OT_min_Limit,
												...
												Max(OUT_Time) as OUT_Time,Shift_ID
										FROM	(											
													...
													GROUP BY EC.Emp_ID,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,out_Date,
															Chk_By_Sup,In_Date
												) Qry
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					IF NOT EXISTS(SELECT 1 FROM #EMP_GEN_SETTINGS WHERE BRANCH_ID=@cBrh)
						...
						SELECT	EMP_ID,@cBrh,@First_In_Last_Out_For_InOut_Calculation,@Chk_otLimit_before_after_Shift_time
						FROM	#EMP_CONS
					...
					ELSE
						BEGIN
							...
							SELECT	EIR.Emp_ID ,EIR.for_Date,						
									...
									CAST(CONVERT(VARCHAR(16),In_Time,120) AS DATETIME)/*In_Time*/,null,0,0,Chk_By_Superior,ISNULL(EIR.is_cmp_purpose ,0),CAST(CONVERT(VARCHAR(16),Out_Time,120) AS DATETIME)/*Out_Time*/
							...
							WHERE	EM.Cmp_ID= @Cmp_ID AND EIR.for_Date >= @From_Date AND EIR.For_Date <=@To_Date AND ec.Branch_ID = @cBrh  --ec.Emp_ID = @curEmp_ID
	...
	IF @DIFF_HOUR > 0
		...
		IF Isnull(@IsNight,0) = 0
			BEGIN 
				...
				FROM	#Data D
						INNER JOIN #EMP_SHIFT_DETAIL SD  ON D.Emp_Id=SD.EMP_ID AND D.For_date=SD.FOR_DATE
	...
	BEGIN			
			...
			IF  object_id('tempdb..#DATA_IO') IS NOT NULL 
				BEGIN      
					...
					--Update #DATA_IO SET Duration_In_Sec = A.TotSec 
					--From (Select Emp_id,For_date,Isnull(SUM(Duration_In_Sec),0) as TotSec from #DATA_IO Group BY Emp_id,For_date)
					--as A Inner Join  #DATA_IO On #DATA_IO.Emp_id = A.Emp_id And #DATA_IO.For_Date = A.For_date
					
					--Update #DATA_IO SET Duration_In_Sec = B.SumDuration From
					--#DATA_IO AS A Inner Join
					--(select emp_id,for_date,Isnull(SUM(Duration_In_Sec),0)as SumDuration from #DATA_IO
					--group by emp_id,for_date) as B
					--ON A.emp_id = B.emp_id And A.for_Date = B.for_date 
