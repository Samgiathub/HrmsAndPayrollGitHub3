...
-- =============================================
CREATE PROCEDURE [dbo].[Get_EmployeeGoalSetting_Evaluation_Level]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
								 ...
								 if @is_branch_manager = 1
										begin 
												...
												If @Rpt_level = 1
													Begin 
														Set @SqlQuery = 	
														'Select LAD.Emp_GoalSetting_Review_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																 ' From V0095_EmployeeGoalSetting_Evaluation LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																Where LAD.Emp_GoalSetting_Review_Id Not In (Select Emp_GoalSetting_Review_Id From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
													...
													Begin	 										
														...
														'Select LAD.Emp_GoalSetting_Review_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	...
																	Where (LAD.Emp_GoalSetting_Review_Id Not In (Select Emp_GoalSetting_Review_Id From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
																				...
																				And LAD.Emp_GoalSetting_Review_Id In (Select T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK) From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
													...
													Begin  
														...
														'Select LAD.Emp_GoalSetting_Review_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave, '  +   cast(@Rpt_level as VARCHAR(2)) +
																...
																Where (LAD.Emp_GoalSetting_Review_Id Not In (Select Emp_GoalSetting_Review_Id From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
																			...
																			And LAD.Emp_GoalSetting_Review_Id In (Select Emp_GoalSetting_Review_Id From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
																												Where Rpt_Level = ' + Cast(@Rpt_level_Minus_1 as varchar(2)) + ')
																		   )'    
			...
			If @Emp_ID_Cur > 0
					Begin	
						  ...
						  insert INTO #Appraisal
									Select distinct	
										ekpi.Emp_GoalSetting_Review_Id,	
										ekpi.Emp_ID,
										ekpi.Emp_Full_Name,
										...
										0,
										0
									From V0095_EmployeeGoalSetting_Evaluation ekpi
										left outer join (
										select RLA.Emp_GoalSetting_Review_Id As App_ID, 
											   Rpt_Level as Rpt_Level , 
											   RLA.approval_Status
											   From T0110_EmployeeGoalSetting_Evaluation_Approval RLA WITH (NOLOCK)
											inner join (Select max(rpt_level) as rpt_level1, Emp_GoalSetting_Review_Id
															From T0110_EmployeeGoalSetting_Evaluation_Approval WITH (NOLOCK)
															Where Emp_GoalSetting_Review_Id In (Select Leave_App_ID From #tbl_Leave_App)
															group by Emp_GoalSetting_Review_Id
														) Qry
											on qry.Emp_GoalSetting_Review_Id = RLA.Emp_GoalSetting_Review_Id and qry.rpt_level1 = RLA.rpt_level
											
										) As Qry1 
						On  ekpi.Emp_GoalSetting_Review_Id = Qry1.App_ID	
						Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = ekpi.Emp_GoalSetting_Review_Id
					...
					begin
						 ...
						 insert INTO #Appraisal
							...
							ekpi.Emp_GoalSetting_Review_Id,
									...
									On  ekpi.Emp_GoalSetting_Review_Id = Qry1.App_ID
								WHERE
								 ekpi.Cmp_ID = @Cmp_ID  and (ekpi.Review_Status = 4 or ekpi.Review_Status = 1)
