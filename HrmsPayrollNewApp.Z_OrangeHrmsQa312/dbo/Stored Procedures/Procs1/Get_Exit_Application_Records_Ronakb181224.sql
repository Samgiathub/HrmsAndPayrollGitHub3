...
CREATE PROCEDURE [dbo].[Get_Exit_Application_Records_Ronakb181224]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
...
BEGIN
	...
	SELECT @Status = SUBSTRING(@Constrains, CHARINDEX('status = ', @Constrains) + 10, 1)
	...
	(
		...
		--Select @Emp_ID_Cur,@is_res_passed
		--return
														...
														If @Rpt_level = 1
															Begin
																...
																begin
																	...
																	Set @SqlQuery = 	
																	'Select VE.Exit_ID, ' + Cast(@Scheme_ID As Varchar(10)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																...
																begin
																	--select 222
																	Set @SqlQuery = 'select Exit_ID, ' + Cast(@Scheme_ID As Varchar(10)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) + ' as rpt_level from V0200_Emp_ExitApplication where cmp_id='+cast(@Cmp_ID as VARCHAR(20))+' and  S_emp_id IN (select top 1 S_Emp_Id from T0300_Emp_Exit_Approval_Level where s_emp_id='+cast(@Emp_ID as VARCHAR(20))+' group by S_Emp_Id order by S_Emp_Id desc)'
																end
												...
												Else if @is_rpt_manager = 1
													Begin
														...
														If @Rpt_level = 1
															...
															Set @SqlQuery = 'Select VE.Exit_Id, ' + Cast(@Scheme_ID As Varchar(10)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' From V0200_Emp_ExitApplication VE
																		inner join #Emp_Cons Ec on VE.Emp_Id = Ec.Emp_ID
																	Where VE.Exit_Id In (Select Exit_ID From T0300_Emp_Exit_Approval_Level WITH (NOLOCK)
																			Where Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
																		  + ' And VE.S_emp_id IN (select top 1 S_Emp_Id from T0300_Emp_Exit_Approval_Level where s_emp_id='+cast(@Emp_ID as VARCHAR(20))+' group by S_Emp_Id order by S_Emp_Id desc)'		

																--select 666
																--Set @SqlQuery = 'select Exit_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) + ' as rpt_level from V0200_Emp_ExitApplication where cmp_id='+cast(@Cmp_ID as VARCHAR(20))+' 															
																--and  S_emp_id IN (select top 1 S_Emp_Id from T0300_Emp_Exit_Approval_Level where s_emp_id='+cast(@Emp_ID as VARCHAR(20))+' group by S_Emp_Id order by S_Emp_Id desc) '
																--select @SqlQuery
															end													 																
												End
														Else
														...
														BEGIN															
															...
															IF Object_ID('tempdb..#EMP_CONS_RM') IS NOT NULL
																DROP TABLE #EMP_CONS_RM
																	...
																	SET @SqlQuery =	   'Select  Exit_ID, ' + CAST(@Scheme_ID AS VARCHAR(10)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   CAST(@Rpt_level AS VARCHAR(2)) + '
													...
													Begin															
														...
														Else
															...
															begin	
																...
																--Set @SqlQuery = 'select Exit_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) + ' as rpt_level from V0200_Emp_ExitApplication where cmp_id='+cast(@Cmp_ID as VARCHAR(20))+' and  S_emp_id IN (select top 1 S_Emp_Id from T0300_Emp_Exit_Approval_Level where s_emp_id='+cast(@Emp_ID as VARCHAR(20))+' group by S_Emp_Id order by S_Emp_Id desc)'
																Set @SqlQuery = 'Select VE.Exit_Id, ' + Cast(@Scheme_ID As Varchar(10)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' From V0200_Emp_ExitApplication VE inner join #Emp_Cons Ec on VE.Emp_Id = Ec.Emp_ID
																	   Where VE.Exit_Id In (Select Exit_ID From T0300_Emp_Exit_Approval_Level WITH (NOLOCK)
																	   Where Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
																	   + ' And VE.S_emp_id IN (select top 1 S_Emp_Id from T0300_Emp_Exit_Approval_Level where s_emp_id='+cast(@Emp_ID as VARCHAR(20))+' group by S_Emp_Id order by S_Emp_Id desc)'		
																--Select @SqlQuery
				...
				If @Emp_ID_Cur > 0
					...
					--		from (
								Select distinct	--top 1
									...
									,VE.s_emp_id
									--,ROW_NUMBER() OVER (PARTITION BY VE.exit_id ORDER BY TLAP.Scheme_ID desc) AS RowNum -- Add ROW_NUMBER()
								From V0200_Emp_ExitApplication VE
