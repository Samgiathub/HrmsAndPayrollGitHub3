...
-- =============================================
CREATE PROCEDURE [dbo].[P_ATTENDANCE_CLOSING_LEAVE] 
 @CMP_ID  NUMERIC ,  
 @EMP_ID  NUMERIC ,  
 @FOR_DATE DATETIME = null ,  
 @Leave_Application numeric(18,0) = 0, 
 @Leave_Encash_App_ID numeric(18,0) = 0,
 @Leave_ID	Numeric(18,0) = 0, 
 @Return_Table Numeric(18,2)=0  
AS  
SET NOCOUNT ON 
...
SET ARITHABORT ON
	...
	if @Branch_Wise_Leave = 1 
		...
		Leave_Closing,#temp_CompOff.Leave_Code COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_CompOff.Leave_Name COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_CompOff.Leave_ID,case when T0040_LEAVE_MASTER.Apply_Hourly = 1 then 'hour(s)' else 'day(s)'  end as Leave_Type 
		...
		union
		select Leave_opening,Leave_used,
		Leave_Closing,#temp_COPH.Leave_Code COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_COPH.Leave_Name COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_COPH.Leave_ID,case when T0040_LEAVE_MASTER.Apply_Hourly = 1 then 'hour(s)' else 'day(s)'  end as Leave_Type 
		...
		union
		select Leave_opening,Leave_used,
		Leave_Closing,#temp_COND.Leave_Code COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_COND.Leave_Name COLLATE SQL_Latin1_General_CP1_CI_AS,#temp_COND.Leave_ID,case when T0040_LEAVE_MASTER.Apply_Hourly = 1 then 'hour(s)' else 'day(s)'  end as Leave_Type 
		...
		where isnull(T0040_LEAVE_MASTER.Display_leave_balance ,0)=1 and cmp_Id = @cmp_ID
			...
			if @Return_Table = 0
			begin
				select LT.* from #Leave_Detail  LT
				INNER JOIN T0040_LEAVE_MASTER LM WITH (NOLOCK) on LM.Leave_ID =LT.Leave_ID  order BY Leave_Sorting_No asc
			end
		end
	else
		begin
		...
		SELECT DISTINCT
				LT.Leave_Opening, (LT.Leave_Used + ISNULL(Q1.Leave_Used,0)) AS Leave_Used, 
				dbo.f_lower_round((LT.Leave_Closing - ISNULL(Q1.leave_used,0)),LT.cmp_id) AS Leave_Closing,
				LM.Leave_Code, LM.Leave_Name, LT.Leave_ID,case when lm.Apply_Hourly = 1 then 'hour(s)' else 'day(s)'  end as Leave_Type  
		FROM	T0140_LEAVE_TRANSACTION AS LT WITH (NOLOCK)
		INNER JOIN (SELECT	MAX(For_Date) AS FOR_DATE, Leave_ID, Emp_ID
					FROM	T0140_LEAVE_TRANSACTION WITH (NOLOCK)
					WHERE	(Emp_ID = @EMP_ID) AND (For_Date <= @FOR_DATE) AND (Leave_ID IN (
