...
create PROCEDURE [dbo].[P0000_Default_Form_New_back_02_04_22]  
...
--added by mansi 01-04-2022 for ESS
	...
	begin    
		select @Temp_Form_ID1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 7000 and Form_ID < 7499
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url], [Is_Active_For_menu], [Alias])
		values (@Temp_Form_ID1,'File Management',-1,322,1,'Home.aspx',@Loan_Claim_Img,1,'File Management')
	end
	else
	begin
		update T0000_DEFAULT_FORM set under_form_id=-1,[Sort_ID]=322,[Sort_ID_Check]=1,[Form_Image_url]=@Loan_Claim_Img where  Form_name = 'File Management' and  Form_ID > 7000 and Form_ID < 7499
	end
	...
	begin    
		select @Temp_Form_ID1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 7000 and Form_ID < 7499
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url], [Is_Active_For_menu], [Alias])
		values (@Temp_Form_ID1,'File Application',@Form_Id_File_ESS,322,1,'ESS_File_Application.aspx',@Loan_Claim_Img,1,'File Application')
	end
	else
	begin
		update T0000_DEFAULT_FORM set under_form_id=@Form_Id_File_ESS,[Sort_ID]=322,[Sort_ID_Check]=1,[Form_Image_url]=@Loan_Claim_Img where  Form_name = 'File Application' and  Form_ID > 7000 and Form_ID < 7499
	end
...
if not exists (select Form_id from T0000_DEFAULT_FORM WITH (NOLOCK) where  Form_name = 'File Management' And Form_ID > 6000 and Form_ID < 6499)
	...
	begin
		update T0000_DEFAULT_FORM set under_form_id=@Form_Id_FM,form_type=1,[Sort_ID]=252,[Sort_ID_Check]=1,[Form_Image_url]=@Loan_Claim_Img where  Form_name = 'FM Status Master'
	end	
	
	if not exists (select Form_id from T0000_DEFAULT_FORM WITH (NOLOCK) where  Form_name = 'File Application')
	begin
		select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000  and Form_ID < 6499  
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url], [Is_Active_For_menu], [Alias],[Sort_Id_Check])
		values(@Menu_id1, N'File Application', @Form_Id_FM, 254, 1, N'File_Admin_Application.aspx', @Loan_Claim_Img, 1, N'File Application',1)
	end
	else
	begin
		update T0000_DEFAULT_FORM set under_form_id=@Form_Id_FM,form_type=1,[Sort_ID]=252,[Sort_ID_Check]=1,[Form_Image_url]=@Loan_Claim_Img where  Form_name = 'FM Status Master'
	...
	if not exists (select Form_id from T0000_DEFAULT_FORM WITH (NOLOCK) where  Form_name = 'File Admin Approval')
	begin
		select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000  and Form_ID < 6499  
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url], [Is_Active_For_menu], [Alias],[Sort_Id_Check])
		values(@Menu_id1, N'File Admin Approval', @Form_Id_FM, 253, 1, N'File_Admin_Approval.aspx', @Loan_Claim_Img, 1, N'File Admin Approval',1)
	end
	else
	begin
		update T0000_DEFAULT_FORM set under_form_id=@Form_Id_FM,form_type=1,[Sort_ID]=252,[Sort_ID_Check]=1,[Form_Image_url]=@Loan_Claim_Img where  Form_name = 'FM Status Master'
...
--------------------------- Ess Mobile Application Form Id from 10000 to 10200  Add by Prakash Patel 26092014 ------------------------------------------------------------------- 
		...
		if not exists (select Form_id from T0000_DEFAULT_FORM WITH (NOLOCK) where  Form_name = 'Mobile Mood Tracker' AND Form_ID > 9800 AND Form_ID < 9999)
			...
			--As discussion with chintan prajapti added this page for privilege
					...
					if not exists (select Form_id from T0000_DEFAULT_FORM WITH (NOLOCK) where  Form_name = 'Medical Treatment Application' AND Form_ID > 9800 AND Form_ID < 9999)
					BEGIN
						select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 9800 and Form_ID < 9999
						INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID],[Form_Name],[Under_Form_ID],[Sort_ID],[Form_Type],[Form_url],  
						[Form_Image_url],[Is_Active_For_menu],[Alias])values  
						(@Menu_id1,N'Medical Treatment Application',@Temp_Form_ID1, 811, 1, N'Android', NULL,1,N'Medical Treatment Application')    
					END
...
IF NOT EXISTS(SELECT Form_id FROM T0000_DEFAULT_FORM WITH (NOLOCK) WHERE Form_name = 'Employee Retaining' and Page_Flag = 'AP')
	BEGIN
		...
		SELECT @Menu_id1 = ISNULL(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000 and Form_ID < 6500
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID],[Form_Name],[Under_Form_ID],[Sort_ID],[Form_Type],[Form_url],[Form_Image_url],[Is_Active_For_menu],[Alias],Module_name,Page_Flag)
		VALUES(@Menu_id1, N'Employee Retaining', @U_Form_Id,124, 1,'Home.aspx',@Emp_Img, 1,'','','AP')
	END
	else
	Begin
	...
	BEGIN
		...
		values(@Menu_id1, N'Assign Retaining Status', @form_id_Retain,268, 1, N'Employee_Retaining_Assign.aspx',@Emp_Img, 'Payroll',1,'AP', N'Assign Retaining Status')
	END 
	...
	BEGIN
		select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000 and Form_ID < 6500
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url],[Module_name], [Is_Active_For_menu],[page_flag], [Alias])
		values(@Menu_id1, N'Retaining Rate Master', @form_id_Retain,267, 1, N'Retaining_Rate_Master.aspx',@Emp_Img, 'Payroll',1,'AP', N'Retaining Rate Master')
	END 	
	...
	BEGIN
		select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000 and Form_ID < 6500
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url],[Module_name], [Is_Active_For_menu],[page_flag], [Alias])
		values(@Menu_id1, N'Retaining Payment Process', @form_id_Retain,269, 1, N'Retaining_Payment.aspx',@Emp_Img, 'Payroll',1,'AP', N'Retaining Payment Process')
	END 	
	...
	BEGIN
		select @Menu_id1 = isnull(MAX(Form_id),0) + 1 from T0000_DEFAULT_FORM WITH (NOLOCK) where Form_ID > 6000 and Form_ID < 6500
		INSERT INTO [dbo].[T0000_DEFAULT_FORM]([Form_ID], [Form_Name], [Under_Form_ID], [Sort_ID], [Form_Type], [Form_url], [Form_Image_url],[Module_name], [Is_Active_For_menu],[page_flag], [Alias])
		values(@Menu_id1, N'Final Retaining Payment', @form_id_Retain,270, 1, N'Final_Retaining_Payment.aspx',@Emp_Img, 'Payroll',1,'AP', N'Final Retaining Payment')
	END 	
