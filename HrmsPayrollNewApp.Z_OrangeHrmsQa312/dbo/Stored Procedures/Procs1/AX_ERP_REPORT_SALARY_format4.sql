...
CREATE PROCEDURE [dbo].[AX_ERP_REPORT_SALARY_format4]
...
SET ARITHABORT ON
	...
	AD_CALCULATE_ON varchar(30) default ''
)
				...
				begin
				 ---print 123 ---mansi
					...
					SELECT @Doc_DT ,AX_Head.Cmp_Code,'',@vou_DT, '',group1.Center_Code,'','','0','','0',AX_Head.Account,'0',Cmp_Account_No,'','',0,AX_Head.[Transaction Text],'INR','100.00','','','D',
						...
						,AD_CALCULATE_ON--mansi 		--,@CUR_AD_ID--for testing
					FROM
						(
...
ELSE
   ...
   -- select * from @AX--mansi
	...
	DECLARE @AD_CALCULATE_ON AS VARCHAR(30)

	DECLARE CUR_AX CURSOR FOR
		SELECT CMP_ID,CC_ID,AD_ID,AD_FLAG,LOAN_ID,BUSIUNIT ,BANK_ID , VENDER_CODE,AD_CALCULATE_ON FROM @AX WHERE CMP_Id = @Cmp_Id and Sorting_No < 100 --AND COSTCENT = 570101
	OPEN CUR_AX
	FETCH NEXT FROM CUR_AX INTO @CUR_CMP_ID, @CUR_CENTER_ID,@CUR_AD_ID,@CUR_AD_FLAG,@CUR_LOAN_ID,@CUR_BUS_AREA , @Bank_id , @VENDOR_CODE,@AD_CALCULATE_ON
	While @@fetch_Status = 0
		begin
			...
			set @sum_amt = 0
				...
				--print @CUR_AD_ID ---mansi
				--print @CUR_AD_FLAG--mansi
			...
			--	end
				...
				BEGIN
					  print @CUR_DEPT_ID
						SELECT @sum_amt = isnull(SUM(MS.Salary_Amount),0) + isnull(SUM(MS.Arear_Basic ),0) FROM  T0200_MONTHLY_SALARY MS WITH (NOLOCK)
							inner join (SELECT I.EMP_ID,I.DEPT_ID,I.CENTER_ID ,E.Dealer_Code FROM T0095_INCREMENT I WITH (NOLOCK) INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON I.EMP_ID = E.EMP_ID INNER JOIN 
										...
										I.EMP_ID = QRY.EMP_ID	AND I.Increment_Id = QRY.Increment_Id ) AS INC ON INC.EMP_ID = MS.EMP_ID
							inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON ms.emp_id = EM.Emp_ID
							where --INC.Dept_ID = @CUR_DEPT_ID and --commneted by mansi
							INC.Center_ID =@CUR_CENTER_ID
							and MONTH(month_end_date) = MONTH(@To_Date)  and YEAR(month_end_date) = YEAR(@To_Date)  --and isnull(ms.is_FNF,0)  = 0 
							--and EM.Dealer_Code = @CUR_BUS_AREA
							and isnull(Emp_Mark_Of_Identification,'') not in ('MD','DI','TR')
					...
					--		and isnull(Emp_Mark_Of_Identification,'') not in ('MD','DI','TR')
						...
						update @AX set AMOUNT = @sum_amt,voucher_Flag ='D',JOURNAME='JVPD',AD_flag ='I'  where AD_ID = @CUR_AD_ID 
						--and Dept_ID = @CUR_DEPT_ID --commneted by mansi
						and cc_id = @CUR_CENTER_ID
						--and BUSIUNIT =  @CUR_BUS_AREA
				...
				BEGIN 
					...
					SELECT @sum_amt = isnull(SUM(MAD.M_AD_AMOUNT),0) + isnull(SUM(MAD.M_AREAR_AMOUNT),0) FROM T0210_MONTHLY_AD_DETAIL MAD  WITH (NOLOCK)
						...
						--and AM.AD_NOT_EFFECT_SALARY <>1 --commented by mansi 
					...
					Ad_Active = 1 And sal_type=1 and mad.ad_id =@CUR_AD_id --and INC.Dept_ID = @CUR_DEPT_ID --commneted by mansi
					and inc.Center_ID = @CUR_CENTER_ID --and  inc.Dealer_Code =  @CUR_BUS_AREA	
				and 1 = (case when  t0050_ad_master.AD_DEF_ID=2 and isnull(Emp_Mark_Of_Identification,'') in ('MD','DI') then 0 else 1 end )
				--and t0050_ad_master.AD_NOT_EFFECT_SALARY <>1 --commented by mansi 
				
					
				
					set @sum_amt = @sum_amt + @sum_amt_Sett		
					
					update @AX set Amount = @sum_amt,voucher_Flag ='D',JOURNAME='JVPD'  where AD_ID = @CUR_AD_ID --and Dept_ID = @CUR_DEPT_ID --commented by mansi
					and cc_id = @CUR_CENTER_ID
					--and BUSIUNIT =  @CUR_BUS_AREA	
			...
			FETCH NEXT FROM CUR_AX INTO @CUR_CMP_ID, @CUR_CENTER_ID,@CUR_AD_ID,@CUR_AD_FLAG,@CUR_LOAN_ID,@CUR_BUS_AREA , @Bank_id , @VENDOR_CODE,@AD_CALCULATE_ON  --mansi
		end 
	close cur_ax
	Deallocate cur_ax
...
--Where COSTCENT = 520003-- order by Sorting_No
	...
	----commented by mansi start 13-12-21
	...
	--		ORDER BY COSTCENT
	--		--commented by mansi end 13-12-21

	--added by mansi start 13-12-21 
	
	SELECT	 '2' AS [Report Type 2],
	--AD_ID,AD_Flag,
	--(ROW_NUMBER() OVER (PARTITION BY COSTCENT ORDER BY COSTCENT , Sorting_No ASC ) * 10) AS [Report Type 2] ,
				--CASE WHEN (AD_flag is NULL) AND (AD_ID = 1002)
				--	THEN '40' 
				--WHEN (vender_Code is Not NULL) AND ((ACCODE1 is Null) or ACCODE1 = '')
				--	THEN '39'
				-- ELSE '50'
				-- END as [POSTING KEY],
				CASE WHEN (AD_flag ='I') --AND (AD_ID = 1002)
					THEN '40' 
				WHEN (AD_flag ='D')and (AD_CALCULATE_ON<>'Import')--(vender_Code is Not NULL) AND ((ACCODE1 is Null) or ACCODE1 = '')
					THEN '50'
				 	WHEN (AD_flag ='D')and (AD_CALCULATE_ON='Import')
					then '39'
				 else
				 ''
				 END as [POSTING KEY]
				 --,AD_CALCULATE_ON --mansi
				 , ACCODE1 AS [ACCOUNT],
				 '' as [SPECIAL G/L INDICATOR],'' as [TAX CODE],'' as [Calculate Tax],
				  AMOUNT as [AMOUNT IN DOCUMENT CURRENCY],
				  '' as [Tax Amount],'' as [DUE DATE],
				  LEFT(COSTCENT, 4)as [PROFIT CENTER],
				  'SALARY FOR THE MONTH OF '+CAST(DATENAME(month, @To_Date) AS varCHAR(3))+'-'+(cast(Year(@To_Date)as varchar(10))) as [TEXT],
				  TRANSACTION_TEXT as [ASSIGNMENT],'' as [CONSO FLOW],'' as [TRADING PARTNER],
				  '' as [PARTNER PROFIT CENTER],COSTCENT as [COST CENTER],'' as [WBS ELEMENT],
				  '' as [INTERNAL ORDER],'' as [MATERIAL COPA],'' as [SALES ORGANIZATION],'' as [DIST CHANNEL],
				  '' as [DIVISION],'' as [CUSTOMER COPA],'' as [Plant]
	--backcolor , forecolor,	--Default Fields for Highlight (Will Hide in Reports)
	--CASE WHEN (AD_flag is NULL) AND (AD_ID = 1002) 
	--				THEN REPLACE(CONVERT(NVARCHAR, DOCDT, 103),'/','.')
	--			 ELSE ''
	--			 END AS DOCDT,
	...
	--				END as [End Mark]				
			...
			WHERE  COSTCENT is NOT NULL AND Sorting_No < 100 --AND VOUCHER_FLAG = 'D'
					--AND (AMOUNT > 0 OR (Bank_id > 0 AND EXISTS (SELECT 1 FROM @AX T1 Where T1.Bank_id= 0 AND T1.AMOUNT > 0 AND T1.COSTCENT=T.COSTCENT)))
			--ORDER BY COSTCENT--commented by mansi
			ORDER BY Sorting_No

   --added by mansi end 13-12-21 
