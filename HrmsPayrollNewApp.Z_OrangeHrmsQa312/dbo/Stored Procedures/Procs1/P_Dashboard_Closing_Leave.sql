...
-- =============================================
CREATE PROCEDURE [dbo].[P_Dashboard_Closing_Leave]
...
SET ARITHABORT ON
	...
	FROM	#Emp_Leave_Bal T
			INNER JOIN (SELECT MIN(FOR_DATE) AS FOR_DATE,sUM(Leave_Encash_Days) AS Leave_Encash_Days, EMP_ID,Leave_ID
						FROM #Emp_Leave_Bal GROUP BY EMP_ID,Leave_ID) T1 ON T.Emp_ID=T1.Emp_ID AND T.For_Date=T1.FOR_DATE and T.Leave_ID  = T1.Leave_ID			
