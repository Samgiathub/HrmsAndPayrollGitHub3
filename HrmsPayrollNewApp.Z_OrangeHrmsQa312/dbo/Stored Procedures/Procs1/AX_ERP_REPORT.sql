...
CREATE PROCEDURE [dbo].[AX_ERP_REPORT]
			...
			if @CUR_AD_ID = 0
				begin
						...
						INNER JOIN (SELECT I.EMP_ID,I.DEPT_ID,I.CENTER_ID ,E.Dealer_Code FROM T0095_INCREMENT I WITH (NOLOCK) INNER JOIN T0080_EMP_MASTER E WITH (NOLOCK) ON I.EMP_ID = E.EMP_ID INNER JOIN 
									...
									AND CMP_ID = @CMP_ID 
									GROUP BY EMP_ID  ) QRY ON
									I.EMP_ID = QRY.EMP_ID	AND I.Increment_Id = QRY.Increment_Id and E.Emp_Left = 'N' OR (E.Emp_Left = 'Y' AND month(@To_Date) > month(E.Emp_Left_Date) AND year(@To_Date) > year(E.Emp_Left_Date)  	)) AS INC ON INC.EMP_ID = TLT.EMP_ID
						WHERE INC.DEPT_ID = @CUR_DEPT_ID AND INC.CENTER_ID = @CUR_CENTER_ID AND MONTH(FOR_DATE) = MONTH(@TO_DATE) AND YEAR(FOR_DATE) = YEAR(@TO_DATE) 
			...
			--	end
				...
				begin				
					...
					SELECT @sum_amt = isnull(SUM(MAD.M_AD_AMOUNT),0) + isnull(SUM(MAD.M_AREAR_AMOUNT),0) FROM T0210_MONTHLY_AD_DETAIL MAD WITH (NOLOCK)
						...
						inner JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON ms.emp_id = EM.Emp_ID
						where INC.Dept_ID = @CUR_DEPT_ID and INC.Center_ID =@CUR_CENTER_ID
						and MONTH(month_end_date) = MONTH(@To_Date) and YEAR(month_end_date) = YEAR(@To_Date) and AD_ID = @CUR_AD_id  and isnull(ms.is_FNF,0)  = 0 and EM.Dealer_Code = @CUR_BUS_AREA
