...
Create PROCEDURE [dbo].[GET_EMP_PRIVILEGE_BKP_Deepali_18102024]
	@CMP_ID AS numeric,
...
AS
	...
	INTO	#TEMP_data 
	FROM (
	select  row_number()over (ORDER BY CTE.cmp_id,CTE.Privilege_ID,CTE.Form_ID asc)as Trans_Id,
	...
	IF EXISTS(SELECT 1 FROM information_schema.tables where table_name='T9999_CACHE_GET_EMP_PRIVILEGE')
		...
		BEGIN
			...
			CREATE UNIQUE CLUSTERED INDEX CLIX_T9999_CACHE_GET_EMP_PRIVILEGE ON T9999_CACHE_GET_EMP_PRIVILEGE(Privilege_ID, Form_ID,Trans_Id)
	...
	--drop table T9999_CACHE_GET_EMP_PRIVILEGE --Added by Mr.Mehul on 11-11-2022 for timesheet reports not showing in report module
