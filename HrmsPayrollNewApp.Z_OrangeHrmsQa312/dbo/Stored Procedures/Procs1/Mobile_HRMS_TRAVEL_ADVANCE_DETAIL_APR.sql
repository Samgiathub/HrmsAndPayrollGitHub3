CREATE PROCEDURE [dbo].[Mobile_HRMS_TRAVEL_ADVANCE_DETAIL_APR]
			...
			(ROW_NUMBER() OVER(ORDER BY Table3.value('(Travel_Advance_Detail_ID/text())[1]','NUMERIC(18,0)'))) AS Rownum,
			 ...
			 WHILE(@COUNT > 0)
				BEGIN
					 ...
					 --IF NOT EXISTS(SELECT Row_Adv_ID FROM T0115_TRAVEL_APPROVAL_ADVDETAIL_LEVEL WITH (NOLOCK) WHERE Cmp_ID=@Cmp_ID AND 
		 		...
		 		--	  END	
					--ELSE
					-- BEGIN
					-- 	IF Exists(SELECT Row_Adv_ID FROM T0115_TRAVEL_APPROVAL_ADVDETAIL_LEVEL WITH (NOLOCK) WHERE Cmp_ID=@Cmp_ID AND 
					-- 				 Expence_Type=@Expence_Type AND Amount=@Amount AND Travel_App_ID=@Travel_App_ID)
					-- 	 Begin
					-- 	 	UPDATE T0115_TRAVEL_APPROVAL_ADVDETAIL_LEVEL SET
					-- 	 	Tran_Id = @Tran_ID, Cmp_ID=@Cmp_ID, Row_Adv_ID=@Row_Adv_ID, Expence_Type = @Expence_Type, Amount = @Amount,
					--		Adv_Detail_Desc =@Adv_Detail_Desc,Travel_App_ID = @Travel_App_ID,Curr_ID=@Curr_ID,Amount_dollar =@Dlr_Amnt
