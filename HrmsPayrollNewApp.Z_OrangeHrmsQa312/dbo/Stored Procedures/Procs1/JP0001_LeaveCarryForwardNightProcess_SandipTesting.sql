...
-- exec JP0001_LeaveCarryForwardNightProcess_SandipTesting 13,0 ,'2024-07-01' ,'2024-06-01','2024-06-30'
CREATE PROCEDURE [dbo].[JP0001_LeaveCarryForwardNightProcess_SandipTesting]  
	@CmpId int,
	@LeavId int,
	@ForData Datetime,
	@FromData Datetime,
...
DECLARE @Cmp_id as int = @CmpId 
			...
			BEGIN
				...
				select ROW_NUMBER() OVER(order by Leave_Name) RowNo , * into #tmpLeave from (
					...
					WHERE Leave_Type <> 'Paternity Leave' 
					AND (1=(case isnull(leave_Status,0) when 0 then (case when isnull(InActive_Effective_Date,getdate())>=getdate() then 1 else 0 end ) else 1 end )) 
					AND L.Cmp_ID = @Cmp_id and (leave_cf_type not in('None','0')) --and Leave_Id in (77)
					UNION
					SELECT DISTINCT L.Leave_ID ,Leave_Name
					...
					WHERE Leave_Type <> 'Paternity Leave' 
					AND (1=(case isnull(leave_Status,0) when 0 then (case when isnull(InActive_Effective_Date,getdate())>=getdate() then 1 else 0 end ) else 1 end )) 
					AND L.Cmp_ID = @Cmp_id and (leave_cf_type not in('None','0')) and CFTD.Duration = 'Yearly'  and MONTH(@FromData) = Release_Month - 1
				) s
				
				select * from #tmpLeave
				return
