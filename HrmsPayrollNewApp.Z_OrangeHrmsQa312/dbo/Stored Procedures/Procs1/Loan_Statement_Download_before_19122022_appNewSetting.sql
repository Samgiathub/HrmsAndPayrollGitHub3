...
-- =============================================
create PROCEDURE [dbo].[Loan_Statement_Download_before_19122022_appNewSetting]
@cmp_ID numeric(18,0),
@emp_ID numeric(18,0),
...
BEGIN
	...
	SELECT @Sal_St_Date = Sal_st_Date,@manual_salary_period=isnull(Manual_Salary_Period ,0) FROM T0040_GENERAL_SETTING WITH (NOLOCK)
		...
		begin    
			...
			if @manual_salary_period = 0 
				begin
					...
					set @Sal_St_Date =  cast(cast(day(@Sal_St_Date)as varchar(5)) + '-' + cast(datename(mm,dateadd(m,-1,@Loan_Application)) as varchar(10)) + '-' +  cast(year(dateadd(m,-1,@Loan_Application) )as varchar(10)) as smalldatetime)    
					set @Sal_End_Date = dateadd(d,-1,dateadd(m,1,@Sal_St_Date)) 
				end 
			else
				begin
	...
	--set @Installment_Date = @Installment_Start_Date --Added by Hardik 07/02/2017 for AIA -- Deepal 1
				...
				begin
						...
						If UPPER(@Interest_Type) = UPPER('Reducing')
							begin
									...
									if month(@Loan_Application) = MONTH(@Pre_Installment_Date) and YEAR(@Loan_Application) = Year(@Pre_Installment_Date)
									   begin
