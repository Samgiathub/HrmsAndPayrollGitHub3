...
create PROCEDURE [dbo].[P_CUSTOMIZE_REPORTS_ENTRY_backup_08052024]  
AS  
BEGIN  
SET NOCOUNT ON   
...
SET ARITHABORT ON  
  ...
  /*  
   Execute following Query to get Next Report ID  
   SELECT IsNull(Max(ReportID),0) + 1 FROM T0250_CUSTOMIZED_REPORT  
  */  
  ...
  */  
  ...
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Salary',@TypeID=2,@ReportName='Salary Variance Report',@SortID=25,@ReportID=122 --::Jimit - 04-Dec-2018 (Client: G&D)  
  ...
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Salary',@TypeID=2,@ReportName='LWP Deduction Report',@SortID=27,@ReportID=128  -- Added By Jimit 12042019  
  ...
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Others',@TypeID=4,@ReportName='Bond Status',@SortID=12,@ReportID=119  
   ...
   /*Customized Report->Others->AX Mapping Navision*/  
  --Added by ronakk 06092022
  ...
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Attendance',@TypeID=5,@ReportName='Attendance Request Report',@SortID=11,@ReportID=123 -- Added By Nilesh Patel on 14-02-2019  
  
  /*Customized Report->Attendance-> Daily Man-Days Report*/  
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Attendance',@TypeID=5,@ReportName='Daily Man-Days Report',@SortID=11,@ReportID=124  -- Added By Nilesh Patel on 07-03-2019  
  
  /*Customized Report->Attendance-> Daily Man-Days Report*/  
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Attendance',@TypeID=5,@ReportName='Daily Attendance Report',@SortID=11,@ReportID=127  -- Added By Jimit 12042019  
  ...
  --EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Salary',@TypeID=2,@ReportName='Salary Register Wage Form I',@SortID=28,@ReportID=147 
   -- EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Salary',@TypeID=2,@ReportName='Form IV',@SortID=28,@ReportID=148 
  /*added by mansi end*/
  

  ---------- Add by Jignesh Patel 09-Mar-2022 --------------
  ...
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='PF & ESIC',@TypeID=9,@ReportName='PF Statement Customized',@SortID=3,@ReportID=110  --Change by ronakk 29122022 Name of Report
  ...
  -- Ended by Divyaraj Kiri  on 06/02/2024

  --Added By Divyaraj Kiri on 14/06/2023
  /*Customized Report->Employee->Employee Template Details*/  
  EXEC P0250_CUSTOMIZED_REPORT @ReportType ='Employee',@TypeID=0,@ReportName='Employee Template Details',@SortID=32,@ReportID=162
  --Ended By Divyaraj Kiri on 14/06/2023

  /**********************************PRIVILEGE ENTRY*******************************/  
  BEGIN  
   /*Customized Report Types*/  
   DECLARE @MaxTranID INT  
   SELECT @MaxTranID = MAX(Trans_Id) FROM T0050_PRIVILEGE_DETAILS WITH (NOLOCK)  
     
   INSERT INTO T0050_PRIVILEGE_DETAILS   
   SELECT IsNull(T.Trans_Id, Row_ID + @MaxTranID) As Trans_Id, T.Privilege_ID,T.Cmp_Id, T.Form_ID,  
   ...
   FROM (  
     ...
     from (SELECT * FROM (SELECT  Form_ID FROM T0000_DEFAULT_FORM WITH (NOLOCK)  
           WHERE Under_Form_ID=6714) C CROSS JOIN T0020_PRIVILEGE_MASTER P WITH (NOLOCK) ) T  
