CREATE PROCEDURE [dbo].[GET_Applicable_Working_Date_For_CompOff]  
...
BEGIN  
 ...
 --  select CompOff_Days_Limit,h_CompOff_Days_Limit,W_CompOff_Days_Limit FROM T0040_General_Setting WITH (NOLOCK) WHERE   
 --Cmp_ID = @Cmp_ID AND   
 --Branch_ID = @Branch_ID   
 --and For_Date = ( SELECT max(For_Date) FROM dbo.T0040_GENERAL_SETTING WITH (NOLOCK)  
 --  WHERE For_Date <= @For_Date AND Branch_ID = @branch_id AND Cmp_ID = @Cmp_ID)      
 ...
 set @From_WD_Date  = Convert(varchar(50),DATEADD(D,@CompOff_WD_App_Days_Limit * -1,@For_Date))   
 set @From_WO_Date  = Convert(varchar(50),DATEADD(D,@CompOff_WO_App_Days_Limit * -1,@For_Date))   
 set @From_HO_Date  = Convert(varchar(50),DATEADD(D,@CompOff_HO_App_Days_Limit * -1,@For_Date))   
     ...
     BEGIN  
      ...
      IF Cast(replace(@Sanctioned_Hours, ':', '.') AS NUmeric(18,2)) >= @From_Hours AND Cast(replace(@Sanctioned_Hours, ':', '.') AS NUmeric(18,2)) <= @To_Hours  
       BEGIN  
        ...
        --SET  @CompOff_Days = FLOOR(@CompOff_Days)+ case when ROUND(@CompOff_Days,0)>FLOOR(@CompOff_Days) then 0.5 else 0 end     Commented by Mr.Mehul (Rounding-off 0.25) 08082022
