...
CREATE PROCEDURE [dbo].[ESS_EMP_IN_OUT_MUSTER_HOME_GET]
	 @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
AS
	...
	Declare @Increment_Effective_Date	Datetime -- Added by Divyaraj Kiri on 31032023
				...
				BEGIN   
					IF @manual_salary_period = 0 
					   BEGIN
							...
							--select @Increment_Effective_Date,@Sal_End_Date,@To_Date
						END 
					ELSE
						BEGIN
	...
	--Select @IS_CANCEL_HOLIDAY_WO_HO_SAME_DAY, @Tras_Week_OT
...
( 
	...
	SET		shift_name=IsNull(SM.shift_name,A.shift_name), 
			...
			--[Status_2] = CONVERT(varchar(5), D.Out_Time, 114),
			 ...
			 [Status] = CONVERT(varchar(5),isnull(D.In_Time,EIOR.In_Date_Time) , 114), --Added by ronakk 14032023
			 [Status_2] = CONVERT(varchar(5),isnull(D.Out_Time,EIOR.Out_Date_Time), 114), --Added by ronakk 14032023
	...
	FROM	#Att_Muster A 
			...
			LEFT OUTER JOIN #TMP_EMP_0150_INOUT EIOR ON EIOR.Emp_ID=A.Emp_ID AND EIOR.For_Date=A.For_Date --Added by ronakk 14032023
...
IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ExmptTable'))
BEGIN
				Alter table #Att_Muster add ExFlag Char(2)
				
				update A
				set A.ExFlag = T.ExemptFlag
				from #Att_Muster A inner join ExmptTable T on A.Emp_ID = T.Emp_Id and A.For_Date = T.For_Date and A.Cmp_ID = T.Cmp_ID
...
END
	...
	exec rpt_Late_Early_Mark_Deduction_Details @Cmp_ID=120,@From_Date=@From_Date,@To_Date=@To_Date,@Branch_ID=0,@Cat_ID=0,@Grd_ID=0,@Type_ID=0
	,@Dept_ID=0,@Desig_ID=0,@Emp_ID=@Emp_ID,@Constraint=@Constraint,@Format_Type='Late',@Report_Type=0	
