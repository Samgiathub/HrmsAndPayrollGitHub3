...
CREATE PROCEDURE [dbo].[Emp_Details_Form_A]             
   @Company_Id NUMERIC                
  ,@From_Date  DATETIME            
  ,@To_Date   DATETIME            
  ,@Branch_ID  VarChar             
...
--declare @Company_id as int --=  119          
 ...
 IF @Constraint <> ''            
  ...
  --   ) Inc_Qry ON EM.Emp_ID = Inc_Qry.Emp_ID  
  	          select 
			 distinct  Emp_code as [Employee/Workmen/Worker_Code], Emp_First_Name AS Name,Emp_Last_Name as Surname,Gender,Father_name as [Father's/Spouse_Name],
	   ...
	   Select ROW_NUMBER() OVER(ORDER BY [Employee/Workmen/Worker_Code]) AS SrNo , * from #tempFormA
