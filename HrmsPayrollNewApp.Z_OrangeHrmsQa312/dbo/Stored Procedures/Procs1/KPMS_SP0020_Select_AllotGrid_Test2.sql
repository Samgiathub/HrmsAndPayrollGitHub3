CREATE PROCEDURE [dbo].[KPMS_SP0020_Select_AllotGrid_Test2]     
(    
@Cmp_ID INT,    
@GoalAlt_ID INT,    
@GS_Id Int = NULL,    
...
DECLARE @lResult varchar(max) =  '',@lPaging VARCHAR(MAX) = '',@lResult3 varchar(MAX)='',@lResult2 varchar(MAX)='',@lResult4 varchar(MAX)='',@lResult5 varchar(MAX) , @Type INT , @levelAssignId INT  
 ...
 BEGIN      
  Declare @PivotColDedVar as NVARCHAR(MAX)        
   ...
   BEGIN    
    ...
    select @strg = @strg + '<td attrGroupId="' + convert(Varchar,1) + '" attrLevelId="' + convert(varchar,@LevelId) + '"> '+ CONVERT(varchar,la_LevelValue) +'</td>'  from tbl_LevelAssignValues   
    set @count = @count + 1    
   END    
   ...
   while @count1 <= @Row1    
   BEGIN    
    select @LevelName1= Level_Name,@LevelId1 = Level_ID from #TmpLevelMaster1 where Sr_no = @count     
    set @strg1 = @strg1 + '<td attrGroupId="' + convert(Varchar,2) + '" attrLevelId="' + convert(varchar,@LevelId1) + '">    
    <input class="form-control form-control-lg txt' + @LevelName + '" type="text" placeholder="Input" onblur="checkWeightage(this);"></td>'    
    set @count1 = @count1 + 1    
   END    
 ...
 ELSE    
 BEGIN    
  SELECT ISNULL(Goal_Allot_ID,0) AS r_Id,ISNULL([Goal_Allot_ID],'') as Goal_Allot_ID,ISNULL([Dept_ID],'') as Dept_ID, ISNULL([GoalSheet_Name],'') as GoalSheet_Name,    
  ISNULL([Desig_ID],'') as Desig_ID, ISNULL([Emp_ID],'') as Emp_ID,ISNULL([Galt_Statusid],'') AS Galt_Status_Name    
  ,CONVERT(VARCHAR, CONVERT(varchar, Galt_Effect_Date, 103)) as Effect_Date    
