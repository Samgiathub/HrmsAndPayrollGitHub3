...
CREATE PROCEDURE [dbo].[Mobile_HRMS_SP_EMP_Absent_History_RECORD_GET]      
     @Cmp_ID 		numeric
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric
...
IF OBJECT_ID('tempdb..#Att_Muster_with_shift') IS NOT NULL
BEGIN
    SELECT 
        REPLACE(CONVERT(varchar(25), For_Date, 103), ' ', '/') AS For_Date, 
        'AB-' + CONVERT(varchar(25), A_Days) AS Status,
        SUBSTRING(DATENAME(weekday, For_Date), 1, 3) AS wday  
    INTO 
        ##Privious_month_Record 
    FROM 
        #Att_Muster_with_shift
END
...
FROM 
	...
	--Drop Table 
	--IF Exists ##Privious_month_Record
