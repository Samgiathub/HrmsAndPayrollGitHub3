...
CREATE procedure [dbo].[KPMS_prc_SaveGoal_annual_Updated]
@GoalsettingID int,
@rCmpId int,
@rUserId int,
...
begin
	...
	if not exists (select 1 from KPMS_T0100_Goal_Setting where GS_Id = @GoalsettingID and Cmp_Id = @rCmpId)
	begin
	...
	begin
		...
		Update KPMS_T0100_Goal_Setting set   
		GS_SheetName = @rSheetName
		,GS_FromDate = @rFromDate
		,GS_ToDate= @rToDate
		,GS_WeightageTypeId = @rWeightageType
		,GS_WeightageValue = @rWeightageVal
		,GS_StatusId = @rStatus where GS_Id = @GoalsettingID and Cmp_Id = @rCmpId
	end
	...
	if @rFlag = 2
	begin
		update KPMS_T0100_Goal_Setting set IsLock = 1,IsDraft = 0 where GS_Id = @GoalsettingID
	end
	else if @rFlag = 3
	begin
		update KPMS_T0100_Goal_Setting set IsDraft = 1 where GS_Id = @GoalsettingID
	end

	select * from KPMS_T0100_Goal_Setting
	select * from KPMS_T0110_Goal_Setting_Section

	MERGE KPMS_T0110_Goal_Setting_Section AS TARGET
	USING @tbltmp AS SOURCE ON GSS_Goal_Setting_Id = @GoalsettingID and Cmp_Id = @rCmpId --and GSS_SectionId = t_SectionId
	WHEN MATCHED THEN
	...
	--USING @tbltmp AS SOURCE ON GSG_GoalSetting_Id = @GoalsettingID and Cmp_Id = @rCmpId 
	--WHEN MATCHED THEN
