...
CREATE PROCEDURE [dbo].[P_GET_EMP_INOUT_dailyAttendance]    
 @Cmp_ID  NUMERIC(9,0),    
 @From_Date DateTime,  
 @To_Date DateTime,  
 ...
 *********************************************************************************/  
  ...
  BEGIN   
   ...
   FETCH NEXT FROM curNightShift INTO @curEmp_ID  
    ...
    BEGIN  
	 ...
	 print @First_In_Last_Out_For_InOut_Calculation
        IF @First_In_Last_Out_For_InOut_Calculation = 1  
         Begin   
          ...
          SELECT Emp_Id,Shift_St_Datetime,datediff(SECOND,min (In_Time), max(OUT_Time)) as Duration_In_sec,Emp_OT,Emp_OT_min_Limit,  
            ...
            Max(OUT_Time) as OUT_Time,Shift_ID --,sum(datediff(SECOND,min(Shift_St_Datetime), min(OUT_Time))) -- Commented by Niraj(20062022)
          FROM (             
 ...
 FROM (  
	...
	FROM #EMP_CONS E left JOIN #TMP_EMP_0150_INOUT EIR ON EIR.EMP_ID=E.EMP_ID  
 ...
 BEGIN     
   ...
   IF  object_id('tempdb..#DATA_IO') IS NOT NULL   
    BEGIN        
     ...
     FROM #DATA AS A left JOIN #TMP_EMP_0150_INOUT AS B  
     ...
     --  INNER JOIN #EMP_CONS EC ON D.EMP_ID=EC.EMP_ID  
     --  INNER JOIN T0095_INCREMENT I  WITH (NOLOCK) ON EC.INCREMENT_ID=I.INCREMENT_ID   
     --Where I.Emp_OT_Max_Limit <> '24:00'   
