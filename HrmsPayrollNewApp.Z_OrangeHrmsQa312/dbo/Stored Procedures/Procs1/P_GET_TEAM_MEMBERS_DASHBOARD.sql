...
CREATE PROCEDURE [dbo].[P_GET_TEAM_MEMBERS_DASHBOARD]
	@Cmp_ID numeric(18,0),  
	@Emp_ID numeric(18,0),
...
SET ARITHABORT ON
	...
	*/
	...
	IF @ExecuteFor  IN ('', 'BOTH', 'DIRECT')
		INSERT INTO #Data_Team_Memebrs(R_Emp_ID,Emp_Id,Emp_Full_Name,Alpha_Emp_Code,Emp_Name_Code,Branch_ID,Dept_ID,Branch_Name,
										Dept_Name,Emp_Full_Name1,Desig_Id,Desig_Name,Sales_Code,Date_Of_Join,Cmp_Id,Cmp_Name,CTC)
		SELECT R_Emp_ID,Emp_id,Emp_Full_Name,Alpha_Emp_Code,Emp_Name_Code,Branch_ID,Dept_ID,branch_name,dept_name,Emp_Full_Name1
				,Desig_Id ,Desig_Name	, Sales_Code,Date_Of_Join,cmp_id,case when cmp_id=@cmp_id then '' else cmp_name end as cmp_name,CTC		--added jimit 03022016
			FROM #TMP
...
/**	
	IF @ExecuteFor  IN ('')
		...
		,case @Emp_Search   --added jimit 03082015
		 ...
		 INNER JOIN T0010_COMPANY_MASTER co on co.Cmp_Id=E.Cmp_ID		
	...
	ELSE IF @ExecuteFor  IN ('BOTH', 'INDIRECT')
		...
		SELECT Emp_id,Emp_Full_Name,Alpha_Emp_Code,Emp_Name_Code,branch_id,Dept_ID,branch_name,dept_name,Emp_Full_Name1 
				,Desig_Id ,Desig_Name , Sales_Code,Date_Of_Join,cmp_id,case when cmp_id=@cmp_id then '' else cmp_name end as cmp_name,CTC--,t.cnt--added jimit 03022016	
				,R_Emp_ID
		FROM #TMP
