...
create PROCEDURE [dbo].[JP0001_LeaveCarryForwardNightProcess_backup_old]  
...
--SET @StartDate = dateADD(m,-3,@StartDate)
--SET @EndDate =   dateADD(m,-3,@EndDate)
--SET @ForDate = dateADD(m,-3,@ForDate)
...
DECLARE @Cmp_id as int = 13 -- Static
DECLARE @Branch_Id as int = 0
		...
		BEGIN
					...
					WHERE Leave_Type <> 'Paternity Leave' and (1=(case isnull(leave_Status,0) when 0 then (case when isnull(InActive_Effective_Date,getdate())>=getdate()
					...
					BEGIN	
						...
						--SET @STR = 'EXEC SP_LEAVE_CF_Display_NightProcess  0,'+ cast(@Cmp_id as nvarchar(2)) +', ''' + cast(@StartDate as nvarchar(50)) + ''',''' + cast(@EndDate as nvarchar(50)) + ''',''' + cast(@ForDate as nvarchar(50)) + ''',0,0,0,0,0,0,'''+ CAST(@EMP_ID as Varchar(50)) +''','''','''+ cast(@LeaveId as nvarchar(50)) +''',0,0,0,0' -- BranchId
