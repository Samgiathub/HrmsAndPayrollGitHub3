...
-- Created By rohit on 07052015 For Calculate Esic On Not Effect On Salary Component for Cera.
---28/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[P_RPT_Not_Effect_On_Salary_With_Esic]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	,@Branch_ID 	numeric
...
SET ARITHABORT ON
	...
	,MAD.M_AD_Amount as Amount
	,Case When isnull(Esic.M_AD_Amount,0) > 0 then (MAD.M_AD_Amount * Esic.AD_PERCENTAGE)/100 else 0 End as Esic_Amount
	,Case When isnull(Esic.M_AD_Amount,0) > 0 then MAD.M_AD_Amount - (MAD.M_AD_Amount * Esic.AD_PERCENTAGE)/100 else MAD.M_AD_Amount End as Net_Amount
	, case when ADM.AD_CALCULATE_ON='Transfer OT' then ms.OT_Hours else 0 end
		...
		T0080_EMP_MASTER E WITH (NOLOCK) on MAD.emp_ID = E.emp_ID INNER  JOIN 
			...
			( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,Increment_effective_Date,Basic_Salary from T0095_Increment I WITH (NOLOCK) inner join 
				...
				on E.Emp_ID = I_Q.Emp_ID  inner join
					...
					T0010_company_Master cm WITH (NOLOCK) on MAD.Cmp_ID = cm.cmp_ID  left join 
				(select MD.*,AM.AD_PERCENTAGE  from T0210_MONTHLY_AD_DETAIL MD WITH (NOLOCK) inner join 
				T0050_AD_MASTER AM WITH (NOLOCK) on MD.ad_id = AM.AD_ID and MD.Cmp_ID = Am.CMP_ID and Am.AD_DEF_ID = 3 
				and Md.Cmp_ID=@Cmp_Id and For_date >=@From_Date and For_date <=@To_Date) ESIC on  Mad.cmp_id = Esic.Cmp_ID and Mad.emp_id = Esic.emp_id
