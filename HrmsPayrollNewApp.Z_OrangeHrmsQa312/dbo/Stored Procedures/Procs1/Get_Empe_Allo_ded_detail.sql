...
CREATE PROCEDURE [dbo].[Get_Empe_Allo_ded_detail]
	@Cmp_ID numeric,
	@Emp_ID numeric
AS
SET NOCOUNT ON 
...
and  eed.increment_id=(select max(increment_id) from t0100_emp_earn_deduction WITH (NOLOCK) where t0100_emp_earn_deduction.emp_ID=@emp_id)
order by AD_FLAG Asc
