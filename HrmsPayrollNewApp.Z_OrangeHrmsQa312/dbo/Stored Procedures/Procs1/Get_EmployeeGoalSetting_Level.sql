...
-- =============================================
CREATE PROCEDURE [dbo].[Get_EmployeeGoalSetting_Level]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
						 ...
						 IF @Emp_ID_Cur > 0
							BEGIN
								Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_rpt_manager,rpt_level)
									...
									Where App_Emp_Id = @Emp_ID_Cur and rpt_level = @Rpt_level And SM.Scheme_Type = 'KPI Objectives'
									
								Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_rpt_manager,rpt_level)
										Select distinct SD.Scheme_Id, Leave, Is_Fwd_Leave_Rej,Is_RM,rpt_level 
										From T0050_Scheme_Detail SD WITH (NOLOCK) Inner Join T0040_Scheme_Master SM WITH (NOLOCK) ON SD.Scheme_Id = SM.Scheme_Id
										Where  rpt_level = @Rpt_level and Is_RM = 1 And SM.Scheme_Type = 'KPI Objectives'
								...
								if @Manager_Branch > 0 
									begin								
										Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_branch_manager,rpt_level)
											...
											From T0050_Scheme_Detail SD WITH (NOLOCK) Inner Join T0040_Scheme_Master SM WITH (NOLOCK) ON SD.Scheme_Id = SM.Scheme_Id
											Where rpt_level = @Rpt_level and Is_BM = 1 And SM.Scheme_Type = 'KPI Objectives'						
									end
								END
						ELSE
							BEGIN
								...
								From T0050_Scheme_Detail  SD WITH (NOLOCK) Inner Join T0040_Scheme_Master SM WITH (NOLOCK) ON SD.Scheme_Id = SM.Scheme_Id
								Where SM.Scheme_Type = 'KPI Objectives'
							END	
								   ...
								   Emp_ID numeric    
										...
										begin 
												Insert Into #Emp_Cons(Emp_ID)    
													...
													From T0095_EMP_SCHEME ES WITH (NOLOCK) Inner Join
														(select MAX(Effective_Date) as For_Date, Emp_ID from T0095_EMP_SCHEME WITH (NOLOCK)
														 where Effective_Date<=GETDATE() And Type='KPI Objectives'
														 GROUP BY emp_ID) Qry on      
														 ES.Emp_ID = Qry.Emp_ID and ES.Effective_Date = Qry.For_Date      
														 and Scheme_Id = @Scheme_ID  And Type='KPI Objectives'
													INNER join 
												...
												If @Rpt_level = 1
													Begin 
														Set @SqlQuery = 	
														'Select LAD.Emp_GoalSetting_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																 ' From V0090_EmployeeGoalSetting LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																Where LAD.Emp_GoalSetting_Id Not In (Select Emp_GoalSetting_Id From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
													...
													Begin	 										
														...
														'Select LAD.Emp_GoalSetting_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	...
																	Where (LAD.Emp_GoalSetting_Id Not In (Select Emp_GoalSetting_Id From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
																				...
																				And LAD.Emp_GoalSetting_Id In (Select T0110_EmployeeGoalSetting_Approval WITH (NOLOCK) From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
													...
													Begin  
														...
														'Select LAD.Emp_GoalSetting_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave, '  +   cast(@Rpt_level as VARCHAR(2)) +
																...
																Where (LAD.Emp_GoalSetting_Id Not In (Select Emp_GoalSetting_Id From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
																			...
																			And LAD.Emp_GoalSetting_Id In (Select Emp_GoalSetting_Id From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
																												Where Rpt_Level = ' + Cast(@Rpt_level_Minus_1 as varchar(2)) + ')
																		   )'    
					...
					Begin	
						  ...
						  insert INTO #Appraisal
									Select distinct	
										...
										0,
										0
									From V0090_EmployeeGoalSetting ekpi
										left outer join (
										select RLA.Emp_GoalSetting_Id As App_ID, 
											   Rpt_Level as Rpt_Level , 
											   RLA.approval_Status
											   From T0110_EmployeeGoalSetting_Approval RLA WITH (NOLOCK)
											inner join (Select max(rpt_level) as rpt_level1, Emp_GoalSetting_Id
															From T0110_EmployeeGoalSetting_Approval WITH (NOLOCK)
															Where Emp_GoalSetting_Id In (Select Leave_App_ID From #tbl_Leave_App)
															group by Emp_GoalSetting_Id
														) Qry
											on qry.Emp_GoalSetting_Id = RLA.Emp_GoalSetting_Id and qry.rpt_level1 = RLA.rpt_level
											
										) As Qry1 
						On  ekpi.Emp_GoalSetting_Id = Qry1.App_ID	
						Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = ekpi.Emp_GoalSetting_Id
					...
					begin
						 ...
						 insert INTO #Appraisal
							...
							ekpi.Emp_GoalSetting_Id,
									...
									On  ekpi.Emp_GoalSetting_Id = Qry1.App_ID
								WHERE
								 ekpi.Cmp_ID = @Cmp_ID  and (ekpi.EGS_Status = 4 or ekpi.EGS_Status = 1)
