...
CREATE PROCEDURE [dbo].[P_GET_ESS_NOTIFICATION]  
 @Cmp_ID   INT,  
 @Emp_ID   INT,  
 @Privilege_ID INT = 0  
AS  
  ...
  */  
  ...
  WHERE D.Page_Flag in ('DE','EP')AND (Case When @Privilege_ID > 0  AND IsNull(PD.Privilage_ID,0) = 0 Then 0 Else 1 End ) = 1    
  ...
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_262')  
   BEGIN  
    ...
    -- Commented by deepali - 06072024- not getting other notifiaction .. Sp result in inproper data  
  ...
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_290')  
   BEGIN      
    ...
    SET  Rec_Value = (SELECT COUNT(Timesheet_ID)   
         ...
         Where Timesheet_Type = 'Weekly' and Status_Type =4 ),   
      IsUpdated=1,  
      Category = 'Payroll',  
  ...
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_265')  
   BEGIN  
    ...
    --set @Y_From_Date = dbo.GET_YEAR_START_DATE (year(GETDATE()-1),month(getdate()),1)  
	set @Y_From_Date = dbo.GET_YEAR_START_DATE (YEAR(DATEADD(YEAR, -1, GETDATE())) ,month(getdate()),1)
...
) AND Pass_To_Emp_id=@EMP_ID AND Type='Comp off')  
     BEGIN          
  ...
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_276')  
   BEGIN  
	...
	--Commented by Deepali - 06072024- Showing incorrect value so blocked all notifications 
	--exec Get_Exit_Application_Records @Cmp_ID=@Cmp_ID,@Emp_ID=@emp_id,@Rpt_level=0,@Constrains=N'1=1 and status = ''H''',@Type = 1  
	 
	 --select * from T0200_Emp_ExitApplication with (NOLOCK) where cmp_id = @Cmp_ID and s_emp_id = @Emp_ID and [status] = 'H'
	 --return

	if exists(select 1 from T0200_Emp_ExitApplication with (NOLOCK) where cmp_id = @Cmp_ID and s_emp_id = @Emp_ID and [status] = 'H')
	begin
		select @Rec_Value = count(1) from T0200_Emp_ExitApplication with (NOLOCK) where cmp_id = @Cmp_ID and s_emp_id = @Emp_ID and [status] = 'H'
	end
	else
	begin
		Select @Rec_Value = 0
    ...
    UPDATE #Notification  
    SET  Rec_Value = @Rec_Value,--Rec_Value = (select TOP 1 Rec_Value FROM #Notification_Value ),  
      IsUpdated=1,  
      Category='Payroll',  
  ...
  --  PRINT 'About Me'  
  -- END  
  --select * from #Notification  WHERE Form_Name='TD_Home_ESS_286'  
  --Travel Approval  
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_286')  
   BEGIN  
     
   Declare @TravelType as integer=(Select Setting_Value from T0040_SETTING where Setting_Name = 'Enable Travel Type in Travel Module / Travel Expense' and Cmp_ID=@Cmp_ID)  
     
   if @TravelType=1  
   begin  
   exec SP_Get_Travel_Application_Records_With_TravelType @Cmp_ID,@Emp_ID,0,N'Application_Status = ''P''',1     
   end  
   else  
   begin  
  ...
  IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_287')  
   BEGIN  
   Declare @Travel_Type as integer=(Select Setting_Value from T0040_SETTING where Setting_Name = 'Enable Travel Type in Travel Module / Travel Expense' and Cmp_ID=@Cmp_ID)  
   ...
   begin  
   print ''  
   --exec SP_Get_Travel_Settlement_Application_Records_No_TravelType @cmp_id ,@Emp_ID ,0 ,'(Status_New = ''P'')', 1     
   end  
    ...
    Declare @N_count as numeric(18,0)=0,@NFB_count as numeric(18,0)=0,@NR_count as numeric(18,0)=0,@NRB_count as numeric(18,0)=0  
     ...
     BEGIN  
      ...
      set @N_count=(Select  count(1)  from(select File_App_Id from VIEW_File_FINAL_N_LEVEL_APPROVAL   
             where F_StatusId=3 and Forward_Emp_Id=@Emp_ID and (updatedbyEmp<>@Emp_ID or isnull(updatedbyEmp,0)=0))as t)   
      --print @N_count  
     ...
     BEGIN  
      ...
      set @NR_count=(Select  count(1)  from(select File_App_Id from VIEW_File_FINAL_N_LEVEL_APPROVAL   
             where F_StatusId=5 and Review_Emp_Id<>Emp_ID and Review_Emp_Id=@Emp_ID and (updatedbyEmp<>@Emp_ID or isnull(updatedbyEmp,0)=0))as t)   
     ...
     IF EXISTS(SELECT 1 FROM #Notification WHERE Form_Name='TD_Home_ESS_File_Application_Review_To')  
     BEGIN  
     
      set @NRApp_count=(Select  count(1)  from(select File_App_Id from VIEW_File_FINAL_N_LEVEL_APPROVAL   
             where F_StatusId=5 and Review_Emp_Id=Emp_ID and Review_Emp_Id=@Emp_ID and (updatedbyEmp<>@Emp_ID or isnull(updatedbyEmp,0)=0))as t)   
        
      UPDATE #Notification  
      SET  Rec_Value = @NRApp_count,  
        IsUpdated=1,  
        Category='Payroll',  
        Form_Url='ESS_File_Application.aspx'  
        ,Alias='File Application Review To Emp'  
      WHERE Form_Name = 'TD_Home_ESS_File_Application_Review_To'  
     END  
...
---------------Performance Assessment Pending(RM_Level)Start------------  
 ...
 ----------------------Goal Setting (Start)-----------------------------------------Added by Deepali -20032023 
				DECLARE @CTR1_R_EMP_ID AS INT
				DECLARE @CTR1_HOD_ID AS INT
								
				SELECT @CTR1_R_EMP_ID=COUNT(1)
				FROM V0055_Hrms_Initiate_KPASetting 
				WHERE R_Emp_ID=@EMP_ID AND Initiate_Status IN(4) AND Cmp_Id=@CMP_ID
				
				SELECT @CTR1_HOD_ID=COUNT(1)
				FROM V0055_Hrms_Initiate_KPASetting 
				WHERE Hod_Id=@EMP_ID AND Initiate_Status IN(4) AND Cmp_Id=@CMP_ID
	 		...
	 		--	CASE WHEN @KPA_Default=1 then 'Ess_EmployeeKPA_Approval_one.aspx' else 'Ess_EmployeeKPA_Approval.aspx' end as Form_Name	
				UPDATE	#Notification
				SET		--Rec_Value = (@CTR_R_EMP_ID+@CTR_HOD_ID), --Commented by Deepali -20032023
						Rec_Value = 1, --Added by Deepali -20032023 
						IsUpdated=1,
						Category='Payroll',
						Form_Url= CASE WHEN @KPA_Default=1 then 'Ess_EmployeeKPA_one.aspx' else 'Ess_EmployeeKPA.aspx' end,
						Alias= 'Fill My' +@CAPTION 
				WHERE	Form_Name like 'My KPA%' or Form_Name = 'TD_Home_ESS_355'
-----------------------Goal Setting(End)-----------------------------------------
 -----------------------Goal Approval(Start)-----------------------------------------  
