...
Create PROCEDURE [dbo].[P_RPT_SALARY_VARIATION_BKP_07022024]
	 @CMP_ID				Numeric
	,@From_Date			DATETIME
	,@To_Date 			DATETIME 
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Basic Salary' Particuler ,
isnull(Prev.salary_Amount,0) Prev_Amt,(isnull(Curr.salary_Amount,0) + isnull(Curr.Arear_Basic,0))  Curr_Amt
,(isnull(Curr.salary_Amount,0) + isnull(Curr.Arear_Basic,0)) - isnull(Prev.salary_Amount,0) Diff_Amt
from T0200_MONTHLY_SALARY Curr
inner join #Emp_Cons EC on EC.Emp_ID = curr.Emp_ID
left join  (select MS.Emp_ID,(salary_Amount  + isnull(Arear_Basic,0)) salary_Amount from T0200_MONTHLY_SALARY MS 
			...
			) Prev on Prev.Emp_ID = Curr.Emp_ID 

where cmp_id =@cmp_id and month_st_date >= @From_Date and Month_End_Date < = @To_Date 
--and isnull(Curr.Basic_Salary,0) - isnull(Prev.Basic_Salary,0) <> 0
...
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Basic Salary' Particuler ,
...
from T0201_Monthly_Salary_Sett Curr
inner join #Emp_Cons EC on EC.Emp_ID = curr.Emp_ID
...
group by Curr.Emp_ID,S_Eff_Date) AS Sett on Sett.Emp_ID = E.Emp_id and E.Particuler = Sett.Particuler
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'OT Amount' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #Earning E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'OT Amount' Particuler ,
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Week Off Working' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #Earning E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Week Off Working' Particuler ,
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Holiday Working' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #Earning E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Holiday Working' Particuler ,
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Leave Encash Amount' Particuler ,
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Claim Amount' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #Earning E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Claim Amount' Particuler ,
...
insert into #Earning   
select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Travel Amount' Particuler ,
...
------------------------------------------------For Earnig ---------------------------------------------------
insert into #Earning  
select 0 as rn,AD.AD_LEVEL,MAD.Emp_ID,AD.AD_ID,AD.AD_NAME,
isnull(Prev.M_AD_Amount,0) PrvAmt,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0)) CurAmt
,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0)) - isnull(Prev.M_AD_Amount,0) DiffAmt
...
left join (select AD_LEVEL,MAD.Emp_ID,AD.AD_ID,(M_AD_Amount  + isnull(M_AREAR_AMOUNT,0)) M_AD_Amount
		   from T0210_MONTHLY_AD_DETAIL MAD 
		   inner join #Emp_Cons EC on EC.Emp_ID = MAD.Emp_ID
		   inner join T0050_AD_MASTER AD on AD.AD_ID = MAD.AD_ID
		   where MAD.Cmp_ID = @Cmp_id and For_Date between @PreF_Date and @PreT_Date
		   and AD_NOT_EFFECT_SALARY =0 and AD_FLAG ='I' and S_Sal_Tran_ID is null
		  ) as Prev on Prev.Emp_ID = MAD.Emp_ID and Prev.AD_ID = MAD.AD_ID

where MAD.Cmp_ID = @Cmp_id and For_Date between @From_Date and @To_Date
and AD_NOT_EFFECT_SALARY =0 and AD_FLAG ='I' and S_Sal_Tran_ID is null
--and isnull(MAD.M_AD_Amount,0) - isnull(Prev.M_AD_Amount,0) <> 0

update E set E.Prev_Amt = (E.Prev_Amt+sett.PrvAmt),
			E.Curr_Amt = (E.Curr_Amt+Sett.CurrAmt),
			E.Diff_Amt = (E.Diff_Amt+Sett.DiffAmt) from #Earning E
inner join  
(select Mss.Emp_ID,S.AD_ID, isnull(sum(M_AD_Amount),0) as CurrAmt ,isnull(sum(PrvAmt),0) PrvAmt
, isnull(sum(M_AD_Amount),0) - isnull(sum(PrvAmt),0) DiffAmt
...
left Join (select Mss.Emp_ID,S.AD_ID,  isnull(sum(M_AD_Amount),0) as PrvAmt
from T0201_MONTHLY_SALARY_SETT MSS 
left join T0210_MONTHLY_AD_DETAIL S on mss.S_Sal_Tran_ID = s.S_Sal_Tran_ID
inner join #Emp_Cons EC on EC.Emp_ID = MSS.Emp_ID
where S.cmp_id =@cmp_id and Month(S_Eff_Date) =Month(@PreT_Date) and Year(S_Eff_Date) = Year(@PreT_Date)   
group by Mss.Emp_ID, s.AD_ID) PRV on PRV.Emp_ID = S.Emp_ID and PRV.AD_ID = S.AD_ID
where S.cmp_id =@cmp_id and Month(S_Eff_Date) =Month(@To_Date) and Year(S_Eff_Date) = Year(@To_Date)   
group by Mss.Emp_ID, s.AD_ID) As Sett on E.Emp_id = Sett.Emp_ID and E.AD_id = Sett.AD_ID
...
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'LWF' Particuler ,
...
where cmp_id =@cmp_id and month_st_date >= @From_Date and Month_End_Date < = @To_Date 
--and isnull(Curr.LWF_Amount,0) - isnull(Prev.LWF_Amount,0) <> 0
...
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'LWF' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'PT' Particuler ,
...
where cmp_id =@cmp_id and month_st_date >= @From_Date and Month_End_Date < = @To_Date 
--and isnull(Curr.PT_Amount,0) - isnull(Prev.PT_Amount,0) <> 0


Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #EmployeeContribution E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'PT' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Advance Amount' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #EmployeeContribution E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Advance Amount' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Loan Amount' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #EmployeeContribution E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Loan Amount' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Loan Interest' Particuler ,
...
Update E set E.Prev_Amt = (E.Prev_Amt + Sett.Prev_Amt),
			 E.Curr_Amt = (E.Curr_Amt + Sett.Curr_Amt ),
			 E.Diff_Amt = (E.Diff_Amt + Sett.Diff_Amt ) from #EmployeeContribution E
inner join (select 0 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Loan Interest' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,0 as sr ,Curr.Emp_ID,0 AD_id,'Travel Advance Amount' Particuler ,
...
insert into  #EmployeeContribution 
select 1 as rn,AD.AD_LEVEL,MAD.Emp_ID,AD.AD_ID,AD.AD_NAME
,isnull(Prev.M_AD_Amount,0) PrvAmt,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  CurAmt
,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  - isnull(Prev.M_AD_Amount,0) DiffAmt
from T0210_MONTHLY_AD_DETAIL MAD 
inner join #Emp_Cons EC on EC.Emp_ID = MAD.Emp_ID
...
left join (select AD_LEVEL,MAD.Emp_ID,AD.AD_ID,(M_AD_Amount  + isnull(M_AREAR_AMOUNT,0)) M_AD_Amount 
		   ...
		   inner join T0050_AD_MASTER AD on AD.AD_ID = MAD.AD_ID
		   where MAD.Cmp_ID = @Cmp_id and For_Date between @PreF_Date and @PreT_Date
		   and AD_PART_OF_CTC = 0 
		   and AD_FLAG ='D' and S_Sal_Tran_ID is null
		  ) as Prev on Prev.Emp_ID = MAD.Emp_ID and Prev.AD_ID = MAD.AD_ID
...
update E set E.Prev_Amt = (E.Prev_Amt+sett.PrvAmt),
			E.Curr_Amt = (E.Curr_Amt+Sett.CurrAmt),
			E.Diff_Amt = (E.Diff_Amt+Sett.DiffAmt) 			
from #EmployeeContribution E
inner join  
(select Mss.Emp_ID,S.AD_ID, isnull(sum(M_AD_Amount),0) as CurrAmt ,isnull(sum(PrvAmt),0) PrvAmt
, isnull(sum(M_AD_Amount),0) - isnull(sum(PrvAmt),0) DiffAmt
from T0201_MONTHLY_SALARY_SETT MSS 
...
select 3 as rn,AD.AD_LEVEL,MAD.Emp_ID,AD.AD_ID,AD.AD_NAME
,isnull(Prev.M_AD_Amount,0) PrvAmt,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  CurAmt
,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  - isnull(Prev.M_AD_Amount,0) DiffAmt
from T0210_MONTHLY_AD_DETAIL MAD 
inner join #Emp_Cons EC on EC.Emp_ID = MAD.Emp_ID
...
left join (select AD_LEVEL,MAD.Emp_ID,AD.AD_ID,(M_AD_Amount  + isnull(M_AREAR_AMOUNT,0)) M_AD_Amount 
		   ...
		   inner join T0050_AD_MASTER AD on AD.AD_ID = MAD.AD_ID
		   where MAD.Cmp_ID = @Cmp_id and For_Date between @PreF_Date and @PreT_Date
		   and AD_PART_OF_CTC = 1 and AD_NOT_EFFECT_SALARY =1
		   and Allowance_Type ='A' and S_Sal_Tran_ID is null
		  ) as Prev on Prev.Emp_ID = MAD.Emp_ID and Prev.AD_ID = MAD.AD_ID
...
select 2 as rn,AD.AD_LEVEL,MAD.Emp_ID,AD.AD_ID,AD.AD_NAME
,isnull(Prev.M_AD_Amount,0) PrvAmt,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  CurAmt
,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  - isnull(Prev.M_AD_Amount,0) DiffAmt
from T0210_MONTHLY_AD_DETAIL MAD
inner join #Emp_Cons EC on EC.Emp_ID = MAD.Emp_ID
...
left join (select AD_LEVEL,MAD.Emp_ID,AD.AD_ID,(M_AD_Amount  + isnull(M_AREAR_AMOUNT,0)) M_AD_Amount 
		   ...
		   where MAD.Cmp_ID = @Cmp_id and For_Date between @PreF_Date and @PreT_Date
			and AD_PART_OF_CTC = 1 and AD_NOT_EFFECT_SALARY =1
			and Allowance_Type ='R' and S_Sal_Tran_ID is null
		  ) as Prev on Prev.Emp_ID = MAD.Emp_ID and Prev.AD_ID = MAD.AD_ID
...
select 4 as rn,AD.AD_LEVEL,MAD.Emp_ID,AD.AD_ID,AD.AD_NAME
,isnull(Prev.M_AD_Amount,0) PrvAmt,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  CurAmt
,(isnull(MAD.M_AD_Amount,0) + isnull(MAD.M_AREAR_AMOUNT,0))  - isnull(Prev.M_AD_Amount,0) DiffAmt
from T0210_MONTHLY_AD_DETAIL MAD
inner join #Emp_Cons EC on EC.Emp_ID = MAD.Emp_ID
...
left join (select AD_LEVEL,MAD.Emp_ID,AD.AD_ID,(M_AD_Amount  + isnull(M_AREAR_AMOUNT,0)) M_AD_Amount 
		   ...
		   inner join T0050_AD_MASTER AD on AD.AD_ID = MAD.AD_ID
		    where MAD.Cmp_ID = @Cmp_id and For_Date between @PreF_Date and @PreT_Date
			and AD_PART_OF_CTC = 0 and AD_NOT_EFFECT_SALARY =1
			and AD_FLAG ='I' and S_Sal_Tran_ID is null
		  ) as Prev on Prev.Emp_ID = MAD.Emp_ID and Prev.AD_ID = MAD.AD_ID
