...
CREATE PROCEDURE [dbo].[P_GET_BRANCHWISE_HR]
	@CMP_ID		NUMERIC,
	@TO_DATE	DATETIME,
...
SET ARITHABORT ON
    ...
    IF OBJECT_ID('tempdb..#EMP_CONS') IS NULL
		...
		SELECT EC.EMP_ID , EC.BRANCH_ID ,EM.ALPHA_EMP_CODE , EM.EMP_FULL_NAME ,
			     (Case When @Email_Type = 'P' 
							   Then EM.Other_Email
						  ELSE
								EM.Work_Email
				  END) as Work_Email,		  
			   EM.Other_Email, T.HR_EMAIL_ID AS HR_WORK_EMAIL , LO.LOGIN_ID
		INTO	#RESULT
		FROM	#EMP_CONS EC
			Left JOIN
				(	SELECT	EMAIL_ID AS HR_EMAIL_ID, B.BRANCH_ID
					FROM	T0030_BRANCH_MASTER B WITH (NOLOCK)
								INNER JOIN T0011_LOGIN L WITH (NOLOCK) ON B.Cmp_ID = L.Cmp_ID AND (CHARINDEX(',' + CAST(B.BRANCH_ID AS VARCHAR(10)) + ',', ',' + L.BRANCH_ID_MULTI + ',') > 0 )
