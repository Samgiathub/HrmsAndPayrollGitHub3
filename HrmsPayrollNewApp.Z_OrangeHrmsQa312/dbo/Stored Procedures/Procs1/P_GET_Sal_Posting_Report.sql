...
-- =============================================
CREATE PROCEDURE  [dbo].[P_GET_Sal_Posting_Report]
	@cmp_id numeric(18)
...
(select I.Emp_Id as Emp_id1, Increment_Effective_Date,i.Dept_ID, i.Grd_ID, i.Branch_ID, BM.Branch_Name,i.SalDate_id, i.Segment_ID, i.Vertical_ID, i.SubVertical_ID, i.subBranch_ID,i.Desig_Id, ISNULL(i.Cat_ID, 0) AS Cat_ID,I.Increment_ID As Max_Increment_ID_1 
	...
	'
	--select @Qry
if @strwhere NOT like '%Sal_Pos_MID%'
BEGIN	
	EXEC(@Qry)


	select convert(varchar, PM.System_Date, 105) as 'Date of Posting',''as 'Transaction Number',EPM.Emp_code 'Empl Code',EPM.Emp_Full_Name 'Full Name',BS.Segment_Name 'Business Segment ',MSP.Branch 'Branch',MSP.Account 'GL A/c',AX.Narration 'GL Name',MSp.Allowance_Name 'Allowance Name'
	,CASE WHEN MSP.AD_ID = 2003 THEN '40' WHEN MSP.AD_ID = 1003 THEN '50' 
		WHEN AM.AD_ID IS NOT NULL THEN CASE WHEN AM.AD_FLAG = 'I' THEN '40' ELSE '50'END ELSE '' END 'Posting Key'
	,
		CAST(MSp.Ammount as varchar(10)) 
		  Amount  
	,MSP.Center_Code 'Cost Ceter'
	from #Temp_Emp EM
	INNER JOIN T0210_MONTHLY_Sal_POS_DETAIL MSP ON MSP.EMP_ID= EM.Emp_ID AND MSP.Post__Date = EM.Month_St_Date
	INNER JOIN T0200_Salary_Posting_Master PM ON PM.Post_Req_ID = MSP.POst_req_ID
	INNER JOIN T0080_EMP_MASTER EPM ON EPM.Cmp_ID= @cmp_id AND EPM.Emp_ID = MSP.EMP_ID
	LEFT JOIN T0050_AD_MASTER AM ON AM.CMP_ID = @cmp_id AND AM.AD_ID = MSP.AD_ID
	INNER JOIN  T9999_Ax_Mapping AX ON AX.Cmp_id= @cmp_id AND AX.Ad_id = MSP.AD_ID AND AX.Segment_ID = MSP.Segment_ID
	INNER JOIN T0040_Business_Segment BS ON BS.Cmp_ID= @cmp_id AND BS.Segment_ID = AX.Segment_ID
	WHERE MSP.Ammount > 0
	ORDER BY MSp.POst_req_ID ,MSP.EMP_ID,'Posting Key'
END
ELSE 
	BEGIN
		...
		,
			 ...
			 Amount  
		,MSP.Center_Code 'Cost Ceter'
		from T0200_Salary_Posting_Master PM
		INNER JOIN T0210_MONTHLY_Sal_POS_DETAIL MSP ON MSP.POst_req_ID = PM.Post_Req_ID
		INNER JOIN T0080_EMP_MASTER EPM ON EPM.Cmp_ID= @cmp_id AND EPM.Emp_ID = MSP.EMP_ID
		LEFT JOIN T0050_AD_MASTER AM ON AM.CMP_ID = @cmp_id AND AM.AD_ID = MSP.AD_ID
		INNER JOIN  T9999_Ax_Mapping AX ON AX.Cmp_id= @cmp_id AND AX.Ad_id = MSP.AD_ID AND AX.Segment_ID = MSP.Segment_ID
		INNER JOIN T0040_Business_Segment BS ON BS.Cmp_ID= @cmp_id AND BS.Segment_ID = AX.Segment_ID
		WHERE PM.Sal_Pos_MID = RIGHT(@strwhere,1) AND  MSP.Ammount > 0
		ORDER BY MSp.POst_req_ID ,MSP.EMP_ID,'Posting Key'
	END
END
