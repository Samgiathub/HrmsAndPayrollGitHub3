...
CREATE PROCEDURE [dbo].[P_IT_Tax_Employee_Details]
 @Cmp_ID  numeric  
 ,@From_Date  datetime  
 ,@To_Date  datetime   
 ,@Branch_ID  numeric   = 0  
...
SET ARITHABORT ON
	...
	--	END	
			...
			select ITD.EMP_ID,SUM(ITD.AMOUNT)as Amount,Detail_1,Detail_2,Detail_3,Comments,
					Em.Emp_Full_Name,Em.Alpha_Emp_Code,Em.Pan_No,Em.Present_Street as emp_Address,@Financial_Year as Financial_Year
					,Em.Father_name,Dm.Desig_Name
			from #Emp_Cons EC LEFT JOIN
			T0100_IT_DECLARATION ITD WITH (NOLOCK)  ON EC.Emp_ID = ITD.EMP_ID INNER JOIN
			T0070_IT_MASTER ITM WITH (NOLOCK) ON ITD.IT_ID = ITM.IT_ID INNER JOIN
			T0110_IT_Emp_Details ITED WITH (NOLOCK) ON ITD.IT_ID = ITED.IT_ID INNER JOIN			
			...
			where --ITD.EMP_ID = EC.Emp_ID and 
			ITD.CMP_ID = @Cmp_ID
			AND ITM.IT_Def_ID = 1 --and ITED.Emp_ID = EC.Emp_ID
			AND ISNULL(ITED.Date,'') = '' and ITED.Amount = 0
			AND ITED.Financial_Year = @Financial_Year
			group by ITD.Emp_ID,Detail_1,Detail_2,Detail_3,Comments,Em.Emp_Full_Name
			,Em.Alpha_Emp_Code,Em.Pan_No,Em.Present_Street,Em.Father_name,Dm.Desig_Name
		RETURN
