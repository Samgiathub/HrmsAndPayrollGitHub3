CREATE  PROCEDURE [dbo].[GetSurvey_QuestionWiseSummary]
	 @Cmp_Id		numeric(18,0)
...
SET ARITHABORT ON;
	...
	WHERE  SR.SurveyQuestion_Id=@SurveyQuestion_Id and SR.Survey_Id = @Survey_Id AND (EM.Alpha_Emp_Code=ISNULL(@emp_code,EM.Alpha_Emp_Code) or EM.Emp_First_Name like ISNULL(@emp_code,EM.Emp_First_Name))
	...
	If @Survey_Type = 'Multiple Choice' or @Survey_Type = 'Radiobutton List' or @Survey_Type='DropdownList' or @Survey_Type='CheckBoxList'
		Begin
			select @optionstr = Question_Option from T0052_SurveyTemplate WITH (NOLOCK) where SurveyQuestion_Id=@SurveyQuestion_Id and Survey_Id = @Survey_Id
