...
-- Description:	exec Get_EmpKPI_Level 9,1355,1,'',0
---23/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
-- =============================================
CREATE PROCEDURE [dbo].[Get_EmpKPI_Level]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
									...
									if @is_branch_manager = 1
										begin 
												...
												If @Rpt_level = 1
													Begin 
														Set @SqlQuery = 	
														'Select LAD.EmpKPI_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																 ' From V0080_EmpKpiObj LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																Where LAD.EmpKPI_Id Not In (Select EmpKPI_Id From T0090_EmpKPI_Approval  WITH (NOLOCK)
													...
													Begin	 										
														...
														'Select LAD.EmpKPI_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	...
																	Where (LAD.EmpKPI_Id Not In (Select EmpKPI_Id From T0090_EmpKPI_Approval WITH (NOLOCK)
																				...
																				And LAD.EmpKPI_Id In (Select T0090_EmpKPI_Approval WITH (NOLOCK) From T0090_EmpKPI_Approval WITH (NOLOCK)
													...
													Begin  
														...
														'Select LAD.EmpKPI_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave, '  +   cast(@Rpt_level as VARCHAR(2)) +
																...
																Where (LAD.EmpKPI_Id Not In (Select EmpKPI_Id From T0090_EmpKPI_Approval WITH (NOLOCK)
																			...
																			And LAD.EmpKPI_Id In (Select EmpKPI_Id From T0090_EmpKPI_Approval WITH (NOLOCK)
																												Where Rpt_Level = ' + Cast(@Rpt_level_Minus_1 as varchar(2)) + ')
																		   )'    
					...
					Begin	
						  ...
						  insert INTO #Appraisal
									Select distinct	
										...
										0,
										0
									From V0080_EmpKpiObj ekpi
										left outer join (
										select RLA.EmpKPI_Id As App_ID, 
											   Rpt_Level as Rpt_Level , 
											   RLA.approval_Status
											   From T0090_EmpKPI_Approval RLA WITH (NOLOCK)
											inner join (Select max(rpt_level) as rpt_level1, EmpKPI_Id
															From T0090_EmpKPI_Approval WITH (NOLOCK)
															Where EmpKPI_Id In (Select Leave_App_ID From #tbl_Leave_App)
															group by EmpKPI_Id
														) Qry
											on qry.EmpKPI_Id = RLA.EmpKPI_Id and qry.rpt_level1 = RLA.rpt_level
											
										) As Qry1 
						On  ekpi.EmpKPI_Id = Qry1.App_ID	
						Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = ekpi.EmpKPI_Id
					...
					begin
						 ...
						 insert INTO #Appraisal
							...
							ekpi.EmpKPI_Id,
									...
									On  ekpi.EmpKPI_Id = Qry1.App_ID
								WHERE
								 ekpi.Cmp_ID = @Cmp_ID  and (ekpi.status = 4 or ekpi.status = 1)
