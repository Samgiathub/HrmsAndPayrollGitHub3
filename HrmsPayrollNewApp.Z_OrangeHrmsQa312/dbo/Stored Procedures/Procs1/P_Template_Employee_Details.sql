...
-- Description:	Get the Template Data for Customized Report
-- =============================================
CREATE PROCEDURE [dbo].[P_Template_Employee_Details] 
	@Cmp_Id		NUMERIC  
	,@From_Date		DATETIME
	,@To_Date 		DATETIME
...
BEGIN
 ...
 SELECT @columns = STUFF((SELECT ',' + COALESCE(@columns + ',', '') + '[' + CAST(REPLACE(REPLACE(Field_Name, '[', ''), ']', '') AS NVARCHAR(MAX)) + ']'
	...
	SET @query = 'SELECT ROW_NUMBER() OVER (ORDER BY Employee_Code) AS ''Sr.No.'',emp_id,Employee_Code as Alpha_Emp_Code,''="'' +Employee_Code +''"'' as Employee_Code,mobile_no,Old_Refrence_Code,Employee_Name as Emp_Full_name,Employee_Name,Emergency_Contact as Home_Telephone,Branch_Name,Dept_Name as Department,Desig_Name as Designation,mobile_no as Personal_Mobile_No,IMEI_NO,EMP_FULL_NAME AS Manager,
			      CASE WHEN IS_FILLED <> '''' THEN ''Yes'' else ''No'' end as Template_Completion_Status,Created_Date,'+ @columns +'
						FROM (
								...
								where Convert(datetime,SV.Created_Date,112) between ''' + Convert(varchar,@From_Date,112) + ''' and  ''' + Convert(varchar,@To_Date,112) + '''
							) as s
					PIVOT
						(						 
							Max(Answer)
