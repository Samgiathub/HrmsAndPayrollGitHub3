...
CREATE PROCEDURE [dbo].[Mobile_HRMS_SCHEME_DETAILS_ESS_GET]    
  @Cmp_ID  Numeric    
 ,@Branch_ID  Numeric   = 0   
 ,@Cat_ID  Numeric  = 0  
 ,@Grd_ID  Numeric  = 0   
 ...
 OPEN Emp_Scheme_Cursor    
  ...
  if ( @DynHierId >0)    
   ...
   WHERE Dy.EMP_ID =@Emp_ID1 and  Rpt_Level=2   for xml path (''))    
        ...
        End      
        --Commented else part to resolved bug #48215 by yogesh on 12122023   
       -- Else    
       --  Begin    
  ...
  --Declare @ABC as Varchar(50)=(select LEFT(cast(@temp as varchar(200)), LEN(cast(@temp as varchar(200))) - 1))
