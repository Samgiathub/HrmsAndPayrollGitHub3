 ...
 --exec Emp_Details_Form_A @Company_Id=119,@From_Date='2020-01-01 00:00:00',@To_Date='2020-01-31 00:00:00',@Branch_ID='',@Cat_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='18412#14938',@Report_Type='ESIC'                  
    ...
    -- exec Get_Rpt_Claim_Details_format4_for_EyeRefraction_Detail @cmp_Id=203,@From_Date='2019-01-01 00:00:00',@To_Date='2021-12-01 00:00:00',@Branch_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='',@Cat_ID='',@Segment_ID='0',@Vertical='0',@SubVertical='0'                  
...
CREATE PROCEDURE [dbo].[Get_Rpt_Claim_Details_format4_for_EyeRefraction_Detail]                           
   @Cmp_ID NUMERIC                                    
  ,@From_Date  DATETIME                                
  ,@To_Date   DATETIME                                
  ,@Branch_ID  VARCHAR                       
 ...
 IF @Constraint <> ''                                
   BEGIN                                 
     ...
     ,cd.Claim_Apr_Dtl_ID,cd.Claim_NoofPerson,cd.Claim_Apr_Date    
	   ,cf.Claim_FamilyMemberName,cf.Claim_Age,cf.Claim_FamilyRelation,cf.Claim_Amount,cd.Claim_App_ID,
	 cf.cf_BillAmount,cf.cf_BillDate,cf.cf_BillNumber
	,[dbo].[F_GET_FINANCIAL_YEAR](cd.Claim_Apr_Date)as Financial_Yr,g.Grd_Name
    --,cfd.Claim_FamilyMemberName,cfd.Claim_Age,cfd.Claim_FamilyRelation,cfd.Claim_Amount,CD.Claim_App_ID    
    --,cfd.cfa_BillDate,cfd.cfa_BillAmount,cfd.cfa_BillNumber,
	,(case when cm.Claim_Name like '%Self%' then 'Self' else '' end)as Flag
    ...
    inner join T0040_CLAIM_MASTER CM with (NoLock) on CM.Claim_ID=cd.Claim_ID    
	 left join T0110_Claim_FamilyDetails cf with (NoLock) on cf.Claim_AppId=cd.Claim_App_ID
    --left join T0110_Claim_Approval_FamilyDetails CFD with  (NoLock) on cfd.Claim_AppId=cd.Claim_App_ID          
    ...
    and cd.Claim_Apr_Date between    @From_Date and @To_Date and cm.Claim_Report_Def_Id=9  
 --and cm.Claim_Name like '%eye refraction %'              
