CREATE PROCEDURE [dbo].[P_Check_Leave_Notice_Period]
	@CMP_ID NUMERIC,
	@LEAVE_ID NUMERIC,
	@APP_DATE DATETIME,
...
AS
	...
	DECLARE @NOTICE_DAYS NUMERIC,@Count_Notice_Period Numeric 
	
	Select @Count_Notice_Period = Count_WeekOff_Notice_Period from T0040_LEAVE_MASTER WHERE Leave_ID=@LEAVE_ID and Cmp_ID = @CMP_ID --Added by Mr.Mehul on 10-03-2023 for (Bug #19826)
	...
	IF @NOTICE_DAYS < 0 AND @FROM_DATE < @APP_DATE	
		BEGIN
	...
	ELSE
	BEGIN
		if @Count_Notice_Period = 1 --Added by Mr.Mehul on 10-03-2023 for (Bug #19826)
		begin 
	...
	if @NOTICE_DAYS > 0 	--added by Mr.Mehul condition for misepl notice period issue 
		begin
			IF @NOTICE_DAYS >= @WorkingDays		
				BEGIN		
	...
	IF @Rais_Error = 0 
		BEGIN
			SELECT @display_leave_period AS NOTICE_MSG
		END
	ELSE
