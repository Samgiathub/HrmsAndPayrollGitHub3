...
CREATE PROCEDURE [dbo].[P_GET_EMP_APPROVAL_RECORDS_OLD] 
...
SET ARITHABORT ON
	 ...
	 --BEGIN
			...
			FROM	T0060_EMP_MASTER_APP APP WITH (NOLOCK)
						INNER JOIN (SELECT	MAX(Emp_Tran_ID)  As Emp_Tran_ID, Emp_Application_ID 
									...
									GROUP BY APP1.Emp_Application_ID)
									 AS APP1 ON APP.Emp_Tran_ID=APP1.Emp_Tran_ID AND APP.Emp_Application_ID=APP1.Emp_Application_ID									
  ...
  --   ELSE
		...
		--			INNER JOIN (SELECT	MAX(Emp_Tran_ID)  As Emp_Tran_ID, Emp_Application_ID 
		--						FROM	T0060_EMP_MASTER_APP APP1
	...
	exec SP_GetPage @Items_Per_Page=@Item_Per_Page,@Page_No=@PageNo,
			@Select_Fields='Emp_Application_ID, Emp_Tran_ID,Emp_full_Name,Enroll_No,Date_Of_Join,Approved_Date, Applicant_Name,Loc_name,Grd_Name,Branch_Name,Dept_Name,Desig_Name,Emp_code,Cmp_Name,Cmp_ID,Branch_ID,Desig_ID,Dept_ID,Alpha_Emp_code,Approve_Status,IsDisabled ',
			@From='#FINAL_RESULT',
