...
CREATE PROCEDURE [dbo].[P0040_Asset_Details_Import]
...
IF @Tran_type = 'I'
    ...
    ----             SET @MyString = @Branch_Name

    --                 -- IF (@Branch_Name LIKE '%[^a-zA-Z0-9]%')
				--	 IF (@Branch_Name LIKE '%[&''"]%')
    --                   begin

				--	Insert Into dbo.T0080_Import_Log Values (@Row_No,@Cmp_Id,0,'Enter Correct Branch Name',0,'Branch Name Contains special characters',GetDate(),'Asset Details',@GUID)						
				--	Set @Log_Status=1
				--	Return
				--	end 
				--END      
  ...
  --    
      ...
      --          if @Branch_Name <> '' 
						-- begin
						-- ---- --Added  by aswini 29/11/2023
						 --set @Branch =  REPLACE(@Branch,'amp;','')
						--if @Branch_Name like '%[^a-zA-Z0-9]%' and @Branch_Name like '%[^&@]%'
       --           begin
	--   print 'Aswini testing'
	--   Print @Branch_Name
	--     --select @Branch_Name= Branch_Name from T0030_BRANCH_MASTER WITH (NOLOCK) where Branch_Name=@Branch_Name and Cmp_ID=@Cmp_ID
	--	 print '1111'
 --Print @Branch_Name
	  ...
	  -- return
	   ...
	   --SET @Branch_Name = TRIM(@Branch_Name)
		--if exists(select 1 from T0030_BRANCH_MASTER WITH (NOLOCK) where convert(nvarchar,Branch_Name)=@Branch_Name and Cmp_ID=@Cmp_ID)
