...
CREATE PROCEDURE [dbo].[KPMS_GOAL_RPT_SCHEME_DETAILS_ESS_GET]  
  @Cmp_ID  Numeric  
 ,@From_Date  Datetime  
 ,@To_Date  Datetime  
 ,@Branch_ID  Numeric   
...
OPEN Emp_Scheme  
 ...
 IF @Report_Type <> ''  
  BEGIN  
   ...
   select Emp_ID1,Rpt_Mgr_1,Rpt_Mgr_2,Rpt_Mgr_3,Rpt_Mgr_4,Rpt_Mgr_5,Rpt_Mgr_6,Rpt_Mgr_7,Rpt_Mgr_8,Max_Level from #EMPSCHEME WHERE Scheme_Id = 640
  END  
 ELSE  
  BEGIN  
   select * from #EMPSCHEME where Scheme_Id = 640 order by RIGHT(REPLICATE(N' ', 500) + ALPHA_EMP_CODE, 500)  
  END   
