...
-- =============================================
CREATE PROCEDURE [dbo].[Home_Listing_Detail_HRMS]
	@Cmp_ID numeric(18,0),  
...
BEGIN
	...
	--DECLARE @module_status as INT
	--SET @module_status = 0
	
	--SELECT @module_status =module_status
	--  FROM T0011_module_detail WHERE Cmp_id = @Cmp_ID and module_name ='Appraisal2'
	...
	--	BEGIN
			...
			--(Team)team assessment
				IF EXISTS(
					...
					BEGIN
						...
						SELECT CASE WHEN isnull(Form_url,'SelfAppraisal_Form.aspx')<>'' THEN Form_url  ELSE 'SelfAppraisal_Form.aspx' END Form_url
				...
				BEGIN	
					...
					WHERE Emp_Id = @emp_id AND   
						  SA_Startdate <= CONVERT(varchar(10),GETDATE(),120) and t.show = 1 --and ( SA_Enddate >= CONVERT(varchar(10),GETDATE(),120) and SA_Status<>1) and SA_SendToRM <> 1
							print @init_Id	
					SELECT CASE WHEN isnull(Form_url,'SelfAppraisal_Form.aspx')<>'' THEN Form_url +'?Initid=' + cast(@init_Id AS VARCHAR) ELSE 'SelfAppraisal_Form.aspx?Initid=' + cast(@init_Id AS VARCHAR) END Form_url
					FROM T0000_DEFAULT_FORM WITH (NOLOCK)
					WHERE Form_Name='TD_Home_ESS_294'		
				END
			ELSE
			...
			-------------SelfAssessmentForm_Pending------------
				...
				SELECT @initcount as InitiateId,
				CASE WHEN isnull(Form_url,'Ess_EmpAssessment.aspx') <> '' THEN Form_url 
				WHEN @KPA_Default=1 THEN 'Ess_EmpAssessment.aspx' ELSE 'Ess_PerformanceAssessment.aspx' END Form_url
				FROM T0000_DEFAULT_FORM WITH (NOLOCK) WHERE Form_Name='TD_Home_ESS_295'	
			----------------------------------------------------
			---------------AppraisalFinalization() HOD/GH-------
			...
			--Select count(InitiateId) AS InitiateId from V0050_HRMS_InitiateAppraisal where  DATEPART(YYYY,SA_Startdate) =" & DateTime.Now.Year & " AND Overall_Status =(case when (isnull(SendToHOD,0) =1 and Overall_Status =0) then null when (isnull(SendToHOD,0) = 0 and Overall_Status=0) then Overall_Status when (isnull(SendToHOD,0) = 1 and Overall_Status=7) then Overall_Status end )  And gh_id=@emp_id
			
			SELECT @initcount AS InitiateId ,CASE WHEN isnull(Form_url,'Ess_AppraisalFinalization.aspx')<>'' THEN Form_url  ELSE 'Ess_AppraisalFinalization.aspx' END Form_url
			FROM T0000_DEFAULT_FORM WITH (NOLOCK)
			...
			SELECT @initcount AS InitiateId ,CASE WHEN isnull(Form_url,'Ess_ApprisalHODApproval.aspx')<>'' THEN Form_url  ELSE 'Ess_ApprisalHODApproval.aspx' END Form_url
			FROM T0000_DEFAULT_FORM WITH (NOLOCK)
	...
	--SET @module_status = 0
	--SELECT @module_status =module_status
	--  FROM T0011_module_detail WHERE Cmp_id = @Cmp_ID and module_name ='Appraisal3'
	...
	--	BEGIN  
			...
			IF @msg  = 1
				BEGIN
					SELECT 'Start the ' + case when @KPI_AlertType =1 then 'Interim' else 'Final' end + ' Appraisal process' as response,
						CASE WHEN isnull(Form_url,'Ess_KPI_PMS_AppraisalForm.aspx')<>'' THEN Form_url  ELSE 'Ess_KPI_PMS_AppraisalForm.aspx' END Form_url
					FROM T0000_DEFAULT_FORM WITH (NOLOCK) 
			...
			If @KPIPMS_Status =1
				BEGIN
					SELECT 'Approve your KPI rating' as response,'Ess_KPI_PMS_AppraisalForm.aspx' as Url
				END
			ELSE
				BEGIN
					SELECT 0 as response
			...
			SELECT Form_url FROM T0000_DEFAULT_FORM WITH (NOLOCK) WHERE (Form_Name='ESS Employee Goal Setting' or Form_Name ='KPI Objectives') and Page_Flag='EP' and Is_Active_For_menu = 1
			...
			-----------------------------------------------------
			------------Apprisal_SupNotify-----------------------
			SELECT COUNT(KPIPMS_Status) AS cnt  
			...
			SELECT COUNT(EmpKPI_Id)EmpKPI_Id--,k.Emp_Id,k.Cmp_Id,Status,FinancialYr,emp_full_name,cat_name,dept_name,desig_name 
			...
			IF EXISTS(Select 1 from T0040_KPI_AlertSetting WITH (NOLOCK) where cmp_id=@cmp_Id and KPI_Type = 4)
				BEGIN
					SELECT isnull(KPI_Preference,0)KPI_Preference from T0040_KPI_AlertSetting WITH (NOLOCK) where cmp_id=@cmp_Id and KPI_Type = 4
				END
			ELSE
				BEGIN
			...
			DECLARE @today_date VARCHAR(12) =GETDATE();
			--SELECT CAST(@today_date AS DATETIME);
			IF EXISTS(SELECT *,Domain_Name  
					  FROM  T0052_HRMS_Posted_Recruitment WITH (NOLOCK) INNER JOIN
							   T0010_COMPANY_MASTER WITH (NOLOCK) on T0010_COMPANY_MASTER.Cmp_Id = T0052_HRMS_Posted_Recruitment.Cmp_id 
					  WHERE T0052_HRMS_Posted_Recruitment.cmp_id=@cmp_id and Posted_status = 1 and Publish_ToEmp = 1 and 
					        Publish_FromDate  <= CAST(@today_date AS DATETIME) and  publish_todate   >= CAST(@today_date AS DATETIME))						
				BEGIN					
					SELECT Form_Url FROM T0000_DEFAULT_FORM WITH (NOLOCK) where Form_Name = 'Current Opening Link'
					SELECT *,Domain_Name  
					FROM  T0052_HRMS_Posted_Recruitment WITH (NOLOCK) INNER JOIN
							   T0010_COMPANY_MASTER WITH (NOLOCK) on T0010_COMPANY_MASTER.Cmp_Id = T0052_HRMS_Posted_Recruitment.Cmp_id 
					WHERE T0052_HRMS_Posted_Recruitment.cmp_id=@cmp_id and Posted_status = 1 and Publish_ToEmp = 1 and 
			...
			-----------------------------------------------------
			-------------PendingRecruitment()--------------------
			SELECT COUNT(REsume_ID) AS Resume_ID 
			...
			---10/08/2017---employee kpa setting ------start-----
			IF EXISTS(SELECT KPA_InitiateId,Cmp_ID,Emp_Id,KPA_StartDate,KPA_EndDate 
					  FROM T0055_Hrms_Initiate_KPASetting WITH (NOLOCK)
					  WHERE Emp_Id = @emp_id AND   --KPA_StartDate <= CONVERT(varchar(10),GETDATE(),120) and
							 ( KPA_EndDate >= CONVERT(varchar(10),GETDATE(),120) AND Initiate_Status in(0,4,3) ))--OR Initiate_Status >=3
				BEGIN				
					...
					FROM  T0055_Hrms_Initiate_KPASetting WITH (NOLOCK) INNER JOIN
							(
								...
								FROM T0055_Hrms_Initiate_KPASetting WITH (NOLOCK)
								WHERE Emp_Id = @emp_id
								AND   KPA_EndDate >= CONVERT(VARCHAR(10),GETDATE(),120)
							)t ON t.initid = T0055_Hrms_Initiate_KPASetting.KPA_InitiateId
					WHERE Emp_Id = @emp_id AND   
						  KPA_EndDate >= CONVERT(VARCHAR(10),GETDATE(),120)  and t.show = 1 --and ( SA_Enddate >= CONVERT(varchar(10),GETDATE(),120) and SA_Status<>1) and SA_SendToRM <> 1
						
					IF(@KPA_Default=1)
						BEGIN					
							SELECT CASE WHEN isnull(Form_url,'Ess_EmployeeKPA_One.aspx')<>'' THEN Form_url +'?Initid=' + cast(@init_Id AS VARCHAR) ELSE 'Ess_EmployeeKPA_One.aspx' END Form_url
							FROM T0000_DEFAULT_FORM WITH (NOLOCK)
							WHERE Form_Name='TD_Home_ESS_355'	
						END
					ELSE
						BEGIN					
			...
			--	EXEC P0055_Send_KPASetting_Approval @cmp_id,@emp_id,@todate,''	
				...
				--SELECT @CTR_GH_ID=COUNT(1)
				--FROM V0055_Hrms_Initiate_KPASetting 
				--WHERE GH_Id=@EMP_ID AND Initiate_Status IN(5,8) AND Cmp_Id=@CMP_ID
				
				--SELECT 111,@CTR_R_EMP_ID,@CTR_HOD_ID
				...
				--FROM #FinTable_1 WHERE (Initiate_Status >= 2 and Initiate_Status<>4)
			
				--SELECT --COUNT(1)apprcnt,
				--DISTINCT KPA_InitiateId,
				--case when @KPA_Default=1 then 'Ess_EmployeeKPA_Approval_one.aspx' else 'Ess_EmployeeKPA_Approval.aspx' end as Form_Name				
				--,emp_id,app_type,Initiate_Status
				--FROM #FinTable_1 WHERE (Initiate_Status >= 2 and Initiate_Status<>4)
				--group by Emp_id,app_type
			
			
				
			--DROP TABLE #FinTable_1
			---10/08/2017---employee kpa setting ------end-------
END
