...
CREATE PROCEDURE [dbo].[P_GET_EMP_APPROVAL_RECORDS] 
...
SET ARITHABORT ON
    ...
    SELECT  APP.Emp_Tran_ID,APP.Emp_Application_ID,APP.Rpt_Level,APP.Branch_ID,APP.Cmp_ID,APP.Grd_ID,App.Emp_Full_Name,
			...
			(Case When APP.Approve_Status = 'P' 
				       Then 'Pending' 
				  When APP.Approve_Status = 'S' and App.Approved_Emp_ID=@Emp_ID
				       Then 'Submitted'
				  When APP.Approve_Status = 'S' and App.Approved_Emp_ID <> @Emp_ID
				       Then 'Pending' 
    ...
    WHERE	APP.Is_Final_Approval=0
			AND EXISTS(SELECT 1 FROM #tbl_Scheme S Where APP.Branch_ID=S.Branch_ID)


    IF EXISTS(SELECT 1 FROM #Status WHERE AprStatus='P')
		DELETE	EP 
		FROM	#tbl_EMP_MASTER_APP EP
				INNER JOIN #tbl_Scheme S ON EP.Branch_ID=S.Branch_ID 
		WHERE	NOT EXISTS(SELECT 1 FROM #tbl_EMP_MASTER_APP EP1 
							WHERE EP.Emp_Tran_ID=EP1.Emp_Tran_ID 
									AND (EP1.Rpt_Level = S.Rpt_Level AND EP1.ActualStatus IN ('A','S')	/*Previous Level Employee has Approved or Applied by Previous Level Employee Only*/
										OR	(EP1.Rpt_Level = (S.Rpt_Level + 1) AND EP1.ActualStatus IN ('P', 'S')) /*Previous Level Employee has Approved or Applied by Previous Level Employee & Accessed/Submitted/Reverted by Self*/
										)
							)
    ...
    IF EXISTS(SELECT 1 FROM #Status WHERE AprStatus='I')
		...
		WHERE	NOT EXISTS(SELECT 1 FROM #tbl_EMP_MASTER_APP EP1 
							WHERE EP.Emp_Tran_ID=EP1.Emp_Tran_ID 
									AND (
											(EP1.Rpt_Level = (S.Rpt_Level + 1) AND EP1.ActualStatus IN ('A')) /*Current Level Employee has Approved Application*/										
											OR (EP1.Rpt_Level = (S.Rpt_Level + 2) AND EP1.ActualStatus IN ('P')) /*Current Level Employee has Approved/Reverted Application & Next Level Employee has just accessed the Record (Not Approved/Submitted/Rejected/Reverted by Next Level Employee)*/										
											OR (EP1.Rpt_Level = 0 AND EP1.ActualStatus IN ('V') AND EP1.Approved_Emp_ID=@Emp_ID) /*Reverted by Current Level Employee*/										
										) 
							)
    IF EXISTS(SELECT 1 FROM #Status WHERE AprStatus='F')
		...
		WHERE	NOT EXISTS(SELECT 1 FROM #tbl_EMP_MASTER_APP EP1 
							WHERE EP.Emp_Tran_ID=EP1.Emp_Tran_ID 
									AND EP1.Approve_Status IN ('Final Approved', 'Rejected')
							)
    ...
    IF EXISTS(SELECT 1 FROM #Status WHERE AprStatus='F')
		BEGIN
			...
			WHERE	APP.Is_Final_Approval=1   
					 AND EXISTS(SELECT 1 FROM #tbl_Scheme S Where APP.Branch_ID=S.Branch_ID) and APP.Approve_Status  <>'P' --Added by ronakk 29072022
    ...
    WHERE	(APP.Rpt_Level in (0) OR (APP.Rpt_Level = 1 AND APP.Approve_Status='S') OR (APP.Rpt_Level = 2 AND APP.Approve_Status='P')) AND APP.Is_Final_Approval=0
		...
		BEGIN
			...
			FROM	T0060_EMP_MASTER_APP APP WITH (NOLOCK)   
					INNER JOIN (SELECT	MAX(Emp_Tran_ID)  As Emp_Tran_ID, Emp_Application_ID 
								FROM	T0060_EMP_MASTER_APP APP1 WITH (NOLOCK)
								GROUP BY APP1.Emp_Application_ID) AS APP1 ON APP.Emp_Tran_ID=APP1.Emp_Tran_ID AND APP.Emp_Application_ID=APP1.Emp_Application_ID													
			WHERE	APP.Is_Final_Approval=1  --and  APP.Approve_Status  <>'P'  --Added by ronakk 29072022
		END
	...
	SELECT  EP.* ,Case When EP.Approved_Emp_ID = 0 
				  Then 'Admin' 
