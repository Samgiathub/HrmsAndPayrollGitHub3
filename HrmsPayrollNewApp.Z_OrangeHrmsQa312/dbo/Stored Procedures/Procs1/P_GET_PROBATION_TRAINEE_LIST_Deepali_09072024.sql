...
create  PROCEDURE [dbo].[P_GET_PROBATION_TRAINEE_LIST_Deepali_09072024]
     @Cmp_ID numeric,
     @flag  varchar(10),
											...
											ELSE --for Days Completion Period	
												BEGIN
													...
													IF @Completed_Month > @Review_month
															update #PROBATION_TRAINEE_DETAILS set Probation_Date= @new_Probation_date where Emp_ID=@Emp_ID												
													ELSE												
															update #PROBATION_TRAINEE_DETAILS set Probation_Date=DATEADD(dd, - 1, DATEADD(DAY,@Completed_Month,@Date_Of_Join)),Review_Type='Final' where Emp_ID=@Emp_ID   --Deepali	--for Days period												
End
										END
									ELSE
										BEGIN
											--update #PROBATION_TRAINEE_DETAILS set Probation_Date=DATEADD(dd, - 1, DATEADD(mm,@Completed_Month,@Date_Of_Join)),Review_Type='Final' where Emp_ID=@Emp_ID											
											IF @Is_Month_Days=0											
												update #PROBATION_TRAINEE_DETAILS set Probation_Date=DATEADD(dd, - 1, DATEADD(mm,@Completed_Month,@Date_Of_Join)),Review_Type='Final' where Emp_ID=@Emp_ID	--for Month Period											
											ELSE
