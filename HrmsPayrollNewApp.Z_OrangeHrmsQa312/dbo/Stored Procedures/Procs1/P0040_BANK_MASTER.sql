...
CREATE PROCEDURE [dbo].[P0040_BANK_MASTER]    
...
--			End
			   --Add by Manisha on 03012025 end
 ...
 Else if @Tran_Type = 'U'     
  begin    
    ...
    If Exists(select Bank_ID From T0040_BANK_MASTER WITH (NOLOCK) Where cmp_ID = @Cmp_ID and Bank_ID <> @Bank_ID and    
         upper(Bank_Name) = upper(@Bank_Name) and upper(Bank_Branch_Name) = upper(@Bank_Branch_Name) )
		-- and  upper(Company_Branch) = upper(@Company_Branch) )    
     Begin    
      set @Bank_ID = 0    
      Return     
     end    
