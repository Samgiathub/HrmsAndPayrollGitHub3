 --exec Emp_Details_Form_A @Company_Id=119,@From_Date='2020-01-01 00:00:00',@To_Date='2020-01-31 00:00:00',@Branch_ID='',@Cat_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='18412#14938',@Report_Type='ESIC'                  
    ...
    -- exec Get_Rpt_Claim_Details_format4_for_briefcase @cmp_Id=203,@From_Date='2021-01-01 00:00:00',@To_Date='2021-12-01 00:00:00',@Branch_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='',@Cat_ID='',@Segment_ID='0',@Vertical='0',@SubVertical='0'              
...
CREATE PROCEDURE Get_Rpt_Claim_Details_format4_for_briefcase_Detail                          
   @Cmp_ID NUMERIC                                
  ,@From_Date  DATETIME                            
  ,@To_Date   DATETIME                            
  ,@Branch_ID  VARCHAR                   
...
IF @Branch_ID = 0                  
 ...
 IF @Constraint <> ''                            
   BEGIN                            
   select distinct EM.Emp_Full_Name,EM.Emp_ID,EM.Branch_ID,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,em.Emp_code,                
   em.Alpha_Emp_Code,cd.Claim_App_Amount,cd.Claim_DateOfPurchase,cd.Claim_ID,                
   cd.Claim_ActualPrice,Cd.Claim_App_Ttl_Amount          
   ,cd.Claim_Apr_Amount,          
   ...
   ,g.Grd_Name,[dbo].[F_GET_FINANCIAL_YEAR](cd.Claim_Apr_Date)as Financial_Yr    
  ...
  left join T0040_GRADE_MASTER G with(Nolock) on G.Grd_ID=EM.Grd_ID    
    ...
    where   cd.Claim_Status='A' and EM.Cmp_ID=@Cmp_ID  and cm.Claim_Report_Def_Id=3                      
   -- and cd.Claim_Apr_Date between    @From_Date and @To_Date and cm.Claim_Name like '%Briefcase%' --and cd.Claim_ID=89             
  ...
  ELSE            
   ...
   --added by mansi end 29-7-21                          
     select distinct EM.Emp_Full_Name,EM.Emp_ID,EM.Branch_ID,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,em.Emp_code,                
     em.Alpha_Emp_Code,  
  cd.Claim_App_Amount,cd.Claim_DateOfPurchase,cd.Claim_ID,                
  ...
  ,g.Grd_Name,[dbo].[F_GET_FINANCIAL_YEAR](cd.Claim_Apr_Date)as Financial_Yr    
   ...
   left join T0040_GRADE_MASTER G with(Nolock) on G.Grd_ID=EM.Grd_ID    
      ...
      EM.Cmp_ID=@Cmp_ID     and cm.Claim_Report_Def_Id=3           
     -- and cd.Claim_Apr_Date between    @From_Date and @To_Date  and  cm.Claim_Name like '%Briefcase%'   --- and cd.Claim_ID=89             
     --  group by Emp_Full_Name,em.Emp_ID,em.Branch_ID,em.Alpha_Emp_Code,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,em.Emp_code                      
     order by em.Alpha_Emp_Code                     
       END                            
