...
CREATE PROCEDURE [dbo].[P0040_CompanyWise_Branch_Table_1302]       
...
--ELSE IF UPPER(@Tran_Type) = 'U'
--BEGIN
--    -- Step 1: Delete existing records for the given Tran_Id, Bank_Id, Effective_Date, CMP_Id
--    DELETE FROM T0040_CompanyWise_Branch_Table
--    WHERE Tran_Id = @Tran_Id
--      AND CMP_Id = @CMP_Id
--      AND Bank_Id = @Bank_Id
--      --AND Effective_Date = @Effective_Date;
--      AND CONVERT(DATE, Effective_Date) = CONVERT(DATE, @Effective_Date); -- Ignore time part

--    -- Step 2: Split Branch_Id and Account_No and insert the new records
--    ; WITH BranchSplit AS (  
--            SELECT Value AS Branch_Id, RowNum FROM dbo.SplitString1(@Branch_Id, ',')  
--        ),  
--        AccountSplit AS (  
...
--    AND t.Account_No = a.Account_No  
--    AND t.Branch_Id = b.Branch_Id  
--	AND CONVERT(DATE, t.Effective_Date) = CONVERT(DATE, @Effective_Date)  -- Check if record already exists for the same Effective_Date
--   ); 
--END
ELSE IF UPPER(@Tran_Type) = 'U'
BEGIN
   --  Step 1: Delete existing records for the given Tran_Id, Bank_Id, Effective_Date, CMP_Id
    ...
    -- Step 2: Split Branch_Id and Account_No for insertion of new records if Branch_Id/Account_No changes
    ;WITH BranchSplit AS (  
            SELECT Value AS Branch_Id, RowNum FROM dbo.SplitString1(@Branch_Id, ',')  
        ),  
        AccountSplit AS (  
    ...
    -- Step 3: Insert new records only if they do not already exist
    INSERT INTO T0040_CompanyWise_Branch_Table (Bank_Id, CMP_Id, Account_No, Branch_Id, Effective_Date, System_Date)  
    SELECT DISTINCT @Bank_Id, @CMP_ID, a.Account_No, b.Branch_Id, @Effective_Date, @System_Date  
    FROM BranchSplit b  
    JOIN AccountSplit a ON b.RowNum = a.RowNum  
