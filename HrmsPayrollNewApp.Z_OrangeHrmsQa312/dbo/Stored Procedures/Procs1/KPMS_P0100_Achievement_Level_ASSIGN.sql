...
CREATE PROCEDURE [dbo].[KPMS_P0100_Achievement_Level_ASSIGN]                 
@rCmpId int,                
@GoalSheet_Id int,                      
@GS_Id int,                      
...
BEGIN                     
 ...
 if @rFlag = 2                
 begin                
  update KPMS_T0020_Goal_Allotment_Master_Test set IsLock =1 from @tblemp where Goal_Allot_ID = e_MainId               
 End              
 ...
 t_WeightageType int,t_LevelValues varchar(5000),t_LevelGrpValues varchar(5000),t_LevelAssignId int,t_TargetValues int,t_AchivementValues int            
 )                            
 INSERT INTO @tbltmp                      
 ...
 T.c.value('@TargetValues','INT') AS TargetValues,
  T.c.value('@AchivementValues','INT') AS AchivementValues
 ...
 (                    
  ...
  tt_WeightageType int,tt_LevelValues varchar(5000),tt_LevelGrpValues varchar(5000),tt_LevelAssignId int,tt_MainId int,tt_TargetValues int,tt_AchivementValues int                    
 )                    
 insert into @tblmain                    
 select MainId,t_GoalSettingId,t_SectionId,t_GoalId,t_SubGoalId,t_WeightageType,t_LevelValues,t_LevelGrpValues,t_LevelAssignId,e_MainId,t_TargetValues,t_AchivementValues from @tbltmp cross apply @tblemp                    
...
-- delete w from KPMS_T0100_Level_Assign w inner join @tblmain on Goal_Allotment_Id =tt_MainId              
 ...
 WHEN NOT MATCHED BY TARGET THEN                              
  INSERT                      
  (                      
   GoalSettingId,SectionId,GoalId,SubGoalId,WeightageType,LevelValues,LevlGrpValues,GoalSheet_Id,Goal_Allotment_Id,TargetValues
  )                      
  VALUES                      
  (                      
   tt_GoalSettingId,tt_SectionId,tt_GoalId,tt_SubGoalId,tt_WeightageType,tt_LevelValues,tt_LevelGrpValues,@GoalSheet_Id,tt_MainId,tt_TargetValues
  );                    
...
from KPMS_T0100_Level_Assign LA inner join @tblmain TM on LA.Goal_Allotment_Id = TM.tt_MainId where GoalSettingId = tt_GoalSettingId            
     and SectionId = tt_SectionId and GoalId = tt_GoalId and SubGoalId = tt_SubGoalId and GoalSheet_Id = @GoalSheet_Id                  
          
 delete w from tbl_LevelAssignValues w inner join @tblmain on la_LevelAssignId = tt_LevelAssignId            
 ...
 Insert into tbl_LevelAssignValues  (la_AllotmentId,la_LevelAssignId,la_SectionId,la_GoalId,la_SubGoalId,la_LevelId,la_LevelValue,la_LvlGrpId)                
  
 select @lMainId,@LevelAssignId,@Sectionid,@GoalId,@SubGoalId,val1,val2 , GrpGroup_Id FROM dbo.Split(@levelstr,',')     
 Inner Join    
...
--select top 1 * from tbl_LevelAssignValues          
