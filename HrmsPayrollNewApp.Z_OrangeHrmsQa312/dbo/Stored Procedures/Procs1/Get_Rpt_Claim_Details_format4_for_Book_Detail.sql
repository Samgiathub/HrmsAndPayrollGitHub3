 --exec Emp_Details_Form_A @Company_Id=119,@From_Date='2020-01-01 00:00:00',@To_Date='2020-01-31 00:00:00',@Branch_ID='',@Cat_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='18412#14938',@Report_Type='ESIC'                  
    ...
    -- exec Get_Rpt_Claim_Details_format4_for_Book_Detail @cmp_Id=203,@From_Date='2021-01-01 00:00:00',@To_Date='2021-12-31 00:00:00',@Branch_ID='',@Grade_ID='',@Type_ID='',@Dept_ID='',@Desig_ID='',@Emp_ID=0,@Constraint='',@Cat_ID='',@Segment_ID='0',@Vertical='0',@SubVertical='0'              
...
CREATE PROCEDURE Get_Rpt_Claim_Details_format4_for_Book_Detail                   
   @Cmp_ID NUMERIC                                
  ,@From_Date  DATETIME                            
  ,@To_Date   DATETIME                            
  ,@Branch_ID  VARCHAR                   
...
--   set @Salary_Cycle_id = null                
 ...
 IF @Constraint <> ''                      
  ...
  End                      
   ...
   BEGIN                            
     select distinct em.Cmp_ID,      
        cm.Claim_Name,EM.Emp_Full_Name,EM.Emp_ID,EM.Branch_ID,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,em.Emp_code,                
     em.Alpha_Emp_Code,cd.Claim_App_ID      
     ,cd.Claim_Apr_Dtl_ID,cd.Claim_NoofPerson,cd.Claim_Apr_Date     
  ,Bk.cb_BookName,bk.cb_Subject,bk.cb_ActualPrice,bk.cb_DiscountedPrice,bk.cb_Amount,    
  cd.Claim_PriceAfterDiscount,cd.Claim_App_Amount,      
     cd.Claim_DateOfPurchase,cd.Claim_ID,                
     cd.Curr_Rate,cd.Claim_Age,Cd.Claim_App_Ttl_Amount          
     ,cd.Claim_Apr_Amount,          
	 ...
	 ,[dbo].[F_GET_FINANCIAL_YEAR](cd.Claim_Apr_Date)as Financial_Yr ,g.Grd_Name
  ...
  left join T0110_Claim_BookDetails Bk with (NoLock) on CD.Emp_ID=bk.cb_EmpId and cd.Claim_App_ID=bk.cb_AppId    
     ...
     and cd.Claim_Apr_Date between    @From_Date and @To_Date and cm.Claim_Report_Def_Id=14  
  --and cm.Claim_Name like '%book%' --and cd.Claim_ID=89             
     order by em.Alpha_Emp_Code              
       
        END                        
 ELSE            
   BEGIN      
       select distinct em.Cmp_ID,cm.Claim_Name,EM.Emp_Full_Name,EM.Emp_ID,EM.Branch_ID,DS.Desig_Name,DM.Dept_Name,dm.Dept_Code,ds.Desig_Code,em.Emp_code,                
     em.Alpha_Emp_Code      
     ,cd.Claim_Apr_Dtl_ID,cd.Claim_NoofPerson,cd.Claim_Apr_Date     
  ,Bk.cb_BookName,bk.cb_Subject,bk.cb_ActualPrice,bk.cb_DiscountedPrice,bk.cb_Amount,    
  cd.Claim_PriceAfterDiscount,cd.Claim_App_Amount,      
