    ...
    -- EXEC GET_COMPOFF_DETAILS_ALL @For_Date='2023-12-31 00:00:00.000',@Cmp_ID=187,@Constraint='28201', @leave_ID=1141,@Exec_For=2     
---01/2/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---    
CREATE PROCEDURE [dbo].[GET_COMPOFF_DETAILS_ALL_Test_28-12-2023]     
 @For_Date datetime,    
 @Cmp_ID numeric(18,0),    
 ...
 *********************************************************************/    
 ...
 set @strLeave_CompOff_dates = ''    
  ...
  CREATE TABLE #LEAVE_COMPOFF_DATES_2    
 (    
	Leave_Tran_id NUMERIC,    
	Cmp_id NUMERIC,    
  EMP_ID NUMERIC,   
 ...
 Declare @Emp_id as integer
  set @emp_id= Convert(int,@Constraint)
   
 INSERT INTO #LEAVE_COMPOFF_DATES_2   
--  select @For_date,@cmp_id,@Emp_id,@leave_ID,0
  --return
  exec GET_COMPOFF_DETAILS @For_Date=@For_date,@Cmp_ID=@cmp_id,@Emp_ID=@Emp_id,@leave_ID=@leave_ID,@Leave_Application_ID=0
 

  
  insert into #LEAVE_COMPOFF_DATES
  select @Emp_id as EMP_ID , STUFF((SELECT '#' + CompOff_Dates +';'+cast(Compoff_Credit as varchar(10))  from #LEAVE_COMPOFF_DATES_2 FOR XML PATH('')), 1, 1, '') AS Leave_CompOff_Dates 
  
 delete from #LEAVE_COMPOFF_DATES where CompOff_Dates=''
