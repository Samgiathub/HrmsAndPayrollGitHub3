...
-- =============================================
CREATE PROCEDURE [dbo].[GetTemplate_EmpResponse_BackupDivyaraj01012024]
	@T_ID		numeric(18,0)
	,@Cmp_Id		numeric(18,0)
	,@Emp_Id		numeric(18,0)
...
BEGIN
	...
	SR.ETR_Id,SR.Response_Flag,SR.T_Id,SR.Cmp_Id,
	TF.F_ID,TF.Field_Name,TF.Field_Type,TF.Sorting_No
	INTO #FINAL_EMP
	FROM T0080_EMP_MASTER EM WITH (NOLOCK)
	INNER JOIN T0100_Employee_Template_Response SR WITH (NOLOCK) ON SR.Emp_Id=EM.Emp_ID 
	INNER JOIN V0080_EMP_MASTER_INCREMENT_GET I ON I.Emp_ID = EM.Emp_ID 
	INNER JOIN T0050_Template_Field_Master TF WITH (NOLOCK) ON TF.F_ID = SR.F_Id
	WHERE  SR.T_Id = @T_ID AND (EM.Alpha_Emp_Code=ISNULL(@emp_code,EM.Alpha_Emp_Code) or EM.Emp_First_Name like ISNULL(@emp_code,EM.Emp_First_Name))	
	and ISNULL(I.Branch_ID,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Branch_ID,ISNULL(I.Branch_ID,0)),',') ) 
	AND ISNULL(I.Dept_ID,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Dept_ID,ISNULL(I.Dept_ID,0)),',') ) 
	AND ISNULL(I.Desig_ID,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Desig_ID,ISNULL(I.Desig_ID,0)),',') ) 
	order by SR.ETR_Id

	--SR.Response_Flag,TF.Sorting_No
	
	
	--insert into #tempTable
	--(ETR_Id,T_ID,Field_Name,Field_Type,Sorting_No,F_ID,Is_Required,Reponse_Flag)	
	--(Select distinct TM.T_ID,TM.Field_Name,TM.Field_Type,TM.Sorting_No,TM.F_ID,TM.Is_Required,TR.Response_Flag
	--from T0100_Employee_Template_Response TR with (NOLOCK)
	--LEFT OUTER JOIN T0050_Template_Field_Master TM WITH (NOLOCK) on TM.T_ID = TR.T_Id
	--where TR.Cmp_Id=@Cmp_Id and TR.T_Id=@T_ID) 
	--order by Response_Flag

	--select * from #tempTable

	--update #tempTable
	--set ETR_Id = T.ETR_Id
	--from (Select Tr.ETR_Id
	--	  from T0100_Employee_Template_Response TR with (NOLOCK)			
	--	  where TR.Cmp_Id=@Cmp_Id and TR.T_Id=@T_ID) T
	
	----select * from #FINAL_EMP
	--select * from #tempTable
	--return	

	--SELECT distinct F_ID FROM #FINAL_EMP
	--return

	DECLARE cur CURSOR
	FOR 
	   SELECT F_ID FROM #FINAL_EMP --where SurveyQuestion_Id=76
		open cur
			...
			WHILE @@FETCH_STATUS = 0
			  ...
			  -- select @col1,@Survey_Id,@Emp_Id	
					...
					UPDATE #FINAL_EMP
					SET Emp_Id = S.Emp_Id ,Answer=S.Answer,Created_Date=S.Created_Date,Response_Flag = S.Response_Flag
					FROM (Select TR.Emp_Id,REPLACE(TR.Answer,'~~','#')Answer,TR.Created_Date,TR.Response_Flag 
					from T0100_Employee_Template_Response TR WITH (NOLOCK)
					INNER JOIN #FINAL_EMP EM ON EM.EMP_ID=TR.EMP_ID 
					where TR.Emp_Id=@Emp_Id and TR.T_Id=@T_ID and TR.F_Id = @col1) S 
	...
	--UPDATE #tempTable
	--SET Emp_Id = S.Emp_Id ,Answer=S.Answer,Created_Date=S.Created_Date,Reponse_Flag = S.Response_Flag
	--FROM (Select TR.Emp_Id,REPLACE(TR.Answer,'~~','#')Answer,TR.Created_Date,TR.Response_Flag 
	--from T0100_Employee_Template_Response TR WITH (NOLOCK)
	--INNER JOIN #FINAL_EMP EM ON EM.EMP_ID=TR.EMP_ID where TR.Emp_Id=@Emp_Id and TR.T_Id=@T_ID and TR.F_Id = EM.F_Id ) S 
	--where T_ID = @T_ID 
	...
	--isnull(Answer,'') as Answer,Created_Date,Is_Required,Reponse_Flag
	--from #tempTable order by Sorting_No
	--drop table #tempTable

	select ROW_NUMBER() OVER (ORDER BY ETR_Id)as SrNo,* from #FINAL_EMP
	drop table #FINAL_EMP
END
