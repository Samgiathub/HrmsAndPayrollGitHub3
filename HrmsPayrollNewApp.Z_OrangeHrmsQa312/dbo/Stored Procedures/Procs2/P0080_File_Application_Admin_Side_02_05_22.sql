...
CREATE PROCEDURE [dbo].[P0080_File_Application_Admin_Side_02_05_22]  
...
-- Add (start)
 ...
 If @tran_type  = 'I'  
  ...
  if exists (Select File_App_Id   from T0080_File_Application WITH (NOLOCK) Where File_App_Id = @File_App_Id )   
    ...
    INSERT INTO T0080_File_Application (Application_Date,Branch_Id,Dept_Id,Desig_Id,Emp_Id,S_Emp_Id,File_Number,
	--F_StatusId,
	F_TypeId,Subject,Description,Process_Date,File_App_Doc,Cmp_ID,CreatedDate,[User ID])
			 ...
			 Select @File_App_Id = ISNULL(MAX(File_App_Id),0) From T0080_File_Application WITH (NOLOCK)
					...
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,1,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@FileName,0,getDate(),@LoginID,@tran_type,0,'T0080_File_Application',@RComment)	
					--for History end
				-- Add for audit(start)
		...
		if exists	(	select File_App_Id 
			...
			begin 
					 ...
					 where File_App_Id = @File_App_Id  
						...
						INSERT INTO dbo.T0115_File_Level_Approval_History
						   (Cmp_Id,File_App_Id,File_Apr_Id,Emp_Id,H_File_Number,H_F_StatusId,H_F_TypeId,H_Subject,H_Description,H_S_Emp_Id,H_Process_Date,H_File_App_Doc,Rpt_Level,CreatedDate,[User ID],H_Trans_Type,H_Tran_Id,Tbl_Type,H_Approval_Comments)
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,@File_Status,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@file_nm,0,getDate(),@LoginID,'U',0,'T0080_File_Application',@RComment)	
					--for History end
					end
	else
	begin
		if exists	(	select File_App_Id 
									from T0115_File_Level_Approval WITH (NOLOCK)
									where Cmp_ID=@Cmp_ID and File_App_Id=@File_App_Id and Emp_ID=@Emp_ID and F_StatusId<>5 
								)
					begin
						RAISERROR (N'File Approval - Reference Exist.', 16, 2); 
						RETURN
			...
			--End
			 ...
			 --set @file_F=cast(FORMAT(GETDATE(), 'dd MM yyyy HH mm ss')as varchar)+'-'+cast(@File_App_Id as varchar)+'-'+cast(@cmp_id as varchar)+'-'+'FP'+'-'+'#'+cast(@fileName as varchar)
					 ...
					 where File_App_Id = @File_App_Id  
						...
						set @File_Status=(select F_StatusId from T0080_File_Application  where File_App_Id = @File_App_Id ) 
						INSERT INTO dbo.T0115_File_Level_Approval_History
						   (Cmp_Id,File_App_Id,File_Apr_Id,Emp_Id,H_File_Number,H_F_StatusId,H_F_TypeId,H_Subject,H_Description,H_S_Emp_Id,H_Process_Date,H_File_App_Doc,Rpt_Level,CreatedDate,[User ID],H_Trans_Type,H_Tran_Id,Tbl_Type,H_Approval_Comments,H_Review_Emp_Id,H_Reviewed_by_Emp_Id)
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,@File_Status,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@file_nm,0,getDate(),@LoginID,'U',0,'T0080_File_Application',@RComment,@review_emp,@Review_by_emp)	
					--for History end
					end
