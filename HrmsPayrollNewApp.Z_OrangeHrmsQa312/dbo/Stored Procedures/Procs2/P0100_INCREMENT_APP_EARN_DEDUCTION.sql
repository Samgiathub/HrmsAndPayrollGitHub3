...
-- Description:	<Description,,Increment Application - Allowance >
---21/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
-- =============================================


CREATE PROCEDURE [dbo].[P0100_INCREMENT_APP_EARN_DEDUCTION]
...
SET ARITHABORT ON
		...
		FROM	T0100_INCREMENT_APPLICATION WITH (NOLOCK) WHERE App_ID =@App_ID
		...
		IF @E_AD_PERCENTAGE >  0
			BEGIN				
				SELECT @AD_Other_Amount = ISNULL(SUM(E_AD_Amount),0) FROM t0100_INCREMENT_APP_EARN_DEDUCTION EED WITH (NOLOCK) inner join 
					T0050_ad_master AM WITH (NOLOCK) on eed.ad_id = am.ad_id WHERE App_ID =@App_ID  
				...
				SELECT @AD_Other_Amount_esic = ISNULL(SUM(E_AD_Amount),0) FROM t0100_INCREMENT_APP_EARN_DEDUCTION EED WITH (NOLOCK) inner join 
					T0050_ad_master AM WITH (NOLOCK) on eed.ad_id = am.ad_id WHERE App_ID = @App_ID and am.AD_CALCULATE_ON <> 'Import'   
						AND eed.AD_ID IN (SELECT AD_ID FROM T0060_EFFECT_AD_MASTER WITH (NOLOCK) WHERE Effect_AD_ID =@AD_ID)
			...
			--	Begin
			--		--Select @Temp_Amount = Isnull(SUM(E_AD_AMOUNT),0)FROM t0100_INCREMENT_APP_EARN_DEDUCTION E Inner Join 
			...
			--				,eed.AD_ID
			--			FROM t0100_INCREMENT_APP_EARN_DEDUCTION EED Inner Join 
			...
			--				 EXISTS(SELECT 1 From T0100_INCREMENT_APPLICATION where App_ID = @App_ID And Increment_Type <> 'Transfer')  --Ankit 11122015--
			--			BEGIN
			...
			--	Begin
			--		--Select @Temp_Amount = Isnull(SUM(E_AD_AMOUNT),0)FROM t0100_INCREMENT_APP_EARN_DEDUCTION WHERE cmp_ID = @Cmp_ID AND Emp_ID = @Emp_ID 
			...
			--				DELETE FROM t0100_INCREMENT_APP_EARN_DEDUCTION WHERE INCREMENT_ID = @App_ID
	...
	IF @tran_type  = 'I' 
			BEGIN		
				...
				IF EXISTS(SELECT AD_TRAN_ID FROM t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK) WHERE cmp_ID = @Cmp_ID AND Emp_ID = @Emp_ID AND App_ID=@App_ID AND AD_ID=@AD_ID)					
					BEGIN	    
						SET @AD_ID = 0				
						RETURN
					END
				
				SELECT  @AD_TRAN_ID = ISNULL(MAX(AD_TRAN_ID),0) + 1 FROM t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK)
				...
				INSERT INTO t0100_INCREMENT_APP_EARN_DEDUCTION
				...
				VALUES (@AD_TRAN_ID,@EMP_ID,@CMP_ID,@AD_ID,@App_ID,@FOR_DATE,@E_AD_FLAG,@E_AD_MODE,@E_AD_PERCENTAGE,@E_AD_AMOUNT,@E_AD_MAX_LIMIT,@E_AD_AMOUNT_YEARLY)
				
				
				if Exists(Select 1 From dbo.T0100_INCREMENT_APPLICATION WITH (NOLOCK) Where Emp_ID = @Emp_ID and Increment_effective_Date > @for_Date and (Increment_Type = 'Transfer' or  Increment_Type = 'Deputation') and App_ID <> @App_ID)
					begin
						...
						Select App_ID,Increment_Type From dbo.T0100_INCREMENT_APPLICATION WITH (NOLOCK) Where Emp_ID = @Emp_ID and Increment_effective_Date > @FOR_DATE 
						...
						Fetch next from cur_inc into @Inc_id_update,@inc_type_update
							...
							Begin 									
									...
									-- Added by rohit for increment update then enter duplicate allowance on 05-mar-2014
									 if EXISTS(SELECT AD_TRAN_ID FROM t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK) WHERE cmp_ID = @Cmp_ID AND Emp_ID = @Emp_ID AND App_ID=@Inc_id_update AND AD_ID=@AD_ID)					
										begin
										select @AD_TRAN_ID = AD_TRAN_ID FROM t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK) WHERE cmp_ID = @Cmp_ID AND Emp_ID = @Emp_ID AND App_ID=@Inc_id_update AND AD_ID=@AD_ID
										
										UPDATE    t0100_INCREMENT_APP_EARN_DEDUCTION
											...
											Where AD_TRAN_ID = @AD_TRAN_ID and EMP_ID = @EMP_ID and CMP_ID = @CMP_ID and App_ID = @Inc_id_update and AD_ID = @AD_ID
										end
	...
	ELSE IF @Tran_Type = 'U' 
		BEGIN
				...
				IF EXISTS(SELECT 1 FROM t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK) WHERE cmp_ID = @Cmp_ID AND Emp_ID = @Emp_ID AND App_ID=@App_ID AND AD_ID=@AD_ID)					
					BEGIN	    
						SET @AD_ID = 0
						RETURN
					END
			...
			SET       FOR_DATE=@FOR_DATE,
					  ...
					  E_AD_MAX_LIMIT=@E_AD_MAX_LIMIT,
					  E_AD_YEARLY_AMOUNT = @E_AD_AMOUNT_YEARLY
            Where AD_TRAN_ID = @AD_TRAN_ID and EMP_ID = @EMP_ID and CMP_ID = @CMP_ID and App_ID = @App_ID and AD_ID = @AD_ID
	...
	Else if @Tran_Type = 'D' 
		begin
				...
				Delete From t0100_INCREMENT_APP_EARN_DEDUCTION Where AD_TRAN_ID = @AD_TRAN_ID and Cmp_Id=@Cmp_ID
	...
	----------- PT update -------------------------------------------------------
		if @Emp_PT = 1
			begin
				Select @AD_Other_Amount = isnull(sum(E_AD_Amount),0) from t0100_INCREMENT_APP_EARN_DEDUCTION WITH (NOLOCK) where App_ID=@App_ID and E_AD_Flag ='I'
		...
		Update T0100_INCREMENT_APPLICATION set Emp_PT_Amount = @PT_Amount where App_ID = @App_ID 
	...
	*/
		...
		IF (@AUTO_CALCULATE_CTC > 0)									
			BEGIN	
				...
				FROM T0100_INCREMENT_APPLICATION WITH (NOLOCK) WHERE App_ID = @App_ID AND Increment_Type <> 'Joining'
				...
				IF @Increment_Amount_CTC = 0.00
					BEGIN
						...
						FROM	t0100_INCREMENT_APP_EARN_DEDUCTION EED WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AM WITH (NOLOCK) ON EED.AD_ID = AM.AD_ID
						WHERE	App_ID=@App_ID and E_AD_Flag ='I' AND AM.AD_PART_OF_CTC = 1
						
						SET @CTC_Allow = @Basic_Salary + ISNULL(@AD_Other_Amount,0)
						
						UPDATE T0100_INCREMENT_APPLICATION SET CTC = @CTC_Allow WHERE App_ID = @App_ID 
