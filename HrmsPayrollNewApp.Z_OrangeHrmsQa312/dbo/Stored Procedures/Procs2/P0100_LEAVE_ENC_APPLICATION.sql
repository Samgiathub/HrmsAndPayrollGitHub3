...
CREATE PROCEDURE [dbo].[P0100_LEAVE_ENC_APPLICATION]
...
--SET @TO_DATE='31-DEC-' + CAST(YEAR(@LV_ENCASH_APP_DATE) AS VARCHAR(5))
	...
	-- Changed and Added by Divyaraj Kiri on 11/01/2024

	Set @Year = YEAR(GETDATE())
	if (@setting_Value=0 or @setting_Value=1)
		Begin			
			Set @date = '01-Jan-'+ convert(varchar(5),@Year)
			SET @START_YEAR = DATEADD(YEAR, DATEDIFF(YEAR, 0, @date), 0)
			SET @END_YEAR = DATEADD(SECOND, -1, DATEADD(YEAR, 1, @START_YEAR));			
		End
	Else
		Begin
			IF MONTH(GETDATE()) > 3
			...
			SET @START_YEAR = DATEFROMPARTS(YEAR(@date) - 1, 4, 1)
			SET @END_YEAR = DATEFROMPARTS(YEAR(@date), 3, 31)			
		End

	-- Ended by Divyaraj Kiri on 11/01/2024
...
If @Max_Leave_Encash > 0
	begin
		If @Max_Leave_Encash < @Lv_Encash_App_Days
			...
			Select @Total_Application = count(LEAP.Lv_Encash_App_ID) 
			from T0100_LEAVE_ENCASH_APPLICATION LEAP WITH (NOLOCK)
			LEFT JOIN T0120_LEAVE_ENCASH_APPROVAL LEA WITH (NOLOCK) on LEAP.Lv_Encash_App_ID = LEA.Lv_Encash_App_ID
			where LEAP.Cmp_ID=@Cmp_ID and LEAP.Emp_ID=@Emp_ID and LEAP.Leave_ID=@leave_ID 
			and (LEA.Lv_Encash_Apr_Status='A' OR LEAP.Lv_Encash_App_Status='P')
			...
			and (LEA.Lv_Encash_Apr_Date BETWEEN @START_YEAR AND @END_YEAR OR LEAP.Lv_Encash_App_Date BETWEEN @START_YEAR AND @END_YEAR)
			--and NOT EXISTS (SELECT 1 FROM T0120_LEAVE_ENCASH_APPROVAL WHERE Lv_Encash_App_ID = LEAP.Lv_Encash_App_ID)
...
begin			
		...
		If @Max_No_Of_Application = 0 or (@Max_No_Of_Application > ISNULL(@Total_Application,0) and @Max_No_Of_Application > 0)  --Change by Jaina 10-01-2018	-- Change by Divyaraj Kiri on 11/01/2024
			Begin 			
		...
		Else 
			Begin
				set @Lv_Encash_App_ID = 0
				set @ErrorMsg = '@@ Max Application Count Exceed @@'
			    RaisError(@ErrorMsg,16,2)				
				return					
			End
