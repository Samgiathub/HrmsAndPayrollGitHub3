...
CREATE PROCEDURE [dbo].[P0050_CALCULATE_CANTEEN_DEDUCTION] 
	@CMP_ID NUMERIC(18,0),
	@EMP_ID NUMERIC(18,0),
	@FROM_DATE DATETIME,
	@TO_DATE DATETIME,
...
BEGIN
   ...
   Where I.Emp_ID = @Emp_ID   and  E.Cmp_ID=@Cmp_ID 
	...
	--INTO	#CANTEEN
	--FROM	T0050_CANTEEN_MASTER M WITH (NOLOCK) 
	--INNER JOIN T0050_CANTEEN_DETAIL D WITH (NOLOCK) ON M.Cmp_Id=D.Cmp_Id AND M.Cnt_Id=D.Cnt_Id			
	--WHERE	D.Effective_Date <= @To_Date and grd_id =@grade 
		
    --Added by ronakk 31012023
	SELECT	@Emp_ID As Emp_ID,M.Cmp_Id,M.Cnt_ID,CAST(From_Time As DateTime) As From_Time, Cast(To_Time As DateTime) As To_Time,
	D.Effective_Date,Amount,D.grd_id,Ip_id
	INTO	#CANTEEN
	FROM	T0050_CANTEEN_MASTER M 
	INNER JOIN T0050_CANTEEN_DETAIL D ON M.Cmp_Id=D.Cmp_Id AND M.Cnt_Id=D.Cnt_Id	
	inner Join (SELECT Max(Effective_Date) AS Effective_Date,grd_id, Cnt_Id 
				FROM   T0050_CANTEEN_DETAIL 
				WHERE  Effective_Date <=@To_Date AND cmp_id = @Cmp_ID and grd_id =@grade 
				GROUP  BY Cnt_Id,grd_id 
				) DM on  DM.Effective_Date = D.Effective_Date and DM.Cnt_Id=D.Cnt_Id	--Added by ronakk 31012023		
	WHERE	D.Effective_Date <= @To_Date and D.grd_id =@grade 
