...
CREATE PROCEDURE  [dbo].[P0060_HRMS_CandidateFinalization_Get]
	 @Cmp_ID  Numeric (18,0) 
			...
			--IF @status=0
				begin				
					set @Query = 'SELECT c.[Resume_ID]
										  ...
										  ,isnull(c.Permanent_Street,'''') as Permanent_Street
										  ,isnull(c.Permanent_City,'''') as Permanent_City
										  ,isnull(c.Permanent_State,'''') as Permanent_State
										  ,isnull(c.Permanentt_Post_Box,'''') as Permanentt_Post_Box
										  ,isnull(c.Home_Tel_no,'''') as Home_Tel_no
										  ,isnull(r.primary_email,'''') as primary_email 
										  ,r.is_Physical
										  ,isnull(r.Aadhar_CardNo,'''') as Aadhar_CardNo
										  ,isnull(c.IFSC_Code,'''') as IFSC_Code
										  ,c.[dept_id]
							    ...
							    where Level2_Approval = 1 and c.Cmp_id=' + cast(@Cmp_ID as varchar(50))	
							    + ' and ( ' + CAST(@status AS VARCHAR(10)) + ' = 0 OR FinalStatus='+cast((@status) as varchar(10)) + ' )' 																 				
