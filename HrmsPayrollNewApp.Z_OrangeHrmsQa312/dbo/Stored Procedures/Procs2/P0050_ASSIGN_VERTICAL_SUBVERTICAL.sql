...
CREATE PROCEDURE [dbo].[P0050_ASSIGN_VERTICAL_SUBVERTICAL]  
  
   @TRAN_ID INT OUTPUT  
  ,@CMP_ID INT  
  ,@EMP_ID NUMERIC  
  ,@EFFECTIVE_DATE DATETIME  
  ,@VERTICAL_ID VARCHAR(500)  
...
SET ARITHABORT ON  
 ...
 IF @TRANS_TYPE ='I'   
  ...
  -- code updated start'08-06-2021'
   if exists(select emp_id from T0050_ASSIGN_VERTICALSUBVERTICAL WITH (NOLOCK) where Emp_ID=@EMP_ID and Cmp_ID=@CMP_ID and Effective_Date = @EFFECTIVE_DATE)
      begin 
	  set @TRAN_ID = -1  
	  return 
	  end 
 ...
 ELSE IF @TRANS_TYPE ='U'   
	...
	---old code started
     ...
     ---old code ended
	  ...
	  update T0050_ASSIGN_VERTICALSUBVERTICAL set CMP_ID=@CMP_ID,
	   Vertical_ID=@VERTICAL_ID,User_ID=@LOGIN_ID WHERE Emp_ID = @EMP_ID  and Tran_ID=@TRAN_ID and EFFECTIVE_DATE=@EFFECTIVE_DATE
       -- code updated end '08-06-2021'
 ...
 ELSE IF @TRANS_TYPE ='D'  
   begin  
    ...
    DELETE  FROM dbo.T0050_ASSIGN_VERTICALSUBVERTICAL WHERE TRAN_ID = @TRAN_ID    
 ...
 ELSE IF @TRANS_TYPE='E'  
   BEGIN  
    ...
    SELECT TRAN_ID,CMP_ID,EMP_ID,EFFECTIVE_DATE,VERTICAL_ID  
