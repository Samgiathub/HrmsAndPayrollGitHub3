...
Create PROCEDURE [dbo].[P0100_EMP_EARN_DEDUCTION_Backup_19112024]
...
AS
	...
	SET ARITHABORT ON
		...
		IF @E_AD_PERCENTAGE >  0
			...
			--SELECT 1 FROM T0100_EMP_EARN_DEDUCTION EED WITH (NOLOCK) INNER JOIN T0050_AD_MASTER AD WITH (NOLOCK) ON EED.AD_ID = AD.AD_ID
			--							WHERE EED.CMP_ID = @Cmp_ID AND Emp_ID = @Emp_ID And Increment_Id = @Increment_Id AND (AD_Calculate_On = 'Arrears')
