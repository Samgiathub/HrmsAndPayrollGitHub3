CREATE PROCEDURE [dbo].[P0060_EMP_MASTER_APP]  
  @Emp_Tran_ID bigint output  
 ,@Emp_Application_ID int output  
 ,@Emp_ID  Int  
 ,@Cmp_ID  Int  
 ,@Branch_ID  Int  
 ,@Cat_ID  Int  
 ...
 ,@Center_ID Int --'Alpesh 23-Sep-2011  
 ...
 ,@Rejoin_Emp_Id int = 0 --Added by Jaina 03-03-2018  
 ...
 ,@Signature_Image_Name varchar(100)='' -- added deepal 22/10/2020  
 ,@Band_ID numeric(18,0)     --mansi  
  ,@Is_PMGKY TINYINT = 0  
  ,@Is_PFMem TINYINT = 0  
...
AS   
 ...
 SELECT * INTO #T0060_EMP_MASTER_APP_DELETED  FROM T0060_EMP_MASTER_APP WHERE Emp_Tran_ID=@Emp_Tran_ID   
 --Emp_ID=@Emp_ID  
 ...
 IF @PF_Start_Date = ''   --Added By Jaina 02-09-2016  
  set @PF_Start_Date = NULL  
   declare @is_for_mobile_Access as tinyint  
   set  @is_for_mobile_Access=0  
 ...
 Declare @IS_OT As Int  
 Declare @IS_PT As Int  
 Declare @IS_PF As Int  
 Declare @IS_LATE_MARK As Int  
 ...
 If @Emp_Code =0   
  ...
  ----added binal temprory 14-march-2019 below binal for temp due to above sp commented  
  --SELECT @Get_Emp_code = ISNULL(MAX(Emp_Tran_ID),0) + 1 FROM T0060_EMP_MASTER_APP   
  ----set @Get_Emp_code=1 --added binal temprory 14-march-2019  
 ...
 BEGIN        
   if exists ( SELECT 1 from T0060_EMP_MASTER_APP WITH (NOLOCK) where Rejoin_Emp_Id =@Rejoin_Emp_Id and Cmp_ID=@Cmp_id)  
   BEGIN  
   ...
   @Leave_Encash_working_Day = Leave_Encash_Working_Days,  
      ...
      @Rpt_Level=Rpt_Level,  
      @Emp_Application_ID=Emp_Application_ID,  
      @Emp_Tran_ID=Emp_Tran_ID,  
      @Approve_Status=Approve_Status,  
      @Ref_Emp_Tran_ID=Ref_Emp_Tran_ID,  
  ...
  FROM T0060_EMP_MASTER_APP WITH (NOLOCK) WHERE CMP_ID = @CMP_ID   
  and Emp_Tran_ID=Emp_Tran_ID   
  --AND EMP_ID= @REJOIN_EMP_ID  
 ...
 FROM T0040_GENERAL_SETTING WITH (NOLOCK) WHERE Cmp_ID=@Cmp_ID and Branch_ID=@Branch_ID   
  and For_Date = (select max(for_date) From T0040_General_Setting WITH (NOLOCK) WHERE Cmp_ID = @Cmp_ID and Branch_ID =@branch_id)    --Modified by Ramiz on 15092014  
 ...
 if @EmpName_Alias_Salary = ''  
  ...
  IF @Emp_Notice_Period = 0  
   BEGIN  
     ...
     select @Is_Short_Fall_Grade_wise = Is_Shortfall_Gradewise , @short_Fall_days = Short_Fall_Days   
     from T0040_GENERAL_SETTING WITH (NOLOCK) where Branch_ID = @Branch_ID and Cmp_ID = @Cmp_ID   
     and For_Date = (select max(for_date) From T0040_General_Setting WITH (NOLOCK) where Cmp_ID = @Cmp_ID and Branch_ID =@branch_id)   -- Modified by Ramiz on 15092014  
 ...
 If @tran_type  = 'I'  
  Begin  
   ...
   and For_Date = (select max(for_date) From T0040_General_Setting WITH (NOLOCK)  
    where Cmp_ID = @Cmp_ID and Branch_ID =@branch_id)    --Modified by Ramiz on 15092014  
    PRINT @Alpha_Emp_Code  
   IF @Is_GroupOFCmp = 0   
    Begin  
     If Exists(select Emp_ID From dbo.T0060_EMP_MASTER_APP WITH (NOLOCK) WHERE   
     Cmp_ID= @Cmp_ID  and Alpha_Emp_Code = @Alpha_Emp_Code)  
      begin     
   ...
   Else If @Is_GroupOFCmp = 1  
    ...
    Begin  
     ...
     If Exists(select Emp_ID From dbo.T0060_EMP_MASTER_APP WITH (NOLOCK)  
     ...
     In (Select Cmp_ID From T0010_COMPANY_MASTER WITH (NOLOCK) Where is_GroupOFCmp = 1))  
      ...
      BEGIN    
       ...
       FROM T0060_EMP_MASTER_APP E WITH (NOLOCK)  
      ...
      END  
     ELSE If Exists(select Emp_ID From dbo.T0060_EMP_MASTER_APP WITH (NOLOCK)  
     WHERE (Alpha_Emp_Code = @Alpha_Emp_Code OR Emp_code =@Emp_code)  
       And Cmp_ID = @Cmp_ID)  
   ...
   --ADDED BY MUKTI(01082020)START   
    DECLARE @AGE INT  
    DECLARE @MaxAgeLimit INT  
    --SET @AGE = dbo.F_GET_AGE (@Date_Of_Birth,GETDATE(),'N','N')  
    SET @AGE = dbo.F_GET_AGE_For_Employee_Master_Application (@Date_Of_Birth,GETDATE(),'N','N')  
    ...
    BEGIN  
     ...
     RETURN  
    END  
   --ADDED BY MUKTI(01082020)END  
      
   Declare @Backdate_Allowed Numeric  
   Select @Backdate_Allowed = Setting_Value From T0040_SETTING WITH (NOLOCK)  
   Where Cmp_ID = @Cmp_ID and Setting_Name='Allowed Backdated Joining upto Days'  
  
     
   if @Backdate_Allowed > 0  
    Begin  
     --Added By Nilesh Patel For Kataria Client After Discuss With Ankur Sir   
     if Not Exists(SELECT 1 FROM T0011_LOGIN WITH (NOLOCK)  
      Where Cmp_ID = @Cmp_ID and Login_ID = @User_Id and Login_Name LIKE 'admin@%')  
     BEGIN  
    ...
    SELECT @Count =Count(Emp_Tran_ID) from dbo.T0060_EMP_MASTER_APP WITH (NOLOCK) where Emp_Left<>'y'          
    ...
    IF EXISTS(Select 1 From T0040_SETTING WITH (NOLOCK) WHERE CMP_ID=@Cmp_ID  AND Setting_Name='Auto Generate Employee PF Number' AND Setting_Value=1)  
     BEGIN  
      IF EXISTS(SELECT 1 FROM T0060_EMP_MASTER_APP WITH (NOLOCK) WHERE SSN_No=@SSN_No AND CMP_ID=@CMP_ID AND IsNull(@SSN_No,'') <> '')  
       BEGIN      
        EXEC dbo.P_GET_NEXT_EMP_PF_NO @Cmp_ID, @SSN_No Output  
       END  
     END  
      
    IF @Comp_offlimit IS NULL  
     SET @Comp_offlimit = '00:00'  
        
    Declare @Is_Camera_enable tinyint  
    Declare @Is_Geofence_enable tinyint  
    set @Is_Camera_enable=0  
    set @Is_Geofence_enable=0  
    
    Set @Emp_Application_ID =@Emp_Application_ID  
          
      
    IF (@Emp_Application_ID =0)  
    BEGIN  
    IF EXISTS(SELECT 1 FROM T0060_EMP_MASTER_APP WITH (NOLOCK))  
     BEGIN  
       Set @Emp_Application_ID=(SELECT isnull(Max(Emp_Application_ID),0) FROM T0060_EMP_MASTER_APP WITH (NOLOCK)  
                       where Emp_Tran_ID=@Emp_Tran_ID)  
      --Added below if 11-apr-2019 due to new transaction id become 0  
        IF @Emp_Application_ID=0  
        BEGIN  
          Set @Emp_Application_ID=(SELECT isnull(Max(Emp_Application_ID),0) FROM T0060_EMP_MASTER_APP WITH (NOLOCK)) + 1  
          
        END           
     END  
     Else   
     Begin  
     --Set @Emp_Application_ID  is zero   
      Set @Emp_Application_ID =1  
     End  
    End  
     ...
     BEGIN  
      ...
      Set @Emp_Tran_ID=(SELECT isnull(Max(Emp_Tran_ID),0) FROM T0060_EMP_MASTER_APP WITH (NOLOCK)) + 1  
    ...
    --select @Cmp_ID  
    INSERT INTO dbo.T0060_EMP_MASTER_APP  
           (Emp_Tran_ID, Emp_Application_ID,Emp_ID, Cmp_ID, Branch_ID, Cat_ID, Grd_ID, Dept_ID, Desig_Id, Type_ID, Shift_ID, Bank_ID, Emp_code,Initial, Emp_First_Name, Emp_Second_Name,   
           ...
           ,Ration_Card_Type, Ration_Card_No,Vehicle_NO,Training_Month,Is_On_Training,Aadhar_card_no,is_for_mobile_Access  
           ,Actual_Date_Of_Birth,is_PF_Trust,PF_Trust_No,Extension_No,LinkedIn_Id,Twitter_ID,Yearly_Bonus_Per,Manager_Probation,PF_Start_Date --Added by Nimesh 2015-05-09  
          ,Leave_Encash_Working_days,Rejoin_Emp_Id,Is_Camera_enable,Is_Geofence_enable  
          ,Is_Probation_Month_Days,Is_Trainee_Month_Days,Induction_Training,Is_Final_Approval  
          ,Approved_Emp_ID,Approved_Date,Rpt_Level,Approve_Status,Ref_Emp_Tran_ID,HolidayCompOffAvail_After_Days,WeekOffCompOffAvail_After_Days,WeekdayCompOffAvail_After_Days,Signature_Image_Name  
          ,Band_Id,Is_Pradhan_Mantri,Is_1time_PF_Member,Emp_Cast_Join)  
       VALUES     (@Emp_Tran_ID,@Emp_Application_ID,@Emp_ID,@Cmp_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Shift_ID,@Bank_ID,@Emp_code,@Initial,@Emp_First_Name,@Emp_Second_Name,  
          @Emp_Last_Name,@Curr_ID,@Date_Of_Join,@SSN_No,@SIN_No,@Dr_Lic_No,@Pan_No,@Date_Of_Birth,@Marital_Status,@Gender,@Dr_Lic_Ex_Date,@Nationality,  
          ...
          ,@Ration_Card_Type, @Ration_Card_No,@Vehicle_NO,@Training_Month,@Is_On_Training,@Aadhar_Card_No,@is_for_mobile_Access,@Actual_Date_Of_Birth,@Is_PF_Trust,@PF_Trust_No,@Extension_No,@LinkedIn_Id,@Twitter_ID,@Yearly_Bonus_Per,@Manager_Probation,@PF_Start_Date --Added by Nimesh 2015-05-09  --Change By Jaina 02-09-2016  
           ,@Leave_Encash_Working_Day,@Rejoin_Emp_Id,@Is_Camera_enable,@Is_Geofence_enable  
           ,@Is_Probation_Month_Days,@Is_Trainee_Month_Days,@Induction_Training  
           ,@Is_Final_Approval,@Approved_Emp_ID,@Approved_Date,@Rpt_Level,@Approve_Status,@Ref_Emp_Tran_ID,@HolidayCompOffAvail_After_Days  
           ,@WeekOffCompOffAvail_After_Days,@WeekdayCompOffAvail_After_Days,@Signature_Image_Name  
           ,@Band_ID,@Is_PMGKY,@Is_PFMem,@Emp_Cast_Join)  
           -- Added By Ali 03042014  
          
        --commented binal below note need here  
    ...
    --Modifed Main completed binal 15-march-2019  
    if @pay_scale_id <> 0  
    begin   
     set @Pay_Scale_Effective_Date=@Date_Of_Join  
    end  
      
    --select @Emp_Tran_ID ,@Emp_Application_ID binal  
    --Select @Increment_ID =max(ISNULL(Increment_ID ,0)) + 1            
    --  From T0095_INCREMENT   
         
    --select @emp_superior as emp_superior  
       EXEC P0070_EMP_INCREMENT_APP @Increment_ID output,@Cmp_ID,@Emp_Tran_ID,@Emp_Application_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Bank_ID,@Curr_ID,@Wages_Type,@Salary_Basis_On,@Basic_Salary,@Gross_salary,@Date_OF_Join output,@Date_OF_Join,@Payment_Mode,@Inc_Bank_AC_No,@Emp_OT,@Emp_OT_Min_Limit,@Emp_OT_Max_Limit,0,0,0,0,'',@Emp_Late_mark,@Emp_Full_PF,@Emp_PT,@Emp_Fix_Salary,@Emp_Late_Limit,@Late_Dedu_Type,@Emp_Part_Time,1,0,@Yearly_Bonus_Amount,Null,@emp_superior,@Dep_Reminder,1,@CTC,0,0,0,0,0,0,@Emp_Early_mark,@Early_Dedu_Type,@Emp_Early_Limit,@Emp_Deficit_mark,@Deficit_Dedu_Type,@Emp_Deficit_Limit,@Center_ID, @Emp_wd_ot_rate, @Emp_wo_ot_rate, @Emp_ho_ot_rate,0,0,0,0,@no_of_chlidren,@is_metro,@is_physical,@Salary_Cycle_id,@auto_Vpf
,@Segment_ID,@Vertical_ID,@SubVertical_ID,@subBranch_ID,@Monthly_Deficit_Adjust_OT_Hrs,@Fix_OT_Hour_Rate_WD,@Fix_OT_Hour_Rate_WO_HO,@Bank_ID_Two,@Payment_Mode_Two,@Inc_Bank_AC_No_Two,@Bank_Branch_Name,@Bank_Branch_Name_Two,0,'',@User_Id,@IP_Address,@Customer_Audit , null, @Sales_Code,@Physical_Percent,@Approved_Emp_ID,@Approved_Date,@Rpt_Level,@Pay_Scale_ID,@Pay_Scale_Effective_Date  
      
    EXEC P0100_EMP_GRADEWISE_ALLOWANCE_FOR_EMP_APP @Cmp_ID,@Grd_ID,@Date_Of_Join,@Increment_ID,@Branch_ID,@Emp_Tran_ID,@Emp_Application_ID,@Approved_Emp_ID,@Approved_Date,@Rpt_Level  
      
    --Modifed Main completed binal 15-march-2019  
    EXEC P0065_EMP_SHIFT_DETAIL_APP @cmp_ID,@Shift_ID,@Date_Of_Join,null,0,@Emp_Tran_ID,@Emp_Application_ID,@Approved_Emp_ID,@Approved_Date,@Rpt_Level  
      
     
   --call sp for insert data in empoyee main tables if is final approve   
     IF  @Is_Final_Approval = 1   
     BEGIN  
      SET @Emp_Id=0  
      EXEC P0060_EMP_MASTER_APP_TO_CONFIRM @Emp_Tran_ID ,@Emp_Id ,@Approved_Date  
     END  
       
   EXEC P_VALIDATE_EMP_APP @EMP_TRAN_ID=@Emp_Tran_ID,@Mode='EMP_APP',@Is_GroupOFCmp=@Is_GroupOFCmp,@Max_Emp_Code=@Max_Emp_Code,@Emp_Code=@Emp_Code,@Alpha_Emp_Code=@Alpha_Emp_Code   
     
   exec P9999_Audit_get @table = 'T0060_EMP_MASTER_APP' ,@key_column='Emp_Tran_ID',@key_Values=@Emp_Tran_ID ,@String=@String output  
 ...
 Else if @Tran_Type = 'U'  
  begin  
     ...
     select @old_Join_Date = DAte_of_join ,@Emp_Superior_o=Emp_Superior, @Old_Code_Date = Code_Date,  
       @Old_Code_Date_Format = Code_Date_Format  
     From dbo.T0060_EMP_MASTER_APP WITH (NOLOCK)  
     Where Emp_Tran_ID=@Emp_Tran_ID   
   
      if (@Date_Of_Join <> @old_Join_Date)  
      Begin        
         
       if Exists(Select top 1 1 from T0070_EMP_INCREMENT_APP WITH (NOLOCK) WHERE Cmp_ID=@Cmp_ID AND  
        Emp_Tran_ID=@Emp_Tran_ID   
         AND (Increment_Type = 'Increment' or Increment_Type='Transfer'))  
        Begin      
         set @Emp_ID = 0  
     ...
     exec P9999_Audit_get @table='T0060_EMP_MASTER_APP' ,@key_column='EMP_Id',@key_Values=@Emp_ID,@String=@String output  
     ...
     UPDATE dbo.T0060_EMP_MASTER_APP  
     ...
     Select @Alpha_Code = Alpha_Code ,@Alpha_Emp_Code=Alpha_Emp_Code,@Emp_code = Emp_code ,  
       @Emp_Application_ID=Emp_Application_ID,@Pay_Scale_Effective_Date=Date_Of_Join   
     ...
     --Select @Increment_ID =max(ISNULL(Increment_ID ,0))            
     -- From T0070_EMP_INCREMENT_APP   
     --  WHERE   Emp_Tran_ID = @Emp_Tran_ID   
  
     -- COMMENTED ABOVE AND ADDED NEW CONDITION BY HARDIK 09/07/2020 AS INCREMENT_ID DUPLICATING  
     IF @INCREMENT_ID=0 --Added by Mukti(08092020) for update emp details not required to check  
     BEGIN  
     ...
     SELECT @FOR_DATE = INCREMENT_EFFECTIVE_DATE , @INCREMENT_MODE = INCREMENT_MODE FROM T0070_EMP_INCREMENT_APP WITH (NOLOCK) WHERE INCREMENT_ID=@INCREMENT_ID --Added By Ramiz on 11/05/2016 as it was Updating Wrong amount in Increment without this Paramater  
       
     EXEC P0070_EMP_INCREMENT_APP @Increment_ID output,@Cmp_ID,@Emp_Tran_ID,@Emp_Application_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Bank_ID,@Curr_ID,@Wages_Type,@Salary_Basis_On,@Basic_Salary,@Gross_salary,@Date_OF_Join output,@Date_OF_Join,@Payment_Mode,@Inc_Bank_AC_No,@Emp_OT,@Emp_OT_Min_Limit,@Emp_OT_Max_Limit,0,0,0,0,'',@Emp_Late_mark,@Emp_Full_PF,@Emp_PT,@Emp_Fix_Salary,@Emp_Late_Limit,@Late_Dedu_Type,@Emp_Part_Time,1,0,@Yearly_Bonus_Amount,Null,@emp_superior,@Dep_Reminder,1,@CTC,
0,0,0,0,0,0,@Emp_Early_mark,@Early_Dedu_Type,@Emp_Early_Limit,@Emp_Deficit_mark,@Deficit_Dedu_Type,@Emp_Deficit_Limit,@Center_ID, @Emp_wd_ot_rate, @Emp_wo_ot_rate, @Emp_ho_ot_rate,0,0,0,0,@no_of_chlidren,@is_metro,@is_physical,@Salary_Cycle_id,@auto_Vpf,@Segment_ID,@Vertical_ID,@SubVertical_ID,@subBranch_ID,@Monthly_Deficit_Adjust_OT_Hrs,@Fix_OT_Hour_Rate_WD,@Fix_OT_Hour_Rate_WO_HO,@Bank_ID_Two,@Payment_Mode_Two,@Inc_Bank_AC_No_Two,@Bank_Branch_Name,@Bank_Branch_Name_Two,0,'',@User_Id,@IP_Address,@Customer_Audit , null, @Sales_Code,@Physical_Percent,@Approved_Emp_ID,@Approved_Date,@Rpt_Level,@Pay_Scale_ID,@Pay_Scale_Effective_Date  
       
     --commented on 22-may-2019  
     --EXEC P0100_EMP_GRADEWISE_ALLOWANCE_FOR_EMP_APP @Cmp_ID,@Grd_ID,@Date_Of_Join,@Increment_ID,@Branch_ID,@Emp_Tran_ID,@Emp_Application_ID,@Approved_Emp_ID,@Approved_Date,@Rpt_Level  
             
     EXEC P0065_EMP_SHIFT_DETAIL_APP @cmp_ID,@Shift_ID,@Date_Of_Join,@Old_Join_Date,0,@Emp_Tran_ID,@Emp_Application_ID,@Approved_Emp_ID,@Approved_Date,@Rpt_Level      
 ...
 Else If @Tran_Type = 'D'  
  Begin   
   ...
   EXEC P0060_EMP_MASTER_APP_DELETE @Emp_Tran_ID=@Emp_Tran_ID  
  ...
  EXEC P9999_AUDIT_LOG @TableName='T0060_EMP_MASTER_APP', @IDFieldName='Emp_ID',@Audit_Module_Name='Employee Master',  
