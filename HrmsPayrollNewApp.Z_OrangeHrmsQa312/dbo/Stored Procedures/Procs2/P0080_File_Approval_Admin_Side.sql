...
CREATE PROCEDURE [dbo].[P0080_File_Approval_Admin_Side]  
 ...
 If @tran_type  = 'I'  
  Begin
   Select @FA_Id = ISNULL(MAX(File_Apr_Id),0) + 1 From T0080_File_Approval WITH (NOLOCK)
			--set @file=cast(FORMAT(GETDATE(), 'dd MM yyyy HH mm ss')as varchar)+'-'+cast(@FA_Id as varchar)+'-'+cast(@cmp_id as varchar)+'-'+'FA'+'-'+'#'+cast(@fileName as varchar)	
  ...
  --IF EXISTS( SELECT 1 FROM T0120_GATE_PASS_APPROVAL WITH (NOLOCK) WHERE Emp_ID = @Emp_Id AND From_Time = @From_Time AND To_Time = @To_Time AND Duration = @Duration )
		...
		IF @Final_Approval = 1 OR (@Is_Fwd_Leave_Rej = 0 )
				...
				print 4
					IF @Rpt_Level = 0 AND @File_App_Id = 0
						BEGIN
						  exec P0080_File_Application_Admin_Side @File_App_Id OUTPUT,@Cmp_ID,@ApplicationDate,@Branch_Id,@Dept_Id,@Desig_Id,@Emp_Id,@S_Emp_Id,@FileNumber,@FileType_Id,'I',@Subject,@Description,@ProcessDate,@FileName,@LoginID,@IP_Address,@Comment
		...
		-- Add for audit(end)	
			...
			INSERT INTO T0080_File_Approval (File_Apr_Id,Approve_Date,Branch_Id,Dept_Id,Desig_Id,Emp_Id,S_Emp_Id,
			File_Number,F_StatusId,F_TypeId,Subject,Description,Process_Date,File_App_Doc,Cmp_ID,
			CreatedDate,[User ID],Forward_Emp_Id,Submit_Emp_Id,CreatedBy,File_App_Id,Approval_Comments)
			VALUES(@FA_Id,@ApproveDate,@Branch_Id,@Dept_Id,@Desig_Id,@Emp_Id,@S_Emp_Id,@FileNumber,
			 @FileStatus_Id,@FileType_Id,@Subject,@Description,@ProcessDate,
			 @FileName,@Cmp_ID,GETDATE(),@LoginID,@Forward_Emp_Id,@Submit_Emp_Id,@LoginID,@File_App_Id,@Comment)
    		--for History start
		...
		-- Add for audit(end)	
			update  T0115_File_Level_Approval set File_Apr_Id=@FA_Id,F_StatusId=@FileStatus_Id,File_App_Doc=@FileName,
		...
		-- Add for audit(end)	
		   ...
		   Approval_Comments=@Comment,File_App_Doc=@fileName
			--,System_Date=GETDATE()
			--,Forward_Emp_Id=@Forward_Emp_Id,Submit_Emp_Id=@Submit_Emp_Id,
			--Subject=@Subject,Description=@Description
			where File_App_Id=@File_App_Id and cmp_id=@Cmp_ID
				...
				--for History start
						INSERT INTO dbo.T0115_File_Level_Approval_History
						   (Cmp_Id,File_App_Id,File_Apr_Id,Emp_Id,H_File_Number,H_F_StatusId,H_F_TypeId,H_Subject,H_Description,H_S_Emp_Id,H_Process_Date,H_File_App_Doc,H_Forward_Emp_Id,H_Submit_Emp_Id,H_Approval_Comments,H_Review_Emp_Id,H_Reviewed_by_Emp_Id,Rpt_Level,CreatedDate,[User ID],H_Trans_Type,H_Tran_Id,Tbl_Type)
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,@FileStatus_Id,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@fileName,@Forward_Emp_Id,@Submit_Emp_Id,@Comment,@Review_Emp_Id,@Reviewed_by_Emp_Id,@Rpt_Level,getDate(),@LoginID,'U',@Tran_ID,'T0115_File_Level_Approval')	
					--for History end
		-- Add for audit(start)
			 exec P9999_Audit_Trail @Cmp_ID,'U','File Level Approve',@OldValue_lvl,@Emp_ID,@LoginId,@IP_Address
		-- Add for audit(end)	
