...
CREATE PROCEDURE [dbo].[P0140_TRAVEL_SETTLEMENT_EXPENSE_MODE_DETAIL_EMAIL_TEMPLATE]
	 @Cmp_ID						Numeric(18,0)
...
BEGIN
	...
	SET @HTML_TABLE = ''
			...
			IF EXISTS(	SELECT		1
						FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
						LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND 
									(TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
						WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 1 AND 
									(TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID)) 
					...
					DECLARE FLIGHT_BOOKING CURSOR  FAST_FORWARD FOR		
						...
						FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
						LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
						WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 1 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID)
					OPEN FLIGHT_BOOKING
		...
		IF EXISTS (	SELECT		1
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 2 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
						BEGIN
		...
		IF EXISTS(SELECT		TAD.MODE_NAME,TAD.CITY,CONVERT(VARCHAR, CONVERT(DATE,TAD.CHECK_OUT_DATE), 9) AS CHECK_OUT_DATE
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 6 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
					BEGIN
		...
		IF EXISTS(	SELECT		1
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 3 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
					BEGIN
		...
		IF EXISTS(	SELECT		1
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 4 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
					BEGIN
		...
		IF EXISTS(	SELECT		1
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 5 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
					BEGIN
		...
		IF EXISTS(	SELECT		1
					FROM		T0140_Travel_Settlement_Expense TOD WITH (NOLOCK)
					LEFT JOIN	T0140_Travel_Settlement_Mode_Expense TAD WITH (NOLOCK) ON TOD.INT_ID = TAD.INT_ID AND (TOD.TRAVEL_SET_APPLICATION_ID = TAD.TRAVEL_SET_APPLICATION_ID OR TOD.TRAVEL_APPROVAL_ID = TAD.TRAVEL_APPROVAL_ID)
					WHERE		TOD.CMP_ID = @CMP_ID AND TAD.TRAVEL_MODE = 7 AND (TAD.TRAVEL_SET_APPLICATION_ID = @Travel_Set_App_ID OR TAD.TRAVEL_APPROVAL_ID = @Travel_Set_App_ID))
					BEGIN
