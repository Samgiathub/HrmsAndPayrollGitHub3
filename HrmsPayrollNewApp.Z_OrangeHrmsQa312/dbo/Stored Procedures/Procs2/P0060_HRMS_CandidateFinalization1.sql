...
CREATE PROCEDURE  [dbo].[P0060_HRMS_CandidateFinalization1]
	 @Cmp_ID  Numeric (18,0) 
			...
			--IF @status=0
				begin				
					set @Query = 'SELECT c.[Resume_ID]
										  ...
										  ,h.Blood_Group as Blood_Group,h.Height
										  ,isnull(c.[Accept_appointment],0)Accept_appointment
							    ...
							    where isnull(Accept_Appointment,0) <>2 and c.Cmp_id=' + cast(@Cmp_ID as varchar(50))  																	 				
							
							--EXEC(@Query + @Condition + 'order by Joining_date asc')  commented By Mukti17112014
							EXEC(@Query + @Condition) --Added By Mukti17112014
