CREATE PROCEDURE [dbo].[P0080_EMP_MASTER_VMS]
...
AS	
	...
	SELECT * INTO #T0080_EMP_MASTER_DELETED FROM T0080_EMP_MASTER WHERE Emp_ID=@Emp_ID
	...
	if @Is_On_Probation = 1 and @Probation = 0  --added by nilesh patel for Probation 
		begin
			SELECT @Probation = isnull(GS.Probation,0),@Is_Probation_Month_Days=GS.Is_Probation_Month_Days FROM T0040_GENERAL_SETTING GS where GS.Cmp_ID = @cmp_ID and GS.Branch_ID = @Branch_ID
		End		
	if @Is_On_Training = 1 and @Training_Month = 0  --added by jimit 07102016
		begin
			SELECT @Training_Month = isnull(GS.Training_Month,0),@Is_Trainee_Month_Days=GS.Is_Trainee_Month_Days FROM T0040_GENERAL_SETTING GS where GS.Cmp_ID = @cmp_ID and GS.Branch_ID = @Branch_ID
		End		
	...
	BEGIN						
			if exists ( SELECT 1 from T0080_EMP_MASTER where Rejoin_Emp_Id =@Rejoin_Emp_Id and Cmp_ID=@Cmp_id)
			BEGIN
	 ...
	 FROM T0080_EMP_MASTER WHERE CMP_ID = @CMP_ID AND EMP_ID= @REJOIN_EMP_ID
	...
	from T0040_GENERAL_SETTING where Cmp_ID=@Cmp_ID and Branch_ID=@Branch_ID and For_Date = (select max(for_date) From T0040_General_Setting where Cmp_ID = @Cmp_ID and Branch_ID =@branch_id)    --Modified by Ramiz on 15092014

	select @Add_Initial_In_Emp_Full_Name = setting_value from T0040_SETTING where cmp_id = @Cmp_ID and setting_name = 'Add initial in employee full name'
		...
		Begin
			...
			Begin
				...
				if @Branch_Desig_Wise_Count >= @Branch_Desig_Strength_Count
					Begin
						set @Emp_ID = 0
						RAISERROR ('@@Employee count is greater then set employee strength count in strength Master@@', 16, 2)
						return
					End
			End
		End 
	...
	select @Retirement_Year = Setting_Value from T0040_SETTING where cmp_id = @Cmp_ID and Setting_Name='Employee Retirement Age'
	  ...
	  -- Added by Ali ` -- Start
		IF @Emp_Notice_Period = 0
			BEGIN
					...
					select @Is_Short_Fall_Grade_wise = Is_Shortfall_Gradewise , @short_Fall_days = Short_Fall_Days from T0040_GENERAL_SETTING where Branch_ID = @Branch_ID and Cmp_ID = @Cmp_ID and For_Date = (select max(for_date) From T0040_General_Setting where Cmp_ID = @Cmp_ID and Branch_ID =@branch_id)   -- Modified by Ramiz on 15092014
					if @Is_Short_Fall_Grade_wise = 1
					BEGIN
						select @short_Fall_days = Short_Fall_Days from T0040_GRADE_MASTER where Grd_ID = @Grd_ID
					END	
		   ...
		   ,@Yearly_Bonus_Per =ISNULL(Bonus_Per,0) --Ankit 19082016
			...
			Else If @Is_GroupOFCmp = 1
				Begin
					...
					If Exists(select Emp_ID From dbo.T0080_EMP_MASTER WHERE Alpha_Emp_Code = @Alpha_Emp_Code And Cmp_ID In (Select Cmp_ID From T0010_COMPANY_MASTER Where is_GroupOFCmp = 1))
						...
						END
					ELSE If Exists(select Emp_ID From dbo.T0080_EMP_MASTER WHERE (Alpha_Emp_Code = @Alpha_Emp_Code) And Cmp_ID = @Cmp_ID)
						...
						END
					ELSE If Exists(select Emp_ID From dbo.T0080_EMP_MASTER WHERE (Alpha_Emp_Code = @Alpha_Emp_Code) And Cmp_ID = @Cmp_ID And Branch_ID = @Branch_Id)
			...
			Select @Backdate_Allowed = Setting_Value From T0040_SETTING Where Cmp_ID = @Cmp_ID and Setting_Name='Allowed Backdated Joining upto Days'

			if @Backdate_Allowed > 0
				Begin
					--Added By Nilesh Patel For Kataria Client After Discuss With Ankur Sir 
					if Not Exists(SELECT 1 FROM T0011_LOGIN Where Cmp_ID = @Cmp_ID and Login_ID = @User_Id and Login_Name LIKE 'admin@%')
					BEGIN
				...
				SELECT @Count =Count(Emp_ID) from dbo.T0080_EMP_MASTER where Emp_Left<>'y'								
				...
				select @Emp_ID = Isnull(max(Emp_ID),0) + 1 	From dbo.T0080_EMP_MASTER
				
				select @Adult_No = Isnull(max(Worker_Adult_No),0) + 1 	From dbo.T0080_EMP_MASTER Where Cmp_ID=@Cmp_ID
			
				-- Added By Alpesh on 25-05-2011 for first time login change password dialogbox for employee 
				if exists (select Module_Id From T0011_module_detail where Cmp_Id=@Cmp_ID And chg_pwd=1)
					Begin
						  Set @Chg_Pwd=1
					End
				---------------------End------------------	
				--Added by Nimesh on 02-Feb-2016 (To generate the Employee PF Number automatically if duplicate)
				IF EXISTS(Select 1 From T0040_SETTING WHERE CMP_ID=@Cmp_ID  AND Setting_Name='Auto Generate Employee PF Number' AND Setting_Value=1)
					BEGIN
						IF EXISTS(SELECT 1 FROM T0080_EMP_MASTER WHERE SSN_No=@SSN_No AND CMP_ID=@CMP_ID AND IsNull(@SSN_No,'') <> '')
							BEGIN				
				...
				INSERT INTO dbo.T0080_EMP_MASTER
									  ...
									  ,Ration_Card_Type, Ration_Card_No,Vehicle_NO,Training_Month,Is_On_Training,Aadhar_card_no,Actual_Date_Of_Birth,is_PF_Trust,PF_Trust_No,Extension_No,LinkedIn_Id,Twitter_ID,Yearly_Bonus_Per,Manager_Probation,PF_Start_Date --Added by Nimesh 2015-05-09
										,Leave_Encash_Working_days,Rejoin_Emp_Id,Is_Probation_Month_Days,Is_Trainee_Month_Days,Induction_Training,HolidayCompOffAvail_After_Days,WeekOffCompOffAvail_After_Days,WeekdayCompOffAvail_After_Days)
							VALUES     (@Emp_ID,@Cmp_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Shift_ID,@Bank_ID,@Emp_code,@Initial,@Emp_First_Name,@Emp_Second_Name,
										...
										,@Ration_Card_Type, @Ration_Card_No,@Vehicle_NO,@Training_Month,@Is_On_Training,@Aadhar_Card_No,@Actual_Date_Of_Birth,@Is_PF_Trust,@PF_Trust_No,@Extension_No,@LinkedIn_Id,@Twitter_ID,@Yearly_Bonus_Per,@Manager_Probation,@PF_Start_Date --Added by Nimesh 2015-05-09  --Change By Jaina 02-09-2016
										 ,@Leave_Encash_Working_Day,@Rejoin_Emp_Id,@Is_Probation_Month_Days,@Is_Trainee_Month_Days,@Induction_Training,@HolidayCompOffAvail_After_Days,@WeekOffCompOffAvail_After_Days,@WeekdayCompOffAvail_After_Days)
							...
							INSERT INTO dbo.T0080_EMP_MASTER_Clone
									  ...
									  Training_Month,Is_On_Training , Aadhar_card_no,Induction_Training,HolidayCompOffAvail_After_Days,WeekOffCompOffAvail_After_Days,WeekdayCompOffAvail_After_Days) -- Added By Ali 03042014
							VALUES     (@Emp_ID,@Cmp_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Shift_ID,@Bank_ID,@Emp_code,@Initial,@Emp_First_Name,@Emp_Second_Name,
										@Emp_Last_Name,@Curr_ID,@Date_Of_Join,@SSN_No,@SIN_No,@Dr_Lic_No,@Pan_No,@Date_Of_Birth,@Marital_Status,@Gender,@Dr_Lic_Ex_Date,@Nationality,
										@Loc_ID,@Street_1,@City,@State,@Zip_code,@Home_Tel_no,@Mobile_No,@Work_Tel_No,@Work_Email,@Other_Email,@Emp_Superior,@Basic_Salary,@Image_Name,
										@Emp_Full_Name,'N',@Present_Street,@Present_City,@Present_State,@Present_Post_Box,@enroll_No,@Blood_Group,@Is_Gr_App,@Is_Yearly_Bonus,@Probation,
										...
										@CompOff_HO_Avail_Days,@Date_Of_Retirement,@Is_Salary_Depends_On_Production_Details,@Training_Month,@Is_On_Training,@Aadhar_Card_No,@Induction_Training,@HolidayCompOffAvail_After_Days,@WeekOffCompOffAvail_After_Days,@WeekdayCompOffAvail_After_Days) -- Added by Gadriwala Muslim 20112014
				...
				select @G_FOR_DATE = max(for_date) from T0040_GENERAL_SETTING where branch_id= @Branch_ID and for_date<= getdate()
				...
				from T0040_GENERAL_SETTING where branch_id= @Branch_ID and for_date= @G_FOR_DATE
				...
				IF Exists(Select 1 From T0040_GRADE_MASTER Where Cmp_ID = @Cmp_ID and Grd_ID = @Grd_ID and Isnull(Grd_WAGES_TYPE,'') <> '')
					Begin
						Select @Wages_Type = Grd_WAGES_TYPE From T0040_GRADE_MASTER Where Cmp_ID = @Cmp_ID and Grd_ID = @Grd_ID
					End
				...
				exec P0100_Emp_Manager_History 0,@Cmp_ID,@Emp_ID,@Increment_ID,@Emp_Superior,@Date_Of_Join	
				 ...
				 if not exists (SELECT 1 from T0011_LOGIN where Login_Alias = isnull(@Login_Alias,'') AND Emp_ID <> @Emp_ID)
					begin
		...
		begin
			...
			--	end	
				...
				--	end
					...
					select @old_Join_Date = DAte_of_join ,@Emp_Superior_o=Emp_Superior, @Old_Code_Date = Code_Date,@Old_Code_Date_Format = Code_Date_Format From dbo.T0080_EMP_MASTER Where Emp_ID = @Emp_ID
					...
					if (@Date_Of_Join <> @old_Join_Date)
						Begin						
							...
							if Exists(select top 1 1 from t0200_monthly_salary where Emp_ID=@Emp_ID and Cmp_ID=@Cmp_ID and Month_St_Date <= @Date_Of_Join)
								Begin									
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Salary Exists.@@', 16, 2)
									return
								End
							Else if Exists(select top 1 1 from T0100_LEAVE_APPLICATION LA inner join T0110_LEAVE_APPLICATION_DETAIL LAD on LA.Leave_Application_ID=LAD.Leave_Application_ID  where LA.Emp_ID=@Emp_ID and LA.Cmp_ID=@Cmp_ID and LAD.From_Date <= @Date_Of_Join)
								Begin
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Leave Records Exists.@@', 16, 2)
									return
								End	
							Else if Exists(select top 1 1 from dbo.T0100_LOAN_APPLICATION where Emp_ID=@Emp_ID and Cmp_ID=@Cmp_ID and Loan_App_Date <= dateadd(M,-1,@Date_Of_Join))
								Begin									
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Loan Records Exists.@@', 16, 2)
									return
								End	
							Else if Exists(select top 1 1 from dbo.T0120_leave_Approval LA inner join T0130_LEAVE_APPROVAL_DETAIL LAD on LA.Leave_Approval_ID=LAD.Leave_Approval_ID  where Emp_ID=@Emp_ID and LA.Cmp_ID=@Cmp_ID and LAD.From_Date <= @Date_Of_Join)
								Begin									
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Leave Records Exists.@@', 16, 2)
									return
								End
							Else if Exists(select top 1 1 from dbo.T0095_LEAVE_OPENING where Emp_ID=@Emp_ID and Cmp_ID=@Cmp_ID and For_Date <= @Date_Of_Join and Leave_Op_Days <> 0) -- Added this condition after discussion with Sandip bhai because leave opening records are not deleting from front end 8/11/2016
								Begin									
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Leave Opening Records Exists.@@', 16, 2)
									return
								End
							Else if Exists(select top 1 1 from dbo.T0120_LOAN_APPROVAL where Emp_ID=@Emp_ID and Cmp_ID=@Cmp_ID and Loan_Apr_Date <= @Date_Of_Join)
								Begin									
									set @Emp_ID = 0
									RAISERROR ('@@Date of Join cannot be updated. Loan Records Exists.@@', 16, 2)
									return
								End
							Else if Exists(Select top 1 1 from T0095_INCREMENT WHERE Cmp_ID=@Cmp_ID AND Emp_ID=@Emp_ID AND (Increment_Type = 'Increment' or Increment_Type='Transfer'))
								Begin				
									set @Emp_ID = 0
					...
					-- Add by Deepal 17072020 Link HRMS and VMS Database for Shaily
					EXEC P0080_DATALINK_HRMS_VMS @CMP_ID ,@BRANCH_ID ,@EMP_ID ,@DEPT_ID ,@DESIG_ID ,@LOC_ID , @Present_State , @Present_City, @Login_Alias, 'U'
					-- END by Deepal 17072020
					
					--Added by nilesh patel on 05042016 , wrong Alphacode insert into Employee Master Clone
					Select @Alpha_Code = Alpha_Code ,@Alpha_Emp_Code=Alpha_Emp_Code,@Emp_code = Emp_code From T0080_EMP_MASTER Where Emp_ID = @Emp_ID
					...
					INSERT INTO dbo.T0080_EMP_MASTER_Clone
									  ...
									  ,Thana_Id,Tehsil,District,Thana_Id_Wok,Tehsil_Wok,District_Wok,SkillType_ID,CompOff_WD_App_Days,CompOff_WD_Avail_Days,CompOff_WO_App_Days,CompOff_WO_Avail_Days,CompOff_HO_App_Days,CompOff_HO_Avail_Days,Date_of_Retirement,
									  Salary_Depends_on_Production,Training_Month,Is_On_Training , Aadhar_card_no,Induction_Training,HolidayCompOffAvail_After_Days,WeekOffCompOffAvail_After_Days,WeekdayCompOffAvail_After_Days) -- Added By Ali 03042014
							VALUES     (@Emp_ID,@Cmp_ID,@Branch_ID,@Cat_ID,@Grd_ID,@Dept_ID,@Desig_Id,@Type_ID,@Shift_ID,@Bank_ID,@Emp_code,@Initial,@Emp_First_Name,@Emp_Second_Name,
										@Emp_Last_Name,@Curr_ID,@Date_Of_Join,@SSN_No,@SIN_No,@Dr_Lic_No,@Pan_No,@Date_Of_Birth,@Marital_Status,@Gender,@Dr_Lic_Ex_Date,@Nationality,
										@Loc_ID,@Street_1,@City,@State,@Zip_code,@Home_Tel_no,@Mobile_No,@Work_Tel_No,@Work_Email,@Other_Email,@Emp_Superior,@Basic_Salary,@Image_Name,
										@Emp_Full_Name,'N',@Present_Street,@Present_City,@Present_State,@Present_Post_Box,@enroll_No,@Blood_Group,@Is_Gr_App,@Is_Yearly_Bonus,@Probation,
										@Adult_No,@Father_Name,@Bank_BSR_No,getdate(),@login_id,@Old_Ref_No,@Chg_Pwd,@Alpha_Code ,@Alpha_Emp_Code,@Ifsc_Code,@Leave_In_Probation,@Is_LWF,
										@DBRD_Code,@Dealer_Code,@CCenter_Remark,@Emp_PF_Opening,@Emp_Category,@Emp_UIDNo,@Emp_Cast
										,@Emp_Anniversary_Date,@Extra_AB_Deduction,@Min_CompOff_Limit,@mother_name,@Segment_ID,@Vertical_ID
										...
										,@EmpName_Alias_PT,@EmpName_Alias_Tax,@EmpName_Alias_ESIC,@EmpName_Alias_Salary,@Emp_Notice_Period,@Dress_Code,@Shirt_Size,@Pent_Size,@Shoe_Size,@Canteen_Code
										,@Thana_Id,@Tehsil,@District,@Thana_Id_Wok,@Tehsil_Wok,@District_Wok,@SkillType_ID,@CompOff_WD_App_Days,@CompOff_WD_Avail_Days,@CompOff_WO_App_Days,@CompOff_WO_Avail_Days,@CompOff_HO_App_Days,@CompOff_HO_Avail_Days,@Date_Of_Retirement,
										@Is_Salary_Depends_On_Production_Details,@Training_Month,@Is_On_Training,@Aadhar_card_no,@Induction_Training,@HolidayCompOffAvail_After_Days,@WeekOffCompOffAvail_After_Days,@WeekdayCompOffAvail_After_Days)  -- Added by Gadriwala 24042014 -- Added By Ali 03042014	--Aadhar Added by Ramiz on 07/08/2015
				...
				--	where Emp_ID = @Emp_ID 
					...
					if exists (select For_date from  T0100_WEEKOFF_ADJ where For_Date = @Old_Join_Date and   Emp_ID =@Emp_ID and Cmp_ID = @Cmp_ID)
						begin
									Update T0100_WEEKOFF_ADJ set For_Date = @Date_Of_Join 
									where Emp_ID = @emp_ID and Cmp_ID =@cmp_ID and For_Date = @old_Join_Date 
						end
					IF EXISTS(SELECT INCREMENT_EFFECTIVE_DATE FROM T0095_INCREMENT WHERE EMP_ID=@EMP_ID AND INCREMENT_EFFECTIVE_DATE=@OLD_JOIN_DATE AND CMP_ID=@CMP_ID AND INCREMENT_TYPE='JOINING')
						BEGIN						
							IF (@DATE_OF_JOIN <> @OLD_JOIN_DATE)
								BEGIN											
								...
								END	
						END
					IF EXISTS(SELECT EFFECTIVE_DATE FROM T0011_LOGIN WHERE EMP_ID=@EMP_ID AND EFFECTIVE_DATE=@OLD_JOIN_DATE AND CMP_ID=@CMP_ID)	
						BEGIN						
							IF (@DATE_OF_JOIN <> @OLD_JOIN_DATE)
								BEGIN											
					...
					if @Emp_Superior is not null
						begin															
							if not exists(Select Row_ID from T0090_EMP_REPORTING_DETAIL Where Cmp_ID=@Cmp_ID and Emp_ID=@Emp_ID and R_Emp_ID=@Emp_Superior)
								begin	
									PRINT 'r'
						...
						begin		
							PRINT @Cmp_ID				
							Select @Reporting_Row_ID = Row_ID from T0090_EMP_REPORTING_DETAIL Where Cmp_ID=@Cmp_ID and Emp_ID=@Emp_ID and R_Emp_ID=isnull(@Emp_Superior_o,0)
	...
	Else If @Tran_Type = 'D'
		Begin 
			...
			Select @EmpJoinDate = Date_Of_Join From T0080_EMP_MASTER Where Emp_ID = @Emp_ID and Cmp_ID = @CMP_ID

			IF EXISTS(SELECT 1 FROM  T0050_EMP_PAY_SCALE_DETAIL WHERE Emp_ID = @Emp_ID and Cmp_ID=@CMP_ID AND Effective_Date > @EmpJoinDate)
				Begin
			...
			DELETE FROM T0090_EMP_PRIVILEGE_DETAILS WHERE Login_Id = (SELECT Login_ID FROM T0011_LOGIN WHERE Emp_ID=@Emp_ID AND Cmp_ID=@Cmp_ID)
			
			Declare @Leave_Approval_ID as numeric(18,0)														-- Added by Mitesh on 27/07/2011	
			Select @Leave_Approval_ID=Leave_Approval_ID FROM T0120_LEAVE_APPROVAL				WHERE		EMP_ID = @EMP_iD And cmp_Id=@Cmp_ID	-- Added by Mitesh on 27/07/2011						
			...
			Select @DLogin_ID=Login_ID FROM T0011_Login		WHERE		EMP_ID = @EMP_iD And cmp_Id=@Cmp_ID	-- Added by Mitesh on 27/07/2011									
		...
		--Added By Mukti(14092015)start
			...
			select @Asset_Application_ID  =ISNULL(Asset_Application_ID,0),@Asset_Approval_ID  =ISNULL(Asset_Approval_ID,0) From dbo.T0120_Asset_Approval where (Emp_id=@Emp_ID and cmp_id=@cmp_id) or (Applied_by=@Emp_ID and cmp_id=@cmp_id)
		...
		SELECT * INTO #T0080_EMP_MASTER_INSERTED FROM T0080_EMP_MASTER WHERE Emp_ID=@Emp_ID
