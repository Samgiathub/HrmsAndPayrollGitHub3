...
CREATE PROCEDURE [dbo].[P0140_Travel_Sattlement_Expense_Test_17022023]     
...
SET ARITHABORT ON
	...
	--Begin     
		INSERT INTO T0140_Travel_Settlement_Expense  
			(  
				...
				City_ID,Travel_Mode_ID,Travel_Set_Application_id,SGST,CGST,IGST,GST_No,GST_Company_Name,SelfPay
			)  
			VALUES        
			  (
				@Int_Id  
			   ...
			   insert into #TravelHySchemeSett
 				SELECT isnull(max(Rpt_Level),0) as RPT_max_level,Scheme_Id,Dyn_Hier_Id,Leave as TravelTypeId,@Emp_ID as App_Emp, @Travel_Approval_ID as AprId--,@setId as SetId
				...
				where Scheme_Id = (
											...
											AND (SELECT distinct TravelTypeId from V0140_Travel_Settlement_Application_New_Level where Travel_Approval_ID = @Travel_Approval_ID) IN (select data from dbo.split(leave,'#'))
				) 
				AND (SELECT Distinct TravelTypeId from V0140_Travel_Settlement_Application_New_Level where Travel_Approval_ID = @Travel_Approval_ID) IN (select data from dbo.split(leave,'#')) 
 ...
 --END  
    ...
    begin  
	 ...
	 ,SelfPay=@SelfPay
     where   
     emp_id=@Emp_ID and Travel_Approval_ID = @Travel_Approval_ID  and int_id=@Int_Id
  ...
  delete from T0140_Travel_Settlement_Expense where emp_id=@Emp_ID and Travel_Approval_ID = @Travel_Approval_ID  and int_id=@Int_Id
