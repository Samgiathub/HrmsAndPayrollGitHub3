...
CREATE PROCEDURE [dbo].[P0100_GRATUITY_CALCULATION]  
 ...
 @Tran_Type  char(1),  
 @Gr_FNF   int = 0,
 @Gr_Year numeric(18,2) =0  -- Added By Deepali -13Dec21
 ,@Gr_Months numeric(18,2) =0-- Added By Deepali -04Jan22
...
SET ARITHABORT ON 
 ...
 Declare @Tot_Exp_year numeric -- Added By Deepali -13Dec21
 Declare @Exit_Type tinyint -- Added By Deepali -13Dec21
 ...
 set @Gr_Min_P_Days=0  -- Added By Deepali -13Dec21
 set @Exit_Type = 0 -- Added By Deepali -13Dec21
 ...
 if upper(@Tran_type) ='I'  
  begin  
	...
	select @gr_Min_Year = gr_Min_Year,@Gr_Cal_Month=Isnull(Gr_ProRata_Cal,0) ,  
	   ...
	   ,@Gr_Min_P_Days=ISNULL(@Gr_Min_P_Days,0) -- Added By Deepali -13Dec21
  ...
  set @Gr_Days = isnull(@Gr_Days,0)  
	...
	if @Is_Gr_Yearly_Paid =1   
		 ...
		 begin				
				...
				IF @Month_Only > = 6
					...
					set @Tot_Exp_year= ROUND(@Year_Only,0) --Added By Deepali 15Dec2021
   ...
   --Added By Jimit 18122018  (case at WCl for death Employees it was not calculating)
	 DECLARE @Emp_Death tinyint
	  ...
	  If(@Emp_Death =1) -- added By Deepali 15Dec2021
		set @Exit_Type = 1 --- added By Deepali 15Dec2021
	  --ended
	 ...
	 begin  
	   INSERT INTO T0100_GRATUITY  
			 (Gr_ID, Cmp_ID, Emp_Id, From_Date, To_Date, Paid_Date, Gr_Calc_Amount, Gr_Days, Gr_Percentage, Gr_Amount, Gr_Calc_Type, Gr_FNF,Gr_Years)  
	  VALUES     (@Gr_ID, @Cmp_ID, @Emp_Id, @Last_Gr_Date, @To_Date, @Gr_Paid_Date, @Gr_Calc_Amount, @Gr_Days, @Gr_Percentage, @Gr_Amount, @Gr_Calc_Type, @Gr_FNF,@Gr_Year)  
			...
			begin  
				...
				From T0200_MONTHLY_SALARY WITH (NOLOCK) WHERE month_end_Date >=@Last_Gr_Date and Month_End_Date <=@To_Date and Emp_ID=@Emp_ID  
				 ...
				 if @Gr_Days > 0   
		           ...
		           -- Changed by Deepali 07012022 -To update - GR_years Count(GR_ID) as GR_years - Start
						  ...
						  From T0100_GRATUITY g inner join (select Gr_Id,sum(Gr_D_Amount) Gr_D_Amount ,  Count(GR_ID) as GR_years, 
						 ...
						 where g.Gr_ID=@gr_ID 
						   -- Changed by Deepali 07012022 -To update - GR_years Count(GR_ID) as GR_years - End
					...
					begin
						  ...
						  update T0100_GRATUITY  
						  set Gr_Calc_Amount =(q.Gr_D_Calc_Amount/12 * @gr_Min_Year) ,  Gr_Years=q.GR_years,
			  ...
			  if @Gr_Days > 0   
					begin  
						 ...
						 set Gr_Calc_Amount = q.Gr_D_Calc_Amount ,  
						  Gr_Amount = round((q.Gr_D_Calc_Amount /@gr_out_of_days ) * @Gr_Days ,0), Gr_Years=q.GR_years
					...
					begin  
						 ...
						 update T0100_GRATUITY  
						 set Gr_Calc_Amount =q.Gr_D_Calc_Amount ,  
						  Gr_Amount = round( q.Gr_D_Calc_Amount * @Gr_Percentage/100 ,0) , Gr_Years=q.GR_years
	 ...
	 --added by Deepali 14122021 -Start
	 if(@Exit_Type= 0)
	  SELECT @Exit_Type = Is_Absconded FROM T0100_LEFT_EMP  WITH (NOLOCK) WHERE Emp_Id=@Emp_ID	
	 else if(@Exit_Type=0)
	   SELECT @Exit_Type = Is_Terminate FROM T0100_LEFT_EMP  WITH (NOLOCK) WHERE Emp_Id=@Emp_ID
	   
	   --select @Exit_Type,@Gr_Months
	   --select * from T0100_GRATUITY g  where g.Gr_ID=@gr_ID 			
		if(@Exit_Type=1 and @Gr_Months>0)
		begin 
							--print 'New calculation'
					      
						  declare @Total_months As numeric(18,2)
						  set @Total_months = 12
						  set  @Total_months = @Tot_Exp_year*12

					 

						 -- print @Tot_Exp_year
						  update T0100_GRATUITY  
						  set Gr_Calc_Amount =(g.Gr_Calc_Amount/@Total_months  * @Gr_Months) ,  
						   Gr_Amount = round(( (g.Gr_Calc_Amount/@Total_months * @Gr_Months)  /@gr_out_of_days ) * @Gr_Days ,0)   
						  From T0100_GRATUITY g  
						  where g.Gr_ID=@gr_ID 					  
					 
					 select g.Gr_Calc_Amount,@Total_months,@Gr_Months ,@gr_out_of_days ,@Gr_Days --, round(( (g.Gr_Calc_Amount/@Total_months * @Gr_Months)  /@gr_out_of_days ) * @Gr_Days ,0)   ,@Total_months,@gr_out_of_days,@Gr_Days,* 
					 from T0100_GRATUITY g  where g.Gr_ID=@gr_ID 	
					--select * from T0100_GRATUITY where Emp_Id=@Emp_ID
					--select * from T0110_GRATUITY_DETAIL where GR_ID =@gr_ID
					
					
		end
				 --added by Deepali 14122021- end
