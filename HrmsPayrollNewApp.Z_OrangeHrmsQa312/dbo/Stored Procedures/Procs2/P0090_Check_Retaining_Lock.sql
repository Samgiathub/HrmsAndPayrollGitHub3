...
-- =============================================
CREATE PROCEDURE [dbo].[P0090_Check_Retaining_Lock] 
...
SET ARITHABORT ON
	...
	IF @Op = 0
		BEGIN
			...
			Set @Enable_Days = 0
			SET @Enable_Days = (Select Setting_Value from T0040_SETTING WITH (NOLOCK) where Setting_Name = 'Enable Retaining Lock for mid join employee upto days' and Cmp_ID = @Cmp_Id)
				...
				BEGIN
					...
					ELSE
						BEGIN
							...
							Select Tran_Id from T0090_Retaining_Lock_Setting WITH (NOLOCK) where  From_Date <= GETDATE() AND To_Date >= GETDATE() And Cmp_Id = @Cmp_Id	
						END
				END
			ELSE
				BEGIN
					 --Financial_Year = @FY and
					...
					Select Tran_Id from T0090_Retaining_Lock_Setting WITH (NOLOCK) where Cmp_Id = @Cmp_Id and (CONVERT(varchar(30),GETDATE(),106) between From_Date AND To_Date )
				END 
		END
	Else
	BEGIN
			Select * from (      
		    Select * from T0090_Retaining_Lock_Setting WITH (NOLOCK) Where Cmp_Id = @Cmp_Id  
		    AND (((From_Date >= @From_Date) AND (From_Date <= @To_Date)) OR ((To_Date >= @From_Date) AND (To_Date <= @To_Date )))  
		    ) as t where t.Tran_Id <> @Tran_Id
	END
END
