...
CREATE PROCEDURE [dbo].[P0120_HRMS_Induction_Details]
...
IF @Tran_type = 'I'
	BEGIN	--added by aswini 19082023
	if exists (Select Induction_ID  from T0120_HRMS_Induction_Details WITH (NOLOCK) Where Dept_Id = @Dept_Id and Schedule_Date=@Schedule_Date)   
    begin  
     set @Induction_ID = 0  
     Return  
    end  
...
ELSE IF @Tran_type = 'U'
	...
	--added by aswini 19082023
   IF Not Exists(Select Induction_ID  from T0120_HRMS_Induction_Details WITH (NOLOCK) Where Induction_ID = @Induction_ID)  
    Begin  
     set @Induction_ID = 0  
     Return   
    End  
