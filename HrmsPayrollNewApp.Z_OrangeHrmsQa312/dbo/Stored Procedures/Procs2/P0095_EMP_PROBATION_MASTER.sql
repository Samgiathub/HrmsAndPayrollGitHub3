...
CREATE PROCEDURE [dbo].[P0095_EMP_PROBATION_MASTER]
		...
		--select @Final_Approver,@Is_Fwd_Leave_Rej
			...
			IF @Final_Approver = 1 AND @Probation_Evaluation_ID <> 0 AND (@Probation_Status = 0 OR (@Flag_Trainee_Prob = 'Trainee' AND @Probation_Status = 2))
				BEGIN
					...
					--select @lnewdate,@lday,@New_Probation_EndDate,@Confirmation_date,@Branchid,@lSalaryCycleDate
					...
					begin							
						...
						if DAY(Day(DateADD(DAY,15,@lSalaryCycleDate))) > DAY(@Confirmation_date)
						BEGIN
							if Month(@Confirmation_date) = 1
							BEGIN								
								Set @lnewdate = cast(Year(@Confirmation_date)-1 as varchar(10)) + '-' + case when Month(@Confirmation_date) = 1 then '12' else Cast((Month(@Confirmation_date) - 1) as varchar(02)) END + '-' + cast(DAY(@lSalaryCycleDate) as varchar(2))
							END
							ELSE
							BEGIN								
								set @lnewdate = cast(Year(@Confirmation_date) as varchar(10)) + '-' + Cast((Month(@Confirmation_date) - 1) as varchar(02)) + '-' + cast(DAY(@lSalaryCycleDate) as varchar(2))
							END
						END
						ELSE
						BEGIN							
							--Select @lSalaryCycleDate
							set @lnewdate = cast(Year(@Confirmation_date) as varchar(10)) + '-' + Cast(Month(@Confirmation_date) as varchar(02)) + '-' + cast((DAY(@lSalaryCycleDate)) as varchar(2))
						END
						--select @lnewdate
					end
					else
