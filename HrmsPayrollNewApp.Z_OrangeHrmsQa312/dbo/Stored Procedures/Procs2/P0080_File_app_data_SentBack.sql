...
CREATE PROCEDURE [dbo].[P0080_File_app_data_SentBack]
  @File_App_Id int
As
		...
		,isnull(f.File_Type_Number,'')as File_Type_Number --added by mansi 23-08-22
		...
		left join	( SELECT MAX(Rpt_Level) AS Rpt_Level, File_App_Id 
				FROM dbo.T0115_File_Level_Approval WITH (NOLOCK)  GROUP BY File_App_Id
								...
								Qry.File_App_Id = f.File_App_Id 
		where H_F_StatusId=5 and FH_ID=(select Max(FH_ID) from T0115_File_Level_Approval_History where File_App_Id=@File_App_Id)
