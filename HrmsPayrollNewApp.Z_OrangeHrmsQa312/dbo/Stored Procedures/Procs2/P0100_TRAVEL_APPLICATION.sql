...
CREATE PROCEDURE [dbo].[P0100_TRAVEL_APPLICATION]  
  @Travel_Application_ID NUMERIC(18,0) OUTPUT  
 ,@Cmp_ID    NUMERIC(18,0)  
 ,@Emp_ID    NUMERIC(18,0)  
...
SET ARITHABORT ON  
 ...
 -- Begin  
 --  Select @Travel_Application_ID = ISNULL(MAX(Travel_Application_ID),0) + 1 From T0100_TRAVEL_APPLICATION  
 ...
 -- Deepal Add to check the multiple type Scheme :- 08022022  
  --Declare @SchDetail as numeric(18,0) = 0  
  --select @SchDetail = Scheme_Id from V0050_Scheme_Detail  VS  
  --inner join T0040_Travel_Type TT on tt.Travel_Type_Name = VS.Travel  
  --where  tt.Travel_Type_Id = @TravelTypeId and VS.Cmp_id = @Cmp_ID  
 -- Deepal Add to check the multiple type Scheme  
 ...
 --  on SD.Dyn_Hier_Id = DY.DynHierColId   
 --  --and sd.Scheme_Id = @SchemeId   
 --  and sd.Scheme_Id = @SchDetail   
 ...
 select @S_Emp_ID = rd.R_Emp_ID from T0080_EMP_MASTER EM inner join T0090_EMP_REPORTING_DETAIL RD on Rd.Emp_ID=em.Emp_ID where EM.Emp_ID = @Emp_ID and em.Cmp_ID = @Cmp_ID and rd.Effect_Date=(select Max(Effect_Date) from T0090_EMP_REPORTING_DETAIL where Emp_ID=@Emp_ID and Effect_Date <=GETDATE()) 
