...
CREATE PROCEDURE [dbo].[P0080_File_Application_Admin_Side_29_04_22]  
...
-- Add (start)
 ...
 If @tran_type  = 'I'  
  ...
  if exists (Select File_App_Id   from T0080_File_Application WITH (NOLOCK) Where File_App_Id = @File_App_Id )   
	...
	Select @File_App_Id = ISNULL(MAX(File_App_Id),0) + 1 From T0080_File_Application WITH (NOLOCK)

    INSERT INTO T0080_File_Application (File_App_Id,Application_Date,Branch_Id,Dept_Id,Desig_Id,Emp_Id,S_Emp_Id,File_Number,
	--F_StatusId,
	F_TypeId,Subject,Description,Process_Date,File_App_Doc,Cmp_ID,CreatedDate,[User ID])
             VALUES(@File_App_Id,@ApplicationDate,@Branch_Id,@Dept_Id,@Desig_Id,@Emp_Id,@S_Emp_Id,@FileNumber,
			 --@FileStatus_Id,
		...
		if exists	(	select File_App_Id 
			...
			begin 
					 ...
					 where File_App_Id = @File_App_Id  
						...
						declare @file_nm varchar(max)=(select File_App_Doc from T0080_File_Application where file_app_id=@File_App_Id)
