...
-- =============================================
CREATE PROCEDURE [dbo].[P0050_CANTEEN_Report_4_Summary_CanteenNamewise] 
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
...
SET ARITHABORT ON
		...
		--CA.Duration  ,CA.From_Date  ,CA.To_Date  ,CM.Cnt_Name  ,CA.App_No,CA.User_ID,IM.Device_Name,
		--GM.Grd_Name , CD.Amount,isnull(CD.Subsidy_Amount,0) as Subsidy_Amount,CD.Total_Amount,
		--isnull(CA.[Description],'') as [Description],
		--CA.App_Type,isnull(RM.Reason_Name,'') as Guest_Type,isnull(CA.Guest_Name,'') as Guest_Name,
		----isnull(CA.Guest_Count,0) * (DATEDIFF(DAY,Convert(varchar,@From_Date,112),Convert(varchar,@To_Date,112)) + 1) as Guest_Count
  ...
  --          CASE WHEN CA.To_Date > @To_Date THEN @To_Date ELSE CA.To_Date END) + 1)
		...
		--LEFT OUTER JOIN T0050_CANTEEN_DETAIL CD with (NoLock) on CD.Cnt_Id = CM.Cnt_Id and Q_I.Grd_ID = CD.grd_id
		----inner join (Select max(Effective_Date),Cnt_Id from T0050_CANTEEN_DETAIL )Q_C on CD.Cnt_Id = Q_C.Cnt_Id
  ...
  --      --(CA.From_Date <= @To_Date AND CA.To_Date >= @From_Date) OR
  --      --(CA.From_Date BETWEEN @From_Date AND @To_Date) OR
  --      --(CA.To_Date BETWEEN @From_Date AND @To_Date)
		--)
		...
		begin
			...
			SELECT E.EMP_ID, E.ALPHA_EMP_CODE, E.EMP_FULL_NAME, E.BRANCH_NAME, E.DEPARTMENT, E.DESIGNATION, T.Grd_Name AS Grade,
						...
						GROUP BY  
							E.EMP_ID, E.ALPHA_EMP_CODE, E.EMP_FULL_NAME, E.BRANCH_NAME, E.DEPARTMENT, E.DESIGNATION, T.Grd_Name,
							T.App_Type, T.Amount, T.Subsidy_Amount,T.Canteen_Name,E.Mobile_No
		...
		begin
			...
			GROUP BY  
				E.EMP_ID, E.ALPHA_EMP_CODE, E.EMP_FULL_NAME, E.BRANCH_NAME, E.DEPARTMENT, E.DESIGNATION, T.Grd_Name,
				T.App_Type, T.Canteen_Name, T.Amount, T.Subsidy_Amount,E.Mobile_No -- Include Amount and Subsidy_Amount in the GROUP BY clause
