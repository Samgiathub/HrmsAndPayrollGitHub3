...
CREATE PROCEDURE [dbo].[P0080_File_Application_Admin_Side]  
...
-- Add (start)
 ...
 If @tran_type  = 'I'  
  ...
  if exists (Select File_App_Id   from T0080_File_Application WITH (NOLOCK) Where File_App_Id = @File_App_Id )   
	...
	--F_StatusId,
	F_TypeId,Subject,Description,Process_Date,File_App_Doc,Cmp_ID,CreatedDate,[User ID],File_Type_Number)
             VALUES(@ApplicationDate,@Branch_Id,@Dept_Id,@Desig_Id,@Emp_Id,@S_Emp_Id,@FileNumber,1,
			 --@FileStatus_Id,
			 @FileType_Id,@Subject,@Description,@ProcessDate,@FileName,@Cmp_ID,GETDATE(),@LoginID,@FileTypeNumber)
