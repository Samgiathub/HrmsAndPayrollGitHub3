...
CREATE PROCEDURE [dbo].[P0080_File_Approval_ESS_Side]  
		...
		IF @Final_Approval = 1 OR (@Is_Fwd_Leave_Rej = 0 )
				...
				--print 4
					IF @Rpt_Level = 0 AND @File_App_Id = 0
						BEGIN
							   exec P0080_File_Application_Admin_Side @File_App_Id OUTPUT,@Cmp_ID,@ApplicationDate,@Branch_Id,@Dept_Id,@Desig_Id,@Emp_Id,@S_Emp_Id,@FileNumber,@FileType_Id,'I',@Subject,@Description,@ProcessDate,@FileName,@LoginID,@IP_Address,@Comment,@FileTypeNumber
							END
	...
	-- Add for audit(start)
		...
		-- Add for audit(end)
				...
				UPDATE T0115_File_Level_Approval_Forward  
					 ...
					 ,System_Date = GETDATE()
					 ,[User ID] = @LoginID
					 where File_App_Id = @File_App_Id   and Rpt_Level=@Rpt_Level  and Tran_Id=@Tran_ID--and Tran_Id=@Tran_ID
		...
		-- Add for Audit(end)	
			DELETE FROM dbo.T0080_File_Approval WHERE File_Apr_Id = @FA_Id AND File_App_Id = @File_App_Id 	
					--for History start
						INSERT INTO dbo.T0115_File_Level_Approval_History
						   (Cmp_Id,File_App_Id,File_Apr_Id,Emp_Id,H_File_Number,H_F_StatusId,H_F_TypeId,H_Subject,H_Description,H_S_Emp_Id,H_Process_Date,H_File_App_Doc,H_Forward_Emp_Id,H_Submit_Emp_Id,H_Approval_Comments,H_Review_Emp_Id,H_Reviewed_by_Emp_Id,Rpt_Level,CreatedDate,[User ID],H_Trans_Type,H_Tran_Id,Tbl_Type,File_Type_Number)
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,@FileStatus_Id,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@FileName,@Forward_Emp_Id,@Submit_Emp_Id,@Comment,@Review_Emp_Id,@Reviewed_by_Emp_Id,@Rpt_Level,getDate(),@LoginID,@tran_type,@Tran_ID,'T0080_File_Approval',@FileTypeNumber)	
					--for History end
		-- Add for Audit(start)
		 exec P9999_Audit_Trail @Cmp_ID,@Tran_Type,'File Approval',@OldValue,@Emp_ID,@LoginId,@IP_Address
		 -- Add for Audit(end)	
		...
		-- Add for Audit(end)
		 UPDATE dbo.T0080_File_Application SET F_StatusId = 1 WHERE File_App_Id=@File_App_Id
		 	--for History start
						INSERT INTO dbo.T0115_File_Level_Approval_History
						   (Cmp_Id,File_App_Id,File_Apr_Id,Emp_Id,H_File_Number,H_F_StatusId,H_F_TypeId,H_Subject,H_Description,H_S_Emp_Id,H_Process_Date,H_File_App_Doc,H_Forward_Emp_Id,H_Submit_Emp_Id,H_Approval_Comments,H_Review_Emp_Id,H_Reviewed_by_Emp_Id,Rpt_Level,CreatedDate,[User ID],H_Trans_Type,H_Tran_Id,Tbl_Type,File_Type_Number)
					VALUES (@Cmp_ID,@File_App_Id,0,@Emp_ID,@FileNumber,@FileStatus_Id,@FileType_Id,@Subject,@Description,@S_Emp_Id,@ProcessDate,@FileName,@Forward_Emp_Id,@Submit_Emp_Id,@Comment,@Review_Emp_Id,@Reviewed_by_Emp_Id,@Rpt_Level,getDate(),@LoginID,@tran_type,'U','T0080_File_Application',@FileTypeNumber)	
					--for History end
		 -- Add for Audit(start)
		 exec P9999_Audit_Trail @Cmp_ID,'U','File Application',@OldValue_app,@Emp_ID,@LoginId,@IP_Address
		 	-- Add for Audit(end)
