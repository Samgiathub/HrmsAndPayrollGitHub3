...
create PROCEDURE [dbo].[P0060_Appraisal_EmployeeKPA_bkp_Deepali_18042024] 
		...
		Begin
				   ...
				   if @KPA_Content <>''
					 ...
					 -- IF NOT EXISTS(SELECT 1 FROM T0060_Appraisal_EmployeeKPA WHERE KPA_Content=@KPA_Content AND Remarks=@Remarks AND Cmp_Id=@Cmp_Id AND EMP_ID=@Emp_Id and Approval_Level=@Approval_Level)
						--BEGIN
							...
							(Emp_KPA_Id,Cmp_Id,Emp_Id,KPA_Content,KPA_Target,KPA_Weightage,Effective_Date,status,KPA_Type_ID,KPA_Performace_Measure,Completion_Date,Attach_Docs,Approval_Level,KPA_InitiateId,Remarks, EMP_Comment ,RM_Comment, Is_Active,SrNo,SystemDate,UserId)  --sep 16 2016  
							VALUES(@Emp_KPA_Id,@Cmp_Id,@Emp_Id,@KPA_Content,@KPA_Target,@KPA_Weightage,@Effective_Date,@status,@KPA_Type_ID,@Performance_Measure,@Completion_Date,@Attach_Docs,@Approval_Level,@KPA_InitiateId,@Remarks,@Remarks,@MngrRemarks,@Is_Active,@SrNo,GetDate(),@User_Id) 
			...
			Else if @uploadtype = 2 --added on 30 Jun 2015
				...
				--Added By Mukti(end)09112016		
					...
					Values(@Emp_KPA_Id,@Cmp_Id,@desig_id,Convert (Nvarchar,@KPA_Content),Convert (Nvarchar,@KPA_Target),@KPA_Weightage,@Dept_Id,@Effective_Date,@KPA_Type_ID,Convert (Nvarchar,@Performance_Measure),@Completion_Date,@Attach_Docs) 
				end
		End		
	Else if @tran_type = 'U'
 		begin
