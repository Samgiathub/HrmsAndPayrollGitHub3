...
CREATE PROCEDURE [dbo].[P0090_HRMS_RESUME_BANK]
...
SET ARITHABORT ON
	...
	If @Tran_Type   = 'I'
		Begin
				--If Exists(Select Resume_Bank_Id From T0090_HRMS_RESUME_BANK WITH (NOLOCK)  Where Cmp_ID = @Cmp_ID and Resume_Bank_Id = @Resume_Bank_Id) commented by aswini
				If Exists(Select Resume_Bank_Id From T0090_HRMS_RESUME_BANK WITH (NOLOCK)  Where Cmp_ID = @Cmp_ID and Resume_ID = @Resume_ID)
					begin
	...
	Else if @Tran_Type  = 'U'
		begin 
				--If Exists(Select Resume_Bank_Id From T0090_HRMS_RESUME_BANK WITH (NOLOCK) Where Cmp_ID = @Cmp_ID and Resume_Bank_Id = @Resume_Bank_Id and Resume_ID = @Resume_ID) --commented by aswini 25092023
