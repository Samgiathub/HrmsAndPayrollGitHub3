...
CREATE PROCEDURE [dbo].[P0100_LEAVE_CF_DETAIL]
...
SET ARITHABORT ON
	...
	ELSE IF @Leave_CF_Type = 'Monthly'
		BEGIN
			--IF @Is_Advance_Leave_Balance = 1 Commented by mehul on 06012023
				--SET @CF_For_Date = @CF_From_Date	
				
			IF @Is_Advance_Leave_Balance = 1 --Added by mehul on 06012023 
			Begin
				SET @CF_For_Date = @CF_From_Date			
			End
			Else
			Begin
				SET @CF_For_Date = @CF_To_Date			
			End
		END
	...
	If @CF_Type <> 'Daily (On Present Day)' and @CF_Type <> 'Daily (Fix)' ----------- Add By Jignesh Patel 09-Sep-2021------
	Begin
	...
	DECLARE @Max_CF_From_Last_Yr_Balance numeric(18,2)
	Declare @Leave_Closing numeric(18,2)
	DECLARE @CF_Laps_Days numeric(18,2)
	DECLARE @Max_CF_From_Last_Yr_Balance_Leave_Detail numeric(18,2)
	...
	ELSE IF @Max_CF_From_Last_Yr_Balance >0 And @Max_CF_From_Last_Yr_Balance < @Leave_Closing AND @Leave_CF_Type = 'Monthly' --Added By Jimit 18042019
		BEGIN	 
				 ...
				 FROM	T0040_TYPE_MASTER T WITH (NOLOCK) INNER JOIN 
					...
					IF @Is_Advance_Leave_Balance = 1  --Added by Ronakk on 21012023 
					Begin
						...
						FROM	T0140_LEAVE_TRANSACTION LT WITH (NOLOCK) Inner Join 
							(
								SELECT	Max(For_Date) AS For_Date 
								FROM	T0140_LEAVE_TRANSACTION WITH (NOLOCK)
								WHERE	Emp_ID=@Emp_Id AND Leave_ID=@Leave_ID AND For_Date < dateadd(MONTH,1,@CF_From_Date) 
							) Qry on LT.For_Date = Qry.For_Date
							WHERE	Emp_ID = @Emp_Id AND Leave_ID = @Leave_ID	
					...
					Begin
						...
						FROM	T0140_LEAVE_TRANSACTION LT WITH (NOLOCK) Inner Join 
								(
									SELECT	Max(For_Date) AS For_Date 
									FROM	T0140_LEAVE_TRANSACTION WITH (NOLOCK)
									WHERE	Emp_ID=@Emp_Id AND Leave_ID=@Leave_ID AND For_Date < @CF_For_Date  --Change by ronakk 05042023
								) Qry on LT.For_Date = Qry.For_Date
						WHERE	Emp_ID = @Emp_Id AND Leave_ID = @Leave_ID
					...
					--If MONTH(@CF_To_Date) = @Release_MONTH  -- Old code comment deepal 09012023
					--If MONTH(@CF_To_Date) = month(cast(CONVERT(VARCHAR(7), dateADD(Month,-2,DATEADD(MONTH,(@Release_MONTH), GETDATE())), 120) + '-01' as date)) --Comment by ronakk 03042023
					If MONTH(@CF_To_Date) = case when (@Release_MONTH-1) = 0 then 12 else (@Release_MONTH-1) end  --Added by ronakk 03042023
					BEGIN
			...
			if @Reset_Flag = 1 
				begin					
					...
					Else
						...
						--select @Cmp_ID,@leave_id,@Emp_ID,@CF_For_Date_Temp,@CF_To_Date
						--return
					...
					BEGIN
						...
						if @New_Joing_Falg <> 1  --Added by nilesh patel on 27072016
							Begin
								...
								--select @Leave_CF_ID as id,@Cmp_ID,@Emp_ID,@Leave_ID as lvid,@CF_For_Date as cffor,@CF_From_Date as cffrom,@CF_To_Date as cfto,@CF_P_Days as cfp,
								--@CF_Leave_Days as  cflvdays,@CF_Type as cftype,@Leave_CompOff_Dates as lvcmpdate,@CF_Laps_Days as cflaps,
								--@Advance_Leave_Balance as advlv,@Advance_Leave_Recover_Balance as advanrecover,@Login_ID,@IsMakerChecker
							...
							INSERT INTO T0100_LEAVE_CF_DETAIL
								...
								if exists(select 1 from T0001_LEAVECF_NightProcess where emp_id = @Emp_ID and Leave_ID = @Leave_ID and CF_From_Date = @CF_From_Date and CF_To_Date = @CF_To_Date)
								BEGIN
									Delete from T0001_LEAVECF_NightProcess where emp_id = @Emp_ID and Leave_ID = @Leave_ID and CF_From_Date = @CF_From_Date and CF_To_Date = @CF_To_Date
								END
	...
	Else if @Tran_Type = 'U'
		begin
			...
			if @Reset_Flag = 1 
				...
				If @CF_Type <> 'Daily (On Present Day)' and  @CF_Type <> 'Daily (Fix)'
				Begin
		...
		else If @CF_Type = 'Daily (On Present Day)' or  @CF_Type = 'Daily (Fix)'
				Begin
					...
					Update T0100_LEAVE_CF_DETAIL				
						...
						,CF_Laps_Days= @CF_Laps_Days
						,CF_IsMakerChecker = @IsMakerChecker
						,CF_To_Date = @CF_To_Date --Added by ronakk 07102022
						where Leave_CF_ID = @Leave_CF_ID and emp_Id = @emp_Id  
	...
	Else if @Tran_Type = 'D'
		...
		if @Reset_Flag = 1 
				...
				--Added By Mukti(end)02072016
					...
					select @CF_From_Date =CF_From_Date,@CF_To_Date = CF_To_Date,@Emp_ID = Emp_ID ,@Leave_ID = Leave_ID
					from T0100_LEAVE_CF_DETAIL Where Leave_CF_ID = @Leave_CF_ID
					...
					if NOT exists(select 1 from T0100_LEAVE_CF_DETAIL where emp_id = @Emp_ID and Leave_ID = @Leave_ID and CF_From_Date = @CF_From_Date and CF_To_Date = @CF_To_Date)
					BEGIN
						--select  @Emp_ID,  @Leave_ID , @CF_From_Date , @CF_To_Date

						insert into T0001_LEAVECF_NightProcess
						...
						,Advance_Leave_Recover_balance,Is_Advance_Leave_Balance,Month,Year from T0001_LEAVECF_NightProcess_History
						where emp_id = @Emp_ID and Leave_ID = @Leave_ID and CF_From_Date = @CF_From_Date and CF_To_Date = @CF_To_Date
