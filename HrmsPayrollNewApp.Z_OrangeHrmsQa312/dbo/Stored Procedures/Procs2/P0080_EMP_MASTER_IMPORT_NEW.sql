...
CREATE PROCEDURE [dbo].[P0080_EMP_MASTER_IMPORT_NEW] 
	@Cmp_ID numeric,  
	...
	open curXml                        
		...
		Begin 
			 ...
			 BEGIN TRY
				...
				if @Basic_Salary is null --or @Basic_Salary=0.00  --AddedbyRonakb171123
				BEGIN
							INSERT INTO dbo.T0080_Import_Log VALUES (@Row_No,@Cmp_Id,@Emp_Code,'Basic Salary is required.',0,'Enter Basic Salary',GETDATE(),'Employee Master','') 
							SET @HasResult = cast(@Emp_Code as varchar(100)) + ','
							Goto ABC;
					END
		
                if @Wages_Type LIKE '%[^A-Z]%' --AddedbyRonakb171123
				BEGIN
							INSERT INTO dbo.T0080_Import_Log VALUES (@Row_No,@Cmp_Id,@Emp_Code,'Wages_Type is not propered.',0,'Enter Proper Wages_Type',GETDATE(),'Employee Master','') 
							SET @HasResult = cast(@Emp_Code as varchar(100)) + ','
							Goto ABC;
					END
               if @Salary_Basis_On LIKE '%[^A-Z]%' --AddedbyRonakb171123
				BEGIN
							INSERT INTO dbo.T0080_Import_Log VALUES (@Row_No,@Cmp_Id,@Emp_Code,'Wages_Type is not propered.',0,'Enter Proper Wages_Type',GETDATE(),'Employee Master','') 
							SET @HasResult = cast(@Emp_Code as varchar(100)) + ','
							Goto ABC;
				 ...
				 -- IF @Salary_Basis_On IS NULL 
				  ...
				  IF @Salary_Cycle IS NULL   
					SET @Salary_Cycle = '0'  
					 ...
					 --if @Is_GuestPrivilege is null     ---added by aswini 27/02/2024
			   --        set @Is_GuestPrivilege=0
				...
				Else
					BEGIN
						...
						Else
						    ...
						    END   
					END  
--commented by aswini 05/01/2023
				-- if @Restrict_Other_Master <> 0 
				--	Begin
				...
				begin
					...
					if @Grd_Name <> ''
						begin
						   --Added by aswini 2/12/2023
						   set @Grd_Name =  REPLACE(@Grd_Name,'amp;','')
				...
				if @Restrict_Other_Master <> 0
					...
					if @Dept_Name <> ''
						begin
						   --Added by aswini 2/12/2023
						   set @Dept_Name =  REPLACE(@Dept_Name,'amp;','')
				...
				if @Restrict_Other_Master <> 0
					...
					if @desig_Name <> ''
						begin
						   --Added by aswini 2/12/2023
						   set @desig_Name =  REPLACE(@desig_Name,'amp;','')
				...
				if @Restrict_Other_Master <> 0
					...
					if @Product_name <> ''
						begin
						   --Added by aswini 2/12/2023
						   set @Product_name =  REPLACE(@Product_name,'amp;','')
				...
				if @Pay_Scale_Name <> ''
						begin
						   --Added by aswini 2/12/2023
						   set @Pay_Scale_Name=  REPLACE(@Pay_Scale_Name,'amp;','')
					...
					Begin
						IF Exists(Select 1 From T0010_COMPANY_MASTER WITH (NOLOCK) Where Cmp_Id = @Cmp_ID and is_GroupOFCmp = 1)
							Begin
								...
								IF Exists(Select 1 From T0080_EMP_MASTER EM WITH (NOLOCK) Inner Join #GroupCompany GC ON EM.Cmp_ID = GC.Cmp_ID Where EM.Enroll_No = @Enroll_No AND Emp_Left_Date is null AND EM.Cmp_ID = @Cmp_ID)	-- Added by Sajid 05-12-2024 Company wise Duplicate Enroll Number Allow
									Begin 
						...
						Else 
							Begin
								IF Exists(Select 1 From T0080_EMP_MASTER EM  WITH (NOLOCK) Where EM.Enroll_No = @Enroll_No AND Emp_Left_Date is null AND EM.Cmp_ID = @Cmp_ID)	-- Added by Sajid 05-12-2024 Company wise Duplicate Enroll Number Allow
									Begin 
										INSERT INTO dbo.T0080_Import_Log VALUES (@Row_No,@Cmp_Id,@Emp_Code,'Employee Enroll No. is exists so please check it.',@Emp_Code,'Employee Enroll No. is exists so please check it.',GETDATE(),'Employee Master','')     
										SET @HasResult = cast(@Emp_Code as varchar(100)) + ',' 
										SET @Log_Status=1
