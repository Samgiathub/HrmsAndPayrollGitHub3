...
CREATE PROCEDURE [dbo].[P0115_FILE_LEVEL_APPROVAL]
...
BEGIN
  ...
  -- set @file=cast(FORMAT(GETDATE(), 'dd MM yyyy HH mm ss')as varchar)+'-'+cast(@tran_id as varchar)+'-'+cast(@cmp_id as varchar)+'-'+'FL'+'-'+'#'+cast(@fileName as varchar)
		...
		--	---for getting filename in format end '
			...
			--for adding in file_level_fwd tbl start
			if(@FileStatus_Id=3)
			begin 
			...
			--for adding in file_level_fwd tbl end
			if(@FileStatus_Id=5)
			begin 
			-- Add for audit(start)
	...
	begin
	 UPDATE T0115_File_Level_Approval  
				...
				--for updating in file_level_fwd tbl start
				 if not exists(select 1 from T0115_File_Level_Approval_Forward where File_App_Id=@File_App_Id and Cmp_Id=@Cmp_ID)
					begin 
						INSERT INTO dbo.T0115_File_Level_Approval_Forward
							   (Tran_Id,Approve_Date,File_Apr_Id,Cmp_Id,File_App_Id,Emp_Id,File_Number,F_StatusId,F_TypeId,Subject,Description,Process_Date,File_App_Doc,Forward_Emp_Id,Submit_Emp_Id,Approval_Comments,S_Emp_Id,Rpt_Level,System_Date,[User ID],Review_Emp_Id,Reviewed_by_Emp_Id,File_Type_Number )
						VALUES (@Tran_ID,@ApproveDate,0,@Cmp_ID,@File_App_Id,@Emp_ID,@FileNumber,@FileStatus_Id,@FileType_Id,@Subject,@Description,@ProcessDate,@FileName,@Forward_Emp_Id,@Submit_Emp_Id,@Comment,@S_Emp_Id,@Rpt_Level,getDate(),@LoginID,@Review_Emp_Id,@Reviewed_by_Emp_Id,@FileTypeNumber)	
