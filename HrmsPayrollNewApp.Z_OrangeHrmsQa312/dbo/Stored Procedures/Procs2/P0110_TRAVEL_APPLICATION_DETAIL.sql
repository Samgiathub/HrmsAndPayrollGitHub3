CREATE PROCEDURE [dbo].[P0110_TRAVEL_APPLICATION_DETAIL]  
...
-- Add By Mukti 11072016(end)   
 ...
 If (UPPER(@Tran_Type) = 'I' or UPPER(@Tran_Type) = 'U' or UPPER(@Tran_Type) = 'M')  
  Begin  
  --select @Cmp_ID,@Travel_App_ID,@Travel_Mode_ID,@Travel_Purpose,@From_Date,@To_Date,@State_ID,@City_ID  
   ...
   if not Exists(select distinct Travel_App_Detail_ID from T0110_TRAVEL_APPLICATION_DETAIL WITH (NOLOCK) where Cmp_ID=@Cmp_ID and Travel_App_ID=@Travel_App_ID and Travel_Mode_ID=@Travel_Mode_ID and Travel_Purpose=@Travel_Purpose and From_Date=@From_Date and To_Date=@To_Date and State_ID=@State_ID and City_ID=@City_ID)  
    Begin  
     ...
     Insert Into T0110_TRAVEL_APPLICATION_DETAIL   
      (Travel_App_Detail_ID, Cmp_ID, Travel_App_ID, Place_Of_Visit, Travel_Purpose, Instruct_Emp_ID, Travel_Mode_ID,   
       From_Date, Period, To_Date, Remarks,State_ID,City_ID,Loc_ID,Project_ID,TravelTypeId,Reason_ID,From_State_id,From_City_id)  
     ...
     where Scheme_Id = (  
       ...
       AND (SELECT distinct Travel_Type_Id from V0110_TRAVEL_APPLICATION_DETAIL where Travel_Application_ID = @Travel_App_ID) IN (select data from dbo.split(leave,'#'))  
     )   
     AND (SELECT distinct Travel_Type_Id from V0110_TRAVEL_APPLICATION_DETAIL where Travel_Application_ID = @Travel_App_ID) IN (select data from dbo.split(leave,'#'))   
     GROUP BY Scheme_Id,Dyn_Hier_Id,Leave,DynHierColValue  
      ...
      select distinct * from #TravelHyScheme  
 ...
 Else if UPPER(@Tran_Type)='U'  
  begin  
   ...
   Select distinct @Travel_App_Detail_ID = ISNULL(MAX(Travel_App_Detail_ID),0) + 1 From T0110_TRAVEL_APPLICATION_DETAIL  
   if not Exists(select Travel_App_Detail_ID from T0110_TRAVEL_APPLICATION_DETAIL where Cmp_ID=@Cmp_ID and Travel_App_ID=@Travel_App_ID and Travel_Mode_ID=@Travel_Mode_ID and Travel_Purpose=@Travel_Purpose and From_Date=@From_Date and To_Date=@To_Date and
 State_ID=@State_ID and City_ID=@City_ID and From_State_id=@From_State_ID and From_City_id=@From_City_ID)  
   Begin  
    Insert Into T0110_TRAVEL_APPLICATION_DETAIL   
     (Travel_App_Detail_ID, Cmp_ID, Travel_App_ID, Place_Of_Visit, Travel_Purpose, Instruct_Emp_ID, Travel_Mode_ID,   
      From_Date, Period, To_Date, Remarks,State_ID,City_ID,Loc_ID,TravelTypeId,Reason_ID,From_State_id,From_City_id)  
    Values (@Travel_App_Detail_ID, @Cmp_ID, @Travel_App_ID, @Place_Of_Visit, @Travel_Purpose, @Instruct_Emp_ID, @Travel_Mode_ID,  
      @From_Date, @Period, @To_Date, @Remarks,@State_ID,@City_ID,@Loc_ID,@TravelTypeId,@Reason_ID,@From_State_ID,@From_City_ID)  
   End     
  End   
