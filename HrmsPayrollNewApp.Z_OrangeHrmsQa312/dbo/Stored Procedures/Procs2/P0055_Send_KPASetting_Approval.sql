...
-- =============================================
CREATE   PROCEDURE [dbo].[P0055_Send_KPASetting_Approval]
	 @cmp_id  numeric(18,0)
...
BEGIN
	...
	FROM #FinTable F INNER JOIN
	     V0055_Hrms_Initiate_KPASetting K ON K.KPA_InitiateId = F.KPA_InitiateId  and f.Emp_id = k.Emp_Id '
	     
	IF @condition =''     
		EXEC (@query + 'where cmp_Id ='+ @cmp_id +'and k.R_EMP_ID  ='+ @app_emp_id +'') 
	ELSE
	  --  EXEC (@query + 'where cmp_Id ='+ @cmp_id +''+ @condition)

		EXEC (@query + 'where cmp_Id ='+ @cmp_id +'and k.R_EMP_ID  ='+ @app_emp_id +''+ @condition)
