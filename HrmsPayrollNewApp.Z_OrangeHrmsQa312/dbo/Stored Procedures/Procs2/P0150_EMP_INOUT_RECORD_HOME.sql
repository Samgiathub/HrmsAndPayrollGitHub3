...
CREATE PROCEDURE [dbo].[P0150_EMP_INOUT_RECORD_HOME]  
...
if @Is_Cancel_Early_Out is null
	...
	if exists(select IO_Tran_Id from T0150_EMP_INOUT_RECORD WITH (NOLOCK) where cmp_id=@cmp_id and emp_id=@emp_id and for_date=@for_date)
	 begin
		...
		-- For Getting 'Enable Night Shift Scenario for In Out' from Setting Table (Added by Divyaraj Kiri on 01/03/2024)
		Declare @SettingValue as tinyint		
		Select @SettingValue = Setting_Value from T0040_SETTING where Setting_Name = 'Enable Night Shift Scenario for In Out' and Cmp_Id = @Cmp_Id
		...
		if @SettingValue = 0
			...
			begin	
				...
				set @IO_Tran_Id = @Max_IO_Tran_Id
				update T0150_EMP_INOUT_RECORD Set reason= @Reason,Other_Reason=@Other_Reason ,Half_Full_day=@Half_Full_Day,Is_Cancel_Late_In=@Is_Cancel_Late_In where IO_Tran_Id = @Max_IO_Tran_Id		
				update T0150_EMP_INOUT_RECORD Set Is_Cancel_Early_Out=@Is_Cancel_Early_Out where IO_Tran_Id = @Max_IO_Tran_Id		
				update T0150_EMP_INOUT_RECORD set App_Date = GETDATE() where cmp_id=@cmp_id and emp_id=@emp_id and for_date=@for_date and App_Date is null	-- Added By Gadriwala 21022013		          
