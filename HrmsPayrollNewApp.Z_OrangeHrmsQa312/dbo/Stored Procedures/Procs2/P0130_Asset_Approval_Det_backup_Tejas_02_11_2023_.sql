...
create PROCEDURE [dbo].[P0130_Asset_Approval_Det_backup_Tejas(02/11/2023)]  
...
ELSE IF @Tran_type = 'U'  
 BEGIN   
  ...
  --begin  
  -- select @Transfer_ID=Transfer_ID from T0130_Asset_Approval_Det where application_type=1 and cmp_id=@cmp_id and Asset_Approval_ID=@Asset_Approval_ID  
  -- if exists(select * from T0130_Asset_Approval_Det where cmp_id=@cmp_id and Asset_Approval_ID=@Transfer_ID and AssetM_ID=@AssetM_ID)   
  --    begin    
  --     update  T0130_Asset_Approval_Det  
  --     set AssetM_ID=@AssetM_ID,  
  --      Asset_ID=@Asset_ID,  
  --      Brand_Id=@Brand_Id,  
  ...
  --     where cmp_id=@cmp_id and Asset_Approval_ID=@Transfer_ID and AssetM_ID=@AssetM_ID  
  --    end    
              
  --   if exists(select * from T0130_Asset_Approval_Det where cmp_id=@cmp_id and Asset_Approval_ID=@Asset_Approval_ID and AssetM_ID=@AssetM_ID)   
  --    begin    
  --     update  T0130_Asset_Approval_Det  
  --     set Cmp_ID=@Cmp_ID,  
  --     AssetM_ID=@AssetM_ID,  
  ...
  --   --else  
  --   -- begin  
  --   --  select @Asset_ApprDet_ID = isnull(max(Asset_ApprDet_ID),0) + 1  from T0130_Asset_Approval_Det  
  ...
  --   --  values(@Asset_ApprDet_ID,@Asset_Approval_ID1,@Cmp_ID,@Asset_ID,@Brand_Id,@Model_Name,@Serial_No,@Purchase_date,@LoginId,GETDATE(),@AssetM_ID,@Asset_Code,@Asset_Status,@Return_date,@Application_Type,@Allocation_date,null,@status,@Installment_date,@Installment_Amount,@Issue_Amount,@Sal_Tran_Id,@Deduction_type)  
  --   -- end  
              
  --   -- update T0130_Asset_Approval_Det set Transfer_Id=@Asset_Approval_ID1  
  --   -- where cmp_id=@cmp_id and Asset_Approval_ID=@Asset_Approval_ID  
  ...
  --else  
   begin   
   --select @OldAsset_Approval_ID  =ISNULL(Asset_Approval_ID,0) ,@OldAsset_ID  =ISNULL(Asset_ID,0),@OldBrand_ID  =ISNULL(Brand_Id,0),@Model_Name  =ISNULL(Model_Name,''),@OldSerialNo  =ISNULL(Serial_No,''),@OldPurchaseDate  =ISNULL(Purchase_date,''),@OldAsset_Code  =ISNULL(Asset_Code,'') From dbo.T0130_Asset_Approval_Det where Asset_ApprDet_ID = @Asset_ApprDet_ID And Cmp_ID = @Cmp_Id  
