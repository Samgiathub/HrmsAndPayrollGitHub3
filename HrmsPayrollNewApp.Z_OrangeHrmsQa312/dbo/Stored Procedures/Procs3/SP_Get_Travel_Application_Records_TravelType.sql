...
CREATE PROCEDURE [dbo].[SP_Get_Travel_Application_Records_TravelType]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
							...
							Begin
								 ...
								 If @Emp_ID_Cur > 0
									Begin
										...
										--------------------------------------- Code Commented by Yogesh on 14062023--------------------------------------------------------------------
											...
											--------------------------------------- Code Commented by Yogesh on 14062023--------------------------------------------------------------------
		--------------------------------------- Code Added by Yogesh on 14062023----- #25418---------------------------------------------------------------
											select es.Scheme_ID,sd.Leave,sd.Is_Fwd_Leave_Rej,sd.Is_RM
												,sd.Rpt_Level,sd.Leave_Days,sd.Is_RMToRM
												from T0095_EMP_SCHEME ES  inner join T0050_Scheme_Detail SD on es.Scheme_ID=sd.Scheme_Id
												where es.Emp_ID=@emp_id and sd.rpt_level = @Rpt_level	And es.Type = 'Travel'
		--------------------------------------- Code Added by Yogesh on 14062023---------#25418-----------------------------------------------------------
												...
												Else if @is_rpt_manager = 1
													Begin
														...
														If @Rpt_level = 1
															Begin
																Set @SqlQuery = 	
																'Select LAD.Travel_Application_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID
																, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +' 
																,isnull(LAD.Travel_Type_Id,0) as Travel_Type_Id From V0100_TRAVEL_APPLICATION LAD
																		Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																	Where LAD.Travel_Application_ID Not In (Select Travel_Application_ID From T0115_TRAVEL_LEVEL_APPROVAL WITH (NOLOCK)
																														Where Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
																		  + ' And ' + @Constrains
																		 -- select @SqlQuery,@Scheme_ID, @Leave, @is_rpt_manager , @is_branch_manager,@is_Reporting_To_Reporting_manager
															End
														Else
															Begin
																Set @SqlQuery = 	
													...
													BEGIN 
														  ...
														  Begin
															...
															--		inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id and LA.Leave_App_ID = VAD.Travel_Application_ID
																...
																DELETE  from #tbl_Leave_App where Travel_Type_ID not in(
																select Distinct La.Travel_Type_ID from #temp  LA 
																inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave
																inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id 
																and LA.Leave_App_ID = VAD.Travel_Application_ID)
			...
			--DELETE TA from #tbl_Leave_App TA where TA.Travel_Type_ID not in (
																	--select * from #tbl_Leave_App
																	 ...
																	 --         AND p.taskID = c.taskID
																			...
																			--select ct.* from
																			--completedTasks ct
																			--left outer join plannedTasks 
																			--pt on ct.taskId = pt.TaskId and ct.userId = pt.userId
																			--where pt.taskId is null

																			--select LA.Scheme_ID,LA.Leave_App_ID into #tmp
																			--	from #temp  LA 
																			--	inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave and la.Scheme_ID = SD.Scheme_Id
																			--	inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id and LA.Leave_App_ID = VAD.Travel_Application_ID
																				
																				--select * from #tmp where (Scheme_ID,Leave_App_ID ) not in (Select  Scheme_ID,Leave_App_ID from #tbl_Leave_App)



																			--Select L.*
																			-- from #tbl_Leave_App L right Outer join #tmp T on L.Leave_App_ID = T.Leave_App_ID and L.Scheme_ID = T.Scheme_ID
																			-- where l.Leave_App_ID is null and t.Scheme_ID is null
																			--inner Join
																			--(	
																				
																			--) Q	on L.Scheme_ID = Q.Scheme_ID and L.Leave_App_ID = Q.Leave_App_ID
																			--where Q.Scheme_ID is null and L.Scheme_ID is null
																	--SELECT *
																	----FROM #tbl_Leave_App
																	--WHERE (SCHEME_ID, LEAVE_APP_ID) NOT IN
																	--      ( SELECT SCHEME_ID, LEAVE_APP_ID
																	--        FROM #TEMP 
																	--		)
															...
															--		select LA.Scheme_ID,LA.Travel_Type_ID 
															--		from #temp  LA 
															--		inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave and la.Scheme_ID = SD.Scheme_Id
															--		inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id and LA.Leave_App_ID = VAD.Travel_Application_ID
															----)
						...
						Insert INTO #Travel
							...
							From V0100_TRAVEL_APPLICATION LAD
								left outer join (select lla.Travel_Application_Id As App_ID, Rpt_Level as Rpt_Level , lla.Approval_Status From T0115_TRAVEL_LEVEL_APPROVAL lla WITH (NOLOCK)
													inner join (Select distinct max(rpt_level) as rpt_level1, Travel_Application_ID
																	From T0115_TRAVEL_LEVEL_APPROVAL WITH (NOLOCK)
																	Where Travel_Application_ID In (Select distinct Leave_App_ID From #tbl_Leave_App)
