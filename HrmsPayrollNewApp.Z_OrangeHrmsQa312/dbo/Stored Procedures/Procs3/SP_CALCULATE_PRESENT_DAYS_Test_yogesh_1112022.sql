...
CREATE PROCEDURE [dbo].[SP_CALCULATE_PRESENT_DAYS_Test_yogesh_1112022]        
	 @Cmp_ID    numeric        
	,@From_Date   datetime        
	,@To_Date    datetime         
	,@Branch_ID   numeric        
...
AS        
		...
		BEGIN
			...
			IF @Constraint <> '' And @Constraint <> '0'
				BEGIN
					...
					FROM	#Emp_Cons EC 
							INNER JOIN T0095_INCREMENT I1 WITH (NOLOCK) ON EC.Emp_ID=I1.Emp_ID
							INNER JOIN (
			...
			ELSE
				BEGIN
					...
					WHERE	Cmp_ID=@Cmp_ID AND ISNULL(Cat_ID,0) = ISNULL(@Cat_ID ,ISNULL(Cat_ID,0))      
							...
							AND EXISTS (select Data from dbo.Split(@PBranch_ID, ',') PB Where cast(PB.data as numeric)=V_Emp_Cons.Branch_ID)
	...
	*************************************************************************/
	...
	--	END
	/*
	FOLLOWING SP EXECUTION IS ADDED BY NIMESH TO GET THE EMPLOYEE INOUT DETAIL IN #DATA TABLE
								FROM T0150_EMP_INOUT_RECORD TABLE
	*/
	--IF NOT EXISTS (SELECT * FROM #DATA D INNER JOIN #EMP_CONS E ON D.EMP_ID = E.EMP_ID)
	--BEGIN
    ...
    FROM	#EMP_GEN_SETTINGS TG WITH (NOLOCK)
	...
	--	BEGIN
			...
			select	D.Emp_Id , For_date , Duration_in_sec , In_Time , Shift_Start_Time , OUT_Time , Shift_End_Time , Working_Hrs_St_Time , Working_Hrs_End_Time into 
					#temp_In_Out_Time 
			from	#Data D INNER JOIN #EMP_GEN_SETTINGS ES WITH (NOLOCK) ON D.EMP_ID=ES.EMP_ID 
		...
		From #Data Group By For_Date,Emp_ID,Emp_Ot,Emp_OT_min_Limit,Emp_OT_Max_Limit,IO_Tran_Id

		Update	DT set In_Time=InTime, OUT_Time=OutTime 
...
--PRINT 'CALC 6 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
	...
	--If @First_In_Last_Out_For_InOut_Calculation = 0
	--	Begin
			Update	t  set Shift_End_Time = d.Shift_End_Time , Shift_Start_Time = d.Shift_Start_Time 
			from	#data d 
					left join #temp_In_Out_Time t on t.For_date = d.For_date  and t.Emp_Id = d.Emp_Id
					INNER JOIN #EMP_GEN_SETTINGS ES WITH (NOLOCK) ON D.EMP_ID=ES.EMP_ID 
			WHERE	First_In_Last_Out_For_InOut_Calculation=0
...
---- chk by sid for case when edit hours are kept editable
	...
	FROM	#Data d
	INNER JOIN dbo.T0150_EMP_INOUT_RECORD TEIR WITH (NOLOCK)
...
----chk by sid ends	
	...
	*/
	...
	from #Data d inner join  dbo.T0150_EMP_INOUT_RECORD TEIR WITH (NOLOCK)
...
*/
	...
	*/
	...
	SET		Duration_in_sec = 0,
			OT_Sec = dbo.F_Return_Without_Sec(T.OT_Sec + T.Duration_in_sec),
			P_Days = 0
	FROM	#DATA D 
			INNER JOIN (
									...
									(
										...
										AND EXISTS(SELECT 1 FROM #EMP_HOLIDAY H1 WHERE D1.EMP_ID=H1.EMP_ID AND D1.FOR_DATE=H1.FOR_DATE AND H1.IS_CANCEL=0)
												   ...
												   WHERE	D1.EMP_ID=W1.EMP_ID AND D1.FOR_DATE=W1.FOR_DATE 
															AND (W1.IS_CANCEL=0
																	OR EXISTS (SELECT 1 FROM T0100_WEEKOFF_ROSTER WR WITH (NOLOCK) WHERE WR.FOR_DATE = W1.For_Date AND WR.EMP_ID=W1.EMP_ID AND WR.is_Cancel_WO = 1)
																)
												  )		
...
UPDATE_LATE_DAYS:
	...
	-- Cliantha --			
	If Exists(Select 1 From T0140_LEAVE_TRANSACTION LT WITH (NOLOCK)
				Inner Join T0040_LEAVE_MASTER L WITH (NOLOCK) On LT.Leave_Id = L.Leave_ID And LT.Cmp_ID = L.Cmp_ID
   ...
   SET @LateEarly_Exemption_Count = 0
	...
	IF ISNULL(@LateEarly_Exemption_MaxLimit,'00:00') <> '00:00' AND @LateEarly_Exemption_Count <> 0 
		...
		BEGIN
			...
			exec rpt_Late_Early_Mark_Deduction_Details @Cmp_ID,@From_Date,@To_Date,@Branch_ID,@Cat_ID,@Grd_ID,@Type_ID,@Dept_ID,@Desig_ID,@Emp_ID,@LateEarly_Exemption_Constraint,'Extra-Exempted',0,1
...
--PRINT 'CALC 14 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
 ...
 --PRINT 'CALC 16 :' + CONVERT(VARCHAR(20), GETDATE(), 114);	   
		...
		inner join  (SELECT For_Date,Emp_ID, Sum(Leave_Used) As Leave_Used 
					FROM	(
								...
								from	dbo.T0140_LEAVE_TRANSACTION LT  WITH (NOLOCK) 
										Inner join dbo.T0040_LEAVE_MASTER LM  WITH (NOLOCK) on  LT.Leave_ID = LM.Leave_ID and isnull(LM.Default_Short_Name,'') <> 'COMP'
	...
	BEGIN
		...
		from T0140_LEAVE_TRANSACTION lt  WITH (NOLOCK) 
			inner join T0040_LEAVE_MASTER lm  WITH (NOLOCK)  on lt.Leave_ID = lm.Leave_ID and isnull(lm.Default_Short_Name,'') <> 'COMP' And Isnull(LM.Add_In_Working_Hour,0) = 0  --cliantha
		...
		from T0140_LEAVE_TRANSACTION lt  WITH (NOLOCK) 
			inner join T0040_LEAVE_MASTER lm  WITH (NOLOCK) on lt.Leave_ID = lm.Leave_ID and isnull(lm.Default_Short_Name,'') = 'COMP' And Isnull(LM.Add_In_Working_Hour,0) = 0
...
--------------------Added by Mitesh on 15/09/2011 for Shift Half Day ----------------------------
    ...
    from #Data  d inner join dbo.T0160_OT_Approval OA WITH (NOLOCK)  on d.emp_ID = Oa.Emp_ID and d.For_Date = oa.For_Date and Is_Month_Wise = 0        
  ...
  from T0040_SETTING  WITH (NOLOCK) where setting_name = 'Make Absent if Employee came in Different Shift' and Cmp_Id = @Cmp_Id
	...
	If Isnull(@Setting_Value,0) = 1
		Begin
			...
			WHERE 	Shift_Change = 1 AND NOT EXISTS (
													SELECT 1 FROM T0050_SHIFT_DETAIL SD  WITH (NOLOCK) 
	...
	While @@Fetch_Status=0
	   begin
		...
		Where I.Emp_ID = @Emp_Id_Cur
			...
			SELECT @Weekoff_Date1_Cancel = COALESCE ( @Weekoff_Date1_Cancel + ';', '') + DATA FROM dbo.Split(@Weekoff_Date1_CancelStr,';') 
			WHERE Data <> '' AND NOT EXISTS ( SELECT For_date FROM T0100_WEEKOFF_ROSTER  WITH (NOLOCK) WHERE Emp_id = @Emp_Id_Temp1 AND is_Cancel_WO = 1 AND For_date = CAST(DATA AS DATETIME ) )
				...
				begin
					...
					if @Trans_Week_OT = 1 
						begin
							...
							ELSE 
								Update	data_t Set OT_Sec = 0, Holiday_OT_Sec = OT_Sec + Holiday_OT_Sec 
								from	#Data as data_t 
								Where	data_t.For_date = @For_Date And Data_t.Emp_Id = @Emp_Id_Cur --and is_Half  = 0 
							...
							IF charindex(cast(@For_Date as varchar(11)),@Half_Holiday_Dates,0) > 0 
								BEGIN
									...
									Update	data_t Set OT_Sec = 0, Holiday_OT_Sec = @shift_Work_time_Sec, P_days = P_days - 0.5 
									from	#Data as data_t 
			...
			--	Begin
				...
				FROM	(SELECT	CASE WHEN Row = 1 
						 ...
						 FROM	(SELECT	ROW_NUMBER() OVER (ORDER BY IO_Tran_Id)
										AS Row, IOUT.*
								 FROM	T0150_EMP_INOUT_RECORD IOUT WITH (NOLOCK) 
				...
				------Added by Sid Ends.
					...
					Update D Set OT_Sec = ISNULL(@Diff_Sec,0)  --ISNULL Added By Ramiz on 01/08/2016
					FROM	#DATA D 
							INNER JOIN #EMP_GEN_SETTINGS ES ON D.EMP_ID=ES.EMP_ID 
					WHERE	First_In_Last_Out_For_InOut_Calculation=0
							AND For_date = @For_Date And D.Emp_Id = @Emp_Id_Cur
	...
	--if isnull(@Chk_otLimit_before_after_Shift_time,0) = 0 
		...
		IF @DIFF_HOUR > 0
			...
			SET		--OT_Sec = DATEDIFF(s,D.Shift_End_Time,D.OUT_Time) -- Comment by nilesh patel on 12102016 -- (ISNULL(D.Emp_OT_Max_limit,0) > 0 Condition) Added By Ramiz on 02/02/2017 as it was not taking Overtime when Max Limit is not Provided 
					OT_Sec = (CASE WHEN DATEDIFF(s,D.Shift_End_Time,D.OUT_Time) >  ISNULL(D.Emp_OT_Max_limit,0) and  ISNULL(D.Emp_OT_Max_limit,0) > 0 Then ISNULL(D.Emp_OT_Max_limit,0) ELSE DATEDIFF(s,D.Shift_End_Time,D.OUT_Time) END) - IsNull(DF.DIFF,0)
...
--PRINT 'CALC 19 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
		...
		Update d set GatePass_Deduct_Days = isnull(qry.GatePass_Deduct_Days,0) 
		From #Data d inner join 
		(
		...
		From #Data dt Inner Join(
				...
				FROM	T0165_Attendance_Approval AA  WITH (NOLOCK) 
		...
		Set @Att_Emp_ID = 0
		If Exists(SELECT 1 FROM T0165_ATTENDANCE_APPROVAL  WITH (NOLOCK) WHERE CMP_ID = @CMP_ID AND FOR_DATE >=@FROM_DATE AND FOR_DATE <=@TO_DATE AND ATT_STATUS = 'A')
			Begin
			...
			IF EXISTS(SELECT 1 FROM T0165_ATTENDANCE_APPROVAL WITH (NOLOCK)  WHERE CMP_ID = @CMP_ID AND FOR_DATE >=@FROM_DATE AND FOR_DATE <= DateAdd(Day,1,@TO_DATE) AND ATT_STATUS = 'A')
				BEGIN
					UPDATE DT
...
while @@fetch_status = 0
begin
	select @OT_Branch = Branch_ID from T0095_INCREMENT t1 inner join (select emp_id,max(Increment_ID) as Increment_ID from t0095_increment  WITH (NOLOCK) where emp_id=@OT_Emp and Increment_Effective_Date <=@To_Date group by emp_id) t2-- Ankit 12092014 for Same Date Increment
	...
	and For_Date = (select max(for_date) From T0040_General_Setting WITH (NOLOCK) where Branch_ID =@OT_Branch)  --Modified By Ramiz on 16092014 -- Modified by yogseh With No LOCK Added on 1112022
   ...
   begin
						...
						WHILE @@FETCH_STATUS = 0  
							BEGIN  
								IF NOT EXISTS(SELECT Tran_Id FROM dbo.t0160_Ot_Approval  WITH (NOLOCK) WHERE Emp_ID=@Emp_ID_W And For_Date=@For_date_W)  
									BEGIN  
										-- case when lad.half_leave_date =@For_date_W then 28920/2 else  28920 end 
		...
		INNER JOIN (SELECT	MAX(I2.Increment_ID) AS Increment_ID
					FROM	T0095_INCREMENT I2  WITH (NOLOCK) 
							INNER JOIN (SELECT	MAX(Increment_Effective_Date) AS Increment_Effective_Date, I3.EMP_ID
										FROM	T0095_INCREMENT I3  WITH (NOLOCK) INNER JOIN #Data_Temp T ON I3.Emp_ID=T.Emp_Id
		...
		INNER JOIN (SELECT	MAX(GEN_ID) AS GEN_ID
					FROM	T0040_GENERAL_SETTING G2 WITH (NOLOCK) 
							INNER JOIN (SELECT	MAX(G3.For_Date) AS FOR_DATE, G3.Branch_ID
										FROM	T0040_GENERAL_SETTING G3 WITH (NOLOCK) 
    ...
    --Commented Above code and New Code Added by Ramiz on 05/03/2016 as filters was not working in OT Approval Form
    	IF @For_OT_APPROVAL = 0
			...
			INNER JOIN T0095_INCREMENT IE  WITH (NOLOCK) ON IE.EMP_ID = QRY.EMP_ID AND IE.Increment_ID=QRY.Increment_ID
			WHERE (OT_SEC > 0    OR WEEKOFF_OT_SEC > 0 OR HOLIDAY_OT_SEC > 0) 
					...
					BEGIN 
						--IF (@Is_WD = 1)
							BEGIN
								...
								INNER JOIN  (select GS.gen_ID,GS.Branch_ID,GS.CompOff_Min_hours from T0040_General_Setting GS  WITH (NOLOCK) inner join 
													(select Branch_ID,max(For_Date) as For_Date from T0040_General_Setting WITH (NOLOCK) 
								...
								FROM	(SELECT	dt.*,
										...
										CONVERT(NVARCHAR(8),In_Time,108) AS In_Time_Actual,CONVERT(NVARCHAR(8),Out_Time,108) AS Out_Time_Actual
										 ...
										 WHERE	For_date NOT IN (
												...
												INNER JOIN T0120_CompOff_Approval AS CA WITH (NOLOCK) ON OA.Emp_Id = CA.Emp_ID AND OA.For_date = CA.Extra_Work_Date
												...
												FROM	[#Data_Temp] AS OA
												INNER JOIN T0080_EMP_MASTER AS E  WITH (NOLOCK) ON OA.Emp_ID = E.Emp_ID
												INNER JOIN T0100_CompOff_Application AS CA WITH (NOLOCK) ON OA.Emp_Id = CA.Emp_ID AND OA.For_date = CA.Extra_Work_Date
												INNER JOIN (SELECT t1.Emp_ID, t1.Branch_ID 
															FROM T0095_INCREMENT t1 WITH (NOLOCK) 
															INNER JOIN (SELECT
															  Emp_ID,
															  ...
															  AS Increment_ID
															  FROM
															  T0095_INCREMENT  WITH (NOLOCK) where cmp_ID = @cmp_ID
															  GROUP BY Emp_ID)
															  AS t2 ON t1.emp_id = t2.Emp_ID AND t1.Increment_ID = t2.Increment_ID)
														AS inc ON OA.Emp_ID = inc.Emp_ID
												INNER JOIN T0040_GENERAL_SETTING AS gs WITH (NOLOCK) ON gs.Branch_ID = inc.Branch_ID
												WHERE	(CAST(REPLACE(dbo.F_Return_Hours(OT_SEC),':', '.') AS numeric(18,3)) >= CASE
															  WHEN CAST(REPLACE(gs.CompOff_Min_hours,':', '.') AS numeric(18,3)) = 0
								...
								BEGIN          
									...
									INNER JOIN (SELECT	t1.emp_id,t1.branch_id
												...
												INNER JOIN (SELECT emp_id, MAX(Increment_ID) AS Increment_ID	-- Ankit 12092014 for Same Date Increment
														...
														AS t2 ON t1.emp_id = t2.emp_id AND t1.Increment_ID = t2.Increment_ID)
											AS inc ON OA.Emp_ID = inc.emp_id
									INNER JOIN  (select GS.gen_ID,GS.Branch_ID,GS.H_CompOff_Min_hours from T0040_General_Setting GS WITH (NOLOCK)  inner join 
													(select Branch_ID,max(For_Date) as For_Date from T0040_General_Setting WITH (NOLOCK) 
													 where cmp_ID = @Cmp_ID and For_Date <= @To_Date group by Branch_ID) qry on Qry.For_Date = GS.For_Date and Qry.Branch_ID = GS.Branch_ID) gs ON gs.branch_id = inc.branch_id
									...
									(select GS.gen_ID,GS.Branch_ID,GS.H_CompOff_Min_hours from T0040_General_Setting GS WITH (NOLOCK)  inner join 
													(select Branch_ID,max(For_Date) as For_Date from T0040_General_Setting WITH (NOLOCK) 
													 where cmp_ID = @Cmp_ID and For_Date <= @To_Date group by Branch_ID) qry on Qry.For_Date = GS.For_Date and Qry.Branch_ID = GS.Branch_ID) gs ON gs.branch_id = inc.branch_id
									WHERE	CAST(REPLACE(dbo.F_Return_Hours(Holiday_OT_Sec),':', '.') AS numeric(18,3)) >= CASE
															  WHEN CAST(REPLACE(gs.H_CompOff_Min_hours,':', '.') AS numeric(18,3)) = 0
									...
									INNER JOIN (SELECT	t1.emp_id,t1.branch_id
												...
												INNER JOIN (SELECT emp_id, MAX(Increment_ID) AS Increment_ID	
														...
														AS t2 ON t1.emp_id = t2.emp_id AND t1.Increment_ID = t2.Increment_ID)
											AS inc ON OA.Emp_ID = inc.emp_id
									INNER JOIN (select GS.For_Date,Gs.Branch_ID,H_CompOff_Min_Hours from T0040_GENERAL_SETTING GS WITH (NOLOCK)  inner join
													( 
									...
									Inner join ( select max(GP.In_Time) as In_Time,GP.emp_id,GP.For_Date,Is_Approved,Reason_id from T0150_EMP_Gate_Pass_INOUT_RECORD GP WITH (NOLOCK)  inner join #Data_Temp OA on OA.Emp_ID = GP.emp_id and OA.For_date = GP.For_date and GP.Is_Approved = 1 
									Inner join T0040_Reason_Master RM WITH (NOLOCK)  on RM.Res_Id = GP.Reason_id and Type = 'GatePass' and Gate_Pass_Type = 'Official' group by GP.Emp_ID,GP.For_Date,GP.Is_Approved,GP.Reason_id ) GPQuery on OA.emp_id = GPQuery.emp_ID and OA.For_date = GPQuery.For_date 
						...
						IF @Return_Record_set = 11
							BEGIN 
								...
								FROM T0040_General_Setting GS  WITH (NOLOCK) 
									INNER JOIN 
										( select Branch_ID,max(For_Date) as For_Date from T0040_General_Setting WITH (NOLOCK)  where cmp_ID = @Cmp_ID and For_Date <= @To_Date group by Branch_ID
									...
									BEGIN    
										...
										INNER JOIN (SELECT	t1.emp_id,t1.branch_id
													...
													INNER JOIN (SELECT emp_id,MAX(Increment_ID)AS Increment_ID	-- Ankit 12092014 for Same Date Increment
															...
															AS t2 ON t1.emp_id = t2.emp_id AND t1.Increment_ID = t2.Increment_ID)
												AS inc ON OA.Emp_ID = inc.emp_id
										INNER JOIN  (select GS.gen_ID,GS.Branch_ID,GS.W_CompOff_Min_hours from T0040_General_Setting GS WITH (NOLOCK)  inner join 
													(select Branch_ID,max(For_Date) as For_Date from T0040_General_Setting WITH (NOLOCK) 
													 where cmp_ID = @Cmp_ID and For_Date <= @To_Date group by Branch_ID) qry on Qry.For_Date = GS.For_Date and Qry.Branch_ID = GS.Branch_ID) gs  ON gs.branch_id = inc.branch_id
										...
										INNER JOIN (SELECT	t1.emp_id,t1.branch_id
													...
													INNER JOIN (SELECT emp_id,MAX(Increment_ID)AS Increment_ID
															...
															AS t2 ON t1.emp_id = t2.emp_id AND t1.Increment_ID = t2.Increment_ID)
												AS inc ON OA.Emp_ID = inc.emp_id
										INNER JOIN (select GS.For_Date,Gs.Branch_ID,W_CompOff_Min_hours from T0040_GENERAL_SETTING GS WITH (NOLOCK)  inner join
													( 
														select MAX(For_date) as For_Date,Branch_ID from T0040_General_Setting gs  WITH (NOLOCK) 
														where Cmp_ID = @Cmp_ID  and For_Date <= @To_Date group by Branch_ID
													 )Qry on Qry.Branch_ID = GS.Branch_ID and Qry.For_Date = GS.For_Date
								...
								BEGIN
									...
									INSERT	INTO #data_temp_test
											...
											FROM	(SELECT	la.Emp_ID,lad.From_Date,lad.To_Date,lad.Leave_Assign_As,lad.Half_Leave_Date, lad.Leave_Period
													 ...
													 Inner JOIN T0130_LEAVE_APPROVAL_DETAIL AS lad  WITH (NOLOCK) ON la.Leave_Approval_ID = lad.Leave_Approval_ID AND la.Cmp_ID = lad.Cmp_ID
									...
									LEFT OUTER JOIN t0100_Compoff_Application AS ca  WITH (NOLOCK) ON dtt.emp_id = ca.emp_id AND dtt.For_Date = ca.Extra_Work_Date
									LEFT OUTER JOIN T0120_CompOff_Approval as CApr WITH (NOLOCK)  on dtt.Emp_Id = CApr.Emp_ID and dtt.For_date = CApr.Extra_Work_Date -- Changed  by Gadriwala Muslim 02042015
...
--hnb									
										...
										FROM	#Data_Temp OA
												INNER JOIN dbo.T0080_emp_master E WITH (NOLOCK)  ON OA.Emp_ID = E.Emp_ID										
												INNER JOIN #Data_Gen as gs WITH (NOLOCK)  ON gs.Emp_id = OA.Emp_Id 
												INNER JOIN ( select max(GP.In_Time) as In_Time,GP.emp_id,GP.For_Date,Is_Approved,Reason_id from T0150_EMP_Gate_Pass_INOUT_RECORD GP WITH (NOLOCK)  inner join #Data_Temp OA on OA.Emp_ID = GP.emp_id and OA.For_date = GP.For_date and GP.Is_Approved = 1
												INNER JOIN T0040_Reason_Master RM WITH (NOLOCK)  on RM.Res_Id = GP.Reason_id and Type = 'GatePass' and Gate_Pass_Type = 'Official' group by GP.Emp_ID,GP.For_Date,GP.Is_Approved,GP.Reason_id ) GPQuery on OA.emp_id = GPQuery.emp_ID and OA.For_date = GPQuery.For_date 
										WHERE	CAST(REPLACE(dbo.F_Return_Hours(DATEDIFF(s,OA.in_Time,GPQuery.in_Time)),':', '.') AS numeric(18,3)) >= CASE
															  WHEN CAST(REPLACE(gs.W_CompOff_Min_hours,':', '.') AS numeric(18,3)) = 0
   ...
   begin  
	...
	--	END
		...
		FROM #Data  OA 
			INNER JOIN dbo.T0080_EMP_MASTER E WITH (NOLOCK) on OA.Emp_ID = E.Emp_ID   
			LEFT OUTER JOIN dbo.T0040_SHIFT_MASTER SM WITH (NOLOCK)  on OA.Shift_ID = SM.Shift_ID  
       ...
       */
       ...
       Else if @OT_Present =0 and @Auto_OT =1
		Begin	
			update #Data         
			set OT_Sec = isnull(Approved_OT_Sec,0)  --* 3600        
			from #Data  d inner join dbo.T0160_OT_Approval OA WITH (NOLOCK)  on d.emp_ID = Oa.Emp_ID and d.For_Date = oa.For_Date         
