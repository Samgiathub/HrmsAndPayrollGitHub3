...
--------------------------------------------------------
CREATE PROCEDURE [dbo].[SP_CALCULATE_ALLOWANCE_DEDUCTION_ROUNDING] 
...
AS                    
  ...
  while @@fetch_status = 0                    
   ...
   If @IS_NOT_EFFECT_ON_LWP = 1
    begin
		...
		set @Salary_Amount   = (@Basic_Salary  * @Salary_Cal_Day )/ @Tot_Salary_Day
		--set @Salary_Cal_Day = @Salary_Cal_Day_LWP
		...
		Begin		
				If @AD_Def_ID=3 
					Begin					
						SELECT @Allow_Amount = SUM(ISNULL(M_AD_AMOUNT,0)) From dbo.T0210_MONTHLY_AD_DETAIL WITH (NOLOCK)
						WHERE Temp_Sal_Tran_ID = @Sal_Tran_ID and Emp_ID = @Emp_ID and m_AD_Flag ='I'      
						AND AD_ID not in (select AD_ID from dbo.T0050_AD_MASTER WITH (NOLOCK) where Cmp_ID =@Cmp_ID and isnull(AD_Not_effect_salary,0) = 1)--Changed by Falak on 03-MAY-2011
    ...
    ELSE IF @varCalc_On = 'Arrears'  --Changed the Spelling from "Arears" to "Arrears" by Ramiz on 16/11/2016
    	 BEGIN
			 ...
			 SET   @Calc_On_Allow_Dedu = @Gr_Salary_amount - (@Basic_Salary  + @Allow_Amount)
    ...
    If @varCalc_On = 'Transfer OT'                     
      Begin  
			...
			select @T_amt = ISNULL(SUM(M_AD_amount),0)  FROM dbo.T0210_MONTHLY_AD_DETAIL WITH (NOLOCK)
				...
				set @T_Amt =  @OT_Sec * ((@T_Amt /@OT_Working_Day) /@Shift_Day_Sec)
				--select @T_amt
 ...
 Else if @varCalc_On='Late'                      
    ...
    if @Late_Mode ='%'  
		begin  
			set @M_AD_Amount = @M_AD_Amount * @Tmp_amount /100
		end  
    else  
		begin  
		...
		Begin
				...
				Declare @Leave_Type varchar(10)
					...
					if 	@Leave_Type='Both'
						...
						if 	@Leave_Type <> 'B'
							BEgin
								-- Changed By Gadriwala Muslim 01102014
   ...
   Else IF @varCalc_On='Present Days'
    ...
    ELSE IF @M_AD_Flag = 'I'            
		BEGIN    
			IF  @M_AD_Percentage > 0                    
			  BEGIN                    
					IF (@Calc_On_Allow_Dedu * @M_AD_Percentage / 100) > @Max_Upper AND @Max_Upper > 0                    
						BEGIN                    
	...
	BEGIN                    
     ...
     ELSE IF @Wages_type = 'Monthly'                          
       BEGIN                  
			IF @varCalc_On <> 'Arrears' --changed by Falak on 03-MAY-2011	--Changed the Spelling from "Arears" to "Arrears" by Ramiz on 16/11/2016
				SET @M_AD_Amount =  round((@M_AD_Amount * @Salary_Cal_Day)/@Tot_Salary_Day,0)
       END                  
     ELSE                     
       BEGIN                    
  ...
  END                    
	...
	IF  @M_AD_Percentage > 0                    
	 BEGIN                    
		...
		ELSE IF @Calc_On_Allow_Dedu * @M_AD_Percentage / 100 > @Max_Upper AND @Max_Upper > 0                    
			BEGIN                    
				SET @M_AD_Amount = @Max_Upper                     
			END                      
		Else                    
	 ...
	 if @M_AD_EFFECT_ON_OT = 1
		Begin
			...
			if @Wages_type ='Monthly'
				...
				set  @OT_Basic_Salary = @OT_Basic_Salary + @Day_Salary * @M_AD_Actual_Per_Amount/100
			else 
  	...
  	if @AD_Effect_Month <> '' and charindex(@StrMonth,@AD_Effect_Month) <> 0 And isnull(@AD_CAL_TYPE,'')<>'' 
			...
			End  
	/* Commented BY Falak 0n 14-MAY-2011
      ...
      if @M_AD_Amount is null
		set @M_AD_Amount =0     
					...
					VALUES     (@M_AD_Tran_ID, null,@Sal_Tran_ID,@L_Sal_Tran_ID, @Emp_ID, @Cmp_ID, @AD_ID, @From_Date, @M_AD_Percentage, @M_AD_Amount, @M_AD_Flag, @M_AD_Actual_Per_Amount,                     
                           @Calc_On_Allow_Dedu,@M_AD_NOT_EFFECT_ON_PT,@M_AD_NOT_EFFECT_SALARY,@M_AD_EFFECT_ON_OT,@M_AD_EFFECT_ON_EXTRA_DAY,0,@M_AD_effect_on_Late,0)                    
