...
CREATE PROCEDURE [DBO].[SP_EMP_INOUT_SYNCHRONIZATION_AUTO_BIRLA]
	@CMP_ID NUMERIC 
As
...
RETURN ;
	...
	SELECT	MaxDt.Emp_ID,InOut.Cmp_ID,InOut.IO_DateTime,InOut.IP_Address,--(CASE WHEN  ISNULL(InOut.In_Out_flag,'') = '' THEN 0 ELSE InOut.In_Out_Flag END) AS In_Out_flag
	...
	FROM	T9999_DEVICE_INOUT_DETAIL AS InOut
			INNER JOIN (
						...
						WHERE isnull(emp_Left,'N') <> 'Y'  
						  ...
						  And InOut.Cmp_ID  = MaxDt.Cmp_ID 
			left join T0040_IP_MASTER I on InOut.IP_Address = I.IP_ADDRESS 
	...
	ORDER BY InOut.Cmp_ID,InOut.Enroll_No,InOut.IO_DateTime
