...
CREATE PROCEDURE [dbo].[P0500_Medical_Application]
...
set @OldValue =''  
	...
	If @TransId = 'I'
		Begin 
					...
					exec P9999_Audit_get @table = 'T0500_Medical_Application' ,@key_column='App_id',@key_Values=@App_Id ,@String=@String output  
   set @OldValue = @OldValue + 'New Value' + '#' + cast(@String as varchar(max)) 
    ...
    set @OldValue = @OldValue + 'old Value' + '#' + cast(@String as varchar(max))  
					...
					exec P9999_Audit_get @table = 'T0500_Medical_Application' ,@key_column='App_id',@key_Values=@App_Id ,@String=@String output  
   set @OldValue = @OldValue + 'New Value' + '#' + cast(@String as varchar(max))   ---added by aswini
    ...
    set @OldValue = @OldValue + 'old Value' + '#' + cast(@String as varchar(max))  
		...
		exec P9999_Audit_Trail @CMP_ID,@TransId,'Medical Application',@OldValue,@Emp_id,@User_Id,@IP_Address 
	RETURN	
