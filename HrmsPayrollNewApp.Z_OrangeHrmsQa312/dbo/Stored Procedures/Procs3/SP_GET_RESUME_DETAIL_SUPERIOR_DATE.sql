...
CREATE PROCEDURE [dbo].[SP_GET_RESUME_DETAIL_SUPERIOR_DATE]    
   @Cmp_ID  Numeric (18,0)   
   ,@rec_post_id Numeric (18,0)   
   ,@Interview_Process_detail_ID Numeric (18,0)   
   ,@status  int    
   ,@emp_id   Numeric (18,0)   
   ,@from_date datetime --added By Mukti 02082014  
...
print @total  
 ...
 if @search = ''  
  begin   
   select distinct isnull(SK.SKill_detail,'#') as SKill_detail,  
   isnull(ETM.Qual_Name+':'+ETM.Specialization,'#') as Education_detail,isnull(PS.Rec_Post_Code,'#') as Job_code,Q.Resume_ID,Q.Rec_post_ID,Ps.job_title,q.Resume_Posted_date as posted_date,q.emp_first_name + ' ' + isnull(q.emp_last_name,'') as App_Full_name,datediff(yy,Date_Of_Birth,getdate()) as age,isnull(mobile_no,'#') as mobile_no,case when gender='M' then 'Male' else 'Female' end as gender,isnull(Primary_email,'#') as Primary_email,Total_Exp as Experiance_detail,  
   case when isnull(Resume_name,'#')='' then [File_Name] else isnull(Resume_name,'#') end as Resume_name,  
   case when Resume_status=0 then 'Pending' when Resume_status=1 then 'Approved' when Resume_status=2 then 'Rejected' else 'Hold' end as status,Resume_status,file_name,isnull(Resume_code,'R' +  cast(Q.cmp_id as varchar(50)) + ':' + cast(10000 + Q.resume_id as varchar(50))) as  Resume_code,  
  ...
  --case when ISNULL(Q.Location,'')='' then ps.Location else q.location end as Location          ----commented by mansi as location shows blank all time
	case when Q.Location='#' then ps.Location_Preference else q.location end as Location                 --added by mansi  for showing location
   
   ,ISNULL(Video_Resume,'')Video_Resume  
   from V0055_HRMS_RESUME_MASTER Q  
   inner join v0052_HRMS_Recruitement_Posted PS on Ps.Rec_Post_Id=@rec_post_id   
...
--commented By Mukti   --and Resume_Status= isnull(@status,Resume_Status)--resume status condition added by sneha on 12 sep 2013 to search by condition  
   ...
   where hs.rec_post_id=@rec_post_id and hs.interview_process_detail_id=@Interview_Process_detail_ID and hs.cmp_id =@cmp_id and hs.From_Date=@from_date  
  end  
 else  
  begin   
     
   select isnull(SK.SKill_detail,'#') as SKill_detail,isnull(ETM.Education_detail,'#')as Education_detail,isnull(PS.Rec_Post_Code,'#') as job_code,Q.Resume_ID,Q.Rec_post_ID,Ps.job_title,q.Resume_Posted_date as posted_date,q.emp_first_name + ' ' + isnull(
q.emp_last_name,'') as App_Full_name,datediff(yy,Date_Of_Birth,getdate()) as age,isnull(mobile_no,'#') as mobile_no,case when gender='M' then 'Male' else 'Female' end as gender,isnull(Primary_email,'#') as Primary_email,Total_Exp as Experiance_detail,  
   case when isnull(Resume_name,'#')='' then [File_Name] else isnull(Resume_name,'#') end as Resume_name, case when Resume_status=0 then 'Pending' when Resume_status=1 then 'Approved' when Resume_status=2 then 'Rejected' else 'Hold' end as status,  
   ...
   case when hs.Status=0 then 'Pending' when hs.Status=1 then 'Approved' when hs.Status=2 then 'Rejected' else 'Hold' end as Process_status,   --Added By Jaina 28-10-2016  
    --case when ISNULL(Q.Location,'')='' then ps.Location else q.location end as Location          ----commented by mansi as location shows blank all time
	case when Q.Location='#' then ps.Location_Preference else q.location end as Location                 --added by mansi  for showing location
	,ISNULL(Video_Resume,'')Video_Resume  
   from V0055_HRMS_RESUME_MASTER Q inner join  
   v0052_HRMS_Recruitement_Posted PS on Ps.Rec_Post_Id=Q.Rec_Post_Id   
   inner join t0055_HRMS_Interview_Schedule hs WITH (NOLOCK) on hs.Rec_Post_ID=@rec_post_id  and  hs.Resume_Id=q.Resume_Id and hs.Cmp_Id=ps.Cmp_id  --Added By Mukti 04082014      
   LEFT OUTER JOIN  (select * from @skill)SK ON SK.Resume_ID=Q.Resume_ID   
   LEFT OUTER JOIN  (select * from @QUALIFICATION) ETM ON ETM.Resume_ID=Q.Resume_ID   
   where Q.cmp_id=@cmp_id and Q.resume_id in (select resume_id from @Resume_MAster)   
   and hs.interview_process_detail_id=@Interview_Process_detail_ID and hs.From_Date=@from_date and hs.Resume_Id in (select resume_id from @Resume_MAster)  --Added By Mukti 04082014  
