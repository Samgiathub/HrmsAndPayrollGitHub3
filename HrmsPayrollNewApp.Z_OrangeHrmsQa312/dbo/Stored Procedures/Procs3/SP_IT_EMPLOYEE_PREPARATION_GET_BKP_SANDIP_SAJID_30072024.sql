CREATE PROCEDURE [dbo].[SP_IT_EMPLOYEE_PREPARATION_GET_BKP_SANDIP_SAJID_30072024]    
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
...
AS      
 ...
 IF @Is_IT_Declaration = 1     
  ...
  if @Regime = ''
	  BEGIN
			select @Regime = Regime from T0095_IT_Emp_Tax_Regime IT inner join #Emp_Cons Ec on IT.Emp_Id = Ec.Emp_Id  where  financial_year = CONCAT(Year(@from_date), '-', Year(@To_Date))
	  END	
  ...
  IF @Format  = 1     
   BEGIN   
	...
	BEGIN
		SELECT I_Q.* ,E.Alpha_Emp_Code as Alpha_Emp_Code,E.Emp_First_Name, ISNULL(E.EmpName_Alias_Tax,E.Emp_Full_Name) as Emp_Full_Name,Emp_superior      
			...
			,CASE WHEN ETR.Regime = 'Tax Regime 2' OR isnull(ETR.Regime ,'') = '' THEN 'New Regime' ELSE 'Old Regime' END AS Regime
	...
	BEGIN
		...
		WHERE E.Cmp_ID = @Cmp_Id --   and (AW.Amount > 160000 or AW.Amount > 190000) --and Emp_Left<>'y'    
			...
			and (Regime = 'Tax Regime 2' OR isnull(Regime,'') = '')
		ORDER BY CASE WHEN IsNumeric(e.Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + e.Alpha_Emp_Code, 20)    
			WHEN IsNumeric(e.Alpha_Emp_Code) = 0 then Left(e.Alpha_Emp_Code + Replicate('',21), 20)    
				ELSE e.Alpha_Emp_Code END    
	END
   ...
   ELSE IF @Format = 0      
	BEGIN    
		...
		Select IED.BankName,IED.Detail_1,IED.Detail_2,IED.Detail_3,IED.Date,IED.Comments,
		Em.emp_Id,Em.Alpha_Emp_Code,Emp_Full_Name,Em.Present_Street,Em.Pan_No,@Financial_Year as financial_Year,isnull(Bm.Branch_City,cm.Cmp_City) as City,Em.Father_name,   
			...
			,Qry.Rent_Amount    
				,qr.LTA  
			,case when ETR.Regime = 'Tax Regime 2' then 'New Regime' else 'Old Regime' end as Regime  
		from T0070_IT_MASTER IT WITH (NOLOCK) inner JOIN    
			T0100_IT_DECLARATION ITD WITH (NOLOCK) On ITD.IT_ID = It.IT_ID 
			INNER JOIN    
			#Emp_Cons EC On Ec.Emp_ID = ITD.EMP_ID inner JOIN    
			...
			(SELECT I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,TYPE_ID FROM T0095_Increment I WITH (NOLOCK) INNER JOIN         
				...
				I.Emp_ID = Qry.Emp_ID AND I.Increment_ID = Qry.Increment_ID  ) I_Q  ON EM.Emp_ID = I_Q.Emp_ID Left Outer JOIN    
			T0040_DESIGNATION_MASTER Dm WITH (NOLOCK) On Dm.Desig_ID = I_Q.Desig_Id INNER JOIN    
			T0030_BRANCH_MASTER Bm WITH (NOLOCK) On Bm.Branch_ID = I_Q.Branch_ID inner join   
			T0110_IT_Emp_Details IED WITH (NOLOCK) On IED.IT_ID = It.IT_ID 
			 inner JOIN    
			T0010_COMPANY_MASTER Cm WITH (NOLOCK) on cm.Cmp_Id = Em.Cmp_ID Left Outer JOIN    
				(    
				Select IsNull(ItE.Amount,0) as Rent_Amount,EC.Emp_ID    
				...
				)QRY On QRY.Emp_ID = EC.Emp_ID  Left Outer JOIN    
              
				(    
				Select EC.Emp_ID,sum(Isnull(rc.Tax_Free_amount,0)) as LTA    
				from T0210_Monthly_Reim_Detail RC WITH (NOLOCK) INNER JOIN    
					#Emp_Cons EC On Ec.Emp_ID = Rc.Emp_ID    
		...
		group by Em.emp_Id,Em.Alpha_Emp_Code,Emp_Full_Name,Em.Present_Street,Em.Pan_No,Em.City,Em.Father_name,Dm.Desig_Name,It_Name,IT.Cmp_ID,IT.IT_ID,IT_Parent_ID    
			,Bm.Branch_City,Cm.Cmp_City,QRY.Rent_Amount,qr.LTA ,ETR.Regime  ,IED.BankName ,IED.Detail_1,IED.Detail_2,IED.Detail_3,IED.Date,IED.Comments
