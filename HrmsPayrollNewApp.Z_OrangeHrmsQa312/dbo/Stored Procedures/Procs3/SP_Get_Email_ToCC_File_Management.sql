...
create PROCEDURE [dbo].[SP_Get_Email_ToCC_File_Management] 
...
SET ARITHABORT ON
   ...
   --print @Is_Manager_CC
			...
			DECLARE @DynHierId as TINYINT ---Added By Jimit 21/12/2017
                ...
                Begin   
                    ...
                    If @Is_Manager_CC = 1
                        Begin
                            If (@Module_Name = 'Leave Application' Or @Module_Name = 'Leave Approval') And @Final_Approval = 0
                                ...
                                End
							 else If (@Module_Name = 'File Application' Or @Module_Name = 'File Approval') And @Final_Approval = 0
                                Begin
                                    insert into #Temp_CC
                                        Select distinct(Work_Email) From T0080_EMP_MASTER WITH (NOLOCK) where Emp_ID = @App_Emp_ID or Emp_ID in (SELECT emp_id from #Rpt_branch_manager)
                                End
                            ...
                            Else If @Module_Name <> 'Leave Application' And @Module_Name <> 'Leave Approval' 
										and @Module_Name <> 'File Application' --added by mansi
                    ...
                    Else
                        ...
                        --SELECT * FROM #Temp_To
                            ...
                            Else If @Module_Name <> 'Leave Application' And @Module_Name <> 'Leave Approval' 
                                    And @Module_Name <> 'File Application'  --added by mansi file
								   And @Module_Name <> 'File Approval'  --added by mansi file
								Begin
                   ...
                   If @Module_Name = 'Travel Application' And @Final_Approval = 0
                        Begin   
							...
							--ADD Comment by Deepal :- 10022022
                            --If Exists (Select App_Emp_ID From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                            --                AND Scheme_Id = (SELECT QES.Scheme_ID from T0095_EMP_SCHEME QES WITH (NOLOCK) INNER join 
                            --                                (select max(effective_date) as effective_date,emp_id from T0095_EMP_SCHEME IES WITH (NOLOCK)
                            ...
                            --                And @Leave_ID In (SELECT Cast(data AS Numeric(18, 0)) FROM dbo.Split(Leave, '#')))
                            --    Begin
							--END Comment by Deepal :- 10022022


								        --           SELECT distinct QES.Scheme_ID,tt.RptLevel from T0095_EMP_SCHEME QES WITH (NOLOCK) INNER join 
																--(select max(effective_date) as effective_date,emp_id from T0095_EMP_SCHEME IES WITH (NOLOCK)
																-- where IES.effective_date <= getdate() AND Emp_ID = @Emp_ID And Type = 'Travel'
																-- GROUP by emp_id 
																--) Tbl1 ON Tbl1.Emp_ID = QES.Emp_ID AND Tbl1.effective_date = QES.Effective_Date And Type = 'Travel'
																--inner join T0080_Travel_HycScheme TT on Qes.Scheme_ID = TT.SchemeIId   and tt.RptLevel = (@Rpt_Level + 1)
															 --   inner join T0050_Scheme_Detail Sd on SD.Dyn_Hier_Id = TT.DynHierId and sd.Scheme_Id = QES.Scheme_ID and Sd.Leave = tt.TravelTypeId

															 

							If Exists (Select App_Emp_ID From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                                            AND Scheme_Id = (
																SELECT distinct QES.Scheme_ID from T0095_EMP_SCHEME QES WITH (NOLOCK) INNER join 
																(select max(effective_date) as effective_date,emp_id from T0095_EMP_SCHEME IES WITH (NOLOCK)
																 where IES.effective_date <= getdate() AND Emp_ID = @Emp_ID And Type = 'Travel'
																 GROUP by emp_id 
																) Tbl1 ON Tbl1.Emp_ID = QES.Emp_ID AND Tbl1.effective_date = QES.Effective_Date And Type = 'Travel'
																inner join T0080_Travel_HycScheme_Email TT on Qes.Scheme_ID = TT.SchemeIId and tt.RptLevel = (@Rpt_Level + 1)
																inner join T0050_Scheme_Detail Sd on SD.Dyn_Hier_Id = TT.DynHierId and sd.Scheme_Id = QES.Scheme_ID and Sd.Leave = tt.TravelTypeId
															))
                                           -- And @Leave_ID In (SELECT Cast(data AS Numeric(18, 0)) FROM dbo.Split(Leave, '#')))
                                Begin
									...
									--ADD Comment by Deepal :- 10022022
                                    --Select @App_Emp_ID = App_Emp_ID , @is_Rm = Is_RM ,@is_Bm = Is_BM 
                                    --,@Is_RMToRm = Is_RMToRM ,@DynHierId = Dyn_Hier_Id --Added By Jimit 2112017
                                    --From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                                    --        AND Scheme_Id = (SELECT QES.Scheme_ID from T0095_EMP_SCHEME QES WITH (NOLOCK) INNER join 
                                    --                        (select max(effective_date) as effective_date,emp_id from T0095_EMP_SCHEME IES WITH (NOLOCK)
                                    --                        where IES.effective_date <= getdate() AND Emp_ID = @Emp_ID And Type = 'Travel'
                                    ...
                                    From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                                            AND Scheme_Id = (
																...
																inner join T0080_Travel_HycScheme_Email TT on Qes.Scheme_ID = TT.SchemeIId and tt.RptLevel = (@Rpt_Level + 1)
																inner join T0050_Scheme_Detail Sd on SD.Dyn_Hier_Id = TT.DynHierId and sd.Scheme_Id = QES.Scheme_ID
															)
                                            --And @Leave_ID In (SELECT Cast(data AS Numeric(18, 0)) FROM dbo.Split(Leave, '#')) 
									...
									ELSE IF @DynHierId <> 0
                                    BEGIN                         
											Select @R_Emp_Id1 = DynHierColValue from 
											T0080_DynHierarchy_Value DV inner join T0095_INCREMENT I on DV.IncrementId = I.Increment_ID 
											where DV.Emp_ID = @Emp_ID and DynHierColId= @Rpt_Level + 1
                                           If @R_Emp_Id1 <> 0
                                           BEGIN
                                                    INSERT INTO #Rpt_branch_manager
												   select @R_Emp_Id1
                            ...
                            If Exists (Select App_Emp_ID From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                                ...
                                Begin
                                    ...
                                    Select @App_Emp_ID = App_Emp_ID , @is_Rm = Is_RM ,@is_Bm = Is_BM,@Is_HOD=Is_HOD 
                                            ,@Is_RMToRm = Is_RMToRM,@DynHierId = Dyn_Hier_Id --Added By Jimit 2112017 --Added By Jimit 2112017
                                        From T0050_Scheme_Detail WITH (NOLOCK) Where Rpt_Level = (@Rpt_Level + 1)
                                            AND Scheme_Id = (SELECT QES.Scheme_ID from T0095_EMP_SCHEME QES WITH (NOLOCK) INNER join 
                                                            (select max(effective_date) as effective_date,emp_id from T0095_EMP_SCHEME IES WITH (NOLOCK)
                                                            where IES.effective_date <= getdate() AND Emp_ID = @Emp_ID And Type = 'Travel Settlement'
...
ELSE IF @APP_EMP_ID = 0  AND @IS_RMTORM = 1 AND @RPT_LEVEL = 1 --Added By Jimit 21122017
                    ...
                    If (@Module_Name = 'Exit Application' or @Module_Name = 'Exit Approval') And @Final_Approval = 0
                        ...
                        Begin
                            If (@Module_Name = 'Exit Application' or @Module_Name = 'Exit Approval') And @Final_Approval = 0
                                Begin
                                    insert into #Temp_CC
                                        Select distinct(Work_Email) From T0080_EMP_MASTER WITH (NOLOCK) 
										where Emp_ID = @App_Emp_ID or Emp_ID in (SELECT emp_id from #Rpt_branch_manager)
                                End
                        End             
                    Else
                        Begin
            ...
            If @Final_Approval = 1  -- Condition Added By Hiral 07 Aug, 2013 For Five Level Leave Approval 
                BEGIN
					...
					Begin
                            Insert into #Temp_CC
                            Select Work_Email From T0080_EMP_MASTER WITH (NOLOCK) Where Emp_ID = @Emp_ID 
							and isnull(Emp_Left_Date,GETDATE()+1) > GETDATE() --And Cmp_ID = @Cmp_ID 
    ...
    --------Added By Jimit 12122017---------------
			...
			--added by mansi start for file
			  Else If (@Module_Name = 'File Application' )   --Ankit 19052014
                Begin
			
			--select * from #Temp_To ---27-06-22
                Insert into #Temp_To
                            Select Work_Email From T0080_EMP_MASTER WITH (NOLOCK) Where Emp_ID = @Emp_ID and isnull(Emp_Left_Date,GETDATE()+1) > GETDATE() -- And Cmp_ID = @Cmp_ID
                
				End
				 Else If (@Module_Name = 'File Approval' )   --Ankit 19052014
                Begin
				select 101--27-06-22 
			--select * from #Temp_To ---27-06-22
        ...
        if @Module_Name = 'Employee Warning'
			...
			begin	
					...
					select Work_Email from T0080_EMP_MASTER EM WITH (NOLOCK)
							 where EM.cmp_id=@Cmp_id and exists (select Rpt_Mng_ID from T0300_Emp_Exit_Approval_Level 
							 EA WITH (NOLOCK) where EM.Emp_id=EA.Rpt_Mng_ID and Ea.emp_id=@Emp_ID and RPT_Level != @Rpt_Level)
					 --select 11,* from 	#Temp_CC
