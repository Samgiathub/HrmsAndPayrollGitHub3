...
-- Created by rohit For Employee Trainee Over Reminder to Hr Email on 20112013
---28/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[SP_Employee_Trainee_Reminder]
...
SET ARITHABORT ON
		...
		( 
			...
			Trainee_Period varchar(10),
			Trainee_Date Datetime,
			New_Trainee_Period Varchar(10),        
			New_Trainee_Date Datetime,
			BRANCH_ID INT,
			...
			BEGIN
				UPDATE	R
					...
					INNER JOIN T0090_EMP_REPORTING_DETAIL RM on RM.Emp_ID = R.Emp_Id AND						
						Effect_Date=(select max(effect_date) from T0090_EMP_REPORTING_DETAIL WITH (NOLOCK) where emp_id=R.Emp_ID)	
			...
			BEGIN     
				...
				DECLARE @TableHead varchar(max),
           		...
           		SET @TableHead = '<html><head>' +
								  ...
								  <tr>
									 <td align="center" valign="middle"><table width="800" border="0" cellspacing="0" cellpadding="0">
									  ...
									  <tr>
										<td width="800" height="24" align="center" valign="middle" style="background:#0b0505; border-radius: 10px 10px 10px 10px; font-family:Arial, Helvetica, sans-serif; color:#FFFFFF;  text-decoration:none; font-weight:bold; text-align:center; font-size:13px;">Employee Trainee Over Reminder</td>
									  </tr>
										  <tr>
											<td height="4" align="center" valign="middle"></td>
										  </tr>
										  <tr>
											<td width="800" align="center" valign="middle" style="border:1px solid #cacaca; border-radius: 10px 10px 10px 10px; font-family:Arial, Helvetica, sans-serif; color:#000000;  text-decoration:none; text-align:center; font-size:12px;">No Of Employee Trainee Over : [ ' + CAST(@ECount AS VARCHAR(255)) + ' ] </td>
										  </tr>
										  <tr>
								  ...
								  <table border="1" width="800" height="24" align="center" valign="middle" style="background: #FFFFF;border-color:solid black;
									...
									font-size: 12px;">' +
										  ...
										  '<td align=center><b><span style="font-size:small">Trainee Period</span></b></td>' +
										  '<td align=center><b><span style="font-size:small">Trainee End Date</span></b></td>' 
                  ...
                  SET @Body = ( SELECT  
										...
										Isnull(Trainee_Period,'-') as [TD],
										Left(Convert(varchar(25), Trainee_Date,113),11) As [TD]										
                                FROM    #Temp
                                WHERE   Cmp_ID = @Cmp_Id AND
										isnull(MANAGER_EMAIL,'') =  COALESCE(@MANAGER_EMAIL,MANAGER_EMAIL,'')
										AND IsNull(HR_EMAIL,'') =  COALESCE(@HR_EMAIL,HR_EMAIL, '')
...
--select 111,@CC_EMAIL_DETAIL,@TO_EMAIL_DETAIL
				EXEC msdb.dbo.sp_send_dbmail @profile_name = @profile, @recipients = @TO_EMAIL_DETAIL, @subject = 'Employee Trainee Over', @body = @Body, @body_format = 'HTML',@copy_recipients = @CC_EMAIL_DETAIL
