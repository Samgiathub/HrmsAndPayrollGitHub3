...
CREATE PROCEDURE [dbo].[Rpt_Salary_Register_Export]  
	@Company_id		numeric  
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric	
	...
	,@Report_type NUMERIC = 0 -- Added by Rajput on 25062018
AS  
	SET NOCOUNT ON		
	 ...
	 If @Branch_ID is null
			Begin 
				select Top 1 @Sal_St_Date  = Sal_st_Date,@manual_salary_period=isnull(Manual_Salary_Period ,0) 
				  from T0040_GENERAL_SETTING where cmp_ID = @Company_id    
				  and For_Date = ( select max(For_Date) from T0040_GENERAL_SETTING where For_Date <=@To_Date and Cmp_ID = @Company_id)    
			End
		Else
			Begin
				select @Sal_St_Date  =Sal_st_Date,@manual_salary_period=isnull(Manual_Salary_Period ,0) 
				  from T0040_GENERAL_SETTING where cmp_ID = @Company_id and Branch_ID = @Branch_ID    
				  and For_Date = ( select max(For_Date) from T0040_GENERAL_SETTING where For_Date <=@To_Date and Branch_ID = @Branch_ID and Cmp_ID = @Company_id)    
			End
 ...
 Create table #Emp_Cons 
	...
	(
	   ...
	   ,PinCode		Varchar(20) --Added by Hardik 19/06/2017 for Aculife
	   ,Father_Name Varchar(500)
	   ,Emp_Left_Date VARCHAR(50)	
	)

	DECLARE @OrdinalPosition INT
	Select	@OrdinalPosition  = max(c.column_id)
	From	tempdb.sys.columns c 						
	Where	c.object_id=OBJECT_ID('tempdb.dbo.#CTCMast') and c.name = 'PinCode'
	...
	SELECT e.Cmp_ID,e.Emp_ID,e.Emp_code,e.Alpha_Emp_Code,
		...
		CT.Cat_Name as Category,VT.Vertical_Name,ST.SubVertical_Name,SB.SubBranch_Name,
		e.Aadhar_Card_No,
		...
		0,0,0
		 ...
		 ,BM.Branch_ID, e.Present_City, e.Present_Post_Box,e.Father_name,e.Emp_Left_Date
		    
		from T0080_EMP_MASTER e	
		--inner join
		--( select I.Emp_id,I.Basic_Salary,I.CTC,I.Inc_Bank_AC_No,Payment_Mode,I.Branch_ID,I.Grd_ID,I.Dept_ID,I.Desig_Id,I.Type_ID,I.Cat_ID,I.Vertical_ID,I.SubVertical_ID,I.subBranch_ID,I.Segment_ID,I.Center_ID from T0095_Increment I inner join 
		...
		INNER JOIN	
			( 
				select	I.Emp_id,I.Basic_Salary,I.CTC,I.Inc_Bank_AC_No,Payment_Mode,I.Branch_ID,I.Grd_ID,I.Dept_ID,I.Desig_Id,I.Type_ID,I.Cat_ID,I.Vertical_ID,
						I.SubVertical_ID,I.subBranch_ID,I.Segment_ID,I.Center_ID 
			...
			)Inc_Qry ON Inc_Qry.Emp_ID = E.Emp_ID
	inner join #Emp_Cons ec on e.Emp_ID = ec.Emp_ID
	left outer join T0030_BRANCH_MASTER bm on Inc_Qry.Branch_ID = bm.Branch_ID
	left outer join T0040_GRADE_MASTER ga on Inc_Qry.Grd_ID = ga.Grd_ID
	left outer join T0040_DEPARTMENT_MASTER dm on Inc_Qry.Dept_ID = dm.Dept_Id
	left outer join T0040_DESIGNATION_MASTER dnm on Inc_Qry.Desig_Id = dnm.Desig_ID
	left outer join T0040_TYPE_MASTER tm on Inc_Qry.Type_ID = tm.Type_ID
	left outer join T0030_CATEGORY_MASTER CT on CT.Cat_ID=Inc_Qry.Cat_Id
	left outer join T0040_Vertical_Segment VT on VT.Vertical_ID=Inc_Qry.Vertical_ID
	left outer join T0050_SubVertical ST on ST.SubVertical_ID=Inc_Qry.SubVertical_ID
	left outer join T0050_SubBranch SB on SB.SubBranch_ID=Inc_Qry.subBranch_ID 
	left outer join T0040_Business_Segment BSG on BSG.Segment_ID=Inc_Qry.Segment_ID
	left outer join T0040_Cost_Center_Master CC on CC.Center_ID = Inc_Qry.Center_ID
	inner join T0200_MONTHLY_SALARY MS on Ms.Emp_ID = Ec.Emp_Id and 
			   MONTH(month_end_date) = month(@To_Date) and YEAR(month_End_Date) = year(@To_Date) 
	           --and ISNULL(Is_FNF,0) = 0 COMMENTED BY RAJPUT ON 26062018 FNF EMPLOYEE ALSO SHOULD COME 
	
	Union ALL
		SELECT 0,0,-999,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,0,NULL,NULL,NULL,NULL,0,0,0,0,0,NULL,NULL,NULL,NULL
	...
	DECLARE Leave_Cursor CURSOR FOR
		...
		*/
		...
		Set @val = '';
		Set @val = @val + 'Alter table  #CTCMast Add Total_Paid_Leave_Days numeric(18,2)  CONSTRAINT CTCMast_Total_Paid_Leave_Days default 0; 
						   Alter table  #CTCMast Add Total_Leave_Days numeric(18,2)  CONSTRAINT CTCMast_Total_Leave_Days default 0;
						   Alter table  #CTCMast Add Late_Days numeric(18,2) CONSTRAINT CTCMast_Late_Days default 0;
						   Alter table  #CTCMast Add Early_Days numeric(18,2)  CONSTRAINT CTCMast_Early_Days default 0'
		exec (@val);
		...
		Select @cnt = count(*) from T0200_MONTHLY_SALARY ms inner join
...
-----ended------
	...
	--added by jimit 27/03/2017
		...
		SET @val_update = ''
		SET @val_update = 'UPDATE  #CTCMast SET Basic_salary = -413036 Where Emp_Code = -999;'
		SET @val_update = @val_update + 'UPDATE  #CTCMast SET Settl_Salary = -413036 Where Emp_Code = -999;'
		SET @val_update = @val_update + 'UPDATE  #CTCMast SET Other_Allow = -413036 Where Emp_Code = -999;'
		exec(@val_update)
	
		Update CM
		--Set Actual_CTC = (Qry.CTC * (datediff(mm,@From_Date,@To_Date) + 1)) 
		Set Actual_CTC = case when Day(@From_date) > 1 and Day(@From_date) < 30 then   QRY.CTC*(datediff(mM,@From_Date,@To_Date)) else QRY.CTC*(datediff(mM,@From_Date,@To_Date)+1) END -- Deepal 27052024 Salary cycle CTC multiple issue raise in Chasswalla
		from #CTCMast CM 
	...
	DECLARE Allow_Dedu_Cursor CURSOR FOR
		...
		OPEN Allow_Dedu_Cursor
			...
			while @@fetch_status = 0
				Begin
					...
					SET @val_update = @val_update+ 'UPDATE  #CTCMast SET [' + REPLACE(@AD_NAME_DYN,' ','_') + '] = -413036 Where Emp_Code = -999;'
				fetch next from Allow_Dedu_Cursor into @AD_NAME_DYN,@AD_LEVEL
				End
		close Allow_Dedu_Cursor	
		deallocate Allow_Dedu_Cursor
		...
		*/
		...
		Set @val = ''
		Set @val = 'UPDATE  #CTCMast SET Production_Bonus = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Leave_Encash_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Uniform_Refund_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET OT_Rate = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET OT_Hours = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET OT_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET M_HO_OT_Hours = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET M_HO_OT_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET M_WO_OT_Hours = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET M_WO_OT_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET WO_HO_Fix_OT_Rate = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Travel_Amount = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Basic_Arrear = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Total_Earning = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Total_Earning_Arrear = -413036 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Gross_Salary = -413036 Where Emp_Code = -999;

					UPDATE  #CTCMast SET PT_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Advance_Amount = -888193 Where Emp_Code = -999;'
		exec (@val)	
	...
	--added by jimit 05042017
		...
		(
			SELECT Inc_Qry.Fix_OT_Hour_Rate_WO_HO,e.Emp_ID,e.Cmp_ID
				from T0080_EMP_MASTER e	inner join
				( select I.Emp_id,I.Fix_OT_Hour_Rate_WD,I.Fix_OT_Hour_Rate_WO_HO from T0095_Increment I inner join 
	...
	Declare @Loan_name varchar(100)
		...
		DECLARE Loan_Cursor CURSOR FOR
				SELECT LM.LOAN_NAME,LA.LOAN_ID FROM T0210_MONTHLY_LOAN_PAYMENT MLP INNER JOIN 
				T0120_LOAN_APPROVAL LA ON MLP.LOAN_APR_ID=LA.LOAN_APR_ID INNER JOIN
		...
		OPEN Loan_Cursor
			...
			while @@fetch_status = 0
				Begin
					...
					SET @val_update = @val_update+ 'UPDATE  #CTCMast SET [' + REPLACE(@Loan_name,' ','_') + '] = 0; '
					SET @val_update = @val_update+ 'UPDATE  #CTCMast SET [' + REPLACE(@Loan_name,' ','_') + '] = -888193 Where Emp_Code = -999;'
	...
	--Bond Deduction Portion Starts from Here--- Added By Ramiz 
		SET @val = 'Alter table  #CTCMast Add Bond_Amount numeric(18,2) default 0 not null'
		SET @val_update = 'UPDATE  #CTCMast SET Bond_Amount = -888193 Where Emp_Code = -999;'
		exec(@val)	
		exec(@val_update)
	...
	----------------ended------------------
		...
		--M_HO_OT_Amount
		UPDATE	T
		SET		M_HO_OT_Amount = MS.M_HO_OT_Amount,
				M_WO_OT_Amount = MS.M_WO_OT_Amount
		FROM	#CTCMast T
				INNER JOIN (SELECT	EMP_ID, SUM(MS.M_HO_OT_Amount) AS M_HO_OT_Amount, SUM(MS.M_WO_OT_Amount) AS M_WO_OT_Amount
							from	T0200_MONTHLY_SALARY MS
							WHERE	MS.MONTH_END_DATE BETWEEN @FROM_DATE AND @TO_dATE
							GROUP BY EMP_ID) MS ON T.EMP_ID=MS.EMP_ID
		...
		OPEN Allow_Dedu_Cursor
			...
			while @@fetch_status = 0
				Begin
					...
					SET @val_update = @val_update+ 'UPDATE  #CTCMast SET [' + REPLACE(@AD_NAME_DYN,' ','_') + '] = -888193 Where Emp_Code = -999;'
				fetch next from Allow_Dedu_Cursor into @AD_NAME_DYN,@AD_LEVEL
				End
	close Allow_Dedu_Cursor	
	deallocate Allow_Dedu_Cursor
	...
	/*
		...
		Set @val = ''
		Set @val = 'UPDATE  #CTCMast SET Revenue_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET LWF_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Other_Deduction = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Gate_Pass_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Asset_Installment_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Travel_Advance_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Uniform_Dedu_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Late_Deduction_Amount = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Arear_Deduction = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Total_Deduction = -888193 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Net_Total_Deduction = -888193 Where Emp_Code = -999;
					
					UPDATE  #CTCMast SET Net_Amount = -9612674 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Net_Round = -9612674 Where Emp_Code = -999;
					UPDATE  #CTCMast SET Total_Net = -9612674 Where Emp_Code = -999;'
	...
	*/
	...
	Set @val = ''
	Set @val = 'Alter table  #CTCMast Add Salary_Status Varchar(10) default 0 not null;
				Alter table  #CTCMast Add Is_FNF numeric default 0 not null;' 
	exec (@val)
	--- END ---
					...
					if @Summary='0'
						...
						if @Type='1'
						   Begin
								...
								SUM(CM.Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(CM.Production_Bonus) as Production_Bonus,SUM(CM.Leave_Encash_Amount) as Leave_Encash_Amount,SUM(CM.Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(CM.Other_Allow)as Other_Allowance, SUM(CM.Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CM.Actual_CTC) as CTC,SUM(cm.advance_amount)as Advance_Amount,SUM(CM.PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(cm.ot_rate)as OT_Rate,SUM(cm.ot_hours)as OT_Hours,SUM(Cm.M_WO_OT_Hours) as WeekOff_OT_Hours,Sum(cm.M_HO_OT_Hours) as Holiday_OT_Hours,SUM(cm.ot_amount)as OT_Amount,SUM(cm.WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(cm.revenue_amount) as Revenue_Amount,SUM(cm.lwf_amount)as LWF_Amount,SUM(isnull(cm.Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(cm.Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(cm.Uniform_Dedu_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Cm.Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
								...
								set @String = @String + ' from #CTCMast CM 
											 group By Branch ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +''  --Where Sal_Cal_Day <> 0
								--select @String
								exec(@String)
						   End
						Else
						...
						if @Type='1'
						   Begin
								...
								set @String = @String + ' from #CTCMast CM 
									 group By Grade ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
								--select @String
								exec(@String)
						   End
						Else
						...
						begin
							if @Type='1'
								   Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Category ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
						...
						if @Type='1'
						   Begin
								...
								DECLARE @Static_Column_1 Varchar(Max)
								Set @Static_Column_1 = ', '''' as Father_Name,'''' as Date_Of_Join,'''' as Emp_Left_Date,'''' as PF_NO,'''' as Pan_No,'''' as Bank_Ac_No,'''' as Inc_Bank_AC_No,'''' as Aadhar_Card_No,'''' as Branch,'''' as Designation,'''' as Grade,'''' as TypeName,'''' as Category,'''' as sub_vertical,'''' as Sub_Branch'

								
								set @String = ' select TOP 1 0 as flag ' + @Str_Emp_Name + ' CM.Department as Department ' + @Static_Column_1 + ' '+ @String_2 +''+ @String_3 +',Count(Emp_ID) As Total_Emp,
								SUM(CM.Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(CM.Production_Bonus) as Production_Bonus,SUM(CM.Leave_Encash_Amount) as Leave_Encash_Amount,SUM(CM.Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(CM.Other_Allow)as Other_Allowance, SUM(CM.Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CM.Actual_CTC) as CTC,SUM(cm.advance_amount)as Advance_Amount,SUM(CM.PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(cm.ot_rate)as OT_Rate,SUM(cm.ot_hours)as OT_Hours,SUM(Cm.M_WO_OT_Hours) as WeekOff_OT_Hours,Sum(cm.M_HO_OT_Hours) as Holiday_OT_Hours,SUM(cm.ot_amount)as OT_Amount,SUM(cm.WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(cm.revenue_amount) as Revenue_Amount,SUM(cm.lwf_amount)as LWF_Amount,SUM(isnull(cm.Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(cm.Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(cm.Uniform_Dedu_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Cm.Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
								+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(cm.net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(CM.Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
								SUM(CM.Present_Day) as Present_Days,SUM(CM.Arear_Day) as Arear_Days,SUM(CM.Absent_Day) as Absent_Day,SUM(CM.Holiday_Day)as Holiday_day,SUM(CM.WeekOff_Day)as Week_Off_Days,SUM(CM.Sal_Cal_Day)as Sal_cal_Day,SUM(cm.total_leave_days)as Total_leave_Days,SUM(CM.total_paid_leave_days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' '
								...
								set @String = @String + ' from #CTCMast CM 
								 group By Department ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +''--Where Sal_Cal_Day <> 0
								exec(@String)
						   End
						Else
							Begin
						...
						begin
							if @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Designation ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN
						...
						begin
							if @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By TypeName ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN				
						...
						begin
							IF @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Division ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN				
						...
						begin
							IF @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By sub_vertical ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +''--Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN				
						...
						begin
							IF @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Sub_Branch ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +'' --Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN
						...
						begin
							IF @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Segment_Name ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +''--Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
								BEGIN				
						...
						-- print 123--mansi
							IF @Type='1'
								Begin
									...
									set @String = @String + ' from #CTCMast CM 
									 group By Center_Code ' + @Str_Emp_Name_Group + ''+@String_2+' '+ @String_3 +''--Where Sal_Cal_Day <> 0
									exec(@String)
								End			
							ELSE
	...
	From #CTCMast M
	Inner JOIN(
			   Select Isnull(SUM(M_Arear_Amount),0) as Arrear_Earn_Amt,Emp_ID From T0210_MONTHLY_AD_DETAIL 
	...
	Inner JOIN(
			   Select Isnull(SUM(M_Arear_Amount),0) as Arrear_Ded_Amt,Emp_ID From T0210_MONTHLY_AD_DETAIL 
	...
	Inner JOIN(
				Select Isnull(SUM(Arear_Basic),0) as Arrear_Basic,Emp_ID From T0200_MONTHLY_SALARY
	...
	Update CM
		Set Basic_Salary = Qry.Salary_Amount,
			...
			Total_Deduction = Qry.Dedu_Amount + Qry.Advance_Amount + Qry.PT_Amount + Qry.LWF_Amount + Qry.Other_Dedu_Amount + Qry.Asset_Installment + Qry.Travel_Advance_Amount + qry.Uniform_Dedu_Amount + qry.Late_Deduction_Amount + Qry.Loan +  Qry.Bond_Amount, 
			...
			M_WO_OT_Hours = Qry.M_WO_OT_Hours,
			M_HO_OT_Hours = Qry.M_HO_OT_Hours,
	...
	from #CTCMast CM 
		Inner Join	(select GS.Branch_ID,GS.ot_fix_shift_hours
						...
						INNER JOIN (
										select max(for_date) as For_Date,Cmp_ID,Branch_ID From T0040_General_Setting 
		...
		Inner JOIN (
					SELECT
					sum(MS.Salary_Amount) + Isnull(Sum(MS.Basic_Salary_Arear_cutoff),0) As Salary_Amount,
					...
					Isnull(sum(MS.Bond_Amount),0) As Bond_Amount, 
					...
					isnull(sum(MS.M_WO_OT_Hours),0) As M_WO_OT_Hours,
					isnull(sum(MS.M_HO_OT_Hours),0) As M_HO_OT_Hours,
					...
					,ISNULL(ms.Is_FNF,0) as Is_FNF -- ADDED BY RAJPUT ON 25062018
					From T0200_MONTHLY_SALARY MS 
	...
	--added by jimit for updating OT_Rate if Fix OT Rate for weekday is available 06042017
		...
		--ended
			--Added By Ramiz on 03/06/2019--For Mafatlals Gradewise Overtime--
			UPDATE C
			SET OT_Hours = ISNULL(msg.Grd_OT_Hours/2,0)
			FROM #CTCMAST C
				INNER JOIN T0200_MONTHLY_SALARY MS ON MS.EMP_ID = C.Emp_ID
				INNER JOIN T0210_Monthly_Salary_Slip_Gradecount MSG on MSG.Sal_tran_id = MS.Sal_Tran_ID
			WHERE MS.Month_st_date between @From_Date and @To_Date and MS.Cmp_ID = @Company_Id
...
--	select * from #CTCMast
							...
							while @@fetch_status = 0
								Begin				
										begin
												...
												begin
													--select * from #Emp_Allowance where Emp_ID=16845
			...
			fetch next from Leave_Cursor into @Leave_Name
				while @@fetch_status = 0
					Begin
						...
						T0040_LEAVE_MASTER LM on LM.Leave_ID = LT.Leave_ID 
				...
				while @@fetch_status = 0
					Begin
						...
						SELECT  LA.Emp_ID,LA.Loan_ID,
								SUM(MLP.LOAN_PAY_AMOUNT + IsNull(MLP.Interest_Amount,0)) 
								,LM.Loan_Name
						FROM	T0210_MONTHLY_LOAN_PAYMENT MLP INNER JOIN 
								T0120_LOAN_APPROVAL LA ON MLP.LOAN_APR_ID=LA.LOAN_APR_ID INNER JOIN
								T0040_LOAN_MASTER LM ON LA.LOAN_ID=LM.LOAN_ID 
	...
	IF NOT EXISTS(SELECT 1 FROM #CTCMast where Cmp_ID <> 0)
		TRUNCATE TABLE #CTCMast 
	...
	begin
		...
		Select	c.name, IsNull(cons.name, o.Name) As Constraint_Name
		From	tempdb.sys.columns c 
				...
				left outer join tempdb.sys.objects o on c.object_id=c.object_id and o.name='CTCMast_' + c.name AND o.TYPE_DESC='DEFAULT_CONSTRAINT'
		Where c.object_id=OBJECT_ID('tempdb.dbo.#CTCMast') and c.column_id > @OrdinalPosition and c.precision > 0
				...
				----PRINT @Con_Name
					if @Con_col_Name is not null
						begin 
							...
							END TRY
							BEGIN CATCH
								PRINT 'ERROR IN DELETING DEFAULT CONSTRAINT ' + @Con_col_Name
							END CATCH	
					...
					END TRY
					BEGIN CATCH
						PRINT 'ERROR IN DELETING COLUMN ' + @Con_Name
					END CATCH
...
--		exec (@drop_cols);
		...
		--added by mansi start
		 --Select 
		 --CM.Cmp_ID, CM.Emp_ID, CM.Emp_code, CM.Alpha_Emp_Code, CM.Emp_Full_Name, CM.Branch, CM.Department, CM.Designation, CM.Grade, CM.TypeName, CM.Category, CM.Division, CM.sub_vertical, CM.sub_branch
		 --, CM.Aadhar_Card_No, CM.Bank_Ac_No, CM.Pan_No, CM.PF_No, CM.Segment_Name, CM.Center_Code, CM.Desig_dis_No, CM.Enroll_No, CM.Date_Of_Birth, CM.Date_Of_Join, CM.Inc_Bank_Ac_NO, CM.Present_Day, CM.Absent_Day
		 --, CM.Holiday_Day, CM.WeekOff_Day, CM.Branch_Id, CM.City, CM.PinCode, CM.Father_Name, CM.Emp_Left_Date
		 --, CM.LWP, CM.Total_Leave_Days, CM.Sal_Cal_Day, CM.Actual_CTC, CM.Basic_salary
		 --, CM.Settl_Salary, CM.Other_Allow, CM.Helper_1, CM.HRA, CM.Transport, CM.Medical, CM.SBonus, CM.Attire, CM.Washing, CM.Driver, CM.Helper, CM.petrol, CM.LTA, CM.Book, CM.Special, CM.Other_Earn
		 --, CM.AA, CM.Arrears, CM.PLI, CM.II, CM.MA, CM.inc, CM.Conveyance, CM.NPA, CM.NPA_1, CM.LE, CM.Helper_1_Arrear,CM.Basic_Arrear, CM.HRA_Arrear, CM.Transport_Arrear, CM.Medical_Arrear, CM.SBonus_Arrear, CM.Attire_Arrear
	     --, CM.Washing_Arrear, CM.Driver_Arrear, CM.Helper_Arrear, CM.petrol_Arrear, CM.LTA_Arrear, CM.Book_Arrear, CM.Special_Arrear, CM.Other_Earn_Arrear, CM.AA_Arrear, CM.Arrears_Arrear
         --, CM.PLI_Arrear, CM.II_Arrear, CM.MA_Arrear, CM.inc_Arrear, CM.Conveyance_Arrear, CM.NPA_Arrear, CM.NPA_1_Arrear, CM.LE_Arrear, CM.Production_Bonus, CM.Leave_Encash_Amount
		 --, CM.Uniform_Refund_Amount, CM.OT_Rate, CM.OT_Hours, CM.OT_Amount, CM.M_HO_OT_Hours, CM.M_HO_OT_Amount, CM.M_WO_OT_Hours, CM.M_WO_OT_Amount, CM.WO_HO_Fix_OT_Rate
		 --, CM.Travel_Amount, CM.Total_Earning, CM.Total_Earning_Arrear, CM.Gross_Salary, CM.PT_Amount, CM.Advance_Amount, CM.Bond_Amount, CM.Retention
		 --, CM.Other_Dedu, CM.TDS, CM.recover, CM.recover1, CM.PF, CM.ESIC, CM.Retention_Arrear, CM.Other_Dedu_Arrear, CM.TDS_Arrear, CM.recover_Arrear, CM.recover1_Arrear, CM.PF_Arrear
		 --, CM.ESIC_Arrear, CM.Revenue_Amount, CM.LWF_Amount, CM.Other_Deduction, CM.Gate_Pass_Amount, CM.Asset_Installment_Amount, CM.Travel_Advance_Amount
		 --, CM.Uniform_Dedu_Amount, CM.Late_Deduction_Amount, CM.Arear_Deduction, CM.Total_Deduction, CM.Net_Total_Deduction, CM.Net_Amount, CM.Net_Round
		 --, CM.Total_Net, CM.Gratuity, CM.CPF, CM.CPFAdmin, CM.Salary_Status
		 --added by mansi end
		
		Select CM.*--,inc.inc_bank_ac_No as inc_bank_ac_No --commented by mansi 
			,(
			...
			) as PF_WAGES,  --- Added by Hardik 30/11/2020 for Emerland Honda
			(
					SELECT  MAD.M_AD_Calculated_Amount 
					FROM	T0210_MONTHLY_AD_DETAIL  MAD WITH (NOLOCK)  
					INNER JOIN T0050_AD_MASTER AD WITH (NOLOCK)  ON MAD.AD_ID = AD.AD_ID
					...
					And year(To_date)=year(@To_Date)
					And MAD.Emp_ID = EM.Emp_Id And S_Sal_Tran_Id Is null
			)as ESI_WAGES --- Added by Hardik 30/11/2020 for Emerland Honda

		into #Temp_CTCMaster from #CTCMast CM
		Left Outer join t0080_emp_master em on cm.emp_id = em.emp_id
		Left Outer join t0095_increment Inc on em.increment_id = inc.increment_id
	...
	--Will be Created in WebService Salary Sp :- SP_Mobile_HRMS_WebService_Salary_ALL
	--Added by Ramiz on 24/05/2019 for Golcha
	--IF OBJECT_ID('tempdb..#WebService_Salary') IS NOT NULL
	--	BEGIN
	...
	--	END
					 ...
					 Select * into #temp_CTCMaster_Sum From #temp_CTCMaster where Emp_code <> -999
	...
	begin
		...
		if @summary2 is not null
			Begin
				Set @String_2 = 'Select (CASE WHEN '+ @summary2 +'=''0'' THEN ''CM.Branch'' When '+ @summary2 +' =''1'' Then ''CM.Grade'' When '+ @Summary2 +' =''2'' Then ''CM.Category'' When '+ @summary2 +' =''3'' Then ''Department'' When '+ @summary2 +' =''4'' Then ''Designation'' When '+ @summary2 +' =''5'' Then ''TypeName'' When '+ @summary2 +' =''6'' Then ''Division'' When '+ @Summary2 +' =''7'' Then ''sub_vertical'' When '+ @Summary2 +' =''8'' Then ''sub_branch'' When '+ @summary2 +' =''9'' Then ''Segment_Name'' When '+ @summary2 +' =''10'' Then ''Center_Code'' End ) as Description into tempgroup2'
				exec(@String_2)
				Select @String_2 = Description From tempgroup2
				Set @String_2 = ','+ @String_2
			End 
		...
		if @summary3 is not null
			Begin
				Set @String_3 = 'Select (CASE WHEN '+ @summary3 +'=''0'' THEN ''CM.Branch'' When '+ @summary3 +' =''1'' Then ''CM.Grade'' When '+ @summary3 +' =''2'' Then ''CM.Category'' When '+ @summary3 +' =''3'' Then ''Department'' When '+ @summary3 +' =''4'' Then ''Designation'' When '+ @summary3 +' =''5'' Then ''TypeName'' When '+ @summary3 +' =''6'' Then ''Division'' When '+ @summary3 +' =''7'' Then ''sub_vertical'' When '+ @summary3 +' =''8'' Then ''sub_branch'' When '+ @summary3 +' =''9'' Then ''Segment_Name'' When '+ @summary3 +' =''10'' Then ''Center_Code'' End ) as Description into tempgroup3'
				exec(@String_3)
				Select @String_3 = Description From tempgroup3
				Set @String_3 = ','+ @String_3
			End
		...
		if @Type = '1' -- For Employee Detail & Summary 
			Begin
				...
				Set @String_1 = 'select 0 as flag, ROW_NUMBER() OVER (Order by CM.Branch) As Row_ID, CM.Alpha_Emp_Code,CM.Emp_Full_Name,CM.Branch as Branch_Name'+ @String_2 +''+ @String_3 +',1 As Total_Emp, 
					CM.Basic_Salary as Basic_Salary '+ @allownaces_earning +',CM.Production_Bonus as Production_Bonus,CM.Leave_Encash_Amount as Leave_Encash_Amount,CM.Uniform_Refund_Amount as Uniform_Refund_Amount,Total_Earning As Total_Earning,Basic_Arrear As Basic_Arrear'
					+ @allownaces_earning_arear +',Total_Earning_Arrear as Total_Earning_Arrear, CM.Other_Allow as Other_Allowance, CM.Gross_Salary as Gross_Salary '+ @allownaces_deduct +',CM.Actual_CTC as CTC,cm.advance_amount as Advance_Amount,CM.PT_Amount as PT_Amount'+ @Loan_Amount_Str +',cm.ot_rate as OT_Rate,cm.ot_hours as OT_Hours,Cm.M_WO_OT_Hours as WeekOff_OT_Hours,cm.M_HO_OT_Hours as Holiday_OT_Hours,cm.ot_amount as OT_Amount,cm.WO_HO_Fix_OT_Rate as WO_HO_Fix_OT_Rate,cm.revenue_amount as Revenue_Amount,cm.lwf_amount as LWF_Amount,isnull(cm.Gate_Pass_Amount,0) as Gate_Pass_Amount,isnull(cm.Asset_Installment_Amount,0) as Asset_Installment_Amount,isnull(cm.Uniform_Dedu_Amount,0) as Uniform_Installment_Amount,ISNULL(Cm.Late_Deduction_Amount,0) as Late_Deduction_Amount,ISNULL(Total_Deduction,0) as Total_Deduction '
				...
				--Set @String_1 = @String_1 + ' INTO ##BRANCH from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
				
				---- ADDED BY RAJPUT ON 25062018 ----				
				IF(ISNULL(@REPORT_TYPE,0) = 1)
					BEGIN
						Set @String_1 = @String_1 + ' INTO ##BRANCH from #CTCMast CM where  CM.IS_FNF <> 1'
					END
				...
				ELSE
					BEGIN
						Set @String_1 = @String_1 + ' INTO ##BRANCH from #CTCMast CM ' 
					END 
				...
				IF EXISTS(SELECT 1 FROM ##BRANCH)
					BEGIN
						...
						SUM(Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
						+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(Other_Allowance)as Other_Allowance, SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CTC) as CTC,SUM(advance_amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,Sum(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(revenue_amount) as Revenue_Amount,SUM(lwf_amount)as LWF_Amount,SUM(isnull(Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
						...
						SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_Day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(total_paid_leave_days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' 
						FROM ##BRANCH CM Group by Branch_Name' + @String_2 +''+ @String_3

						exec(@String)
						
						set @String = 'Select * FROM ##BRANCH CM Order By Branch_Name '+ @String_2 +''+ @String_3 +',flag';
		...
		Else   -- For Only Summary Grouping Wise 
			Begin
				...
				--exec(@String)
					...
					--set @String = @String + ' INTO ##BRANCH from #CTCMast CM  group By Branch'+ @String_2 +''+ @String_3 +''--Where Sal_Cal_Day <> 0 COMMENTED BY RAJPUT ON 25062018
					...
					IF EXISTS(SELECT 1 FROM ##BRANCH)
						BEGIN
							...
							SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##BRANCH CM';
		...
		if @Type = '1' -- For Employee Detail & Summary 
			Begin
				...
				--Set @String_1 = @String_1 + ' INTO ##GRADE from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
				
				---- ADDED BY RAJPUT ON 25062018 ----				
				IF(ISNULL(@REPORT_TYPE,0) = 1)
					BEGIN
						Set @String_1 = @String_1 + ' INTO ##GRADE from #CTCMast CM where CM.IS_FNF <> 1'
					END
				ELSE IF(ISNULL(@Report_type,0) = 2)
					BEGIN
						Set @String_1 = @String_1 + ' INTO ##GRADE from #CTCMast CM where CM.IS_FNF = 1 ' 
				...
				ELSE
					BEGIN
						Set @String_1 = @String_1 + ' INTO ##GRADE from #CTCMast CM ' 
					END 
				...
				IF EXISTS(SELECT 1 FROM ##GRADE)
					BEGIN
						...
						SUM(Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
						+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(Other_Allowance)as Other_Allowance, SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CTC) as CTC,SUM(advance_amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(Cm.weekoff_OT_Hours) as WeekOff_OT_Hours,SUM(cm.Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(revenue_amount) as Revenue_Amount,SUM(lwf_amount)as LWF_Amount,SUM(isnull(Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
						+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
		...
		Else
			Begin
				...
				--exec(@String)
					...
					--set @String = @String + ' INTO ##GRADE from #CTCMast CM  group By Grade'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0
					
					---- ADDED BY RAJPUT ON 25062018 ----				
					IF(ISNULL(@REPORT_TYPE,0) = 1)
						BEGIN
							Set @String = @String  + ' INTO ##GRADE from #CTCMast CM where CM.IS_FNF <> 1 group By Grade'+ @String_2 +''+ @String_3 +''
						END
					ELSE IF(ISNULL(@Report_type,0) = 2)
						BEGIN
							Set @String = @String  + ' INTO ##GRADE from #CTCMast CM where CM.IS_FNF = 1 group By Grade'+ @String_2 +''+ @String_3 +'' 
					...
					ELSE
						BEGIN
							Set @String = @String  + ' INTO ##GRADE from #CTCMast CM group By Grade'+ @String_2 +''+ @String_3 +'' 
						END 
					...
					IF EXISTS(SELECT 1 FROM ##GRADE)
						BEGIN
							set @String = 'Insert into ##GRADE Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
							+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
							+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
							SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##GRADE';
		...
		begin
			if @Type = '1' -- For Employee Detail & Summary 
				Begin
					...
					--Set @String_1 = @String_1 + ' INTO ##CATEGORY from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
					
					---- ADDED BY RAJPUT ON 25062018 ----				
					IF(ISNULL(@REPORT_TYPE,0) = 1)
						BEGIN
							Set @String_1 = @String_1 + ' INTO ##CATEGORY from #CTCMast CM where CM.IS_FNF <> 1'
						END
					ELSE IF(ISNULL(@Report_type,0) = 2)
						BEGIN
							Set @String_1 = @String_1 + ' INTO ##CATEGORY from #CTCMast CM where CM.IS_FNF = 1 ' 
					...
					ELSE
						BEGIN
							Set @String_1 = @String_1 + ' INTO ##CATEGORY from #CTCMast CM ' 
						END 
			...
			Else
				Begin
					...
					--set @String = @String + ' INTO ##CATEGORY from #CTCMast CM  group By Category'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0 COMMENTED BY RAJPUT ON 25062018
					
					---- ADDED BY RAJPUT ON 25062018 ----				
					IF(ISNULL(@REPORT_TYPE,0) = 1)
						BEGIN
							Set @String = @String + ' INTO ##CATEGORY from #CTCMast CM where CM.IS_FNF <> 1 group By Category'+ @String_2 +''+ @String_3 +''
						END
					ELSE IF(ISNULL(@Report_type,0) = 2)
						BEGIN
							Set @String = @String + ' INTO ##CATEGORY from #CTCMast CM where CM.IS_FNF = 1 group By Category'+ @String_2 +''+ @String_3 +'' 
					...
					ELSE
						BEGIN
							Set @String = @String + ' INTO ##CATEGORY from #CTCMast CM group By Category'+ @String_2 +''+ @String_3 +'' 
						END 
					...
					IF EXISTS(SELECT 1 FROM ##CATEGORY)
						BEGIN
							...
							set @String = 'Insert into ##CATEGORY Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
							+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(weekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
							+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
							SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##CATEGORY';
		...
		begin
			...
			if @Type = '1' -- For Employee Detail & Summary 
				Begin
					...
					--exec(@String_1 +' Union ALL ' + @String)
						...
						DECLARE @Static_Column Varchar(Max)
						Set @Static_Column = ', CM.Father_Name,CM.Date_Of_Join,CM.Emp_Left_Date,CM.PF_NO,CM.Pan_No,CM.Bank_Ac_No,CM.Inc_Bank_AC_No,CM.Aadhar_Card_No,CM.Branch,CM.Designation,CM.Grade,CM.TypeName,CM.Category,CM.sub_vertical,CM.Sub_Branch'

					
						Set @String_1 = 'select 0 as flag, ROW_NUMBER() OVER (Order by CM.Department) As Row_ID, CM.Alpha_Emp_Code,CM.Emp_Full_Name,CM.Department as Department  ' + @Static_Column + ' '+ @String_2 +''+ @String_3 +',1 As Total_Emp, 
							CM.Basic_Salary as Basic_Salary '+ @allownaces_earning +',CM.Production_Bonus as Production_Bonus,CM.Leave_Encash_Amount as Leave_Encash_Amount,CM.Uniform_Refund_Amount as Uniform_Refund_Amount,Total_Earning As Total_Earning,Basic_Arrear As Basic_Arrear'
							+ @allownaces_earning_arear +',Total_Earning_Arrear as Total_Earning_Arrear, CM.Other_Allow as Other_Allowance, CM.Gross_Salary as Gross_Salary '+ @allownaces_deduct +',CM.Actual_CTC as CTC,cm.advance_amount as Advance_Amount,CM.PT_Amount as PT_Amount'+ @Loan_Amount_Str +',cm.ot_rate as OT_Rate,cm.ot_hours as OT_Hours,M_WO_OT_Hours as WeekOff_OT_Hours,cm.M_HO_OT_Hours as Holiday_OT_Hours,cm.ot_amount as OT_Amount,cm.WO_HO_Fix_OT_Rate as WO_HO_Fix_OT_Rate,cm.revenue_amount as Revenue_Amount,cm.lwf_amount as LWF_Amount,isnull(cm.Gate_Pass_Amount,0) as Gate_Pass_Amount,isnull(cm.Asset_Installment_Amount,0) as Asset_Installment_Amount,isnull(cm.Uniform_Dedu_Amount,0) as Uniform_Installment_Amount,ISNULL(Cm.Late_Deduction_Amount,0) as Late_Deduction_Amount,ISNULL(Total_Deduction,0) as Total_Deduction '
							+ @allownaces_deduct_arear + ',Arear_Deduction As Arear_Deduction, Net_Total_Deduction as Net_Total_Deduction,cm.net_amount as Net_Amount,Net_Round As Net_Round, Total_Net As Total_Net '+ @Allownaces_Earning_CTC +' ,(Net_Round + CM.Gross_Salary '+ @Allownaces_Earning_CTC_Total +') AS Total_Amount,
							CM.Present_Day as Present_Days,CM.Arear_Day as Arear_Days,CM.Absent_Day as Absent_Day,CM.Holiday_Day as Holiday_day,CM.WeekOff_Day as Week_Off_Days, CM.Sal_Cal_Day as Sal_cal_Day,cm.total_leave_days as Total_leave_Days, CM.total_paid_leave_days as Total_Paid_Leave_Days'+ @allownaces_earning_reim +' '
						...
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DEPT from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DEPT from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DEPT from #CTCMast CM ' 
							END 
						...
						DECLARE @Static_Column_Sum Varchar(Max)
						Set @Static_Column_Sum = ', '''' as Father_Name,'''' as Date_Of_Join, '''' as Emp_Left_Date, '''' as PF_NO, '''' as Pan_No, '''' as Bank_Ac_No, '''' as Inc_Bank_AC_No, '''' as Aadhar_Card_No, '''' as Branch, '''' as Designation, '''' as Grade, '''' as TypeName, '''' as Category, '''' as sub_vertical, '''' as Sub_Branch'
					
						IF EXISTS(SELECT 1 FROM ##DEPT)
							BEGIN
								set @String = 'Insert into ##DEPT select 1 as flag,(IsNull(Max(Row_ID),0) +1 ),'''' As Alpha_Emp_Code ,''Total'' As Emp_Full_Name,Department as Department ' + @Static_Column_Sum + ' ' + @String_2 +''+ @String_3 +',1 As Total_Emp,
								SUM(Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(Other_Allowance)as Other_Allowance, SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CTC) as CTC,SUM(advance_amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(weekoff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(revenue_amount) as Revenue_Amount,SUM(lwf_amount)as LWF_Amount,SUM(isnull(Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
								+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_Day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(total_paid_leave_days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' 
								FROM ##DEPT Group By Department '+ @String_2 +''+ @String_3 +'';

								exec(@String)
								set @String = 'Select * FROM ##DEPT Order By Department '+ @String_2 +''+ @String_3 +',flag';
			...
			Else
				BEGIN
					...
					--set @String = @String + ' INTO ##DEPT from #CTCMast CM  group By Department'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0
					
					---- ADDED BY RAJPUT ON 25062018 ----				
					IF(ISNULL(@REPORT_TYPE,0) = 1)
						BEGIN
							Set @String = @String + ' INTO ##DEPT from #CTCMast CM where CM.IS_FNF <> 1 group By Department'+ @String_2 +''+ @String_3 +''
						END
					ELSE IF(ISNULL(@Report_type,0) = 2)
						BEGIN
							Set @String = @String + ' INTO ##DEPT from #CTCMast CM where CM.IS_FNF = 1 group By Department'+ @String_2 +''+ @String_3 +'' 
					...
					IF EXISTS(SELECT 1 FROM ##DEPT)
						BEGIN
							set @String = 'Insert into ##DEPT Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
							+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(weekoff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
							+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
			...
			begin
				...
				if @Type = '1' -- For Employee Detail & Summary 
					Begin
						...
						--SUM(CM.Basic_Salary) as Basic_Salary '+ @allownaces_earning +',SUM(CM.Production_Bonus) as Production_Bonus,SUM(CM.Leave_Encash_Amount) as Leave_Encash_Amount,SUM(CM.Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
						...
						--Set @String_1 = @String_1 + ' INTO ##DESIG from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
						 
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DESIG from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DESIG from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DESIG from #CTCMast CM ' 
							END 
						...
						--set @String = 'Insert into ##DESIG Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
				...
				Else
					Begin
						...
						--set @String = @String + ' INTO ##DESIG from #CTCMast CM  group By Designation'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0 COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##DESIG from #CTCMast CM where CM.IS_FNF <> 1 group By Designation'+ @String_2 +''+ @String_3 +''
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##DESIG from #CTCMast CM where CM.IS_FNF = 1 group By Designation'+ @String_2 +''+ @String_3 +'' 
						...
						ELSE
							BEGIN
								Set @String = @String + ' INTO ##DESIG from #CTCMast CM group By Designation'+ @String_2 +''+ @String_3 +'' 
							END 
						...
						IF EXISTS(SELECT 1 FROM ##DESIG)
							BEGIN
								set @String = 'Insert into ##DESIG Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##DESIG';
			...
			begin
				if @Type = '1' -- For Employee Detail & Summary 
					Begin
						...
						Set @String_1 = 'select 0 as flag, ROW_NUMBER() OVER (Order by CM.TypeName) As Row_ID, CM.Alpha_Emp_Code,CM.Emp_Full_Name,CM.TypeName as Type_Name'+ @String_2 +''+ @String_3 +',1 As Total_Emp, 
							CM.Basic_Salary as Basic_Salary '+ @allownaces_earning +',CM.Production_Bonus as Production_Bonus,CM.Leave_Encash_Amount as Leave_Encash_Amount,CM.Uniform_Refund_Amount as Uniform_Refund_Amount,Total_Earning As Total_Earning,Basic_Arrear As Basic_Arrear'
							+ @allownaces_earning_arear +',Total_Earning_Arrear as Total_Earning_Arrear, CM.Other_Allow as Other_Allowance, CM.Gross_Salary as Gross_Salary '+ @allownaces_deduct +',CM.Actual_CTC as CTC,cm.advance_amount as Advance_Amount,CM.PT_Amount as PT_Amount'+ @Loan_Amount_Str +',cm.ot_rate as OT_Rate,cm.ot_hours as OT_Hours,Cm.M_WO_OT_Hours as WeekOff_OT_Hours,cm.M_HO_OT_Hours as Holiday_OT_Hours
							,cm.ot_amount as OT_Amount,cm.WO_HO_Fix_OT_Rate as WO_HO_Fix_OT_Rate,cm.revenue_amount as Revenue_Amount,cm.lwf_amount as LWF_Amount,isnull(cm.Gate_Pass_Amount,0) as Gate_Pass_Amount,isnull(cm.Asset_Installment_Amount,0) as Asset_Installment_Amount,isnull(cm.Uniform_Dedu_Amount,0) as Uniform_Installment_Amount,ISNULL(Cm.Late_Deduction_Amount,0) as Late_Deduction_Amount,ISNULL(Total_Deduction,0) as Total_Deduction '
							+ @allownaces_deduct_arear + ',Arear_Deduction As Arear_Deduction, Net_Total_Deduction as Net_Total_Deduction,cm.net_amount as Net_Amount,Net_Round As Net_Round, Total_Net As Total_Net '+ @Allownaces_Earning_CTC +' ,(Net_Round + CM.Gross_Salary '+ @Allownaces_Earning_CTC_Total +') AS Total_Amount,
							CM.Present_Day as Present_Days,CM.Arear_Day as Arear_Days,CM.Absent_Day as Absent_Day,CM.Holiday_Day as Holiday_day,CM.WeekOff_Day as Week_Off_Days, CM.Sal_Cal_Day as Sal_cal_Day,cm.total_leave_days as Total_leave_Days, CM.total_paid_leave_days as Total_Paid_Leave_Days'+ @allownaces_earning_reim +''

						--Set @String_1 = @String_1 + ' INTO ##TYPE from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##TYPE from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##TYPE from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##TYPE from #CTCMast CM ' 
							END 
						...
						IF EXISTS(SELECT 1 FROM ##TYPE)
							BEGIN
								...
								SUM(Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(Other_Allowance)as Other_Allowance, SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CTC) as CTC,SUM(advance_amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(revenue_amount) as Revenue_Amount,SUM(lwf_amount)as LWF_Amount,SUM(isnull(Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
								+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
				...
				Else
					Begin
						...
						--set @String = @String + ' INTO ##TYPE from #CTCMast CM  group By TypeName'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##TYPE from #CTCMast CM where CM.IS_FNF <> 1 group By TypeName'+ @String_2 +''+ @String_3 +''
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##TYPE from #CTCMast CM where CM.IS_FNF = 1 group By TypeName'+ @String_2 +''+ @String_3 +'' 
						...
						ELSE
							BEGIN
								Set @String = @String + ' INTO ##TYPE from #CTCMast CM group By TypeName'+ @String_2 +''+ @String_3 +'' 
							END 
						...
						IF EXISTS(SELECT 1 FROM ##TYPE)
							BEGIN
								set @String = 'Insert into ##TYPE Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##TYPE';
			...
			begin
				if @Type = '1' -- For Employee Detail & Summary 
					Begin
						...
						--Set @String_1 = @String_1 + ' INTO ##DIVISION from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DIVISION from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DIVISION from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##DIVISION from #CTCMast CM ' 
							END 
				...
				Else
					Begin
						...
						--set @String = @String + ' INTO ##DIVISION from #CTCMast CM group By Division'+ @String_2 +''+ @String_3 +''--Where Sal_Cal_Day <> 0  COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##DIVISION from #CTCMast CM where CM.IS_FNF <> 1 group By Division'+ @String_2 +''+ @String_3 +''
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##DIVISION from #CTCMast CM where CM.IS_FNF = 1 group By Division'+ @String_2 +''+ @String_3 +'' 
						...
						ELSE
							BEGIN
								Set @String = @String + ' INTO ##DIVISION from #CTCMast CM group By Division'+ @String_2 +''+ @String_3 +'' 
							END 
						...
						IF EXISTS(SELECT 1 FROM ##DIVISION)
							BEGIN
								set @String = 'Insert into ##DIVISION Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##DIVISION';
			...
			begin
				...
				Else
					Begin
						...
						set @String = @String + ' INTO ##SUBVERTICAL from #CTCMast CM  group By sub_vertical'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0
						...
						set @String = 'Insert into ##SUBVERTICAL Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
						+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
						,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
						+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
						SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##SUBVERTICAL';
			...
			begin
				if @Type = '1' -- For Employee Detail & Summary 
					Begin
						...
						--Set @String_1 = @String_1 + ' INTO ##SUBBRANCH from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
						 
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SUBBRANCH from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SUBBRANCH from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SUBBRANCH from #CTCMast CM ' 
							END 
				...
				Else
					Begin
						...
						--set @String = @String + ' INTO ##SUBBRANCH from #CTCMast CM  group By Sub_Branch'+ @String_2 +''+ @String_3 +'' --Where Sal_Cal_Day <> 0 COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##SUBBRANCH from #CTCMast CM where CM.IS_FNF <> 1 group By Sub_Branch'+ @String_2 +''+ @String_3 +''
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##SUBBRANCH from #CTCMast CM where CM.IS_FNF = 1 group By Sub_Branch'+ @String_2 +''+ @String_3 +'' 
						...
						IF EXISTS(SELECT 1 FROM ##SUBBRANCH)
							BEGIN
								...
								set @String = 'Insert into ##SUBBRANCH Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##SUBBRANCH';
			...
			begin
				if @Type = '1' -- For Employee Detail & Summary 
					Begin
						...
						--Set @String_1 = @String_1 + ' INTO ##SEGMENT from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SEGMENT from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SEGMENT from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##SEGMENT from #CTCMast CM ' 
							END 
						EXEC(@String_1)
						---- END ----
				...
				Else
					Begin
						...
						--set @String = @String + ' INTO ##SEGMENT from #CTCMast CM  group By Segment_Name'+ @String_2 +''+ @String_3 +''--Where Sal_Cal_Day <> 0 COMMENTED BY RAJPUT ON 25062018
						
						---- ADDED BY RAJPUT ON 25062018 ----	
						
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##SEGMENT from #CTCMast CM where CM.IS_FNF <> 1 group By Segment_Name'+ @String_2 +''+ @String_3 +''
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##SEGMENT from #CTCMast CM where CM.IS_FNF = 1 group By Segment_Name'+ @String_2 +''+ @String_3 +'' 
						...
						ELSE
							BEGIN
								...
								Set @String = @String + ' INTO ##SEGMENT from #CTCMast CM group By Segment_Name'+ @String_2 +''+ @String_3 +'' 
							END 
						--print @string
						EXEC(@STRING)
						---- END ----
						...
						IF EXISTS(SELECT 1 FROM ##SEGMENT)
							BEGIN
								...
								set @String = 'Insert into ##SEGMENT Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'''+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##SEGMENT';
    ...
    ---CM.Branch as Branch_Name
	     --              --commented by mansi start
						...
						--	--commented by mansi end
							--added by mansi start
								Set @String_1 = 'select 0 as flag, ROW_NUMBER() OVER (Order by CM.Center_Code) As Row_ID, CM.Alpha_Emp_Code,CM.Emp_Full_Name,CM.Center_Code'+ @String_2 +''+ @String_3 +',1 As Total_Emp, 
							CM.Basic_Salary as Basic_Salary '+ @allownaces_earning +',CM.Production_Bonus as Production_Bonus,CM.Leave_Encash_Amount as Leave_Encash_Amount,CM.Uniform_Refund_Amount as Uniform_Refund_Amount,Total_Earning As Total_Earning,Basic_Arrear As Basic_Arrear'
							+ @allownaces_earning_arear +',Total_Earning_Arrear as Total_Earning_Arrear, CM.Other_Allow as Other_Allowance, CM.Gross_Salary as Gross_Salary '+ @allownaces_deduct +',CM.Actual_CTC as CTC,cm.advance_amount as Advance_Amount,CM.PT_Amount as PT_Amount'+ @Loan_Amount_Str +',cm.ot_rate as OT_Rate,cm.ot_hours as OT_Hours,Cm.M_WO_OT_Hours as WeekOff_OT_Hours,cm.M_HO_OT_Hours as Holiday_OT_Hours,cm.ot_amount as OT_Amount,cm.WO_HO_Fix_OT_Rate as WO_HO_Fix_OT_Rate,cm.revenue_amount as Revenue_Amount,cm.lwf_amount as LWF_Amount,isnull(cm.Gate_Pass_Amount,0) as Gate_Pass_Amount,isnull(cm.Asset_Installment_Amount,0) as Asset_Installment_Amount,isnull(cm.Uniform_Dedu_Amount,0) as Uniform_Installment_Amount,ISNULL(Cm.Late_Deduction_Amount,0) as Late_Deduction_Amount,ISNULL(Total_Deduction,0) as Total_Deduction '
							+ @allownaces_deduct_arear + ',Arear_Deduction As Arear_Deduction, Net_Total_Deduction as Net_Total_Deduction,cm.net_amount as Net_Amount,Net_Round As Net_Round, Total_Net As Total_Net '+ @Allownaces_Earning_CTC +' ,(Net_Round + CM.Gross_Salary '+ @Allownaces_Earning_CTC_Total +') AS Total_Amount,
							CM.Present_Day as Present_Days,CM.Arear_Day as Arear_Days,CM.Absent_Day as Absent_Day,CM.Holiday_Day as Holiday_day,CM.WeekOff_Day as Week_Off_Days, CM.Sal_Cal_Day as Sal_cal_Day,cm.total_leave_days as Total_leave_Days, CM.total_paid_leave_days as Total_Paid_Leave_Days'+ @allownaces_earning_reim +''
						...
						-- Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM'-- Where Sal_Cal_Day <> 0' COMMENTED BY RAJPUT ON 
						 
						---- ADDED BY RAJPUT ON 25062018 ----				
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF <> 1'
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF = 1 ' 
						...
						ELSE
							BEGIN
								Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM ' 
							END 
						...
						IF EXISTS(SELECT 1 FROM ##CenterCode)
							BEGIN
							 ----commented by mansi start
								...
								----commented by mansi end
								--added by mansi start
									set @String = 'Insert into ##CenterCode select 0 as flag,(IsNull(Max(Row_ID),0)+1),'''' As Alpha_Emp_Code ,''Total'' As Emp_Full_Name,''Total'' as Center_Code'+ @String_2 +''+ @String_3 +',1 As Total_Emp,
								SUM(Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(Other_Allowance)as Other_Allowance, SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CTC) as CTC,SUM(advance_amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(revenue_amount) as Revenue_Amount,SUM(lwf_amount)as LWF_Amount,SUM(isnull(Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
								+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
				...
				Else
					Begin
					 ...
					 --added by mansi start
						set @String = ' select 0 as Flag, ROW_NUMBER() OVER (Order by CM.Center_Code) As Row_ID ,CM.Center_Code as Center_Code'+ @String_2 +''+ @String_3 +',Count(Emp_ID) As Total_Emp,
						Round((Count(Emp_ID) / '  + cast(@Avg_Emp as varchar(10)) +' ),0)AS Avg_Emp ,SUM(CM.Basic_Salary) as Basic_Salary '+ @sum_of_allownaces_earning +',SUM(CM.Production_Bonus) as Production_Bonus,SUM(CM.Leave_Encash_Amount) as Leave_Encash_Amount,SUM(CM.Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning,Sum(Basic_Arrear) As Basic_Arrear'
						+ @sum_of_allownaces_earning_Arear +',Sum(Total_Earning_Arrear) as Total_Earning_Arrear, SUM(CM.Other_Allow)as Other_Allowance, SUM(CM.Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(CM.Actual_CTC) as CTC,SUM(cm.advance_amount)as Advance_Amount,SUM(CM.PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(cm.ot_rate)as OT_Rate,SUM(cm.ot_hours)as OT_Hours,SUM(Cm.M_WO_OT_Hours) as WeekOff_OT_Hours,SUM(cm.M_HO_OT_Hours) as Holiday_OT_Hours
						,SUM(cm.ot_amount)as OT_Amount,SUM(cm.WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(cm.revenue_amount) as Revenue_Amount,SUM(cm.lwf_amount)as LWF_Amount,SUM(isnull(cm.Gate_Pass_Amount,0)) as Gate_Pass_Amount,SUM(isnull(cm.Asset_Installment_Amount,0)) as Asset_Installment_Amount,SUM(isnull(cm.Uniform_Dedu_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Cm.Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction ' 
						+ @sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(cm.net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,(SUM(Net_Round) + SUM(CM.Gross_Salary) '+ @Sum_Of_Allownaces_Earning_CTC_Total +') AS Total_Amount,
						...
						--		IF(ISNULL(@REPORT_TYPE,0) = 1)
						--	BEGIN
						--		Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF <> 1 group By Center_Code'+ @String_2 +''+ @String_3 +'' 
						--	END
						--ELSE IF(ISNULL(@Report_type,0) = 2)
						--	BEGIN
						--		Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF = 1 group By Center_Code'+ @String_2 +''+ @String_3 +'' 
						...
						--ELSE
						--	BEGIN
						--		Set @String_1 = @String_1 + ' INTO ##CenterCode from #CTCMast CM group By Center_Code'+ @String_2 +''+ @String_3 +'' 
						--	END 
						...
						IF(ISNULL(@REPORT_TYPE,0) = 1)
							BEGIN
								Set @String = @String + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF <> 1 group By Center_Code'+ @String_2 +''+ @String_3 +'' 
							END
						ELSE IF(ISNULL(@Report_type,0) = 2)
							BEGIN
								Set @String = @String + ' INTO ##CenterCode from #CTCMast CM where CM.IS_FNF = 1 group By Center_Code'+ @String_2 +''+ @String_3 +'' 
						...
						IF EXISTS(SELECT 1 FROM ##CenterCode)
							...
							----commented by mansi start
							--	set @String = 'Insert into ##CenterCode Select 0 as Flag,(IsNull(Max(Row_ID),0)+1)'+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
							--	+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
							--	,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
							--	+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
							--	SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##CenterCode';
							...
							--added by mansi start
								set @String = 'Insert into ##CenterCode Select 0 as Flag,(IsNull(Max(Row_ID),0)+1),''Total'' '+ @String_2 +', Sum(Total_Emp),Round((Sum(Total_Emp)/'  + cast(@Avg_Emp as varchar(10)) +'),0) ,Sum(Basic_Salary) ' + @sum_of_allownaces_earning + ',SUM(Production_Bonus) as Production_Bonus,SUM(Leave_Encash_Amount) as Leave_Encash_Amount,SUM(Uniform_Refund_Amount) as Uniform_Refund_Amount,Sum(Total_Earning) As Total_Earning, Sum(Basic_Arrear) As Basic_Arrear'
								+ @sum_of_allownaces_earning_Arear + ',Sum(Total_Earning_Arrear) as Total_Earning_Arrear,SUM(Gross_Salary)as Gross_Salary '+ @sum_of_allownaces_deduct +',SUM(Other_Allowance)as Other_Allowance,SUM(CTC) as CTC,SUM(Advance_Amount)as Advance_Amount,SUM(PT_Amount)as PT_Amount'+ @sum_of_Loan_Amount_Str + ',SUM(ot_rate)as OT_Rate,SUM(ot_hours)as OT_Hours,SUM(WeekOff_OT_Hours) as WeekOff_OT_Hours,SUM(Holiday_OT_Hours) as Holiday_OT_Hours
								,SUM(ot_amount)as OT_Amount,SUM(WO_HO_Fix_OT_Rate) as WO_HO_Fix_OT_Rate,SUM(Revenue_Amount) as Revenue_Amount,SUM(LWF_Amount)as LWF_Amount,SUM(Gate_Pass_Amount) as Gate_Pass_Amount,SUM(Asset_Installment_Amount) as Asset_Installment_Amount,SUM(isnull(Uniform_Installment_Amount,0)) as Uniform_Installment_Amount,SUM(ISNULL(Late_Deduction_Amount,0)) as Late_Deduction_Amount,SUM(ISNULL(Total_Deduction,0)) as Total_Deduction'
								+@sum_of_allownaces_deduct_Arear + ',Sum(Arear_Deduction) As Arear_Deduction,Sum(Net_Total_Deduction) as Net_Total_Deduction, SUM(net_amount)as Net_Amount,SUM(Net_Round) As Net_Round, Sum(Total_Net) As Total_Net '+ @Sum_Of_Allownaces_Earning_CTC +' ,Sum(Total_Amount) AS Total_Amount,
								SUM(Present_Days) as Present_Days,SUM(Arear_Days) as Arear_Days,SUM(Absent_Day) as Absent_Day,SUM(Holiday_day)as Holiday_day,SUM(Week_Off_Days)as Week_Off_Days,SUM(Sal_Cal_Day)as Sal_cal_Day,SUM(total_leave_days)as Total_leave_Days,SUM(Total_Paid_Leave_Days)as Total_Paid_Leave_Days '+ @sum_of_allownaces_earning_reim +' FROM ##CenterCode';
