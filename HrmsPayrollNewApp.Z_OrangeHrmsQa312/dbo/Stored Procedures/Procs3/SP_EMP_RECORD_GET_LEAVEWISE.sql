...
CREATE PROCEDURE [dbo].[SP_EMP_RECORD_GET_LEAVEWISE]  
  @Cmp_ID  numeric  
 ,@From_Date  datetime  
 ,@To_Date  datetime   
 ,@Branch_ID  numeric   = 0  
...
select @Leave_Type=Leave_Type from T0040_LEAVE_MASTER WITH (NOLOCK) WHERE LEAVE_ID =@Leave_ID

if @Leave_Type <> 'Maternity Leave'
	Begin
		
		select I_Q.* , cast( E.Alpha_Emp_Code as varchar) + ' - '+E.Emp_Full_Name as Emp_Full_Name,Emp_superior --Change  Empcode instead of Alfacode paras 02 - july -2013 
  ...
  WHERE E.Cmp_ID = @Cmp_Id And E.Gender='F'   
    And E.Emp_ID in (select Emp_ID From @Emp_Cons) order by E.Emp_Code asc
	End
 RETURN  
