...
CREATE PROCEDURE [dbo].[SP_GET_LEAVE_ENCASH_FACTORIES_ACT]
	 @Cmp_ID		Numeric
	,@From_Date		Datetime
	,@To_Date		Datetime
	,@Branch_ID		Numeric 
				...
				If @Lv_Encash_Cal_On = 'Gross'
						Begin
							...
							-- And LEA.Lv_Encash_Apr_Date > = @From_Date and LEA.Lv_Encash_Apr_Date <= @To_Date
							 ...
							 select distinct E.Emp_ID,E.Branch_ID,E.Grd_ID,E.Desig_Id,(I_Q.Gross_Salary) AS Gross_Salary,
									@From_Date  as From_Date, @To_Date as To_Date,'' as Wages_Type ,I_Q.Branch_ID 
									,E.Alpha_Emp_code,E.Emp_First_Name,0 as Lv_Encash_W_Day, 0 as Leave_Encash_Amount
									...
									--	ELSE 0 End as numeric(18,2)) as Leave_Encash_Day_Rate--,Leave_Encash_Amount,Lv_Encash_Apr_Days							
									,Grd_Name,Type_Name,Dept_Name,Desig_Name,Branch_Name,Leave_EncashDay_Half_payment,0 as Lv_Encash_Apr_Days,Branch_Address,Cmp_Name,Comp_Name
								from T0080_EMP_MASTER E  WITH (NOLOCK) inner join

								t0120_LEAVE_Encash_APPROVAL LEA  WITH (NOLOCK) on E.emp_ID=LEA.Emp_id 
								inner join
								...
								T0040_TYPE_MASTER TM WITH (NOLOCK) ON TM.type_Id = I_Q.Type_ID  INNER JOIN	
								...
								t0040_Leave_Master LM WITH (NOLOCK) On Lm.Leave_ID = Lea.Leave_ID and Lm.Cmp_ID = LEA.Cmp_ID 
