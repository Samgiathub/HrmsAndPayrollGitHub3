CREATE PROCEDURE [dbo].[SP_Get_Travel_Settlement_Application_Records_No_TravelType]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
	...
	(
								 ...
								 If @Emp_ID_Cur > 0
									Begin
										...
										----------------------------------------------Added by yogesh on 14102022-----------------------------------------------------------------------------------
											...
											--where sd.App_Emp_ID=@Emp_ID and sd.Scheme_Id=(
											...
											--where sd.App_Emp_ID=@Emp_ID and es.Type='Travel Settlement' and sd.Leave ='0'))
											--end
		...
		--Where App_Emp_Id = @Emp_ID and rpt_level = @Rpt_level	And T0040_Scheme_Master.Scheme_Type = 'Travel Settlement' 'Commented by yogesh and added a new line below on 14102022
									...
									--select * from #tbl_Scheme_Leave
										...
										If @Manager_Branch is not null and @Manager_Branch <>'0'
											Begin
													...
													Begin
														...
														If @Rpt_level = 1
															Begin
																...
																Set @SqlQuery = 	
																'Select LAD.Travel_Set_Application_id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' From V0140_Travel_Settlement_Application_New_Level_No_Travel_Type LAD
																		Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																	Where LAD.Travel_Set_Application_id Not In (Select Travel_Set_Application_id From T0115_Travel_Settlement_Level_Approval WITH (NOLOCK)
																														Where Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
																		  + ' And ' + @Constrains 
										 				...
										 				If @Rpt_level = 1
															Begin
																...
																--	  + ' And ' + @Constrains	 
																	  ...
																	  Set @SqlQuery = 	
																'Select Distinct LAD.Travel_set_Application_id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +  cast(@Rpt_level as VARCHAR(2)) +
																 ' From V0140_Travel_Settlement_Application_New_Level_No_Travel_Type LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																	Where LAD.Travel_set_Application_id Not In (Select Travel_set_Application_id From T0115_Travel_Settlement_Level_Approval WITH (NOLOCK)
																													Where Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
