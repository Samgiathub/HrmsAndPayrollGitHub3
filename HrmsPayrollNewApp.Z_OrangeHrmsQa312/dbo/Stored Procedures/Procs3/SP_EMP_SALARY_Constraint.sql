...
CREATE PROCEDURE [dbo].[SP_EMP_SALARY_Constraint]   
  ...
  begin  
      ...
      --  AND ISNULL(Emp_Id,0) = ISNULL(@Emp_Id,ISNULL(Emp_Id,0)) AND  
        ...
        AND ((@From_Date >= join_Date AND @From_Date <= left_date)        
          ...
          OR 1=(CASE WHEN ((@Show_Left_Employee_for_Salary = 1) AND (left_date >= 
		  case when (isnull(Sal_St_Date,'')) = ''  then @From_Date  
		  when day(Sal_St_Date) = 1  then @From_Date  else  (cast(cast(day(Sal_St_Date)as varchar(5)) + '-' + cast(datename(mm,dateadd(m,-1,@To_Date)) as varchar(10)) + '-' +  cast(year(dateadd(m,-1,@To_Date) )as varchar(10)) as smalldatetime)) end 
		  AND left_date <= case when (isnull(Sal_St_Date,'')) = ''  then @to_date when day(sal_st_date)=1 then @to_date else    
   ...
   INSERT INTO #Emp_Cons  
    SELECT CAST(DATA AS NUMERIC), CAST(DATA AS NUMERIC), CAST(DATA AS NUMERIC) FROM dbo.Split(@Constraint,'#')   
     ...
     INSERT INTO #Emp_Cons  
      ...
      WHERE V_Emp_Cons.cmp_id in(CC.Cmp_ID)      
        --AND ISNULL(Cat_ID,0) = ISNULL(@Cat_ID ,ISNULL(Cat_ID,0))   -- Comment by nilesh on 01112014  
        AND ISNULL(Cat_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@Cat_ID,ISNULL(Cat_ID,0)),'#')  T Where T.Data <> '')  -- Added by nilesh on 01112014   
        AND ISNULL(Grd_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@Grd_ID,ISNULL(Grd_Id,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 26-08-2014    
        AND ISNULL(Dept_ID,0) in( SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Dept_ID ,ISNULL(Dept_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 26-08-2014        
        --AND ISNULL(TYPE_ID,0) = ISNULL(@Type_ID ,ISNULL(TYPE_ID,0))     
        AND ISNULL([TYPE_ID],0) in( SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Type_ID ,ISNULL(Type_ID,0)),'#')  T Where T.Data <> '' )  
        AND ISNULL(Desig_ID,0) in( SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Desig_ID ,ISNULL(Desig_ID,0)),'#')  T Where T.Data <> '' ) -- ISNULL(@Desig_ID ,ISNULL(Desig_ID,0))  
        AND ISNULL(SalDate_id,0) = ISNULL(@Salary_Cycle_id ,ISNULL(SalDate_id,0))       
        --AND ISNULL(Segment_ID,0) = ISNULL(@Segment_ID,ISNULL(Segment_ID,0))  
        --AND ISNULL(Vertical_ID,0) = ISNULL(@Vertical,ISNULL(Vertical_ID,0))  
        --AND ISNULL(SubVertical_ID,0) = ISNULL(@SubVertical,ISNULL(SubVertical_ID,0))  
        --AND ISNULL(subBranch_ID,0) = ISNULL(@subBranch,ISNULL(subBranch_ID,0)) -- Added on 06082013   
        AND ISNULL(Segment_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@Segment_ID,ISNULL(Segment_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
        AND ISNULL(Vertical_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@Vertical,ISNULL(Vertical_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
        AND ISNULL(SubVertical_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@SubVertical,ISNULL(SubVertical_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
        AND ISNULL(subBranch_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@subBranch,ISNULL(subBranch_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
    ...
    Where Qry.Increment_ID is null  
      ...
      WHERE V_Emp_Cons.cmp_id=@Cmp_ID       
        ...
        AND ISNULL(SubVertical_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@SubVertical,ISNULL(SubVertical_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
        AND ISNULL(subBranch_ID,0) in (SELECT  CAST(DATA  AS Numeric) FROM dbo.Split(ISNULL(@subBranch,ISNULL(subBranch_ID,0)),'#')  T Where T.Data <> '' )  -- Added by nilesh on 03-Nov-2014   
        --AND  ISNULL(V_Emp_Cons.branch_id,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Branch_Constraint,ISNULL(V_Emp_Cons.branch_id,0)),'#')  T Where T.Data <> '' )  
        AND ISNULL(Emp_Id,0) = ISNULL(@Emp_Id,ISNULL(Emp_Id,0))   
        AND Increment_Effective_Date <= @To_Date   
        AND ((@From_Date >= join_Date AND @From_Date <= left_date)        
          OR (@To_Date >= join_Date AND @To_Date <= left_date)        
          ...
          OR 1=(CASE WHEN ((@Show_Left_Employee_for_Salary = 1)   
          AND (left_date >= case when (isnull(Sal_St_Date,'')) = ''  then @From_Date  when day(Sal_St_Date) = 1  then @From_Date    
          else  (cast(cast(day(Sal_St_Date)as varchar(5)) + '-' + cast(datename(mm,dateadd(m,-1,@To_Date)) as varchar(10)) + '-' +  cast(year(dateadd(m,-1,@To_Date) )as varchar(10)) as smalldatetime)) end AND left_date <= case when (isnull(Sal_St_Date,'')) = ''  then @to_date when day(sal_st_date)=1 then @to_date else    
          dateadd(d,-1,dateadd(m,1,(cast(cast(day(Sal_St_Date)as varchar(5)) + '-' + cast(datename(mm,dateadd(m,-1,@To_Date))   
          as varchar(10)) + '-' +  cast(year(dateadd(m,-1,@To_Date) )as varchar(10)) as smalldatetime)))) end))  THEN 1 ELSE 0 END)  
           
         )
		         AND  ISNULL(V_Emp_Cons.branch_id,0) in(SELECT  CAST(DATA  AS Numeric) FROM dbo.Split (ISNULL(@Branch_Constraint,ISNULL(V_Emp_Cons.branch_id,0)),'#') ) -- Add Tejas  :- 26122023  
        and For_Date = (select max(for_date) From T0040_General_Setting WITH (NOLOCK) where Cmp_ID = @Cmp_ID and Branch_ID = Isnull(@branch_id,V_Emp_Cons.Branch_ID))  --Modified By Ramiz on 17092014  
       ORDER BY Emp_ID  
