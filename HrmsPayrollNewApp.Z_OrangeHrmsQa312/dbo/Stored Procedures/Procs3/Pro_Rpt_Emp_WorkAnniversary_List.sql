...
CREATE PROCEDURE [dbo].[Pro_Rpt_Emp_WorkAnniversary_List]
	@Cmp_Id		numeric  
	,@From_Date		datetime
	,@To_Date 		datetime
	,@Branch_ID		numeric	
...
EXEC SP_RPT_FILL_EMP_CONS  @CMP_ID,@FROM_DATE,@TO_DATE,@BRANCH_ID,0,@Grade_ID,@TYPE_ID,@DEPT_ID,@DESIG_ID ,@EMP_ID ,@CONSTRAINT 
	...
	--  END      
			...
			REPLACE(convert(varchar(6),E.Date_Of_Join,106),' ',' - ') as Date_Of_Join 
			...
			from T0080_Emp_master E WITH (NOLOCK)
				...
				--)QRY1 
					...
					inner join (select distinct * from #month )M on month(E.Date_Of_Join) = m.month_id and (DAY(E.Date_Of_Join)>= m.from_Day_id and day(E.Date_Of_Join) <= m.to_Day_id)
		 where E.cmp_id=@Cmp_Id
				and E.emp_id in (Select emp_id from t0080_emp_Master WITH (NOLOCK) where Emp_Left_Date is null or (CONVERT(VARCHAR(10),Emp_Left_Date,120) >= CONVERT(VARCHAR(10),@To_Date,120) and day(Date_Of_Join) <= day(Emp_Left_Date)) and cmp_id=@Cmp_Id) 
		...
		order by month(E.Date_Of_Join),day(E.Date_Of_Join)
