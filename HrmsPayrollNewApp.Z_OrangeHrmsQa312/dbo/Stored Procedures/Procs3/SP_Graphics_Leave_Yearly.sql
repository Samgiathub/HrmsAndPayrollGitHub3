...
CREATE PROCEDURE [dbo].[SP_Graphics_Leave_Yearly]  
  @Cmp_ID  Numeric  
 ,@From_Date  Datetime  
 ,@To_Date  Datetime  
 ,@Branch_ID  Numeric   
...
/* select el.*,Leave_Name,Emp_Full_Name,Emp_Code,g.Grd_Name,b.BRanch_Address,b.Comp_name  
 ...
 From @Emp_Leave_Bal el Inner Join T0040_LEAVE_MASTER as l on el.Leave_ID = l.Leave_ID inner join  
  T0080_EMP_MASTER e  on el.Emp_ID =e.Emp_ID inner join   
  (select I.Emp_Id ,Grd_ID,Branch_ID,Dept_ID,Desig_ID,Type_ID from T0095_Increment I inner join   
  ...
  T0010_Company_master as CM on e.cmp_ID = cm.Cmp_ID   */


select sum(leave_used),el.cmp_id,l.leave_name from @emp_leave_bal as el inner join 

T0040_LEAVE_MASTER as L WITH (NOLOCK) on el.Leave_ID = l.Leave_ID group by 

el.cmp_id,l.leave_name
