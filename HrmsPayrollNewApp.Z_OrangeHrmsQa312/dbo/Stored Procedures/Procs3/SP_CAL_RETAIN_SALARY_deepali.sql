Create PROCEDURE [dbo].[SP_CAL_RETAIN_SALARY_deepali]
...
AS
	...
	Begin
		...
		select @MnLock_St_Date = From_Date,@MnLock_end_Date = To_Date  
		from T0090_Retaining_Lock_Setting where Cmp_Id = @Cmp_ID and  datediff(d,From_Date,@Ret_Start_Date) >=0  
		delete from T0210_Retaining_MonthLock_Details
		delete from T0210_Retaining_Datewise_Payment
		...
		Declare Curs_MNLOCKSlab cursor for	                  
			...
			where M.RRate_Id = D.RRate_ID and M.ad_id= @ad_id and M.Grd_ID=@Grd_ID  order by From_Limit
				...
				Begin    
					...
					--select @MnLock_Slab_STdate,@MnLock_Slab_Enddate
	...
	if @Retain_Days >0 
		begin
			...
			begin
				...
				select @MNLock_slab_id= Slab_Id, @MNLock_slab_per = Slab_per , @MNLock_mode = mode 
   ...
   insert into #Temp_Emp
	...
	isnull(Q_I.Basic_Salary,0),'',0,0,@Ad_Id,0 ,Re.Tran_Id
  ...
  if exists(select 1 from tempdb.dbo.sysobjects where name ='#Temp_other' and type='U')
	...
	select * from #Temp_other where emp_id =27765
