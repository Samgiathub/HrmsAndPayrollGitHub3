CREATE PROCEDURE [dbo].[P0200_MONTHLY_SALARY_GENERATE_PRORATA]    
...
AS    
	...
	Declare @Late_Sal_Dedu_Days_cutoff NUMERIC(18, 4)    -- Added by tejas 18092024
    ...
    set @Late_Sal_Dedu_Days_cutoff = 0.0 -- Added by tejas 18092024
	...
	DECLARE @Early_Sal_Dedu_Days_cutoff NUMERIC(18, 4)  -- Added by tejas 18092024
    ...
    set @Early_Sal_Dedu_Days_cutoff = 0.0 -- Added by tejas 18092024
    ...
    declare @Late_after_Cutoff_date as NUMERIC(18, 4)
	declare @Early_after_Cutoff_date as NUMERIC(18, 4)
    ...
    WHILE ( @intFlag_monthly <= @Count_emp_monthly )
        BEGIN
         ...
         SET @TotASSET_Closing = 0;
            ...
            --Alpesh 23-Mar-2012 put this to get Branch_Id to get Salary_St_Date when Branches have diff Salary_St_date but chk for Mid Increment
            select  @Branch_ID = Branch_ID,@CUST_AUDIT = ISNULL(I.CUSTOMER_AUDIT,0)  
			--Change By Jaina 06-10-2016
            ...
            set @Late_after_Cutoff_date =0
			set @Early_after_Cutoff_date =0
            ...
            If @M_Sal_Tran_ID > 0     
                Begin    
					--if exists(select 1 from T0200_MONTHLY_SALARY where Sal_Tran_ID =  @M_Sal_Tran_ID)
					--	set @Sal_Tran_Id = @M_Sal_Tran_ID + 1
					--else
            ...
            Else    
                Begin    
					...
					if exists(select 1 from T0200_MONTHLY_SALARY where Sal_Tran_ID =  @Sal_Tran_Id)
						set @Sal_Tran_Id = @Sal_Tran_ID + 1
					else
            ...
            */
                        ...
                        if @is_present_on_holiday = 1
                            BEGIN
                               ...
                               INNER JOIN (
											...
											and Month(H_From_DATE) = Month(@Month_St_Date) and  Year(H_from_date) = YEAR(@Month_St_Date)
              ...
              If @Is_Manual_Present = 0     --Added by Ramiz on 04/07/2015 , in order to Add regular OT hours & Uploaded OT Hours ( For Bhaskar )
                            ...
                            Begin   
                                ...
                                Else
                                    Begin
                                        ...
                                        exec SP_CALCULATE_LATE_DEDUCTION @emp_Id,@Cmp_ID,@T_MONTH_START_DATE,@T_MONTH_END_DATE,@Late_Absent_Day output,@Late_Sal_Dedu_Days_cutoff output,@Total_LMark output,@Total_Late_Sec output,@Increment_ID,@StrWeekoff_Date,@StrHoliday_Date,0,'',0,@Absent_date_String,0,@total_count_all_incremnet,@Mid_Inc_Late_Mark_Count
				...
				--Century Enka Late Early Scenario 2 Develop by Jignesh bhai 20-03-2024
					...
					if @Late_Mark_Scenario = 2  and @LateEarly_MonthWise = 0 
						and  (Exists(select 1 from T0050_GENERAL_LATEMARK_SLAB where DEDUCTION_TYPE = 'Hours' and CMP_ID = @Cmp_ID) or @IsDeficit = 1)
						begin
								...
								BEGIN
										...
										--Drop Table #TempData1 if Exists
										...
										from (
											...
											INNER JOIN T0095_INCREMENT I WITH (NOLOCK) ON I.INCREMENT_ID = @Increment_ID
											INNER JOIN T0040_GENERAL_SETTING GS WITH (NOLOCK) ON  GS.Branch_ID = @Branch_Id
											INNER JOIN(
											...
											WHERE (CASE WHEN GS.IS_LATE_MARK = 0 THEN 0 ELSE I.Emp_Late_Mark END) = 1  
											or	(I.Emp_Early_mark = 1) and AM.Emp_id = @emp_id
										) a
										
										UPDATE #Emp_Late_Early set Early_Deduction = 0 , Late_Deduction = 0 
										select ROW_NUMBER() OVER(partition by (emp_Id) ORDER BY (SELECT NULL)) AS RowNo,* into #TempData 
										from #TempData1
										...
										Declare @TotalMinute as numeric(18,2) = 0
										Declare @RowDeduct as numeric(18,2) = 0
										
										SELECT @TotalMinute = CAST((SUM(LATE_SEC) + SUM(EARLY_SEC))/60 AS INT) 
										FROM #EMP_LATE_EARLY E 
										WHERE LATE_SEC > 0 OR  EARLY_SEC > 0 AND E.EMP_ID = @EMP_ID
										GROUP BY E.EMP_ID
										
										Select @RowDeduct = G1.DEDUCTION 
										from T0040_GENERAL_SETTING G 
										inner join T0050_GENERAL_LATEMARK_SLAB G1 on G.Branch_Id = @Branch_ID  and  G.GEN_ID = G1.GEN_ID
										where @TotalMinute Between FROM_MIN and TO_MIN
										
										

										UPDATE #Emp_Late_Early  set Late_Deduction = 0 ,Early_Deduction = 0
										
										UPDATE #Emp_Late_Early set Deduct_Days = @RowDeduct  where Emp_ID = @Emp_Id
										
										set @Late_Absent_Day = @RowDeduct 
										
										set @Total_Late_Sec = dbo.F_Return_Sec(Replace(@RowDeduct,'.',':'))
										--SELECT   
										--distinct 
										--@Late_Absent_Day = @RowDeduct 
										--,@Total_Late_Sec = [dbo].[F_Return_Sec](@RowDeduct)
										--from #Emp_Late_Early Where Emp_ID = @Emp_ID
										--Group by @Emp_Id
				...
				-- Deepal
					--Select @MonthExemptLimitInSec,@LateEarly_MonthWise,@IsDeficit
					--IF @MonthExemptLimitInSec > 0 and @LateEarly_MonthWise = 0 and @IsDeficit = 1
					--BEGIN
					--	exec MonthLimitIsDeficitOrNot @CMP_ID,@MonthExemptLimitInSec, @Emp_Id -- deepal
					--	select @Late_Absent_Day = Sum(P_Days), @Total_Late_Sec = Sum(P_Days)
					--	from #Data where p_days	= 0.5 
					--END
                ...
                If @Is_Early_Mark = 1  and @Is_Manual_Present = 0 and @Late_Early_Ded_Combine = 0 and @Early_Mark_Scenario = 1 -- Added by Hardik 10/09/2012
                    Begin
                       exec SP_CALCULATE_EARLY_DEDUCTION @emp_Id,@Cmp_ID,@T_MONTH_START_DATE,@CutoffDate_Salary,@Early_Sal_Dedu_Days output,@Early_Sal_Dedu_Days_cutoff output,@Total_EarlyMark output,@Total_Early_Sec output,@Increment_ID,@StrWeekoff_Date,@StrHoliday_Date,0,'',0,@Absent_date_String,@total_count_all_incremnet,@Mid_Inc_Early_Mark_Count
					   ...
					   --comment by tejas for cutoff salary
            ...
            ----------hasmukh OD leave                
              ...
              ---End-- Slabwise calculation for late/early
                        ...
                        If @Inc_Weekoff = 0  ---Added by Hasmukh 30102013
                            Begin
                                ...
                                Else
                                    ...
                                    End
								------Added below by tejas with chintan bhai for Rayzon Solar at 06082024
								If @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 
                                    Begin
                                        SET @Absent_Days = @Present_Days - (@Present_Days + @Paid_Leave_Days + @OD_leave_Days + isnull(@Out_of_day_before_join,0) + Isnull(@OutOf_Days_left,0) + Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0) ) --Added gate pass by Muslim 18/03/2015                                      
									End
								IF  (year(@Join_Date) = year(@Month_St_Date) and month(@Join_Date) = month(@Month_St_Date) and day(@Join_Date) > day(@Month_St_Date) and @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 ) 
								BEGIN
										SET @Absent_Days = @Present_Days - (@Present_Days + @Paid_Leave_Days + @OD_leave_Days  + Isnull(@OutOf_Days_left,0) + Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0) ) --Added gate pass by Muslim 18/03/2015                                      
								END
								IF (year(@Left_Date) = year(@Month_End_Date) and month(@Left_Date) = month(@Month_End_Date) and day(@Left_Date) < day(@Month_End_Date) and @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 ) 
								BEGIN
									SET @Absent_Days = @Present_Days - (@Present_Days + @Paid_Leave_Days + @OD_leave_Days + isnull(@Out_of_day_before_join,0)  + Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0) ) --Added gate pass by Muslim 18/03/2015                                      
								END
                            End
                        Else
                            Begin
								...
								If @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 
                                    Begin
                                        SET @Absent_Days = @Present_Days - (@Present_Days +  @WeekOff_Days + @Paid_Leave_Days + @OD_leave_Days + isnull(@Out_of_day_before_join,0)+ Isnull(@OutOf_Days_left,0)+ Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0))                                          
                                    End
								IF  (year(@Join_Date) = year(@Month_St_Date) and month(@Join_Date) = month(@Month_St_Date) and day(@Join_Date) > day(@Month_St_Date) and @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 ) 
								BEGIN
										SET @Absent_Days = @Present_Days - (@Present_Days +  @WeekOff_Days + @Paid_Leave_Days + @OD_leave_Days + Isnull(@OutOf_Days_left,0)+ Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0))                                          
								END
								IF (year(@Left_Date) = year(@Month_End_Date) and month(@Left_Date) = month(@Month_End_Date) and day(@Left_Date) < day(@Month_End_Date) and @Inc_Holiday = 0 and @Emp_OT = 0 and @Cancel_Weekoff = 0 ) 
								BEGIN
									SET @Absent_Days = @Present_Days - (@Present_Days +  @WeekOff_Days + @Paid_Leave_Days + @OD_leave_Days + isnull(@Out_of_day_before_join,0)+ Isnull(@Compoff_leave_Days,0)+ ISNULL(@GatePass_Deduct_Days,0))                                          
								END
                ...
                --SET @Absent_Days = @Mid_Inc_Working_Day - (@Present_Days +  @Paid_Leave_Days + @OD_leave_Days + isnull(@Out_of_day_before_join,0))    -- changed on 19062012 , already deducted aboce from @Mid_Inc_Working_Day 
				  ...
				  --select @join_date,@Left_Date,@Present_Days,@Absent_Days
                If @Absent_Days < 0    
                    BEGIN
				...
				--Added new setting by Mr.Mehul on 10-May-2023
				-- Added by tejas 18092024
				IF exists(select 1 from T0200_MONTHLY_SALARY where MONTH(Month_End_Date) =  month(dateadd(m,-1,@Month_End_Date)) 
                and year(Month_End_Date) =  Year( dateadd(m,-1,@Month_End_Date)) and Emp_ID=@Emp_Id 
                and cutoff_date <> Month_End_Date)  
                BEGIN
				  Set @Absent_after_Cutoff_date = @Absent_after_Cutoff_date - (@Late_Sal_Dedu_Days_cutoff + @Early_Sal_Dedu_Days_cutoff)
				END
				
                
                 --select @Absent_after_Cutoff_date,@Late_Sal_Dedu_Days_cutoff,@Early_Sal_Dedu_Days_cutoff -- tejas
                If @SalaryBasis ='Day'    
                    Begin    
                ...
                */
                ...
                IF @EMP_OT = 1 And @Shift_Wise_OT_Calculated = 0   
					...
					--select @Absent_after_Cutoff_date,@Arear_Month_cutoff,@Arear_Year_cutoff,@Salary_amount_Arear_cutoff,@Day_Salary_Arear_cutoff,@Working_days_Arear_cutoff
                    ...
                    EXEC dbo.SP_CALCULATE_ALLOWANCE_DEDUCTION @Sal_Tran_ID,@emp_ID,@Cmp_ID,@Increment_ID,@month_St_Date,@Month_End_Date,@Wages_type,@Basic_Salary,@Gross_Salary_ProRata,@Salary_Amount,@Present_Days,@Absent_Days,@Paid_leave_Days,@Sal_Cal_Days,@OutOf_Days,@OT_Amount output,@Day_Salary ,@Branch_ID,@M_IT_Tax,null,@late_Extra_Amount,@Allo_On_Leave,@Working_days_Day_Rate  ,@Areas_Amount, @IS_ROUNDING , @WO_OT_Amount output , @HO_OT_Amount output , @tmp_Month_St_Date , @tmp_Month_End_Date,@Arear_Day,@Arear_Month,@Arear_Year,@Salary_Amount_Arear,@total_count_all_incremnet,@Working_days_Arear,@Absent_after_Cutoff_date,@Arear_Month_cutoff,@Arear_Year_cutoff,@Salary_amount_Arear_cutoff,@Working_days_Arear_cutoff,@Extra_Audit_Total_OT_Amount,@CutoffDate_Salary,@Night_Shift_Count,@Shift_Wise_OT_Rate,0,0,0,@Late_Sal_Dedu_Days_cutoff,@Early_Sal_Dedu_Days_cutoff
                    --Added the code by Ramiz on 04/05/2016 for Only Gradewise Salary used in Mafatlals , discussed with Hardik bhai , as I need to Consolidate OT Hours in one single Header
                    IF (@Gradewise_Salary_Enabled > 0)
                        BEGIN
                    ...
                    WHERE   TEMP_SAL_TRAN_ID=@SAL_TRAN_ID
						...
						--SELECT M_AREAR_AMOUNT,* FROM dbo.T0210_MONTHLY_AD_DETAIL 
      --              WHERE   TEMP_SAL_TRAN_ID=@SAL_TRAN_ID -- tejas
             ...
             --Begin    
                ...
                --TDS Condition Ended By Ramiz on 03/08/2016.
                 ...
                 If @Salary_Depends_on_Production = 1
                    ...
                    if @mid_OT_Hours <> @Emp_OT_Hours_Num -- Deepal shah 30112024
					BEGIN
            ...
            BEGIN
                ...
                INSERT INTO T0200_MONTHLY_SALARY    
                         ...
                         ,Late_Early_Penalty_days,M_WO_OT_Hours,M_WO_OT_Amount,M_HO_OT_Hours,M_HO_OT_Amount, Is_Monthly_Salary, Arear_Basic, Arear_Gross, Arear_Day, OD_leave_Days,Extra_AB_Days,Extra_AB_Rate,Extra_AB_Amount,Settelement_Amount,
						  ...
						  ,Late_Days_Arear_Cutoff,Early_Days_Arear_Cutoff)
                VALUES  (@Sal_Tran_ID,@Sal_Receipt_No,@Emp_ID,@Cmp_ID,@Increment_ID,@tmp_Month_St_Date,@tmp_Month_End_Date,@Sal_Generate_Date,@mid_Sal_Cal_Days,@mid_Present_Days
						...
						,@Late_Sal_Dedu_Days_cutoff,@Early_Sal_Dedu_Days_cutoff)
