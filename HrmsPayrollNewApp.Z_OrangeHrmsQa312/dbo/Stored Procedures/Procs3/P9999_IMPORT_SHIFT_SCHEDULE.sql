...
CREATE PROCEDURE [dbo].[P9999_IMPORT_SHIFT_SCHEDULE]	
...
SET ARITHABORT ON
	...
	IF @Sup_Emp_Id > 0
		...
		UPDATE S SET CMP_ID=EM.Cmp_ID,
					 Emp_id = EM.emp_id --Added by ronakk 07122022
		FROM #SHIFT S INNER JOIN T0080_EMP_MASTER EM ON S.EMP_CODE = EM.ALPHA_EMP_CODE and EM.Cmp_ID = @Cmp_Id_Para -- Added by ronakk 06122022 cmp id 
		...
		IF (@Import_By = 1)
			BEGIN
				...
				begin
					...
					INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON S.EMP_CODE = EM.ALPHA_EMP_CODE  and EM.Cmp_ID = @Cmp_Id_Para -- Added by ronakk 06122022 cmp id 
				...
				IF EXISTS(		
							SELECT	SHIFT
							FROM	#SHIFT SFT
							WHERE SFT.SHIFT <>'' and	NOT EXISTS 
									(
										 SELECT SHIFT_NAME 
		...
		ELSE
			BEGIN
					...
					IF EXISTS(SELECT 1 
						FROM #SHIFT 
						WHERE ISNUMERIC(SHIFT) = 0 and isnull([SHIFT],'') <> '')
						BEGIN
				...
				ORDER BY FOR_DATE,EMP_ID ASC
					...
					IF (SELECT COUNT(1) FROM #SHIFT where SHIFT is null) <>(SELECT COUNT(1) FROM #SHIFT)
			...
			--			SELECT	1
			--			FROM	#SHIFT S  
			--			INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON S.Emp_Code = EM.Alpha_Emp_Code and EM.Cmp_ID = @Cmp_Id_Para --Change by ronakk 07122022 for get proper ID
			--			INNER JOIN T0100_WEEKOFF_ROSTER WR WITH (NOLOCK) ON S.EMP_ID = WR.EMP_ID AND S.FOR_DATE = WR.FOR_DATE
			...
			--			--Change by ronakk 07122022 Delete Logic

					

			--			Delete from T0100_WEEKOFF_ROSTER 
			--			where Tran_Id in (	SELECT	WR.Tran_Id
			--			FROM	#SHIFT S 
			--			INNER JOIN T0080_EMP_MASTER EM WITH (NOLOCK) ON S.Emp_Code = EM.Alpha_Emp_Code and EM.Cmp_ID = @Cmp_Id_Para
			--			INNER JOIN T0100_WEEKOFF_ROSTER WR WITH (NOLOCK) ON S.EMP_ID = WR.EMP_ID AND S.FOR_DATE = WR.FOR_DATE)
			...
			-- INSERT RECORDS IN T0100_WEEKOFF_ROSTER
			...
			FROM		#SHIFT WHERE ISNULL(WFLAG,'') <> '' -- Change by ronakk 07122022
