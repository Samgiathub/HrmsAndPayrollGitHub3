...
-- =============================================
CREATE PROCEDURE [dbo].[SP_CALCULATE_LATE_DEDUCTION_SLABWISE]
     @EMP_ID             NUMERIC
    ,@CMP_ID             NUMERIC
    ...
    IF  @EMP_LATE_MARK = 1
                            ...
                            IF UPPER(@DEDUCTION_TYPE) = 'DAYS'
                                BEGIN
                                    ...
                                    Else
                                        ...
                                        End
                                END
							ELSE IF UPPER(@DEDUCTION_TYPE) = 'HOURS'
								BEGIN
									UPDATE  #LATE_MARK_SLAB 
        ...
        IF @LATE_ADJ_AGAIN_OT = 0
            BEgin
				...
				SELECT @TOTAL_LATE_OT_HOURS = SUM(DEDUCTION),@TOTAL_LMARK = SUM(CURR_COUNT) FROM #LATE_MARK_SLAB Where DEDUCTION_TYPE = 'Hours'
				SELECT @TOTAL_LATE_SEC = SUM(TOTAL_DEDUCT_DAYS)*3600,@TOTAL_LMARK = SUM(CURR_COUNT),@TOTAL_LATE_OT_HOURS=0 FROM #LATE_MARK_SLAB Where DEDUCTION_TYPE = 'Hours'

            End
        Else
            Begin
                SELECT @LATE_SAL_DEDU_DAYS = SUM(TOTAL_DEDUCT_DAYS),@TOTAL_LMARK = SUM(CURR_COUNT),@TOTAL_LATE_SEC=0 FROM #LATE_MARK_SLAB Where DEDUCTION_TYPE = 'Days'
                SELECT @TOTAL_LATE_OT_HOURS = SUM(TOTAL_DEDUCT_DAYS),@TOTAL_LMARK = SUM(CURR_COUNT),@TOTAL_LATE_SEC=0 FROM #LATE_MARK_SLAB Where DEDUCTION_TYPE = 'Hours'
            End 
            
        --Select * From #LATE_MARK_SLAB
        --Select @VAR_RETURN_LATE_DATE
        --Select @TOTAL_LATE_OT_HOURS
