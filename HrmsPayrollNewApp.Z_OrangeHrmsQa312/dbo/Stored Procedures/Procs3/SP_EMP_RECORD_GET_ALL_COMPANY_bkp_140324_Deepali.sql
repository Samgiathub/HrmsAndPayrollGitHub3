...
create PROCEDURE [dbo].[SP_EMP_RECORD_GET_ALL_COMPANY_bkp_140324_Deepali]     
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
 ,@Branch_ID  numeric   
 ...
 if @Constraint <> ''      
	begin
		...
		INNER JOIN (
					...
					WHERE	--I1.Cmp_ID = @Cmp_Id And 
					I1.Increment_ID=(
												SELECT	MAX(INCREMENT_ID)
												FROM	T0095_INCREMENT I2 WITH (NOLOCK)
												WHERE	--I2.Cmp_ID = @Cmp_Id And 
												I2.Increment_Effective_Date = (
																						SELECT	MAX(INCREMENT_EFFECTIVE_DATE)
																						FROM	T0095_INCREMENT I3 WITH (NOLOCK)
																						WHERE	--I3.Cmp_ID = @Cmp_Id And 
																						I3.Emp_ID=I2.Emp_ID 
			...
			IF (@Mode = 'DROPDOWN' and @Flag = 0)
				BEGIN
					...
					FROM	(
								...
								E.Cmp_ID = @Cmp_Id AND --added by mansi 25-07-23
					...
					Else if  @Flag = 2
						BEGIN
							...
							SELECT	I_Q.* ,E.Emp_Code, 
								...
								(E.Emp_Left_Date IS NULL OR E.Emp_Left_Date > @To_Date) AND E.Date_Of_Join <= @To_Date
