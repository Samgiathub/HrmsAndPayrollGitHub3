...
CREATE PROCEDURE [dbo].[SP_EMP_Warning_Get]
	...
	,@Desig_ID			numeric = 0
	,@Emp_ID			numeric  = 0
	,@Vertical_ID		numeric  = 0	-- Added by Ramiz 24-Jul-2015 
	,@SubVertical_ID	numeric  = 0	-- Added by Ramiz 24-Jul-2015 
	,@Shift_Id			numeric  = 0	-- Added by Ramiz 27-Jul-2015 
 ...
 if @Constraint <> ''        
	  begin        
	   ...
	   and Increment_Effective_Date <= @To_Date 
		...
		delete  from #emp_cons where Increment_ID not in (select max(Increment_ID) from dbo.T0095_Increment WITH (NOLOCK)
	...
	End
		...
		--WHILE (@For_Date <= @To_Date) BEGIN
			...
			FROM	(#Warning D INNER JOIN (SELECT esd.Shift_ID, esd.Emp_ID, esd.Shift_Type,esd.For_Date
					FROM T0100_EMP_SHIFT_DETAIL esd WHERE Cmp_ID = ISNULL(@Cmp_ID,Cmp_ID) AND For_Date = @For_Date) ESD ON
