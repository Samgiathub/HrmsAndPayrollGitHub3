...
-- Created by rohit for payment slip publish on 21042017
---29/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[SP_EMP_Payment_View_Publish]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   = 0
	   ...
	   if @Sal_View_Type = 0 
		   begin		
			   select Distinct E.Emp_ID,E.Alpha_Emp_Code as Emp_Code,E.Emp_Full_Name as Employee_Name,DM.Dept_Name as Department, 
							case when IsNull(SPE.Is_Publish,0) = 1 then 'Publish' else 'Unpublish' end  as Is_Publish,Comments,MS.Process_Type,MS.Ad_Id
				from dbo.T0080_EMP_MASTER E WITH (NOLOCK)
					inner join
					( select DISTINCT I.Emp_Id,Branch_ID,Dept_ID from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							...
							Inner JOIN MONTHLY_EMP_BANK_PAYMENT MS WITH (NOLOCK) on EC.Emp_ID = MS.Emp_ID And Month(MS.For_Date) = Month(@To_Date) and Year(MS.For_Date) = Year(@To_Date)  
							and MS.Process_Type = @process_type 
							and MS.Ad_Id=@ad_id 
							LEFT OUTER JOIN dbo.T0250_Payment_PUBLISH_ESS SPE WITH (NOLOCK)  on E.Emp_ID = SPE.Emp_ID and sPE.Month = Month(@To_Date) and SPE.Year = Year(@To_Date) 
							and MS.Ad_Id =  SPE.Ad_id and MS.Process_Type = MS.Process_Type
							Left OUTER JOIN dbo.T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) on I_Q.Dept_ID = DM.Dept_Id 
				WHERE E.Cmp_ID = @Cmp_Id 
						AND MS.Status = ISNULL(@Salary_Status ,MS.Status) 
	     ...
	     begin
			   ...
			   from dbo.T0080_EMP_MASTER E WITH (NOLOCK) left outer join dbo.T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
					( select  I.Emp_Id,Branch_ID,I.Emp_Full_PF,I.Emp_Auto_Vpf,Dept_ID  from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							( select max(Increment_ID) as Increment_ID , Emp_ID from dbo.T0095_Increment WITH (NOLOCK)
							where Increment_Effective_date <= @To_Date
							and Cmp_ID = @Cmp_ID
							group by emp_ID  ) Qry on
							...
							dbo.T0030_BRANCH_MASTER BM WITH (NOLOCK) ON I_Q.BRANCH_ID = BM.BRANCH_ID  
							Inner join #Emp_Cons EC on E.Emp_ID = EC.Emp_ID 
							INNER JOIN MONTHLY_EMP_BANK_PAYMENT MS WITH (NOLOCK) ON EC.Emp_ID = MS.Emp_ID AND MONTH(MS.For_Date) = MONTH(@To_Date) AND YEAR(MS.For_Date) = YEAR(@To_Date)  
							and MS.Process_Type = @process_type and MS.Ad_Id=@ad_id 
							Inner Join dbo.T0250_Payment_PUBLISH_ESS SPE  WITH (NOLOCK) on E.Emp_ID = SPE.Emp_ID and IsNull(SPE.Is_Publish,0) = 1 and sPE.Month = Month(@From_Date) and SPE.Year = Year(@From_Date) 
							and SPE.Process_Type = @process_type and SPE.Ad_Id=@ad_id 
							Left outer join  dbo.T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) on I_Q.Dept_ID = DM.Dept_Id 
							LEFT OUTER JOIN  dbo.T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID  
				...
				from dbo.T0080_EMP_MASTER E WITH (NOLOCK) left outer join dbo.T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
					( select  I.Emp_Id,Branch_ID,I.Emp_Full_PF,I.Emp_Auto_Vpf,Dept_ID  from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							...
							dbo.T0010_COMPANY_MASTER CM WITH (NOLOCK) ON E.CMP_ID = CM.CMP_ID  
							 left outer join  dbo.T0250_Payment_PUBLISH_ESS SPE WITH (NOLOCK)  on E.Emp_ID = SPE.Emp_ID and sPE.Month = Month(@From_Date) and SPE.Year = Year(@From_Date)
							  and SPE.Process_Type = @process_type and SPE.Ad_Id=@ad_id 
							  Inner join  #Emp_Cons EC on E.Emp_ID = EC.Emp_ID INNER JOIN
							MONTHLY_EMP_BANK_PAYMENT MS WITH (NOLOCK) ON EC.Emp_ID = MS.Emp_ID AND MONTH(MS.For_Date) = MONTH(@To_Date) AND YEAR(MS.For_Date) = YEAR(@To_Date)  
							and MS.Process_Type = @process_type and MS.Ad_Id=@ad_id 
