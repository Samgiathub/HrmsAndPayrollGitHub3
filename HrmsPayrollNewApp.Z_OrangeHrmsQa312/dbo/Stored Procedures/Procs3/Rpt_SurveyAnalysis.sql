CREATE PROCEDURE [dbo].[Rpt_SurveyAnalysis]    
  @Cmp_Id  numeric(18,0)    
   ...
   --WHERE  SR.Survey_Id = @Survey_Id and (Survey_Type='Text' OR Survey_Type='Paragraph Text') --order by Sorting_No    
	  ...
	  select Distinct a.* from (
		 SELECT distinct 
			--A2.Data as Answer,
			SR.Answer,
		A1.Survey_Id,A1.SurveyQuestion_Id
		,Survey_Type
		,SR.Emp_Id,'="' + EM.Alpha_Emp_Code + '"' as Alpha_Emp_Code
		,EM.Emp_Full_Name as EmployeeName
		,A1.Survey_Question,    
		convert(varchar(15),em.Response_Date,103) as Response_Date
		,em.Mobile_No
		,EID.IMEI_No
		,SM.Survey_Title
		,convert(NVARCHAR(11),SM.SurveyStart_Date,103) as SurveyStart_Date
		,convert(NVARCHAR(11),SM.Survey_OpenTill,103)  as Survey_OpenTill
		
		--into #Temp
		FROM  T0052_SurveyTemplate A1 WITH (NOLOCK) 
		INNER JOIN    
		 (    
				SELECT t.Data,t1.SurveyQuestion_Id     
				FROM T0052_SurveyTemplate as t1 WITH (NOLOCK)    
				CROSS APPLY [dbo].Split(Question_Option,'#') as t    
				WHERE Survey_Id = @Survey_Id and Cmp_Id=@Cmp_Id and isnull(Question_Option,'') <> ''    
				AND t.Data<>''    
		 ) A2 ON A1.SurveyQuestion_Id = A2.SurveyQuestion_Id and a1.Cmp_Id=@Cmp_Id 
		INNER JOIN T0050_SurveyMaster SM WITH (NOLOCK) on SM.Survey_ID = A1.Survey_Id  and sm.Cmp_ID=@Cmp_Id   
		LEFT JOIN T0060_SurveyEmployee_Response SR WITH (NOLOCK) ON SR.Answer like '%' + A2.Data + '%' AND  sr.Cmp_Id=sm.Cmp_ID and  
		    A1.Survey_Id = SR.Survey_Id and A1.SurveyQuestion_Id = SR.SurveyQuestion_Id and sr.Cmp_Id=@Cmp_Id   
		LEFT JOIN #FINAL_EMP EM on EM.Emp_ID = SR.Emp_Id     
		LEFT JOIN(
						SELECT IMEI_NO,Emp_ID FROM T0095_Emp_IMEI_Details EI    
						WHERE TRAN_ID=(SELECT MAX(Tran_ID) FROM T0095_Emp_IMEI_Details WITH (NOLOCK) WHERE Emp_ID=EI.Emp_ID)
			  )EID ON EID.Emp_ID=SR.EMP_ID     
		WHERE A1.Survey_Id = @Survey_Id and (Survey_Type <> 'Title' and Survey_Type<>'Text' and Survey_Type<>'Paragraph Text') 
   ) A 
   inner join T0060_SurveyEmployee_Response Sr on Sr.Answer = A.Answer and Cmp_Id = @Cmp_Id and A.Emp_Id = sr.Emp_Id 

   UNION ALL
   SELECT distinct SR.Answer,SR.Survey_Id,SR.SurveyQuestion_Id,T.Survey_Type,SR.Emp_Id,'="' + EM.Alpha_Emp_Code + '"' as Alpha_Emp_Code,EM.Emp_Full_Name as EmployeeName,T.Survey_Question,    
   convert(varchar(15),em.Response_Date,103) as Response_Date,em.Mobile_No,EID.IMEI_No,SM.Survey_Title,convert(NVARCHAR(11),SM.SurveyStart_Date,103) as SurveyStart_Date,convert(NVARCHAR(11),SM.Survey_OpenTill,103) as Survey_OpenTill   
   FROM T0052_SurveyTemplate T WITH (NOLOCK)    
   INNER JOIN T0060_SurveyEmployee_Response SR WITH (NOLOCK) ON T.Survey_Id = SR.Survey_Id AND SR.SurveyQuestion_Id = T.SurveyQuestion_Id and sr.Cmp_Id=@Cmp_Id  
   INNER JOIN #FINAL_EMP EM ON EM.Emp_ID = SR.Emp_Id    
   INNER JOIN T0050_SurveyMaster SM WITH (NOLOCK) on SM.Survey_ID = T.Survey_Id   
   LEFT JOIN(SELECT IMEI_NO,Emp_ID FROM T0095_Emp_IMEI_Details EI WITH (NOLOCK)    
      WHERE TRAN_ID=(SELECT MAX(Tran_ID) FROM T0095_Emp_IMEI_Details WHERE Emp_ID=EI.Emp_ID))EID ON EID.Emp_ID=SR.EMP_ID  
   WHERE  SR.Survey_Id = @Survey_Id and (Survey_Type='Text' OR Survey_Type='Paragraph Text') 
