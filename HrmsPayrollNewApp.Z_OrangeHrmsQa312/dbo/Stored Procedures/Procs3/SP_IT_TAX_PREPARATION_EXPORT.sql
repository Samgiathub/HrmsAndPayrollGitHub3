CREATE PROCEDURE [dbo].[SP_IT_TAX_PREPARATION_EXPORT]
	 @Cmp_ID				numeric
	,@From_Date				Datetime
	,@To_Date				Datetime	 
...
AS
	...
	CREATE table #Tax_Report_output
	  ( 
		...
		grade_name Varchar(max),  ----added by aswini 12/01/2024
	...
	Declare @Output_tax Table
	 (
		...
		exec SP_IT_TAX_PREPARATION @Cmp_ID=@Cmp_ID,@From_Date=@From_Date,@To_Date=@To_Date,@Branch_ID=@Branch_ID,@Cat_ID=@Cat_ID,@Grd_ID=@Grd_ID,@Type_ID=@Type_ID,@Dept_ID=@Dept_ID,@Desig_ID=@Desig_Id,@Emp_ID=@Emp_ID,@Constraint=@Constraint,@Product_ID=0,@Taxable_Amount_Cond=0,@Form_ID=@form_id,@Salary_Cycle_id=0,@Segment_ID=0,@Vertical=0,@SubVertical=0,@subBranch=0,@IT_Declaration_Calc_On='On_Regular',@Sp_Call_For = @Sp_Call_For
...
--			select distinct(Emp_ID) , alpha_emp_code ,Emp_full_name from #Tax_Report_output
			SELECT DISTINCT(tro.Emp_ID) , tro.alpha_emp_code ,tro.Emp_full_name,tro.branch_Name,tro.Desig_Name,tro.Dept_Name,tro.grade_name,em.mobile_no,em.work_email,em.date_of_birth,em.date_of_join,em.emp_left_date,em.pan_no 
			from #Tax_Report_output tro inner join
			t0080_emp_master em on em.emp_id = tro.Emp_id 
		OPEN CUR_AD_Tax_emp 
		FETCH NEXT FROM CUR_AD_Tax_emp INTO @Emp_id_cur,@alpha_emp_code_cur,@Emp_Full_name_cur,@Branch,@Designation,@Dept,@Grade,@mo_no,@work_email,@dob,@doj,@left_date,@pan    -----here @branch,@designation,@Dept,@grade added by aswini 12/01/2024
			...
			BEGIN
				...
				if @flg = 0
					begin
						...
						update @Output_tax set val = 'Emp_Code,Name,Branch,Designation,Dept,Grade,Mobile,Work_Email,Date_Of_Birth,Join_Date,Left_Date,pan_no,' + val  where id = 4  -----here @branch,@designation,@Dept,@grade added by aswini 12/01/2024
						set @flg = 1
			    ...
			    update @Output_tax set val =  '="' + @alpha_emp_code_cur + '",' + @Emp_Full_name_cur + ',' + ISNULL(@Branch,'') + ',' + ISNULL(@Designation,'') + ',' + ISNULL(@Dept,'') + ',' + ISNULL(@Grade,'') + ',' +  ISNULL(@mo_no,'') + ',"' + ISNULL(@work_email,'') + '",' + isnull(convert(nvarchar(11),@dob ,103),'-') + ',' + isnull(convert(nvarchar(11),@doj ,103),'-') + ',' + isnull(convert(nvarchar(11),@left_date,103),'-')  + ',' + isnull(@pan,'-')   + ',' + val   where id = @count_cur
