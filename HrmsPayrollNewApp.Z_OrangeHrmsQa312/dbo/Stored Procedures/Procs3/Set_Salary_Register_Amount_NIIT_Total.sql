...
CREATE  PROCEDURE [dbo].[Set_Salary_Register_Amount_NIIT_Total]
 @Cmp_ID		numeric
,@From_Date		datetime
,@To_Date		datetime 
,@Branch_ID		numeric  
...
SET ARITHABORT ON
	...
	Declare @PF As Numeric(22,2)--Nikunj 27-04-2011
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
					...
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'P Days', @P_Days,'',2,'N',0)
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'A Days', @A_Days,'',3,'N',0)
				...
				/*	INSERT INTO dbo.#Temp_Salary_Muster_Report
					...
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year,'Basic', @Basic_Salary,'',5,'I',0)
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Settl', @Settl,'',6,'I',0)
					
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Other', @OTher_Allow,'',7,'I',0)
					...
					while @@fetch_Status = 0
						begin
							INSERT INTO dbo.#Temp_Salary_Muster_Report
							(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,AD_ID)
							VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, @Label_Name, 0,'',@Row_ID,'',0)
							fetch next from Cur_label into @Label_Name,@Row_ID
						end
					close Cur_Label
					deallocate Cur_Label
					
					--Added By Falak on 12-MAY-2011 ----------------------
					...
					------------ 12-MAY-2011 ------------------------
					  Declare @AD_ID numeric(18,0)
						...
						select Ad_Sort_Name ,M_Ad_Amount,t0050_ad_master.AD_Flag,t0050_ad_master.AD_ID from t0210_monthly_ad_detail MAD WITH (NOLOCK) inner join
						...
						and MAD.Emp_ID  = @Emp_ID
					open cur_allow
					fetch next from cur_allow  into @Allow_Name ,@Amount,@AD_FLAG,@AD_ID
					while @@fetch_status = 0
						begin
					...
					deallocate Cur_Allow
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year,
											   Amount = @CO_Amount, Value_String = '' ,M_AD_Flage='N' --Added by Falak on 25-MAR-2011
					...
					declare Cur_Dedu   cursor for
						select Ad_Sort_Name ,M_Ad_Amount, t0050_ad_master.AD_FLAG,t0050_ad_master.AD_ID from t0210_monthly_ad_detail MAD WITH (NOLOCK) inner join
							t0050_ad_master WITH (NOLOCK) on MAD.Ad_Id = t0050_ad_master.Ad_ID 
							--left outer join #Temp_Salary_Muster_Report---Left Outer Forgot by Nilay put by nikunj 27-04-2011
						...
						and MAD.Emp_ID  = @Emp_ID
					open Cur_Dedu
					fetch next from cur_DEDU  into @Allow_Name ,@Amount,@AD_FLAG,@AD_ID
					while @@fetch_status = 0
						begin
							select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like @Allow_Name 
							...
							SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = @Amount, 
												  Value_String = '',M_AD_FLAGE=@AD_FLAG,AD_ID =@AD_ID
							WHERE     (Label_Name = @Allow_Name) AND (Row_id = @Row_ID)
									and Emp_ID = @Emp_ID
							fetch next from Cur_Dedu into @Allow_Name,@Amount,@AD_FLAG,@AD_ID
						end
					...
					--Select * From #Temp_Salary_Muster_Report
					  ...
					  on AD.Ad_Sort_Name =E1.Label_Name where E1.Emp_ID =@Emp_ID and E1.Cmp_ID=@Cmp_ID
					   and E1.M_AD_Flage ='' 
						...
						--select * from T0200_Monthly_salary where Emp_ID = @Emp_ID and Month(Month_St_Date) = @Month and Year(Month_St_Date) = @Year
						--Select @Other_Dedu  = 0						
		...
		--				VALUES     (@Emp_ID, @Company_ID, @Transaction_ID, @Month, @Year, 'Other Dedu', @Other_Dedu,'',@Row_ID)
						...
						/*select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'PF'
						
						UPDATE dbo.#Temp_Salary_Muster_Report
						SET    Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = @PF, Value_String = '',M_AD_Flage='D'
						WHERE  (Label_Name = 'PF') AND (Row_id = @Row_ID) and Emp_ID = @Emp_ID*/
						...
						--if @LWF_amt > 0
							--begin
								...
								UPDATE    dbo.#Temp_Salary_Muster_Report
								SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = isnull(@lwf_Amt,0), 
													  Value_String = '',M_AD_Flage='D'
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, 
											  Amount = isnull(@Total_Deduction,0), Value_String = '',M_AD_Flage='D'
						...
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = @Net_Salary, 
											  Value_String = '' ,M_AD_Flage='N' --Added by Falak on 25-MAR-2011
	...
	---	Select * from #Temp_Salary_Muster_Report where Amount > 0
