...
CREATE PROCEDURE [dbo].[SP_EMP_SALARY_RECORD_GET1_Backup_Tejas(26/12/2023)]  
  @Cmp_ID  NUMERIC  
 ,@From_Date  DATETIME  
 ,@To_Date  DATETIME   
 ,@Branch_ID  NUMERIC   = 0  
   ...
   ELSE  
    BEGIN      
     ...
     FROM T0080_EMP_MASTER E WITH (NOLOCK)  
       ...
       LEFT OUTER JOIN T0200_Emp_ExitApplication EA WITH (NOLOCK) ON EA.emp_id=E.Emp_ID and EA.[status]<>'R' --Mukti(19122019)  
     WHERE E.Cmp_ID = @Cmp_Id and E.Branch_ID in(@Branch_Constraint) and LO.Is_Active = 1                 
     ORDER BY  Emp_Code ASC  
...
-- Changed By rohit on 27082014  
 ...
 IF @FILTERTYPE = 0  
  BEGIN  
   SELECT Distinct * FROM #SALARY_TEMP  
  END  
 IF @FILTERTYPE = 1   
  BEGIN  
   SELECT Distinct * FROM #SALARY_TEMP WHERE COLOR_FLAG = 1   
  END  
 ELSE IF @FILTERTYPE = 2  
  BEGIN  
   SELECT Distinct * FROM #SALARY_TEMP WHERE COLOR_FLAG = 2   
  END  
 ELSE IF @FILTERTYPE = 3  
  BEGIN  
   SELECT Distinct * FROM #SALARY_TEMP WHERE EMP_FIX_SALARY = 1   
  END  
 ELSE IF @FILTERTYPE = 4  
  BEGIN  
   SELECT Distinct * FROM #SALARY_TEMP WHERE COLOR_FLAG = 3  
  END  
