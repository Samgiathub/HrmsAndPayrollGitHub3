...
CREATE PROCEDURE [dbo].[SP_CALCULATE_PRESENT_DAYS_GRAPHICS]       
  @Cmp_ID    numeric        
 ,@From_Date   datetime        
 ,@To_Date    datetime         
 ,@Branch_ID   numeric        
  ...
  begin        
   ...
   group by eir.Emp_ID  ,eir.For_Date,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit  
    ...
    WHILE @Tmp_Date <=@To_Date BEGIN 
		--Updating Shift ID From Latest Employee Shift Detail by Default
		UPDATE	#Data        
 ...
 Deallocate Cur_Shift         
  ...
  if @Return_Record_set =2         
   ...
   begin        
    ...
    select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name, CONVERT(decimal(10,2), sum(Duration_in_Sec))   as Working_Hour ,sum(OT_SEc)  as OT_Hour ,sum(P_days) as Present_Days        
   From #Data_Temp_3  OA inner join T0080_emp_master E WITH (NOLOCK) on OA.Emp_ID = E.Emp_ID        
   Group by OA.emp_ID,E.Emp_Full_Name     
      
      
      
     select OA1.Emp_ID,Max(For_Date)For_Date,E1.Emp_Full_Name,Working_Hour,dbo.F_Return_Hours(OT_HOur) as OT_Hour ,P_days  
    ...
    Group by OA1.emp_ID,E1.Emp_Full_Name ,For_Date,  Working_Hour,OT_Hour,P_days  
      ...
      insert into @Emp_Inout(Working_HOurs,OT_Hours,P_days)
		
		 select  CONVERT(decimal(10,2), sum(Duration_in_Sec))   as Working_Hour ,CONVERT(decimal(10,2),(sum(OT_SEc)))  as OT_Hour ,sum(P_days) as Present_Days        
    From #Data_Temp_3  
