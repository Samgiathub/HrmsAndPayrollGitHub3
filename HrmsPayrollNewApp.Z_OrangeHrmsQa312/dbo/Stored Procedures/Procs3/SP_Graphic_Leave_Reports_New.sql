...
CREATE PROCEDURE [dbo].[SP_Graphic_Leave_Reports_New]
	 @Cmp_ID  Numeric  
	,@From_Date  Datetime  
	,@To_Date  Datetime  
 	,@Branch_ID		numeric   = 0
	...
	if @Constraint <> ''
		...
		begin
		 if @PBranch_ID <> '0' and isnull(@Branch_ID,0) = 0
	       ...
	       End 
		Begin
			select (Sum(T.Leave_Used) + SUM(T.CompOff_Used)) as Leave_Used,l.Leave_Code,L.leave_name +'-('+l.Leave_Code+')' as Leave_Name  From T0140_LEave_Transaction T WITH (NOLOCK) inner -- Changed By Gadriwala Muslim 01102014
			join T0040_Leave_Master L WITH (NOLOCK)  on T.Leave_id = L.Leave_id
				...
				and T.Emp_Id in(select Emp_Id from @Emp_Cons)
				 group by L.leave_name,Leave_Code order by Leave_Name
        end
