...
-- =============================================
CREATE PROCEDURE [dbo].[SP_Get_OT_Level_Approval_Records] @Cmp_ID NUMERIC(18, 0)
	,@Emp_ID NUMERIC(18, 0)
...
BEGIN
	...
	BEGIN
		...
		IF (select top 1 rpt_level from #OVERTIME ) > 1
		BEGIN
			SELECT OVT.*
			,dbo.F_Get_OT_QUARTERLYHOURS(OVT.Emp_ID, OVT.For_DATE) AS TOT_Qtr_Hours
			,EMP.Alpha_Emp_Code + EMP.Emp_Full_Name as 'Prv_Emp_Full_Name' 
		FROM #OVERTIME OVT
		LEFT JOIN T0115_OT_LEVEL_APPROVAL OTL ON OTL.Emp_ID = OVT.Emp_ID AND OTL.For_Date = OVT.For_DATE AND  OTL.Rpt_Level  = CASE WHEN OVT.rpt_level = 1 THEN OVT.rpt_level ELSE (OVT.rpt_level -1) END  
		INNER JOIN T0080_EMP_MASTER EMP ON EMP.Emp_ID = OTl.S_Emp_Id
	...
	END
	ELSE IF @Type = 3 -- Added By tejas for Export Pending request at any level of reporting
		BEGIN
			
			WITH RankedEmployees AS (
		    SELECT 
		        emp_ID,For_Date,dbo.F_Return_Sec(MAX(Ot_Hour))as 'Ot_Sec',dbo.F_Return_Sec(MAX(Weekoff_OT_Hour))as 'WOt_Sec',dbo.F_Return_Sec(MAX(Holiday_OT_Hour))as 'HOt_Sec'
		    FROM #OVERTIME
			where cmp_ID = @Cmp_ID 
			group by Emp_ID,For_Date
		)
		SELECT  RS.Emp_ID,RS.For_Date
		,EMPS.Scheme_ID,EMPS.App_Emp_ID,EMPS.Rpt_Level ,RS.Ot_Sec,RS.WOt_Sec,RS.HOt_Sec
		INTO #Emp_OT_LISt
		...
		,dbo.F_Return_Hours(ESL.Ot_Sec + Esl.WOt_Sec + Esl.HOt_Sec) as 'OT_Hours'
