...
CREATE Procedure [dbo].[Rolling_Manpower_Report_backupbymanisha_17022025]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Emp_ID		int = 0
...
Begin
		...
		select @previousMonthStartDate = DATEADD(MONTH, DATEDIFF(MONTH, 0, cast(@From_Date as date))-1, 0)
  ...
  --      WHERE I.CMP_ID = @Cmp_ID 
	...
	select  ROW_NUMBER() OVER(ORDER BY T.Dept_ID ASC) AS RowID,T.Dept_ID,DM.Dept_Name,BM.Branch_Name,T.Month_name,T.Emp_list into #EMp_List
	from (
	SELECT 
				I_Q.Branch_ID
				,I_Q.Dept_ID
	...
	Declare @curmonth as varchar(10) = Datename(MONTH,@From_Date)
	...
	BEGIN
		...
		BEGIN
			...
			and DM.For_date >= DATEFROMPARTS(YEAR(@From_Date), MONTH(@monthName + '1,1'), 1) and For_Date <= EOmonth(DATEFROMPARTS(YEAR(@To_Date), MONTH(@monthName + '1,1') , 1)) 
		END
	...
	select  @CurMonthBRCount = Count(distinct Branch_Name) from #EMp_List where Month_name = DATENAME(MONTH, @from_date) 
	if @PreMonthBRCount < @CurMonthBRCount
	BEGIN
		INSERT INTO #EMp_List
		Select Distinct RowID,Dept_ID,Dept_Name,Branch_name,@Premonth,Emp_list,null,NULL  from #EMp_List  where Branch_Name not in (Select Distinct Branch_Name from #EMp_List where Month_name = @Premonth)
	END
	select  @PreMonthBRCount =CounT(Distinct Branch_Name)  from #EMp_List  where Month_name = @Premonth 
	select  @CurMonthBRCount = Count(distinct Branch_Name) from #EMp_List where Month_name = DATENAME(MONTH, @from_date) 
	...
	BEGIN
		INSERT INTO #EMp_List
		Select Distinct RowID,Dept_ID,Dept_Name,Branch_name,DATENAME(MONTH, @from_date),Emp_list,null,NULL  from #EMp_List  where Branch_Name not in (Select Distinct Branch_Name from #EMp_List where Month_name = DATENAME(MONTH, @from_date) )
	END
	...
	select * INTO #Pre_month_data from #EMp_List  where Month_name = @monthName
	select * INTO #Cur_month_data from #EMp_List where Month_name = DATENAME(MONTH, @from_date)
