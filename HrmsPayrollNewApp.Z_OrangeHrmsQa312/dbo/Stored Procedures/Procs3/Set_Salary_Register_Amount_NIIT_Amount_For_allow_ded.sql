...
CREATE PROCEDURE [dbo].[Set_Salary_Register_Amount_NIIT_Amount_For_allow_ded]
 @Cmp_ID		numeric
,@From_Date		datetime
,@To_Date		datetime 
,@Branch_ID		numeric   = 0
...
SET ARITHABORT ON
	...
	Declare @Ar_Salary_Amt_Basic as numeric (22,2)--mansi
	...
	Declare @WeekOff_Amount as numeric(18,2)	
	Declare @Holiday_Amount as numeric(18,2)	
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
						...
						set @Claim_Amount = 0 
						set @Ar_Salary_Amt_Basic=0 -- mansi
					...
					If @Sal_Type = 0
						Begin
							...
							select @P_Days = isnull(Present_Days,0) ,@Sal=Basic_Salary,@A_Days = isnull(Absent_Days,0),@TDS=isnull(M_IT_TAX,0), @Basic_Salary = Salary_Amount + Isnull(Basic_Salary_Arear_cutoff,0), @Act_Gross_salary = Actually_Gross_salary,@Settl = Settelement_Amount,@OTher_Allow = ISNULL(Other_Allow_Amount,0),@Total_Allowance = Allow_Amount, @Leave_Amount = isnull(Leave_Salary_Amount,0),
								@Round_Value = Isnull(Total_Earning_Fraction ,0) , @Net_Round_Value = ISNULL(Net_Salary_Round_Diff_Amount,0)
								--,@OT_Amount = (ISNULL(OT_Amount,0) + ISNULL(M_WO_OT_Amount,0) + isnull(M_HO_OT_Amount,0)) 
								,@OT_Amount = ISNULL(OT_Amount,0) 
								,@WeekOff_Amount = ISNULL(M_WO_OT_Amount,0)
								,@Holiday_Amount = isnull(M_HO_OT_Amount,0)
								,@GatePass_Deduct_Days = ISNULL(GatePass_Deduct_Days,0),@GatePass_Amount = ISNULL(GatePass_Amount,0), -- Added by Gadriwala Muslim 10112014
				...
				/*	INSERT INTO dbo.#Temp_Salary_Muster_Report
					...
					--	and isnull(t0050_ad_master.Ad_Not_Effect_Salary,0) = 0 and Ad_Active = 1 and AD_Flag = 'I' and MAD.Emp_ID  = @Emp_ID
					  ...
					  where E1.Emp_ID =@Emp_ID and E1.Cmp_ID=@Cmp_ID
					   ...
					   --select * from #Temp_Salary_Muster_Report---mansi
					...
					Else
						BEGIN
							...
							inner join t0050_ad_master on MAD.Ad_Id = t0050_ad_master.Ad_ID  
							inner join #Temp_Salary_Muster_Report
						...
						Select  @Emp_ID as Emp_ID,@Cmp_ID as Cmp_ID,0 as Transaction_ID,@month as Month,@Year as Year,MAD.AD_ID,AD_NAME,mad.M_AD_Flag,AD_SORT_NAME,Isnull(SUM(M_AD_Amount),0)as M_Arrear_Amt  
						,'' as Value_String,0 as INCOME_TAX_ID
						into #TmpArrear_Amount From t0210_monthly_ad_detail MAD WITH (NOLOCK) inner join
								...
								Group By AD_SORT_NAME,MAD.AD_ID,MSS.Emp_ID,mad.M_AD_Flag,AD_NAME
												   --select * from #Temp_Salary_Muster_Report---mansi
											  --added by mansi start	   
										insert into #TmpArrear_Amount(Emp_ID,Cmp_ID,Transaction_ID,Month,Year,AD_ID,AD_NAME,M_AD_Flag,AD_SORT_NAME,M_Arrear_Amt,Value_String,INCOME_TAX_ID) 
										                 values(@Emp_ID,@Cmp_ID,0,@month,@Year,0,'','I','Basic',@Ar_Salary_Amt_Basic,'',0)		  
												 --added by mansi end  
...
--Added By Mukti(start)24032017
				...
				--Select * From #Temp_Salary_Muster_Report Where emp_Id=1 And MONTH=2 And Year=2011
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, 
											 Amount = @Total_Allowance+@Basic_Salary+isnull(@Settl,0)+ISNULL(@OTher_Allow,0)+isnull(@CO_Amount,0) + ISNULL(@Leave_Amount,0) + isnull(@OT_Amount,0)  + isnull(@WeekOff_Amount,0)   + isnull(@Holiday_Amount,0)  + isnull(@Arear_Net,0), Value_String = '',M_AD_Flage='I'
		...
		--				VALUES     (@Emp_ID, @Company_ID, @Transaction_ID, @Month, @Year, 'Other Dedu', @Other_Dedu,'',@Row_ID)
						...
						select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'OT'
							...
							where   Label_Name = 'OT' and Row_id = @row_Id                    
									...
									select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'OT WeekOff'
										Update dbo.#Temp_Salary_Muster_Report
							SET  Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year,
												   Amount = @WeekOff_Amount, Value_String = '',M_AD_Flage='I' 
							where   Label_Name = 'OT WeekOff' and Row_id = @row_Id                    
									and Emp_ID = @Emp_ID
									and S_Sal_Tran_ID = @Curr_S_Sal_Tran_ID
									
									
									select @Row_ID = Row_ID from dbo.#TEMP_REPORT_LABEL where Label_Name like 'OT Holiday' 

									Update dbo.#Temp_Salary_Muster_Report
							SET  Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year,
												   Amount = @Holiday_Amount, Value_String = '',M_AD_Flage='I' 
							where   Label_Name = 'OT Holiday' 
									and Row_id = @row_Id                    
									and Emp_ID = @Emp_ID
									and S_Sal_Tran_ID = @Curr_S_Sal_Tran_ID
	...
	--	IF OBJECT_ID('tempdb..#TmpArrear_Amount') IS NOT NULL
	--BEGIN
	  Select Distinct ta.Emp_ID,ta.Cmp_ID,ta.Transaction_ID,ta.Month,ta.Year, (ta.AD_SORT_NAME+'_Arear')as Label_name,
	  ta.M_Arrear_Amt as Amount,ta.Value_String,ta.INCOME_TAX_ID,
	 tr.Row_Id,ta.M_AD_Flag,0.00 as Rate,tr.S_Sal_Tran_Id
	  into #FinalTmpArrear_Amount
	  from dbo.#Temp_Salary_Muster_Report  Tr
	  inner join #TmpArrear_Amount ta on ta.AD_SORT_NAME=tr.Label_name

	 select t.* into #Final_Temp_Salary_Muster_Report from
	 (select * from #Temp_Salary_Muster_Report
	 Union all
	 select * from #FinalTmpArrear_Amount)as t
	 order by t.Row_id
	  --select * from  #Final_Temp_Salary_Muster_Report
	  	--added by mansi end
	 --Select Distinct Emp_ID,Cmp_ID,Transaction_ID,Month,Year,Label_name,Amount,Value_String,INCOME_TAX_ID,
	 --Row_Id,M_AD_Flage,Rate,S_Sal_Tran_Id
	 --from dbo.#Temp_Salary_Muster_Report  
	 --union all 
	 -- Select Distinct Emp_ID,Cmp_ID,Transaction_ID,Month,Year,Label_name,Amount,Value_String,INCOME_TAX_ID,
	 --Row_Id,M_AD_Flage,Rate,S_Sal_Tran_Id,
	 -- (Label_name+'_Arrear')as Label_name,Row_ID,M_AD_Flage  from dbo.#Temp_Salary_Muster_Report  Tr
	 -- inner join #TmpArrear_Amount ta on ta.AD_SORT_NAME=tr.Label_name
	...
	delete from #Final_Temp_Salary_Muster_Report where Row_id=6


	select dbo.#Final_Temp_Salary_Muster_Report.* 
	,ISNULL(E.EmpName_Alias_Salary,E.Emp_Full_Name) as Emp_Full_Name ,E.Alpha_Emp_Code as 'Emp_Code',GM.Grd_name,ETM.Type_name,DGM.Desig_Name,DM.Dept_Name,BM.Branch_Name, 
	E.Dept_ID,Cmp_Name,Cmp_Address,isnull(Inc_Qry.Inc_Bank_Ac_no,'')as Inc_Bank_Ac_no,isnull(Inc_Qry.Payment_Mode,'') as Payment_Mode,E.Father_name ,E.Alpha_Emp_Code, RIGHT(REPLICATE(N' ', 500) + E.ALPHA_EMP_CODE, 500) as Ord_Emp_code,E.Emp_First_Name as ord_Name,isnull(VS.Vertical_Name,'') as Vertical_Name , isnull(SV.SubVertical_Name,'') as SubVertical_Name
		,DGM.Desig_Dis_No,E.Emp_First_Name  --Added by Gadriwala Muslim 23122016        --added jimit 24082015
		,BM.Comp_Name,BM.Branch_Address, --add by chetan 27-12-16
		E.Worker_Adult_no,ISNULL(BA.bank_name,'')as Bank_Name,case when e.Gender='M' then 'Male' else 'Female' end as[Gender], Inc_Qry.Grd_ID,Inc_Qry.Type_ID,Inc_Qry.Desig_Id,
		Inc_Qry.Branch_ID,Inc_Qry.Vertical_ID,Inc_Qry.SubVertical_ID, Inc_Qry.subBranch_ID,SB.SubBranch_Name, E.Date_Of_Join,E.SIn_No
	Into #Final_Temp_Salary_Muster_Report1 -- Added by Hardik 17/04/2019
	from dbo.#Final_Temp_Salary_Muster_Report Inner join 
		T0080_Emp_Master E WITH (NOLOCK) on dbo.#Final_Temp_Salary_Muster_Report.Emp_Id = E.Emp_ID 
		inner join
		 	(select I.Emp_Id ,Grd_ID,DEsig_ID ,Dept_ID,Inc_Bank_Ac_no,Branch_ID,Type_ID,Payment_Mode,Vertical_ID ,SubVertical_ID,Bank_ID, I.subBranch_ID 
		 	   from t0095_Increment I WITH (NOLOCK)
		 		INNER JOIN 
						( SELECT MAX(INCREMENT_ID) AS INCREMENT_ID, I.EMP_ID 
							FROM T0095_INCREMENT I WITH (NOLOCK)
							INNER JOIN #Final_Temp_Salary_Muster_Report T ON T.EMP_ID = I.EMP_ID 
							INNER JOIN 
							(
									SELECT MAX(i3.INCREMENT_EFFECTIVE_DATE) AS INCREMENT_EFFECTIVE_DATE, I3.EMP_ID
									FROM T0095_INCREMENT I3 WITH (NOLOCK)
	...
	If @Summary_Option = 0
	Begin
		--Select  *,1 as Emp_Count ,'' as ReportGroupName from #Temp_Salary_Muster_Report1 --commented by mansi
		Select  *,1 as Emp_Count ,'' as ReportGroupName from #Final_Temp_Salary_Muster_Report1  --added by mansi
	End
   ...
   Select @sGroupByName = Case When @Summary_Option= 0 then 'Grd_name' 
		...
		,count(isnull(Emp_Count,0) ) as Emp_Count
		 ,''' + @sGroupByName +'''  as ReportGroupName
		From #Final_Temp_Salary_Muster_Report1
		Left Outer Join (Select Isnull(Count(Distinct Emp_Id),0) as Emp_Count,' + @sSqlSummaryOn +' as cnt_dept_id From #Final_Temp_Salary_Muster_Report1 Group By ' + @sSqlSummaryOn +' ) 
		AS tblCnt On #Final_Temp_Salary_Muster_Report1.' + @sSqlSummaryOn +' = tblCnt.cnt_dept_id
		Group By 
		' + @sSqlSummaryOn +',Cmp_ID,Transaction_ID,Month,Year,Label_Name, Value_String,INCOME_TAX_ID,M_AD_Flage
		, ' + @sGroupByName +',Cmp_Name,Cmp_Address,Comp_Name
	...
	--  --commented by mansi end
	 --added by mansi start
	  Select Distinct Label_name,Row_ID,M_AD_Flage  from dbo.#Final_Temp_Salary_Muster_Report  
	   order by Row_ID
	 --Select Distinct Label_name,Row_ID,M_AD_Flage  from dbo.#Temp_Salary_Muster_Report  
	 --union all 
	 -- Select Distinct (Label_name+'_Arrear')as Label_name,Row_ID,M_AD_Flage  from dbo.#Temp_Salary_Muster_Report  Tr
	 -- inner join #TmpArrear_Amount ta on ta.AD_SORT_NAME=tr.Label_name
	 --  --where dbo.#Temp_Salary_Muster_Report.Amount > 0
	 --order by Row_ID
	 --added by mansi end
	----Record set 3 for Total amount
	Declare @Total_earnings as numeric(18,2)
	declare @total_dedu as numeric(18,2)
	declare @Net_Amt as numeric(18,2)
	--select Sum(Amount) from #Final_Temp_Salary_Muster_Report where M_AD_Flage='I' and Emp_ID=@Emp_ID and Label_Name not like'%Gross%'
    set @Total_earnings=(select Sum(Amount) from #Final_Temp_Salary_Muster_Report where M_AD_Flage='I' and Emp_ID=@Emp_ID and Label_Name not like'%Gross%')
	print @Total_earnings
	 set @total_dedu=(select Sum(Amount) from #Final_Temp_Salary_Muster_Report where M_AD_Flage='D' and Emp_ID=@Emp_ID and Label_Name not like'%dedu%')
	print @total_dedu
	
	update #Final_Temp_Salary_Muster_Report set Amount=@total_dedu where Row_id=45
	set @Net_Amt=@Total_earnings-@total_dedu
	update #Final_Temp_Salary_Muster_Report set Amount=@Net_Amt where Row_id=46
	Select Sum(Amount) as Amount ,Sum(Rate) as Rate,Row_ID,Label_Name,M_AD_Flage 
	--from dbo.#Temp_Salary_Muster_Report--commented by mansi 
	from dbo.#Final_Temp_Salary_Muster_Report--added by mansi 
	...
	Select	
	Sum(Amount) as Amount ,Sum(Rate) as Rate,Row_ID,Label_Name,M_AD_Flage 
	--from dbo.#Temp_Salary_Muster_Report--commented by mansi 
	from dbo.#Final_Temp_Salary_Muster_Report--added by mansi 
	where M_AD_Flage='I'
	Group by Label_Name,Row_ID,M_AD_Flage 
	...
	IF @Group_Name = 0	-- Grade wise
		BEGIN
			...
			--FROM dbo.#Temp_Salary_Muster_Report1 --commented by mansi
			FROM dbo.#Final_Temp_Salary_Muster_Report1 --added by mansi
			WHERE M_AD_Flage in('I','D')
