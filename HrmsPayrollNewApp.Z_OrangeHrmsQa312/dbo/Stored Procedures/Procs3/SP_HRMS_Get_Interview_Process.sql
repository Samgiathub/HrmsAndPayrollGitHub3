...
CREATE PROCEDURE [dbo].[SP_HRMS_Get_Interview_Process]  
 @Cmp_ID  numeric(18,0) , 
...
SET ARITHABORT ON
  ...
  --select @Process_data
	...
	--	order by rec_post_code,QA.resume_code asc
		...
		--if @Process_data <> '' 
		--	set @Query = 'select *, isnull(Q.resume_status,0) as resume_status,case When isnull(RM.lock,0)=0 then 'No' else 'Yes' end as Lock_Status from @data QA
		--	left outer join T0060_RESUME_FINAL Q	on QA.resume_id=Q.resume_id
		--	left join T0055_Resume_Master RM on QA.resume_id = RM.Resume_Id
		...
		--	or status like + ''%' + @Process_data + '%'''
	--else
	--select * from #data
			set @Query = 'select *,isnull(Q.resume_status,0)as resume_status from #data QA
