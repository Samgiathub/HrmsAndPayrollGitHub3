...
CREATE PROCEDURE [dbo].[SP_EMP_RECORD_GET_ALL_COMPANY]     
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
 ,@Branch_ID  numeric   
 ...
 ,@TDS_Flag int = 0-- Added by deepali
		...
		if(@TDS_Flag = 1) 
			begin
			  ...
			  from		V_Emp_Cons  VE  inner join 
		       ...
		       where VE.cmp_id=@Cmp_ID and Isnull(Cat_ID,0) = Isnull(@Cat_ID ,Isnull(Cat_ID,0))      
				...
				and Increment_Effective_Date <= @To_Date 
				and ((@From_Date  >= join_Date  or  isnull(DateDiff(M,left_date,GETDATE()),0) <=6 ) or ( @To_Date  >= join_Date  and @To_Date <= left_date )   or (Left_date is null and @To_Date >= Join_Date) or (@To_Date >= left_date  and  @From_Date <= left_date ))
				order by Emp_ID

				
						--old query - 13032024 -start -Deepali 
					 -- ( (@From_Date  >= join_Date  and  @From_Date <= left_date )      
						...
						--old query - 13032024 -end -Deepali 
		  ...
		  print'ddddd'
		   ...
		   and Emp_ID = isnull(@Emp_ID ,Emp_ID)  
			  ...
			  and 
					...
					(@St_Date <= isnull(left_date,@St_Date) )
					-- and @end_date >= isnull(left_date,@end_date) ) --Commented by Hardik 23/06/2017 for BMA, If employee left on 23/05/2017 and IT Declaration checked for 2016-2017 Year then employee is not showing, it will only show in 2017-2018
				...
				BEGIN
				 ...
				 BEGIN
					...
					FROM	(
								...
								WHERE	E.Cmp_ID = @Cmp_Id AND EMP_LEFT = 'Y' and Emp_left_Date >= @From_Date  -- (Emp_left_Date >= @To_Date and Emp_left_Date <= @To_Date)
					...
					BEGIN
						...
						FROM	(
								...
								WHERE	E.Cmp_ID = @Cmp_Id AND EMP_LEFT = 'Y' and Emp_left_Date > DATEADD(d,1,@To_Date)  -- (Emp_left_Date >= @To_Date and Emp_left_Date <= @To_Date)
									--(CASE WHEN @ShowLeftEmp = 1 THEN 1 WHEN @ShowLeftEmp=0 AND Emp_left_Date > @To_Date OR Emp_Left_Date IS NULL THEN 1 ELSE 0 END) = 1				
							) T
					ORDER BY EMP_FULL_NAME 
					END
						...
						BEGIN
							...
							ELSE
								...
								Select * INTO	#RecordSet from (
									...
									WHERE	E.Cmp_ID = @Cmp_Id and Emp_Left<>'Y'  --Ronakb170824 Bug #29931 only showing current emp  
									UNION 
									Select I_Q.* ,E.Emp_Code, E.Alpha_Code,
											CAST('' AS VARCHAR(256)) Emp_Full_Name
											,Lo.Login_ID,E.Emp_Full_Name as Emp_Full_Name_only,Emp_superior      
									...
									FROM	T0080_EMP_MASTER E WITH (NOLOCK)
											...
											LEFT OUTER JOIN T0040_DEPARTMENT_MASTER DM WITH (NOLOCK) ON I_Q.Dept_Id = DM.Dept_Id 
											LEFT OUTER JOIN T0030_CATEGORY_MASTER CGM WITH (NOLOCK) ON I_Q.Cat_ID = CGM.Cat_ID
									WHERE	E.Cmp_ID = @Cmp_Id and Emp_Left = 'Y' and Emp_left_Date >= @From_Date  --and Emp_left_Date > DATEADD(d,1,@To_Date) 
								) e

								ORDER BY	(Case @Emp_Search When 3 Then e.Emp_First_Name When 4 Then e.Emp_First_Name Else e.Alpha_Code End),
								e.Emp_code
