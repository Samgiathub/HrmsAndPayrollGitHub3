...
CREATE PROCEDURE [dbo].[SP_EMP_INOUT_SYNCHRONIZATION_KnowCraft]
    @EMP_ID         NUMERIC ,    
    @CMP_ID         NUMERIC ,    
...
SET ANSI_WARNINGS OFF
    ...
    AND IsNull(Reason,'') = '' AND IsNull(Chk_By_Superior,0) = 0
    AND IsNull(ManualEntryFlag,'N') ='N'  ---- Add by jignesh 19-Jul-2018----
                    ...
                    BEGIN
                        ...
                        IF @IO_DATETIME BETWEEN DATEADD(HOUR,-2,@F_Shift_In_Time) AND DATEADD(HOUR,2,@F_Shift_In_Time) --Added by hardik 21/05/2015 as when shift IS changed for night then it will not consider new Punch as IN
                            BEGIN
								...
								DECLARE @LAST_PUNCH DATETIME
						
								SELECT  @LAST_PUNCH=MAX(isNull(Out_Time,In_time))
                                FROM    T0150_EMP_INOUT_RECORD WITH (NOLOCK)
                                WHERE   emp_ID=@emp_ID  And For_Date <= @IO_DATETIME AND Cmp_ID=@CMP_ID
                                ...
                                IF @In_Time IS NOT NULL  
                                    BEGIN
										...
										--set @varFor_Date_P = CAST(@For_Date as varchar(11)) 
										SELECT	@varFor_Date_P = CAST(For_Date as varchar(11)) 
										FROM	T0150_EMP_INOUT_RECORD WITH (NOLOCK)
                        ...
                        */
                        ...
                        IF @In_Time IS NULL  
                            BEGIN  
                                ...
                                INSERT INTO T0150_EMP_INOUT_RECORD    
                                    (IO_Tran_Id, Emp_ID, Cmp_ID, For_Date, In_Time, Out_Time, Duration, Reason, Ip_Address, In_Date_Time, Out_Date_Time, Skip_Count, Late_Calc_Not_App)    
                                VALUES     
                                    (@IO_Tran_Id,@Emp_ID,@Cmp_ID,@For_Date,@IO_DATETIME,null,'','',@Ip_Address,null,null, 0, 0)         
                        ...
                        ELSE  
                            BEGIN 
								...
								DECLARE @TEMP_SHIFT_ST_TIME DATETIME
								SET @TEMP_SHIFT_ST_TIME = DATEADD(HH, -2, convert(datetime,convert(varchar(10), @IO_DATETIME, 103), 103) + @Shift_St_Time1)
								

                             
								--IF @TEMP_SHIFT_ST_TIME < @IO_DATETIME 
								--	SET @Diff_sec_Temp = 0
        --                        ELSE 
            ...
            BEGIN       
                ...
                IF @In_Out_flag = 2 
                    BEGIN                                                       
                        ...
                        VALUES     
                            (@IO_Tran_Id,@Emp_ID,@Cmp_ID,@For_Date,null,@IO_DATETIME,'','',@Ip_Address,null,null, 0, 0,@In_Out_flag,1)          
                    END         
                IF @In_Out_flag = 3
                    BEGIN
                        SELECT  @IO_Tran_ID = IsNull(MAX(IO_Tran_ID),0)+ 1 
