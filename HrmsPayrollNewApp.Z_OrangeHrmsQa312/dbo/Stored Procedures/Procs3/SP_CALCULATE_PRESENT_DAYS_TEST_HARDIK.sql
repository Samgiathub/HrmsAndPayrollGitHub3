...
CREATE PROCEDURE [dbo].[SP_CALCULATE_PRESENT_DAYS_TEST_HARDIK]        
  @Cmp_ID    numeric        
 ,@From_Date   datetime        
 ,@To_Date    datetime         
 ,@Branch_ID   numeric        
...
--if @Return_Record_set = 1 or @Return_Record_set = 2 or @Return_Record_set =3  or @Return_Record_set = 5 or @Return_Record_set = 7        
-- Begin        
...
Delete @Emp_WeekOFf_Detail Where Emp_ID Not In (Select Emp_ID From #Data Group By Emp_ID) --Hardik 07/09/2012
	...
	Fetch next from curautobranch into @curEmp_ID
		...
		Begin     
	----- Added for Weekoff from below code to here Hardik 28/02/2012
			...
			select @Week_oF_Branch=Branch_ID from dbo.T0095_Increment EI where Increment_Effective_Date in (select max(Increment_effective_Date) as Increment_effective_Date from dbo.T0095_Increment  where Increment_Effective_date <= @From_Date  and Cmp_ID = @Cmp_ID and Emp_ID = @curEmp_ID) and Emp_ID = @curEmp_ID
			
		 
			Select @Is_Cancel_weekoff = Is_Cancel_weekoff ,@tras_week_ot=isnull(tras_week_ot,0)  ,@Auto_OT = Is_OT_Auto_Calc ,@OT_Present = OT_Present_days,@Is_Negative_Ot = ISNULL(Is_Negative_Ot,0)
				...
				and For_Date = ( select max(For_Date) from dbo.T0040_GENERAL_SETTING where For_Date <=@To_Date and Branch_ID = @Week_oF_Branch and Cmp_ID = @Cmp_ID)    
					...
					Exec SP_EMP_HOLIDAY_DATE_GET @Emp_Week_Detail,@Cmp_ID,@From_Date,@To_Date,null,null,@Is_Cancel_Holiday,@StrHoliday_Date output,@Holiday_days output,@Cancel_Holiday output,0,@Branch_ID,@StrWeekoff_Date
			...
			--		select h_from_date,h_to_date  from dbo.t0040_holiday_master 
			--		where cmp_Id = @Cmp_ID
			...
			--		Update 	@Emp_WeekOFf_Detail set Strweekoff=@StrWeekoff_Date where Emp_ID=@curEmp_ID
			...
			--	Begin 
			--		Insert into #Data_Weekoff values(@curEmp_ID,@Weekoff_Days)
					
			--	End

----- End for Weekoff from Hardik 28/02/2012
         
			Declare @First_In_Last_Out_For_InOut_Calculation tinyint 
			
			declare @cBrh as numeric
			Set @cBrh = @Week_oF_Branch -- Hardik 29/02/2012 and below line is commented
			--select @cBrh  = Branch_ID from dbo.T0095_Increment EI where Increment_Effective_Date in (select max(Increment_effective_Date) as Increment_effective_Date from dbo.T0095_Increment  where Increment_Effective_date <= @From_Date  and Cmp_ID = @Cmp_ID and Emp_ID = @curEmp_ID) and Emp_ID = @curEmp_ID
			select @First_In_Last_Out_For_InOut_Calculation  = First_In_Last_Out_For_InOut_Calculation from dbo.T0040_GENERAL_SETTING where Branch_ID = @cBrh  and For_Date in (select MAX(For_Date) as for_date from dbo.T0040_GENERAL_SETTING where For_Date <= @From_Date and Cmp_ID = Cmp_ID and Branch_ID = @cBrh) and Cmp_ID = @Cmp_ID			
			...
			if @First_In_Last_Out_For_InOut_Calculation = 1
				Begin				
					...
					----- changed to get record with only Min(InTime) and Max(OutTime) ------
						  ...
						  Insert into dbo.#Data (Emp_ID,For_Date,Duration_In_sec,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,Shift_Start_Time,OT_Start_Time,Shift_Change,Chk_By_Superior,IO_Tran_Id)        
					              
					   select distinct eir.Emp_ID ,EIR.for_Date,isnull(datediff(s,In_Date,Case when Max_In_Date > Out_Date Then Max_In_Date Else Out_Date End),0),isnull(Emp_OT,0),dbo.F_Return_Sec(Emp_OT_min_Limit),dbo.F_Return_Sec(Emp_OT_max_Limit),In_Date,null,0,0,isnull(Q3.Chk_By_Sup,0),isnull(EIR.is_cmp_purpose ,0)
					   ...
					   group by eir.Emp_ID,eir.For_Date,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,In_Date,out_Date,Chk_By_Sup ,EIR.is_cmp_purpose,Max_In_Date   
					   order by For_Date
					 ------------------end--------------------
			...
			Else
				Begin
					...
					Insert into dbo.#Data (Emp_ID,For_Date,Duration_In_sec,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,Shift_Start_Time,OT_Start_Time,Shift_Change,Chk_By_Superior,IO_Tran_Id)        
					  ...
					  select eir.Emp_ID ,EIR.for_Date,sum(isnull(datediff(s,in_time,out_time),0)) ,isnull(Emp_OT,0),dbo.F_Return_Sec(Emp_OT_min_Limit),dbo.F_Return_Sec(Emp_OT_max_Limit),In_Time,null,0,0,Chk_By_Superior,isnull(EIR.is_cmp_purpose ,0)
					   from T0150_emp_inout_Record  EIR Inner join @Emp_Cons Ec on EIR.Emp_Id = ec.Emp_ID inner Join			
						(select I.Emp_ID,Emp_OT,isnull(Emp_OT_min_Limit,'00:00')Emp_OT_min_Limit,isnull(Emp_OT_max_Limit,'00:00')Emp_OT_max_Limit from T0095_Increment  I inner join         
						(select max(increment_effective_Date)IE_Date ,Emp_ID from T0095_Increment         
						 where increment_effective_Date <=@To_Date and Cmp_ID =@Cmp_ID group by Emp_ID)q on I.emp_ID =q.Emp_ID and         
					   ...
					   Where cmp_Id= @Cmp_ID        
					   and EIR.for_Date >=@From_Date and EIR.For_Date <=@To_Date and ec.Emp_ID = @curEmp_ID
					   group by eir.Emp_ID,eir.For_Date,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,Chk_By_Superior ,EIR.is_cmp_purpose 
					   order by For_Date
				End
    ...
    Insert Into @Data_temp1 (Emp_ID,For_Date,Duration_In_sec,Emp_OT,Emp_OT_min_Limit,Emp_OT_max_Limit,In_Time,Shift_Start_Time,OT_Start_Time,Shift_Change,Chk_By_Superior,IO_Tran_Id)
   Select Emp_ID,for_Date,sum(isnull(Duration_in_sec,0)),isnull(Emp_OT,0),isnull(Emp_OT_min_Limit,0),isnull(Emp_OT_max_Limit,0),null,null,0,0,Chk_By_Superior,IO_Tran_Id             
    From dbo.#Data Group By For_Date,Emp_ID,Emp_Ot,Emp_OT_min_Limit,Emp_OT_Max_Limit,Chk_By_Superior,IO_Tran_Id

	   Update @Data_temp1 set In_Time=InTime
	   from  @Data_temp1 as DT	
	   inner join
	   (select Min(In_Time) as InTime,For_Date,Emp_ID from dbo.#Data Group by For_Date,Emp_ID)Q
    ...
    begin  
		   ...
		   Shift_Type = q1.Shift_type 
			 from dbo.#Data d inner Join        
			 (select q.Shift_ID ,q.Emp_ID,shift_type,q.For_Date from dbo.T0100_Emp_Shift_Detail sd inner join        
			 (select for_Date ,Emp_Id,Shift_ID   from dbo.T0100_Emp_Shift_Detail    as esdsub       
			 where Cmp_Id =@Cmp_ID and shift_Type = 0 and for_Date = (select max(for_Date) from dbo.T0100_Emp_Shift_Detail where emp_id = esdsub.emp_id and Cmp_Id =@Cmp_ID and shift_Type = 0 and For_Date <= @Tmp_Date ) )q on sd.Emp_ID =q.Emp_ID and sd.For_Date =q.For_Date)q1  on d.emp_ID = q1.emp_ID         
		  ...
		  from dbo.#Data d inner Join        
			(select q.Shift_ID ,q.Emp_ID,shift_type,q.For_Date from dbo.T0100_Emp_Shift_Detail   sd inner join        
			(select for_Date ,Emp_Id,Shift_ID   from dbo.T0100_Emp_Shift_Detail          as esdsub   
			 where Cmp_Id =@Cmp_ID and shift_Type = 1 and for_Date = (select max(for_Date) from dbo.T0100_Emp_Shift_Detail where  emp_id = esdsub.emp_id and Cmp_Id =@Cmp_ID and  shift_Type = 1 and For_Date <= @Tmp_Date ) )q on sd.Emp_ID =q.Emp_ID and sd.For_Date =q.For_Date)q1  on d.emp_ID = q1.emp_ID         
		   Where D.For_Date = @tmp_Date 

	---Commented by hardik 29/02/2012, put this portion below While loop
 --  Update dbo.#Data
 --   set Shift_Start_Time = q.Shift_St_Time,
 --   OT_Start_Time=isnull(q.OT_Start_Time,0) 
 --   from dbo.#data d inner join 
 --   (select ST.Shift_st_time,ST.Shift_ID,isnull(SD.OT_Start_Time,0) as OT_Start_Time from dbo.t0040_shift_master ST left outer join t0050_shift_detail SD 
 --   on ST.Shift_ID=SD.Shift_ID ) q on d.shift_id=q.shift_id
    
	--Update dbo.#Data set Shift_Start_Time= cast(CONVERT(VARCHAR(11), In_time, 121)  + CONVERT(VARCHAR(12), Shift_Start_Time, 114) as datetime)  from #Data
    set @Tmp_Date = dateadd(d,1,@tmp_date)            
 end  
 
	---Added by hardik 29/02/2012, put from above while loop
   Update dbo.#Data
    set Shift_Start_Time = q.Shift_St_Time,
    OT_Start_Time=isnull(q.OT_Start_Time,0) 
    from dbo.#data d inner join 
	...
	Update dbo.#Data set Shift_Start_Time= cast(CONVERT(VARCHAR(11), In_time, 121)  + CONVERT(VARCHAR(12), Shift_Start_Time, 114) as datetime)  from #Data
	---- End for hardik added part on 29/02/2012
 
 Update dbo.#Data set Shift_Change=1 where isnull(datediff(s,in_time,Shift_Start_Time),0)  < -14400  
 Update dbo.#Data set Shift_Change=1 where isnull(datediff(s,in_time,Shift_Start_Time),0)  > 14400  
...
Declare @New_Shift_ID numeric
 Declare curautoshift cursor for	                  
  select Emp_ID,In_Time,Shift_ID from dbo.#Data where isnull(Shift_Change,0)=1 And isnull(Emp_OT,0)=1 order by In_time,Emp_ID
	Open curautoshift                      
	  Fetch next from curautoshift into @Emp_ID_AutoShift,@In_Time_Autoshift,@New_Shift_ID
       ...
       and datediff(s,@In_Time_Autoshift,cast(CONVERT(VARCHAR(11), @In_Time_Autoshift, 121)  + CONVERT(VARCHAR(12), Shift_St_Time, 114) as datetime)) >-14400
        if isnull(@Shift_ID_Autoshift,0) > 0 And isnull(@Shift_ID_Autoshift,0)<>isnull(@New_Shift_ID,0)
			...
			if isnull(@Shift_ID_Autoshift,0) > 0 And isnull(@Shift_ID_Autoshift,0)<>isnull(@New_Shift_ID,0)
					Begin 
						Update dbo.#Data set Shift_ID=@Shift_ID_Autoshift,Shift_Start_Time= cast(CONVERT(VARCHAR(11), In_time, 121)  + CONVERT(VARCHAR(12), @Shift_start_time_Autoshift, 114) as datetime)  from #Data
				        where Emp_ID=@Emp_ID_AutoShift and In_time=@In_Time_Autoshift And Shift_ID <> @Shift_ID_Autoshift
					End
			End
    fetch next from curautoshift into @Emp_ID_AutoShift,@In_Time_Autoshift,@New_Shift_ID
 ...
 --   from dbo.#data d inner join 
 --   (select ST.Shift_st_time,ST.Shift_ID,isnull(SD.OT_Start_Time,0) as OT_Start_Time from dbo.t0040_shift_master ST left outer join dbo.t0050_shift_detail SD 
 --   on ST.Shift_ID=SD.Shift_ID ) q on d.shift_id=q.shift_id where isnull(d.shift_Change,0)=1
 
 
 update dbo.#Data set Duration_in_sec = Duration_in_sec - datediff(s,In_time,Shift_Start_Time)
 where datediff(s,In_time,Shift_Start_Time) > 0  And isnull(Emp_OT,0)=1 And isnull(OT_Start_Time,0)=1
 
 --Commented by Hardik 28/02/2012
  --Update dbo.#Data        
  --set Shift_ID   = Q1.Shift_ID,          
  -- Shift_Type = q1.Shift_type        
...
--Commented by Hardik on 28/02/2012 for Weekoff as it put above  
...
-- deallocate curEmp_weekoff_Detail   
  ...
  ---Hardik 07/09/2012 for Weekoff1 Cursor
   declare curweekoff1 cursor for                    
 ...
 open curweekoff1                      
  ...
  while @@fetch_status = 0                    
   begin       
			...
			Set @Weekoff_Date1 =''
			Select @Weekoff_Date1 = StrWeekoff_Holiday from @Emp_WeekOFf_Detail where Emp_ID = @Emp_Id_Temp1
		 ...
		 open curweekoff                      
		  ...
		  while @@fetch_status = 0                    
		   ...
		   select @Week_oF_Branch=Branch_ID  from t0095_increment where Increment_id in (select Max(Increment_id) from t0095_increment where emp_id=@Emp_Id_Temp1)
 ...
 Declare @Ot_Sec_Neg Numeric(18,0)--Nikunj
	 ...
	 Declare Cur_shift cursor for         
		  ...
		  dbo.T0040_shift_master sm on sd.shift_ID= sm.Shift_ID inner join         
		   (select distinct Shift_ID from dbo.#Data ) q on sm.shift_Id=  q.shift_ID        
 ...
 Deallocate Cur_Shift         		
				...
				--Added by Hardik 28/02/2012 for Half day from below side to this cursor
 ...
 ---Hardik 07/09/2012 for Weekoff1 Cursor
	...
	begin  
			...
			exec GET_HalfDay_Date @Cmp_ID,@Emp_Id_Temp1,@From_Date,@To_Date,0,@HalfDayDate output
			...
			declare cur_shift_half_day Cursor for
					...
					Where For_Date In (Select Data from dbo.Split(@HalfDayDate,';') where DATA<>'') --Hardik 07/09/2012 Where Condition
...
---Commented by Hardik 28/02/2012 for Put this to above cursor
...
--------------------Added by Mitesh on 15/09/2011 for Shift Half Day ----------------------------
  ...
  --  from dbo.#Data  d inner join dbo.T0160_OT_Approval OA on d.emp_ID = Oa.Emp_ID and d.For_Date = oa.For_Date         
...
--		Select Emp_Id,For_Date from dbo.#Data Where OT_Sec > 0
--	open Cur_HO
...
--			Exec dbo.SP_EMP_WEEKOFF_DATE_GET @Emp_ID_Cur,@Cmp_ID,@From_Date,@To_Date,@Join_Date,@left_Date,@Is_Cancel_Weekoff_OT,@StrHoliday_Date,@StrWeekoff_Date_OT output,@Weekoff_Days_OT output ,@Cancel_Weekoff_OT output   
--			Exec dbo.SP_EMP_HOLIDAY_DATE_GET @Emp_ID_Cur,@Cmp_ID,@From_Date,@To_Date,@Join_Date,@left_Date,@Is_Cancel_Holiday,@StrHoliday_Date output,@Holiday_days output,@Cancel_Holiday output,0,@Branch_ID,@StrWeekoff_Date_OT
...
--					Update dbo.#Data Set  OT_Sec = 0, Weekoff_OT_Sec = OT_Sec Where For_date = @For_Date And Emp_Id = @Emp_Id_Cur
--				end 
...
--				    Update dbo.#Data Set OT_Sec = 0, Holiday_OT_Sec = OT_Sec Where For_date = @For_Date And Emp_Id = @Emp_Id_Cur					
--				end
...
--  if @Return_Record_set =2   or @Return_Record_set =5
--   begin       
...
--        delete from dbo.#Data_Temp  
--        DECLARE OT_cursor CURSOR  
--        FOR  
--           SELECT Emp_ID,For_Date FROM dbo.#Data   
--         OPEN OT_cursor  
--          fetch next from OT_cursor into @T_Emp_ID,@T_For_Date  
--          while @@fetch_status = 0  
--         BEGIN  
            
--          if Not Exists(select Tran_Id from t0160_Ot_Approval where Emp_ID=@T_Emp_ID And For_Date=@T_For_Date)  
--           Begin  
...
--        Set @P_Days_Count = (Select SUM(Isnull(P_days,0)) from dbo.#data where Emp_ID=@T_Emp_ID And Month(For_Date)=Month(@T_For_Date) and IO_Tran_Id  = 0  )
...
--        DECLARE OT_cursor CURSOR  
--        FOR  
--           SELECT Emp_ID,For_Date,Flag FROM dbo.#Data   
--         OPEN OT_cursor  
--          fetch next from OT_cursor into @T_Emp_ID_3,@T_For_Date_3,@Flag_cur   
--          while @@fetch_status = 0  
--         BEGIN  
            
--				If Not Exists(Select Tran_Id from t0160_Ot_Approval where Emp_ID=@T_Emp_ID_3 And For_Date=@T_For_Date_3)  
--					Begin  	           
--							insert into #Data_Temp_3   
--							select * from dbo.#Data where Emp_ID=@T_Emp_ID_3 And For_Date=@T_For_Date_3  						
--						If @Flag_cur=1
--							Begin
...
--       Select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name,dbo.F_Return_Hours(Sum(Isnull(Duration_in_Sec,0))) As Working_Hour ,CONVERT(decimal(10,2),(sum(Isnull(OT_SEc,0))))  as OT_Hour ,CONVERT(decimal(10,2),(sum(Isnull(Weekoff_OT_Sec,0))))  as Weekoff_OT_Sec,CONVERT(decimal(10,2),(sum(Isnull(Holiday_OT_Sec,0)))) as Holiday_OT_Sec,sum(Isnull(P_days,0)) as Present_Days
...
--   end  
 ...
 --         insert into #Data_Temp_5   
	--				select  * from dbo.#Data where Emp_ID=@T_Emp_ID_5 And For_Date=@T_For_Date_5  
	...
	--		         From dbo.#Data where Emp_ID=@T_Emp_ID_5 And For_Date=@T_For_Date_5
 ...
 --               ) 
	...
	--						select OA.Emp_ID,Max(For_Date)For_Date,E.Emp_Full_Name, CONVERT(decimal(10,2), sum(Isnull(Duration_in_Sec,0))/3600)   as Working_Hour ,CONVERT(decimal(10,2),(sum(Isnull(OT_SEc,0))))  as OT_Hour ,sum(Isnull(P_days,0)) as Present_Days , CONVERT(decimal(10,2),(sum(Isnull(Weekoff_OT_Sec,0))))  as Weekoff_OT_Hour,CONVERT(decimal(10,2),(sum(Isnull(Holiday_OT_Sec,0))))  as Holiday_OT_Hour       
 ...
 -- 							Select OA1.Emp_ID,P_days,dbo.F_Return_Hours(OT_HOur),0,0,0,0,0,0,0,dbo.F_Return_Hours(Weekoff_OT_Hour),dbo.F_Return_Hours(Holiday_OT_Hour)
	--						From @Emp_Temp_5  OA1 --inner join dbo.T0080_emp_master E1 on OA1.Emp_ID = E1.Emp_ID          -- Commented by Hardik 28/02/2012
	--						Group by OA1.emp_ID,For_Date,  Working_Hour,OT_Hour,P_days ,Weekoff_OT_Hour,Holiday_OT_Hour--,E1.Emp_Full_Name
