CREATE PROCEDURE [dbo].[SP_CAL_RETAIN_SALARY_Deep]
 ...
 --    (
  ...
  if exists (select 1 from  T0060_EFFECT_AD_MASTER WITH (NOLOCK) where effect_AD_ID=@Ad_Id)
    begin
		...
		-----------------------------------------------------------------
		--update #Temp_Emp 
		--set Emp_Ret_Count = RET.Ret_Cnt		
		--from #Temp_Emp TEMP inner join 
		--				(	select count (Emp_id) as Ret_Cnt , Emp_id  from T0100_EMP_RETAINTION_STATUS 
		--					where  Cmp_Id = @Cmp_ID and Start_Date between  @MnLock_St_Date and @MnLock_end_Date  
		--					and  End_Date between @MnLock_St_Date and @MnLock_end_Date
		--					group by Emp_id   ,Cmp_Id
		--				)
		--			RET on TEMP.Emp_ID = RET.Emp_ID	 and tran_Id =@Tran_ID		
	...
	if(@Ret_Count= 1)
		begin
			...
			where M.RRate_Id = D.RRate_ID and M.ad_id= @ad_id and M.Grd_ID=@Grd_ID  order by From_Limit
			...
			--Retaining calculation start
							...
							Re.Start_Date as 'Ret_Start_Date', Re.End_Date  as 'Ret_End_Date'   into #Temp_Emp_INC_RET_Dates 
							...
							set Inc_To_Date =(Select increment_effective_date -1 from #Temp_Emp_INC_RET_Dates as it , t0095_increment I where   I.Emp_ID  =@emp_Id  and I.Cmp_ID =@Cmp_ID
				...
				set @Month_day = datediff(d,@Month_St_Date,@Month_End_Date) +1
					...
					if @Retain_Days >0 
						...
						--	select I.Increment_ID, I.Branch_ID,I.Grd_ID,I.Basic_Salary ,I.Emp_Id , I.increment_effective_date  as 'Inc_From_Date'  ,I.increment_effective_date  as 'Inc_To_Date ',  
						--	Re.Start_Date as 'Ret_Start_Date', Re.End_Date  as 'Ret_End_Date'   into #Temp_Emp_INC_RET_Dates 
						--	from  T0100_EMP_RETAINTION_STATUS Re inner join  t0095_increment I on  I.Emp_ID  =Re.Emp_ID  where Re.Emp_ID =@emp_Id 

							 select top 1 @New_BasicAmt  = Basic_Salary  from #Temp_Emp_INC_RET_Dates I where  Emp_ID =@emp_Id  and  I.Inc_From_Date <@For_Date  order by Inc_From_Date desc
								--print @New_BasicAmt
								--print @For_Date
		...
		print @Ret_Count
				...
				select  @Temp_Emp_Ret_Count =ROW_NUMBER () Over(order by t.Start_Date) 				
					from 		Temp_Emp_Retain t , T0090_Retaining_Lock_Setting MON where t.Emp_id = @emp_Id					
							...
							Group by t.Emp_id , t.Start_Date ,t.Tran_Id, t.End_Date, MON.Tran_Id
	...
	Begin   	
		...
		update T0210_Retaining_Datewise_Payment 
						  ...
						  --Retain_Amount = (((select (Basic_Salary + Other_Amount) as Ad_Amount from #Temp_Emp where Emp_ID= @emp_Id and tran_id = @Tran_ID)/Month_Day) * 1) *Slab_Per/100
						    ...
						    Per_Day_Salary = (Basic_Amount+ (select Other_Amount  from #Temp_Emp where Emp_ID= @emp_Id and tran_id = @Tran_ID)) /Month_Day,
						  Retain_Amount = (((Basic_Amount + (select Other_Amount  from #Temp_Emp where Emp_ID= @emp_Id and tran_id = @Tran_ID))/Month_Day) * 1) *Slab_Per/100
