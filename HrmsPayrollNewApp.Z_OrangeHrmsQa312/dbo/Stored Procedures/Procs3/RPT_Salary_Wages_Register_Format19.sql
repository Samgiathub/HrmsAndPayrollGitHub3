...
CREATE PROCEDURE [dbo].[RPT_Salary_Wages_Register_Format19]
 @Cmp_ID		numeric
,@From_Date		datetime
,@To_Date		datetime 
,@Branch_ID		numeric   = 0
...
SET ARITHABORT ON
	...
	EXEC Set_Salary_Wages_register_Lable_With_Late_Format19 @Cmp_ID,@month,@Year,@From_Date,@To_Date
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
					...
					select @Sal = I.Basic_Salary from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							( select max(Increment_ID) as Increment_ID from dbo.T0095_Increment I1	WITH (NOLOCK) -- Ankit 05092014 for Same Date Increment
							...
							and Cmp_ID = @Cmp_ID And Emp_Id = @Emp_ID
											GROUP BY emp_ID) Qry1 On I1.Emp_ID = Qry1.Emp_ID And I1.Increment_Effective_Date = Qry1.Increment_Effective_Date
							where I1.Increment_Effective_date <= @To_Date
							and Cmp_ID = @Cmp_ID And I1.Emp_Id = @Emp_ID
							group by I1.emp_ID  ) Qry on
					...
					--select * from #TEMP_REPORT_LABEL
							...
							FROM	dbo.#Temp_Salary_Muster_Report T 
									INNER JOIN (
												SELECT	DISTINCT ISNULL(SUM(M_AD_Amount),0) as Amount
				...
				--Select * From #Temp_Salary_Muster_Report Where emp_Id=1 And MONTH=2 And Year=2011
					...
					declare Cur_Dedu   cursor for
						...
						Union  --Hardik 07/06/2018 for AIA
						select distinct  t0050_ad_master.AD_ID,Ad_Sort_Name ,0,t0050_ad_master.AD_Flag,
						 Case 
						     when   t0050_ad_master.AD_PERCENTAGE > 0 then t0050_ad_master.AD_PERCENTAGE
						     Else   MAD.M_AD_Actual_Per_Amount
						   End 
						   ,0
						From t0210_monthly_ad_detail MAD WITH (NOLOCK) inner join
										T0201_MONTHLY_SALARY_SETT MSS WITH (NOLOCK) on MAD.s_Sal_Tran_ID=MSS.s_Sal_Tran_ID inner join 
										T0050_AD_MASTER WITH (NOLOCK) on MAD.Ad_Id = T0050_AD_MASTER.Ad_ID and MAD.Cmp_ID = T0050_AD_MASTER.Cmp_Id 
										inner  join #Temp_Salary_Muster_Report On MAD.Emp_Id=#Temp_Salary_Muster_Report.Emp_Id
						...
						begin
							...
							If @With_Arear_Amount = 1
								Begin
									...
									Select  @S_Total_Deduction_1 = Isnull(SUM(M_AD_Amount),0) +  Isnull(SUM(M_AREAR_AMOUNT),0)
									From t0210_monthly_ad_detail MAD WITH (NOLOCK) inner join
									--T0201_MONTHLY_SALARY_SETT MSS on MAD.Sal_Tran_ID=MSS.Sal_Tran_ID inner join
										T0201_MONTHLY_SALARY_SETT MSS WITH (NOLOCK) on MAD.S_Sal_Tran_ID=MSS.S_Sal_Tran_ID inner join      ---changed BY Jimit 05022018 as there is case at AIA PF Amount added twice.
					...
					Set @Arear_Net = (@Arear_Basic + @Arear_Earn_Amount) --change by nilesh patel on 05062017
		...
		--				VALUES     (@Emp_ID, @Company_ID, @Transaction_ID, @Month, @Year, 'Other Dedu', @Other_Dedu,'',@Row_ID)
						...
						FROM	(
									...
									FROM(		
											...
											WHERE   CMP_ID = @CMP_ID AND MONTH(LOAN_PAYMENT_DATE) = @MONTH AND YEAR(LOAN_PAYMENT_DATE) = @YEAR 
													AND LOAN_SHORT_NAME <> ''		
													AND	EMP_ID = @EMP_ID 
													AND not Sal_Tran_ID is null --Added Condition by Hardik 05/06/2017 for AIA, Manual Loan Payment adding in Totals
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = Isnull(Amount,0) + @Advance, 
											  Value_String = '',M_AD_Flage='D'
						WHERE     (Label_Name LIKE '%Advance%') AND (Row_id = @Row_ID)
								and Emp_ID = @Emp_ID
						...
						UPDATE    #Temp_Salary_Muster_Report
						SET       Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, Amount = Mad.M_AD_Amount, 
											  Value_String = '',M_AD_Flage='D'
						From	   dbo.#Temp_Salary_Muster_Report T Inner join								 
								   T0210_MONTHLY_AD_DETAIL Mad on Mad.Emp_Id = T.Emp_Id and month = month(For_Date) and year(for_date) = @year Inner join
								   T0050_AD_MASTER Am On Am.Ad_Id = Mad.AD_ID and Mad.Cmp_Id = AM.CMP_ID and Am.AD_SORT_NAME = T.Label_Name		
						WHERE     (Label_Name = 'TDS') AND (Row_id = @Row_ID) and AD_DEF_ID = 1 and month(For_Date) = @month and year(for_date) = @year
								and T.Emp_ID = @Emp_ID
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month, Year = @Year, 
											  Amount = @Late_Deduction + ISNULL(@S_Total_Deduction,0), Value_String = '',M_AD_Flage='D'
	...
	select dbo.#Temp_Salary_Muster_Report.* ,ISNULL(E.EmpName_Alias_Salary,E.Emp_Full_Name) as Emp_Full_Name,E.Emp_Code,E.Alpha_Emp_Code,GM.Grd_name,ETM.Type_name,DGM.Desig_Name,DM.Dept_Name,BM.Branch_Name, 
		ISnull(Inc_Qry.Dept_ID,0) as Dept_ID,Cmp_Name,Cmp_Address,isnull(Inc_Qry.Inc_Bank_Ac_no,0)Inc_Bank_Ac_no,isnull(Inc_Qry1.Payment_Mode,'') as Payment_Mode,
		...
		,ISNULL(Inc_Qry1.Master_Gross_Salary,0) As Master_Gross_Salary	--Ankit 10032014
		...
		,DM.Dept_Code
		,CCM.Center_Name , isnull(CCM.CENTER_ID,0) AS CENTER_ID , CCM.Center_Code  --added by jimit 05042017
	...
	from dbo.#Temp_Salary_Muster_Report Inner join 
		...
		( select I.Emp_Id ,Grd_ID,DEsig_ID ,Dept_ID,Inc_Bank_Ac_no,Branch_ID,Type_ID,I.Vertical_ID,I.SubVertical_ID,I.Center_ID,
		--Payment_Mode 
		case when @Payment_Mode = '' then 'ALL' else Payment_Mode end as Payment_Mode,Gross_Salary As Master_Gross_Salary
		from t0095_Increment I WITH (NOLOCK) inner join 
					( 
					select max(Increment_ID) as Increment_ID, I1.Emp_ID from t0095_Increment I1 WITH (NOLOCK)	-- Ankit 05092014 for Same Date Increment
						Inner JOIN (Select I2.Emp_ID,Max(I2.Increment_Effective_Date) Increment_Effective_Date 
							From T0095_INCREMENT I2 WITH (NOLOCK) 
							Where Increment_Effective_date <= @To_Date
			...
			)Inc_Qry1 on  E.Emp_ID = Inc_Qry1.Emp_ID inner join 
					( select I.Emp_Id ,Grd_ID,DEsig_ID ,Dept_ID,Inc_Bank_Ac_no,Branch_ID,Type_ID,I.Vertical_ID,I.SubVertical_ID,I.Center_ID					
					from t0095_Increment I WITH (NOLOCK) inner join 
						( 
						select max(Increment_ID) as Increment_ID, I1.Emp_ID from t0095_Increment I1 WITH (NOLOCK)	-- Ankit 05092014 for Same Date Increment
		...
		and (@Payment_Mode ='' or isnull(Inc_Qry1.Payment_Mode,'')=@Payment_Mode)
		-----End---------- 
...
*/-----End---------- 
	...
	Declare @Branch_Id1 as varchar(max)
	if @Branch_ID is null
		Begin	
			select  @Branch_Id1 = COALESCE(@Branch_ID1 + '#', '') + cast(Branch_ID as nvarchar(5))  from T0030_BRANCH_MASTER WITH (NOLOCK) where Cmp_ID=@Cmp_ID 
			set @Branch_Id1 = @Branch_Id1 + '#0'
	...
	SELECT	G.Branch_ID, G.Sal_st_Date, DATEADD(m, 1, G.Sal_st_Date) As Sal_End_Date,G.ESIC_EMPLOYER_CONTRIBUTION
			...
			FROM	(
						...
						FROM	T0040_GENERAL_SETTING G1 WITH (NOLOCK)
								inner JOIN (Select Cast(data as numeric) as Branch_ID FROM dbo.Split(@Branch_Id1,'#')) T ON T.Branch_ID=G1.Branch_ID
						WHERE	For_Date = (
	...
	If @group_Name = 0
		BEGIN
				...
				(
					select Count(Ec.Emp_Id) as PF_Count,grd_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
								 #TEMP_SALARY_MUSTER_REPORT1 T1 On T1.emp_Id = Ec.emp_Id and UPPER(LABEL_NAME) = UPPER(@ad_Name_Pf)
							where ad_def_Id = @Pf_Def_Id and Upper(@ad_Name_Pf) = UPPER(AD_SORT_NAME)							
							group By 	grd_Id
				)Q On Q.grd_Id = T.grd_Id
				Left Outer join
				(
					select Count(Ec.Emp_Id) as ESIC_Count,grd_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK)On Am.AD_ID = mad.AD_ID	left Join	
								 #TEMP_SALARY_MUSTER_REPORT1 T1 On T1.emp_Id = Ec.emp_Id and UPPER(LABEL_NAME) = UPPER(@ad_Name_Esic)
								where ad_def_Id = @Esic_Def_Id and Upper(@ad_Name_Esic) = UPPER(AD_SORT_NAME)  and isnull(amount,0)>0
							group By 	grd_Id
				...
				Left Outer join
					(
						...
						from    #TEMP_SALARY_MUSTER_REPORT1 T1 left Join							 
								t0210_Monthly_Ad_Detail Mad WITH (NOLOCK) on Mad.emp_Id = T1.emp_Id and t1.month = month(@To_DAte) and t1.year = year(@To_DAte) and UPPER(T1.LABEL_NAME) = UPPER(@ad_Name_Esic) INNER JOIN 
								...
								(Select isnull(SUM(M_AREAR_AMOUNT),0)+ isnull(SUM(M_AREAR_AMOUNT_cutoff),0) as Arear_Calc_Amount,Emp_ID	--Ankit 13012014
								 ...
								 Where AD_ID in (
								  ...
								  (select	For_Date,emp_id,sum(Amount) as Amount,sum(ESIC) as ESIC,sum(Net_Amount) as Net_Amount 
									...
									where   Es.Cmp_Id = @Cmp_Id and ad_def_Id  = @AD_Def_ID
								  group by  emp_id,For_Date)	ESICNT on month(ESICNT.For_Date)=month(MAD.To_date) and year(ESICNT.For_Date) = year(MAD.To_date)
										    and ESICNT.Emp_Id=MAD.Emp_ID					 
						where   T1.amount <> 0 and  					
								AM.AD_NOT_EFFECT_SALARY <>1 And	 Sal_Type<>1 and 
								AM.ad_def_Id = @AD_Def_ID and t1.cmp_Id = @Cmp_Id
								and month(Mad.for_date) = month(@To_date) and year(Mad.for_date) =year(@To_DAte)
						GROUP by T1.grd_Id						
		...
		BEGIN
				...
				from #Temp_Salary_Muster_Report1 T Left Outer join
					(
						select Count(Ec.Emp_Id) as PF_Count,[type_ID]
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
					...
					Left Outer join
					(
						select Count(Ec.Emp_Id) as ESIC_Count,[type_ID]
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN
			If exists(Select 1 from  #TEMP_SALARY_MUSTER_REPORT1 where ISNUMERIC(Dept_Code) > 0)
				Begin					
					...
					from #Temp_Salary_Muster_Report1 T Left Outer join
						(
							select COUNT(EC.emp_Id) as PF_Count, cast(CASE WHEN ISNUMERIC(Dept_Code) = 1 THEN Dept_Code ELSE 0 END as Numeric) as Dept_Code
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad WITH (NOLOCK)  On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
						...
						Left Outer join
						(
							select COUNT(EC.emp_Id) as ESIC_Count, cast(CASE WHEN ISNUMERIC(Dept_Code) = 1 THEN Dept_Code ELSE 0 END as Numeric) as Dept_Code
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
					...
					(
						select  Sum( MAD.M_AD_Calculated_Amount ) + sum( ISNULL(ES.M_AD_Calculate_Amount,0)) + sum(Isnull(Arear_Calc_Amount,0) )+ sum( ISNULL(MS.Arear_Basic,0)) + 
								sum(ISNULL(MS.Basic_Salary_Arear_cutoff ,0))  + sum( ISNULL(ESICNT.Amount,0)) as ESIC_WAGES,
								 cast(CASE WHEN ISNUMERIC(Dept_Code) = 1 THEN Dept_Code ELSE 0 END as Numeric) as Dept_Code
						from    #TEMP_SALARY_MUSTER_REPORT1 T1 left Join							 
								t0210_Monthly_Ad_Detail Mad WITH (NOLOCK) on Mad.emp_Id = T1.emp_Id and t1.month = month(@To_DAte) and t1.year = year(@To_DAte) and UPPER(T1.LABEL_NAME) = UPPER(@ad_Name_Esic) INNER JOIN 
								T0080_EMP_MASTER E WITH (NOLOCK) on MAD.emp_ID = E.emp_ID INNER  JOIN 
								#Emp_Cons EC ON E.EMP_ID = EC.EMP_ID	inner join
				...
				begin 
					...
					from #Temp_Salary_Muster_Report1 T Left Outer join
						(
							select Count(Ec.Emp_Id) as PF_Count,dept_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
						...
						Left Outer join
						(
							select Count(Ec.Emp_Id) as ESIC_Count,dept_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN	
					...
					from #Temp_Salary_Muster_Report1 T Left Outer join
						(
							select Count(Ec.Emp_Id) as PF_Count,desig_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
						...
						Left Outer join
						(
							select Count(Ec.Emp_Id) as ESIC_Count,desig_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad WITH (NOLOCK)  On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN
					...
					from #Temp_Salary_Muster_Report1 T Left Outer join
						(
							select Count(Ec.Emp_Id) as PF_Count,EC.branch_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad WITH (NOLOCK)  On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	
							where ad_def_Id = @Pf_Def_Id
							group By 	EC.branch_Id					
						)Q On Q.branch_Id = T.branch_Id
						Left Outer join
						(
							select Count(Ec.Emp_Id) as ESIC_Count,EC.branch_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN
					...
					from #Temp_Salary_Muster_Report1 T Left Outer join
						(
							select Count(Ec.Emp_Id) as PF_Count,vertical_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
						...
						Left Outer join
						(
							select Count(Ec.Emp_Id) as ESIC_Count,vertical_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN
				...
				from #Temp_Salary_Muster_Report1 T Left Outer join
					(
						select Count(Ec.Emp_Id) as PF_Count,subvertical_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
					...
					Left Outer join
					(
						select Count(Ec.Emp_Id) as ESIC_Count,subvertical_Id
							from #Emp_Cons Ec CROSS APPLY
								 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
								 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
								 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
		...
		BEGIN		
			...
			from #Temp_Salary_Muster_Report1 T Left Outer join
				(
					select Count(Ec.Emp_Id) as PF_Count,Center_code
					from #Emp_Cons Ec CROSS APPLY
						 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
						 T0100_EMP_EARN_DEDUCTION  Mad  WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
						 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
						 #TEMP_SALARY_MUSTER_REPORT1 T1 On T1.emp_Id = Ec.emp_Id and UPPER(LABEL_NAME) = UPPER(@ad_Name_Pf)
					where ad_def_Id = @Pf_Def_Id and Upper(@ad_Name_Pf) = UPPER(AD_SORT_NAME)
					group By 	Center_code --Center_ID  Added By Jimit 06060219 as there are chages at AIA.
				)Q On Q.Center_code = T.Center_code
				Left Outer join
				(
					select Count(Ec.Emp_Id) as ESIC_Count,Center_code
					from #Emp_Cons Ec CROSS APPLY
						 dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,EC.Emp_ID,@To_Date) F Inner join  
						 T0100_EMP_EARN_DEDUCTION  Mad WITH (NOLOCK) On F.Increment_Id = MAd.Increment_Id and F.emp_Id = Mad.emp_Id inner join
						 T0050_AD_MASTER Am WITH (NOLOCK) On Am.AD_ID = mad.AD_ID	left Join	
				...
				)Q1 On Q1.Center_code = T.Center_code
				Left Outer join
				(
					select  Sum( MAD.M_AD_Calculated_Amount ) + sum( ISNULL(ES.M_AD_Calculate_Amount,0)) + sum(Isnull(Arear_Calc_Amount,0) )+ sum( ISNULL(MS.Arear_Basic,0)) + 
										sum(ISNULL(MS.Basic_Salary_Arear_cutoff ,0))  + sum( ISNULL(ESICNT.Amount,0)) as ESIC_WAGES,
						...
						where   T1.amount <> 0 and  					
								...
								AM.ad_def_Id = @AD_Def_ID and t1.cmp_Id = @Cmp_Id
								and month(Mad.for_date) = month(@To_date) and year(Mad.for_date) =year(@To_DAte)
						GROUP by T1.Center_code
				)Q2 On Q2.Center_code = T.Center_code
			Order by Case When IsNumeric(Alpha_Emp_Code) = 1 then Right(Replicate('0',21) + Alpha_Emp_Code, 20)
			              When IsNumeric(Alpha_Emp_Code) = 0 then Left(Alpha_Emp_Code + Replicate('',21), 20)
						  Else Alpha_Emp_Code
						  End ,Row_ID		
...
-----End---------- 
	...
	DECLARE CUR_EMP CURSOR FOR
		...
		OPEN  CUR_EMP
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN		
					Select @Increment_Id=Increment_Id From dbo.fn_getEmpIncrementWithoutTransfer(@Cmp_Id,@Emp_Id,@To_Date)
				...
				union
					SELECT @Emp_Id,@CUR_SAL_TRAN_ID,AD_SORT_NAME,E_AD_MODE, E_AD_AMOUNT ,E_AD_FLAG 
							FROM (
							SELECT EED.AD_ID,
								...
								ADM.AD_LEVEL,ADM.is_rounding,E_AD_MODE
							...
							) Qry Inner Join T0050_AD_MASTER AM WITH (NOLOCK) on Qry.Ad_Id=AM.AD_ID
					--SELECT EE.EMP_ID,@CUR_SAL_TRAN_ID,AM.AD_SORT_NAME,E_AD_MODE, E_AD_AMOUNT ,E_AD_FLAG 
					--FROM T0100_EMP_EARN_DEDUCTION EE 
					--INNER JOIN
					--(
					--		SELECT MAX(INCREMENT_ID) AS INCREMENT_ID,EED.AD_ID,EED.EMP_ID FROM T0100_EMP_EARN_DEDUCTION EED   Inner JOIN
					--		( 
					--			SELECT MAX(FOR_DATE)AS FOR_DATE,EED.AD_ID,EED.EMP_ID FROM T0100_EMP_EARN_DEDUCTION EED
					...
					--INNER JOIN T0050_AD_MASTER AM ON AM.AD_ID = EE.AD_ID  		
	...
	Else If @group_Name = 9 
		Begin
			Select Label_Name,Row_Id, M_ad_Flage, SUM(Amount) As Total_Amt,TMP.Center_Code as group_Name,0 as Group_Id--IsNull(Tmp.Center_Id,0) as Group_Id
			from dbo.#Temp_Salary_Muster_Report1  as TMP 
			Group by Center_Code,Label_Name,Row_Id,M_ad_Flage 
			Order By Row_Id,Label_Name			
		End
	...
	If @group_Name = 2
		...
		Begin	
			SELECT  T.LABEL_NAME AS ALLOWANCE_NAME,ISNULL(Sum(RW.ALLOWANCE_AMOUNT),0) AS ALLOWANCE_AMOUNT,
					M_AD_FLAGE AS ALLOWANCE_TYPE,ROW_ID
					,T.Center_Code as group_Name,0 as Group_Id--IsNull(T.Center_Id,0) as Group_Id
			FROM	#TEMP_SALARY_MUSTER_REPORT1 T 
					LEFT OUTER JOIN #RATEOFWAGES RW ON	T.EMP_ID = RW.EMP_ID	AND T.LABEL_NAME = RW.ALLOWANCE_NAME				
			GROUP BY Center_Code,Label_Name,M_AD_FLAGE,ROW_ID
			ORDER BY Row_ID,Label_Name
					--End
		end
	---ended
