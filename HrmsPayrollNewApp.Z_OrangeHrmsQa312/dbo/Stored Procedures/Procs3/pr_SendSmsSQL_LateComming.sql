...
CREATE PROCEDURE [dbo].[pr_SendSmsSQL_LateComming]
...
IF OBJECT_ID(N'tempdb..#tmpEmpMaster', N'U') IS NOT NULL 
		DROP TABLE #tmpEmpMaster_LateComming,#tmpEmpMaster_LateComming1,#TempEmpList
...
,right('0' + convert(varchar,LTRIM(DATEDIFF(MINUTE, 0,(select Shift_St_Time from T0040_SHIFT_MASTER where Shift_ID=E.Shift_ID))+05) / 60),2) + ':' + right('0' + convert(varchar,LTRIM(DATEDIFF(MINUTE, 0,(select Shift_St_Time from T0040_SHIFT_MASTER where Shift_ID=E.Shift_ID))+05) % 60),2) as ShiftTime
--,CONVERT(char(5),(select top 1 D.IO_DateTime where D.Enroll_No=E.Enroll_No and Convert(date,D.IO_DateTime,103)=Convert(date,@For_Date,103) ) , 108) as InTime
,CONVERT(char(5),(select top 1 IO_DateTime from T9999_DEVICE_INOUT_DETAIL where Enroll_No=E.Enroll_No and Convert(date,IO_DateTime,103)=Convert(date,getdate(),103)),108) as  Intime
...
and (CONVERT(char(5),(select top 1 IO_DateTime from T9999_DEVICE_INOUT_DETAIL where Enroll_No=E.Enroll_No and Convert(date,IO_DateTime,103)=Convert(date,getdate(),103)),108))>
...
E.Enroll_No ,E.Emp_Full_Name, Mobile_No 
,right('0' + convert(varchar,LTRIM(DATEDIFF(MINUTE, 0,(select Shift_St_Time from T0040_SHIFT_MASTER where Shift_ID=E.Shift_ID))+05) / 60),2) + ':' + right('0' + convert(varchar,LTRIM(DATEDIFF(MINUTE, 0,(select Shift_St_Time from T0040_SHIFT_MASTER where Shift_ID=E.Shift_ID))+05) % 60),2) as ShiftTime
--,CONVERT(char(5),(select top 1 M.IO_DateTime where M.Emp_ID=E.Emp_ID and Convert(date,M.IO_DateTime,103)=Convert(date,@For_Date,103)) , 108) as InTime
,CONVERT(char(5),(select top 1 IO_DateTime from T9999_DEVICE_INOUT_DETAIL where Enroll_No=E.Enroll_No and Convert(date,IO_DateTime,103)=Convert(date,getdate(),103)),108) as  Intime
--,(select Shift_St_Time from T0040_SHIFT_MASTER where Shift_ID=E.Shift_ID) as Time
...
AND CONVERT(datetime,M.IO_DateTime ,103) > cast(@For_Date as DATE)
 Order by Emp_Full_Name
  ...
  select ROW_NUMBER() OVER(ORDER BY Emp_Full_Name ASC) AS RowNo,* into #TempEmpList from( 
select * from #tmpEmpMaster_LateComming  
Union 
select * from #tmpEmpMaster_LateComming1 )a
...
select @Cnt = COUNT(1) from #TempEmpList

end
while @rowCnt <= @Cnt
BEGIN 
Declare @Intime as datetime,@shifttime as datetime,@Latetime as varchar(50)
	SELECT @MobileNo = Mobile_No --, @EmpName = Emp_Name 
	from #TempEmpList where rowNo = @rowCnt 
	select @Intime= Convert(Varchar,InTime,8)
	from #TempEmpList  where rowNo = @rowCnt 
	select @ShiftTime= Convert(Varchar,ShiftTime,8)
	from #TempEmpList  where rowNo = @rowCnt 
	set @Latetime=(Select CAST((@intime-@shifttime) as time(0))) 

	--((Select format(InTime from #TempEmpList), 'HH:mm') - (Select format(InTime from #TempEmpList), 'HH:mm'))
	...
	set @smstext ='Today You are Late by  ' + Convert(Char(5),@Latetime,108) +' as per our biometric record.Thank You.' 
	--Today you are late by 10 minutes. Thank you
