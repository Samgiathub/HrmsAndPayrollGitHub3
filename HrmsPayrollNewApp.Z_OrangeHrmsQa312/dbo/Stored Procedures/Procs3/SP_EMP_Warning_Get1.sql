...
CREATE PROCEDURE [dbo].[SP_EMP_Warning_Get1]
	 @Cmp_ID			numeric
	,@From_Date			datetime
	,@To_Date			datetime 
 ...
 --else        
	...
	--End
		...
		FROM	T0080_EMP_MASTER E WITH (NOLOCK) inner join T0010_Company_Master CM WITH (NOLOCK) on Cm.Cmp_Id =E.Cmp_ID INNER JOIN 
				...
				T0040_Shift_Master SM WITH (NOLOCK) on E.Shift_ID = SM.Shift_ID LEFT OUTER JOIN--inner join
				( 
		...
		WHERE	E.Cmp_ID = @Cmp_Id	
				...
				--AND E.Branch_ID IN (
				--	--cast((SELECT REPLACE(@Branch_ID, '#',',')) as numeric(100))
				--	select cast(data as numeric(50)) as Branch_ID from dbo.Split(@Branch_ID,'#')
				--)
		ORDER BY E.Emp_Code ASC
		--SELECT REPLACE(@Branch_ID, '#',',')
		--select cast(data as numeric(18)) from dbo.Split(@Branch_ID,'#')
		--return
