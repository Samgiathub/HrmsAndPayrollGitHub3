...
CREATE PROCEDURE [dbo].[SP_Employee_Inout_Reminder_Inductotherm]
...
SET ANSI_WARNINGS OFF;
     ...
     (
		...
		----- CUTOFF DATE WISE REPORT FOR INDUCTOTHERM CLIENT REQUIREMENT
		
		DECLARE @CUTOFFDATE_SALARY AS DATETIME
		DECLARE @CUTOFF_START_DATE DATETIME
		SET @CUTOFFDATE_SALARY = '2011-10-24'
		IF ISNULL(@CUTOFFDATE_SALARY,'') <> ''
				BEGIN
					
					SET @CUTOFFDATE_SALARY =  CAST(CAST(DAY(@CUTOFFDATE_SALARY)AS VARCHAR(5)) + '-' + CAST(DATENAME(MM,@from_date) AS VARCHAR(10)) + '-' +  CAST(YEAR(@from_date )AS VARCHAR(10)) AS SMALLDATETIME)    
					IF MONTH(@from_date) = 1
						BEGIN
							SET @CUTOFF_START_DATE = CAST(CAST(DAY(@CUTOFFDATE_SALARY+1)AS VARCHAR(5)) + '-' + CAST(DATENAME(MM,@from_date-1) AS VARCHAR(10)) + '-' +  CAST(YEAR(@from_date) - 1 AS VARCHAR(10)) AS SMALLDATETIME)    
						END
					ELSE
						BEGIN
							SET @CUTOFF_START_DATE = CAST(CAST(DAY(@CUTOFFDATE_SALARY+1)AS VARCHAR(5)) + '-' + CAST(DATENAME(MM,@from_date-1) AS VARCHAR(10)) + '-' +  CAST(YEAR(@from_date )AS VARCHAR(10)) AS SMALLDATETIME)    
						END
						
					SET @FROM_DATE = @CUTOFF_START_DATE
					SET @TO_DATE = DATEADD(DD,2,@CUTOFFDATE_SALARY)
	...
	WHILE @@FETCH_STATUS = 0                    
		BEGIN     
			...
			SELECT TOP 1 @Manager_Email = Work_Email FROM T0080_EMP_MASTER E WITH (NOLOCK) Where emp_id = @Manager_ID 
			  ...
			  ---ALTER dynamic template for Employee.				
                  ...
                  SET @Body = ( SELECT  
                                ...
                                FROM    #Temp_inout  
                                WHERE   Emp_ID = @Emp_Id ORDER BY  #Temp_inout.FOR_DATE,#Temp_inout.Emp_code For XML raw('tr'), ELEMENTS) 
