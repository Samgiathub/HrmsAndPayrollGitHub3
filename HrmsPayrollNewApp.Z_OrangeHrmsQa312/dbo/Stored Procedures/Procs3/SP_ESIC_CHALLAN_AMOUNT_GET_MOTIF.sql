...
CREATE PROCEDURE [dbo].[SP_ESIC_CHALLAN_AMOUNT_GET_MOTIF]  
 @CMP_ID   NUMERIC,  
 @FROM_DATE  DATETIME,  
 @TO_DATE   DATETIME,  
 @BRANCH_ID  NUMERIC,
...
AS  
 ...
 SELECT MAD.CMP_ID ,@BRANCH_ID,COUNT(MAD.EMP_ID),SUM(M_AD_CALCULATED_AMOUNT),MAX(M_AD_Actual_Per_Amount),SUM(MAD.M_AD_AMOUNT),SUM(CEILING(M_AD_CALCULATED_AMOUNT *@EMPLOYER_CONT_PER/100)) ,@EMPLOYER_CONT_PER FROM T0210_MONTHLY_aD_DETAIL MAD WITH (NOLOCK) INNER JOIN   
  ( select I.Emp_Id,BRANCH_ID,Segment_ID,Vertical_ID,SubVertical_ID,subBranch_ID from T0095_Increment I WITH (NOLOCK)inner join   
 ...
 AND Iq.Segment_ID = ISNULL(@Segment_Id,IQ.Segment_ID) --Added By Gadriwala 21082013
 AND IQ.Vertical_ID = ISNULL(@Vertical_Id,IQ.Vertical_ID)--Added By Gadriwala 21082013
 AND IQ.SubVertical_ID = ISNULL(@SubVertical_Id,IQ.SubVertical_ID)--Added By Gadriwala 21082013
 AND IQ.subBranch_ID = ISNULL(@SubBranch_Id,IQ.subBranch_ID)	--Added By Gadriwala 21082013
 ...
 if @PBranch_ID <> '0' 
		begin
			SELECT * FROM @ESIC_CHALLAN where BRANCH_ID in (select cast(data as numeric) from dbo.Split(@pBranch_id,'#'))
		end
	else
		begin
