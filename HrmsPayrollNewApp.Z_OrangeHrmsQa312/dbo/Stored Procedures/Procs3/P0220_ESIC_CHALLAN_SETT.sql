...
CREATE PROCEDURE [dbo].[P0220_ESIC_CHALLAN_SETT]
...
SET ARITHABORT ON
	...
	if @tran_type = 'I'
			begin
					If Exists(select ESIC_Challan_ID From T0220_ESIC_CHALLAN_SETT WITH (NOLOCK) Where Cmp_ID = @Cmp_ID  and Month = @Month AND Year=@Year and Branch_ID=@Branch_ID )
						begin
							set @ESIC_Challan_ID = 0
							return 
					end
					begin
					 select @ESIC_Challan_ID = Isnull(max(ESIC_Challan_ID),0) + 1 	From T0220_ESIC_CHALLAN_SETT WITH (NOLOCK)
					end
				
				INSERT INTO T0220_ESIC_CHALLAN_SETT
				                      (
											ESIC_Challan_ID
											,Cmp_ID
											,Branch_ID
								...
								(
										...
										-- Added for audit trail By Ali 19102013 -- Start
											...
											exec P9999_Audit_Trail @Cmp_ID,@tran_type,'ESIC Challan Sett',@OldValue,@ESIC_Challan_ID,@User_Id,@IP_Address
										-- Added for audit trail By Ali 19102013 -- End	
			END
				...
				begin
										-- Added for audit trail By Ali 19102013 -- Start
											...
											FROM dbo.T0220_ESIC_CHALLAN_SETT WITH (NOLOCK) 
						...
						UPDATE    T0220_ESIC_CHALLAN_SETT
						SET        
		...
		Begin
										...
										-- Added for audit trail By Ali 19102013 -- Start
											...
											exec P9999_Audit_Trail @Cmp_ID,@tran_type,'ESIC Challan Sett',@OldValue,@ESIC_Challan_ID,@User_Id,@IP_Address
										-- Added for audit trail By Ali 19102013 -- End	
				DELETE FROM T0220_ESIC_CHALLAN_SETT where ESIC_Challan_ID = @ESIC_Challan_ID 
