...
CREATE PROCEDURE [dbo].[P0200_MONTHLY_SALARY_GENERATE_SHORT_FALL]
...
SET ARITHABORT ON
		...
		SELECT @temp_t_amt = ISNULL(SUM(E_AD_amount),0)
					FROM dbo.T0100_EMP_EARN_DEDUCTION EED WITH (NOLOCK) INNER JOIN  
						( Select Max(For_Date) For_Date, Ad_Id From T0100_EMP_EARN_DEDUCTION WITH (NOLOCK) 
							Where Emp_Id  = @Emp_Id And For_date <= @Month_End_Date and INCREMENT_ID = @Increment_ID
	...
	select 	
	--@Short_Fall_W_Days = isnull(Short_Fall_W_Days,0) -- Change the Column from  Short_Fall_W_Days to Fnf_Fix_Day Base on Genus request from chintan and Sachin 18052022
	@Short_Fall_W_Days = isnull(Fnf_Fix_Day,0)
	,@sal_St_Date=isnull(Sal_St_Date,'')  
