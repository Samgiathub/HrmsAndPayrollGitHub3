...
CREATE PROCEDURE [dbo].[SP_Get_Travel_Settlement_Application_Records]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
...
BEGIN
	...
	(
		...
		select distinct AppEmp,Cmp_ID into #AppEmp1 from T0080_Travel_HycScheme_Sett TT
		inner join T0080_DynHierarchy_Value DV on DV.Emp_ID = TT.AppEmp and Dv.DynHierColValue = Tt.RptEmp and DV.DynHierColId = TT.DynHierId
		where Dv.DynHierColValue = @Emp_ID
		...
		inner join (
			SELECT DISTINCT T.Scheme_Id from T0095_EMP_SCHEME T 
			Inner Join T0050_Scheme_Detail T1 ON T.Scheme_ID = T1.Scheme_Id 
			inner join T0140_Travel_Settlement_Application TAD on Convert(Varchar,TAD.emp_id) in (select AppEmp from #AppEmp) --Code Added by Yogesh on 02022023
				where tad.Emp_ID in (select AppEmp from #AppEmp) And Type = 'Travel Settlement' 
			...
			AND Effective_Date in (SELECT Effective_Date from T0095_EMP_SCHEME where Emp_ID in (select AppEmp from #AppEmp) And Type = 'Travel Settlement' AND Effective_Date <= getdate())  
		) Q on ES.Scheme_Id = Q.Scheme_id
		inner join T0080_Travel_HycScheme_Sett TTH on ES.Emp_ID = TTH.AppEmp and  ES.Scheme_ID = TTh.SchemeIId 
										   ...
										   Emp_ID numeric    
														...
														If @Rpt_level = 1
															Begin	
																...
																'Select LAD.Travel_Set_Application_id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' ,isnull(LAD.TravelTypeId,0) as Travel_Type_Id From V0140_Travel_Settlement_Application_New_Level LAD
															 ...
															 IF @Rpt_level = 2
																	BEGIN
																		...
																		IF Object_ID('tempdb..#EMP_CONS_RM') IS NOT NULL
																			...
																			-- Code Commented by yogesh on  25052023=========================================================================
																				...
																				--					FROM	(SELECT LAD.Travel_Set_Application_id,LAD.Status_New,For_date,LAd.Alpha_Emp_Code,Emp_First_Name, ,isnull(LAD.TravelTypeId,0) as Travel_Type_Id
												...
												Else if @is_rpt_manager = 0 and @is_branch_manager = 0 and @is_Reporting_To_Reporting_manager = 0
													...
													--select * from #Emp_Cons
										 				If @Rpt_level = 1
															Begin
																...
																--select * from Emp_Cons
																	  Set @SqlQuery = 	
																'Select distinct LAD.Travel_approval_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +  cast(@Rpt_level as VARCHAR(2)) +
																 '  ,isnull(LAD.TravelTypeId,0) as Travel_Type_Id From V0140_Travel_Settlement_Application_New_Level LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																	Where LAD.Travel_Set_Application_id not In (Select Travel_Set_Application_id From T0115_Travel_Settlement_Level_Approval WITH (NOLOCK)
																													Where  Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')'  									
																	  ...
																	  --Made Change  on Last where condtion  from "Not in" to "In"  Lineno: 764 by Yogesh on 10022023
													...
													--	select * from #Emp_Cons
																...
																'Select case  when LAD.Travel_approval_Id=0 then LAD.Travel_Set_Application_id else LAD.Travel_approval_Id end as Travel_approval_Id, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																 ' ,isnull(LAD.TravelTypeId,0) as Travel_Type_Id From V0140_Travel_Settlement_Application_New_Level LAD
																	Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																Where (LAD.Travel_Set_Application_id Not In (Select Travel_Set_Application_id From T0115_Travel_Settlement_Level_Approval WITH (NOLOCK)
																												Where   Rpt_Level = ' + Cast(@Rpt_level as varchar(2)) + ')
												...
												Begin
													...
													SELECT *,cast(data  as numeric) as Val into #temp FROM #tbl_Leave_App
													...
													Begin 
															...
															---- need to check on Single schem is applied Leave = 1#2
														...
														DELETE  from #tbl_Leave_App where Leave_App_ID not in(
																select Distinct La.Leave_App_ID from #temp  LA 
																inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave
																inner join V0140_Travel_Settlement_Application_New_Level VAD
																on LA.Val = VAD.TravelTypeId and LA.Leave_App_ID = VAD.Travel_Set_Application_id or LA.Leave_App_ID = VAD.Travel_Approval_ID  --and VAD.TravelTypeId in (Sd.Leave)
														)
													...
													BEGIN 
														...
														--BEGIN
														--	--print 12
														--	--	DELETE TA from #tbl_Leave_App TA where TA.Travel_Type_ID not in (
														--	--		select Distinct LA.Val from #temp  LA 
														--	--		inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave 
														--	--		inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id and LA.Leave_App_ID = VAD.Travel_Application_ID
														--	--)-- a on TA.Travel_Type_ID <> A.Travel_Type_Id
														--	--select * from #tbl_Leave_App
														...
														--	--select * from #tbl_Leave_App
														--	delete C
														--			FROM #tbl_Leave_App AS c 
														--			WHERE NOT EXISTS 
														--			( Select   LA.Scheme_ID,LA.Leave_App_ID
														--				from #temp  LA 
														--						inner join T0050_Scheme_Detail SD on LA.rpt_level = SD.Rpt_Level and LA.Leave = SD.Leave and la.Scheme_ID = SD.Scheme_Id
														--						inner join V0100_TRAVEL_APPLICATION VAD on LA.Val = VAD.Travel_Type_Id and LA.Leave_App_ID = VAD.Travel_Application_ID
														--			  --SELECT LA.SCHEME_ID , LA.LEAVE_APP_ID
														--			  --FROM #tmp LA 
														--			  where c.Leave_App_ID = La.Leave_App_ID
														--			  and C.Scheme_ID = La.Scheme_ID
														--			) 
					...
					--Select * From #tbl_Leave_App
						Insert INTO #Travel
							Select distinct	LAD.Emp_ID, LAD.Emp_Full_Name,LAD.Supervisor,
								...
								,LAD.Advance_Amount,LAD.Cmp_ID,LAD.Tran_ID,isnull(SL.Is_Aprvl_Ovrlimit,0),LAD.Travel_App_Code
								,Qry1.Approved_Expence,LAD.DirectEntry
								,Convert(int,(select count(1) from T0080_Emp_Travel_Proof where TravelApp_Code=Lad.Travel_App_Code and Cmp_Id=LAD.Cmp_ID and Emp_ID=LAD.Emp_id	)) as ProofCount  
							From V0140_Travel_Settlement_Application_New_Level LAD
								left outer join (select lla.Travel_Approval_ID As App_ID, Rpt_Level as Rpt_Level , lla.Status,lla.Approved_Expance as Approved_Expence
								,lla.Travel_Set_Application_id as Travel_Set_App_ID From T0115_Travel_Settlement_Level_Approval lla WITH (NOLOCK)
													inner join (Select max(rpt_level) as rpt_level1, Travel_Approval_ID,Travel_Set_Application_id
								...
								--Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = LAD.Travel_Set_Application_id -- Comment by deepal :- 18022022
								inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = LAD.Travel_Set_Application_id
								or TLAP.Leave_App_ID = LAD.Travel_Approval_ID
								inner Join #tbl_Scheme_Leave SL On SL.Scheme_ID = TLAP.Scheme_ID And SL.Leave = TLAP.Leave
								and  SL.rpt_level > isnull(Qry1.Rpt_Level,0) and  SL.rpt_level = TLAP.rpt_level -- or Qry1.Rpt_Level = 0)
						Where Travel_Set_Application_id In (Select Leave_App_ID From #tbl_Leave_App) or Travel_Approval_ID In (Select Leave_App_ID From #tbl_Leave_App)	 and LAD.cmp_id=@Cmp_ID
						--Where Travel_Approval_ID In (Select Leave_App_ID From #tbl_Leave_App)
		...
		If @Type = 0
			Begin
				...
				If @Emp_ID_Cur > 0
					Begin
						set @queryExe=''
						SET @queryExe='select distinct  * from #Travel ' + @OrderBy --order by #Travel.Application_Date desc
						exec (@queryExe);
					End
				Else
					Begin
						set @queryExe=''
						set @queryExe = 'select distinct  * from #Travel where ' + @Constrains  + ' ' + @OrderBy	
						exec (@queryExe);
					End
