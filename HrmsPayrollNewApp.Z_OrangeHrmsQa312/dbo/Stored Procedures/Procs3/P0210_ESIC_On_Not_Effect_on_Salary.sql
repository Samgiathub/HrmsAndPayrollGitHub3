...
-- Created By Rohit for Esic Calculation on Not Effect on Salary Component 
-- Created Date 20052015
---25/1/2021 (EDIT BY MEHUL ) (SP WITH NOLOCK)---
CREATE PROCEDURE [dbo].[P0210_ESIC_On_Not_Effect_on_Salary]
     @Emp_ID	numeric(18, 0)
	,@Cmp_ID	numeric(18, 0)
	,@For_Date	datetime
	...
	,@tran_type varchar(1)
	,@Comp_Esic numeric(18,2) = 0
	,@Tds Numeric(18,2)=0
	,@Loan_Amount Numeric(18,2)=0
	,@str_LoanDetail Varchar(max) = ''
	,@Working_Days numeric(18,2) = 0 --Added by Jaina 09-08-2017
	,@Calculate_On numeric(18,2) = 0  --Added by Jaina 09-08-2017
	,@Hour_Rate numeric(18,4) = 0  --Added by Jaina 30-08-2017
	,@Shift_Sec numeric(18,0) = 0 --Added by Jaina 30-08-2017
	--,@Tran_ID Numeric(18,0) output = 0
AS
SET NOCOUNT ON 
...
SET ARITHABORT ON
	...
	If @tran_type  = 'I' 
		Begin
			...
			if exists (select emp_id from T0210_ESIC_On_Not_Effect_on_Salary WITH (NOLOCK) where Emp_ID =@Emp_ID and month(For_Date) = month(@For_Date) and YEAR(For_date) = YEAR(@For_Date) and isnull(Ad_id,0) = @ad_id)
				begin	
					delete from T0210_ESIC_On_Not_Effect_on_Salary where Emp_ID =@Emp_ID and month(For_Date) = month(@For_Date) and YEAR(For_date) = YEAR(@For_Date) and isnull(Ad_id,0) = @ad_id
				end
				
					INSERT INTO T0210_ESIC_On_Not_Effect_on_Salary
							  (Cmp_Id,Emp_Id,For_Date,Hours,Amount,Esic,Net_Amount,Ad_Id,Modify_Date,Comp_Esic,TDS,Loan_Amount,Working_Days,Calculate_On,Hour_Rate,Shift_Sec)
					VALUES     (@Cmp_Id,@Emp_Id,@For_Date,@Hours,@Amount,@Esic,@Net_Amount,@Ad_Id,@Modify_Date,@Comp_Esic,@Tds,@Loan_Amount,@Working_Days,@Calculate_On,@Hour_Rate,@Shift_Sec)
					
					Select @Pay_Tran_ID = Tran_Id From T0210_ESIC_On_Not_Effect_on_Salary WITH (NOLOCK)
					Where Emp_Id = @Emp_Id and Month(For_Date) = Month(@Month_End_Date) and YEAR(For_Date) = YEAR(@Month_End_Date) and Ad_Id = @Ad_Id
					

					--Added by nilesh patel on 07012017 For Loan Allowance
					if @Pay_Tran_ID > 0 --and @Loan_Amount > 0
						Begin
							Exec SP_CALCULATE_LOAN_PAYMENT_ALLOWANCE @Cmp_Id,@Emp_Id,@Month_St_Date,@Month_End_Date,0,0,1,0,@Pay_Tran_ID,@str_LoanDetail
						End
								
		End
	Else if @Tran_Type = 'U' 
		begin
			UPDATE    T0210_ESIC_On_Not_Effect_on_Salary
			SET    Hours=@Hours ,Amount = @Amount ,Esic= @Esic 
	...
	Else if @Tran_Type = 'D' 
		begin
			...
			if exists(SELECT 1 From T0210_ESIC_On_Not_Effect_on_Salary WITH (NOLOCK) Where Month(For_Date) = @Sal_Month AND Year(For_Date) = @Sal_Year AND Emp_ID = @Emp_ID and Cmp_ID = @Cmp_ID)
				BEGIN
			...
			BEGIN
					...
					return
			END	
			Select @Pay_Tran_ID = Tran_Id From T0210_ESIC_On_Not_Effect_on_Salary WITH (NOLOCK) where Emp_ID = @Emp_ID and For_Date = @For_Date and isnull(Ad_id,0) = @ad_id
			Delete FROM T0210_MONTHLY_LOAN_PAYMENT Where Loan_Payment_Date = @For_Date and Pay_Tran_ID = @Pay_Tran_ID
			DELETE 	from T0210_ESIC_On_Not_Effect_on_Salary where Emp_ID = @Emp_ID and For_Date = @For_Date and isnull(Ad_id,0) = @ad_id
