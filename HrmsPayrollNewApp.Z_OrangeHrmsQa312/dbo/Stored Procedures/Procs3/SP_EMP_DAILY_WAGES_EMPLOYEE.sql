...
CREATE PROCEDURE [dbo].[SP_EMP_DAILY_WAGES_EMPLOYEE]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   = 0
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
		...
		select Sal_tran_ID,e.Emp_Id,Pan_No,BM.Comp_Name,CM.Cmp_Name,Cm.Cmp_Address,BM.Branch_address,cast( E.Emp_Code as varchar) + ' - '+E.Emp_Full_Name as Emp_Full_Name,M_IT_Tax,M_OT_Hours,Other_Dedu_Amount,M_LOAN_AMOUNT,M_ADV_AMOUNT,Other_Allow_Amount
				,@Date_Diff	Month_days	, isnull(Present_Days,@Date_Diff)Present_Days
			
		from T0080_EMP_MASTER E WITH (NOLOCK) LEFT OUTER join 
			(select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID,I.Wages_type from T0095_Increment I WITH (NOLOCK) inner join 
					( select max(Increment_effective_Date) as For_Date , Emp_ID from T0095_Increment WITH (NOLOCK)
					where Increment_Effective_date <= @To_Date
					and Cmp_ID = @Cmp_ID
					group by emp_ID  ) Qry on
			...
			WHERE E.Cmp_ID = @Cmp_Id And ((@From_Date < E.Emp_LEft_Date And @To_Date < E.Emp_LEft_Date) or E.Emp_LEft_Date is null)	and I_Q.Wages_type='Weekly'
