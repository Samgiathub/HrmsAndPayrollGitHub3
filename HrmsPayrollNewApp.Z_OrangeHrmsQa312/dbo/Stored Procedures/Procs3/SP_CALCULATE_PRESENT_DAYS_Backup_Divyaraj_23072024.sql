CREATE PROCEDURE [dbo].[SP_CALCULATE_PRESENT_DAYS_Backup_Divyaraj_23072024]    
	 @Cmp_ID  numeric    
	,@From_Date datetime    
	,@To_Date  datetime   
	,@Branch_ID numeric    
...
--hnb									
 ...
 begin  
	...
	--return
		...
		--Declare @CheckBySupirior int

		--set @CheckBySupirior = (select MAX(isnull(Chk_By_Superior,0)) from #Data where Emp_Id = @Emp_ID and For_date  between cast(@From_Date as date) and cast(@To_Date as date)
		--						and Chk_By_Superior = 1)

		----Select @CheckBySupirior
		----return

		--if @CheckBySupirior = 0
		--begin
		--	Truncate table #Data
		--end
		--else
		...
		--		INNER JOIN dbo.T0080_EMP_MASTER E  on OA.Emp_ID = E.Emp_ID 
		--		LEFT OUTER JOIN dbo.T0040_SHIFT_MASTER SM on OA.Shift_ID = SM.Shift_ID  
		--	WHERE OA.Chk_By_Superior = 1
		--	AND OA.Emp_ID = @Emp_ID
		--	AND OA.For_Date BETWEEN cast(@From_Date as date) and cast(@To_Date as date)
		--	ORDER BY E.emp_ID,For_Date   
		--end
