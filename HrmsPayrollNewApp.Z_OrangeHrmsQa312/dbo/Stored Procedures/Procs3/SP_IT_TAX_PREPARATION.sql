...
CREATE PROCEDURE [dbo].[SP_IT_TAX_PREPARATION] 
  @Cmp_ID NUMERIC
, @From_Date DATETIME
, @To_Date DATETIME
...
AS
  ...
  , Is_Salary_comp, Exem_Againt_row_Id, Exempted_Amount, Show_In_SalarySlip, Display_Name_For_SalarySlip, Column_24Q)
    ...
    I.Cmp_ID = @Cmp_ID
  AND  Default_Def_Id NOT IN   (101, -102, -103, 103, 104, 105, 106, 107, 108, 120, 121, 102, 173)   -- tejas
  ...
  INSERT INTO #Tax_Report_Male (Field_Name, Default_def_ID, IT_L_ID, Is_Show, Gender)
    ...
    INNER JOIN (SELECT DISTINCT top 1
        Cmp_ID
 ...
 -- select * from #Tax_Report
  ...
  --- Added by Sajid 29-07-2024 For Standard Deduction for Tax Regime 2
	UPDATE	TR 
	SET		Amount_Col_Final = CASE WHEN LE.Emp_ID IS NOT NULL AND (LE.Is_Death = 1 OR LE.Is_Retire = 1) THEN ISNULL(IFD.Field_Value2,0) ELSE (ISNULL(IFD.Field_Value2,0)/12)* dbo.F_GET_STANDARD_DED_MONTH_COUNT(TR.Emp_ID,@From_Date,@To_Date) END,
			Amount_Col_Actual = CASE WHEN LE.Emp_ID IS NOT NULL AND (LE.Is_Death = 1 OR LE.Is_Retire = 1) THEN ISNULL(IFD.Field_Value2,0) ELSE (ISNULL(IFD.Field_Value2,0)/12)* dbo.F_GET_STANDARD_DED_MONTH_COUNT(TR.Emp_ID,@From_Date,@To_Date) END
	FROM	#Tax_Report TR 
...
--	      select Amount_Col_Final,Tax_Regime,Is_Exempted,* from #Tax_Report where Row_ID in (256
--,257
--,258
--,259) --Default_Def_Id = 174
  ...
  BEGIN
    ...
    SET @query = NULL;
    SELECT
      @query = COALESCE(@query + ' UNION ', '') + 'SELECT Emp_ID, Formula_Name,  Cast(' + Replace(Formula_Name,'/ 0','') + ' as  BIGINT) AS CalcValue FROM #Tbl_Formula_Result WHERE Formula_Name = ''' + Replace(Formula_Name,'/ 0','') + ''' '
  ...
  ---- Nilay20062014Perpuisite amount calculation-----
    ...
    -- Deepal  NPS  Logic 09-10-2024
	UPDATE #Tax_Report
	SET Amount_Col_Final = a.BasicAmount
	FROM #Tax_Report Tr 
	inner join #Emp_Cons EC on TR.Emp_ID = EC.Emp_ID
	INNER JOIN (
		Select ((Amount_Col_Final * 14) / 100)   as BasicAmount ,T_ID,Emp_ID
		from #Tax_Report where Default_Def_Id = @Cont_Basic_Sal
	) a on Tr.Emp_ID= A.Emp_ID
	where   Tr.Default_Def_Id = @NPSExempt AND 
	(Tr.Tax_Regime = 'Tax Regime 2') AND To_Date >= '2024-04-01' 
		...
		UPDATE #Tax_Report
			...
			inner join #Emp_Cons EC on TR.Emp_ID = EC.Emp_ID
			INNER JOIN (
				Select ((Amount_Col_Final * 10) / 100)   as BasicAmount ,T_ID,Emp_ID
				from #Tax_Report where Default_Def_Id = @Cont_Basic_Sal
			) a on Tr.Emp_ID= A.Emp_ID
			where   Tr.Default_Def_Id = @NPSExempt AND 
  ...
  EXEC GET_EMP_PERQUISITES @Cmp_ID
	...
	--	where emp_id = @Emp_ID and Effective_Date between @From_Date  and @To_Date
	--	Group by Emp_ID
	--) a  on m.Emp_id = A.emp_id and m.Effective_DATE = a.MaxEffectiveDate

	--SET @Perquisit_amount = @Perquisit_amount + @TaxablePreqValue
	Declare @TaxablePreqValue  as numeric(18) = 0
    Select @TaxablePreqValue = SUM(TaxablePerqValue) from T0040_Emp_ESOP_Allocation  m inner join
    (
      Select  Emp_Id
      from T0040_Emp_ESOP_Allocation 
      where emp_id = @EMP_ID_Per and Effective_Date between @From_Date  and @To_Date
      Group by Emp_ID
    ) a  on m.Emp_id = A.emp_id 
  ...
  END
  ELSE
  IF @IS_TOTAL = 1 AND RTRIM(@Multiple_Row_ID) <> ''
  BEGIN
  ...
  END
  ELSE
  IF @IS_TOTAL = 2 AND @From_Row_ID > 0 AND @TO_ROW_ID > 0
  BEGIN
  ...
  END
  ELSE
  IF @IS_TOTAL = 3 AND @From_Row_ID > 0 AND @TO_ROW_ID > 0 AND @Max_Limit > 0 AND @Exem_Ag_RowID > 0 -- Added by Deepal To check the NPS condition 01022022 As discussed with chintan bhai related to tradebull client
  BEGIN
  ...
  BEGIN
    ...
    --Commented by deepal
    SET @query = NULL;
    SELECT @query = COALESCE(@query + ' UNION ', '') + 'SELECT Emp_ID, Formula_Name, ' + Replace(Formula_Name,'/ 0','') + ' AS CalcValue 
					FROM #Tbl_Formula_Result WHERE Formula_Name = ''' + Replace(Formula_Name,'/ 0','') + ''' '
    FROM #Tbl_Formula_Result

	

    SET @query = 'INSERT INTO #Tbl_Result ' + @query
...
--  select * from #Tax_Report where Row_ID = 259
...
--	select * from #Tax_Report where Row_ID = 259
  ...
  IF @Taxabel_Income_S >= 50000000
  BEGIN
	IF (select top 1 Field_Name from #Tax_Report where Emp_ID = @Emp_ID and Field_Name = 'Margin relief' and From_Date >= '2024-04-01' and To_Date = '2025-03-31' and  @TAX_REGIME = 'Tax Regime 1') = 'Margin relief' 
		BEGIN
			DECLARE @MR1 numeric(18,2), @MR2 numeric(18,2),@MR3 numeric(18,2);

			select @MR1 = ISNULL(Amount_Col_Final,0) from #Tax_Report where Default_Def_Id = 101 and Emp_ID = @Emp_ID
			select @MR2 = FLOOR(@MR1 -FLOOR(FLOOR(Amount_Col_Final-50000000)*0.3)) from #Tax_Report where Field_Name = '12. Total Taxable Income (9 - 11)' and Emp_ID = @Emp_ID

			select @MR3 =   ROUND((select ISNULL(Amount_Col_Final,0) from #Tax_Report where Default_Def_Id = 102 and Emp_ID = @Emp_ID) - ROUND(ROUND(0.7*(Amount_Col_Final-50000000),2)+FLOOR(0.25*@MR2),2) ,2)
			from #Tax_Report where Field_Name = '12. Total Taxable Income (9 - 11)' and Emp_ID = @Emp_ID

			IF @MR3 > 0
			BEGIN
				UPDATE #Tax_Report
				SET Amount_Col_Final = ROUND(@mr3 + ROUND((@mr3*0.04),2),2) from #Tax_Report where Default_Def_Id = 173 and Emp_ID = @Emp_ID
				UPDATE #Tax_Report
				SET Amount_Col_Final = ROUND(Amount_Col_Final - (@mr3 + ROUND(ROUND(@mr3*0.04,2),2)),2) from #Tax_Report where  Emp_ID = @Emp_ID and Default_Def_Id in (103,108) 
			END
			ELSE
			BEGIN
				UPDATE #Tax_Report
				SET Amount_Col_Final = 0 from #Tax_Report where Default_Def_Id = 173 and Emp_ID = @Emp_ID
			END
		END
  END
  --//////
