...
CREATE PROCEDURE [dbo].[SP_EMP_Settlement_View_Publish]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   = 0
		   ...
		   begin		
			   select Distinct E.Emp_ID,E.Alpha_Emp_Code as Emp_Code,E.Emp_Full_Name as Employee_Name,DM.Dept_Name as Department, 
				...
				from dbo.T0080_EMP_MASTER E WITH (NOLOCK)
					...
					inner join
					( select  I.Emp_Id,Branch_ID,Dept_ID from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							...
							dbo.T0250_SALARY_PUBLISH_ESS SPE  WITH (NOLOCK) on E.Emp_ID = SPE.Emp_ID and sPE.Month = Month(@To_Date) and SPE.Year = Year(@To_Date) and SPE.Sal_Type='Settlement' Inner join --Added By Mukti Sal_Type(24062016)
	     ...
	     begin
			   ...
			   from dbo.T0080_EMP_MASTER E WITH (NOLOCK) left outer join dbo.T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
					( select  I.Emp_Id,Branch_ID,I.Emp_Full_PF,I.Emp_Auto_Vpf,Dept_ID  from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							...
							dbo.T0250_SALARY_PUBLISH_ESS SPE  WITH (NOLOCK) on E.Emp_ID = SPE.Emp_ID and IsNull(SPE.Is_Publish,0) = 1 and sPE.Month = Month(@From_Date) and SPE.Year = Year(@From_Date) and SPE.Sal_Type='Settlement' Inner join --Added By Mukti Sal_Type(24062016)
							#Emp_Cons EC on E.Emp_ID = EC.Emp_ID INNER JOIN
				...
				from dbo.T0080_EMP_MASTER E WITH (NOLOCK) left outer join dbo.T0100_Left_Emp l WITH (NOLOCK) on E.Emp_ID =  l.Emp_ID inner join
					( select  I.Emp_Id,Branch_ID,I.Emp_Full_PF,I.Emp_Auto_Vpf,Dept_ID  from dbo.T0095_Increment I WITH (NOLOCK) inner join 
							...
							--dbo.T0250_SALARY_PUBLISH_ESS SPE  on E.Emp_ID = SPE.Emp_ID and SPE.Is_Publish = 0 and sPE.Month = Month(@From_Date) and SPE.Year = Year(@From_Date) Inner join
							 left outer join  dbo.T0250_SALARY_PUBLISH_ESS SPE  WITH (NOLOCK) on E.Emp_ID = SPE.Emp_ID and sPE.Month = Month(@From_Date) and SPE.Year = Year(@From_Date) and SPE.Sal_Type='Settlement' Inner join --Added By Jaina 23-10-2015
							#Emp_Cons EC on E.Emp_ID = EC.Emp_ID INNER JOIN
							T0200_MONTHLY_SALARY MS WITH (NOLOCK) ON EC.Emp_ID = MS.Emp_ID AND MONTH(MS.Month_End_Date) = MONTH(@To_Date) AND YEAR(MS.Month_End_Date) = YEAR(@To_Date)  --Ankit 25032016
