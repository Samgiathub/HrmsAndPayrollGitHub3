...
-- Description:	For Export to Word and PDF of Template Data
-- =============================================
CREATE PROCEDURE Rpt_TemplateAnalysis 
	@Cmp_Id		numeric(18,0)
	,@T_ID		numeric(18,0)
	,@flag			int = 0
	,@Emp_ID		int=0
...
BEGIN
	...
	IF @emp_code=''
		SET @emp_code=NULL
			...
			SELECT Distinct EM.Emp_ID,EM.Emp_Full_Name,Answer,EM.Alpha_Emp_Code,Created_Date,ETR_Id,F_Id,em.Mobile_No,Branch_Name,Desig_Name,Dept_Name
			INTO #FINAL_EMP
		...
		CREATE TABLE #Table2
			(
				...
				,Answer				nvarchar(1500) 
				,Field_Type		nvarchar(50)  
				,Field_Name	nVARCHAR(1500)
				,Created_Date		varchar(15)
				,Mobile_No			varchar(20)
				,IMEI_NO			varchar(250)
				,Template_Title nvarchar(500)		
				,Template_Date varchar(15)		
	...
	BEGIN
		...
		INSERT INTO #TEMPLATE_DETAILS(CMP_ID,Answer,T_Id,F_Id,Emp_id,Employee_Code,Employee_Name,Field_Name,sorting_no,Branch_Name,Desig_Name,Dept_Name,Created_Date,Template_Title)						
		SELECT TF.Cmp_Id,TR.Answer,TR.T_Id,TR.F_Id,EI.Emp_Id, EI.Alpha_Emp_Code ,EI.Emp_Full_Name as EmployeeName,
		REPLACE(REPLACE(TF.Field_Name,'[',''),']',''),TF.Sorting_No,EI.Branch_Name,EI.Desig_Name,EI.Dept_Name,
		CONVERT(varchar(15),EI.Created_Date,103),TM.Template_Title
		FROM T0050_Template_Field_Master TF	WITH (NOLOCK)	
		INNER JOIN T0040_Template_Master TM WITH (NOLOCK) ON TM.T_ID=TF.T_ID
		LEFT JOIN T0100_Employee_Template_Response TR WITH (NOLOCK) ON TR.F_Id = TF.F_ID and  TR.T_Id = TF.T_ID
		INNER JOIN #FINAL_EMP EI ON EI.Emp_ID = TR.Emp_Id 
		WHERE TF.Cmp_Id=@cmp_id and  TF.T_ID = @T_ID and TR.Emp_Id=@Emp_ID
		ORDER BY TF.Sorting_No
		--LEFT JOIN V0080_EMP_MASTER_INCREMENT_GET EI EI.EMP_ID=SR.Emp_Id
		----
		SELECT DISTINCT TD.*,c.Cmp_Name,'' AS cmp_logo FROM #TEMPLATE_DETAILS TD
		INNER JOIN T0010_COMPANY_MASTER c WITH (NOLOCK) on c.Cmp_Id= TD.Cmp_Id	
	END
--IF @flag = 2 
--	BEGIN
--			--create table #Table1
...
--		SELECT TF.Cmp_Id,TR.Answer,TR.T_Id,TR.F_Id,EI.Emp_Id, EI.Alpha_Emp_Code ,EI.Emp_Full_Name as EmployeeName,
--		REPLACE(REPLACE(TF.Field_Name,'[',''),']',''),TF.Sorting_No,EI.Branch_Name,EI.Desig_Name,EI.Dept_Name,
--		CONVERT(varchar(15),EI.Created_Date,103)+' - '+CONVERT(varchar(5),EI.Created_Date,108),TM.Template_Title
--		FROM T0050_Template_Field_Master TF	WITH (NOLOCK)	
--		INNER JOIN T0040_Template_Master TM WITH (NOLOCK) ON TM.T_ID=TF.T_ID
--		LEFT JOIN T0100_Employee_Template_Response TR WITH (NOLOCK) ON TR.F_Id = TF.F_ID and  TR.T_Id = TF.T_ID
--		INNER JOIN #FINAL_EMP EI ON EI.Emp_ID = TR.Emp_Id 
