...
CREATE PROCEDURE [dbo].[SP_EMP_SUPERIOR_RECORD_GET]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
	,@Branch_ID		numeric   
...
SET ARITHABORT ON
	...
	if @Constraint <> ''
		...
		from T0080_EMP_MASTER E  WITH (NOLOCK) inner join 		
		  T0010_company_master Cm WITH (NOLOCK) on E.Cmp_ID = Cm.Cmp_ID inner join
			( select I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,Type_ID from T0095_Increment I WITH (NOLOCK) inner join 
				...
				left outer join t0090_emp_reporting_detail erd WITH (NOLOCK) on e.emp_id = erd.emp_id inner join
		...
		WHERE E.Cmp_ID = @Cmp_Id And erd.R_Emp_ID = @R_Emp_ID	
