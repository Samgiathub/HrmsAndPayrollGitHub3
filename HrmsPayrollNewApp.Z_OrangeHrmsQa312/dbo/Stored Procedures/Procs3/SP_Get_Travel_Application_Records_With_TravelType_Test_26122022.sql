...
CREATE PROCEDURE [dbo].[SP_Get_Travel_Application_Records_With_TravelType_Test_26122022]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
...
SET ARITHABORT ON
	...
	(
		...
		inner join (
			...
			inner join T0110_TRAVEL_APPLICATION_DETAIL TAD on TAD.Instruct_Emp_ID=@AppEmp
				where Emp_ID = @AppEmp And Type = 'Travel' and t1.leave LIKE Convert(Varchar,tad.TravelTypeId)+'%'--and CONVERT(varchar,TAD.TravelTypeId) in( t1.Leave) --and Leave in (18,17,16,15)
						...
						WHILE @Rpt_level <= @MaxLevel
							...
							If @Emp_ID_Cur > 0
									Begin
										...
										-- Code UnCommented by Yogesh on 03122022-----------------------------------------------------------------------------------
											--Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_rpt_manager,rpt_level,Max_Leave_Days,Is_RMToRM)
