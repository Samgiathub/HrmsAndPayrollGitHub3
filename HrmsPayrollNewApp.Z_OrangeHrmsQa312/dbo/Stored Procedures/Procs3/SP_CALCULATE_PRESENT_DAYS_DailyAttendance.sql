...
CREATE PROCEDURE [dbo].[SP_CALCULATE_PRESENT_DAYS_DailyAttendance]    
	 @Cmp_ID  numeric    
	,@From_Date datetime    
	,@To_Date  datetime   
	,@Branch_ID numeric    
			...
			ELSE
				BEGIN
					...
					WHERE	Cmp_ID=@Cmp_ID AND ISNULL(Cat_ID,0) = ISNULL(@Cat_ID ,ISNULL(Cat_ID,0))  
							...
							AND EXISTS (select Data from dbo.Split(@PBranch_ID, ',') PB Where cast(PB.data as numeric)=Isnull(V_Emp_Cons.Branch_ID,0))
	...
	IF @Reload_InOut = 1
		BEGIN 		
		  --print 'mansi'--mansi
			...
			EXEC P_GET_EMP_INOUT_dailyAttendance @Cmp_ID, @FROM_DATE, @TO_DATE, @First_In_Last_Out_For_InOut_Calculation
...
--PRINT 'CALC 6 :' + CONVERT(VARCHAR(20), GETDATE(), 114);
	...
	--BEGIN
		 ...
		 -- Update 	D
			...
			UPDATE	D 
			SET		Duration_in_sec = D.Duration_in_sec - (DD.TotalWorkDur - DD.Duration_in_sec)
				--, OT_Sec = DD.TotalWorkDur - DD.Duration_in_sec
 ...
 begin
  ...
  (   
   ...
   BEGIN  
    ...
    if Not Exists(select Tran_Id from dbo.t0160_Ot_Approval where Emp_ID=@T_Emp_ID And For_Date=@T_For_Date)  
     Begin  
    insert into #Data_Temp 
