...
CREATE PROCEDURE [dbo].[Set_Salary_Wages_Register_Amount_With_Late]
 @Cmp_ID		numeric
,@From_Date		datetime
,@To_Date		datetime 
,@Branch_ID		numeric   = 0
...
,@Group_Name	integer = 0  --added jimit 12052016
AS
SET NOCOUNT ON 
...
SET ARITHABORT ON
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
					...
					--Hardik 08/08/2012
					--select @Sal = I.Basic_Salary from dbo.T0095_Increment I inner join 
					--		( select max(Increment_ID) as Increment_ID from dbo.T0095_Increment	-- Ankit 05092014 for Same Date Increment
					--		where Increment_Effective_date <= @To_Date
					...
					--Where Cmp_ID = @Cmp_ID And Emp_Id = @Emp_ID
						...
						--Changed By Jimit 30012019
						select	@Sal = I.Basic_Salary 
						from	dbo.T0095_Increment I WITH (NOLOCK) inner join 
								(
									...
									GROUP BY I2.Emp_ID 
								)Q_I ON Q_I.Emp_ID = I.Emp_ID AND Q_I.Increment_ID = I.Increment_ID
					Where Cmp_ID = @Cmp_ID And Q_I.Emp_Id = @Emp_ID
					--Ended
					...
					select @P_Days = isnull(Present_Days,0) ,--@Sal=Basic_Salary,
						...
						@Basic_Salary = Salary_Amount + Isnull(Basic_Salary_Arear_cutoff,0), --changed By Jimit 29052018 (Golcha)
					...
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Present', @P_Days,'',2,'P')
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Absent', @A_Days,'',3,'P')

					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Late', @Late,'',4,'P')
					...
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Holiday', @Holiday_Days,'',8,'P')					
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Week Off', @WeekOff,'',9,'P')
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'T.Days', @Total_Days,'',10,'P')
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'Arrear Days', @Arear_Days,'',11,'P')

					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year, 'M.W.', 0,'',12,'P')

					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,Rate)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year,'Basic', @Sal,'',13,'I',@Sal)
					
					INSERT INTO dbo.#Temp_Salary_Muster_Report
					(Emp_ID, Cmp_ID, Transaction_ID, Month, Year, Label_Name, Amount, Value_String,Row_id,M_AD_Flage,Rate)
					VALUES     (@Emp_ID, @Cmp_ID, @Transaction_ID, @Month, @Year,'Wages', @Basic_salary,'',14,'I',@Basic_salary)
					...
					SELECT Label_Name ,Row_ID FROM dbo.#TEMP_REPORT_LABEL where Row_ID > 29
					open Cur_label
					...
					Declare @M_AREAR_AMOUNT as Numeric(22,6)	--Alpesh 3-Aug-2012
						...
						select distinct Ad_Sort_Name ,M_Ad_Amount + isnull(@M_AREAR_AMOUNT,0) + isnull(MAD.M_AREAR_AMOUNT_Cutoff,0),  --changed By Jimit 29052018 (Golcha)
				...
				--Select * From #Temp_Salary_Muster_Report Where emp_Id=1 And MONTH=2 And Year=2011
					...
					declare Cur_Dedu   cursor for
						select distinct  t0050_ad_master.AD_ID,Ad_Sort_Name ,IsNULL(M_Ad_Amount,0) + isnull(MAD.M_AREAR_AMOUNT_Cutoff,0)   --Added By Jimit 03102018 (Golcha Cut Off case Arrear amount is not recover in deduction portion (EPF -- JDS00021 for sept - 2018))  
										,t0050_ad_master.AD_Flag,  
						   Case 
		...
		--				VALUES     (@Emp_ID, @Company_ID, @Transaction_ID, @Month, @Year, 'Other Dedu', @Other_Dedu,'',@Row_ID)
						...
						UPDATE    dbo.#Temp_Salary_Muster_Report
						SET              Emp_ID = @Emp_ID, Cmp_ID = @Cmp_ID, Transaction_ID = @Transaction_ID, Month = @Month,
										 Year = @Year, Amount = @Late_Deduction, 
											  Value_String = '',M_AD_Flage='D'
	...
	select dbo.#Temp_Salary_Muster_Report.* ,ISNULL(E.EmpName_Alias_Salary,E.Emp_Full_Name) as Emp_Full_Name,E.Emp_Code,E.Alpha_Emp_Code,GM.Grd_name,ETM.Type_name,DGM.Desig_Name,DM.Dept_Name,BM.Branch_Name, 
		ISnull(E.Dept_ID,0) as Dept_ID,Cmp_Name,Cmp_Address,isnull(Inc_Qry.Inc_Bank_Ac_no,0)Inc_Bank_Ac_no,isnull(Inc_Qry.Payment_Mode,'') as Payment_Mode,
