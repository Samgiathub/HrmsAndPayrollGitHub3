...
CREATE PROCEDURE [DBO].[SP_EMP_SALARY_RECORD_GET1_Test_yogesh_GTPL]
	 @Cmp_ID		NUMERIC
	,@From_Date		DATETIME
	,@To_Date		DATETIME 
	,@Branch_ID		NUMERIC   = 0
...
AS
	...
	SELECT @Emp_Pt_Setting = ISNULL(Setting_Value,0) 
		FROM T0040_SETTING WHERE Cmp_ID = @Cmp_ID AND Setting_Name LIKE 'Show list of employees whose PT settings are pending during Salary Process'
		
	DECLARE @Pending_Leave_Setting AS TINYINT
	SET @Pending_Leave_Setting = 0
	
	SELECT @Pending_Leave_Setting = ISNULL(Setting_Value,0) 
		FROM T0040_SETTING WHERE Cmp_ID = @Cmp_ID AND Setting_Name LIKE 'Leave not Approval Popup in Salary'
		...
		BEGIN
			...
			FROM	#Emp_Cons EC						
					...
					INNER JOIN T0080_EMP_MASTER EM ON EC.Emp_ID=EM.Emp_ID 
							AND (EM.Date_Of_Join > SAL.Sal_End_Date OR ISNULL(EM.Emp_Left_Date, SAL.Sal_End_Date) < SAL.Sal_St_Date )			
		END
	...
	*/
	...
	FROM	(
				...
				FROM	T0080_EMP_MASTER E INNER JOIN #Emp_Cons EC ON EC.Emp_ID = E.Emp_ID
						CROSS JOIN (SELECT DOC_ID FROM T0040_DOCUMENT_MASTER DM WHERE DM.Cmp_ID= @Cmp_ID AND Doc_Required = 1) DM
				WHERE	E.Cmp_ID = @Cmp_ID
			)T 
	WHERE	NOT EXISTS(SELECT 1 FROM T0090_EMP_DOC_DETAIL EDM WHERE T.Emp_ID=EDM.Emp_ID AND T.Doc_ID=EDM.Doc_ID AND EDM.Cmp_ID = @Cmp_ID )
 ...
 --Added by nilesh patel on 20042017 
  IF Object_ID('tempdb..#Emp_Salary_On_Hold') is not null
	  ...
	  From #Emp_Cons EC Inner Join T0090_Change_Request_Approval CRA
  ...
  --Added by nilesh patel on 20042017 
	...
	(
		...
		PMonth_Status Varchar(100),
		Color_flag NUMERIC(1,0),
		IT_M_Amount  NUMERIC(18,2),
				...
				BEGIN
					...
					SELECT Sal_tran_ID, e.Emp_Id, Pan_No, BM.Comp_Name, CM.Cmp_Name, Cm.Cmp_Address, BM.Branch_address
						...
						FROM T0080_EMP_MASTER E 
							...
							INNER JOIN T0030_BRANCH_MASTER BM ON I_Q.BRANCH_ID = BM.BRANCH_ID LEFT OUTER JOIN
							(select Emp_ID,Salary_Status from T0200_MONTHLY_SALARY where Month_St_Date <= DateAdd(m,-1,@From_Date) and Month_End_Date >= DateAdd(m,-1,@To_Date))MSP ON
							...
							LEFT OUTER JOIN (SELECT MPI.EMP_ID, P_DAYS, Over_Time 
												FROM T0190_MONTHLY_PRESENT_IMPORT MPI 
							...
							LEFT OUTER JOIN (SELECT Adv_Closing AS M_ADV_AMOUNT,ADVT.Emp_ID  FROM T0140_Advance_Transaction ADVT INNER JOIN 
													( SELECT MAX(For_Date) AS For_Date , Emp_ID FROM T0140_Advance_Transaction 
							...
							T0011_Login LO  ON LO.Emp_Id = E.Emp_Id	
							Left OUTER JOIN 
							(
							...
							LEFT OUTER JOIN T0200_Emp_ExitApplication EA ON EA.emp_id=E.Emp_ID and EA.[status]<>'R'  --Mukti(19122019)
			...
			ELSE
				BEGIN
					...
					SELECT Sal_tran_ID, e.Emp_Id, Pan_No, BM.Comp_Name, CM.Cmp_Name, Cm.Cmp_Address, BM.Branch_address
							...
							--,CASE WHEN NOT Sal_Tran_ID IS NULL THEN 'Done' ELSE '' END Status
							,SG.Salary_Status AS Status
							,P_DAYS,I_Q.Branch_ID, I_Q.Vertical_ID,I_Q.SubVertical_ID,I_Q.Dept_ID,MSP.Salary_Status as PMonth_Status -- add by Gadriwala 16102013 E.Vertical_ID,E.SubVertical_ID
							,(case when MSP.Salary_Status = 'Hold' and Pan_No = '' then 3 when MSP.Salary_Status = 'Hold' then 2  when Pan_No = '' OR Isnull(Doc.Emp_ID,0) <> 0 then 1 else 0 end) as Color_flag,0,0,0
							,E.Emp_Left--Ankit 27062016
							,I_Q.Emp_Fix_Salary,case when EA.exit_id > 0 then 1 else 0 end  --Jaina 07-09-2016
						FROM T0080_EMP_MASTER E 
		...
		BEGIN
			...
			IF @Show_Left_Employee_for_Salary = 0
				BEGIN
					...
					FROM	T0080_EMP_MASTER E 
							INNER JOIN #Emp_Cons EC ON E.Emp_ID=EC.Emp_ID
							INNER JOIN T0095_Increment I_Q ON EC.Increment_ID=I_Q.Increment_ID
							INNER JOIN T0010_COMPANY_MASTER CM ON E.CMP_ID = CM.CMP_ID 
							LEFT OUTER JOIN ##Att_Muster1 am1 ON am1.emp_id = E.Emp_ID 
							--LEFT OUTER JOIN (SELECT I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,TYPE_ID,Vertical_ID,SubVertical_ID,I.Emp_Fix_Salary  
							--				FROM	T0095_Increment I 
							...
							LEFT OUTER JOIN (SELECT Adv_Closing AS M_ADV_AMOUNT,ADVT.Emp_ID  
											 FROM	T0140_Advance_Transaction ADVT 
							...
							INNER JOIN T0011_Login LO  ON LO.Emp_Id = E.Emp_Id
			...
			ELSE
				BEGIN
					...
					SELECT Sal_tran_ID,e.Emp_Id,Pan_No,BM.Comp_Name,CM.Cmp_Name,Cm.Cmp_Address,BM.Branch_address,CAST( E.Alpha_Emp_Code AS VARCHAR) + ' - '+E.Emp_Full_Name AS Emp_Full_Name,M_IT_Tax,
					CASE WHEN M_OT_Hours > 0 THEN
						M_OT_Hours ELSE Over_Time END AS M_OT_Hours
						,Other_Dedu_Amount,M_LOAN_AMOUNT,M_ADV_AMOUNT,Other_Allow_Amount
							--,@Date_Diff	Month_days	, Isnull(isnull(Present_Days,P_DAYS),@Date_Diff)Present_Days ,
							,am1.total_present  	Month_days	, ISNULL(ISNULL(Present_Days,P_DAYS),am1.total_present  )Present_Days ,
							--CASE WHEN NOT Sal_Tran_ID IS NULL THEN
					...
					FROM T0080_EMP_MASTER E LEFT OUTER JOIN 
						...
						( SELECT I.Emp_Id , Grd_ID,Branch_ID,Cat_ID,Desig_ID,Dept_ID,TYPE_ID,Vertical_ID,SubVertical_ID,I.Emp_Fix_Salary  FROM T0095_Increment I INNER JOIN 
							...
							ON E.Emp_ID = I_Q.Emp_ID  INNER JOIN
							   ...
							   (select Emp_ID,Salary_Status from T0200_MONTHLY_SALARY where Month_St_Date <= DateAdd(m,-1,@From_Date) and Month_End_Date >= DateAdd(m,-1,@To_Date))MSP ON
								...
								(SELECT MPI.EMP_ID,P_DAYS,Over_Time FROM 	T0190_MONTHLY_PRESENT_IMPORT MPI  INNER JOIN 
								...
								(SELECT Adv_Closing AS M_ADV_AMOUNT,ADVT.Emp_ID  FROM T0140_Advance_Transaction ADVT INNER JOIN 
													( SELECT MAX(For_Date) AS For_Date , Emp_ID FROM T0140_Advance_Transaction 
														WHERE For_Date <= @To_Date AND Cmp_ID = @Cmp_ID GROUP BY emp_ID  ) Qry ON ADVT.Emp_ID = Qry.Emp_ID AND ADVT.For_Date = Qry.For_Date	
												) I_Q1 ON E.Emp_ID = I_Q1.Emp_ID 
								INNER JOIN T0011_Login LO  ON LO.Emp_Id = E.Emp_Id
			...
			ELSE
				BEGIN				
					...
					SELECT	Sal_tran_ID,e.Emp_Id,Pan_No,BM.Comp_Name,CM.Cmp_Name,Cm.Cmp_Address,BM.Branch_address,CAST( E.Alpha_Emp_Code AS VARCHAR) + ' - '+E.Emp_Full_Name AS Emp_Full_Name,M_IT_Tax,
							...
							AM1.total_present  	Month_days	, ISNULL(ISNULL(Present_Days,P_DAYS),am1.total_present) Present_Days,							
							(Case When Isnull(SH.Emp_ID,0) <> 0 Then 'Hold' Else SG.Salary_Status END) AS Status,
							P_DAYS,I_Q.Branch_ID, I_Q.Vertical_ID,I_Q.SubVertical_ID,I_Q.Dept_ID,MSP.Salary_Status as PMonth_Status,
							(case when MSP.Salary_Status = 'Hold' and Pan_No = '' then 3 when MSP.Salary_Status = 'Hold' then 2  when Pan_No = '' OR Isnull(Doc.Emp_ID,0) <> 0 then 1 else 0 end) as Color_flag,0,0,0,
					...
					FROM	T0080_EMP_MASTER E 
							...
							LEFT OUTER JOIN (SELECT	MPI.EMP_ID,P_DAYS,Over_Time 
											 ...
											 WHERE	CMP_ID = @CMP_ID AND MPI.month = MONTH(@to_date) 
													AND MPI.YEAR = YEAR(@to_date)) Q_MPI ON E.EMP_ID =Q_MPI.EMP_ID  
							LEFT OUTER JOIN (SELECT Adv_Closing AS M_ADV_AMOUNT,ADVT.Emp_ID  
											 FROM	T0140_Advance_Transaction ADVT INNER JOIN 
														( SELECT MAX(For_Date) AS For_Date , Emp_ID FROM T0140_Advance_Transaction 
															WHERE For_Date <= @To_Date AND Cmp_ID = @Cmp_ID GROUP BY emp_ID  ) Qry ON ADVT.Emp_ID = Qry.Emp_ID AND ADVT.For_Date = Qry.For_Date	
													) I_Q1 ON E.Emp_ID = I_Q1.Emp_ID 
							INNER JOIN T0011_Login LO  ON LO.Emp_Id = E.Emp_Id
...
-- Changed By rohit on 27082014
	...
	if exists(select TP.emp_id from T0190_Tax_Planning TP INNER JOIN #Emp_Cons E ON TP.Emp_ID = E.Emp_ID WHERE Cmp_ID=@Cmp_ID AND Tp.for_Date>= @FROM_DATE AND Tp.for_Date<=@To_Date)
		begin
			--SELECT * FROM T0190_Tax_Planning TP INNER JOIN #Emp_Cons E ON TP.Emp_ID = E.Emp_ID WHERE Cmp_ID=@Cmp_ID AND Tp.for_Date>= @FROM_DATE AND Tp.for_Date<=@To_Date
			UPDATE ST
	...
	Select Distinct Alpha_Emp_Code AS Emp_Code, Emp_Full_Name,EM.Emp_ID,B_T.Branch_ID,BM.Branch_Name from T0100_Leave_Application as LA  
		Inner join T0110_LEAVE_APPLICATION_DETAIL as LAD on LA.Leave_Application_ID = LAD.Leave_Application_ID
		Inner join T0080_EMP_MASTER as EM on  LA.Emp_ID = Em.Emp_ID 		
		Inner join ( SELECT I.Emp_Id , Branch_ID FROM T0095_Increment I INNER JOIN 
							( SELECT MAX(Increment_ID) AS Increment_ID , Emp_ID FROM T0095_Increment
							WHERE Increment_Effective_date <= @To_Date
		...
		Inner Join T0030_BRANCH_MASTER BM ON BM.Branch_ID = B_T.Branch_ID		
	...
	BEGIN
		...
		from T0080_EMP_MASTER as EM 
		inner join #Emp_Cons as EC on Em.emp_id =EC.emp_id
			Inner join ( SELECT I.Emp_Id , Branch_ID,Emp_PT FROM T0095_Increment I INNER JOIN 
							( SELECT MAX(Increment_ID) AS Increment_ID , Emp_ID FROM T0095_Increment
							WHERE Increment_Effective_date <= @To_Date
							AND Cmp_ID = @Cmp_ID GROUP BY emp_ID  ) Qry ON
							I.Emp_ID = Qry.Emp_ID AND I.Increment_ID = Qry.Increment_ID	 ) as B_T
			ON B_T.Emp_ID = EM.Emp_ID
			Inner Join T0030_BRANCH_MASTER BM ON BM.Branch_ID = B_T.Branch_ID
			Inner Join T0040_GENERAL_SETTING GS on BM.Branch_ID = GS.Branch_ID
			where GS.IS_PT = 1 and B_T.Emp_PT=0
			and GS.For_Date = (select max(for_date) From T0040_General_Setting where Cmp_ID = @Cmp_ID 
