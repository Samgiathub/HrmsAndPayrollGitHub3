...
CREATE  PROCEDURE [dbo].[SP_CALCULATE_PT_AMOUNT_FOR_EMP_APP]
	@CMP_ID					Int ,
	@Emp_Tran_ID           bigint,
	@Emp_Application_ID    Int,
	@FOR_DATE				DATETIME ,
	@PT_CALCULATED_AMOUNT	Int,
	@PT_AMOUNT				Int OUTPUT ,
...
SET ARITHABORT ON
		...
		BEGIN
			...
			IF @Applicable_PT_Male_Female_State = 1
				BEGIN
					...
					SELECT @Emp_Gender = Gender FROM T0060_EMP_MASTER_APP WITH (NOLOCK) WHERE 
					--Emp_ID = @EMP_ID 
		...
		begin		
			declare curPT cursor for
				select from_limit,to_limit,Amount from dbo.T0040_professional_setting WITH (NOLOCK) where Cmp_ID = @Cmp_ID and isnull(Branch_ID,0) = isnull(@Branch_ID,isnull(Branch_ID,0))
					...
					and For_date = (
					select max(For_Date) from dbo.T0040_professional_setting WITH (NOLOCK) where Cmp_ID = @Cmp_ID and For_Date <=@For_Date And isnull(Branch_ID,0) = isnull(@Branch_ID,isnull(Branch_ID,0)) and month(for_date) <= month(@for_date) And Applicable_PT_Male_Female = @Applicable_PT_Male_Female)
					order by row_id
		end
	...
	Select @PT_Deduction_Month = ISNULL(PT_Deduction_Month,'0'),@PT_Deduction_Type = ISNULL(PT_Deduction_Type,'Monthly') From T0020_STATE_MASTER S WITH (NOLOCK) , t0030_branch_master B WITH (NOLOCK) where S.State_ID = B.State_ID and B.branch_id = @Branch_ID
	...
	-- Added by nilesh patel on 09102014 --End	
		...
		Begin
			...
			--	Begin
			--		Select @Last_PT_Deduction_Date = Isnull(MAX(Month_End_Date),'') 
			--		From T0200_MONTHLY_SALARY Where Cmp_ID = @CMP_ID and Emp_ID = @EMP_ID and Month_St_Date <= @FOR_DATE and PT_Amount <> 0
			--	End
			...
			if @PT_Deduction_Type = 'Quaterly'
			  Begin
				...
				--Select @PT_Amount_Calaculate_Qutertly = Isnull(SUM(Gross_Salary),0) From T0200_MONTHLY_SALARY where Emp_ID = @EMP_ID and Cmp_ID = @CMP_ID  and Month_St_Date >= @For_Date_PT and Month_End_Date <= @To_Date_PT
