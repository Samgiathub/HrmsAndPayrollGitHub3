...
CREATE PROCEDURE [dbo].[SP_Get_File_Application_Records]
	@Cmp_ID		NUMERIC(18,0),
	@Emp_ID		NUMERIC(18,0),
...
CREATE TABLE #File
	(
		...
		,File_Type_Number	   varchar(MAX)		--added by mansi
	)
	...
	IF NOT EXISTS(SELECT 1 FROM T0050_Scheme_Detail SD WITH (NOLOCK) INNER JOIN T0040_Scheme_Master SM WITH (NOLOCK) ON SD.Scheme_Id = SM.Scheme_Id WHERE SM.Scheme_Type = 'File Management')
					...
					--select * from #tbl_Scheme_Leave
						...
						SELECT DISTINCT	
							 ...
							 ,'' as Forward_Employee--added 1 st july
							  ...
							  ,isnull(lad.File_Type_Number,'')as File_Type_Number--added by mansi
		...
		declare @sqlqr as varchar(max)--added by mansi
		IF @Type = 0
			BEGIN
				--print 1
				IF @Emp_ID_Cur > 0
					...
					--print 11
						...
						--max(Rpt_Level),
						--group by Cmp_ID,Emp_ID,File_App_Id,Alpha_Emp_code,Emp_first_name,Emp_Full_Name
						--,S_Emp_Id,Application_Date,File_Number,F_StatusId,F_TypeId,Subject,Description
						--,Process_Date,File_App_Doc,[User ID],Branch_id,Branch_Name,Final_Approver,Rpt_Level
						--,Scheme_ID,Is_Fwd_Leave_Rej,Emp_Remarks,Forward_Emp_Id,Submit_Emp_Id,Status,Tran_ID
