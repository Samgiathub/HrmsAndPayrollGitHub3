...
--GO
--/****** Object:  StoredProcedure [dbo].[SP_EMP_RECORD_GET_ALL_COMPANY]    Script Date: 13/03/2024 15:11:43 ******/
--SET ANSI_NULLS ON
--GO
...
CREATE PROCEDURE [dbo].[SP_EMP_RECORD_GET_ALL_COMPANY_BACKUP_MEHUL_18102024]     
  @Cmp_ID  numeric      
 ,@From_Date  datetime      
 ,@To_Date  datetime       
 ,@Branch_ID  numeric   
   ...
   set @TDS_Flag= @Show_Left_Employee_for_Salary
		...
		if(@TDS_Flag = 1) 
		   ...
		   and Emp_ID = isnull(@Emp_ID ,Emp_ID)   
			  and Increment_Effective_Date <= @To_Date 
			  and 
			  --New  query - 13032024 -start -Deepali 
					  ( (@From_Date  >= join_Date  or  isnull(DateDiff(M,left_date,GETDATE()),0) <=6 )
					--  isnull(DateDiff(M,Emp_Left_Date,GETDATE()),0)<=6 
