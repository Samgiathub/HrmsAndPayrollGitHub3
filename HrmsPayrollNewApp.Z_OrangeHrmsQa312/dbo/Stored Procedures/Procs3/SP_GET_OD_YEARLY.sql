...
CREATE PROCEDURE [dbo].[SP_GET_OD_YEARLY]
	 @Cmp_ID 		numeric
	,@From_Date 	datetime
	,@To_Date 		datetime
	,@Emp_ID 		numeric
...
IF @Emp_ID = 0  
	...
	(
		...
		insert into @Yearly_Leave (Cmp_ID,Emp_ID,Leave_ID)
			...
			t0040_Leave_Master lm WITH (NOLOCK) where lm.cmp_ID = @cmp_ID and isnull(Lm.Default_Short_Name,'')  <> 'COMP'  -- Changed By Gadriwala Muslim 01102014
