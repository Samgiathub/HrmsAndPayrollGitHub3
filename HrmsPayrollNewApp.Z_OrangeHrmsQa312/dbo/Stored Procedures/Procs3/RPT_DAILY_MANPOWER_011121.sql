...
---------------------------------------------
CREATE PROCEDURE [dbo].[RPT_DAILY_MANPOWER_011121]  
	 @CMP_ID	NUMERIC  
	,@FROM_DATE		DATETIME
	,@TO_DATE 		DATETIME
	,@BRANCH_ID		NUMERIC	
...
AS  
	...
	FROM		T0080_EMP_MASTER E	WITH (NOLOCK) INNER JOIN
				...
				CREATE TABLE #Data         
					...
					SELECT DISTINCT la.Emp_ID, CAST(EM.Alpha_Emp_Code  AS varchar(100))  AS EMP_CODE,( em.EMP_FULL_NAME ) AS Emp_Full_Name,DM.Desig_Name,'L'
