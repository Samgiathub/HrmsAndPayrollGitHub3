...
CREATE PROCEDURE [dbo].[SP_CALCULATE_LOAN_PAYMENT_ALLOWANCE]
	@CMP_ID			NUMERIC ,
	@EMP_ID			NUMERIC,
	@From_Date		Datetime,
	@To_Date		DATETIME,
...
---Test Loan
	...
	--Set @str_LoanDetail = REPLACE(@str_LoanDetail,'{,','')
	Set @str_LoanDetail = SUBSTRING(@str_LoanDetail,2,LEN(@str_LoanDetail))
	Set @str_LoanDetail = REPLACE(@str_LoanDetail,'#,','#')

	DECLARE @TEMP_STR VARCHAR(MAX)
	SET @TEMP_STR = @str_LoanDetail;
	
	declare @tempRow varchar(max)
	...
	if @TEMP_STR = '' -- Added by nilesh for other case of payment return it 
		Begin
			return
		End
		

	DECLARE @iCell As INT
	DECLARE @SQL NVARCHAR(MAX)
	
	DECLARE @tempCell varchar(max)
	while @TEMP_STR <> ''
		begin					
			INSERT INTO #tempTable (ID)VALUES(@iRowIndex)
			
			set @tempRow = SUBSTRING(@TEMP_STR, 0, CHARINDEX('#',@TEMP_STR))
			SET @iCell = 1
			while @tempRow <> ''
				BEGIN
					IF @iRowIndex  = 1
						BEGIN					
							SET @SQL = 'ALTER  TABLE #tempTable ADD DATA_' + CAST(@iCell AS VARCHAR(10)) + ' VARCHAR(MAX)'
							PRINT @SQL
							EXEC(@SQL);							
						END

					if (CHARINDEX(',', @tempRow) > 0)			
						set @tempCell = substring(@tempRow,0,charindex(',',@tempRow))					
					ELSE
						set @tempCell = @tempRow
					
					SET @SQL = 'UPDATE #tempTable  SET DATA_' + CAST(@iCell AS VARCHAR(10)) + ' = ''' + @tempCell + ''' WHERE ID=' + CAST(@iRowIndex AS VARCHAR(10))
					EXEC(@SQL);
					
					if (CHARINDEX(',', @tempRow) > 0)			
						SET @tempRow =SUBSTRING(@tempRow, charindex(',',@tempRow)+1, LEN(@tempRow)-charindex(',',@tempRow))
					else
						SET @tempRow = REPLACE(@tempRow, @tempCell, '')
					SET @iCell = @iCell + 1;
				END
				if (CHARINDEX('#', @TEMP_STR) > 0)	
					Begin		
						SET @TEMP_STR =SUBSTRING(@TEMP_STR, charindex('#',@TEMP_STR)+1, LEN(@TEMP_STR)-charindex('#',@TEMP_STR))
					End
				else
					Begin
						SET @TEMP_STR = ''	
	...
	if OBJECT_ID('tempdb..#TempLoanDetail') is not null
		Begin
			drop table #TempLoanDetail
		End

	Create table #TempLoanDetail
	(
		ID NUmeric(18,0),
		Loan_Apr_ID Numeric(18,0),
		Loan_Name Varchar(max),
		Loan_Amount Numeric(18,4),
		Emp_ID Numeric(18,0),
		Loan_Actual_Amoount Numeric(18,4)
	)

	Insert into #TempLoanDetail 
		...
		BEGIN		
			...
			DECLARE curLoan CURSOR FOR
				...
				SELECT LA.loan_id,La.Loan_Apr_ID,
						...
						--Loan_Apr_Amount,
						TL.Loan_Actual_Amoount,Loan_apr_Deduct_From_sal
				...
				FROM	T0120_loan_approval la WITH (NOLOCK)  INNER JOIN T0040_LOAN_MASTER LM WITH (NOLOCK) ON la.Cmp_ID=LM.Cmp_ID AND la.Loan_ID = LM.Loan_ID
						inner join #TempLoanDetail TL on la.Loan_Apr_ID = TL.Loan_Apr_ID and la.Emp_ID = TL.Emp_ID 
				...
				WHERE	la.emp_id = @emp_id and la.Cmp_ID = @Cmp_ID
						...
						and Loan_Apr_Deduct_From_sal = 2  AND IsNull(LM.Is_GPF,0)=0 And ISNULL(LM.Is_Intrest_Amount_As_Perquisite_IT,0) = 0
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					--		and @To_Date and Emp_ID = @emp_id And Loan_Amount = 0 AND Month_End_Date < @To_Date	
					Declare @Loan_Payment_Date Datetime
					Declare @Manual_Salary_Period Numeric(5,0)
					...
					FROM	T0210_MONTHLY_LOAN_PAYMENT LP1 WITH (NOLOCK) Inner Join T0120_LOAN_APPROVAL LA1 WITH (NOLOCK)
					ON LP1.Loan_Apr_ID = LA1.Loan_Apr_ID
					WHERE	LP1.Loan_Payment_Date  BETWEEN (SELECT ISNULL( MAX(Loan_Payment_Date),@From_Date) 
														FROM	T0210_MONTHLY_LOAN_PAYMENT LP WITH (NOLOCK) Inner Join T0120_LOAN_APPROVAL LA WITH (NOLOCK)
														ON LP.LOAN_APR_ID = LA.LOAN_APR_ID
														WHERE  LA.Emp_ID = @emp_id and Isnull(Pay_Tran_ID,0) > 0 ) 
							and  @To_Date and LA1.Emp_ID = @emp_id and Isnull(Pay_Tran_ID,0) > 0 and Loan_Pay_Amount = 0
							--and Month(@Installment_Start_Date) <> Month(Loan_Payment_Date) and year(@Installment_Start_Date) <> year(Loan_Payment_Date)
					OPEN curLoantest		
					FETCH NEXT FROM curLoantest INTO @Loan_Payment_Date
					WHILE @@FETCH_STATUS = 0
						BEGIN		
							...
							where	Increment_Effective_date <= @Loan_Payment_Date and Cmp_ID = @Cmp_ID group by emp_ID) Qry on    
							...
							from	T0040_GENERAL_SETTING WITH (NOLOCK) where cmp_ID = @cmp_ID and Branch_ID = @Branch_ID_Temp    
									and For_Date = ( select max(For_Date) from T0040_GENERAL_SETTING WITH (NOLOCK) where For_Date <=@Loan_Payment_Date and Branch_ID = @Branch_ID_Temp and Cmp_ID = @Cmp_ID)    
							if @Manual_Salary_Period = 1
								Begin
									Select @Temp_Month_St_Date = from_date,@Temp_Month_End_Date = end_date  From Salary_Period SP Where SP.month = month(@Loan_Payment_Date) and SP.year = year(@Loan_Payment_Date)
								End
							Else
								Begin
									Set @Temp_Month_End_Date = @Loan_Payment_Date
									Set @Temp_Month_St_Date = dateadd(m,-1,@Temp_Month_End_Date) + 1
								End 
							...
							if @Is_FNF =1 
								BEGIN
									...
									if @Is_FNF =1 
										BEGIN
											set @Loan_Inst_Amount = @Pending_Loan
											exec P0210_MONTHLY_LOAN_PAYMENT_INSERT 0,@Loan_Apr_ID,@Cmp_Id,@Salary_Tran_ID,@Loan_Inst_Amount,'',@To_Date,'','','','',@Loan_Interest_Amount,@Interest_Percent,@Interest_Subsidy_Amount,0,0,@Pay_Tran_ID
										END
									...
									ELSE																																																																								
										...
										--	SET @Pending_Loan  = @Loan_Apr_Amount - @Return_Amount
											...
											IF @Is_FNF =1 
												...
												END
											ELSE IF (@Loan_Inst_Amount + @Loan_Interest_Amount) >= 0 AND @Loan_apr_Deduct_From_sal = 2
												BEGIN	
													...
													IF @PAYMENT_TYPE ='Cash'
														BEGIN
															...
															EXEC P0210_MONTHLY_LOAN_PAYMENT_INSERT 0,@Loan_Apr_ID,@Cmp_Id,@Salary_Tran_ID,@Return_Amount,'',@To_Date,'',@PAYMENT_TYPE,'','',@Loan_Interest_Amount,@Interest_Percent,@Interest_Subsidy_Amount,0,0,@Pay_Tran_ID
														END
													ELSE 
														BEGIN
															EXEC P0210_MONTHLY_LOAN_PAYMENT_INSERT 0,@Loan_Apr_ID,@Cmp_Id,@Salary_Tran_ID,@Loan_Inst_Amount,'',@To_Date,'','','','',@Loan_Interest_Amount,@Interest_Percent,@Interest_Subsidy_Amount,0,0,@Pay_Tran_ID	
