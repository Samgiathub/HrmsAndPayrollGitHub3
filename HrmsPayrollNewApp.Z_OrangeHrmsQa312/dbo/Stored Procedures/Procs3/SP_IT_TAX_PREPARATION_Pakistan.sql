...
CREATE PROCEDURE [dbo].[SP_IT_TAX_PREPARATION_Pakistan]
	 @Cmp_ID				NUMERIC
	,@From_Date				DATETIME
	,@To_Date				DATETIME
	,@Branch_ID				NUMERIC 
...
SET ARITHABORT ON
	...
	SELECT '3. Tax on Total Income ',101
--	insert into #Tax_Report_Male (Field_Name,Default_def_ID)
...
--	select 'Total Tax Liabilities',103
	...
	--INSERT INTO #Tax_Report_Male (Field_Name,Default_def_ID,T_F_Row_ID,T_T_Row_ID)
	--SELECT '5. Tax Payable',103,@Max_From_Row_ID,@Max_Row_ID-1
	...
	INSERT INTO #Tax_Report_Male (Field_Name,Default_def_ID,T_F_Row_ID,T_T_Row_ID)
	SELECT '5. Less: TDS Paid',107,@Max_From_Row_ID,@Max_Row_ID-1

	INSERT INTO #Tax_Report_Male (Field_Name,Default_def_ID)
	SELECT '6. TAX PAYABLE/REFUNDABLE',108
...
------------------ Allowance Exemption ---------------
	...
	FROM #Tax_Report Tr INNER JOIN (  SELECT MS.EMP_ID ,SUM(MS.S_Salary_Amount) S_Salary_Amount FROM 
											...
											AND Ms.S_Month_End_Date <= @Month_En_Date
										GROUP BY MS.EMP_ID ) Q ON TR.EMP_ID =Q.EMP_ID
 ...
 --                            Begin 
		...
		BEGIN
			...
			IF @Incentive_Amount > 0 
				BEGIN					
					...
					EXEC dbo.SP_IT_TAX_CALCULATION_PAKISTAN @Cmp_ID,@Emp_ID,@To_Date,@TAXABLE_AMOUNT_Inc ,@Return_Tax_Amount_Inc OUTPUT
							,@Surcharge_amount_Inc OUTPUT ,@ED_Cess_Inc OUTPUT ,@ED_Cess_Per ,@SurCharge_Per
			...
			EXECUTE dbo.SP_IT_TAX_CALCULATION_PAKISTAN @Cmp_ID,@Emp_ID,@To_Date,@TAXABLE_AMOUNT ,@Return_Tax_Amount OUTPUT
						,@Surcharge_amount OUTPUT ,@ED_Cess OUTPUT ,@ED_Cess_Per ,@SurCharge_Per
			...
			SELECT @M_AD_Amount = ISNULL(SUM(M_AD_Amount),0)  FROM T0210_Monthly_AD_Detail mad WITH (NOLOCK) INNER JOIN
					T0050_AD_MAster am WITH (NOLOCK) ON mad.AD_ID= am.AD_ID AND mad.cmp_id = am.cmp_id AND AD_DEF_ID = 1
	...
	IF @Sp_Call_For='Tax Planning'
		BEGIN
			...
			SELECT  tr.*,em.Emp_Full_Name,em.emp_code,em.Alpha_Emp_Code
	...
	ELSE
		BEGIN
			SELECT  Row_ID,SPACE(Concate_Space)+ FIELD_NAME AS  FIELD_NAME,Amount_Col_Final,Amount_Col_1,Amount_Col_2,Amount_Col_3,Amount_Col_4,Default_def_ID,AD_ID,IT_ID 
					,tr.Emp_ID,em.Emp_Code,em.Alpha_Emp_Code,em.Emp_Full_Name,DM.Desig_Name,em.Date_Of_Join,em.Pan_No,@From_Date P_From_Date ,@To_Date P_To_Date
