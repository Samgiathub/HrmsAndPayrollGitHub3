...
Create PROCEDURE [dbo].[prc_CheckClaimLimit_backup_tejas(08-03-2024)]
@rClaimId INT,
@rAmount float,
@rEmpId BIGINT,
@rRowId INT = null,
...
BEGIN
	...
	begin
		select @Limit = City_cat_limit,@lRatePerKM = 0,@lflagRateOrUnit = 2,@lMinKm = 0 from T0041_Claim_Maxlimit_GradeDesig_CityWise where Claim_Id = @rClaimId  and Grd_ID = @lGradeId 
	end
	...
	Begin
		...
		@lAfter_Joining_Days as AfterJoiningDays,@lAllowFlag as AllowFlag,@lflagRateOrUnit as UnitFlag,@lUnitName as UnitName,isnull(@lConversionRate,0) as ConversionRate,isnull(@lMinKm,0) as MinKm,isnull(YearBy,0) as YearBy,isnull(Application_Limitwise,0) as Application_Limitwise
