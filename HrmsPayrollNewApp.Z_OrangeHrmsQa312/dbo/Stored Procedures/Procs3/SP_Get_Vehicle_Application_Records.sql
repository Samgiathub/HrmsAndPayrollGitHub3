CREATE PROCEDURE [dbo].[SP_Get_Vehicle_Application_Records]
	@Cmp_ID		Numeric(18,0),
	@Emp_ID		Numeric(18,0),
...
BEGIN
	...
	--set @MaxLevel =5
	SELECT @MaxLevel = ISNULL(MAX(Rpt_Level),1) FROM T0050_Scheme_Detail SD INNER JOIN T0040_Scheme_Master SM ON SD.Scheme_Id = SM.Scheme_Id WHERE SM.Scheme_Type = 'Own Vehicle'
	...
	(
		...
		,Vehicle_App_ID			numeric(18,0)		
		,Branch_Name			nvarchar(100)
		,Desig_Name				nvarchar(100)
		,Alpha_Emp_code			nvarchar(100)
		,Vehicle_App_Date			datetime
		,Vehicle_App_Status		varchar(20)		
		,Vehicle_approval_id		numeric(18,0)
		,Rpt_Level				numeric(18,0)
		,Scheme_ID				numeric(18,0)
		,Final_Approver			TinyInt
		,Is_Fwd_Leave_Rej		TinyInt	
		,Is_Intimation		TINYINT
		,Vehicle_Name		varchar(150)
		,Vehicle_Option		varchar(100)
		)
				
		--IF SCHEME ARE NOT IN MASTER THEN RETURN	
		IF NOT EXISTS(SELECT 1 FROM T0050_Scheme_Detail SD INNER JOIN T0040_Scheme_Master SM ON SD.Scheme_Id = SM.Scheme_Id WHERE SM.Scheme_Type = 'Own Vehicle')
			BEGIN
				IF @Type = 0
					BEGIN
						SELECT * FROM #Vehicle
					...
					BEGIN
						IF OBJECT_ID('tempdb..#Notification_Value') IS NOT NULL
							BEGIN
								...
								SELECT COUNT(1) AS VehicleAppCnt from #Vehicle
							END
						ELSE
							SELECT COUNT(1) AS VehicleAppCnt from #Vehicle
					END	
		...
		WHILE @@FETCH_STATUS = 0
					...
					Begin
						...
						if exists (SELECT 1 from T0095_MANAGERS where Emp_id = @Emp_ID_Cur)
		 				...
		 				WHILE @Rpt_level <= @MaxLevel
									 ...
									 If @Emp_ID_Cur > 0
										Begin
											...
											Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_rpt_manager,rpt_level,Max_Leave_Days,Is_RMToRM,Is_Intimation)
												...
												Where App_Emp_Id = @Emp_ID and rpt_level = @Rpt_level	And T0040_Scheme_Master.Scheme_Type = 'Own Vehicle'
												...
												BEGIN
													...
													SET @string_1 = 'Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_rpt_manager,rpt_level,Max_Leave_Days,Is_RMToRM,Is_Intimation)
																	...
																	Where  rpt_level = '+ CAST(@Rpt_level AS VARCHAR(2)) +' and Is_RM = 1 
																		And T0040_Scheme_Master.Scheme_Type = ''Own Vehicle'' and T0040_Scheme_Master.Cmp_Id In  ('+ @Emp_Cmp_Id +')'
										...
										If @Manager_Branch > 0 
											Begin
												Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,is_branch_manager,rpt_level,Max_Leave_Days,Is_RMToRM,Is_Intimation)
													...
													From T0050_Scheme_Detail 
													Inner Join T0040_Scheme_Master ON T0050_Scheme_Detail.Scheme_Id = T0040_Scheme_Master.Scheme_Id
													Where rpt_level = @Rpt_level and Is_BM = 1 And T0040_Scheme_Master.Scheme_Type = 'Own Vehicle'
											End
									end
								 Else
									Begin
											Insert Into #tbl_Scheme_Leave (Scheme_ID, Leave,Is_Fwd_Leave_Rej,rpt_level,Max_Leave_Days,Is_RMToRM,Is_Intimation)
												...
												From T0050_Scheme_Detail 
												Inner Join T0040_Scheme_Master ON T0050_Scheme_Detail.Scheme_Id = T0040_Scheme_Master.Scheme_Id
												Where T0040_Scheme_Master.Scheme_Type = 'Own Vehicle'
									End
										   ...
										   Emp_ID numeric    
												...
												If @is_branch_manager = 1
									 					...
									 					Insert Into #Emp_Cons(Emp_ID)    
															...
															From T0095_EMP_SCHEME ES Inner Join
																(Select MAX(Effective_Date) as For_Date, Emp_ID from T0095_EMP_SCHEME
																 Where Effective_Date<=GETDATE() And Type='Own Vehicle'
																 GROUP BY emp_ID) Qry on      
																 ES.Emp_ID = Qry.Emp_ID and ES.Effective_Date = Qry.For_Date and Scheme_Id = @Scheme_ID And Type='Own Vehicle'
															INNER JOIN 
													...
													If @Rpt_level = 1
															Begin
																Set @SqlQuery = 	
																'Select LAD.Vehicle_App_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' From V0100_VEHICLE_APPLICATION LAD
																	...
																	Where (VEHICLE_ID in (select data from dbo.split(T1.leave,''#'')) or (T1.Leave=''0''))
																	AND LAD.Vehicle_App_ID Not In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL 
															...
															Begin
																...
																'Select LAD.Vehicle_App_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	...
																	AND (LAD.Vehicle_App_ID Not In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL 
																				...
																				And LAD.Vehicle_App_ID In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL 
												...
												Else if @is_rpt_manager = 1
													...
													Select ERD.Emp_ID From T0090_EMP_REPORTING_DETAIL ERD INNER JOIN 
														...
														INNER JOIN
														(select MAX(Effective_Date) as For_Date, Emp_ID from T0095_EMP_SCHEME
															 where Effective_Date<=GETDATE() And Type = 'Own Vehicle'--and Scheme_Id = @Scheme_ID -- max date issue on 12092013 - mitesh															
															 GROUP BY emp_ID) Qry on  ES.Emp_ID = Qry.Emp_ID and ES.Effective_Date = Qry.For_Date      and Scheme_Id = @Scheme_ID And Type = 'Own Vehicle'
														...
														If @Rpt_level = 1
															Begin														
																...
																'Select LAD.Vehicle_App_ID, ' + Cast(@Scheme_ID As Varchar(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   cast(@Rpt_level as VARCHAR(2)) +
																	 ' From V0100_VEHICLE_APPLICATION LAD
																		Inner Join #Emp_Cons Ec on LAD.Emp_Id = Ec.Emp_ID
																		INNER JOIN T0050_Scheme_Detail T1 WITH (NOLOCK) ON  is_RM = 1 '
																														+ ' And rpt_level = ' + cast(@Rpt_level as varchar(2)) +
																														+ ' And Scheme_ID = ' + cast(@Scheme_ID as varchar(3)) +'
																	Where (VEHICLE_ID in (select data from dbo.split(T1.leave,''#'')) or (T1.Leave=''0''))
																	AND LAD.Vehicle_App_ID Not In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL 
																		...
																		IF Object_ID('tempdb..#EMP_CONS_RM') IS NOT NULL
																			DROP TABLE #EMP_CONS_RM
																				...
																				EXEC SP_RPT_FILL_EMP_CONS_WITH_REPORTING	@Cmp_ID=@Cmp_ID,@From_Date=@date,@To_Date=@date,@Branch_ID=0,
																															...
																															@Scheme_Id = @Scheme_ID ,@Rpt_Level = 2 ,@SCHEME_TYPE = 'Own Vehicle' 										
																				
																			
																					SET @SqlQuery =	   'Select  Vehicle_App_ID, ' + CAST(@Scheme_ID AS VARCHAR(3)) + ' As Scheme_ID, ''' + @Leave + ''' As Leave , '  +   CAST(@Rpt_level AS VARCHAR(2)) + '
																									FROM	(SELECT LAD.Vehicle_App_ID,LAD.App_Status,Vehicle_App_Date,LAd.Alpha_Emp_Code,Emp_First_Name
																											From	V0100_VEHICLE_APPLICATION LAD 
																													INNER JOIN #EMP_CONS_RM Ec on LAD.Emp_Id = Ec.Emp_ID  
																													LEFT OUTER JOIN (SELECT Vehicle_App_ID,Emp_ID,S_Emp_ID,Approval_Status As App_Status FROM T0115_VEHICLE_APPROVAL_LEVEL LA WHERE S_Emp_ID = ' + CAST(@Emp_ID_Cur AS VARCHAR(10)) + ') LA 
																																		ON LAD.Vehicle_App_ID=LA.Vehicle_App_ID And LAD.EMP_ID=LA.EMP_ID
																													INNER JOIN T0050_Scheme_Detail T1 WITH (NOLOCK) ON  T1.rpt_level = ' + cast(@Rpt_level as varchar(2)) +
																														+ ' And T1.Scheme_ID = ' + cast(@Scheme_ID as varchar(3)) +'
																											Where (VEHICLE_ID in (select data from dbo.split(T1.leave,''#'')) or (T1.Leave=''0''))	
																											AND	 (LAD.Vehicle_App_ID Not In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL Where Rpt_Level = EC.Rpt_Level) ' +  --' + CAST(@Rpt_level AS VARCHAR(2)) + ')
																															'And LAD.Vehicle_App_ID In (Select Vehicle_App_ID From T0115_VEHICLE_APPROVAL_LEVEL Where  Rpt_Level = EC.Rpt_Level - 1) ' +-- and Ec.R_Emp_Id = S_Emp_Id) ' + --+ CAST(@Rpt_level_Minus_1 AS VARCHAR(2)) + ')
																														')																										
																											) T
							...
							End
					End
					--SELECT * FROM [V0100_VEHICLE_APPLICATION]
			...
			If @Emp_ID_Cur > 0
					Begin			
						Insert INTO #Vehicle
							Select distinct	
								LAD.Emp_ID, LAD.Emp_Full_Name, LAD.Supervisor,LAD.Emp_Superior,LAD.Vehicle_App_ID, LAD.Branch_Name
								,LAD.Desig_Name, LAD.Alpha_Emp_code, LAD.Vehicle_App_Date ,LAD.App_Status
								,LAD.Vehicle_Apr_ID,isnull(Qry1.rpt_level + 1,'1') As Rpt_Level, TLAP.Scheme_ID, SL.Final_Approver,
								SL.Is_Fwd_Leave_Rej,SL.Is_Intimation,Vehicle_Type,lad.Vehicle_Option
							From V0100_VEHICLE_APPLICATION LAD
								left outer join (select lla.Vehicle_App_ID As App_ID, Rpt_Level as Rpt_Level , lla.Vehicle_Appr_Status,Vehicle_Option From T0115_VEHICLE_APPROVAL_LEVEL lla WITH (NOLOCK)
													inner join (Select max(rpt_level) as rpt_level1, Vehicle_App_ID
																	From T0115_VEHICLE_APPROVAL_LEVEL WITH (NOLOCK)
																	Where Vehicle_App_ID In (Select Leave_App_ID From #tbl_Leave_App)
																	group by Vehicle_App_ID 
																) Qry
													on qry.Vehicle_App_ID = lla.Vehicle_App_ID and qry.rpt_level1 = lla.rpt_level													
												) As Qry1 
								On  LAD.Vehicle_App_ID = Qry1.App_ID
								Inner join #tbl_Leave_App TLAP On TLAP.Leave_App_ID = LAD.Vehicle_App_ID
				...
				Else
					Begin
						Insert INTO #Vehicle
							Select distinct	
								LAD.Emp_ID, LAD.Emp_Full_Name, LAD.Supervisor,LAD.Vehicle_App_ID,LAD.Branch_Name
								,LAD.Desig_Name, LAD.Alpha_Emp_code, LAD.Vehicle_App_Date ,LAD.App_Status 
								,LAD.Vehicle_Apr_ID,isnull(Qry1.rpt_level + 1,'1') As Rpt_Level,'0' as Scheme_ID, '1' as Final_Approver, '0' as Is_Fwd_Leave_Rej,0 AS Is_Intimation,Vehicle_Type,
								LAD.Vehicle_Option
							From V0100_VEHICLE_APPLICATION LAD
									left outer join (select lla.Vehicle_App_ID As App_ID, Rpt_Level  as Rpt_Level,lla.Vehicle_Appr_Status From T0115_VEHICLE_APPROVAL_LEVEL lla WITH (NOLOCK)
														inner join (Select max(rpt_level) as rpt_level1, Vehicle_App_ID
																		From T0115_VEHICLE_APPROVAL_LEVEL WITH (NOLOCK)
																		group by Vehicle_App_ID 
																	) Qry
		...
		If @Type = 0
			Begin
				If @Emp_ID_Cur > 0
					Begin
						set @queryExe ='select *,0 AS TRAN_ID,Vehicletbl.Vehicle_app_status as Application_Status from #Vehicle AS Vehicletbl ' + @OrderBy --order by #Vehicle.Vehicle_App_Date desc						
						exec (@queryExe)
					End
				Else
					Begin					
						set @queryExe='';						
						set @queryExe = 'select *,0 as TRAN_ID,Vehicletbl.Vehicle_app_status as Application_Status from #Vehicle as Vehicletbl where ' + @Constrains + ' ' + @OrderBy -- order by #Vehicle.Vehicle_App_Date desc 
						exec (@queryExe)
					End
			End
		Else if @Type = 1
