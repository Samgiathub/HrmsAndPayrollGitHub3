...
CREATE PROCEDURE [dbo].[SP_HRMS_Get_Interview_Process_Summary]  
 @Cmp_ID  numeric(18,0) , 
 @rec_post_id numeric(18,0),  
 @Process_data varchar(100),
 @From_Date Datetime = null, -- Added by rohit for Filter data on 27042016
...
-- added by rohit on 22042016
if isnull(@From_Date,'')=''
	set @From_Date = DATEADD(yy, DATEDIFF(yy,0,getdate()), 0)

if isnull(@To_Date,'')=''
	set @To_Date = DATEADD(yy, DATEDIFF(yy,0,getdate()) + 1, -1)
	...
	left outer join  v0055_HRMS_Interview_Schedule R on IS1.Resume_ID =R.Resume_ID and IS1.rec_post_id =R.rec_post_id and IS1.Dis_no =R.Dis_no 
	where isnull(rm.Resume_ID,0)<>0 and isnull(IS1.rec_post_id,0)<>0
	and rm.Resume_Posted_date >=@From_Date and rm.Resume_Posted_date <=@To_date and Rm.Cmp_id=@Cmp_ID
  ...
  --select @Process_data
	...
	if @Process_data <> ''	
	    select *, isnull(Q.resume_status,0) as resume_status,case When isnull(RM.lock,0)=0 then 'No' else 'Yes' end as Lock_Status,HRR.Vertical_Id as Re_Vertical_Id,VS.Vertical_Name,Hrr.SubVertical_Id as Re_SubVertical_Id,HRR.Branch_id as Re_Branch_Id
	    ,case when level2_Approval = 1  then 'Corporate HR Confirmed' when level2_Approval = 2  then 'Corporate HR Rejected' when level2_Approval = 4 then 'On Review by Corporate HR' when level2_Approval = 5  then 'Corporate HR Pending Approval'
	    when q.resume_status = 1 then 'Approved by Sub-vertical Head' when  q.resume_status = 2 then 'Rejected by Sub-vertical Head' when q.Resume_Status=3 then 'Sub-vertical Head Approval Pending' else next_status end as process_schedule 
	     from @data QA
		...
		left join T0052_HRMS_Posted_Recruitment HPR WITH (NOLOCK) on qa.rec_post_id = HPR.Rec_Post_Id
		left join T0050_HRMS_Recruitment_Request HRR WITH (NOLOCK) on HPR.Rec_Req_ID = HRR.Rec_Req_ID
		left join T0040_Vertical_Segment VS WITH (NOLOCK) on VS.Vertical_ID=HRR.Vertical_Id --Added by sumit for searching with resume code and vertical wise
		where Process like + '%' + @Process_data + '%' or next_process like + '%' + @Process_data + '%' or QA.Job_title like + '%' + @Process_data + '%' or QA.rec_post_code like + '%' + @Process_data + '%'
		or app_full_name like + '%' + @Process_data + '%' or QA.resume_code like + '%' + @Process_data + '%' or status like + '%' + @Process_data + '%'
		or VS.Vertical_Name like + '%' + @Process_data + '%' 
		or case when level2_Approval = 1  then 'Corporate HR Confirmed' when level2_Approval = 2  then 'Corporate HR Rejected' when level2_Approval = 4 then 'On Review by Corporate HR' when level2_Approval = 5  then 'Corporate HR Pending Approval'
	    when q.resume_status = 1 then 'Approved by Sub-vertical Head' when  q.resume_status = 2 then 'Rejected by Sub-vertical Head' when q.Resume_Status=3 then 'Sub-vertical Head Approval Pending' else next_status end  like + '%' + @Process_data + '%'
		order by QA.rec_post_code,QA.resume_code asc
	else
		 select *,isnull(Q.resume_status,0)as resume_status,case When isnull(RM.lock,0)=0 then 'No' else 'Yes' end as Lock_Status,HRR.Vertical_Id as Re_Vertical_Id,Hrr.SubVertical_Id as Re_SubVertical_Id,HRR.Branch_id as Re_Branch_Id 
	    ,case when level2_Approval = 1  then 'Corporate HR Confirmed' when level2_Approval = 2  then 'Corporate HR Rejected' when level2_Approval = 4 then 'On Review by Corporate HR' when level2_Approval = 5  then 'Corporate HR Pending Approval'
	    when q.resume_status = 1 then 'Approved by Sub-vertical Head' when  q.resume_status = 2 then 'Rejected by Sub-vertical Head' when q.Resume_Status=3 then 'Sub-vertical Head Approval Pending' else next_status end as process_schedule 
	    from @data QA
		left outer join T0060_RESUME_FINAL Q WITH (NOLOCK)	on QA.resume_id=Q.resume_id 
