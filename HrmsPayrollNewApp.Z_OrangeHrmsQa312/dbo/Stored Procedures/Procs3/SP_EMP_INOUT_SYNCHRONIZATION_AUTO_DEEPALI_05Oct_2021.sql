...
CREATE PROCEDURE [dbo].[SP_EMP_INOUT_SYNCHRONIZATION_AUTO_DEEPALI_05Oct_2021]
	@CMP_ID NUMERIC 
As
...
SET ANSI_WARNINGS OFF;
	...
	--/*Added by Deepali 06102021 start*/

	--	DECLARE Emp_IO_Status_cursor CURSOR FOR 
	...
	--OPEEmp_IO_Status_cursor CURSOR FOR 

	--FETCH NEXT FROM Emp_InOut_cursor INTO @Emp_ID, @Cmp_ID,@IO_DateTime,@IP_Address,@In_Out_flag,@Enroll_No
	--WHILE @@FETCH_STATUS = 0
	--	BEGIN
	--		SET @IO_DATETIME = CAST(CAST(@IO_DATETIME AS VARCHAR(11)) + ' ' + dbo.F_GET_AMPM(@IO_DATETIME) AS DATETIME)
	--				if Isnull(@Prev_Cmp_Id,0) <> @CMP_ID
	...
	--			update	T0150_EMP_INOUT_RECORD set StatusFlag='D'   
	--				where For_Date = @IO_DATETIME and Cmp_ID =@Cmp_ID and Emp_ID =@Emp_ID  and  In_Time=
