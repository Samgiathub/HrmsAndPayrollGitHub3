CREATE PROCEDURE [dbo].[SP_GET_TRAVEL_TYPE]    
@CMP_ID NUMERIC    
,@EMP_ID NUMERIC    
...
SET ARITHABORT ON    
 ...
 --   ) Qry on es.Emp_ID = Qry.Emp_ID AND es.Effective_Date = Qry.EffDate     
    inner join T0040_Travel_Type TT on TT.Travel_Type_Id = sd.Traveltype    
 where sd.Scheme_Id = @SCHEME_ID and es.Emp_ID = @EMP_ID AND es.Cmp_Id = @CMP_ID  
  
--and Travel_Type_Name=@TTypeName   --Commented by Yogesh on 05-01-2024 to resovle bug #27414
