...
CREATE PROCEDURE [dbo].[SP_CALCULATE_ALLOWANCE_DEDUCTION]
	  @Sal_Tran_ID   NUMERIC, 
	  @Emp_Id     NUMERIC ,                    
	  @Cmp_ID     NUMERIC ,                    
					...
					*/
					...
					IF @varCalc_On = 'Transfer OT'                     
					  BEGIN 
					   ...
					   --    --Fix OT Rate--
					  END      
					  IF @varCalc_On = 'Weekday OT'                     
					  BEGIN 
						IF @After_Salary = 0
							BEGIN	
								---Added by Hardik 07/03/2019 for Cliantha, To Set Fix Hourly OT Rate Calculation
								DECLARE @OT_Amount_WD numeric(18,2)
								--DECLARE @OT_WO_AMOUNT_SALARY numeric(18,2)
								--DECLARE @OT_HO_AMOUNT_SALARY numeric(18,2)
								
								SET @OT_Amount_WD = ISNULL(@OT_Amount,0)
								--SET @OT_WO_AMOUNT_SALARY = ISNULL(@OT_WO_AMOUNT,0)
								--SET @OT_HO_AMOUNT_SALARY = ISNULL(@OT_HO_AMOUNT,0)
							
								
								DECLARE @GRADEWISE_SALARY_ISENABLED_WD	TINYINT
								SET @GRADEWISE_SALARY_ISENABLED_WD = 0
								
								SELECT @GRADEWISE_SALARY_ISENABLED_WD = ISNULL(SETTING_VALUE,0) FROM T0040_SETTING WITH (NOLOCK) WHERE CMP_ID = @CMP_ID AND SETTING_NAME = 'SHOW GRADEWISE SALARY TEXTBOX IN GRADE MASTER'
								
								DECLARE @T_amt_WD NUMERIC
								--DECLARE @T_amt_WO NUMERIC
								--DECLARE @T_amt_HO NUMERIC
								DECLARE @temp_t_amt_WD NUMERIC
								--DECLARE @Ot_temp_Amount_WO NUMERIC(18, 4)
								--DECLARE @Ot_temp_Amount_HO NUMERIC(18, 4)
								DECLARE @OT_Max_Limit_Sec_WD as Numeric	
								DECLARE @Emp_WO_HO_OT_Sec_WD as Numeric
								SET @OT_Max_Limit_Sec_WD = dbo.F_Return_Sec(replace(cast(@OT_Max_Limit as varchar(20)),'.',':')) 
								...
								SELECT @temp_t_amt_WD = ISNULL(SUM(E_AD_amount),0)
											FROM (
											SELECT 
						...
						---------------------------------End ---------------------------------------------------------------------------------								
								...
								If @Shift_Wise_OT_Rate = 1 And Isnull(@Emp_WD_OT_Rate,0) = 9 And Isnull(@Emp_WO_OT_Rate,0) = 9 And Isnull(@Emp_HO_OT_Rate,0) = 9  -- FOR SHOFT SHIP YARD, ADDED BY HARDIK 22/11/2018
									BEGIN
										IF Cast(@Hour_Salary_OT As Numeric(18,2)) <> CAST(((@OT_Basic_Salary+ISNULL(@temp_t_amt,0)) /@OT_Working_Day) /(@Shift_Day_Sec/3600) As Numeric(18,2))
											BEGIN
												...
												SELECT 
													--@OT_HO_AMOUNT = Isnull(Case When @IS_ROUNDING = 1 Then Round(Sum(HO_OT_Amount),0) Else Sum(HO_OT_Amount) End,0),
													--@OT_WO_AMOUNT = Isnull(Case When @IS_ROUNDING = 1 Then Round(Sum(WO_OT_Amount),0) Else Sum(WO_OT_Amount) End,0),
													@OT_Amount = Isnull(Case When @IS_ROUNDING = 1 Then Round(Sum(WD_OT_Amount),0) Else Sum(WD_OT_Amount) End,0)
												FROM #ShiftWiseOT
									...
									END
										...
										IF @OT_Working_Day <> 0 AND @Shift_Day_Sec <> 0 and @Wages_type ='Monthly' And @Shift_Wise_OT_Rate = 0	
											BEGIN
												IF @OT_Max_Limit_Sec > 0 AND (@Emp_WD_OT_Rate = @Emp_WO_OT_Rate) AND (@Emp_WO_OT_Rate = @Emp_HO_OT_Rate)
													BEGIN
														SET @Emp_WO_HO_OT_Sec	= ISNULL(@OT_Sec,0) --+ ISNULL(@WO_OT_Sec,0) + ISNUll(@HO_OT_Sec,0)
							...
							IF(ISNULL(@OT_RATE_TYPE,0) = 0) And @Shift_Wise_OT_Rate = 0 
								BEGIN
							...
							ELSE
								...
								BEGIN
									...
									If @OT_Sec > 0 OR  @WO_OT_Sec > 0 OR @HO_OT_Sec > 0
										BEGIN 
											...
											CREATE TABLE #OT_SLAB_MASTER_WD
											(
												 ROW_ID INT,
												 FROM_HOURS NUMERIC(18,2),
												 TO_HOURS NUMERIC(18,2),
											...
											DECLARE @OT_SLAB_DWOHO_HOURS_WD AS NUMERIC(18,2) = 0
											
											IF EXISTS(SELECT 1 FROM DBO.T0050_GENERAL_OT_RATE_SLABWISE WITH (NOLOCK) WHERE Gen_ID=@GEN_ID AND @OT_SLAB_TYPE = 0)
												BEGIN
														INSERT INTO #OT_SLAB_MASTER_WD
														SELECT	ROW_NUMBER() OVER (ORDER BY WO_RATE ASC) AS ROW_ID,FROM_HOURS,TO_HOURS,WD_RATE,WO_RATE,HO_RATE,0 AS PERIOD_HOURS,0 AS OT_HOURS,0 AS OT_SLAB_AMOUNT,0 AS FLAG 
														FROM DBO.T0050_GENERAL_OT_RATE_SLABWISE WITH (NOLOCK) 
														WHERE GEN_ID=@GEN_ID 	
														...
														SET @OT_SLAB_DWOHO_HOURS_WD = @Emp_OT_Hours_Num + @Emp_WO_OT_Hours_Num +  @Emp_HO_OT_Hours_Num
														
														
														DECLARE @FROM_HOURS_WD AS NUMERIC(18,2),@TO_HOURS_WD AS NUMERIC(18,2),@WD_RATE_WD AS NUMERIC(18,2),@FLAG_WD AS INTEGER	
														DECLARE @SLAB_DIFF_WD AS NUMERIC(18,2) = 0 ,@OT_HOURS_WD AS NUMERIC(18,2) = 0,@OT_SLAB_AMOUNT_WD AS NUMERIC(18,2) = 0 
														DECLARE @CUR_COUNT_WD AS INTEGER = 0,@PREV_TO_HOURS_WD AS NUMERIC(18,2) = 0
														
														SET @TO_HOURS_WD = @OT_SLAB_DWOHO_HOURS_WD
														
														
														DECLARE OT_SLAB_CURSOR CURSOR fast_forward FOR 
															SELECT FROM_HOURS,TO_HOURS,WD_RATE,FLAG FROM #OT_SLAB_MASTER
														OPEN OT_SLAB_CURSOR    
														FETCH next FROM OT_SLAB_CURSOR INTO @FROM_HOURS_WD,@TO_HOURS_WD,@WD_RATE_WD,@FLAG_WD
														WHILE @@FETCH_STATUS = 0 
														BEGIN    
															...
															FETCH NEXT FROM OT_SLAB_CURSOR     
															INTO @FROM_HOURS_WD,@TO_HOURS_WD,@WD_RATE_WD,@FLAG_WD
															--@FROM_HOURS,@TO_HOURS,@WD_RATE,@FLAG
														END     
														CLOSE OT_SLAB_CURSOR;    
														DEALLOCATE OT_SLAB_CURSOR;  
												...
												BEGIN
													IF EXISTS(SELECT 1 FROM DBO.T0050_GENERAL_OT_RATE_SLABWISE WITH (NOLOCK) WHERE Gen_ID=@GEN_ID AND @OT_SLAB_TYPE = 1)
														BEGIN
															...
															SET @OT_SLAB_DWOHO_HOURS_WD = @Emp_OT_Hours_Num + @Emp_WO_OT_Hours_Num +  @Emp_HO_OT_Hours_Num
															SET @OT_HOURS = @OT_SLAB_DWOHO_HOURS_WD
								...
								END
								   ...
								   /* 
										Day Rate Need to Update Manual In T0030_CATEGORY_MASTER Table : Column Name -OT_Rate_11PM : Guru Krupa Client
								   */
								   Declare @Cat_ID_WD NUmeric
								   set @Cat_ID_WD =0
							       
								   SELECT @Cat_ID_WD = ISNULL(Cat_ID,0) FROM dbo.T0095_Increment WITH (NOLOCK) WHERE Increment_Id=@Increment_Id AND Emp_ID = @Emp_Id AND Cmp_ID = @Cmp_ID
							       
								   IF @Cat_ID_WD <> 0 AND EXISTS( SELECT 1 FROM T0030_CATEGORY_MASTER WITH (NOLOCK) WHERE Cmp_ID = @Cmp_ID AND Cat_ID = @Cat_ID AND OT_Rate_11PM > 0 )
										BEGIN
											DECLARE @OT_Sec_B_11_WD NUMERIC
											SET @OT_Sec_B_11_WD = 0
											
											DECLARE @OT_Sec_A_11_WD NUMERIC
											SET @OT_Sec_A_11_WD = 0
											
											DECLARE @OT_Rate_11PM_WD NUMERIC(10,3)
											SET @OT_Rate_11PM_WD = 0
											
											DECLARE @Fix_OT_Rate_Amount_WD NUMERIC(18,2)
											SET @Fix_OT_Rate_Amount_WD = 0	
																			
											DECLARE @Fix_OT_Salary_WD NUMERIC(18,2)
											SET @Fix_OT_Salary_WD = 0
											
											DECLARE @Fix_OT_Day_D_WD NUMERIC
											SET @Fix_OT_Day_D_WD = 0
											
											Declare @Fix_OT_Rate_Amount_A_WD Numeric(18,2)
											set @Fix_OT_Rate_Amount_A_WD = 0
											
											SELECT @OT_Rate_11PM_WD = OT_Rate_11PM 
											FROM T0030_CATEGORY_MASTER WITH (NOLOCK) 
											WHERE Cmp_ID = @Cmp_ID and OT_Rate_11PM > 0  and Cat_ID = @Cat_ID
											
											SET @Fix_OT_Salary_WD = ISNULL(@OT_Basic_Salary,0) + ISNULL(@temp_t_amt,0)
											...
											IF @OT_Sec_A_11 <> 0
												BEGIN
													DECLARE @OT_Sec_B_11_OT_WD as NUMERIC
													DECLARE @OT_Sec_B_11_OT_AMT_WD as NUMERIC(18,2)
													SET @OT_Sec_B_11_OT_WD = 0
													SET @OT_Sec_B_11_OT_AMT_WD = 0 
												...
												--NEW CODE ADDED BY RAMIZ ON 01/11/2017 FOR CALCULATING OT ON DIFFERENT RATES --			
													...
													IF @Wages_type ='Monthly'
														BEGIN
															SET @OT_Sec_B_11_OT_AMT_WD = ISNULL(ROUND( (@OT_Sec_B_11_OT * (@Fix_OT_Salary/@OT_Working_Day)) /@Shift_Day_Sec,0) * @Emp_WD_OT_Rate,0)
															--SET @OT_Sec_B_11_WO_AMT = ISNULL(ROUND( (@OT_Sec_B_11_WO * (@Fix_OT_Salary/@OT_Working_Day)) /@Shift_Day_Sec,0) * @Emp_WO_OT_Rate,0)
															--SET @OT_Sec_B_11_HO_AMT = ISNULL(ROUND( (@OT_Sec_B_11_HO * (@Fix_OT_Salary/@OT_Working_Day)) /@Shift_Day_Sec,0) * @Emp_HO_OT_Rate,0)
															
															SET @Fix_OT_Rate_Amount = ISNULL(@OT_Sec_B_11_OT_AMT_WD,0) + ISNULL(@OT_Sec_B_11_WO_AMT,0) + ISNULL(@OT_Sec_B_11_HO_AMT,0) --ADDED BY RAMIZ ON 01/11/2017
															--SET @Fix_OT_Rate_Amount = ISNULL(ROUND( (@OT_Sec_B_11 * (@Fix_OT_Salary/@OT_Working_Day)) /@Shift_Day_Sec,0) * @Emp_WD_OT_Rate,0)
															SET @Fix_OT_Rate_Amount_A = ISNULL(ROUND( (@OT_Sec_A_11 * (@Fix_OT_Salary/@OT_Working_Day)) /@Shift_Day_Sec,0) * @OT_Rate_11PM,0)
													...
													ELSE
														BEGIN
															SET @OT_Sec_B_11_OT_AMT_WD = ISNULL(ROUND( (@OT_Sec_B_11_OT * (@Fix_OT_Salary)) /@Shift_Day_Sec,0) * @Emp_WD_OT_Rate,0)
															--SET @OT_Sec_B_11_WO_AMT = ISNULL(ROUND( (@OT_Sec_B_11_WO * (@Fix_OT_Salary)) /@Shift_Day_Sec,0) * @Emp_WO_OT_Rate,0)
															--SET @OT_Sec_B_11_HO_AMT = ISNULL(ROUND( (@OT_Sec_B_11_HO * (@Fix_OT_Salary)) /@Shift_Day_Sec,0) * @Emp_HO_OT_Rate,0)
								 ...
								 BEGIN 
									...
									ELSE                    
										BEGIN
											...
											-- Added By tejas at 18012025 for bug #32821
													...
													IF @varCalc_On <> 'Arrears' AND @Wages_type = 'Daily'  --Changed the Spelling from "Arears" to "Arrears" by Ramiz on 16/11/2016
														BEGIN													
															...
															ELSE
																BEGIN															
																	--If condition added Alpesh 20-Jul-2011
																		--SET @M_AD_Amount = ROUND((@M_AD_Amount * @Salary_Cal_Day),0)   -- comment by tejas  
																		--SELECT @Calc_On_Allow_Dedu,@AD_ID
																	IF @IS_ROUNDING = 1              
																		SET @M_AD_Amount = ROUND((@Calc_On_Allow_Dedu * @M_AD_Percentage / 100),0)  
																	ELSE															
																		SET @M_AD_Amount =  (@Calc_On_Allow_Dedu * @M_AD_Percentage / 100)                    
