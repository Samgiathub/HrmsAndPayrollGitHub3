...
CREATE PROCEDURE [dbo].[P0220_24Q_REPORT]    
 @Cmp_Id Numeric,    
 @From_Date Datetime,    
 @To_Date Datetime,    
...
CREATE TABLE #Tax_Report_output    
 ...
 Select * from (    
  ...
  SELECT    Row_number() over (Order by TCD.Challan_Id,TCD.Emp_Id) Serial_No,E.Alpha_Emp_Code as Employee_reference_No_provided_by_employer,    
     ...
     '' as Period_of_Employment, 
	 --Old Query -Commented by Deepali - 30012024-Start 
	 -- (Isnull(MS.Gross_Salary,0) - Isnull(MS.Settelement_Amount,0)) + ISNULL(MSS.S_Gross_Salary,0) As Taxable_Amount_on_which_Tax_deducted, -- Change Condition for Gross by Hardik 23/07/2018 for Inductotherm as gross showing NetSettlement amount which is wrong    
	 --New Query -Added by Deepali - 30012024-Start 
	(Isnull(MS.Gross_Salary,0) - Isnull(MS.Settelement_Amount,0)) + ISNULL(MSS.S_Gross_Salary,0) +	isnull(ESIC.Amount,0) 
	As Taxable_Amount_on_which_Tax_deducted, -- Change Condition for Gross by Hardik 23/07/2018 for Inductotherm as gross showing NetSettlement amount which is wrong    
   
    --Old Query -Commented by Deepali - 30012024-End 
	 ...
	 -- added By Deepali - 30012024 as per Sandip bhai Discussion for TDS ---- Start 
	 Left Outer Join  T0210_ESIC_On_Not_Effect_on_Salary ESIC  WITH (NOLOCK) on TC.Month = Month(For_Date) and TC.Year = Year(For_Date) And TCD.Emp_Id = ESIC.Emp_ID  
	 -- added By Deepali - 30012024 ---- End
