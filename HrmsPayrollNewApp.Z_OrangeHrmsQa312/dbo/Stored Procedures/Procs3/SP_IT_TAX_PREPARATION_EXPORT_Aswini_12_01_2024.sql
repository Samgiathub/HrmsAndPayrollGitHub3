...
create PROCEDURE [dbo].[SP_IT_TAX_PREPARATION_EXPORT_Aswini_12/01/2024]
	 @Cmp_ID				numeric
	,@From_Date				Datetime
	,@To_Date				Datetime	 
...
--			select distinct(Emp_ID) , alpha_emp_code ,Emp_full_name from #Tax_Report_output
			SELECT DISTINCT(tro.Emp_ID) , tro.alpha_emp_code ,tro.Emp_full_name ,em.mobile_no,em.work_email,em.date_of_birth, em.date_of_join,em.emp_left_date,em.pan_no 
			from #Tax_Report_output tro inner join
			t0080_emp_master em on em.emp_id = tro.Emp_id 
		OPEN CUR_AD_Tax_emp 
		FETCH NEXT FROM CUR_AD_Tax_emp INTO @Emp_id_cur,@alpha_emp_code_cur,@Emp_Full_name_cur,@mo_no,@work_email,@dob,@doj,@left_date,@pan
			...
			BEGIN
				...
				if @flg = 0
					begin
						...
						update @Output_tax set val = 'Emp_Code,Name,Mobile,Work_Email,Date_Of_Birth,Join_Date,Left_Date,pan_no,' + val  where id = 4
						set @flg = 1
			    ...
			    update @Output_tax set val =  '="' + @alpha_emp_code_cur + '",' + @Emp_Full_name_cur + ',' + ISNULL(@mo_no,'') + ',"' + ISNULL(@work_email,'') + '",' + isnull(convert(nvarchar(11),@dob ,103),'-') + ',' + isnull(convert(nvarchar(11),@doj ,103),'-') + ',' + isnull(convert(nvarchar(11),@left_date,103),'-')  + ',' + isnull(@pan,'-')   + ',' + val   where id = @count_cur
