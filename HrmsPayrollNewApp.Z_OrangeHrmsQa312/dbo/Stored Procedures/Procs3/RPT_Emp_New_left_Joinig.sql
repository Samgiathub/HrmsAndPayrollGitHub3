...
CREATE PROCEDURE [dbo].[RPT_Emp_New_left_Joinig]
	 @Cmp_ID		numeric
	,@From_Date		datetime
	,@To_Date		datetime 
...
SET ARITHABORT ON
    ...
    Begin
		...
		Insert into @Emp_Details(Cmp_ID,branch_id,branch_name,Description,Last_month,id,Company_Name,Company_Address)
				Select EM.Cmp_ID,EM.branch_id,branch_name,'Left Employees' as emp_Desc,
				Count(Case WHEN Emp_Left_Date <= @To_Date And Emp_Left_Date >= @From_Date and emp_left='Y'  THEN EM.Emp_ID END) ,
		...
		Insert into @Emp_Details(Cmp_ID,branch_id,branch_name,Description,Last_month,id,Company_Name,Company_Address)
				...
				Count(Case WHEN Date_of_Join <= @To_Date And Date_of_Join >= @From_Date  THEN EM.Emp_ID END),
		...
		Insert into @Emp_Details(Cmp_ID,branch_id,branch_name,Description,Last_month,id,Company_Name,Company_Address) 
				...
				Count(Case WHEN qry1.Increment_Effective_Date <= @To_Date And qry1.Increment_Effective_Date >= @From_Date  THEN qry1.Emp_ID END),
		 ...
		 Insert into @Emp_Details(Cmp_ID,branch_id,branch_name,Description,Last_month,id,Company_Name,Company_Address)
				...
				Count(Case WHEN Date_of_Retirement <= @To_Date And Date_of_Retirement >= @From_Date  THEN EM.Emp_ID END) ,
		 ...
		 Insert into @Emp_Details(Cmp_ID,branch_id,branch_name,Description,Last_month,id,Company_Name,Company_Address) 
				...
				Count(Case WHEN dateadd(yyyy,1,qry1.Increment_Effective_Date) >= @To_Date And dateadd(yyyy,1,qry1.Increment_Effective_Date) <= @From_Date  THEN qry1.Emp_ID END) ,
	...
	Else
		Begin
			...
			(
				...
				Department Varchar(200) -- Added By Sajid 15-07-2022
				,Category Varchar(200) -- Added By Sajid 15-07-2022
				,DOB Varchar(50),
				...
				From_date datetime,    ---added jimit 17082015
				To_date  datetime,      ---added jimit 17082015
				Resign_Date	datetime--added by chetan 20122017
				...
				if exists(Select EM.Emp_ID from v0080_employee_master EM 
				...
				Left JOIN T0040_DEPARTMENT_MASTER DEM WITH (NOLOCK) on DEM.Dept_Id = EM.Dept_Id -- Added By Sajid 15-07-2022
				Left JOIN T0030_CATEGORY_MASTER CAT WITH (NOLOCK) on CAT.Cat_ID = EM.Cat_ID -- Added By Sajid 15-07-2022
				where Emp_Left_Date <= @To_date And Emp_Left_Date >= @From_Date and emp_left='Y')
					BEGIN
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date,Resign_Date)
						Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id,  EM.Cmp_ID,EM.branch_id,branch_name,'Left Employees' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name
						,DM.Desig_Name
						,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name  -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),EM.Date_Of_Join,103),CONVERT(varchar(50),EM.Emp_Left_Date,103),
						...
						Em.Basic_Salary + ISNULL((qry.E_AD_AMOUNT),0) as Gross_salary
						,@From_Date,@To_Date,EM.Reg_Date					
						...
						Left Outer JOIN( SELECT EED.INCREMENT_ID, SUM(EED.E_AD_AMOUNT) AS E_AD_AMOUNT 
									 FROM T0100_EMP_EARN_DEDUCTION EED WITH (NOLOCK) INNER JOIN 
										T0050_AD_MASTER AM WITH (NOLOCK) ON EED.AD_ID = AM.AD_ID INNER JOIN
										#Emp_Cons EC On Ec.Increment_ID = EED.INCREMENT_ID
										and AM.AD_FLAG = 'I' and AM.AD_NOT_EFFECT_SALARY = 0
										--AND AM.Add_in_sal_amt = 1 
										and AM.AD_PART_OF_CTC = 1
									 WHERE AM.CMP_ID = @Cmp_Id
									 GROUP by eed.INCREMENT_ID
									) qry ON qry.Increment_ID = EC.INCREMENT_ID					
						where Emp_Left_Date <= @To_Date And Emp_Left_Date >= @From_Date and emp_left='Y'
					END
				Else
					Begin
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name
						,Designation
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date,Resign_Date)
						Select 0,0,0,'','Left Employees' as emp_Desc,'','','','','','','',1,Cmp_Name,Cmp_Address,11,'',0,@From_Date,@To_Date,'1900-01-01' from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End
					
				--if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = EM.Cmp_ID Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id where Emp_Left_Date <= GetDate() And Emp_Left_Date >= dateadd(mm,-3,GetDate()) and emp_left='Y')
				--	BEGIN
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				...
				--		Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id 
				--		Left JOIN T0100_LEFT_EMP LE on LE.Emp_ID = EM.Emp_ID 
				--		INNER JOIN ( SELECT EED.INCREMENT_ID, SUM(EED.E_AD_AMOUNT) AS E_AD_AMOUNT 
				--					 FROM T0100_EMP_EARN_DEDUCTION EED INNER JOIN 
				--						T0050_AD_MASTER AM ON EED.AD_ID = AM.AD_ID AND AM.Add_in_sal_amt = 1 
				--					 WHERE AM.CMP_ID = @Cmp_Id
				--					 GROUP by eed.INCREMENT_ID
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Left Employees' as emp_Desc,'','','','','',1,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Left Employees' as emp_Desc,'','','','','',1,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Left Employees' as emp_Desc,'','','','','',1,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				Left JOIN T0040_DEPARTMENT_MASTER DEM WITH (NOLOCK) on DEM.Dept_Id = EM.Dept_Id -- Added By Sajid 15-07-2022
				Left JOIN T0030_CATEGORY_MASTER CAT WITH (NOLOCK) on CAT.Cat_ID = EM.Cat_ID -- Added By Sajid 15-07-2022
				where Date_of_Join <= @To_Date And Date_of_Join >= @from_date)
					BEGIN
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date,Resign_Date)
						Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id , EM.Cmp_ID,EM.branch_id,branch_name,'New Joinings' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name,DM.Desig_Name
						,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name   -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),EM.Date_Of_Birth,103),CONVERT(varchar(50),EM.Date_Of_Join,103),
						2,CM.Cmp_Name,CM.Cmp_Address,11,''
						,EM.Basic_Salary + ISNULL((qry.E_AD_AMOUNT),0) as Gross_salary
						,@from_date,@To_Date,Reg_date
						from v0080_employee_master EM 
						inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID 
						inner JOIN T0010_COMPANY_MASTER CM WITH (NOLOCK) on CM.Cmp_Id = EM.Cmp_ID 
						Left JOIN T0040_DESIGNATION_MASTER DM WITH (NOLOCK) on DM.Desig_ID = EM.Desig_Id 
				...
				Else
					Begin
						...
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date,Resign_Date)
						Select 0,0,0,'','New Joinings' as emp_Desc,'','','','','','','',2,Cmp_Name,Cmp_Address,11,'',0,@From_Date,@To_Date,'1900-01-01' from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End
				
				--if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = EM.Cmp_ID Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id where Date_of_Join <= GetDate() And Date_of_Join >= dateadd(mm,-3,GetDate()))
				--	BEGIN
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','New Joinings' as emp_Desc,'','','','','',2,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				
				--if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = EM.Cmp_ID Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id where Date_of_Join <= GetDate() And Date_of_Join >= dateadd(mm,-6,GetDate()))
				--	BEGIN
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id ,EM.Cmp_ID,EM.branch_id,branch_name,'New Joinings' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name,DM.Desig_Name,CONVERT(varchar(50),EM.Date_Of_Birth,103),CONVERT(varchar(50),EM.Date_Of_Join,103),
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','New Joinings' as emp_Desc,'','','','','',2,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
					
				--if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = EM.Cmp_ID Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id where Date_of_Join <= GetDate() And Date_of_Join >= dateadd(mm,-12,GetDate()))
				--	BEGIN
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id ,EM.Cmp_ID,EM.branch_id,branch_name,'New Joinings' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name,DM.Desig_Name,CONVERT(varchar(50),EM.Date_Of_Birth,103),CONVERT(varchar(50),EM.Date_Of_Join,103),
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','New Joinings' as emp_Desc,'','','','','',2,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
					
				if exists(
						Select M.Emp_ID	from v0080_employee_master M   Left JOIN 
						(
							SELECT MAX(I.Increment_Effective_Date) as Increment_Effective_Date ,I.Emp_ID as Emp_ID FROM v0080_employee_master EM 
							INNER JOIN  T0095_INCREMENT I WITH (NOLOCK) on EM.Cmp_ID = I.Cmp_ID 
							and EM.Emp_ID = I.Emp_ID
							where  I.Increment_Type = 'Increment' --and I.Branch_ID = @branch_ID --Comment due to transfer case not consider in increment count
							GROUP BY I.Emp_ID
						...
						Left JOIN T0040_DEPARTMENT_MASTER DEM WITH (NOLOCK) on DEM.Dept_Id = M.Dept_Id -- Added By Sajid 15-07-2022
						Left JOIN T0030_CATEGORY_MASTER CAT WITH (NOLOCK) on CAT.Cat_ID = M.Cat_ID -- Added By Sajid 15-07-2022

						Where  qry1.Increment_Effective_Date <= @To_Date And qry1.Increment_Effective_Date >= @From_Date)
					BEGIN
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date,Resign_Date)
						Select ROW_NUMBER() OVER(order BY M.Emp_id) as id , M.Cmp_ID,M.branch_id,branch_name,'Increment' as emp_Desc,M.Alpha_Emp_Code,M.Emp_Full_Name
						,DM.Desig_Name
												,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name   -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),M.Date_Of_Birth,103),CONVERT(varchar(50),qry1.Increment_Effective_Date,103),
						3,CM.Cmp_Name,CM.Cmp_Address,11,''		
						,M.Basic_Salary as Gross_salary
						,@from_date,@To_Date,M.Reg_date
						from v0080_employee_master M   Left JOIN 
						(
							SELECT MAX(I.Increment_Effective_Date) as Increment_Effective_Date ,I.Emp_ID as Emp_ID FROM v0080_employee_master EM 
							INNER JOIN  T0095_INCREMENT I WITH (NOLOCK) on EM.Cmp_ID = I.Cmp_ID 
				...
				Else
					Begin
						...
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select 0,0,0,'','Increment' as emp_Desc,'','','','','','','',3,Cmp_Name,Cmp_Address,11,'',0,@From_Date,
						@To_Date from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End
				
				--if exists(
				--		Select M.Emp_ID	from v0080_employee_master M   Left JOIN 
				--		(
				--			SELECT MAX(I.Increment_Effective_Date) as Increment_Effective_Date ,I.Emp_ID as Emp_ID FROM v0080_employee_master EM INNER JOIN  T0095_INCREMENT I  on EM.Cmp_ID = I.Cmp_ID 
				--			and EM.Emp_ID = I.Emp_ID
				--			where  I.Increment_Type = 'Increment' --and I.Branch_ID = @branch_ID --Comment due to transfer case not consider in increment count
				--			GROUP BY I.Emp_ID
				--		)  as qry1 ON qry1.Emp_ID = M.Emp_ID 
				--		inner JOIN #Emp_Cons EC on M.Emp_ID = EC.Emp_ID 
				--		inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = M.Cmp_ID	 
				--		Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = M.Desig_Id 
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Increment' as emp_Desc,'','','','','',3,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Increment' as emp_Desc,'','','','','',3,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Increment' as emp_Desc,'','','','','',3,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
					
				if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID 
				inner JOIN T0010_COMPANY_MASTER CM WITH (NOLOCK) on CM.Cmp_Id = EM.Cmp_ID 
				Left JOIN T0040_DESIGNATION_MASTER DM WITH (NOLOCK) on DM.Desig_ID = EM.Desig_Id 
				Left JOIN T0040_DEPARTMENT_MASTER DEM WITH (NOLOCK) on DEM.Dept_Id = EM.Dept_Id -- Added By Sajid 15-07-2022
							Left JOIN T0030_CATEGORY_MASTER CAT WITH (NOLOCK) on CAT.Cat_ID = EM.Cat_ID -- Added By Sajid 15-07-2022
				where Date_of_Retirement <= @To_Date And Date_of_Retirement >= @From_Date)
					BEGIN
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation
                        ,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id ,EM.Cmp_ID,EM.branch_id,branch_name,'Retired Employees' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name
						,DM.Desig_Name
												,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name   -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),EM.Date_Of_Birth,103),CONVERT(varchar(50),EM.Date_of_Retirement,103),
				...
				Else
					Begin
						...
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select 0,0,0,'','Retired Employees' as emp_Desc,'','','','','','','',4,Cmp_Name,Cmp_Address,11,'',0,@From_Date,@To_date from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End
				
				--if exists(Select EM.Emp_ID from v0080_employee_master EM inner JOIN #Emp_Cons EC on EM.Emp_ID = EC.Emp_ID inner JOIN T0010_COMPANY_MASTER CM on CM.Cmp_Id = EM.Cmp_ID Left JOIN T0040_DESIGNATION_MASTER DM on DM.Desig_ID = EM.Desig_Id where Date_of_Retirement <= GetDate() And Date_of_Retirement >= dateadd(mm,-3,GetDate()))
				--	BEGIN
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				...
				--					 GROUP by eed.INCREMENT_ID
				--					) qry ON qry.Increment_ID = EM.INCREMENT_ID
				--		where Date_of_Retirement <= GetDate() And Date_of_Retirement >= dateadd(mm,-3,GetDate())
				--	END 
				--Else
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Retired Employees' as emp_Desc,'','','','','',4,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--					 GROUP by eed.INCREMENT_ID
				--					) qry ON qry.Increment_ID = EM.INCREMENT_ID
				--		where Date_of_Retirement <= GetDate() And Date_of_Retirement >= dateadd(mm,-6,GetDate())
				--	END 
				--Else
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Retired Employees' as emp_Desc,'','','','','',4,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--					 GROUP by eed.INCREMENT_ID
				--					) qry ON qry.Increment_ID = EM.INCREMENT_ID
				--		where Date_of_Retirement <= GetDate() And Date_of_Retirement >= dateadd(mm,-12,GetDate())
				--	END 
				--Else
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Retired Employees' as emp_Desc,'','','','','',4,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				if exists(
						...
						inner JOIN T0010_COMPANY_MASTER CM WITH (NOLOCK) on CM.Cmp_Id = M.Cmp_ID	 
						Left JOIN T0040_DESIGNATION_MASTER DM WITH (NOLOCK) on DM.Desig_ID = M.Desig_Id 
						Where  dateadd(yyyy,1,qry1.Increment_Effective_Date) >= @To_Date And dateadd(yyyy,1,qry1.Increment_Effective_Date) <= @From_Date)
					BEGIN
						Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation
						,Department  -- Added By Sajid 15-07-2022
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select ROW_NUMBER() OVER(order BY M.Emp_id) as id ,M.Cmp_ID,M.branch_id,branch_name,'Due for Increment' as emp_Desc,M.Alpha_Emp_Code,M.Emp_Full_Name
						,DM.Desig_Name
												,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name   -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),M.Date_Of_Birth,103),CONVERT(varchar(50),qry1.Increment_Effective_Date,103),
						5,CM.Cmp_Name,CM.Cmp_Address,11,''			
						,M.Basic_Salary + ISNULL((qry.E_AD_AMOUNT),0) as Gross_salary
						,@From_date,@To_Date
						from v0080_employee_master M   Left JOIN 
						(
							SELECT MAX(I.Increment_Effective_Date) as Increment_Effective_Date ,I.Emp_ID as Emp_ID FROM v0080_employee_master EM 
				...
				Else
					Begin
						...
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select 0,0,0,'','Due for Increment' as emp_Desc,'','','','','','','',5,Cmp_Name,Cmp_Address,11,'',0,@From_date,@To_date from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End		
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Increment' as emp_Desc,'','','','','',5,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End	
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Increment' as emp_Desc,'','','','','',5,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Increment' as emp_Desc,'','','','','',5,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				where Date_of_Retirement >= @To_Date And Date_of_Retirement <= @From_Date)
					BEGIN
						...
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select ROW_NUMBER() OVER(order BY EM.Emp_id) as id ,EM.Cmp_ID,EM.branch_id,branch_name,'Due for Retirement' as emp_Desc,EM.Alpha_Emp_Code,EM.Emp_Full_Name
						,DM.Desig_Name
												,DEM.Dept_Name -- Added By Sajid 15-07-2022
						,CAT.Cat_Name   -- Added By Sajid 15-07-2022
						,CONVERT(varchar(50),EM.Date_Of_Birth,103),CONVERT(varchar(50),EM.Date_of_Retirement,103),
				...
				Else
					Begin
						...
						,Category -- Added By Sajid 15-07-2022
						,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary,From_date,To_date)
						Select 0,0,0,'','Due for Retirement' as emp_Desc,'','','','','','','',6,Cmp_Name,Cmp_Address,11,'',0,@From_date,@To_Date from T0010_COMPANY_MASTER WITH (NOLOCK) where Cmp_Id = @Cmp_ID
					End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Retirement' as emp_Desc,'','','','','',6,Cmp_Name,Cmp_Address,12,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Retirement' as emp_Desc,'','','','','',6,Cmp_Name,Cmp_Address,13,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End	
				...
				--	Begin
				--		Insert into @Emp_Details_1(Sr_No,Cmp_ID,branch_id,branch_name,Description,Emp_Code,Emp_Name,Designation,DOB,Left_Date,id,Company_Name,Company_Address,id_Month,Left_Type,Gross_salary)
				--		Select 0,0,0,'','Due for Retirement' as emp_Desc,'','','','','',6,Cmp_Name,Cmp_Address,14,'',0 from T0010_COMPANY_MASTER where Cmp_Id = @Cmp_ID
				--	End	
				--if @Report_id = '11' 
				--	Begin
				...
				if @Report_For = 0           --added jimit 01082015
					begin
						...
						select *,
						case when ISNULL(Left_Date,'')<>''
							 then (case when DATEDIFF(DAY,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103)) > 0 
									THEN DATEDIFF(d,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103))
									else 0 
									END)	
							else Null 
						END as Remaing_Days	 
						from @Emp_Details_1 where id IN(1,2)   order by Remaing_Days,Sr_No,id
					end	
				else if @Report_For = 1 
					begin
						
						select *,case when ISNULL(Left_Date,'')<>'' then (case when DATEDIFF(DAY,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103) ) > 0 THEN DATEDIFF(d,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103))  else 0 END)	else 0 END as Remaing_Days from @Emp_Details_1 where id IN(1,2) and Description = 'Left Employees' order by Remaing_Days,Sr_No,id
					end
				else if @Report_For = 2
					begin
						select *,case when ISNULL(Left_Date,'')<>'' then (case when DATEDIFF(DAY,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103)) > 0 THEN DATEDIFF(d,convert(datetime,GETDATE(),103),convert(datetime,Left_Date,103))  else 0 END)	else 0 END as Remaing_Days from @Emp_Details_1 where id IN(1,2) and Description = 'New joinings' order  by Remaing_Days,Sr_No,id
					end
