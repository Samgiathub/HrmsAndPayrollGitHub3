...
-- Create By : HARDIK BAROT  
-- Create Date : 17/02/2020  
-- Description : For Late and Early count deduction combine, with Multiple time Exempt, like First 2 Exempt then 3rd Deduction, then again 2 exempt then again 6th deduction  
CREATE PROCEDURE [dbo].[SP_CALCULATE_LATE_EARLY_DEDUCTION_COMBINE_MULTIPLE_EXEMPT]  
     @emp_Id                NUMERIC  
    ,@Cmp_ID                NUMERIC  
...
AS  
    ...
    DECLARE @Curr_Month_LateMark   numeric (18,1)  
 DECLARE @Curr_Month_EarlyMark   numeric (18,1)  
    DECLARE @Curr_Month_LateMark_WithOut_Exemption numeric (18,1)  
 ...
 DECLARE @Shift_Exemption_St_MAX_DATETIME DATETIME  
    ...
    SET @Curr_Month_LateMark   = 0  
 Set @Curr_Month_EarlyMark = 0  
    SET @Curr_Month_LateMark_WithOut_Exemption = 0  
 ...
 DECLARE @Gen_ID numeric   = 0
 Declare @Max_Limit_Sec numeric = 0
 ...
 Set @Late_Deduction = 0  
   ...
   @Gen_ID = Gen_ID
    ...
    SELECT @Late_Exempted_limit_sec = dbo.F_Return_Sec(@Late_Exempted_limit)    
	select @Max_Limit_Sec = dbo.F_Return_Sec(@Max_Late_Limit)  
 ...
 IF @Chk_Last_Late_Early_Month = 0   
  Begin  
   ...
   --select @From_Laps_Date,@Month_St_Date  
 ...
 ------------------- Late Validation for Enpay ----------------------  
   ...
   SET @FLAG_PENLATY = 0  
     ...
     @Is_Early_Mark = Is_Late_Mark  
			...
			BEGIN
				...
				SELECT  IN_TIME, OUT_TIME,FOR_DATE,Emp_ID,D.Shift_Id 
				FROM    #DATA D   
				...
				WHERE   NOT EXISTS(SELECT 1 FROM T0150_EMP_INOUT_RECORD EIO WITH (NOLOCK)    
									WHERE   EIO.EMP_ID=D.EMP_ID AND ISNULL(IS_CANCEL_LATE_IN,0) <> 0 AND IN_TIME=D.IN_TIME And Isnull(Chk_By_Superior,0) = 1)  
            ...
            FETCH NEXT FROM curLMark INTO @In_Date,@Out_Date,@For_DateCurr,@Emp_ID,@Shift_ID  
   WHILE @@FETCH_STATUS = 0  
       ...
       @Shift_End_Time=SM.Shift_End_Time  
                    ...
                    IF @Return_Record_SET = 1  AND   
                        ...
                        BEGIN  
                            ...
                            SELECT @Emp_ID,@Cmp_ID,@Month,@Year,Late_Balance_BF,Late_Curr_Days,Late_total_Days,Late_Tobe_Adj_days,LeavE_Id,0,Late_adj_Agn_Leave,Late_adj_Agn_Leave,Late_total_adj_Days,0,Late_closing From T0100_EMP_LATE_DETAIL where Emp_ID =
							@Emp_ID and Month(for_DatE)=@month and Year(for_Date) =@Year
                        END  
                    ELSE  
                        BEGIN  
                            ...
                            WHERE   Emp_ID =@emp_Id AND For_Date = CONVERT(NVARCHAR,@In_Date,106)  
                                    AND ISNULL(Late_Calc_Not_App,0)=0 AND Chk_By_Superior = 1 -- Change Condition from Chk_By_Superior <> 0  by Hardik 01/10/2020 for Honda as Rejected Regularise is not considered Late Mark  
						   ...
						   WHERE Emp_ID =@emp_id AND For_Date = CONVERT(nvarchar,@Out_Date,106)  
							 AND Chk_By_Superior = 1  -- Change Condition from Chk_By_Superior <> 0  by Hardik 01/10/2020 for Honda as Rejected Regularise is not considered Late Mark  
       ...
       IF @FLAG_LATE_MARK = 1  
        SET @Curr_Month_LateMark = @Curr_Month_LateMark + 1  
       ELSE IF @FLAG_LATE_MARK = 2  
        SET @Curr_Month_LateMark_WithOut_Exemption = @Curr_Month_LateMark_WithOut_Exemption + 1  
   
       SET @FLAG_PENLATY = 0  
       --IF @Flag_Laps_Exemption_Count = 0  
       -- Begin  
       --  IF (@Curr_Month_LMark > @Late_Exempted_Days or @Curr_Month_LateMark_WithOut_Exemption > @Extra_Count_Exemption) and (@FLAG_LATE_MARK = 2)   
       --   Begin  
       --    SET @FLAG_PENLATY = 1  
       --   End  
       --  --Else if @Curr_Month_LMark > @Late_Exempted_Days and @Curr_Month_LateMark_WithOut_Exemption > @Extra_Count_Exemption and (@FLAG_LATE_MARK = 1)  
       --  Else if @Curr_Month_LMark > @Late_Exempted_Days AND (@Curr_Month_LMark + @Curr_Month_LateMark_WithOut_Exemption) > (@Late_Exempted_Days + @Extra_Count_Exemption) and (@FLAG_LATE_MARK = 1)  
       --   Begin  
       --    SET @FLAG_PENLATY = 1  
       --   End  
       ...
       IF @Report_Flag = 1 and (@FLAG_LATE_MARK >= 1 )  
        Begin  
         ...
         Values(@Cmp_ID,@Emp_ID,@For_DateCurr,@In_Date,@Out_Date,cast(@var_Shift_St_Date as DATETIME),cast(@var_Shift_End_Date as DATETIME),(@Differnce_Rounding_Late_Sec + @Late_Limit_Sec),0,@Emp_Late_Limit,@Emp_Early_Limit,@Late_Deduction,0)  
        End   
       ...
       IF @FLAG_EARLY_MARK = 1  
        SET @Curr_Month_EarlyMark = @Curr_Month_EarlyMark + 1  
       ...
       --IF @Flag_Laps_Exemption_Count = 0  
       -- Begin  
       --  IF (@Curr_Month_LateMark > @Late_Exempted_Days or @Curr_Month_LateMark_WithOut_Exemption > @Extra_Count_Exemption) and (@FLAG_EARLY_MARK = 2)   
       --   Begin  
       --    SET @FLAG_PENLATY = 1  
       --   End  
       --  Else if (@Curr_Month_LMark + @Curr_Month_LateMark_WithOut_Exemption) > (@Late_Exempted_Days + @Extra_Count_Exemption) and (@FLAG_EARLY_MARK = 1)  
       --   Begin  
       --    SET @FLAG_PENLATY = 1  
       --   End  
       ...
       --  SET @FLAG_PENLATY = 1   
       -- End  
       IF @FLAG_EARLY_MARK = 2  
        SET @FLAG_PENLATY = 1    
          
		  
      -- deepal  
      -- select @Curr_Month_LateMark,@Curr_Month_EarlyMark,@For_DateCurr,@Late_Deduction,@Late_Dedu_Days,@Late_Adj_Day  
        
      --if @Curr_Month_LateMark = @Late_Adj_Day Or @Curr_Month_EarlyMark = @Late_Adj_Day   
      --begin   
      -- Set @Late_Deduction = @Late_Deduction + @Late_Dedu_Days  
      -- select @Curr_Month_LateMark,@Curr_Month_EarlyMark,@For_DateCurr,@Late_Deduction,@Late_Dedu_Days,@Late_Adj_Day  
      --END  
      --begin  
      -- Set @Late_Deduction = @Late_Deduction + @Late_Dedu_Days  
      -- If @Curr_Month_LateMark > @Late_Adj_Day  
      --  Set @Curr_Month_LateMark = @Curr_Month_LateMark - @Late_Adj_Day  
      -- Else  
      --  set @Curr_Month_LateMark= 0  
  
      --end  
      --Else if @Late_Adj_Day <= (Isnull(@Curr_Month_LateMark,0) + Isnull(@Curr_Month_EarlyMark,0))  
      --Begin  
      -- Set @Early_Deduction = @Early_Deduction + @Late_Dedu_Days  
      -- set @Curr_Month_LateMark = 0  
      -- Set @Curr_Month_EarlyMark = 0  
      --End  
      --Else If @Late_Adj_Day <= @Curr_Month_EarlyMark  
      --begin  
      -- Set @Early_Deduction = @Early_Deduction + @Late_Dedu_Days  
      -- set @Curr_Month_EarlyMark= 0  
      --end  
				 ...
				 if Exists(Select 1 From #Emp_Late_Early Where Emp_ID = @Emp_ID and For_Date = @For_DateCurr)  
				  Begin  
				   ...
				   Update #Emp_Late_Early  
					...
					-- Early_Limit = @Max_Early_Limit,  --Deepal 30102024
					 Early_Deduction = @Early_Deduction  
				   Where Emp_ID = @Emp_ID and For_Date = @For_DateCurr  
				  End  
				 Else  
				  Begin  
				   INSERT INTO #Emp_Late_Early(Cmp_ID,Emp_ID,For_Date,In_Time,Out_Time,Shift_St_Time,Shift_End_Time,Late_Sec,Early_Sec,Late_Limit,Early_Limit,Late_Deduction,Early_Deduction)  
				   Values(@Cmp_ID,@Emp_ID,@For_DateCurr,@In_Date,@Out_Date,cast(@var_Shift_St_Date as DATETIME),cast(@var_Shift_End_Date as DATETIME),0,(@Differnce_Rounding_Early_Sec + @Early_Limit_Sec),@Emp_Late_Limit,@Emp_Early_Limit,0,@Early_Deduction)  
				  End  
				End   
   ...
   --Select   
   --  Select 'Late' as Column_Name,For_date, Late_Sec,Emp_Id  From #Emp_Late_Early where Late_Sec > 0  AND Late_Deduction =0  
   --  Union All   
   --  Select 'Early' as Column_Name,For_date, Early_Sec,Emp_Id  From #Emp_Late_Early where Early_Sec > 0  And late_Sec=0 AND Early_Deduction=0  
        END  
  ...
  Begin  
    ...
    Select ROW_NUMBER() Over(Order by For_date Asc, Column_Name Desc) as Row_No
	,ROW_NUMBER() Over(Order by For_date Asc, Column_Name Desc) % @Late_Adj_Day as Remainder
	, *, '0.00' As Ded_Days   
    Into #tmp From 
	(  
     Select 'Late' as Column_Name,For_date, Late_Sec,Emp_Id  From #Emp_Late_Early where Late_Sec > 0  AND Late_Deduction =0 and DATEDIFF(S,IN_TIME,OUT_TIME) <=  dateDiff(s,Shift_St_Time,Shift_End_Time) 
     Union All   
	 Select 'Early' as Column_Name,For_date, Early_Sec,Emp_Id  From #Emp_Late_Early where Early_Sec > 0  And Early_Deduction=0  and  DATEDIFF(S,IN_TIME,OUT_TIME) <=  dateDiff(s,Shift_St_Time,Shift_End_Time)
    ) Qry  

	
	 --if ((Select distinct GEN_ID from #tmp  as A  Inner Join T0050_GENERAL_LATEMARK_SLAB  AS B
	 --On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
	 --Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Late'  and GEN_ID = 10) = 10)
	 --begin 
		-- Update #tmp set Ded_Days = DEDUCTION  from #tmp  as A  Inner Join T0050_GENERAL_LATEMARK_SLAB  AS B
		--On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
		-- Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Late'  and GEN_ID = 10
	 --END
	 --ELSE
	 --Begin 
	 --select * from #tmp
		--Update #tmp set Ded_Days = DEDUCTION  from #tmp  as A  Inner Join T0050_GENERAL_LATEMARK_SLAB  AS B
		-- On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
		-- Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Late'  and Gen_id = 1184

		--	select B.* from #tmp as A
		--	Inner Join T0050_GENERAL_LATEMARK_SLAB  AS B On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
		--	Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Late'  and Gen_id = 1184
	 --END
	
	 if exists(select 1 from T0050_GENERAL_EARLYMARK_SLAB where Gen_ID in (@Gen_ID))
	 Begin 
		Update #tmp set Ded_Days = DEDUCTION  from #tmp  as A  Inner Join T0050_GENERAL_LATEMARK_SLAB  AS B
		 On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
		 Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Late'  and B.GEN_ID = @Gen_ID
     
		 Update #tmp set Ded_Days = DEDUCTION  from #tmp  as A  Inner Join T0050_GENERAL_EARLYMARK_SLAB  AS B
		 On A.Late_sec/60 between B.FROM_MIN And B.TO_MIN
		 Where B.CMP_ID = @Cmp_ID And Remainder = 0  And  Column_Name='Early' and B.GEN_ID = @Gen_ID 
	 END
	 else
	 Begin 
		
		Update #tmp set Ded_Days = @Late_Dedu_Days  Where Remainder = 0  And  Column_Name='Late' 
	 
		Update #tmp set Ded_Days = @Early_Dedu_Days Where Remainder = 0  And  Column_Name='Early'
		 
		update #Emp_Late_Early set Late_Deduction = @Late_Dedu_Days where Late_sec > @Max_Limit_Sec
		update #Emp_Late_Early set Early_Deduction = @Early_Dedu_Days where Early_sec > @Max_Limit_Sec
		
	 END

	 
	 ------------------------ End ------------------
	Update EMP Set Late_Deduction = Ded_Days From #Emp_Late_Early EMP 
	Inner Join #Tmp T ON EMP.For_Date = T.For_Date And EMP.Emp_Id = T.Emp_Id   
	Where T.Column_Name='Late'   
      
	Update EMP Set Early_Deduction = Ded_Days From #Emp_Late_Early EMP
	Inner Join #Tmp T ON EMP.For_Date = T.For_Date And EMP.Emp_Id = T.Emp_Id   
    Where T.Column_Name='Early'  
  ...
  Else  
   Begin  
		...
		Select ROW_NUMBER() Over(Order by For_date Asc, Column_Name Desc) as Row_No,ROW_NUMBER() Over(Order by For_date Asc, Column_Name Desc) % 3 as Remainder, *, '0.00' As Ded_Days   
		Into #tmp1 From  (  
		 Select 'Late' as Column_Name,For_date, Late_Sec,Emp_Id  From T0160_Late_Early_Validation WITH (NOLOCK)  where Late_Sec > 0  AND Late_Deduction =0 And Emp_Id = @Emp_Id  
		 Union All   
		 Select 'Early' as Column_Name,For_date, Early_Sec,Emp_Id  From T0160_Late_Early_Validation WITH (NOLOCK)  where Early_Sec > 0  And late_Sec=0 AND Early_Deduction=0 And Emp_Id = @Emp_Id  
		) Qry  

		Update #tmp1 Set Ded_Days = @Late_Dedu_Days Where Remainder = 0  
  
		Update EMP Set Late_Deduction = Ded_Days   
		From T0160_Late_Early_Validation EMP WITH (NOLOCK)    
		Inner Join #Tmp1 T ON EMP.For_Date = T.For_Date And EMP.Emp_Id = T.Emp_Id   
		Where T.Column_Name='Late'  
  
		Update EMP Set Early_Deduction = Ded_Days From T0160_Late_Early_Validation EMP WITH (NOLOCK)  
		Inner Join #Tmp1 T ON EMP.For_Date = T.For_Date And EMP.Emp_Id = T.Emp_Id   
		Where T.Column_Name='Early'  
   ...
   Begin  
    ...
    Begin   
	  ...
	  -- Commneted by Divyaraj Kiri on 13/03/2024
   --   set @qry = 'Select Top ' + cast (cast(@Late_Exempted_Days as int) as varchar(5))  + ' * into TmpLateEmpDays
	  --from #Emp_Late_Early Where (Late_sec < ' + cast(@Late_Exempted_limit_sec as varchar(50)) + ' or Early_sec < ' + cast(@Late_Exempted_limit_sec as varchar(50)) + ') order by for_date'

	  -- Started by Divyaraj Kiri on 13/03/2024		

	  ----SET @qry = 'SELECT TOP ' + CAST(CAST(@Late_Exempted_Days AS INT) AS VARCHAR(5)) + ' *	FROM #Emp_Late_Early WHERE Late_sec > ' 
	  ----+ CAST(@Late_Exempted_limit_sec AS VARCHAR(50)) + ' ORDER BY for_date
	  ----INSERT INTO #TmpLateEmpDays	SELECT TOP ' + CAST(CAST(@Early_Exempted_Days AS INT) AS VARCHAR(5)) + 
	  ----' *	FROM #Emp_Late_Early	WHERE Early_sec > ' + CAST(@Early_Exempted_limit_sec AS VARCHAR(50)) + ' ORDER BY for_date'
	  --Select * From #Tmp
	  --Select * From #Emp_Late_Early
	  --Select @Late_Exempted_limit_sec,@Early_Exempted_limit_sec
	  --Select @Late_Limit_Sec,@Early_Limit_Sec,@Emp_Late_Limit,@Emp_Early_Limit,@Late_Exempted_limit_sec,@Early_Exempted_limit_sec
	  
	  --Select DATEDIFF(SECOND,IN_TIME,OUT_TIME) as inOutTime,  dateDiff(SECOND,Shift_St_Time,Shift_End_Time) as sinOutTime
	  --,dateDiff(SECOND,Shift_St_Time,Shift_End_Time) - DATEDIFF(SECOND,IN_TIME,OUT_TIME) as diffInOutTime,
	  --Late_sec,Early_sec
	  --from #Emp_Late_Early

	  --Select @Late_Limit_Sec,@Early_Limit_Sec,@Late_Exempted_limit_sec,@Early_Exempted_limit_sec,@Emp_Late_Limit,@Emp_Early_Limit
	  --Select * from #data
	  --Select * from #Emp_Late_Early
	  --return
		SET @qry = '
			SELECT TOP ' + CAST(CAST(@Late_Exempted_Days AS INT) AS VARCHAR(5)) + ' LE.*
			INTO TmpLateEmpDays
			FROM #Emp_Late_Early LE			
			RIGHT OUTER JOIN #data D with (NOLOCK) on D.Emp_Id = LE.Emp_Id
			WHERE 
			D.P_days = 1.000
			AND LE.For_Date >= (SELECT MIN(For_Date) FROM #data WHERE P_days = 1.000)
			' +
			--Late_sec < ' + CAST(@Late_Exempted_limit_sec AS VARCHAR(50)) + '			
			--+ ' Late_sec < 14400
			' ORDER BY LE.for_date

			INSERT INTO TmpLateEmpDays
			SELECT TOP ' + CAST(CAST(@Early_Exempted_Days AS INT) AS VARCHAR(5)) + ' LE.*
			FROM #Emp_Late_Early LE
			RIGHT OUTER JOIN #data D with (NOLOCK) on D.Emp_Id = LE.Emp_Id
			WHERE 
			D.P_days = 1.000
			AND LE.For_Date >= (SELECT MIN(For_Date) FROM #data WHERE P_days = 1.000)
			' +
			--Early_sec < ' + CAST(@Early_Exempted_limit_sec AS VARCHAR(50)) + '			
			--+' Early_sec < 14400
			' ORDER BY LE.for_date'
	  
	 --SET @qry =  '
		--			SELECT TOP ' + CAST(CAST(@Late_Exempted_Days AS INT) AS VARCHAR(5)) + ' *
		--			INTO TmpLateEmpDays
		--			FROM #Emp_Late_Early
		--			WHERE (Late_sec > 0 OR Early_sec > 0)  -- Skip records where both Late_sec and Early_sec are zero
		--			AND (Late_sec < 14400 AND Early_sec < 14400)
		--			AND CAST(DATEDIFF(SECOND, IN_TIME, OUT_TIME) AS VARCHAR(50)) <=  CAST(DATEDIFF(SECOND, Shift_St_Time, Shift_End_Time) AS VARCHAR(50)) 
		--			ORDER BY for_date

		--			INSERT INTO TmpLateEmpDays
		--			SELECT TOP ' + CAST(CAST(@Early_Exempted_Days AS INT) AS VARCHAR(5)) + ' *
		--			FROM #Emp_Late_Early
		--			WHERE (Late_sec > 0 OR Early_sec > 0)  -- Skip records where both Late_sec and Early_sec are zero
		--			AND (Late_sec < 14400 AND Early_sec < 14400)
		--			AND CAST(DATEDIFF(SECOND, IN_TIME, OUT_TIME) AS VARCHAR(50)) <=  CAST(DATEDIFF(SECOND, Shift_St_Time, Shift_End_Time) AS VARCHAR(50)) 
	  ...
	  -- Ended by Divyaraj Kiri on 13/03/2024
      exec(@qry)
	  --Select @qry
	  --exec(@qry1)

	  --Select @Late_Exempted_limit_sec
	  --Select * from TmpLateEmpDays
		  UPDATE A  
		  SET A.Late_Deduction = 0 
		  ,A.Early_Deduction = case when @Late_Exempted_limit_sec > B.Early_Sec then B.Early_Deduction 
									when @Late_Exempted_limit_sec > B.Late_Sec then B.Late_Deduction  else 0 end
		 -- ,a.ExemptFlag = 'Ex'
		  FROM #Emp_Late_Early A JOIN TmpLateEmpDays B  ON A.For_date = B.for_date  

		  --Update #Emp_Late_Early set ExemptFlag = 'Ex'
		  --where  Late_Deduction =0 and Early_Deduction = 0
