...
CREATE PROCEDURE [dbo].[P9999_DEVICE_INOUT_DETAIL_INSERT_Alpha_Enroll_No]
	@ENROLL_NO		NUMERIC,
	...
	if not exists(select enroll_no from dbo.T9999_DEVICE_INOUT_DETAIL WITH (NOLOCK) Where Enroll_No=@Enroll_No and IO_DateTime =@IO_DateTime)
		Begin	
			...
			If  OBJECT_ID (N'tempdb..##Enroll_No') Is null
				Begin
					--if  not exists( select Cmp_ID from dbo.T0080_emp_Master where Enroll_no = @Enroll_No) 
					if  not exists( select Cmp_ID from dbo.T0080_emp_Master WITH (NOLOCK) where DBRD_Code = @Enroll_No) 
						begin 
							return
						end
					Else
						Begin
							----Select @Cmp_ID = Cmp_ID from dbo.T0080_emp_Master where Enroll_no =@Enroll_No 
							Select @Cmp_ID = Cmp_ID from dbo.T0080_emp_Master WITH (NOLOCK) where DBRD_Code =@Enroll_No 
						End
				End
			Else
				Begin
