...
--CreateD By Ramiz on 12/12/2017
---10/3/2021 (EDIT BY MEHUL ) (Table-valued function WITH NOLOCK)---
CREATE FUNCTION [dbo].[fn_getEmpIncrementWithoutTransfer] 
(	
	@Cmp_ID NUMERIC,
	@Emp_ID NUMERIC , 
	@For_Date DateTime
...
AS
	BEGIN
		...
		IF (ISNULL(@Emp_ID,0) = 0)
			BEGIN
				INSERT INTO @EmpInc	
				SELECT	I.Increment_ID,I.Emp_ID,I.Branch_ID , I.INCREMENT_EFFECTIVE_DATE
						...
						INNER JOIN (SELECT	MAX(INCREMENT_ID) AS INCREMENT_ID, I1.Emp_ID
									FROM	T0095_INCREMENT I1 WITH (NOLOCK)
											INNER JOIN (SELECT	MAX(INCREMENT_EFFECTIVE_DATE) AS INCREMENT_EFFECTIVE_DATE, I2.EMP_ID
														...
														WHERE	I2.INCREMENT_EFFECTIVE_DATE <= @For_Date AND I2.CMP_ID=@Cmp_ID AND I2.Increment_Type <> 'Transfer' AND I2.Increment_Type <> 'Deputation'
														GROUP BY I2.EMP_ID
														) I2 ON I1.EMP_ID=I2.EMP_ID AND I1.INCREMENT_EFFECTIVE_DATE=I2.INCREMENT_EFFECTIVE_DATE
									WHERE	I1.Increment_Effective_Date <= @For_Date AND I1.CMP_ID=@Cmp_ID AND I1.Increment_Type <> 'Transfer' AND I1.Increment_Type <> 'Deputation'
				...
				FROM	T0095_INCREMENT I WITH (NOLOCK)
						INNER JOIN (SELECT	MAX(INCREMENT_ID) AS INCREMENT_ID, I1.Emp_ID
									FROM	T0095_INCREMENT I1 WITH (NOLOCK)
											INNER JOIN (SELECT	MAX(INCREMENT_EFFECTIVE_DATE) AS INCREMENT_EFFECTIVE_DATE, I2.EMP_ID
														FROM	T0095_INCREMENT I2 WITH (NOLOCK)
														WHERE	I2.INCREMENT_EFFECTIVE_DATE <= @For_Date AND I2.Emp_ID=@Emp_ID AND I2.CMP_ID=@Cmp_ID AND I2.Increment_Type <> 'Transfer' AND I2.Increment_Type <> 'Deputation'
														GROUP BY I2.EMP_ID
														) I2 ON I1.EMP_ID=I2.EMP_ID AND I1.INCREMENT_EFFECTIVE_DATE=I2.INCREMENT_EFFECTIVE_DATE 
									WHERE	I1.Increment_Effective_Date <= @For_Date AND I1.Emp_ID=@Emp_ID AND I1.CMP_ID=@Cmp_ID AND I1.Increment_Type <> 'Transfer' AND I1.Increment_Type <> 'Deputation'
									GROUP BY I1.Emp_ID) I1 ON I.INCREMENT_ID=I1.INCREMENT_ID
