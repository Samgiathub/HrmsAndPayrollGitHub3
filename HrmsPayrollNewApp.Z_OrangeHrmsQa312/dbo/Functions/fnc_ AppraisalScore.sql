CREATE function [dbo].[fnc_ AppraisalScore](@GSG_FrequecyId int,@la_LevelAssignId int,@la_AllotmentId int,@Emp_ID int)                          
returns varchar(max)                          
as                          
begin                       
  ...
  declare @total varchar(max)=''        
			...
			select @total = @total + Achievement from kpms_FrqWise_Target_Achievement where emp_id = @Emp_ID and freq_id = @GSG_FrequecyId;  
