...
------------Created By sumit for count Age with month and days--01/01/2015---------------------------------------
CREATE FUNCTION [DBO].[Get_Age_CountDMY](@DAYOFBIRTH DATETIME, @TODAY DATETIME,@GETDMY VARCHAR(2) ='')
   RETURNS varchar(100)
AS
...
if @GetDMY = '' 
	...
	begin
		...
		set @Value=cast(@years as varchar(2)) + '.' + cast(@months as varchar(2)) --+ ' months,' + cast(@days as varchar(3)) + ' days'
	end	
