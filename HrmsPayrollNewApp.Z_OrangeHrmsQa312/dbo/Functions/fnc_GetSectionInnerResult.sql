...
CREATE function [dbo].[fnc_GetSectionInnerResult](@rXML varchar(max),@rSectionName varchar(300),@rCmpId int)  
returns varchar(max)  
as  
begin  
 ...
 while @li <= @lCnt  
 begin  
  declare @lGoalName varchar(300) = '',@lSubGoalName varchar(300) = '',@lWeightageType varchar(50) = '',@lWeightageValue varchar(50) = '',  
  @lGoalStatus varchar(10) = '',@lIsDependent varchar(10) = '',@lDependentModule varchar(300) = '',@lDependentType varchar(50) = '',  
  @lDependentValue varchar(50) = '',@lFrequency varchar(50) = ''  
  
  select @lGoalName = t_GoalName,@lSubGoalName = t_SubGoalName,@lWeightageType = t_WeightageType,@lWeightageValue = t_WeightageValue,@lFrequency = t_Frequency,  
  @lGoalStatus = t_GoalStatus,@lIsDependent = t_IsDependent,@lDependentModule = t_DependentModule,@lDependentType = t_DependentType,@lDependentValue = t_DependentValue from @tbltmp where tid = @li  
  ...
  <td><select class="custom-select drpWeightageType"><option value="0"> -- Select -- </option>  
  <option value="1" ' + CASE WHEN @lWeightageType = 'Number' then 'selected="selected"' ELSE '' END + '>Number</option>  
  <option value="2" ' + CASE WHEN @lWeightageType = 'Percentage' then 'selected="selected"' ELSE '' END + '>Percentage</option></select></td>  
  ...
  <td><select class="custom-select drpStatusGoal"><option value="0"> -- Select -- </option>  
  <option value="1" ' + CASE WHEN @lGoalStatus = 'Active' then 'selected="selected"' ELSE '' END + '>Active</option>  
  <option value="2" ' + CASE WHEN @lGoalStatus = 'Inactive' then 'selected="selected"' ELSE '' END + '>Inactive</option></select></td>  
  ...
  <td><input class="form-control form-control-lg txtValue" type="text" placeholder="51%" value="' + @lDependentValue + '" /></td>  
