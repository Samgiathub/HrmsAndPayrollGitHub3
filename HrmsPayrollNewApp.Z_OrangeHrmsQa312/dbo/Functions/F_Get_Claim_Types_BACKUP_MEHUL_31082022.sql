...
CREATE FUNCTION [dbo].[F_Get_Claim_Types_BACKUP_MEHUL_31082022]
(
	@Emp_ID		numeric,
	@Cmp_ID		numeric,
	@For_Date	DATETIME
...
BEGIN 
	...
	IF EXISTS(SELECT 1 FROM T0095_EMP_SCHEME ES WITH (NOLOCK)
		...
		BEGIN
				...
				SELECT DISTINCT CM.Claim_ID,CM.Claim_Name ,isnull(CM.Sorting_no,'') as Sorting_no FROM T0040_CLAIM_MASTER CM WITH (NOLOCK)
	...
	ELSE
		BEGIN
			INSERT INTO @RtnValue 
			SELECT DISTINCT CM.Claim_ID,CM.Claim_Name,CM.Sorting_no FROM T0040_CLAIM_MASTER CM WITH (NOLOCK)
