...
-- =============================================
CREATE FUNCTION [dbo].[F_Get_OT_QUARTERLYHOURS_New]
(
	@cmp_ID numeric,
	@Emp_ID		numeric,
	@For_Date	datetime,
...
BEGIN 
	...
	BEGIN
		...
		SELECT @monthStDate =  DATEFROMPARTS(YEAR(@For_Date),@month,DAY(@For_Date))
		...
		SELECT @TOT_QTR_HOURS=SUM(dbo.f_return_sec(format((CAST(Approved_OT_Hours as decimal(10,2))),'00.00')))+SUM(dbo.f_return_sec(format((CAST(Approved_WO_OT_Hours as decimal(10,2))),'00.00')))+SUM(dbo.f_return_sec(format((CAST(Approved_HO_OT_Hours as decimal(10,2))),'00.00')))
	...
	BEGIN
		...
		BEGIN
			SELECT @TOT_QTR_HOURS=SUM(dbo.f_return_sec(format((CAST(Approved_OT_Hours as decimal(10,2))),'00.00')))+SUM(dbo.f_return_sec(format((CAST(Approved_WO_OT_Hours as decimal(10,2))),'00.00')))+SUM(dbo.f_return_sec(format((CAST(Approved_HO_OT_Hours as decimal(10,2))),'00.00')))
				...
				WHERE Emp_ID=@Emp_ID AND Is_Approved=1   and Rpt_Level=@Rpt_level AND For_Date between @For_Date and @To_Date
		END
		ELSE
		BEGIN
	...
	BEGIN
		...
		ELSE IF MONTH(@For_Date) IN(10,11,12)
			...
			END	
	END
	--return dbo.f_return_HOURs(cast(@TOT_QTR_HOURS as numeric(18,2)))
	return  isnull(@TOT_QTR_HOURS,0.0)
END
