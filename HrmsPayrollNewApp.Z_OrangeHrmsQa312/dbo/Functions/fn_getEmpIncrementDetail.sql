...
-- =============================================  
CREATE FUNCTION [dbo].[fn_getEmpIncrementDetail]   
(   
...
AS   
 BEGIN  
  ...
  if @Cmp_ID = 0  
   ...
   FROM T0095_INCREMENT I WITH (NOLOCK)   
          --INNER JOIN (SELECT CAST(DATA AS NUMERIC) AS EMP_ID FROM dbo.Split(@Constraint, '#') T WHERE DATA <> '') T ON T.Emp_ID = I.Emp_ID  
     INNER JOIN (SELECT MAX(INCREMENT_ID) AS INCREMENT_ID, I1.Emp_ID  
        ...
        WHERE I1.Increment_Effective_Date <= @For_Date   
   ...
   WHERE I.Increment_Effective_Date <= @For_Date  
     --AND I.CMP_ID=@Cmp_ID  
     --AND I.Cmp_ID=@Cmp_ID   
  ELSE  
   INSERT INTO @Cons(Increment_ID, Emp_ID, Effective_Date)  
  ...
  */  
  ...
  INSERT INTO @EmpInc  
  SELECT distinct C.Increment_ID, C.Emp_ID, EED.AD_ID,EED.FOR_DATE, EED.E_AD_FLAG, EED.E_AD_PERCENTAGE, EED.E_AD_AMOUNT  
  FROM @Cons C  
  ...
  */  
  INSERT INTO @EmpInc  
  SELECT distinct C.Increment_ID, C.Emp_ID, EDR.AD_ID,EDR.FOR_DATE, EDR.E_AD_FLAG, EDR.E_AD_PERCENTAGE, EDR.E_AD_AMOUNT  
  FROM @Cons C  
