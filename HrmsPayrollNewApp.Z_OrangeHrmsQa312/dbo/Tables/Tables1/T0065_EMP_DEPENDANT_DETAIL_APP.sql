CREATE TABLE [dbo].[T0065_EMP_DEPENDANT_DETAIL_APP] (
    [Emp_Tran_ID]        BIGINT          NOT NULL,
    [Emp_Application_ID] INT             NOT NULL,
    [Row_ID]             INT             NOT NULL,
    [Cmp_ID]             INT             NOT NULL,
    [Name]               VARCHAR (100)   NOT NULL,
    [RelationShip]       VARCHAR (20)    NOT NULL,
    [BirthDate]          DATETIME        NULL,
    [D_Age]              NUMERIC (18, 1) NULL,
    [Address]            VARCHAR (1000)  NULL,
    [Share]              NUMERIC (18, 2) NULL,
    [Is_Resi]            NUMERIC (1)     NULL,
    [NomineeFor]         VARCHAR (30)    NULL,
    [Pan_Card_No]        VARCHAR (20)    NULL,
    [Adhar_Card_No]      VARCHAR (20)    NULL,
    [Approved_Emp_ID]    INT             NULL,
    [Approved_Date]      DATETIME        NULL,
    [Rpt_Level]          INT             NULL,
    CONSTRAINT [FK_T0065_EMP_DEPENDANT_DETAIL_APP_T0060_EMP_MASTER_APP] FOREIGN KEY ([Emp_Tran_ID]) REFERENCES [dbo].[T0060_EMP_MASTER_APP] ([Emp_Tran_ID])
);

