using System;
using System.Collections.Generic;

namespace HrmsPayrollNewApp.DataAccessLayer.Data;

public partial class V0080EmpMasterIncrementGetBackupDivyaraj07022024
{
    public decimal EmpCode { get; set; }

    public string Initial { get; set; } = null!;

    public string EmpFirstName { get; set; } = null!;

    public string EmpSecondName { get; set; } = null!;

    public string EmpLastName { get; set; } = null!;

    public DateTime DateOfJoin { get; set; }

    public string SsnNo { get; set; } = null!;

    public string SinNo { get; set; } = null!;

    public string DrLicNo { get; set; } = null!;

    public string PanNo { get; set; } = null!;

    public DateTime DateOfBirth { get; set; }

    public string MaritalStatus { get; set; } = null!;

    public string Gender { get; set; } = null!;

    public DateTime DrLicExDate { get; set; }

    public string Nationality { get; set; } = null!;

    public decimal LocId { get; set; }

    public string Street1 { get; set; } = null!;

    public string City { get; set; } = null!;

    public string State { get; set; } = null!;

    public string ZipCode { get; set; } = null!;

    public string HomeTelNo { get; set; } = null!;

    public string MobileNo { get; set; } = null!;

    public string WorkTelNo { get; set; } = null!;

    public string WorkEmail { get; set; } = null!;

    public string OtherEmail { get; set; } = null!;

    public string ImageName { get; set; } = null!;

    public string EmpFullName { get; set; } = null!;

    public string EmpLeft { get; set; } = null!;

    public DateTime EmpLeftDate { get; set; }

    public decimal IncrementId { get; set; }

    public string PresentStreet { get; set; } = null!;

    public string PresentCity { get; set; } = null!;

    public string PresentState { get; set; } = null!;

    public string PresentPostBox { get; set; } = null!;

    public decimal EmpSuperior { get; set; }

    public decimal EnrollNo { get; set; }

    public string BloodGroup { get; set; } = null!;

    public string TallyLedName { get; set; } = null!;

    public string Religion { get; set; } = null!;

    public string Height { get; set; } = null!;

    public string EmpMarkOfIdentification { get; set; } = null!;

    public string Despencery { get; set; } = null!;

    public string DoctorName { get; set; } = null!;

    public string Despenceryaddress { get; set; } = null!;

    public string InsuranceNo { get; set; } = null!;

    public byte IsGrApp { get; set; }

    public byte IsYearlyBonus { get; set; }

    public decimal YearlyLeaveDays { get; set; }

    public decimal YearlyLeaveAmount { get; set; }

    public decimal YearlyBonusPer { get; set; }

    public decimal YearlyBonusAmount { get; set; }

    public DateTime EmpConfirmDate { get; set; }

    public byte IsOnProbation { get; set; }

    public decimal TallyLedId { get; set; }

    public decimal EmpId { get; set; }

    public decimal CmpId { get; set; }

    public decimal ShiftId { get; set; }

    public decimal BranchId { get; set; }

    public decimal CatId { get; set; }

    public decimal GrdId { get; set; }

    public decimal DeptId { get; set; }

    public decimal DesigId { get; set; }

    public decimal TypeId { get; set; }

    public decimal BankId { get; set; }

    public decimal CurrId { get; set; }

    public string WagesType { get; set; } = null!;

    public string SalaryBasisOn { get; set; } = null!;

    public decimal BasicSalary { get; set; }

    public decimal GrossSalary { get; set; }

    public DateTime IncrementEffectiveDate { get; set; }

    public string PaymentMode { get; set; } = null!;

    public string IncBankAcNo { get; set; } = null!;

    public decimal EmpOt { get; set; }

    public string EmpOtMinLimit { get; set; } = null!;

    public string EmpOtMaxLimit { get; set; } = null!;

    public decimal EmpLateMark { get; set; }

    public decimal EmpFullPf { get; set; }

    public decimal EmpPt { get; set; }

    public decimal EmpFixSalary { get; set; }

    public byte EmpPartTime { get; set; }

    public string LateDeduType { get; set; } = null!;

    public string EmpLateLimit { get; set; } = null!;

    public decimal EmpPtAmount { get; set; }

    public byte EmpChildran { get; set; }

    public byte IsMasterRec { get; set; }

    public byte IsEmpFnf { get; set; }

    public decimal Probation { get; set; }

    public byte IsDeputationReminder { get; set; }

    public string DeputationEndDate { get; set; } = null!;

    public string IncrementType { get; set; } = null!;

    public decimal WorkerAdultNo { get; set; }

    public string FatherName { get; set; } = null!;

    public string BankBsr { get; set; } = null!;

    public string OldRefNo { get; set; } = null!;

    public string AlphaEmpCode { get; set; } = null!;

    public string? AlphaCode { get; set; }

    public byte? LeaveInProbation { get; set; }

    public byte IsLwf { get; set; }

    public decimal Ctc { get; set; }

    public decimal CenterId { get; set; }

    public string DbrdCode { get; set; } = null!;

    public string DealerCode { get; set; } = null!;

    public string CcenterRemark { get; set; } = null!;

    public decimal EmpEarlyMark { get; set; }

    public string EarlyDeduType { get; set; } = null!;

    public string EmpEarlyLimit { get; set; } = null!;

    public string IfscCode { get; set; } = null!;

    public decimal EmpWeekdayOtRate { get; set; }

    public decimal EmpHolidayOtRate { get; set; }

    public decimal EmpPfOpening { get; set; }

    public string EmpCategory { get; set; } = null!;

    public string EmpUidno { get; set; } = null!;

    public string EmpCast { get; set; } = null!;

    public string EmpAnnivarsaryDate { get; set; } = null!;

    public decimal LoginId { get; set; }

    public decimal ExtraAbDeduction { get; set; }

    public string CompoffMinHrs { get; set; } = null!;

    public string MotherName { get; set; } = null!;

    public byte IsMetroCity { get; set; }

    public byte IsPhysical { get; set; }

    public byte Expr2 { get; set; }

    public DateTime EmpOfferDate { get; set; }

    public decimal SaldateId { get; set; }

    public byte EmpAutoVpf { get; set; }

    public decimal SegmentId { get; set; }

    public decimal VerticalId { get; set; }

    public decimal SubverticalId { get; set; }

    public decimal SubbranchId { get; set; }

    public DateTime Groupjoiningdate { get; set; }

    public byte MonthlyDeficitAdjustOtHrs { get; set; }

    public decimal FixOtHourRateWd { get; set; }

    public decimal FixOtHourRateWoHo { get; set; }

    public string IfscCodeTwo { get; set; } = null!;

    public decimal BankIdTwo { get; set; }

    public string PaymentModeTwo { get; set; } = null!;

    public string IncBankAcNoTwo { get; set; } = null!;

    public string BankBranchName { get; set; } = null!;

    public string BankBranchNameTwo { get; set; } = null!;

    public string CodeDate { get; set; } = null!;

    public string CodeDateFormat { get; set; } = null!;

    public string EmpnameAliasPrimarybank { get; set; } = null!;

    public string EmpnameAliasPf { get; set; } = null!;

    public string EmpnameAliasPt { get; set; } = null!;

    public string EmpnameAliasSecondarybank { get; set; } = null!;

    public string EmpnameAliasTax { get; set; } = null!;

    public string EmpnameAliasEsic { get; set; } = null!;

    public string EmpnameAliasSalary { get; set; } = null!;

    public decimal EmpNoticePeriod { get; set; }

    public string EmpShoeSize { get; set; } = null!;

    public string EmpPentSize { get; set; } = null!;

    public string EmpShirtSize { get; set; } = null!;

    public string EmpDressCode { get; set; } = null!;

    public string EmpCanteenCode { get; set; } = null!;

    public decimal ThanaId { get; set; }

    public string Tehsil { get; set; } = null!;

    public string District { get; set; } = null!;

    public decimal ThanaIdWok { get; set; }

    public string TehsilWok { get; set; } = null!;

    public string DistrictWok { get; set; } = null!;

    public int SkilltypeId { get; set; }

    public string AboutMe { get; set; } = null!;

    public string UanNo { get; set; } = null!;

    public decimal CompoffWoAppDays { get; set; }

    public decimal CompoffWoAvailDays { get; set; }

    public decimal CompoffWdAppDays { get; set; }

    public decimal CompoffWdAvailDays { get; set; }

    public decimal CompoffHoAppDays { get; set; }

    public decimal CompoffHoAvailDays { get; set; }

    public DateTime DateOfRetirement { get; set; }

    public decimal SalaryDependsOnProduction { get; set; }

    public string RationCardType { get; set; } = null!;

    public string RationCardNo { get; set; } = null!;

    public string VehicleNo { get; set; } = null!;

    public decimal IsOnTraining { get; set; }

    public decimal TrainingMonth { get; set; }

    public string AadharCardNo { get; set; } = null!;

    public DateTime ActualDateOfBirth { get; set; }

    public byte IsPfTrust { get; set; }

    public string PfTrustNo { get; set; } = null!;

    public string? SystemDate { get; set; }

    public string ExtensionNo { get; set; } = null!;

    public string LinkedinId { get; set; } = null!;

    public string TwitterId { get; set; } = null!;

    public byte CustomerAudit { get; set; }

    public decimal ManagerProbation { get; set; }

    public DateTime PfStartDate { get; set; }

    public string DeptName { get; set; } = null!;

    public string DesigName { get; set; } = null!;

    public string BranchName { get; set; } = null!;

    public string GrdName { get; set; } = null!;

    public string SalesCode { get; set; } = null!;

    public string SignatureImageName { get; set; } = null!;

    public decimal LeaveEncashWorkingDays { get; set; }

    public decimal PhysicalPercent { get; set; }

    public byte IsProbationMonthDays { get; set; }

    public byte IsTraineeMonthDays { get; set; }

    public decimal WeekoffcompoffavailAfterDays { get; set; }

    public decimal HolidaycompoffavailAfterDays { get; set; }

    public decimal WeekdaycompoffavailAfterDays { get; set; }

    public byte IsVba { get; set; }

    public byte? IsPieceTransSalary { get; set; }

    public decimal? EmpWeekOffOtRate { get; set; }

    public decimal BandId { get; set; }

    public bool IsPradhanMantri { get; set; }

    public bool Is1timePfMember { get; set; }

    public string EmpCastJoin { get; set; } = null!;

    public string EmpFavSportId { get; set; } = null!;

    public string EmpFavSportName { get; set; } = null!;

    public string EmpHobbyId { get; set; } = null!;

    public string EmpHobbyName { get; set; } = null!;

    public string EmpFavFood { get; set; } = null!;

    public string EmpFavRestro { get; set; } = null!;

    public string EmpFavTrvDestination { get; set; } = null!;

    public string EmpFavFestival { get; set; } = null!;

    public string EmpFavSportPerson { get; set; } = null!;

    public string EmpFavSinger { get; set; } = null!;

    public string? IncDate { get; set; }
}
